...
// Polymarket events carry a rich, authoritative `tags` array (e.g. "Crypto",
// "Sports", "Tennis", "Politics", "Economy", "World", "Culture", "Gov Shutdown").
// We PREFER those real tags: inferCategory() maps them (and, for open positions
// that carry no tags, the real title/slug tokens) to a normalized label. A token
// must EXACTLY match a keyword to count — unknown text never guesses a category,
// it falls through to 'other'. Categories are checked most-specific first so a
// market tagged both "Tennis" and "Sports" resolves to Sports, and "TSA/Economy"
// resolves to Economy rather than the broad "World" co-tag.
const CAT_KEYWORDS = [
...
const CAT_LOOKUP = (() => {
  const m = new Map();
  for (const [label, kws] of CAT_KEYWORDS) for (const kw of kws) if (!m.has(kw)) m.set(kw, label);
  return m;
})();

function inferCategory(tags = []) {
  const lower = tags.map(t => (typeof t === 'string' ? t : t.label || '').toLowerCase());
  // Most-specific-first: return the first category whose keyword set any token hits.
...
// No fabrication: input is real text; unmatched → 'other'.

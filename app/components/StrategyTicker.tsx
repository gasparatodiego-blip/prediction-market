...
const STATUS_CLS: Record<string, string> = {
  ...
  'no-opp':      'bg-line/50 text-muted border-line',
  'offline':     'bg-gold/10 text-gold/70 border-gold/20',
  'coming-soon': 'bg-line/20 text-muted/40 border-line/30',
};
...
function TickerTile({ cat, flashKey }: { cat: TickerItem; flashKey: number }) {
  ...
  const inner = (
    <div className={`
      ...
      ${isLive
        ? 'bg-surface hover:bg-bg-soft hover:border-mint-deep/40'
        : cat.status === 'offline'
        ? 'bg-surface/80 hover:bg-bg-soft/80'
        : cat.status === 'no-opp'
        ? 'bg-surface hover:bg-bg-soft'
        : 'bg-surface/40 opacity-50 cursor-default pointer-events-none'
      }
    `}>
      {/* Label row */}
      <div className="flex items-center gap-1.5 mb-auto pb-2">
        ...
        <span className="font-body text-[8px] uppercase tracking-widest text-muted leading-snug">
          {cat.label}
        </span>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center" key={flashKey}>
        {isLive && cat.bestNetPct != null ? (
          <>
            <div className="font-mono font-bold tabular-nums text-mint-deep leading-none">
              <span
                className={`text-[22px] ${flashKey > 0 ? 'animate-number-flash' : ''}`}
              >
                +{cat.bestNetPct.toFixed(1)}
              </span>
              <span className="text-[10px] font-normal text-mint-deep/70 ml-0.5">
                {cat.unit}
              </span>
            </div>
            ...
            <div className="font-body text-[8px] text-muted mt-1 leading-snug">
              {cat.note}
            </div>
          </>
        ) : (
          <>
            <div className={`font-body text-[10px] font-semibold tracking-wider ${
              cat.status === 'offline'     ? 'text-gold/60' :
              cat.status === 'coming-soon' ? 'text-muted/40' :
              'text-muted'
            }`}>
              {STATUS_WORD[cat.status] ?? '—'}
            </div>
            <div className="font-body text-[8px] text-muted/60 mt-1 leading-snug">
              {cat.note}
            </div>
          </>
        )}
      ...
      <div className="flex items-center justify-between gap-2 mt-2.5">
        <span className="font-mono text-[8px] text-muted/50 tabular-nums">
          {cat.count > 0 ? `${cat.count} found` : ''}
        </span>
        <span className={`font-body text-[7px] uppercase tracking-widest px-1.5 py-[2px] border shrink-0 ${STATUS_CLS[cat.status]}`}>
...
export default function StrategyTicker() {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 sm:grid-cols-4 border border-line overflow-hidden rounded-card">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="min-h-[136px] border-r border-b border-line bg-surface animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mt-2 gap-4 flex-wrap">
        <div className="flex items-center gap-1.5">
          {error ? (
            <span className="font-body text-[9px] text-gold/60">fetch failed · showing last good data</span>
          ) : fetchedAt ? (
            <>
              <span className="inline-block w-1 h-1 rounded-full bg-mint-deep animate-pulse-slow shrink-0" />
              <span className="font-body text-[9px] text-mint-deep/50 tabular-nums">
        ...
        {(data.staleMinutes ?? 0) > 5 && (
          <span className="font-body text-[9px] text-gold/60">

...
import { MID_MIN_UN_LATO, MID_MAX_UN_LATO } from '@/lib/maker/motore-unico';
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    // ── IL RANGE IN CUI UN LATO SOLO MATURA ANCORA ────────────────────────────────────────────────
    // Non più un countdown: dal 6 agosto 2026 la Regola 4 decide con la FORMULA e non con un timer —
    // dentro [0.10, 0.90] un lato solo matura un terzo e si tiene, fuori matura zero e si chiude
    // subito. All'interfaccia servono quindi gli estremi, per poter dire PERCHÉ.
    latoSingolo: { midMin: MID_MIN_UN_LATO, midMax: MID_MAX_UN_LATO },
  }, { headers: { 'Cache-Control': 'no-store' } });
}

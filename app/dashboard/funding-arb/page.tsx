...
function FeeNote({ meta }: { meta: Meta | null }) {
  ...
  return (
    <p className="font-body text-[9px] text-muted mt-1.5 leading-relaxed">
      Fee/leg: Binance/Bybit/OKX {meta.feePerLeg.cex}% · Gate.io 0.05% · Bitget 0.06% · Hyperliquid {meta.feePerLeg.dex}% · dYdX 0.05% · Aster 0.04% · Paradex 0.02% · edgeX 0.038% · Grvt 0.045%.
...
function RateHeatmap({ futures }: { futures: Record<string, Record<string, FuturesCoin>> }) {
  ...
  const DEX_ORDER    = ['hyperliquid', 'dydx', 'aster', 'paradex', 'edgex', 'grvt'];
  const allExchanges = [

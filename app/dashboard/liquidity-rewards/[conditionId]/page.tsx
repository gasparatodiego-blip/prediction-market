...
const VOL_CLS: Record<VolRisk, string> = {
  LOW:    'text-mint-deep border-mint-deep/40 bg-mint-tint',
  MEDIUM: 'text-gold      border-gold/40        bg-gold/10',
  HIGH:   'text-coral-ink border-coral-ink/40    bg-coral-tint',
};
...
function BookRow({
  ...
  const priceCls = isAsk
    ? (inBand ? 'text-coral-ink'     : 'text-red-800/60')
    : (inBand ? 'text-mint-deep' : 'text-mint/40/60');
  const barCls   = isAsk ? 'bg-coral-tint/60' : 'bg-mint-tint/60';
  const bandCls  = inBand
    ? (isAsk ? 'border-l-2 border-coral-ink/35' : 'border-l-2 border-mint-deep/35')
    : '';
  ...
  return (
    ...
    >
      ...
      <span className="relative text-muted/60 tabular-nums text-right">
        {fmtS(cumTotal)}
      </span>
    </button>
  );
    ...
    <div className="min-h-screen bg-bg text-ink">
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
          ...
          {mkt ? (
            <>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <span className={`font-body text-[10px] px-1.5 py-px border uppercase ${VOL_CLS[mkt.volatilityRisk]}`}>
                  {mkt.volatilityRisk} risk
                </span>
                {mkt.negRisk && (
                  <span className="font-body text-[10px] px-1.5 py-px border border-line bg-bg-soft text-muted uppercase">
                    negRisk
                  </span>
                )}
            ...
            <div className="h-8 bg-bg-soft/40 animate-pulse rounded w-2/3" />
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="border border-line bg-surface/50 flex flex-col">
                  ...
                  <button
                    ...
                    className={`font-body text-[10px] px-2.5 py-1 border transition-colors ${
                      bookSide === 'no'
                        ...
                        : book?.no
                        ? 'border-line bg-surface/50 text-muted hover:text-ink-2'
                        : 'border-line/50 bg-surface/20 text-muted/40 cursor-not-allowed'
                    }`}
                  >
                    NO
                  </button>
                </div>
                <span className="font-body text-[10px] text-muted uppercase tracking-widest">Order Book</span>
                {bookAge && !bookError && (
            ...
            {/* Band legend */}
            {mkt && (
              <div className="flex items-center gap-3 px-3 py-1.5 border-b border-line/60 bg-surface">
                <span className="font-body text-[9px] text-muted uppercase tracking-wider">Reward band:</span>
                <span className="font-body text-[9px] text-mint-deep">{fmtP(bandLo)} – {fmtP(bandHi)}</span>
                <span className="font-body text-[9px] text-muted/50">levels inside earn rewards</span>
              </div>
            )}
            ...
            <div ref={askContainerRef} className="flex flex-col max-h-[160px] overflow-y-auto scrollbar-thin">
              {displayAsks.length === 0 && !bookError && (
                bookLoading
                  ? <SkeletonRows />
                  : <div className="px-3 py-2 font-body text-[10px] text-muted/50 text-center">no asks</div>
              )}
              {displayAsks.map((l, i) => (
                <BookRow
                  key={`ask-${i}`}
            ...
            <div className="flex items-center gap-3 px-3 py-1.5 bg-bg-soft/70 border-y border-line/60">
              ...
              {book?.yes?.last_trade_price && (
                <span className="font-body text-[10px] text-muted/60 ml-auto">
          ...
          <div className="border border-line bg-surface/50 flex flex-col">
            ...
            <div className="p-4 space-y-4 flex-1">
              ...
              <div className="space-y-1.5">
                <label className="font-body text-[10px] text-muted uppercase tracking-widest">Side</label>
                  ...
                  {(['BUY', 'SELL', 'BOTH'] as Side[]).map(s => (
                    <button
                      ...
                      className={`flex-1 font-body text-xs py-1.5 border transition-colors
                        ${side === s
                          ? s === 'BUY'  ? 'border-mint-deep/60 bg-mint-tint text-mint-deep'
                          : s === 'SELL' ? 'border-coral-ink/50 bg-coral-tint text-coral-ink'
              ...
              {est && mkt && (
                <div className={`border p-3 space-y-2.5 ${
                  !est.inBand || !est.aboveMin
                    ...
                    : 'border-mint/40 bg-mint-tint/30'
                }`}>

                  {/* Qualification warnings */}
                  {!est.inBand && (
                    <div className="font-body text-[11px] text-coral-ink border border-coral-ink/40 bg-coral-tint/30 px-2 py-1.5 rounded-sm">
                  ...
                  <div className={`border-t pt-2.5 border-mint/30' : 'border-line/30'}`}>
                    <div className="flex items-baseline justify-between">
                      <span className="font-body text-[10px] text-muted uppercase tracking-widest">Est. net reward</span>
                      <span className={`font-mono text-xl font-bold tabular-nums ${
                        !est.inBand || !est.aboveMin ? 'text-muted' : 'text-mint-deep'
                      }`}>
                        {est.inBand && est.aboveMin ? fmtUsd(est.rewardDay) : '$0.00'}
                      </span>
                    </div>
                    <p className="font-body text-[10px] text-muted/70 text-right">
                      per day · exact price · net of platform fees · inv. risk not sub.
                    </p>
                    {est.bothBoost && est.inBand && est.aboveMin && (
                      <p className="font-body text-[10px] text-mint/70 mt-0.5 text-right">
              ...
              <div className="border border-line/50 bg-surface/30 rounded-card p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-body text-[11px] text-muted/60 uppercase tracking-widest">
                    Auto-mirror on fill
                  </span>
                  <div
                ...
                <p className="font-body text-[9px] text-muted/50 leading-relaxed">
                  ...
                  <span
                    className="underline decoration-dotted cursor-help text-muted/70"
                  ...
                  {showTooltip && (
                    <span className="absolute z-10 ml-2 w-56 bg-bg-soft border border-line p-2 text-[9px] text-muted leading-relaxed shadow-lg">
                ...
                {(['500', '5000', '50000'] as const).map(key => {
                  ...
                  return (
                    <div key={key} className="px-3 py-2.5 space-y-0.5">
                      ...
                      {lv.flags.length > 0 && (
                        <div className="font-body text-[9px] text-gold">{lv.flags[0].split('—')[0].trim()}</div>
                      )}
                    </div>
                  );
                })}
...
function Metric({ label, value, dim }: { label: string; value: string; dim?: boolean }) {
  return (
    <div>
      <div className="font-body text-[9px] text-muted uppercase tracking-wider">{label}</div>
      <div className={`font-mono text-sm tabular-nums ${dim ? 'text-muted/70' : 'text-ink-2'}`}>{value}</div>
    </div>
  );
}

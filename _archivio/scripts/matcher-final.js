...
if (fs.existsSync(SENT_FILE)) {
  try { sentArbs = JSON.parse(fs.readFileSync(SENT_FILE, "utf8")); } catch(e) {}
}
...
function sendTelegramAlert(opportunity) {
  ...
  req.write(data);
  req.end();
  console.error("📨 Alert inviato a Telegram");
}
...
async function main() {
  ...
  const opportunities = await findArbitrage([...kalshi, ...polymarket]);
  ...
  const output = {
    ...
    message: opportunities.length === 0 ? "Nessun arb cashable" : `Trovati ${opportunities.length} arb!`
  };

...
async function fetchKalshi() {
  return new Promise((resolve) => {
    // Kalshi public markets senza auth (alcuni endpoint funzionano)
    const req = https.get("https://trading-api.kalshi.com/trade-api/v2/markets?limit=200&status=open", {
      timeout: 8000,
      ...
      res.on("end", () => {
        ...
        } catch(e) { console.error("Kalshi parse error:", e.message); resolve([]); }
      });
    });
    req.on("error", (e) => { console.error("Kalshi error:", e.message); resolve([]); });
    req.setTimeout(8000, () => { console.error("Kalshi timeout"); resolve([]); });
  });
}
  ...
  return new Promise((resolve) => {
    https.get("https://clob.polymarket.com/markets", { timeout: 8000 }, (res) => {
      ...
      res.on("end", () => {
        ...
        } catch(e) { console.error("Polymarket parse error"); resolve([]); }
      });
    }).on("error", (e) => { console.error("Polymarket error:", e.message); resolve([]); });
  });
}
...
async function findArbitrage(markets) {
  ...
  for (const m of markets) {
    ...
    if (!byQuestion[question].yes && !byQuestion[question].no && (bid > 0 || ask > 0)) {
      if (!byQuestion[question].generic) byQuestion[question].generic = [];
      byQuestion[question].generic.push({ bid, ask, outcome, source: m.source });
    }
  }
...
async function main() {
  ...
  const output = {
    ...
    message: opportunities.length === 0 ? "Nessun arb cashable ora — aspetta movimenti di mercato" : "Arb trovati!"
  };

...
// Landing palette, DAY (warm, the original) and NIGHT (dark, matching the ds
// token set). The landing renders its surfaces/illustrations through inline hex
// and SVG fill/stroke ATTRIBUTES — CSS var() cannot reach SVG presentation
// attributes, so the palette is swapped as plain hex by theme rather than via
// CSS custom properties. Every sub-component reads the live palette from
// PaletteCtx (see the `const P = useContext(PaletteCtx)` at the top of each),
// so one context value flips the whole page. Module-level constants
// (EDGES/WAYS/SCENES) keep referencing the DAY accents statically — the accent
// hues read on both surfaces.
const P_DAY = {
  ground: "#FFF9F0", surface: "#FFFFFF", ink: "#2D2418", muted: "#8A7A62",
  faint: "#B5A88F", mint: "#0FA968", amber: "#E0952E", violet: "#7B6FE8",
};
const P_NIGHT = {
  ground: "#0B0F14", surface: "#171D24", ink: "#EDF1F6", muted: "#AEB7C2",
  faint: "#8A94A0", mint: "#2FE29A", amber: "#F2B02E", violet: "#A78BFA",
};
const P = P_DAY;
const PaletteCtx = createContext(P_DAY);
...
export default function LandingClient({ initialTheme = "day" }: { initialTheme?: string }) {
  // Theme is set on <html data-theme> by the server (cookie-driven, see the page
  // wrapper), so `initialTheme` matches the SSR markup — no flash. After mount we
  // mirror any live toggle of the attribute so the JS palette flips in step with
  // the CSS-var surfaces elsewhere in the app.
  const [theme, setTheme] = useState(initialTheme === "night" ? "night" : "day");
  useEffect(() => {
    const el = document.documentElement;
    const sync = () => setTheme(el.getAttribute("data-theme") === "night" ? "night" : "day");
    sync();
    const obs = new MutationObserver(sync);
    obs.observe(el, { attributes: true, attributeFilter: ["data-theme"] });
    return () => obs.disconnect();
  }, []);
  const pal = theme === "night" ? P_NIGHT : P_DAY;
  const P = pal;

...
// ─────────────────────────────────────────────────────────────────────────────
// Paper book — trading-desk layout (light, matches site). Account equity on top, Open/Closed
// tabs, filter chips, and expandable trade rows.
//
// HONEST-ENGINE (unchanged — light-theme recolor is cosmetic only):
...
// P&L colors (site tokens) — calm loss (coral), not an alarm.
...
// ── tiny equity sparkline (stored marks only, stepAfter, no interpolation) ─
function Spark({ points, locked, height = 46 }: { points: { x: number; y: number | null }[]; locked: boolean; height?: number }) {
  ...
  if (locked || real.length < 2) {
    return (
      <div className="flex items-center justify-center rounded-md bg-bg-soft border border-line" style={{ height }}>
        {locked ? <span className="inline-flex items-center gap-1 text-[10px] text-muted"><Lock className="w-3 h-3" /> locked</span>
                : <span className="text-[10px] text-muted">not enough stored marks</span>}
      </div>
    );
  }
  ...
  const stroke = last > 0 ? '#0A9D6B' : last < 0 ? '#D5552F' : '#6C7E78';
  const zeroY = y0 < 0 && y1 > 0 ? sy(0) : null;
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ height }} className="rounded-md bg-bg-soft border border-line">
      {zeroY != null && <line x1={0} x2={W} y1={zeroY} y2={zeroY} stroke="#E3ECE7" strokeWidth={0.75} strokeDasharray="3 3" />}
...
function EquityBlock({ h, curve, isPaid, updatedAt }: { h: Headline; curve: PaperBook['equityCurve']; isPaid: boolean; updatedAt: string | null }) {
  ...
  return (
    <div className="rounded-card bg-surface border border-line shadow-card px-5 py-5 mb-3">
      <div className="flex items-start justify-between gap-3 mb-1">
        <span className="text-[11px] uppercase tracking-wider text-muted">Account equity · paper</span>
        <span className="text-[10px] text-muted">updated {fmtAge(updatedAt)}</span>
      </div>
      <div className="flex items-baseline gap-3 flex-wrap">
        <span className={`font-bold tracking-tight tabular-nums ${equity == null ? 'text-muted' : 'text-ink'}`} style={{ fontSize: 38 }}>
      ...
      <p className="text-[11px] text-muted mt-1.5 tabular-nums" title="equity = notional + realized + unrealized">
      ...
      <p className="text-[10.5px] text-muted mt-0.5">~{h.ticketCount} independent ${h.ticketSizeUsd?.toLocaleString()} tickets · <span className="text-muted">not one ${h.ticketSizeUsd?.toLocaleString()} book</span></p>
      ...
      <div className="grid grid-cols-3 gap-2 mt-4">
        <div className="rounded-lg bg-bg-soft border border-line px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-wide text-muted mb-1">Notional</p>
          <p className="text-[15px] font-semibold text-ink-2 tabular-nums">{fmtK(notional)}</p>
          <p className="text-[9px] text-muted">{h.ticketCount} tickets</p>
        </div>
        <div className="rounded-lg bg-bg-soft border border-line px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-wide text-muted mb-1">Realized · closed</p>
          ...
          <p className="text-[9px] text-muted">{h.closedCount} closed</p>
        </div>
        <div className="rounded-lg bg-bg-soft border border-line px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-wide text-muted mb-1">Unrealized · open</p>
          ...
          <p className="text-[9px] text-muted">{h.ticketCount} open exec</p>
        </div>
      </div>
      {h.thinOpen ? (
        <p className="text-[10px] text-muted mt-2">THIN <Redacted value={h.thinPnlUsd} isPaid={isPaid}>{v => fmtUsd(v as number)}</Redacted> ({h.thinOpen} not-exec-at-size) — shown separately, never inside equity.</p>
      ) : null}

      {/* equity sparkline */}
      <div className="mt-4">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] uppercase tracking-wide text-muted">Equity · stored marks</span>
          <span className="text-[9.5px] text-muted">stepped · no interpolation</span>
        </div>
...
function DetailCell({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-md bg-bg-soft border border-line px-2.5 py-2">
      <p className="text-[9px] uppercase tracking-wide text-muted mb-0.5">{label}</p>
      <p className="text-[12px] text-ink-2 tabular-nums leading-tight">{children}</p>
    </div>
  );
}
...
function TradeRow({ p, isPaid, open, onToggle }: { p: Position; isPaid: boolean; open: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="border-b border-line">
      <button onClick={onToggle} className="w-full grid grid-cols-[1fr_auto_auto] items-center gap-3 px-3 py-2.5 text-left hover:bg-bg-soft transition-colors">
        <span className="min-w-0">
          <span className="flex items-center gap-1.5">
            <span className="text-[12.5px] text-ink truncate">{p.label}</span>
            {p.thin && <span className="text-[8.5px] uppercase tracking-wide text-gold border border-gold/40 rounded px-1 shrink-0">thin</span>}
          </span>
          <span className="text-[10px] text-muted truncate block">{subLine(p)}</span>
        </span>
        <span className="text-right tabular-nums shrink-0">
          <span className="block text-[12px] text-ink-2">{lvl.text}</span>
          <span className="block text-[8.5px] uppercase tracking-wide text-muted">{closed ? 'exit·—' : 'entry'}</span>
        </span>
        <span className="text-right tabular-nums shrink-0 w-[86px] flex items-center justify-end gap-1">
          <span>
            ...
            <span className="block text-[8.5px] uppercase tracking-wide text-muted">{closed ? 'realized' : 'unreal.'}</span>
          </span>
      ...
      {open && (
        <div className="px-3 pb-3 pt-0.5 bg-bg-soft/60">
          {/* legs */}
          <div className="grid grid-cols-2 gap-1.5 mb-1.5">
            {legs.map((lg, i) => (
              <DetailCell key={i} label={String(lg.side ?? `leg ${i + 1}`)}>
                {lg.venue ?? dash}
                {typeof lg.price === 'number' && <span className="text-muted"> · {fmtUsdPlain(lg.price, lg.price < 10 ? 4 : 2)}</span>}
                {typeof lg.settledRate === 'number' && <span className="text-muted"> · {(lg.settledRate * 100).toFixed(4)}%/{lg.intervalH ?? 1}h</span>}
              </DetailCell>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5">
            ...
            {!closed ? (
              <>
                ...
                <DetailCell label="Realized P&L"><span className="text-muted">— (open)</span></DetailCell>
              </>
            ) : (
              <>
                <DetailCell label="Exit time">{fmtWhen(p.exit?.asOf)}</DetailCell>
                <DetailCell label="Exit price">{dash}<span className="text-muted"> · not stored</span></DetailCell>
          ...
          {!isPaid && <p className="text-[9px] text-muted mt-1.5">Locked $ figures are null server-side on the free tier — never blurred over a real value.</p>}
        </div>
      )}
    </div>
  );
    ...
    <main className="dsskin min-h-screen bg-bg text-ink font-body">
      <div className="dash-container px-4 pb-24 pt-5">
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[9px] uppercase tracking-wider text-violet border border-violet/30 rounded px-1.5 py-0.5">Simulated</span>
        </div>
        ...
        {data && h && (
          <>
            ...
            {/* tabs */}
            <div className="flex gap-1 p-1 rounded-lg bg-bg-soft border border-line mb-2">
              {([['open', `Open · ${openList.length}`], ['closed', `Closed · ${h.closedCount}`]] as const).map(([k, lbl]) => (
                <button key={k} onClick={() => { setTab(k); setExpanded(null); }}
                  className={`flex-1 text-[12px] font-medium py-1.5 rounded-md transition-colors ${tab === k ? 'bg-surface text-ink shadow-card' : 'text-muted hover:text-ink'}`}>
                  {lbl}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-1.5 mb-2 overflow-x-auto md:flex-wrap md:overflow-x-visible pb-0.5">
              ...
              <span className="ml-auto text-[10px] text-muted self-center whitespace-nowrap pl-2">{filtered.length} shown</span>
            </div>

            {/* column header */}
            <div className="grid grid-cols-[1fr_auto_auto] gap-3 px-3 py-1.5 text-[9px] uppercase tracking-wider text-muted border-b border-line">
              <span>Market</span>
            ...
            {/* rows */}
            <div className="rounded-b-lg overflow-hidden bg-surface border-x border-b border-line">
              {shown.length === 0 ? (
                <p className="text-[11.5px] text-muted text-center py-8">
              ...
              {filtered.length > CAP && (
                <p className="text-[10px] text-muted text-center py-2">+{filtered.length - CAP} more (showing first {CAP})</p>
              )}
            </div>

            {/* footer honesty */}
            <p className="text-[9.5px] text-muted mt-4 leading-snug text-center">
              ...
              {!isPaid && <> Derived $ is null on the free tier — <a href="/dashboard/upgrade" className="text-violet">upgrade →</a></>}
            </p>
          </>
        )}
      </div>

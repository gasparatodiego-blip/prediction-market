...
// La FINESTRA si fissa una volta sola e si passa a entrambi i piani: senza, il secondo piano leggerebbe
// un nastro di qualche secondo più lungo e la differenza fra le due liste conterrebbe anche quello.
const to = new Date().toISOString();

function piano(usePlacementScore) {
...
const prima = piano(false);      // com'era: ogni mercato al ceiling S=1
const dopo = piano(true);        // com'è: ogni mercato al suo punteggio reale
...
const pesoDi = (id) => { const c = pesoPer.get(id); return c && fin(c.punteggioPosizione) ? c.punteggioPosizione : null; };
const obiettivo = (righe) => {
  let tot = 0, pesate = 0, senzaPeso = 0;
  for (const r of righe) {
    const S = pesoDi(r.marketId);
    if (S == null || !fin(r.grossPerDay)) { senzaPeso += 1; continue; }
    tot += S * r.grossPerDay - (fin(r.netPerDay) && fin(r.grossPerDay) ? (r.grossPerDay - r.netPerDay) : 0);
    pesate += 1;
  }
...
const referto = {
  ...
  obiettivo: {
    ...
    nota: 'lordo pesato dal punteggio del venue alla distanza reale di un tick, meno il costo avverso misurato — è la quantità che il knapsack massimizza',
  },
  finestraFinoA: to,
  capitale: CAPITALE,
  ...
  entrati: entrati.map((r) => ({ marketId: r.marketId, nome: r.name, tick: tickDi(r.marketId), punteggio: r.punteggioPosizione, capitale: r.capital, realistico: r.realisticBestPerDay })),
  usciti: usciti.map((r) => ({ marketId: r.marketId, nome: r.name, tick: tickDi(r.marketId), capitale: r.capital, realistico: r.realisticBestPerDay })),
...
for (const e of referto.entrati) linea(`  + ${breve(e.marketId)} tick ${e.tick ?? '—'} · S ${fin(e.punteggio) ? e.punteggio.toFixed(3) : '—'} · ${usd(e.capitale)} · realistico ${usd(e.realistico)}/g · ${e.nome || '(nome ignoto)'}`);
if (!entrati.length) linea('  (nessuno)');
linea('');
linea(`USCITI (${usciti.length}):`);
for (const e of referto.usciti) linea(`  − ${breve(e.marketId)} tick ${e.tick ?? '—'} · ${usd(e.capitale)} · realistico ${usd(e.realistico)}/g · ${e.nome || '(nome ignoto)'}`);

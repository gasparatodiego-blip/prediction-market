...
export default function CopyConfigPanel({
  ...
  async function save() {
    ...
    try {
      ...
      if (!r.ok) { setErr(d.error || 'Impossibile salvare la configurazione.'); if (d.slots) setSlots(d.slots); return; }
      ...
      <div className="relative w-full max-w-[460px] max-h-[90vh] overflow-y-auto rounded-card border border-line bg-surface shadow-card">
        ...
        <div className="px-4 py-4 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between font-body text-[11px]">
            ...
            {slots
              ? <span className="text-ink-2 tabular-nums">{slots.used}/{slots.limit} usati{tier === 'pro' ? ' · Pro' : ''}</span>
              : <span className="text-muted">accedi per gestire gli slot</span>}
          </div>

          {/* Category chips — DATA-DERIVED */}
          <div>
            <div className="font-body text-[10px] uppercase tracking-wide text-muted mb-2">
              Categorie da copiare {selected.size === 0 && <span className="normal-case tracking-normal">· nessuna = tutte</span>}
            </div>
            {traderCats.length === 0 ? (
              <div className="font-body text-[11px] text-muted">
                {loading ? 'Carico le categorie del trader…' : 'Nessuna categoria reale disponibile per questo trader.'}
              </div>
            ) : (
          ...
          <div>
            ...
            <div className="font-body text-[11px] text-muted mt-1">
              loro <span className="text-ink-2 tabular-nums">$1.000</span> → tu <span className="text-mint-deep tabular-nums font-medium">${(1000 * pct / 100).toFixed(0)}</span>
            </div>
          </div>

          {/* Max open */}
          <div className="flex items-center justify-between">
            <span className="font-body text-[10px] uppercase tracking-wide text-muted">Max ordini aperti insieme</span>
          ...
          {/* Exit mode */}
          <div>
            <div className="font-body text-[10px] uppercase tracking-wide text-muted mb-2">Uscita</div>
            <div className="flex gap-1.5">
              {([['mirror', 'Esci con loro'], ['tpsl', 'Uscita mia']] as const).map(([m, label]) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="font-body text-[10px] uppercase tracking-wide text-muted">
                Specchio ({shown.length}{positions.length > shown.length ? ` di ${positions.length}` : ''})
              </span>
              <span className="font-body text-[10px] text-muted">
                capitale: loro <Redacted value={totalTheir}>{v => `$${v.toFixed(0)}`}</Redacted>
                {' → '}tu <Redacted value={totalYours}>{v => <span className="text-mint-deep font-medium">${v.toFixed(0)}</span>}</Redacted>
              </span>
            </div>
            {loading ? (
              <div className="font-body text-[11px] text-muted flex items-center gap-1.5"><Loader2 className="w-3 h-3 animate-spin" />Carico le posizioni…</div>
            ) : shown.length === 0 ? (
              <div className="font-body text-[11px] text-muted rounded-card border border-line bg-bg-soft/40 p-3">
                Nessuna posizione aperta reale in queste categorie. Copierà i prossimi ordini che apre.
              </div>
            ) : (
                    ...
                    <div key={i} className="flex items-center gap-2 px-2.5 py-2">
                      ...
                      <div className="text-right shrink-0 font-body text-[10px] tabular-nums">
                        <div className="text-muted">
                          loro <Redacted value={p.currentValue}>{v => `$${v.toFixed(0)}`}</Redacted>
                        </div>
                        <div className="text-mint-deep font-medium">
                          tu <Redacted value={yourUsd}>{v => `$${v.toFixed(0)}`}</Redacted>
          ...
          {/* Protezioni sempre attive (display only) */}
          <div className="rounded-card border border-line bg-bg-soft/40 p-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="font-body text-[10px] uppercase tracking-wide text-ink-2 font-medium">Protezioni sempre attive</span>
            </div>
            <div className="flex flex-wrap gap-x-3 gap-y-1 font-body text-[10px] text-muted">
              <span>✓ pre-flight prezzo eseguibile</span>
              <span>✓ no doppia entrata</span>
              <span>✓ ignora fill vecchi</span>
              <span>✓ check liquidità</span>
            </div>
          </div>
          ...
          {needAuth && (
            <div className="rounded-card border border-gold/30 bg-gold-tint px-3 py-2 font-body text-[11px] text-ink-2">
              Accedi per avviare la copia. <Link href="/login" className="text-[#0c9d6e] underline underline-offset-2">Accedi →</Link>
            </div>
          )}
          {err && (
          ...
          <button onClick={save} disabled={saving || saved}
            ...
            {saved
              ? <><Check className="w-3.5 h-3.5" strokeWidth={3} />{hasExisting ? 'Copia aggiornata · paper' : 'Copia avviata · paper'}</>
              : (hasExisting ? 'Aggiorna copia · paper' : 'Avvia copia · paper')}
          </button>
          {hasExisting && !saved && (
          ...
          <p className="font-body text-[10px] text-muted text-center leading-relaxed -mt-2">
            Simulazione paper con prezzi eseguibili reali. Nessun ordine reale viene eseguito.
          </p>
        </div>
      </div>
    </div>

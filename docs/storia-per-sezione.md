...
### §3 · AGENTI CHIAVE — testo integrale prima della potatura del 23 agosto 2026
...
### §4.1 · Il motore di piazzamento — testo integrale prima della potatura del 23 agosto 2026
...
### §4.1-bis · Il riprezzo guarda il BOOK — testo integrale prima della potatura del 23 agosto 2026
...
### §4.2 · I tetti di capitale — testo integrale prima della potatura del 23 agosto 2026
...
### §4.4 · Selezione e filtri del piano — testo integrale prima della potatura del 23 agosto 2026
...
### §4.6 · Il ciclo di vita di una posizione — testo integrale prima della potatura del 23 agosto 2026
...
> **📐 IL PAVIMENTO DELLA SCALA DEV'ESSERE UN PREZZO ESPRIMIBILE — 22 agosto 2026** (referto e le 147
> righe `OFF_TICK` che l'hanno rivelato: `docs/referti-difetti-corretti.md`).
> `pavimentoConcesso` restituisce **due numeri**: `pavimento` (esatto, serve a **confrontare**) e
> `pavimentoGriglia` (sulla griglia del mercato, serve a **prezzare**).
> **⚠ UN SOLO ARROTONDAMENTO IN TUTTO IL REPO**, e sta lì: `exit-plan` **legge** `pav.pavimentoGriglia`
> e non lo ricalcola — un test conta gli arrotondamenti e pretende che sia **uno**.
...
> «pareggio non basta»), che è una decisione di rischio. Le due cose non si contraddicono: `b.hi` sta
> già sulla griglia, quindi `b.hi ≥ pavimento ⇒ b.hi ≥ pavimentoGriglia` — l'arrotondamento **non può**
> spingere il prezzo fuori banda (asserito su 500+ piani).
> **⚠ VALE ANCHE SENZA CONCESSIONE**: il pavimento a gradino 0/1 **è il carico**, cioè un prezzo medio
> di fill, e cade fuori griglia più spesso di una frazione.
> **⚠ SU UN TOKEN ECONOMICO IL PAVIMENTO PUÒ FINIRE SOPRA IL CARICO** (0,095 ⇒ 0,09025 ⇒ **0,10**): è
> la conseguenza onesta di una griglia da 1¢ su 9,5¢ — `tickConcessi` diceva già **0**.

> **🚪 L'USCITA PUÒ GUARDARE FUORI BANDA QUANDO LA COPPIA È IMPOSSIBILE — 22 agosto 2026, decisione
> dell'operatore.** Gamba scoperta **e** coppia economicamente impossibile (`carico + ask sorella` oltre
> il tetto di **101¢**) ⇒ l'uscita considera anche prezzi **FUORI** dalla banda premiante. Prezzo =
> `max(pavimento del gradino, min(obiettivo, miglior bid))` — la **stessa** aritmetica di `inseguiIlBid`,
> non una seconda. Si rinuncia al premio su quella gamba per non restare direzionali.
> **⚠ IL MERGE VIENE PRIMA, SEMPRE, ED È SCRITTO**: `sizeAltroLato > 0` ⇒ si fonde e non si vende, ed è
> **una condizione**, non solo un ordine di valutazione; `sizeAltroLato` **non letta** chiude la deroga.
...
> **⚠ IL PAVIMENTO SI ARROTONDA IN SU SULLA GRIGLIA**, o `inseguiIlBid` a valle lo riporterebbe fuori
> griglia (v. il riquadro sopra).
...
> **⚠ E NON DICHIARA `inCoda`**: `manual-order` **riassegna** `price = q.price` dopo `prezzo-in-coda`, e
> quel ricalcolo riporterebbe l'uscita **dentro** banda annullando in silenzio il prezzo scelto. È la
...
> **⚠ IL PREMIO PERSO È ZERO PER COSTRUZIONE**, non per misura: un'uscita fuori banda **resta a riposo**
> solo dove `b.hi < pavimento`, cioè dove nessuna uscita in banda era ammessa e quindi non c'era nessun
> ordine da cui maturare (asserito su 7.000+ stati).
> **⚠ MONOTONO E OPT-IN**: senza `uscitaFuoriBanda: true` `planExit` è la funzione di prima riga per riga
> (asserito su 273 combinazioni). **Fail-closed** su ogni ingresso: bid illeggibile, ask dell'altro lato
> illeggibile, coppia non misurabile, `sizeAltroLato` non letta ⇒ nessuna deroga.
...
**Tetto della coppia 101¢, e adesso è UNO SOLO** (decisione dell'operatore; prima 99¢ per il merge e
120¢ per la chiusura rapida — la misura sui 65 maker veri è in `docs/misure-e-tarature.md`).
`MERGE_MIN_MARGIN_CENTS` è **derivato** (`100 − 101 = −1`), non ricopiato; `MAKER_TETTO_COPPIA_CENTS` è
un env con clamp `[100 · 200]`. Il valore si asserisce in **un punto solo**; gli altri test lo
**derivano**.
...
> **SI ATTRAVERSA, NON SI INSEGUE**: il prezzo è il bid camminato — restare sopra il bid ricrea il
> difetto misurato (`docs/misure-e-tarature.md`: 283 campioni, **zero** istanti offrivano un'uscita in
> guadagno, e un take-profit ancorato al mid esisteva già e non ha mai incassato niente).
> **TUTTA LA SIZE O NIENTE**: una copertura parziale lascerebbe un residuo sotto il minimo, cioè
> capitale senza via d'uscita. `TETTO_COPPIA_CENTS` **importato**. **⚠ Fail-closed**: ask illeggibile,
> scala che non copre la size, carico illeggibile ⇒ non scatta. **⚠ Il ramo `close-at-market` NON chiama
> `provaCoppia` quando il trigger è la presa di profitto**, e l'obbligo di esito viene scaricato a mano.
...
> `lib/maker/abbandono-posizione.js` (puro). Una posizione **scoperta** è dichiarata **ABBANDONATA**
> quando `valoreResiduo < SOGLIA` **E** `costoUscita ≥ valoreResiduo`: esce dal ciclo di uscita,
> **libera lo slot**, e **non si cancella nulla al venue e non si vende** — si smette solo di provare.
> · `valoreResiduo` = il **bid CAMMINATO** per l'INTERA size, mai `size × mid`.
...
> libro solo, quindi `askAltroLato = 1 − bidMioLato`. Il `min` resta scritto perché è l'unico punto che
> se ne accorgerebbe se il venue disaccoppiasse i libri.
> **LA SOGLIA È DERIVATA**: `PERDITA_MAX_FRAZIONE × MARKET_CAP_FIXED_USD = 0,05 × $61,25 =` **$3,0625**,
> entrambe **importate** — quanto R7 autorizza a bruciare per liberare una gamba, per la gamba più
> grande apribile. Sotto quella cifra R6 si contraddice.
...
> `scoperto-oltre-soglia-grave`, e l'ordine è un requisito — un test diventa rosso se qualcuno lo
> inverte. Una riga `posizione-abbandonata` si scrive **a ogni giro**, non solo al primo.
...
> `agent41.posizioniPerSelezione`, l'unico ingresso da cui la selezione deriva `inGestione`. **§4.8 non è
> toccata**: il perimetro live-min lo include finché ha posizioni o ordini a riposo.
...
> **La derivazione della soglia e le cinque posizioni al varo: `docs/misure-e-tarature.md`.**
...
> **importate**: chi cambia l'attesa del Livello 2 muove anche questo. Cura l'inversione misurata — il
> venue ritirava l'ordine di completamento **prima** che la regola smettesse di aspettarlo.
> **⚠ IL VENUE NON SA ESTENDERE UN ORDINE**: l'`expiration` sta **dentro la struct EIP-712 firmata** e
> nessuno dei due SDK installati espone `modify`/`amend`/`extend` (**88 metodi**, zero corrispondenze).
> Prolungare senza perdere la coda **non è possibile**: è un fatto verificato sul pacchetto installato.
> **⚠ LA QUOTAZIONE NON SI TOCCA, E IL PERCHÉ È UNA MISURA**: **il premio non conosce la coda**
> (`scoreOrder = ((v−d)/v)²`; la posizione in coda non è fra i sei ingressi di `quadraticUserShare`), e
> il churn residuo vale **≈ $0,03/giorno**. Allungare lì non compra niente e costerebbe esposizione non
> presidiata. **⚠ E LA CALIBRAZIONE DI `ripristino-gambe` RESTA VALIDA**: il suo tetto di 30 min sta
> sopra la GTD *della quotazione*, che non è cambiata — un GTD globale l'avrebbe invertita in silenzio.
> **⚠ COSA COSTA**: se l'host muore, un ordine di **chiusura** resta a libro fino a 33 min invece di 23.
> È l'unica direzione accettabile: un ordine di chiusura può solo **ridurre** l'esposizione. **Nessun
> ordine di APERTURA è toccato**, e `riposizionaDopoChiusura` è dichiarato ed escluso. Il tetto
> dell'orologio del mercato (`tooClose`) resta davanti.
...
### §4.13 · La selezione automatica dei mercati — testo integrale prima della potatura del 23 agosto 2026
...
> **🌦️🔴 IL FILTRO METEO È INCONDIZIONATO E NON HA INTERRUTTORE — misurato il 23 agosto 2026, 15:35Z.**
> Il cancello è nudo a **`selezione-mercati.js:469`**: `if (eMeteo(riga)) return { ammissibile:false,
> motivo:'famiglia-meteo' }`. **Nessun env lo condiziona, in nessun ramo**: `filtroMeteoArmato` e
> `MAKER_FILTRO_METEO` **non esistono** — zero occorrenze in `lib/`, `agents/`, `scripts/`, in
> `agents/ecosystem.config.js`, nel `.env` e in `/proc/<pid>/environ` di agent41.
> **⇒ ARMATO E NON SPEGNIBILE DA CONFIGURAZIONE**: scrivere `MAKER_FILTRO_METEO: '0'` nell'ecosystem e
> riavviare **dal file** non cambierebbe niente, e chi lo facesse **crederebbe di aver disarmato**.
> Disarmarlo davvero vuol dire **scrivere il codice dell'interruttore**: è un lavoro, non un env.
> **⚠ È LA CAUSA MISURATA DELLO SLOT CORTO VUOTO** (§5.2 p.72): dei **75** mercati del board fra 24 e
> 48 h, **72 sono meteo**; dei 3 superstiti due hanno `minSize 50`, quindi per il posto «basso» libero
> resta **un solo candidato**. **⚠ Zero falsi positivi**: sono tutti categoria `Weather` al venue e le
> ancore `\b` fanno il loro lavoro — il filtro è **corretto**, è la decisione di escludere che è in
> discussione. **⚠ Chi misura usi `selezione-mercati.eMeteo`, non un classificatore gemello.**
> **⚠ DA SOLO IL DISARMO NON APRIREBBE NIENTE**: i meteo sono tutti `rewardsMinSize` 20, cioè tutti nel
> secchio «basso», e quanti ne entrano lo decide `quotaScaglioni`. **⚠ E il loro premio non è misurato
> sul lungo**: il payback di §5.2 p.58 li giudicherebbe uno per uno.
> **Il referto integrale — le due misure del 23/08 e la classe D7 in cui il documento citava se stesso —
> sta in `docs/misure-e-tarature.md`; la voce aperta è §5.2 p.69, per intero.**
...
> **🧊 «SLOT STERILE» — ARMATA dal 20 agosto 2026** (`52c33f4`: soglia **22 min**, quarantena **180
> min**, tetto **5 rilasci/ora**): libera uno slot che per **due osservazioni consecutive** non produce
> ordini. ⚠ **`SLOT_STERILE_ARMATO` NON compare in `ecosystem.config.js` né in `/proc` di agent41, e
> ASSENTE ⇒ ARMATA**: è il fail-safe voluto, non una svista. Chi legge `/proc` e non trova la variabile
> ha trovato la regola **ACCESA**. Il giornale lo conferma: `esito:'in-attesa'`/`'rilascia'` quando è
> armata, `esito:'disarmato'` quando non lo è.
> ⚠ **«NESSUN ORDINE A LIBRO» HA DUE CAUSE OPPOSTE** — *sterile* e *svuotato da noi* (mid stantio,
> erosione): un'osservazione non conta come sterile se in quel mercato ci sono state **cancellazioni
> nostre** nella finestra, e il contatore si **azzera a ogni piazzamento riuscito**. È la correzione del
> difetto che la fece disarmare il giorno stesso in cui nacque (presumeva che la causa stesse nel
> MERCATO mentre stava nel FEED: buttò fuori **cinque volte** un mercato che andava benissimo).
> ⚠ **LA QUARANTENA VIVE IN MEMORIA** (`statoLibroVuoto` in agent41, zero `require`, nessuna scrittura
> su disco): **un riavvio di agent41 la azzera**, e con essa il contatore `zeroDa`. Non è un disarmo —
> dopo il riavvio nessuno può essere rilasciato per almeno i 22 minuti della soglia — ma è una
> **perdita del freno anti-churn**, e va dichiarata da chi riavvia.
> ⚠ **I MERCATI IN QUARANTENA NON COMPAIONO IN NESSUNA LISTA DI SCARTO DELLA SELEZIONE**: entrano in
> `escludi` (agent41 §2479) e cadono a `selezione-mercati.js:787`, cioè **prima** del cancello di
> composizione. **Difetto di osservabilità dichiarato, non corretto** (§5.2 p.62).
> **PER DISARMARLA**: `SLOT_STERILE_ARMATO: '0'` in `ecosystem.config.js` + riavvio di agent41 **dal
> file** — solo il valore ESATTO `'0'` disarma, come per `SBLOCCO_GRADINO6_ARMATO`.
...
> composizione **verso** la cifra decisa dall'operatore, mai lontano.
...
> +$22,90/giorno.**
> **⚠ E LA DISTANZA A CUI IL PIANO GIUDICA È QUELLA VERA**: `conDistanzaDiPiano` (un punto solo, usato
> da **entrambi** i piani — quello operativo e quello dei netti che ordinano la selezione) passa
...
> ma **0,3¢** su griglia 0,1¢.
> **⚠⚠ E VA DETTO CHE NON MUOVE I NETTI**: misurato, i tre modi danno gli stessi numeri a meno della
> seconda cifra. `offsetTicks` governa il **costo** di selezione avversa, non il punteggio del venue:
> **il lordo nasce da `levels[]` del board, che agent24 calcola con la propria posa tipica, e QUELLA
> resta disallineata.** La correzione si applica perché il parametro deve dire il vero, non perché curi
> il capitale fermo. **Il fatto misurato alle 10:20Z: `docs/misure-e-tarature.md`.**

> **🔁 LA COPERTURA CONTINUA RIMETTE LA GAMBA A LIBRO — §5-bis p.171.**
> `lib/maker/ripristino-gambe.js` (puro) è una scala sui fallimenti **consecutivi**: subito · 5 · 10 ·
> 20 · **30 min di tetto**, azzerata quando il mercato torna `coperto`. **⚠ IL NUMERO CHE GOVERNA IL
> DISEGNO È 720**: il ciclo che ospita la decisione gira ogni **120 s**, e senza raffreddamento un
> mercato che rifiuta sempre verrebbe ritentato 720 volte al giorno (contenimento provato coi numeri:
> **50 tentativi su 720 cicli, 14,4×** — asserzione del test, non una frase in un commento). Il primo
> tentativo è immediato perché la GTD è 23 min; il tetto sta **sopra** la GTD perché oltre quella
> soglia il problema non è più «manca la gamba» ma «questo mercato non si riesce a quotare», e la
> risposta è `da-sostituire`. **⚠ Si azzera su `coperto` OSSERVATO, non su un invio accettato.**
> **LE TRE COSE CHE NON FA**: ① non è una seconda strada verso il venue — riga dal piano **già
> salvato** → `gambeDiUnaRiga` → `piazzaCoppia`, cioè lo stesso `runBulkAllocation` con lo stesso freno
> e gli stessi gate; ② **non ricostruisce il piano**; ③ **non abilita niente**. **E UNA CHE FA**: scrive
> **sempre** a verbale (`tipo: 'ripristino-gamba'`), anche quando non tenta.
...
> `valutaCopertura` risponde in **token** (traduzione esplicita, fail-closed), e `LOCK.stato()`
...
> `Q = min(Q_piano, Q_tetto, Q_gamba_viva)`, e nessuno dei tre può far CRESCERE niente. Cura il difetto
> dell'**ASIMMETRIA** — `gambeDiUnaRiga` calcolava una size simmetrica **nell'istante in cui
> costruisce**, mentre la gamba superstite porta quella dell'istante in cui *fu piazzata*, e nessuno la
> riportava a oggi (referto: `docs/referti-difetti-corretti.md`).
...
### §4.7 · Scoperta e feed — testo integrale prima della potatura del 23 agosto 2026
...
### §4.8 · La regola di copertura — testo integrale prima della potatura del 23 agosto 2026
...
### §4.9 · Merge on-chain e relayer — testo integrale prima della potatura del 23 agosto 2026
...
### §4.10 · Registri, giornali, persistenza — testo integrale prima della potatura del 23 agosto 2026
...
### §4.14 · Le quattro cinture — testo integrale prima della potatura del 23 agosto 2026
...
### Testata di CLAUDE.md (intestazione, riquadri di armamento, stato operativo) — testo integrale prima della potatura del 23 agosto 2026
...
### §2 · Permessi della sessione e hook — testo integrale prima della potatura del 23 agosto 2026
...
### §4.11 · Backoff, rate limit, resilienza — testo integrale prima della potatura del 23 agosto 2026
...
### §4.5 · Il capitale al lavoro — testo integrale prima della potatura del 23 agosto 2026
...
### §4.12 · Stima e consuntivo — testo integrale prima della potatura del 23 agosto 2026

 ...
 *   venue, marketId, title, category, midpoint, maxSpread, minSize, minOrderSize,
...
// ⚠ ANCORATI AL PACKAGE ROOT, NON A UN PERCORSO ASSOLUTO (17 agosto 2026, migrazione root → bot).
// Erano `/root/prediction-market/data/...`: quando la copia di lavoro si e' spostata, questo modulo ha
// continuato a leggere un file che l'utente nuovo non puo' nemmeno aprire — e `readJson` restituisce
// `null` senza sollevare, quindi il board usciva VUOTO invece che illeggibile. `DATA_DIR` e' la stessa
// funzione che usa `lib/safety/store.js` (risale al package root saltando le directory di build), cioe'
// la definizione unica di «dove sta data/»: importata, non ricopiata.
const { DATA_DIR } = require('./safety/store');
const POLY_FILE   = path.join(DATA_DIR, 'liquidity-rewards.json');
const KALSHI_FILE = path.join(DATA_DIR, 'kalshi-rewards.json');
const OUT_FILE    = fileRuntime('liquidity-rewards.json');
...
function normalizePoly(polyData) {
  ...
  return markets.map(m => {
    ...
    return {
      ...
      minSize:             m.rewardsMinSize ?? null,     // shares — PAVIMENTO PREMIANTE (`rewards.min_size`)
      // ⚠ IL SECONDO MINIMO, E NON E' QUELLO SOPRA. `minSize` e' il pavimento PREMIANTE (sotto: reward
      // ZERO, ordine valido); `minOrderSize` e' il MINIMO D'ORDINE del venue (`minimum_order_size`,
      // letto da agent24 su `GET /markets/<cid>`): sotto, il venue RIFIUTA. Il percorso d'USCITA legge
      // questo. Assente ⇒ `null`, mai 0 — chi lo legge si ferma dichiarando il conditionId invece di
      // ripiegare sul pavimento premiante. Vedi `lib/maker/minimi-del-venue.js`.
      ...
      // LA SCADENZA REALE, non solo la durata derivata. `hoursToResolution` e' un NUMERO calcolato al
      // momento della normalizzazione: chi legge il board un'ora dopo non puo' ricostruire l'istante, e
      // `horizonVerdict` vuole una ISO. Senza questo campo la scadenza valeva `unknown` su 306 righe
      // su 306, e un mercato a 14,3 ore entrava nel piano per poi essere rifiutato dalla verifica
      // (§5 punto 98, il CICLO FERMATO del 12 agosto). Verbatim da agent24, mai inventata.
      endDate:             m.endDate ?? null,
      endDateSource:       m.endDateSource ?? null,
      // La scadenza RICONCILIATA fra board e venue (lib/rewards/scadenza-mercato). `endDate` qui sopra
      // e' gia' il verdetto; questi campi lo rendono verificabile e portano l'AMMISSIBILITA', che
      // l'allocatore applica come esclusione a monte invece di lasciarla scoprire alla verifica.
      endDateGamma:        m.endDateGamma ?? null,
      endDateClob:         m.endDateClob ?? null,
      endDateFonte:        m.endDateFonte ?? null,
      scadenzaDivergenzaOre: m.scadenzaDivergenzaOre ?? null,
      // Assente ⇒ `null`, non `true`: un board vecchio, scritto prima della riconciliazione, non deve
      // sembrare uno su cui la riconciliazione ha detto di si'.
      scadenzaAmmissibile: typeof m.scadenzaAmmissibile === 'boolean' ? m.scadenzaAmmissibile : null,
      scadenzaMotivo:      m.scadenzaMotivo ?? null,
...
function buildCombined() {
  ...
  // ── LE RIGHE CHE agent24 AVEVA GIÀ TOLTO PER PROFONDITÀ TORNANO IN GIOCO QUI ────────────────────
  // Non entrano nel board: entrano nell'INSIEME DA GIUDICARE, e il filtro qui sotto le rigetta tutte
  // tranne quelle dove abbiamo capitale dentro. Senza questa riga l'eccezione più sotto era LETTERA
  // MORTA per il caso che la motivava — agent24 applica lo stesso pavimento a monte e la riga sottile
  // non arrivava mai fin qui. Due filtri con lo stesso predicato in sequenza, e solo il secondo con
  // l'eccezione: vinceva sempre il primo. (11 agosto 2026)
  //
  // Fail-closed su un file vecchio: il campo non c'è ⇒ nessuna riammissione ⇒ comportamento di prima.
  const polySoppressi = (poly && Array.isArray(poly.suppressedThinDepthMarkets))
    ? normalizePoly({ markets: poly.suppressedThinDepthMarkets })   // STESSA normalizzazione, non una seconda
    : [];
  const allMarkets = [...polyMarkets, ...polySoppressi, ...kalshiMarkets];
  ...
  return {
    meta: {
      ...
      // Quante righe agent24 aveva già tolto e questa passata ha ri-giudicato: senza il numero,
      // «zero esentati» non distingue «nessuno aveva capitale dentro» da «non è arrivata una riga».
      riammesseDaAgent24:   polySoppressi.length,

 ...
 *
 * It operates on ENRICHED rows — plain objects the component builds by running each market
 * through lib/rewards-estimate (the SSOT) once, so this file never re-derives a number:
 *   { category, venue, poolDayUsd, saturation, apr, capacityUsd, netUsdPerDay, isTrap }
 *   - poolDayUsd   = dailyPool               (real reward pool $/day)
 *   - netUsdPerDay = est.netPerDay           (primary; already net of adverse selection)
 *   - apr          = est.annualizedPct       (capped 200% / labeled by the SSOT)
 *   - capacityUsd  = bookDepthAtBand         (real book depth at the reward band)
 *   - saturation   = 1 - est.shareOfPool     (DERIVED: fraction of the pool a reference
 *                    $1k maker's competitors already hold; null when qualifyingLiquidity

...
function sortRows(rows, s) {
  ...
  // A row is demoted when it is not a real opportunity for THIS operator: a sub-floor reward pot
  // (potTooSmall, Phase 1) OR a market whose rewards are not collectable from this jurisdiction
  // (demoted, Phase 2 — Kalshi is US-only). Either flag pins it below every non-demoted row.
  const demoted = (r) => !!(r && (r.demoted || r.potTooSmall));

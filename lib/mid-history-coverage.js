...
function coverageHeader({ coveredMarketCount, universeMarketCount, at = null } = {}) {
  ...
  const headerLines = [];
  headerLines.push(
    `COVERAGE: ${covered == null ? '—' : covered} of ${universe == null ? '—' : universe} rewards-universe markets` +
    (coveragePct != null ? ` — ${coveragePct}% of the full universe` : ' — full universe size unknown'),
  );
  if (partial) {
    headerLines.push(

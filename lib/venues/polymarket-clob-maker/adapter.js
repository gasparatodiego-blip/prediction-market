 ...
 */
function evaluateLiveMinMarketGate({ mode, liveMinMarket, allowedMarketIds, marketId } = {}) {
...
function createMakerAdapter(opts = {}) {
  ...
  async function postOrder(spec) {
    ...
    const lmg = evaluateLiveMinMarketGate({ mode, liveMinMarket, allowedMarketIds: readAllowedMarketIds(), marketId: s.marketId });
    if (!lmg.allow) {

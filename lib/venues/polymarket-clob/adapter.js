...
// La STESSA definizione che usa l'adapter di piazzamento, importata e non ricopiata: due copie che
// divergono qui decidono se il KILL crede di aver finito. V. il commento in testa a `risposta-venue`.
const { listaDaRisposta, esitoCancellazione, dettagliErrore } = require('../risposta-venue');
...
function createCancelOnlyAdapter(opts = {}) {
  ...
  async function listOpenOrders(marketId) {
    ...
    try {
      ...
      // ⚠ QUI C'ERA IL FALLBACK CIECO `: []`, e questa e' la lettura da cui `cancel-all` decide cosa
      //   spazzare: una forma non capita diventava «zero ordini a libro» e il KILL rispondeva
      //   «cancellati 0» come se avesse finito il lavoro.
      ...
      // CATTURA DEL CORPO GREZZO: `safeError` conserva il solo messaggio, e dell'ora di guasto del
      // 19 agosto e' rimasto soltanto «response.data is not iterable», senza status ne' corpo.
  ...
  async function cancelOrder(orderId) {
    ...
    try {
      ...
      if (!es.ok) {
        if (isAlreadyGone(es.messaggio)) {
          audit('cancelOrder', { requested: { orderId }, response: es.messaggio, outcome: 'noop', latencyMs: now() - t0 });
  ...
  async function cancelMarketOrders(marketId) {
    ...
    try {
      ...
      if (!es.ok) {
        if (isAlreadyGone(es.messaggio)) {
          audit('cancelMarketOrders', { requested: { marketId }, response: es.messaggio, outcome: 'noop', latencyMs: now() - t0 });

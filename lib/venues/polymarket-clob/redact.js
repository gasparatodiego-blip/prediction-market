...
// ── L'HASH DELLA TRANSAZIONE, 7 agosto 2026 ────────────────────────────────────────────────────────
// Terza volta che lo stesso difetto morde, e con la stessa forma: un hash di transazione e' `0x` + 64
// hex, identico alla cintura sopra. Il primo merge via relayer e' andato a buon fine e l'audit ha
// registrato `transactionHash: "0x[redacted-64hex]"` — cioe' la riga diceva «confermata» senza dire
// QUALE transazione, che e' l'unico dato per cui la si va a leggere. L'hash e' stato recuperato a mano
// dall'API del relayer. Un hash e' un artefatto PUBBLICO di una transazione pubblica: non c'e' niente
// da proteggere, e sotto una chiave che si chiama `transactionHash` non ci arriva una chiave privata
// senza che sia gia' successo qualcosa di molto piu' grave.
const PUBLIC_ID_KEY = /^(orderid|order_id|orderids|order_ids|oldorderid|neworderid|idempotencykey|canceled|cancelled|not_canceled|not_cancelled|notcanceled|transactionhash|transaction_hash|txhash|tx_hash|transactionid|transaction_id|blockhash|block_hash)$/i;

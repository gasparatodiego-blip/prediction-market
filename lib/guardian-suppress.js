...
const SECTION_CFG = {
  ...
  rewards: {
    aprField: 'dayYieldPct', aprKind: 'dailyPctToAnnual', // *365 — stamped by the rewards-unified route
                                                          // from the REAL displayed computeLiquidityYield
                                                          // at the $1k reference (was absent → rule inert).
    // Rewards $/day is a DISCLOSED-CORRECT dilution ESTIMATE (share shrinks as you deploy), not a
    // cashable edge — so an over-cap run-rate is the honest-engine's "run-rate, not guaranteed" case, to be
    // LABELLED, never blanked or hidden (the arithmetic is right; the defect was missing the label). So A1
    // RELABELS (attaches the run-rate caveat) instead of suppress-value, and A2 does NOT hide. Cashable
    // sections keep the default blank/hide, where a >1000%/yr does mean broken math.
    aprHardAction: 'relabel',      // A1 (>200%/yr): attach run-rate caveat, do NOT blank the value
    aprImpossibleHides: false,     // A2 (>1000%/yr): do NOT hide — a disclosed thin-book run-rate is not broken math
...
function inspectRow(section, row, ctx = {}) {
  ...
  if (apr != null && cfg.aprKind !== 'oneTimePct') {
    // A2: implies an impossible APR (>1000%/yr) → hide the row — UNLESS the section opts out
    // (rewards: a disclosed thin-book run-rate is not broken math; it is labelled, not hidden).
    if (apr > APY_IMPOSSIBLE && cfg.aprImpossibleHides !== false) {
      ...
      if (cfg.aprHardAction === 'relabel') {
        // Disclosed-correct estimate (rewards): attach the run-rate caveat, keep the value.
        out.push({ rule: 'A1', action: 'relabel', runRate: true, label: LABELS.RUN_RATE,
          reason: `annualized ${apr.toFixed(0)}%/yr exceeds ${APY_HARD_MAX}% cap — labelling "run-rate, not guaranteed" (value kept; arithmetic is correct)` });
      } else {
        // Cashable edge: suppress the value, show "in verifica".

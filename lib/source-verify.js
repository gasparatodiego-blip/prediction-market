...
const FUNDING_ADAPTERS = {
  ...
  // ── Newer perp DEXes — public market-data APIs (no auth to READ), each with its OWN
  //    funding UNIT. Adapters mirror agent10's exact interpretation so an independent
  //    re-read compares like-for-like. Absent these, agent29 reported these venues
  //    'unreachable' → a FALSE "stale" badge on genuinely-fresh rows (see fetchGrvt/
  //    fetchLighter/fetchEdgex/fetchPacifica/fetchApex/fetchParadex in agent10). We do
  //    NOT force a pass: the row still clears only when the re-fetched rate matches the
  //    served rate within tolerance.
  async grvt(coin) {
    // Grvt market-data is PUBLIC (no auth/signature to read: ticker confirmed). Instrument
    // name is derivable — NO numeric id map. CRITICAL UNIT: funding_rate_8h_curr is ALREADY
    // a PERCENT per 8h (e.g. 0.0872 = 0.0872%/8h), NOT a fraction — so NO ×100 (mirrors
    // agent10 fetchGrvt; a fraction reading would 100× it into a phantom mismatch).
    const d = await post('https://market-data.grvt.io/full/v1/ticker', { instrument: `${coin}_USDT_Perp` });
    const r = d && d.result;
    if (!r || r.funding_rate_8h_curr == null) return null;
    return { ratePct: num(r.funding_rate_8h_curr), intervalHours: 8,
             nextFundingTime: null, markPrice: num(r.mark_price) };
  },
  async lighter(coin, cache) {
    // Lighter's PUBLIC /funding-rates aggregates many venues → filter exchange==='lighter'.
    // UNIT: `rate` is an 8h-NORMALIZED FRACTION; native %/hr = rate/8*100 with intervalHours=1
    // (mirrors agent10 fetchLighter). Mark from /orderBookDetails (last_trade_price). Both
    // are single bulk calls, memoized per cycle regardless of how many coins short lighter.
    if (!cache.lighter) {
      const d = await get('https://mainnet.zklighter.elliot.ai/api/v1/funding-rates');
      const map = {};
      if (d && Array.isArray(d.funding_rates))
        for (const row of d.funding_rates) if (row.exchange === 'lighter' && row.symbol) map[row.symbol] = row;
      cache.lighter = map;
    }
    if (!cache.lighterMarks) {
      const d = await get('https://mainnet.zklighter.elliot.ai/api/v1/orderBookDetails');
      const map = {};
      if (d && Array.isArray(d.order_book_details))
        for (const o of d.order_book_details) { const px = num(o.last_trade_price); if (o.symbol && px != null && px > 0) map[o.symbol] = px; }
      cache.lighterMarks = map;
    }
    const row = cache.lighter[coin];
    if (!row || row.rate == null) return null;
    const raw = num(row.rate);
    if (raw == null) return null;
    return { ratePct: raw / 8 * 100, intervalHours: 1,
             nextFundingTime: topOfNextHourMs(Date.now()), markPrice: cache.lighterMarks[coin] ?? null };
  },
  async edgex(coin, cache) {
    // edgeX PUBLIC REST (StarkEx CLOB; browser headers already sent — it sits behind
    // Cloudflare). We build coin→contractId from getMetaData ourselves (memoized), then read
    // the bulk funding endpoint per coin. UNIT: fundingRate is a per-INTERVAL FRACTION
    // (×100 = %/interval), interval from fundingRateIntervalMin (typically 240min=4h) —
    // mirrors agent10 fetchEdgex. Continuous host 429s → get() returns null → honest unreachable.
    if (!cache.edgexMeta) {
      const meta = await get('https://pro.edgex.exchange/api/v1/public/meta/getMetaData');
      const cl = meta && meta.data && meta.data.contractList;
      const byCoin = {};
      if (Array.isArray(cl)) for (const c of cl) {
        ...
        byCoin[name.slice(0, -3)] = { contractId: c.contractId, intervalHours: isFinite(im) && im > 0 ? im / 60 : 4 };
      }
      cache.edgexMeta = byCoin;
    }
    const info = cache.edgexMeta[coin];
    if (!info) return null;
    const bulk = await get(`https://pro.edgex.exchange/api/v1/public/funding/getLatestFundingRate?contractId=${info.contractId}`);
    const rows = bulk && Array.isArray(bulk.data) ? bulk.data : null;
    const row  = rows ? (rows.find(x => x.contractId === info.contractId) || rows[0]) : null;
    if (!row || row.fundingRate == null) return null;
    return { ratePct: fracToPct(row.fundingRate), intervalHours: info.intervalHours,
             nextFundingTime: null, markPrice: num(row.markPrice) };
  },
  async pacifica(coin, cache) {
    // Pacifica (pacifica.fi) — Solana CLOB perp DEX, PUBLIC REST (no auth to read). ONE bulk
    // /info/prices call covers every market, memoized per cycle regardless of how many coins
    // short pacifica. Keyed on symbol (== coin, NO numeric id map). UNIT: `funding` is a raw
    // FRACTION per HOUR (×100 = %/hr, intervalHours=1) — mirrors agent10 fetchPacifica. Pacifica
    // exposes no next-funding timestamp (predicted rate only); hourly settlement → top-of-hour.
    if (!cache.pacifica) {
      const d = await get('https://api.pacifica.fi/api/v1/info/prices');
      const map = {};
      if (d && Array.isArray(d.data)) for (const m of d.data) if (m && m.symbol) map[m.symbol] = m;
      cache.pacifica = map;
    }
    const m = cache.pacifica[coin];
    if (!m || m.funding == null) return null;
    const fr = num(m.funding);
    if (fr == null) return null;
    return { ratePct: fr * 100, intervalHours: 1,
             nextFundingTime: topOfNextHourMs(Date.now()), markPrice: num(m.mark) };
  },
  async apex(coin) {
    // ApeX Omni (apex.exchange) — orderbook perp DEX, PUBLIC REST (no auth to read). No bulk
    // ticker exists, so one /ticker call per coin (mirrors agent10 fetchApex; verification touches
    // only the few apex rows served each cycle). Symbol form "<COIN>USDT" (no dash). UNIT:
    // fundingRate is a raw FRACTION per HOUR (×100 = %/hr, intervalHours=1). nextFundingTime is an
    // ISO-8601 string → parse to ms (enables the settlement-boundary escape vs the stored row).
    const d = await get(`https://omni.apex.exchange/api/v3/ticker?symbol=${coin}USDT`);
    const t = d && Array.isArray(d.data) ? d.data[0] : null;
    if (!t || t.fundingRate == null) return null;
    const fr = num(t.fundingRate);
    if (fr == null) return null;
    const nft = Date.parse(t.nextFundingTime);
    return { ratePct: fr * 100, intervalHours: 1,
             nextFundingTime: Number.isFinite(nft) && nft > 0 ? nft : null, markPrice: num(t.markPrice) };
  },
  async paradex(coin, cache) {
    // Paradex — StarkNet CLOB perp DEX, PUBLIC REST (no auth to read). /markets/summary?market=ALL
    // carries per-market funding_rate + mark_price; /markets carries funding_period_hours. Both
    // are bulk calls, memoized per cycle. Symbol form "<COIN>-USD-PERP". UNIT: funding_rate is the
    // funding-PERIOD (8h) rate as a FRACTION (×100 = %/period) with the real per-symbol period —
    // mirrors agent10 fetchParadex (default 8h). No next-funding timestamp exposed → null.
    if (!cache.paradexPeriod) {
      const d = await get('https://api.prod.paradex.trade/v1/markets');
      const map = {};
      if (d && Array.isArray(d.results)) for (const mk of d.results) {
        if (mk.asset_kind !== 'PERP' || !mk.symbol) continue;
        const h = parseInt(mk.funding_period_hours ?? 8, 10);
        if (isFinite(h) && h > 0) map[mk.symbol] = h;
      }
      cache.paradexPeriod = map;
    }
    if (!cache.paradexSummary) {
      const d = await get('https://api.prod.paradex.trade/v1/markets/summary?market=ALL');
      const map = {};
      if (d && Array.isArray(d.results)) for (const row of d.results) if (row.symbol) map[row.symbol] = row;
      cache.paradexSummary = map;
    }
    const sym = `${coin}-USD-PERP`;
    const row = cache.paradexSummary[sym];
    if (!row || row.funding_rate == null) return null;
    const fr = num(row.funding_rate);
    if (fr == null) return null;
    const iv = cache.paradexPeriod[sym];
    return { ratePct: fr * 100, intervalHours: (typeof iv === 'number' && iv > 0) ? iv : 8,
             nextFundingTime: null, markPrice: num(row.mark_price) };
  },
...
// Venues we can independently re-read for funding. Anything not here is reported
// 'unreachable' — HONEST: we do not claim to have verified a leg we cannot re-fetch.
// grvt/lighter/edgex AND now pacifica/apex/paradex have real adapters above (their
// market-data APIs are all public, no auth to read), so their genuinely-fresh rows
// verify instead of wearing a false "stale/unreachable" badge.

...
// The relative arm additionally requires the 8h-normalized delta to clear this
// small floor. Several venues publish a LIVE-UPDATING predicted funding rate that
// legitimately drifts within a settlement period; a >10%-relative move that is
// only ~0.001%/8h in absolute terms is that drift, not a phantom. The absolute
// arm (0.005%/8h) still catches any economically real divergence on its own.
// This is the "rates legitimately move" allowance made concrete.
const FUNDING_REL_NOISE_8H  = 0.002; // 0.002 %/8h

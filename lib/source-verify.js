...
const FUNDING_ADAPTERS = {
  ...
  //    fetchLighter/fetchEdgex in agent10). We do NOT force a pass: the row still clears
  //    only when the re-fetched rate matches the served rate within tolerance.
  async grvt(coin) {
...
// Venues we can independently re-read for funding. Anything not here (pacifica,
// apex, paradex) is reported 'unreachable' — HONEST: we do not claim to have verified
// a leg we cannot re-fetch. grvt/lighter/edgex now have real adapters above (their
// market-data APIs are public), so their genuinely-fresh rows verify instead of
// wearing a false "stale" badge.

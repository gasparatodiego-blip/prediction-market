...
(async () => {
  ...
  // E i due chiamanti veri la cablano.
  {
    ...
    ok('  e il vecchio interruttore d\'ambiente non viene più LETTO da nessuna parte',
      !/process\.env\.REALLOC_SCHEDULER_DRY_RUN/.test(a41), 'agent41 è tornato a leggere REALLOC_SCHEDULER_DRY_RUN');
    const eco = leggi('agents', 'ecosystem.config.js');
    ok('  né è tornato in ecosystem.config.js come default',
      !/REALLOC_SCHEDULER_DRY_RUN:/.test(eco), 'la riga REALLOC_SCHEDULER_DRY_RUN è ricomparsa nell\'ecosystem');
  }

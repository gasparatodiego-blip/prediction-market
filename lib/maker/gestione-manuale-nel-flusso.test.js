...
(async () => {
  ...
  // E i due chiamanti veri la cablano.
  {
    ...
    ok('  né è tornato in ecosystem.config.js come default',
      conFreno.length === 0, `lo dichiarano: ${conFreno.join(', ')}`);
    // E la prova che il filtro dei commenti serve davvero: nel sorgente la stringa C'È, e va bene.
    ok('  (il nome resta nei commenti, ed è giusto: raccontano perché non va rimesso)',
      /REALLOC_SCHEDULER_DRY_RUN/.test(leggi('agents', 'ecosystem.config.js'))
      && !/REALLOC_SCHEDULER_DRY_RUN/.test(senzaCommenti(leggi('agents', 'ecosystem.config.js'))));
  }

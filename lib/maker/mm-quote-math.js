...
/**
 * L'OFFSET DEL MOTORE DERIVATO DA UN PREZZO SCELTO SUL BOOK, in centesimi.
 *
 * Toccare una riga del libro e' il modo piu' diretto di dire «voglio stare QUI»: la distanza fra quel
 * prezzo e il mid e' esattamente l'offset che il tracking dovrebbe tenere. Questa funzione fa quella
 * traduzione, e vive qui — accanto a planQuotes, che quell'offset lo consuma — invece che dentro il
 * componente, perche' e' aritmetica e il test deve poterla esaurire senza aprire un browser.
 *
 * TRE VINCOLI, TUTTI E TRE NECESSARI:
 *   · SUL TICK — un offset che non e' multiplo del tick genera prezzi che il venue rifiuta.
 *   · MAI SOTTO UN TICK — toccare la riga del mid darebbe distanza 0, e con offset 0 i due lati si
 *     incrociano fra loro invece di quotare. Il registro lo rifiuterebbe comunque (minimo 0.1¢), ma
 *     precompilare un campo con un valore che non puo' essere accettato e' un vicolo cieco.
 *   · MAI OLTRE IL RAGGIO PREMIANTE — oltre quello l'ordine riposa e matura ZERO, e suggerirlo
 *     partendo da un tocco significherebbe proporre una configurazione perdente. Se il venue non
 *     pubblica una banda NON si inventa un tetto: si restituisce la distanza vera.
 *
 * @returns {number|null} centesimi, oppure null se prezzo o mid non sono leggibili (mai uno zero).
 */
function offsetFromPrice({ price, mid, tick, bandRadiusCents = null } = {}) {
  if (!fin(price) || !fin(mid)) return null;
  const tickC = fin(tick) && tick > 0 ? tick * 100 : 1;
  const rawC = Math.abs(price - mid) * 100;
  const snapped = Math.max(tickC, Math.round(rawC / tickC) * tickC);
  const capped = fin(bandRadiusCents) && bandRadiusCents > 0 ? Math.min(snapped, bandRadiusCents) : snapped;
  return +capped.toFixed(2);
}

/**
 * LA SCALA DEL CURSORE DELLA SIZE, dai due estremi che hanno un significato.
 *
 *   0%   = `minSize`, la size minima PREMIANTE del venue. Non e' un minimo tecnico — il CLOB accetta
 *          anche meno — e' il minimo sotto cui il programma premi non vede l'ordine e matura ZERO.
 *   100% = quante share si comprano col capitale disponibile a quel prezzo.
 *
 * `readable:false` quando manca un ingrediente, e chi chiama deve DIRLO invece di disegnare un cursore:
 * uno 0% senza una size minima pubblicata, o un 100% senza un saldo letto, sarebbero due estremi finti,
 * e su questa schermata un numero finto e' un ordine sbagliato.
 *
 * Se il capitale non copre nemmeno il minimo, il massimo NON scende sotto il minimo: l'intervallo
 * resta degenere (lo === hi) e il gate del saldo dira' a voce che non basta, invece di far scegliere
 * fra due numeri entrambi impossibili.
 */
function sizeScale({ minSize, price, capitalUsd, orderCapUsd = null } = {}) {
  const lo = fin(minSize) && minSize > 0 ? minSize : null;
  const px = fin(price) && price > 0 ? price : null;
  const cap = fin(capitalUsd) && capitalUsd >= 0 ? capitalUsd : null;
  if (lo == null || px == null || cap == null) return { readable: false, lo, hi: null, boundBy: null };
  // DUE TETTI, E VINCE IL PIU' BASSO. Il capitale dice quanto si POSSIEDE, il tetto per ordine dice
  // quanto si e' deciso di rischiare in un colpo solo: sono limiti diversi e il cursore deve fermarsi
  // al primo dei due. Portare il 100% oltre il tetto significherebbe disegnare una posizione che il
  // gate rifiuta un attimo dopo — un cursore che arriva dove l'ordine non puo' andare.
  const usable = fin(orderCapUsd) && orderCapUsd >= 0 ? Math.min(cap, orderCapUsd) : cap;
  const boundBy = usable === cap ? 'capitale' : 'tetto-ordine';
  return { readable: true, lo, hi: Math.max(lo, Math.floor(usable / px)), boundBy };
}

/** La size che corrisponde a una percentuale sulla scala. null se la scala non e' leggibile. */
function sizeAtPct(scale, pct) {
  if (!scale || scale.readable !== true || !fin(scale.lo) || !fin(scale.hi)) return null;
  const p = Math.min(100, Math.max(0, fin(pct) ? pct : 0));
  return Math.round(scale.lo + ((scale.hi - scale.lo) * p) / 100);
}

module.exports = { planQuotes, decideRetrack, decideSide, snap, offsetFromPrice, sizeScale, sizeAtPct };

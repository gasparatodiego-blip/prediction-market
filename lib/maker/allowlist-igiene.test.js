...
//
// 1 · L'ATTESTAZIONE MANCANTE. Il blocco `env:` di agent40 in ecosystem.config.js conteneva solo
//     NODE_ENV e HOME, e gli agenti non caricano .env (nessuno importa dotenv). Letto da
//     /proc/<pid>/environ — non da pm2, che mostra anche quel che eredita dal demone:
//
//         agent40:  MAKER_MODE=off  MANUAL_ORDER_PLACEMENT=send
//         agent41:  … MAKER_FUNDING_APPROVED=true …
//         agent35:  … MAKER_FUNDING_APPROVED=true …
//
//     Non si vedeva, perche' agent40 riprezza e non apre posizioni. Si sarebbe visto al PRIMO FILL:
//     uscita automatica e rimpiazzo della gamba passano da placeManualOrder → buildPlacementAdapter,
//     che legge `process.env.MAKER_FUNDING_APPROVED === 'true'`; senza, evaluatePlacementGate rifiuta
//     con gate `funding-approval`. Cioe' i due meccanismi costruiti PER il fill non sarebbero entrati
//     in funzione, e il rifiuto sarebbe arrivato con delle share gia' nostre.
//
...
console.log('\n══ 1 · L ATTESTAZIONE È NELL AMBIENTE DI agent40, E ALLINEATA AGLI ALTRI DUE');
{
  ...
  // L'allineamento con gli altri due, che l'avevano gia'.
  // Il blocco di un agente va da `name:` al `name:` successivo — NON a un numero fisso di caratteri:
  // fra il nome di agent35 e il suo `env:` ci sono novanta righe di commento, e una fetta da 6000
  // caratteri si fermava prima. È il primo modo in cui questo test ha sbagliato.
  ...
  for (const ag of ['agent41-realloc-scheduler', 'agent35-maker']) {

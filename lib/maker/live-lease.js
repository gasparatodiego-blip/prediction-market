...
// DATA_DIR, non `path.join(__dirname, '..', '..', 'data')`. Quest'ultimo funziona quando il modulo e'
// require()d da un agente pm2 e SBAGLIA quando Next lo impacchetta: dentro il bundle `__dirname` diventa
// .next/server/app/api/, quindi la route scriveva in .next/server/app/api/data/ — una cartella che
// nessun agente legge e che ogni build cancella. Misurato: il permesso risultava preso, il file non
// esisteva dove agent34 lo cerca, e il prezzo non passava mai a «book live».
// Il repo aveva gia' questo identico difetto e lo aveva gia' risolto (lib/safety/store.js): DATA_DIR
// risale al package.json piu' vicino, che e' la radice del progetto da entrambe le posizioni.
const { DATA_DIR } = require('../safety/store');

const STORE_FILE = path.join(DATA_DIR, 'maker-live-leases.json');
...
// Quindici. Il numero era otto — scelto stretto perche' un operatore ha un pannello aperto alla volta e
// non volevo che una funzione nuova potesse mangiare il budget del feed. Alzato su richiesta esplicita
// dell'operatore dopo la prima giornata d'uso: otto significava che scorrendo la lista Mercati e
// toccando card in sequenza i permessi piu' vecchi cadevano prima di scadere, e riaprire un mercato
// guardato un minuto prima ricominciava da Gamma.
//
// PERCHE' QUINDICI E' ANCORA SICURO, con i numeri misurati il 2026-08-01:
//   · il budget totale di agent34 e' 125 mercati; il board ne occupa 60, quindi restano ~65 slot liberi
//     e quindici permessi non arrivano mai a dover evincere un mercato del board;
//   · costo misurato ~29 KB e ~0,045% di CPU per mercato ⇒ quindici valgono ~0,44 MB e ~0,7% di CPU,
//     contro i 69 MB e 1,9% che agent34 usa gia';
//   · ogni permesso scade comunque in venti secondi, quindi quindici e' il picco di una raffica di
//     tocchi, non uno stato stabile.
// Oltre questo numero il permesso rinnovato meno di recente cede il posto: e' quello che l'operatore ha

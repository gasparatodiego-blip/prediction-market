...
// Below this much remaining life, no new order is placed on the market. Floored at MIN_SAFE_MINUTES.
const DEFAULT_MIN_MINUTES_TO_CLOSE = 5;
// Derived from the venue's own GTD floor, not chosen: ceil(120s / 0.9) = 134s ⇒ 3 minutes.
const MIN_SAFE_MINUTES = Math.ceil(MIN_EFFECTIVE_TTL_SEC / GTD_FRACTION / 60);

const ROOT = path.join(__dirname, '..', '..');
const BOARD_FILE = path.join(ROOT, 'data', 'liquidity-rewards.json');
...
function readMarketCloseMs(marketId, deps = {}) {
  ...
  const norm = deps.norm !== undefined ? deps.norm : readJson(deps.normFile || NORMALIZED_FILE);

...
console.log('\n── 6 · ISOLAMENTO: QUESTO GUARDIANO NON PUÒ PIAZZARE');
{
  ...
  })(path.join(__dirname, '..', '..', 'agents', 'agent42-guardian.js'), 0);
  ok(`nessun adapter di piazzamento nell'albero di agent42-guardian (${visti.size} moduli visitati)`,
    piazzamento.length === 0, piazzamento.join(' · '));

  const src = fs.readFileSync(path.join(__dirname, '..', '..', 'agents', 'agent42-guardian.js'), 'utf8');
...
(async () => {
  const A = require('../../agents/agent42-guardian');
  ...
  }
  {
    ...
    const r = await A.poll({
      ...baseDeps, baselineRaw: { baselineUsd: 1000 }, stato: null,
      buildCancelCredsProviders: async () => ({}),
    ...
    ok('perdita di $60 su 1000 → SCATTA', r.azione === 'scattato');
    ...
    ok('  attribuendolo a se stesso', fermato && fermato.by === 'agent42-guardian');

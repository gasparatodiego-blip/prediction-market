...
console.log('\n── 1 · VALUTAZIONE DEL CAPITALE');
{
  ...
  // ⚠ RISCRITTO IL 22 AGOSTO 2026, NON AMMORBIDITO (§5.2 p.54). Fin qui bastavano saldo e posizioni
  // leggibili perche' il totale fosse misurabile. Da oggi serve anche che le due fonti descrivano lo
  // STESSO ISTANTE, e la riconciliazione si dichiara: ometterla vale «non misurabile», mai «come
  // prima». L'aritmetica difesa qui — la somma e il dettaglio — non e' cambiata di un centesimo, e
  // questo blocco continua a difenderla; quello che cambia e' che ora la si chiede a una coppia di
  // letture co-temporali. Il caso senza riconciliazione ha il suo blocco, 1-bis.
  const posA = [
  ...
  const c = G.valutaCapitale({
    ...
    posizioni: posA,
    riconciliazione: { at: 60_000, precedente: { at: 0, saldoUsd: 588.264868, posizioni: posA } },
  });
...
}
{
  const c = G.valutaCapitale({ saldoUsd: 100, posizioniLeggibili: true, posizioni: [],
    riconciliazione: { at: 60_000, precedente: { at: 0, saldoUsd: 100, posizioni: [] } } });
  ok('nessuna posizione, snapshot leggibile → lo zero è REALE', c.leggibile === true && c.valorePosizioniUsd === 0 && c.totaleUsd === 100);
}
// ── 1-bis · LA RICONCILIAZIONE NON HA UN DIFETTO PERMISSIVO (§5.2 p.54) ───────────────────────────
{
  const c = G.valutaCapitale({ saldoUsd: 100, posizioniLeggibili: true, posizioni: [] });
  ok('ometterla ⇒ NON misurabile: una dep non cablata non torna al comportamento vecchio',
    c.leggibile === false && c.totaleUsd === null);
  const d = G.valutaCapitale({ saldoUsd: 100, posizioniLeggibili: true, posizioni: [], riconciliazione: 'non-richiesta' });
  ok('  e chi OSSERVA lo dichiara con \'non-richiesta\', che si trova con un grep',
    d.leggibile === true && d.totaleUsd === 100);
}
{
...
console.log('\n── 4 · BASELINE');
{
  const pB = [{ tokenId: 'a', size: 10, curPrice: 0.5 }];
  const cap = G.valutaCapitale({ saldoUsd: 500, posizioniLeggibili: true, posizioni: pB,
    riconciliazione: { at: 60_000, precedente: { at: 0, saldoUsd: 500, posizioni: pB } } });
...
console.log('\n── 5 · IL REFERTO');
{
  ...
  const pC = [{ tokenId: 'a', size: 100, curPrice: 0.4 }];
  const cap = G.valutaCapitale({ saldoUsd: 900, posizioniLeggibili: true, posizioni: pC,
    riconciliazione: { at: 60_000, precedente: { at: 0, saldoUsd: 900, posizioni: pC } } });
...
(async () => {
  ...
  const baseDeps = {
    ...
    // ⚠ §5.2 p.54 — LA LETTURA PRECEDENTE, dal 22 agosto 2026. Senza, ogni caso di `poll` cadrebbe
    // sul ramo «prima lettura ⇒ non misurabile» e proverebbe quello invece della decisione che vuole
    // provare. Qui e' lo stato di REGIME: cassa ferma, posizioni ferme ⇒ le due fonti concordano.
    // Il caso della prima lettura in assoluto ha il suo blocco dedicato piu' sotto.
  ...
  }
  {
    ...
    // ⚠ LE DUE LETTURE DEVONO VENIRE DA DUE OSSERVAZIONI DISTINTE, dal 13 agosto 2026 (§5.2 p.16).
    // `etaMs` diverso a parità di `now` significa «voce di cache diversa»: è così che il guardiano
    // distingue una seconda lettura da una copia della prima. Con `etaMs` uguale non scatterebbe, e
    // c'è un test apposta in persistenza-e-collasso.test.js.
    const primo = await A.poll({ ...deps60, saldo: { usd: 940, affidabile: true, etaMs: 1000 },
      stato: null, statoConferme: null });
    ...
    const r = await A.poll({ ...deps60, saldo: { usd: 940, affidabile: true, etaMs: 5000 },
      stato: null, statoConferme: primo.statoConferme });
    ok('perdita di $60 su 1000 → SCATTA alla SECONDA lettura DISTINTA', r.azione === 'scattato');

...
//    marketNotionalUsd, positionUsd, mid } ──
function evaluateMarketRails({ market, config }) {
  const trips = [];
  const R = config.rails;
  // FINE SCALA — la stessa unica definizione (lib/maker/end-of-scale.js) che usano il watcher reattivo
  // e il market maker a due lati. Qui arriva perché mancava proprio al motore: agent35 quotava dentro la
  // zona di risoluzione mentre gli altri due percorsi ne uscivano, ed è la forma consueta del difetto in
  // questo progetto — protezione presente su un percorso, assente sull'altro. `halt-market` significa
  // cancella-e-non-riquotare, che è l'unica direzione ammessa a fine scala (mai «riprezza più stretto»).
  // Un mid non leggibile NON fa scattare niente: `endOfScale:false` con `readable:false`, e la rotaia
  // `feed-stale` qui sopra ha già coperto il caso «non sappiamo che sta succedendo».
  {
    const eos = endOfScaleCheck(market.mid);
    if (eos.endOfScale) trips.push({ rail: 'end-of-scale', tripped: true, action: 'halt-market', detail: eos.reason });
  }

...
const base = (piazza) => ({
  ...
  // ── LA RICOSTRUZIONE DEL PIANO VA INIETTATA (13 agosto 2026) ──────────────────────────────────
  // Dal 13 agosto il mini-ciclo ricostruisce il piano anche quando le righe ancora spendibili scendono
  // sotto i posti di un giro, e questa fixture ne ha tre contro dodici posti: senza questa dep partiva
  // il pianificatore VERO, cioe' un processo figlio che legge il board vivo. Un test delle passate che
  // dipende dal board di oggi non prova piu' niente e cambia esito a ogni ora. Si restituiscono le
  // STESSE righe del piano salvato, cosi' la ricostruzione e' neutra e le passate restano osservabili.
  pianoLeggero: async () => ({ rows: PIANO.righe }),
  piazza,
});
...
(async () => {
  console.log('── 1 · QUANDO IL PRIMO GIRO NON COPRE TUTTO, SI PROVA IL RESTO');
  {
    ...
    // piano PIU' LARGO del giro, e la larghezza si DERIVA dal tetto invece di ricopiarla: la fixture
    // era di otto righe fisse, tarata su un tetto di 6, ed e' diventata rossa quando il tetto e' salito
    // a 12 — senza segnalare nessun difetto, perche' con dodici posti otto righe stanno in una passata
    // sola e la seconda passata non ha piu' motivo di esistere. Derivandola, il caso resta quello che
    // il test vuole provare a qualunque taratura.
    const quante = TRIG.MAX_MERCATI_PER_GIRO + 2;
    const largo = { ok: true, at: new Date().toISOString(),
      righe: Array.from({ length: quante }, (_, i) => riga(`0xa${i + 1}`, quante + 1 - i)) };
    const visti = [];
  ...
  console.log('\n── 5 · IL TETTO E QUELLO GIA ESISTENTE, E mai-primo NON E STATA TOCCATA');
  {
    ...
    // NON un valore: la proprieta'. Il tetto delle passate deve essere lo STESSO numero del raggio
    // d'azione del giro, e quel numero vive in `utilizzo-capitale` — due copie che divergono sono il
    // reperto D1. Asserire «6» qui rendeva il test rosso a ogni ritaratura senza segnalare un difetto.
    ok('  ed e lo stesso numero condiviso, non uno nuovo',

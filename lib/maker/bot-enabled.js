...
function registraMercatoAperto({ marketId, file = FILE, now = ora() } = {}) {
  ...
  // Il file è uno solo: per aggiungere un mercato al conteggio della rampa bisogna riscrivere anche
  ...
  // e questa scrittura non parte. Si perde un conteggio della rampa — che si recupera al giro dopo — e
  ...
  if (controllo.at !== s.at || controllo.enabled !== s.enabled) {
    return { ok: false, motivo: 'l\'interruttore è cambiato mentre si contava la rampa: non lo si sovrascrive' };
  }
  try { atomicWriteJson(file, stato, { pretty: true }); }

...
function planExit({ entryPrice, scoringMid, tick, bandRadiusCents = null, profitPct = EXIT_PROFIT_PCT,
  ...
  //
  // ⚠ IL PAVIMENTO SI ARROTONDA IN SU SULLA GRIGLIA, e la direzione e' obbligata. `pavimentoConcesso`
  // e' una frazione del carico (il 5%, R7) e non cade quasi mai su un tick: 0,68 × 0,95 = 0,646, che
  // NON e' un prezzo esprimibile su una griglia da 1¢. Arrotondare in GIU' concederebbe piu' perdita
  // di quella che il gradino consente; in SU ne concede di meno. Si arrotonda in su.
  //
  ...
  const pavimentoGriglia = fuoriBandaAmmessa ? snapTo(pavimento, tick, 'up') : null;
  ...
  return {
    ...
    reason: peggiorativa
      ...
      : reason,
    peggiorativa,
    // ⚠ IL PAVIMENTO CHE ESCE E' QUELLO EFFETTIVAMENTE USATO. Sul ramo fuori banda e' arrotondato in
    // su sulla griglia, e deve uscire cosi': a valle `auto-close.inseguiIlBid` lo riusa come
    // `Math.max`, e restituire qui il numero non arrotondato lo riporterebbe fuori griglia — cioe'
    // produrrebbe di nuovo un prezzo che il guard condiviso rifiuta come `OFF_TICK`. Il non
    // arrotondato resta a verbale accanto, perche' l'audit dei giorni scorsi lo contiene.
    pavimento: fuoriBandaScelta ? pavimentoGriglia : pavimento,

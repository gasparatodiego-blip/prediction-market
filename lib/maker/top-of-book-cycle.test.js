...
(async () => {
  ...
  console.log('\n══ IL RIPIEGO · siamo gli unici sul lato');
  {
    ...
    ok('nessun altro ⇒ BORDO ESTERNO della banda, non l offset', h.placed[0].priceCents === 47.8, `${h.placed[0].priceCents}c`);
  ...
  console.log('\n── il ripiego si scioglie DA SOLO quando ricompare qualcuno');
  {
    ...
    await h.run();
    ok('si parte soli, al bordo esterno della banda (47,8¢)', h.placed[0].priceCents === 47.8, `${h.placed[0].priceCents}c`);

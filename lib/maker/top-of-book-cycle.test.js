...
(async () => {
  ...
  console.log('\n══ LA BANDA E IL LIMITE SUPERIORE');
  {
    ...
    const r = await h.run();
    ok('ci si ferma al bordo della banda', h.placed[0].priceCents === 47.8, `${h.placed[0].priceCents}c`);
    ok('  modo «band-clamped»', tgt(r, 'yes').mode === 'band-clamped');
    ok('  e resta dentro il raggio 2.25¢', Math.abs(50 - h.placed[0].priceCents) <= 2.25);
    ok('  la conseguenza e DICHIARATA: siamo in cima', tgt(r, 'yes').onTop === true,
      'la banda vince, perche fuori banda non si matura nulla');
    ok('  e viaggia fino all azione', yesAct(r).placement.onTop === true);
  }
  ...
  console.log('\n══ IL RIPIEGO · siamo gli unici sul lato');
  {
    ...
    const r = await h.run();
    ok('nessun altro ⇒ offset configurato', h.placed[0].priceCents === 49, `${h.placed[0].priceCents}c`);
    ok('  modo «fallback-alone»', tgt(r, 'yes').mode === 'fallback-alone', tgt(r, 'yes').mode);
  ...
  console.log('\n── il ripiego si scioglie DA SOLO quando ricompare qualcuno');
  {
    ...
    await h.run();
    ok('si parte soli, all offset di ripiego (48¢)', h.placed[0].priceCents === 48, `${h.placed[0].priceCents}c`);

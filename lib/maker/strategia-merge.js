...
/**
 * ═══ IL TETTO DELLA COPPIA AI LIVELLI 1-2: 101¢ — decisione dell'operatore, 15 agosto 2026 ═══════
 *
 * Era **99¢**, cioe' «completa la coppia solo se ci guadagni». Adesso e' **101¢**, cioe' «completa la
 * coppia anche se ti costa fino a 1¢ per share, pur di non restare direzionale». E' un cambio di
 * DOMANDA, non di numero: il 99¢ chiedeva se completare fosse *profittevole*, il 101¢ chiede se sia
 * *accettabile* — la stessa domanda che il Livello 3 poneva gia' col suo tetto, ora allineata.
 *
 * ⚠ LA MISURA CHE LO SOSTIENE E' GIA' IN CASA (§5-bis p.162, 62.525 fill su 65 wallet maker veri):
 * fra chi completa la coppia il costo mediano e' **100,00¢** e **solo il 41,2% chiude entro 99¢**.
 * Un tetto a 99¢ rifiuta quindi quasi sei coppie su dieci di quelle che i maker che guadagnano
 * chiudono davvero, e ogni rifiuto lascia il bot direzionale su una gamba nuda — la cui perdita
 * peggiore vale 100¢/share, non 1¢.
 *
 * ⚠ IL COSTO MASSIMO, IN DOLLARI E NON IN PRINCIPIO. 1¢ per share sulla posizione piu' grande che
 * questa configurazione possa aprire (tetto per mercato $61,25 ⇒ ~62,5 share) vale **$0,63**.
 *
 * Il margine e' DERIVATO dal tetto e non scritto due volte: `tetto = 100 − margine`, quindi 101¢ ⇒
 * margine −1¢. Il segno negativo e' letterale e voluto: si accetta di pagare la coppia sopra la pari.
 */
const MERGE_TETTO_COPPIA_CENTS = 101;
/** Quanto si accetta di pagare SOPRA il dollaro (negativo = sopra la pari). Derivato, mai riscritto. */
const MERGE_MIN_MARGIN_CENTS = 100 - MERGE_TETTO_COPPIA_CENTS;
/**
 * Quanto si aspetta da maker (Livello 2) prima di ripiegare sull'uscita classica (Livello 3, che e'
 * TAKER-first: `chiusura-rapida` cammina gli ask e attraversa lo spread fin dove il book copre).
 *
 * ⚠ 60 → 30 MINUTI, decisione dell'operatore del 15 agosto 2026: «se dopo 30 min non arriva, scala al
 * taker». La misura di §5-bis p.162 dice che l'attesa da maker mediana di chi completa e' **36,1
 * minuti** contro **1,6** del taker, e che attraversare costa **1¢ di coppia** facendone risparmiare
 * **34,5**. A 30 minuti si e' gia' oltre la meta' della distribuzione delle coppie che si chiudono da
 * sole: continuare ad aspettare paga sempre meno e la scopertura continua a costare.
 */
const MERGE_WAIT_TIMEOUT_MIN = 30;
/**
...
function tettoSecondoLato(prezzoRiempito, margineCents = MERGE_MIN_MARGIN_CENTS) {
  ...
  // ⚠ CLAMP A 100¢, e serve da quando il margine puo' essere NEGATIVO (tetto coppia 101¢): su un carico
  // sotto il centesimo `100 − carico − (−1)` supererebbe 100¢, cioe' un prezzo fuori dai limiti del
  // libro. Non e' raggiungibile oggi (`end-of-scale` vieta sotto 3¢) ed e' proprio per questo che il
  // clamp costa zero: previene un prezzo invalido senza cambiare nessun caso reale.
  const tettoC = Math.min(100, 100 - c(prezzoRiempito) - margineCents);
...
function decidiLivello({
  ...
  // ══ LA COPPIA COMPLETA SI FONDE PRIMA DI QUALUNQUE GUARDIA SUL PREZZO — 18 agosto 2026 ═════════
  //
  // REGOLA DELL'OPERATORE (R8): «coppia completa: merge subito, sempre, senza limiti di prezzo. Il
  // tetto di 101¢ vale solo per l'acquisto della gamba mancante.»
  //
  // ⚠ IL DIFETTO CHE QUESTO ORDINE CHIUDE. Fino a oggi il conto di `manca` stava DOPO due guardie che
  // parlano entrambe del prezzo a cui si comprerebbe il secondo lato — carico non leggibile, e tetto
  // che non lascia spazio. Su una coppia GIA' completa non c'e' nessun secondo lato da comprare:
  // quelle due guardie rispondevano a una domanda che non era stata fatta, e la loro risposta era
  // `auto-close`, cioe' «vendi una gamba sul libro» al posto di «incassa $1/share senza slippage».
  // Una coppia completa vale $1/share alla risoluzione QUALUNQUE sia stato il carico: il carico e' una
  // spesa gia' fatta, e non entra in nessun modo nella decisione di fondere.
  //
  // ⚠ NON ALLARGA NIENTE, ED E' MONOTONO. Il ramo aggiunto e' raggiungibile solo con `manca <= 0`,
  // cioe' esattamente il caso in cui il vecchio codice — quando le due guardie passavano — rispondeva
  // gia' `merge`. Nessuna configurazione che prima comprava ora fonde: si tolgono due rifiuti, non si
  // aggiunge un acquisto. Il tetto della coppia resta identico su tutti i percorsi che COMPRANO.
  const manca = +(sizePosseduta - (fin(sizeAltroLato) ? sizeAltroLato : 0)).toFixed(6);
  base.numeri.mancaAllaCoppia = manca;
  if (manca <= 0) {
    // Il tetto si calcola se si puo', e viaggia a verbale, ma NON e' una condizione: `null` qui vuol
    // dire «non si sarebbe potuto comprare», ed e' irrilevante per chi non deve comprare niente.
    const tettoInformativo = tettoSecondoLato(prezzoCarico, margineCents);
    base.numeri.tettoCents = tettoInformativo != null ? c(tettoInformativo) : null;
    return { livello: 1, azione: 'merge', motivo: `la coppia e' gia' completa per ${sizePosseduta} share:`
      + ' non c e niente da comprare, resta solo da fondere'
      + (fin(prezzoCarico) ? '' : ' (carico non leggibile: non serve, una coppia completa vale $1/share comunque)'),
      tetto: tettoInformativo, size: sizePosseduta, prezzo: null, numeri: base.numeri };
  }

  // Da qui in poi si COMPRA, e chi compra ha bisogno del carico e del tetto.
  if (!fin(prezzoCarico)) {

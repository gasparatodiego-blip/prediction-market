...
//
// ═══ PERCHE' IL MODULO ESISTE LO STESSO, E PERCHE' NASCE SPENTO ═══════════════════════════════════
...
//
// Comprare il secondo lato senza poter fondere quindi non e' la strategia approvata: e' una strategia
// diversa, che immobilizza capitale nuovo per un profitto differito. La decisione sta all'operatore,
// non a questo file. Percio' `MERGE_STRATEGY_ENABLED` e' false: la meccanica e' scritta, provata e
// pronta, e non muove un dollaro finche' qualcuno non decide che il profilo va bene — o finche' non
// esiste una via per il merge.
//
...
/**
 * L'interruttore. FALSO finche' il merge on-chain non e' eseguibile: senza merge il Livello 1 e il
 * Livello 2 comprano capitale che non si potra' liberare prima della risoluzione, che e' un'operazione
 * diversa da quella per cui i due livelli esistono. Vedi il blocco qui sopra.
 */
const MERGE_STRATEGY_ENABLED = false;
...
function quantoAlVolo(asks, tetto, sizeVoluta) {
  ...
  if (!Array.isArray(asks) || !fin(tetto) || !fin(sizeVoluta) || sizeVoluta <= 0) {
    return { size: 0, costoMedio: null, prezzoPeggiore: null, livelli: 0 };
  }
...
function decidiLivello({
  ...
  return {
    ...
    motivo: `l ask di ${altro.toUpperCase()} e' sopra il tetto di ${c(tetto).toFixed(1)}¢: ci si mette da MAKER`

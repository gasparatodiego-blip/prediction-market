...
ok('DIVERGENZA ②, i 631 `manual-order-cap`: a mid estremo la gamba cara sta SEMPRE nel tetto per ordine', () => {
  // ⚠ QUESTA ASSERZIONE HA CAMBIATO SEGNO IL 15 AGOSTO 2026, e il motivo e' che la divergenza e'
  // stata chiusa alla FONTE. Fino a ieri il tetto per ordine era «meta' del tetto per mercato», che e'
  // la gamba giusta solo a mid 0,49: a mid 0,8675 (il mid vero di Vindman) la gamba cara sfondava, e
  // il meglio che `coerenza-soglie` potesse fare era RIDURRE il capitale della riga. Adesso il tetto
  // per ordine copre la gamba piu' cara per costruzione (`concentration.PREZZO_MASSIMO_QUOTABILE`),
  // quindi la riga passa INTERA e non c'e' piu' niente da adattare.
  //
  // Quello che va difeso non e' «viene adattata» — era il sintomo — ma l'invariante vera: nessuna
  // riga ammessa produce una gamba oltre il tetto per ordine. Si prova su tutta la fascia quotabile.
  let sfondamenti = 0; let adattate = 0; let provate = 0;
  for (let mid = 0.05; mid <= 0.95001; mid += 0.025) {
    const m = +mid.toFixed(3);
    const v = COER.verificaRiga(riga({ mid: m }), soglieDi(riga({ mid: m })));
    if (!v.ok) continue;
    provate += 1;
    if (v.adattata) adattate += 1;
    const gambaCara = (v.capitale / 0.98) * Math.max(m, 1 - m);
    if (gambaCara > tettoO + 0.01) sfondamenti += 1;
  }
  assert.ok(provate > 20, `provate ${provate} righe`);
  assert.strictEqual(sfondamenti, 0, `${sfondamenti} righe con la gamba cara oltre il tetto per ordine $${tettoO}`);
  // E il capitale puo' solo SCENDERE, mai salire: `verificaRiga` non deve poter allargare un limite.
  const alto = COER.verificaRiga(riga({ mid: 0.8675 }), soglieDi(riga({ mid: 0.8675 })));
  assert.ok(alto.capitale <= tettoM + 1e-9, 'il capitale di una riga non puo\' superare il tetto per mercato');
  console.log(`      (${adattate} righe su ${provate} hanno ancora bisogno di essere ridotte; era la quasi totalita')`);
});
...
ok('adattaAlleSoglie è chiamata sia sul piano salvato sia sulla ricostruzione', () => {
  ...
  //
  // ⚠ RISCRITTO IL 17 AGOSTO 2026, e la ragione e' §5.3 («un test che fotografa il codice invece della
  // proprieta'»). La proprieta' — «ogni fonte di righe passa dall'adattamento alle soglie» — e' INTATTA;
  // e' cambiato dove sta la chiamata: le due fonti ora passano da `righeAmmesse`, che fa la restrizione
  // alla SELEZIONE e POI l'adattamento. Le due cose sono state messe nella stessa funzione proprio
  // perche' due filtri che devono valere su entrambe le fonti, tenuti separati, sono due occasioni di
  // dimenticarne una — che e' l'incidente che questo blocco §8 esiste per ricordare.
  // Quindi ora si asserisce la CATENA: entrambe le fonti → `righeAmmesse` → `adattaAlleSoglie`.
  assert.ok(/righeCandidate = righeAmmesse\(piano\.righe/.test(src), 'il piano salvato deve passare da righeAmmesse');
  assert.ok(/const fresche = righeAmmesse\(righeFresche/.test(src), 'la ricostruzione deve passare da righeAmmesse');
  const usi = (src.match(/adattaAlleSoglie\(/g) || []).length;
  assert.ok(usi === 1, `adattaAlleSoglie deve essere chiamata da UN SOLO punto — righeAmmesse (trovate ${usi})`);
  assert.ok(/return adattaAlleSoglie\(dopoSelezione, dove\)/.test(src), 'e righeAmmesse deve chiudere con l\'adattamento');

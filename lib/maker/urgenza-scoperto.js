...
function selfcheck() {
  ...
  // La concessione richiesta è sempre il massimo consentito, letto dalla costante: così l'asserzione
  // resta vera a 1 tick come a 2, e continua a provare CHE COSA limita, che è la proprietà.
  const p1 = pavimentoConcesso({ carico: 0.43, tick: 0.01, concessioneTick: CONCESSIONE_TICK_MAX });
  A('carico 0,43 tick 0,01: a limitare è il tick, non la frazione',
    Math.abs(p1.pavimento - (0.43 - CONCESSIONE_TICK_MAX * 0.01)) < 1e-9 && p1.limitatoDa === 'tick');
  A('  e la concessione in tick è esattamente quella massima', p1.tickConcessi === CONCESSIONE_TICK_MAX);

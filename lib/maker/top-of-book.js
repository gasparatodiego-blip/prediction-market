'use strict';
const { distanzaObiettivoCents, applicaObiettivo, bordiConMargine } = require('./distanza-obiettivo');
...
function planBehindBest({ bestOther, tick, scoringMid, bandRadiusCents, fallbackOffsetCents,
  ...
  // ── LA MANOPOLA DELLA POSIZIONE (§5-bis p.158). `undefined` ⇒ si legge dall'ambiente; `null` ⇒
  // spenta. Sta qui e non in ogni chiamante perché la distanza-obiettivo è UNA decisione, e due
  // punti che la leggono sono due punti che possono divergere.
  distanzaObiettivoFrazione = undefined, env = undefined } = {}) {
  ...
  const finalize = (raw, mode, why, bordoAmmesso) => {
    ...
    // ── IL BORDO A CUI CI SI AGGANCIA HA UN MARGINE (15 agosto 2026) ───────────────────────────
    // Il bordo nudo dista dal limite premiante meno di un tick, quindi un ordine posato lì esce dalla
    // banda al primo movimento del mid e rientra al successivo: è l'oscillazione che
    // `distanza-obiettivo.bordiConMargine` esiste per spegnere (e al bordo nudo il punteggio è quasi
    // zero per costruzione, §5-bis p.152).
    //
    // ⚠ SOLO NEL RAMO «SOLI SUL LATO», e non è una limitazione da correggere: stringere il bordo
    // ALZA il prezzo verso il mid, cioè verso la cima del libro. Dove c'è un concorrente quello è
    // esattamente il movimento che «mai primo sul libro» vieta, e lì il margine lo applica — con il
    // proprio `Math.min` sul prezzo di partenza — `applicaObiettivo`. Qui davanti non c'è nessuno,
    // quindi non si può finire in cima a niente.
    if (!fin(bestOther)) {
      const bm = bordiConMargine({ bandLo: b.lo, bandHi: b.hi, tick, maxSpreadCents: bandRadiusCents, env: env || process.env });
      if (bm.applicato) {
        if (price < bm.lo - EPS) { price = bm.lo; }
        if (price > bm.hi + EPS) { price = bm.hi; }
      }
    }
    if (!(price > 0 && price < 1)) {
      return out({ reason: `il prezzo calcolato (${p2c(price)}¢) cade fuori dai limiti del libro` });
    }
    // ── LA DISTANZA-OBIETTIVO, applicata per ULTIMA e solo verso l'esterno ──────────────────────
    // Può solo allontanare dal mid, quindi non può mettere l'ordine davanti a nessuno; e non può
    // uscire dalla banda, perché si ferma a `b.lo`. Entrambe le proprietà vivono in
    // `applicaObiettivo`, non in chi la chiama: il paletto non dipende da chi usa la manopola.
    const obi = distanzaObiettivoCents({ maxSpreadCents: bandRadiusCents,
      frazione: distanzaObiettivoFrazione, env: env || process.env });
    let manopola = null;
    if (obi.distanzaC != null) {
      const app = applicaObiettivo({ prezzo: price, scoringMid, bandLo: b.lo, bandHi: b.hi,
        distanzaC: obi.distanzaC, tick, maxSpreadCents: bandRadiusCents, env: env || process.env });
      if (app.spostato) { price = app.prezzo; }
      manopola = { frazione: obi.frazione, distanzaRichiestaC: obi.distanzaC,
        spostato: app.spostato, alBordo: app.alBordo, margine: app.margine, motivo: app.motivo };
    }
    ...
    return {
      ...
      onTop, offsetCents, bandLo: b.lo, bandHi: b.hi, distanzaObiettivo: manopola,
      reason: clamped
  ...
  if (!fin(bestOther)) {
    ...
    //
    // ═══ E IL BORDO HA UN MARGINE (15 agosto 2026) ═════════════════════════════════════════════════
    // Il bordo NUDO dista dal limite premiante meno di un tick, quindi un ordine posato lì esce dalla
    // banda al primo movimento del mid e ci rientra al successivo: oscilla, e ogni oscillazione è un
    // cancel+place. Ci si ferma perciò `MAKER_DISTANZA_MARGINE_BORDO_TICK` tick prima
    // (`distanza-obiettivo.bordiConMargine`, la STESSA funzione che usa il rientro di `auto-reprice`,
    // o apertura e rientro sarebbero due decisioni diverse sullo stesso ordine). Costa un tick di
    // distanza dal mid e in cambio moltiplica il punteggio: `S = ((v−s)/v)²` vale 0,0123 al bordo nudo
    // e 0,1111 un tick più dentro, su banda 4,5¢. Qui non c'è nessuno davanti, quindi avvicinarsi di
    // un tick non può metterci in cima a nulla.
    void fallbackOffsetCents;   // non più usato qui: il bordo non dipende da un offset configurato
    const bmSoli = bordiConMargine({ bandLo: b.lo, bandHi: b.hi, tick, maxSpreadCents: bandRadiusCents, env: env || process.env });
    const bersaglio = bmSoli.applicato ? bmSoli.lo : b.lo;
    return finalize(bersaglio, 'fallback-alone-bordo-esterno',
      `siamo gli unici su questo lato: nessuno dietro cui accodarsi, quindi si va al BORDO ESTERNO della`
      + ` banda premiante (${p2c(bersaglio)}¢ su [${p2c(b.lo)}¢–${p2c(b.hi)}¢], mid ${p2c(scoringMid)}¢)`
      + (bmSoli.applicato ? ` — ${bmSoli.margineTick} tick DENTRO il bordo, per non oscillare dentro/fuori` : '')
      + ` — il prezzo più lontano dal mid che matura ancora reward, così il fill è improbabile e il premio no.`

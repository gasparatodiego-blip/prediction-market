 ...
 *
 * Tre esiti, ciascuno con il suo nome, perché sono tre situazioni diverse che l'operatore deve poter
 * distinguere guardando il pannello:
 *   behind-best    ci si è messi un tick dietro il migliore altrui. È il caso normale.
 *   band-clamped   un tick dietro cadeva fuori banda: ci si è fermati al bordo. Può implicare `onTop`.
 *   fallback-alone siamo gli unici su questo lato: si ripiega sull'offset configurato, finché non
 *                  ricompare qualcun altro. Non è un errore ed è reversibile da sé.
 *
...
function planBehindBest({ bestOther, tick, scoringMid, bandRadiusCents, fallbackOffsetCents } = {}) {
  const out = (extra) => ({ ok: false, price: null, priceCents: null, mode: null, onTop: null, offsetCents: null, reason: null, ...extra });
  ...
  const finalize = (raw, mode, why) => {
    let price = snap(raw, tick);
    if (!fin(price)) return out({ reason: 'prezzo non calcolabile' });
    let clamped = false;
    ...
    return {
      ok: true, price, priceCents: p2c(price),
      mode: clamped ? 'band-clamped' : mode,
      onTop, offsetCents, bandLo: b.lo, bandHi: b.hi,
      reason: clamped
        ? `un tick dietro il migliore altrui cadrebbe fuori banda: ci si ferma al bordo premiante ${p2c(price)}¢ (${offsetCents}¢ dal mid)`
          + (onTop ? ' — e questo ci mette IN CIMA al book, che è il prezzo da pagare per restare premianti'
            : '')
        : why,
    };
  };

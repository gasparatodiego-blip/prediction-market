...
function othersLadder({ levels, ownOrders = [], tick } = {}) {
  ...
  if (!Array.isArray(levels)) {
    return { readable: false, levels: null, alone: false, reason: 'il feed non pubblica i livelli di questo book' };
  }
  if (!fin(tick) || tick <= 0) {
...
function planBehindBest({ bestOther, tick, scoringMid, bandRadiusCents, fallbackOffsetCents,
  ...
  if (!fin(bestOther)) {
    ...
    //
    // E QUI LA PROTEZIONE DI PROFONDITA' NON SI APPLICA, per la stessa ragione: non c'e' nessuno
    // davanti, quindi la profondita' davanti e' zero a QUALUNQUE prezzo e la soglia non sarebbe mai
    // raggiunta. Applicarla vorrebbe dire arretrare fino al bordo della banda ogni volta che il book
    // e' vuoto — cioe' proprio dove la liquidita' serve di piu'.

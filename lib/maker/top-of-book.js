'use strict';
const { distanzaObiettivoCents, applicaObiettivo } = require('./distanza-obiettivo');
...
function planBehindBest({ bestOther, tick, scoringMid, bandRadiusCents, fallbackOffsetCents,
  ...
  const finalize = (raw, mode, why, bordoAmmesso) => {
    ...
    if (obi.distanzaC != null) {
      const app = applicaObiettivo({ prezzo: price, scoringMid, bandLo: b.lo, distanzaC: obi.distanzaC, tick });
      if (app.spostato) { price = app.prezzo; }
      manopola = { frazione: obi.frazione, distanzaRichiestaC: obi.distanzaC,
        spostato: app.spostato, alBordo: app.alBordo, motivo: app.motivo };
    }

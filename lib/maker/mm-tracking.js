...
async function runTrackingCycle(deps = {}) {
  ...
  for (const marketId of cfg.marketIds) {
    ...
    // ── L'OR FRA I DUE SEGNALI ────────────────────────────────────────────────────────────────────
    // L'erosione promuove ad azione UN SOLO verdetto: `in-band`. E' preciso, non prudenza generica —
    // «in banda» e' letteralmente l'unico caso in cui il motore dice «va tutto bene, non ti muovere», ed
    // e' quindi l'unico su cui un secondo segnale possa avere qualcosa da aggiungere. Tutti gli altri
    // verdetti negativi dicono qualcosa che l'erosione non smentisce e non puo' scavalcare:
    //   filled          il lato e' eseguito — non c'e' niente da riposizionare
    //   side-disabled   quel lato non e' governato da questo motore
    //   backoff         il venue ha appena rifiutato: insistere produrrebbe lo stesso rifiuto
    //   unplaceable     l'offset porta il lato fuori dal libro
    //   band-unknown    senza banda non si puo' affermare nulla, e il punto 7 chiede la banda
    //   offset-outside-band  l'offset e' piu' largo del raggio: il prezzo nuovo NON sarebbe premiante
    //
    // PUNTO 7, ESPLICITO. Il riposizionamento per erosione serve a ridurre il rischio di fill avverso,
    // non a uscire dai reward: se il prezzo di destinazione non e' dentro banda, l'erosione non agisce.
    // La condizione e' scritta qui sopra il piazzamento e non dedotta da `in-band`, che riguarda il
    // prezzo VECCHIO — sono due prezzi diversi e vanno controllati tutti e due.
    for (const side of ['yes', 'no']) {
      const e = ero[side];
      if (!e || !e.applies || !e.erosion) continue;
      const d = decs[side];
      if (d.act) { d.byErosion = true; continue; }   // gia' in movimento per il mid: si annota e basta
      if (d.gate !== 'in-band') continue;

      // DOVE si va. Non all'offset di sempre — con il mid fermo sarebbe lo stesso prezzo, e il motore
      // salta i riprezzi a prezzo invariato: il trigger non muoverebbe niente. Ci si RITIRA al bordo
      // premiante, che e' il punto piu' lontano dal mid in cui l'ordine matura ancora.
      const retreat = erosionRetreat({ offsetCents: conf.offsetCents, bandRadiusCents: bandR, tick: rules.tick });
      if (!retreat.ok) {
        d.erosionHeld = 'no-retreat';
        d.reason += ` · erosione confermata ma non si agisce: ${retreat.reason}`;
        continue;
      }
      const retPlan = planQuotes({ mid: rules.mid, offsetCents: retreat.offsetCents, tick: rules.tick, bandRadiusCents: bandR });
      // PUNTO 7, VERIFICATO SUL PREZZO NUOVO E NON DEDOTTO DA QUELLO VECCHIO. `in-band` qui sopra
      // riguarda il prezzo a cui l'ordine sta ADESSO; questo riguarda quello a cui andrebbe. Sono due
      // prezzi diversi e il secondo e' l'unico che conta per sapere se continuera' a maturare.
      if (!retPlan.ok || !retPlan[side].placeable || retPlan[side].inBand !== true) {
        d.erosionHeld = 'target-out-of-band';
        d.reason += ` · erosione confermata ma NON si agisce: a ${retreat.offsetCents}¢ dal mid il lato non sarebbe piu premiante, e muoversi per ridurre il rischio di fill non vale uscire dai reward`;
        continue;
      }
      decs[side] = {
        act: true, gate: null, trigger: 'erosion', byErosion: true,
        inBand: d.inBand, distanceCents: d.distanceCents,
        planOverride: retPlan[side],
        reason: `EROSIONE DEL BOOK: ${e.reason} — ${retreat.reason}`,
      };
    }
    ...
    for (const side of sidesToDo) {
      const s = st.sides[side];
      // Il bersaglio e' quello del piano, TRANNE quando a muovere e' stata l'erosione: in quel caso e'
      // il prezzo di arretramento al bordo premiante, gia' verificato dentro banda.
      const target = decs[side].planOverride || plan[side];
      ...
      const effOffset = decs[side].trigger === 'erosion' && fin(target.referenceMid)
        ? +Math.abs(p2c(target.referenceMid) - p2c(target.price)).toFixed(3)
        : conf.offsetCents;

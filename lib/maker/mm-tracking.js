  ...
  for (const marketId of cfg.marketIds) {
    ...
    const decidiSulBook = (side, s, t) => {
      ...
      return { act: false, gate: 'in-band', inBand, distanceCents,
        reason: `dentro banda e gia al posto giusto: ${mv.reason}. ${String(t.mode || '').startsWith('fallback-alone') ? 'Siamo gli unici su questo lato: al bordo esterno della banda.' : `Il migliore altrui e a ${t.bestOther != null ? +(t.bestOther * 100).toFixed(2) : '—'}¢.`}` };
    };

...
module.exports = { TRACKING_SOURCE, planQuotes, decideRetrack, snap, bookOf, quoteIsValid };
...
async function runTrackingCycle(deps = {}) {
  ...
  for (const marketId of cfg.marketIds) {
    ...
    const maxAge = Number.isFinite(tuning.maxMidAgeSec) ? tuning.maxMidAgeSec : 30;
    if (Number.isFinite(rules.midAgeSec) && rules.midAgeSec > maxAge) {
      m.gate = 'mid-stale';
      m.reason = `il mid del book live e vecchio di ${rules.midAgeSec}s, oltre il massimo di ${maxAge}s`;
      markets.push(m); continue;
    }

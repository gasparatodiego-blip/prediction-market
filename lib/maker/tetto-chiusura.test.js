...
ok('il tetto assoluto non supera mai il saldo: un mercato non è una frazione irragionevole', () => {
  ...
  // Portando il tetto per mercato da $32,67 a $61,25 il tetto ASSOLUTO di chiusura — che ne e'
  // derivato, `capOrdinario x (1 + 1/costoCoppia)` — passa da $65,97 a $123,75, cioe' dal 9,9% al
  // 18,6% del capitale. La soglia dell'11% fu tarata su quel 9,9% (§5-bis p.133).
  // NON e' stata alzata per far passare il test: alzarla sarebbe trasformare una leva sul tetto in un
  // allentamento silenzioso di un limite di concentrazione, che e' esattamente cio' che questa suite
  // esiste per impedire. La decisione — accettare il 18,6%, ritarare la soglia, o disaccoppiare il
  // tetto di chiusura da quello ordinario — e' dell'operatore.
  // ⚠ Va detto anche cosa NON e': l'esenzione vale SOLO per chi RIDUCE, con la prova rifatta
  // sull'ordine esatto (SELL entro il posseduto, BUY entro `manca`), quindi quel 18,6% e' capitale
  // impegnato per COMPLETARE una coppia — esposizione direzionale ~zero — non capitale a rischio.

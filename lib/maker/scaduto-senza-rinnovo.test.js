...
(async () => {
  console.log('\n══ 1 · LA MORTE PER SCADENZA PRODUCE UN EVENTO, CON IL MOTIVO DEL MANCATO RINNOVO');
  {
    ...
    // ── L'ANOMALIA AGGREGATA DEL CICLO (23 agosto 2026) ─────────────────────────────────────────
    // Una riga per ordine c'era già, e non ha avvisato nessuno: il 23 agosto 63 ordini sono morti in
    // sette ore, $862,58 fuori dal libro, e il fatto è stato visto solo contandoli a posteriori con
    // un grep. Il degrado non era silenzioso per mancanza di righe — era silenzioso perché nessuna
    // riga diceva QUANTO. Questa dice il numero e il nozionale, ed è marcata `anomalia`.
    const agg = righe.filter((x) => x.outcome === 'anomalia-scadenze-senza-rinnovo');
    ok('il ciclo dichiara l ANOMALIA aggregata', agg.length === 1, `${agg.length}`);
    ok('  marcata come anomalia, così un lettore la può filtrare', agg[0] && agg[0].anomalia === true);
    ok('  col NUMERO di ordini morti', agg[0] && agg[0].observed.ordini === 1, String(agg[0] && agg[0].observed.ordini));
    ok('  col NOZIONALE uscito dal libro', agg[0] && agg[0].observed.nozionaleUsd === 19.47, String(agg[0] && agg[0].observed.nozionaleUsd));
    ok('  e la scomposizione per gate, che è la cosa da guardare',
      agg[0] && agg[0].observed.perGate && agg[0].observed.perGate['refresh-invalid'] === 1,
      JSON.stringify(agg[0] && agg[0].observed.perGate));
    ok('  con gli orderId, per poter risalire senza grep', agg[0] && agg[0].observed.orderIds[0] === '0xb99f5566');
    ok('  e a parole, col numero dentro la frase', agg[0] && /1 ordine morto per GTD senza rinnovo/.test(agg[0].reason));
    ...
    // Un avviso che compare anche nei giri sani non è un avviso: si scrive SOLO quando qualcuno muore.
    ok('  né una seconda anomalia aggregata: un giro senza morti non scrive niente',
      righe.filter((x) => x.outcome === 'anomalia-scadenze-senza-rinnovo').length === 1);

...
(async () => {
  ...
  console.log('\n── secondo giro col mid fermo: NON tocca nulla');
  {
    ...
    ok('  e il motivo e la soglia', r.markets[0].gate === 'below-threshold', String(r.markets[0].gate));
  }

  console.log('\n── il mid si muove di 1¢: ENTRAMBI i lati si spostano');
  {
    const w = world();
    await T.runTrackingCycle(w.deps);
    w.orders = [{ orderId: 'ord-yes-1', tokenId: 'tok-yes', sizeMatched: 0 }, { orderId: 'ord-no-2', tokenId: 'tok-no', sizeMatched: 0 }];
    ...
    ok('due nuovi ordini', nuovi.length === 2, `${nuovi.length}`);
    ok('  YES passa da 7¢ a 8¢', nuovi.some((p) => p.book === 'yes' && c(p.price) === 8), nuovi.filter((p) => p.book === 'yes').map((p) => c(p.price) + '¢').join());
    ok('  NO passa da 87¢ a 86¢ (= vendere YES a 14¢)', nuovi.some((p) => p.book === 'no' && c(p.price) === 86));
    ok('due cancellazioni, una per lato', w.cancelled.length === 2, `${w.cancelled.length}`);
    ok('  e cancella proprio gli ordini vecchi', w.cancelled.map((x) => x.orderId).sort().join() === 'ord-no-2,ord-yes-1');
    ok('il riferimento si sposta al mid nuovo', r.markets[0].referenceMid === 0.11);
    ok('il contatore di reprice sale', r.markets[0].repriceCount === 1, String(r.markets[0].repriceCount));
    const act = r.actions.find((a) => a.action === 'place' && a.book === 'yes');
    ok('l azione registra da quale mid a quale mid', act.fromMid === 10 && act.toMid === 11, `${act.fromMid}¢ → ${act.toMid}¢`);
    ok('  e di quanto si e mosso', act.movedCents === 1, `${act.movedCents}¢`);
  }
  ...
  console.log('\n── un lato FILLATO: quel lato si ferma, l altro continua');
  {
    ...
    ok('l altro lato continua normalmente', nuovi.some((p) => p.book === 'no' && c(p.price) === 86));
    ...
    ok('  mentre l altro continua a inseguire', w.placed.slice(b2).some((p) => p.book === 'no'));
  }
  ...
  console.log('\n── i gate di sicurezza');
  {
    ...
    ok('  e il motivo spiega che gli ordini scadono per GTD', /scadono per GTD/.test(r.markets[0].reason));

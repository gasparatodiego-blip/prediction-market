...
console.log('\n── 2 · buildCombined RIAMMETTE SOLO DOVE C\'È CAPITALE ────────────────────────────');
{
  ...
  // ⚠ LA SOSTITUZIONE DEVE POTER FALLIRE RUMOROSAMENTE (17 agosto 2026). La regex precedente pretendeva
  // un letterale fra apici singoli (`= '[^']*';`); dal momento in cui i due percorsi sono diventati
  // `path.join(DATA_DIR, …)` non avrebbe matchato piu' NIENTE, `String.replace` non solleva, e questo
  // blocco avrebbe letto il file di produzione dichiarandosi verde. Ora si prende la riga intera,
  // qualunque forma abbia, e si ASSERISCE che la sostituzione sia avvenuta.
  const sostituisci = (txt, nome, valore) => {
    const re = new RegExp(`^const ${nome}\\s*=.*$`, 'm');
    if (!re.test(txt)) throw new Error(`punti-di-filtro: non trovo la riga di ${nome} in rewards-normalize.js — la sostituzione sarebbe stata un no-op silenzioso`);
    return txt.replace(re, `const ${nome} = ${JSON.stringify(valore)};`);
  };
  let sorgente = fs.readFileSync(path.join(REPO, 'lib', 'rewards-normalize.js'), 'utf8');
  sorgente = sostituisci(sorgente, 'POLY_FILE', polyFile);
  sorgente = sostituisci(sorgente, 'KALSHI_FILE', path.join(tmp, 'nessuno.json'));
  sorgente = sorgente
...
console.log('\n── 3 · NC-2 · auto-close VISITA I MERCATI DOVE C\'È CAPITALE ──────────────────────');
{
  ...
  // ⚠ I COMMENTI SI TOLGONO PRIMA DI MISURARE UNA DISTANZA NEL SORGENTE (§5.3): altrimenti la finestra
  // misura quanto è lungo il commento, non quanto sono vicine le due istruzioni. È successo il
  // 13 agosto 2026 — il cablaggio del ritentativo dei residui ha spinto `marketIds: visitare` oltre i
  // 4000 caratteri e questo test è diventato rosso senza che niente di ciò che protegge fosse cambiato.
  const src40nudo = src40.replace(/^\s*\/\/.*$/gm, '');
  const inizio = src40nudo.indexOf('async function closeTask');
  const corpo = src40nudo.slice(inizio, inizio + 4000);

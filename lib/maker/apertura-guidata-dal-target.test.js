...
console.log('── 1 · LA REGOLA NUOVA: SI APRE FINCHE\' IL CAPITALE NON E\' AL LAVORO');
{
  ...
  const pieno = util(50, 400, 100);
  const b = UTIL.aperturaNuoviMercati({ utilizzo: pieno });
  ok('utilizzo 90,9% ⇒ nessun mercato NUOVO', b.ammessi === 0, `${pieno.pct}% · ammessi ${b.ammessi}`);
...
console.log('\n── 3 · IL TETTO DI VELOCITA\' RESTA, E NON E\' MAI INFINITO');
{
  ...
  ok('  ed è il numero dichiarato', UTIL.MAX_NUOVI_PER_GIRO === 6);
  ...
  ok('leggiMaxNuoviPerGiro difetta a 6', UTIL.leggiMaxNuoviPerGiro({}) === 6 && UTIL.leggiMaxNuoviPerGiro({ MAKER_MAX_NUOVI_PER_GIRO: '0' }) === 6);

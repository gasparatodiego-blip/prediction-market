...
console.log('\n4 · il cablaggio: il giro si ferma davvero, e lo dichiara');
{
  ...
  // Il blocco parte da `const ric =`, non da `UTIL.riconcilia(`: la guardia che decide SE confrontare
  // sta prima della chiamata, e tagliando dalla chiamata resterebbe fuori.
  const blocco = src.slice(src.indexOf('const ric ='), src.indexOf('const capitaleTotale'));
  ...
  // E il caso che NON deve fermare: posizioni illeggibili e' tollerato per progetto (il cancello del
  // trigger e' il saldo, gia' letto). La prima stesura del guard lo trattava come divergenza e fermava
  // giri sani — `tetti-dal-miniciclo.test.js` l'ha preso.
  ok('una misura non leggibile NON ferma il giro',
    /utilPrima\.leggibile\s*$/m.test(blocco) || /\? UTIL\.riconcilia\(/.test(blocco),
    'si prosegue col saldo del trigger, che e\' il comportamento documentato');

...
// Il piano nuovo si calcola con al massimo il 20% del capitale su un singolo mercato — lo stesso tetto
// che il motore di piazzamento applica al quoting (motore-unico.MARKET_CAP_PCT). Non è una
...
// Il tetto non è più scritto qui: è in lib/rewards/concentration.js perché il pannello «Ottimizza» legge
// LO STESSO numero. Due strade che rispondono alla stessa domanda non possono avere due costanti.
const { CONCENTRATION_CAP_FRAC } = require('../rewards/concentration');
...
async function runReallocCycle({ dryRunOnly = false, valueTriggerFrac = VALUE_TRIGGER_FRAC, attesa = ATTESA_DIFETTO } = {}, deps = {}) {
  ...
  traccia('capitale', 'letto', {
    ...
    tettoPerMercatoUsd: tetto, frazione: CONCENTRATION_CAP_FRAC,
  });
...
module.exports = { runReallocCycle, CONCENTRATION_CAP_FRAC, INTERVAL_MS, VALUE_TRIGGER_FRAC, PLAN_VERIFY_MAX_PASSES, BOARD_MAX_AGE_MS };

...
function regoleFinte({ midYes = 0.40, maxSpreadCents = 4.5, minSize = 20 } = {}) {
  return {
    ...
    rewardProgramme: 'active', bandRadiusCents: maxSpreadCents / 2,
    books: {
...
(async () => {
  console.log('\n════ 1 · FILL PARZIALE 40/100 ════');
  {
    ...
    ok('le 60 share NON FILLATE sono state cancellate', g.cancellati.includes('RESIDUO'), g.cancellati.join(','));
    ok('anche la sorella sovradimensionata è stata cancellata', g.cancellati.includes('SORELLA'));

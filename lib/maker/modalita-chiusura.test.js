...
(async () => {
  console.log('\n════ 1 · FILL PARZIALE 40/100 ════');
  {
    ...
    // ⚠ ASSERZIONE RISCRITTA IL 17 AGOSTO 2026, e il vecchio testo era la fotografia di un difetto.
    // Diceva «al secondo giro non si ricancella nulla», e la fixture del secondo giro passa gli STESSI
    // ordini — cioe' il residuo e' ANCORA A LIBRO. Ricancellarlo non e' churn: e' un ritentativo, ed e'
    // la sola risposta giusta a «la cancellazione di prima non ha funzionato». Il vecchio verde veniva
    // dalla guardia `statoChiusura.nuova`, che dopo la decisione dell'operatore (residuo cancellato
    // SEMPRE) non c'e' piu'. L'idempotenza vera vive nel LIBRO: appena l'ordine non c'e' piu',
    // `residuiDaCancellare` non lo elenca — e la riga sotto lo prova con la fixture giusta.
    ok('al secondo giro il residuo ANCORA a libro viene ritentato (non e\' churn, e\' un ritentativo)',
      g2.cancellati.includes('RESIDUO'), g2.cancellati.join(',') || 'nessuna');
    const g2b = await giro({ liv, rules, ordiniMercato: ordini.filter((o) => o.orderId !== 'RESIDUO'), chiusura, prezzoCarico: 0.40 });
    ok('  e con il residuo GIA\' via dal libro non si cancella piu\' niente: l\'idempotenza sta nel libro',
      !g2b.cancellati.includes('RESIDUO'),
      g2.cancellati.join(','));
  ...
  console.log('\n════ 1-bis · COPPIA COMPLETA: IL RESIDUO VIENE CANCELLATO ════');
  {
    ...
    // ⚠ L'ESITO E' CAMBIATO NOME: `coppia-completa-residuo-cancellato` veniva dal PASSO 2-bis, che
    // aspettava `manca <= 0` e che dal 17 agosto non esiste piu' — il residuo si cancella subito, quindi
    // quel ramo non poteva piu' scattare. Da oggi l'esito e' uno solo, con `alPrimoGiro` a dire QUANDO.
    ok('  e il motivo finisce a verbale con un esito suo, distinguibile',
      g.auditRighe.some((r) => r.outcome === 'modalita-chiusura-residuo-non-fillato-cancellato'
        && r.observed && r.observed.alPrimoGiro === false),
      g.auditRighe.map((r) => r.outcome).join(',') || 'nessuna riga');
  ...
  console.log('\n════ 2 · LA SORELLA È ESENTE DA «MAI PRIMO», E SOLO LEI ════');
  {
    ...
    // Diceva «NON e' esente: dichiara inCoda:true, perche' e' un ordine che aspetta e matura premi».
    // Era vera, ed e' esattamente la riga che ha prodotto il guasto: quel ragionamento vale finche' la
    // scala sceglie un prezzo DENTRO la banda, e diventa falso appena lo sceglie fuori. Su
    // `0x4d79d306` la scala ha deciso `SELL no 0.495` («carico +1 tick, fuori banda, si puo' uscire in
    // pari») e `prezzo-in-coda` l'ha riportato a **0.288** — 8,3 volte la concessione di §7. Dal 24/08
    // il prezzo deciso e' vincolante su TUTTA la classe delle chiusure, e la proprieta' da difendere
    // e' piu' forte: non «matura premi stando in coda» ma «il prezzo non lo tocca nessuno».
    // ⚠ E la differenza fra le due gambe NON sparisce: resta, ed e' l'aggressivita' — la sorella
    // attraversa, la posseduta riposa. Si asserisce quella, sotto.
    ok('  la gamba POSSEDUTA e\' marcata USCITA: il prezzo deciso e\' vincolante',
      !!posseduta && posseduta.uscita === true, posseduta ? JSON.stringify({ uscita: posseduta.uscita, inCoda: posseduta.inCoda }) : '-');
    ok('  e nemmeno lei e\' `inCoda`: l\'esenzione e\' della CLASSE, non del chiamante',
      !!posseduta && posseduta.inCoda === undefined, posseduta ? String(posseduta.inCoda) : '-');
    ok('  e il prezzo che parte E\' quello deciso dalla scala',
      !!posseduta && Number.isFinite(posseduta.prezzoDeciso) && Math.abs(posseduta.prezzoDeciso - posseduta.price) < 1e-9,
      posseduta ? `deciso ${posseduta.prezzoDeciso} vs partito ${posseduta.price}` : '-');
    ok('  MA RESTA LA DIFFERENZA CHE CONTA: la posseduta RIPOSA, la sorella ATTRAVERSA',
      !!posseduta && posseduta.attraversaApposta !== true && !!sorella && sorella.attraversaApposta !== true
      ? posseduta.side === 'SELL' && sorella.side === 'BUY' : true);
  ...
  console.log('\n════ 8 · FILL TOTALE 100/100 ════');
  {
    ...
    // ⚠ RISCRITTA (24/08). «Le stesse regole del parziale» resta la proprieta' vera e va difesa: il
    // fill totale e il parziale sono lo STESSO percorso (§4.6), e la ramificazione sta nei dati, non
    // in un `if`. Cio' che e' cambiato e' QUALI sono quelle regole: non piu' «inCoda:true» ma «marcata
    // uscita, non in coda, prezzo deciso intatto» — e si asserisce che la gamba fillata le riceva
    // IDENTICHE alla sorella, che e' il modo di dire «e' lo stesso percorso» senza fotografare un flag.
    const fillata = g.piazzati.find((o) => o.book === 'yes' && o.side === 'SELL');
    ok('la gamba fillata viene piazzata con le stesse regole del parziale',
      !!fillata && fillata.uscita === true && fillata.inCoda === undefined,
      fillata ? JSON.stringify({ uscita: fillata.uscita, inCoda: fillata.inCoda }) : 'assente');
    ok('  e le due gambe hanno la STESSA marcatura: un percorso solo, non due',
      !!fillata && !!sorella && fillata.uscita === sorella.uscita && fillata.inCoda === sorella.inCoda);
  ...
  console.log('\n════ 9 · FAIL-CLOSED E NON REGRESSIONE ════');
  {
    ...
    // ⚠ RISCRITTA (24/08): il punto di questo blocco e' che senza il registro iniettato NON si cancella
    // niente — cioe' il comportamento di prima — e quello resta asserito qui sopra. La coda non e' mai
    // stata la sostanza di questa asserzione: era il modo di dire «la sorella e' una quotazione
    // normale». Oggi le chiusure non sono in coda per costruzione, e cio' che va difeso e' che il
    // completamento parta comunque e col prezzo deciso.
    ok('  e la sorella maker parte marcata USCITA, mai in coda',
      piazzati.filter((o) => !o.attraversaApposta).every((o) => o.uscita === true && o.inCoda === undefined),
      JSON.stringify(piazzati.filter((o) => !o.attraversaApposta).map((o) => ({ u: o.uscita, c: o.inCoda }))));

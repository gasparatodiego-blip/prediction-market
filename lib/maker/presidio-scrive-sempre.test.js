...
function giro({ posizioni, piazza = async () => ({ ok: true }) }) {
  ...
  return A.presidioPosizioniVecchie({
    ...
    fileAncore,
    // ⚠ IL MINIMO PREMIANTE HA TRE FONTI DAL 18 AGOSTO 2026 (`min-size-mercato.js`), E LA TERZA È LA
    // RETE. Qui si iniettano un catalogo vuoto e un venue che non risponde: è ESATTAMENTE lo stato che
    // questo test descriveva prima — nessun minimo, quindi nessuna marcatura sotto-minimo — con la
    // differenza che adesso è dichiarato invece che accidentale, e il test non esce sulla rete. Un
    // test che chiama il venue vero è verde o rosso a seconda di com'è internet quel minuto.
    leggiCatalogo: () => ({}),
    leggiVenue: async () => ({ readable: false, error: 'nessun venue in questo test' }),

...
const REGISTRI = [
  ...
  ['ripianifica', 'la coda di ripianificazione dopo una gamba orfana', 'data/da-ripianificare.json'],
...
function selfcheck() {
  ...
  // ⚠ DERIVATO da `REGISTRI`, non ricopiato: il 12 agosto 2026 i registri sono passati da sei a sette
  // (la coda di ripianificazione) e queste asserzioni erano diventate rosse perché contavano a mano.
  // Un test che ricopia l'elenco che difende va aggiornato a ogni aggiunta; uno che lo deriva no.
  const tutti = (rimosso) => Object.fromEntries(REGISTRI.map(([k]) => [k, () => ({ ok: true, rimosso })]));
  const N = REGISTRI.length;

  const r1 = pulisciRegistri({ marketId: '0xm', causa: 'closed', libroLibero: true, registri: tutti(true) });
  ok(`libro libero ⇒ tutti e ${N} ripuliti`, r1.ok === true && r1.puliti.length === N);
  const r2 = pulisciRegistri({ marketId: '0xm', causa: 'closed', libroLibero: false, registri: tutti(true) });
  ok('libro NON libero ⇒ non si tocca niente', r2.ok === false && r2.puliti.length === 0 && r2.saltati.length === N);
  ...
  ok('un registro che esplode non ferma gli altri', r5.puliti.length === N - 1 && r5.falliti.length === 1);
  ...
  ok('i registri non cablati sono `saltati`, non falliti', r6.ok === true && r6.puliti.length === 1 && r6.saltati.length === N - 1);

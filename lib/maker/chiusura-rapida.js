...
//
// Questa regola risponde a una domanda diversa: quanto sono disposto a pagare per NON restare esposto?
// La risposta e' una decisione dell'operatore, e vale **120¢** — `TETTO_COPPIA_DEFAULT_CENTS` qui sotto.
// Fu 110¢ dal 9 agosto 2026 e passo' a 120¢ il 12; il perche' del cambio sta nella sezione «110 → 120».
// ⚠ QUESTA INTESTAZIONE HA DETTO «110¢» FINO AL 15 AGOSTO 2026, cioe' per tre giorni dopo che il numero
// era gia' cambiato dieci righe piu' sotto: e' il reperto D7 dell'audit (il commento e' cio' che si
// legge, il codice cio' che accade) applicato al file che DEFINISCE la costante. Per questo adesso il
// testo NOMINA la costante invece di ripeterne il valore dove non serve.
//
// VA DETTO CHIARO, PERCHE' NESSUN COMMENTO DEVE FARLO SEMBRARE GRATIS: sopra i 100¢ la coppia e' una
// PERDITA garantita. Una coppia comprata a 120¢ paga $1 alla risoluzione: −20¢ certi. Non e' un difetto
...
/** Il miglior prezzo di una scala del libro: il piu' ALTO fra i bid, il piu' BASSO fra gli ask. */
function migliore(livelli, verso) {
  if (!Array.isArray(livelli)) return null;
  const p = livelli.map((l) => Number(l && (l.price !== undefined ? l.price : l))).filter((x) => fin(x) && x > 0);
  if (!p.length) return null;
  return verso === 'max' ? Math.max(...p) : Math.min(...p);
}

/**
 * IL TETTO DELLA COPPIA, IN CENTESIMI. Decisione esplicita dell'operatore.
 *
 * Non e' 100: a 100¢ esatti la chiusura sarebbe possibile solo quando il mercato la regala, cioe' quasi
 * mai proprio quando serve — il momento in cui un lato si riempie e l'altro no e' per definizione un
 * momento in cui il book si e' mosso contro di noi. Il margine sopra la pari e' cio' che rende la regola
 * applicabile invece che teorica.
 *
 * ═══ 110 → 120 IL 12 AGOSTO 2026, POI 120 → 101 IL 15 AGOSTO 2026 ═══════════════════════════════════
 * Il numero e' il COSTO MASSIMO che si accetta di pagare per non restare direzionali. A 120¢ una coppia
 * chiusa e' una perdita certa di 20¢ per share; a 101¢ e' **un centesimo**.
 *
 * ⚠ PERCHE' SI STRINGE INVECE DI ALLARGARE, ED E' UNA MISURA E NON UN'INTUIZIONE. §5-bis p.162 ha
 * classificato 62.525 fill di 65 wallet maker veri: fra chi completa la coppia il costo mediano e'
 * **100,00¢**, il **98,5%** chiude entro 120¢ — ma la valvola oltre 110¢ e' usata dal **2,7%** e dal
 * **1,3%** dei due gruppi, cioe' da nessuno. Un tetto a 120¢ non stava proteggendo: stava autorizzando
 * una perdita di 20¢/share che in pratica nessun maker paga. La via d'uscita che i wallet efficienti
 * usano davvero quando la coppia non arriva NON e' pagarla cara, e' **smontare la gamba a −0,89¢
 * mediani** (§5-bis p.163) — che qui e' il gradino 2 di `urgenza-scoperto`, non questo tetto.
 *
 * ⚠ E QUESTO NUMERO ORA COINCIDE CON QUELLO DEL MERGE (`strategia-merge.MERGE_TETTO_COPPIA_CENTS`),
 * per decisione dell'operatore del 15 agosto 2026 — un tetto solo per tutta la scala d'uscita. Le due
 * domande restano diverse (li' «conviene completare?», qui «e' accettabile chiudere?»), ma la risposta
 * dell'operatore e' la stessa: 101¢. Restano DUE costanti e non una perche' rispondono a due domande:
 * il giorno in cui una si muove, l'altra deve poter restare ferma senza che nessuno se ne dimentichi.
 *
 * ⚠ NON TOCCA LA CHIUSURA FORZATA PRE-SCADENZA, che spinge il tetto a 200¢ sotto le tre ore: li' non si
 * sta scegliendo se chiudere, si sta chiudendo comunque.
 *
 ...
 * nemmeno su richiesta: il tetto e' il costo che si accetta di pagare, non un obiettivo di profitto.
 */
const TETTO_COPPIA_DEFAULT_CENTS = 101;
function leggiTetto(env = process.env) {
  const v = Number(env.MAKER_TETTO_COPPIA_CENTS);
  return fin(v) && v >= 100 && v <= 200 ? v : TETTO_COPPIA_DEFAULT_CENTS;
}
...
function pianificaRiposizionamentoScoperto({
  ...
  bandaHiControparte = null, bidsControparte = null, asksControparte = null,
  profitPct = 1, tettoCents = TETTO_COPPIA_CENTS,
  // ── MODALITÀ CHIUSURA (11 agosto 2026, decisione di Diego) ─────────────────────────────────────
  // FALSO di difetto, e va tenuto così: con `false` questa funzione si comporta ESATTAMENTE come prima
  // di questo lavoro, riga per riga. Il flag lo alza un solo chiamante (`auto-close.completaCoppia`) e
  // solo dopo che il tentativo immediato a mercato è fallito — vedi `modalita-chiusura.js`.
  modalitaChiusura = false,
} = {}) {
  ...
  if (!fin(sizePosseduta) || sizePosseduta <= 0) {
    ...
    if (fin(bandaHi) && prezzo > bandaHi + 1e-9) {
      ...
      // ── IN MODALITA' CHIUSURA NON SI TACE: +1 TICK SOPRA IL CARICO, ANCHE FUORI BANDA ──────────
      // Decisione di Diego, 11 agosto 2026, e ribalta il comportamento del 9 agosto (§5 punto 51), che
      // qui restituiva `null` — cioe' NESSUN ordine sul lato posseduto, quindi una posizione
      // direzionale, senza premi e senza uscita, a tempo indeterminato.
      //
      // Il ragionamento nuovo: «meglio un ordine fuori banda che nessun ordine — l'importante e' poter
      // uscire in pari». Un ordine fuori banda non matura premi, ma e' valido (`OUT_OF_BAND` e' l'unico
      // codice che questo sistema declassa ad avviso) e puo' chiudere la posizione al costo di carico.
      // La banda resta la PREFERENZA: si arriva qui solo dopo che il prezzo in banda si e' rivelato
      // impossibile, quindi non si perde un centesimo di premio che fosse ottenibile.
      //
      // ⚠ IL VINCOLO ASSOLUTO NON E' TOCCATO: `carico + 1 tick` e' per costruzione SOPRA il carico, e
      // l'arrotondamento e' IN SU. Non esiste un ramo di questa funzione che venda sotto il carico, e
      // un test lo verifica per sweep su tutte le combinazioni.
      const sopraIlCarico = +(Math.ceil((prezzoCarico + tick - 1e-9) / tick) * tick).toFixed(6);
      if (modalitaChiusura && fin(sopraIlCarico) && sopraIlCarico > prezzoCarico + 1e-9
          && sopraIlCarico > 0 && sopraIlCarico < 1) {
        latoPosseduto = { prezzo: sopraIlCarico, size: sizePosseduta, fuoriBanda: true };
        latoPossedutoMotivo = `banda sotto il carico: in modalita' chiusura si piazza comunque a`
          + ` ${(sopraIlCarico * 100).toFixed(1)}¢ = carico +1 tick, FUORI banda — nessun premio, ma si puo' uscire in pari`;
      } else {
  ...
  if (!fin(manca) || manca <= 0) controparteMotivo = 'non manca niente per completare la coppia';
    ...
    if (!fin(massimo) || massimo <= 0) controparteMotivo = `il carico ${(prezzoCarico * 100).toFixed(1)}¢ non lascia spazio sotto il tetto di ${tetto}¢`;
      ...
      //
      // ═══ «PRIMO ASSOLUTO» SI MISURA SUL LIBRO, NON SULLA BANDA ══════════════════════════════════
      // La prima versione (commit 5abf893) prezzava al BORDO DELLA BANDA, ed era sbagliato: il bordo
      // non ha nessuna relazione con dove sono gli altri. Misurato in produzione il 9 agosto alle
      // 10:25, subito dopo il riavvio: `riposizionamento-scoperto-controparte-reject-would-cross` x4
      // (il bordo banda stava OLTRE il miglior ask, quindi l'ordine avrebbe attraversato lo spread e
      // `manual-order` lo rifiuta perche' non dichiara `attraversaApposta`) e
      // `reject-mai-primo-sul-libro` x1. Zero controparti piazzate.
      //
      // Essere primi in coda su un BUY vuol dire stare **un tick sopra il miglior bid altrui**, non
      // vicino al mid. E per non diventare un taker bisogna restare **sotto il miglior ask**. Il prezzo
      // e' quindi il piu' BASSO fra tre limiti, e ognuno dei tre e' li' per una ragione diversa:
      //   · bestBid + tick   → scavalca la coda: e' lo scopo
      //   · bestAsk − tick   → non attraversa lo spread: resta un maker
      //   · massimo          → il tetto della coppia (`TETTO_COPPIA_CENTS`, oggi 120¢), che resta DURO
      // La banda non e' fra i limiti: restarci e' preferibile ma non e' un divieto — `OUT_OF_BAND` e'
      // l'unico codice che il sistema declassa ad avviso, e la priorita' dichiarata dall'operatore e'
      // scavalcare la coda. Se il prezzo esce di banda l'ordine e' valido e non matura premi, ed e'
      // esattamente il compromesso accettato.
      // ── NON SI ATTRAVERSA MAI LO SPREDD, NEMMENO SENZA L'ECCEZIONE ────────────────────────────────
      // Misurato il 9 agosto alle 10:25 su Chengdu (`cid_462e0287`): carico 61,64¢ ⇒ prezzo da tetto
      // 48¢, che stava OLTRE il miglior ask di YES. Esito:
      //   riposizionamento-scoperto-controparte-reject-would-cross · {"price":0.48,"size":21.69}
      // e li' `primoAssoluto` era FALSO — cioe' l'incrocio colpisce anche il percorso normale, non solo
      // il caso nuovo. Questa gamba e' un LIMIT che aspetta, non un taker: se il prezzo da tetto
      // supera il miglior ask lo si abbassa a un tick sotto. Il clamp puo' solo ABBASSARE, quindi non
      // apre niente e non tocca il tetto della coppia.
      const askAltro = migliore(asksControparte, 'min');
      let prezzo = fin(askAltro) ? Math.min(massimo, giuAlTick(askAltro - tick, tick)) : massimo;
      let primoAssoluto = false;
      let fuoriBanda = false;
      if (modalitaChiusura) {
        // ── LA SORELLA IN MODALITA' CHIUSURA: IL PIU' VICINO POSSIBILE AL MID ────────────────────
        // Decisione di Diego, 11 agosto 2026. Qui l'obiettivo dichiarato NON e' guadagnare premi su
        // questo ordine: e' farsi riempire in fretta, perche' finche' la coppia non e' completa non si
        // puo' fondere e il capitale resta esposto in una sola direzione. Quindi:
        //   · il prezzo e' il PIU' ALTO che non attraversi lo spread — `bestAsk − 1 tick`, che e' il
        //     punto piu' vicino al mid raggiungibile restando un maker. NON `bestBid + 1 tick`, che e'
        //     «primo in coda» ma resta in fondo allo spread e puo' aspettare a lungo;
        //   · e si OMETTE `inCoda`, cioe' l'esenzione da «mai primo sul libro».
        // Il prezzo qui sopra e' gia' `min(massimo, ask − tick)`: in modalita' chiusura non serve
        // ricalcolarlo, serve solo dichiarare l'esenzione. Se l'ask non e' leggibile si ripiega su
        // `bestBid + tick`, che e' comunque davanti a tutti gli altri.
        if (!fin(askAltro)) {
          const bid = migliore(bidsControparte, 'max');
          if (fin(bid)) prezzo = giuAlTick(Math.min(massimo, bid + tick), tick);
        }
        primoAssoluto = true;
        fuoriBanda = fin(bandaHiControparte) && prezzo > bandaHiControparte + 1e-9;
      } else if (bandaSottoCarico) {
        const bid = migliore(bidsControparte, 'max');
        const ask = askAltro;
        const limiti = [massimo];
        if (fin(bid)) limiti.push(+(bid + tick).toFixed(6));
        if (fin(ask)) limiti.push(+(ask - tick).toFixed(6));
        const scelto = giuAlTick(Math.min(...limiti), tick);
        // Serve almeno un lato del libro: senza, «davanti a chi» non ha risposta e non si indovina.
        if ((fin(bid) || fin(ask)) && fin(scelto) && scelto > 0) {
          prezzo = scelto;
          primoAssoluto = true;
          fuoriBanda = fin(bandaHiControparte) && scelto > bandaHiControparte + 1e-9;
        } else if (fin(bandaHiControparte)) {
          // Ripiego: senza profondita' si torna al bordo banda, che e' il comportamento del 5abf893.
          const aggressivo = giuAlTick(Math.min(bandaHiControparte, massimo), tick);
          if (fin(aggressivo) && aggressivo > 0) { prezzo = aggressivo; primoAssoluto = true; }
        }
      }
      ...
      controparte = { prezzo, size: manca, primoAssoluto, fuoriBanda };
    }
  }
...
module.exports = { pianificaChiusuraRapida, pianificaRiposizionamentoScoperto, migliore, rispettaIlTetto, leggiTetto, TETTO_COPPIA_CENTS, giuAlTick };

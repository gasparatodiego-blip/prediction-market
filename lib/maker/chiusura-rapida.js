 ...
 *
 * ═══ 110 → 120, IL 12 AGOSTO 2026 ═══════════════════════════════════════════════════════════════════
 * Decisione dell'operatore, gia' presa e mai applicata. Il numero e' il COSTO MASSIMO che si accetta di
 * pagare per non restare direzionali, e va letto per quello che e': una coppia chiusa a 120¢ e' una
 * perdita certa di 20¢ per share. Si accetta perche' l'alternativa non e' «zero»: e' un'esposizione
 * direzionale aperta a tempo indeterminato su un mercato che si sta muovendo contro di noi, il cui
 * esito peggiore vale 100¢ per share. Venti centesimi di perdita nota battono un dollaro di perdita
 * possibile — ed e' lo stesso ragionamento con cui la chiusura forzata pre-scadenza spinge il tetto a
 * 200¢ sotto le tre ore.
 *
 * ⚠ IL TETTO DEL MERGE (99¢) E' UN'ALTRA COSA E NON SI MUOVE: quello e' la soglia oltre la quale
 * completare la coppia smette di essere PROFITTEVOLE, e vive in `strategia-merge`. Questo e' la soglia
 * oltre la quale smette di essere ACCETTABILE. Il primo decide se conviene, il secondo quanto si e'
 * disposti a perdere pur di chiudere: sono due domande diverse e devono restare due numeri diversi.
 *
 ...
 */
const TETTO_COPPIA_DEFAULT_CENTS = 120;

...
const REGOLE_VINDMAN = {
  readable: true, tokenId: TOK_YES, tokenIdNo: TOK_NO, tick: 0.001, minSize: 20, maxSpreadCents: 2.25,
...
(async () => {
  ...
  console.log('\n── 1 · LA SCENA DEL 12 AGOSTO, RIPRODOTTA NUMERO PER NUMERO');
  {
    ...
    ok('  con il motivo testuale della produzione',
      // La PROPRIETA': il motivo nomina la banda e il pavimento (che senza concessione E' il carico).
      // La frase l'ha riscritta la scala di urgenza (§5 p.138); i due NUMERI sono quelli di sempre.
      /banda premiante corrente \(fino a 0.115\)/.test(d.reason)
      && /sotto il pavimento di uscita \(0.124\)/.test(d.reason), d.reason.slice(0, 90));
    ...
    // ══ AGGIORNATA IL 22 AGOSTO 2026, NON AMMORBIDITA ═══════════════════════════════════════════
    // Fino a oggi l'esito era `skip-no-target`: la banda stava sotto il pavimento e NESSUN prezzo era
    // producibile. Da oggi l'uscita puo' guardare FUORI banda quando completare la coppia e'
    // economicamente impossibile (qui 12,4¢ + 91,3¢ = 103,7¢, oltre il tetto di 101¢), quindi il
    // ripiego non e' piu' il silenzio ma un'uscita vera — v. `uscita-fuori-banda.test.js`.
    // ⚠ LA PROPRIETA' DIFESA E' LA STESSA E RESTA STRETTA: «un completamento rifiutato non lascia la
    // posizione senza esito, e il tentativo della coppia e' dichiarato». Cambia quale esito e', non
    // che ce ne sia uno. Se domani sparisse l'esito, questo blocco tornerebbe rosso.
    const ESITI_RIPIEGO = /^(skip-no-target|trigger|merge-saltato-)/;
    ok('completamento rifiutato ⇒ la posizione riceve comunque un esito',
      outRif.some((o) => ESITI_RIPIEGO.test(o)), outRif.join(', '));
    ok('  e dichiara di aver tentato la coppia',
      rifiutato.righe.some((r) => (r.outcome === 'skip-no-target' && r.observed && r.observed.coppiaTentata === true)
        || /^merge-saltato-/.test(String(r.outcome))), outRif.join(', '));
    ...
    // Il tetto si chiede a `strategia-merge` (101¢ dal 15 agosto 2026), non si ricopia: era 86,6¢
    // quando valeva 99¢, ed e' 88,6¢ adesso. La proprieta' e' «non oltre il tetto», non il numero.
    const tettoMerge = require('./strategia-merge').tettoSecondoLato(0.124);
    ok(`  al tetto del merge (${(tettoMerge * 100).toFixed(1)}¢ su un carico di 12,4¢)`,
      !!buy && buy.price <= tettoMerge + 1e-9, buy ? String(buy.price) : '—');
  ...
  console.log('\n── 4 · SE UN RAMO FUTURO SFUGGISSE, IL FLUSH LO PRENDE');
  {
    ...
    // ⚠ `const|let`, NON `const` SOLO: il 17 agosto il ciclo e' passato a `let` perche' il carico di
    // ripiego sostituisce `pos` quando il venue non ha ancora pubblicato `avgPrice`. La proprieta' da
    // difendere e' «c'e' un flush in cima al ciclo delle posizioni», non la parola chiave con cui la
    // variabile e' dichiarata: ancorarsi a quella e' la classe «test che fotografa il codice invece
    // della proprieta'» (CLAUDE.md §5.3), e si rompe al primo refactor senza nessun difetto.
    ok('  uno in cima al ciclo delle posizioni',
      /for \((?:const|let|var) pos of mine\) \{[\s\S]{0,600}?flushObblighi\(\);/.test(corpo));
...
{
  const r = await ciclo({
    rules: { ...REGOLE_VINDMAN, midAgeSec: undefined },
    posizioni: [{ tokenId: TOK_YES, size: 60, avgPrice: 0.90 }],
    depth: { readable: true, yes: { asks: [{ price: 0.913, size: 96 }], bids: [{ price: 0.901, size: 126 }] },
      no: { asks: [{ price: 0.099, size: 200 }], bids: [{ price: 0.087, size: 200 }] } },
  });
  const sell = r.piazzati.filter((x) => x && x.side === 'SELL');
  ok('gate: senza eta del book nessuna vendita', sell.length === 0, `${sell.length} SELL su ${r.piazzati.length}`);
  ...
  ok('  CONTROLLO: con l eta il ciclo agisce', c.piazzati.length > 0 || (c.righe || []).length > 0,
    `${c.piazzati.length} piazzati`);
}

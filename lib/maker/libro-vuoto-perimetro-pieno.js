 ...
 * @param {number} p.ora
 * @param {Array<string>} [p.svuotatiDaNoi]  mercati dove NOI abbiamo cancellato ordini nostri nella
 *        finestra osservata: l'osservazione non conta come sterile
 ...
 *            conteggi:object}}
 */
function valuta({ attivi, ordini, stato, ora, svuotatiDaNoi = [], piazzatiConSuccesso = [] } = {}) {
  ...
  const fermo = (motivo, statoNuovo) => ({
    ...
    conteggi: { ...(statoNuovo || S).conteggi }, nonContate: nonContateFuori,
  });
  ...
  for (const raw of attivi) {
    ...
    // ⚠ AZZERA, non «non conta»: un piazzamento riuscito dimostra che in questo mercato il capitale ci
    // si puo' mettere. Qualunque cosa sia successa prima non descrive piu' questo mercato.
    if (piazzati.has(id)) continue;
    // ⚠ NON CONTA, e il contatore resta dov'era: qui il libro e' vuoto perche' l'abbiamo svuotato NOI.
    // Non e' una prova di sterilita' — e' una prova del contrario, perche' per cancellare un ordine
    // bisogna prima averlo piazzato. Si conserva il conteggio invece di azzerarlo, cosi' un mercato
    // davvero sterile che ogni tanto riceve una cancellazione non riparte da zero all'infinito.
    if (svuotati.has(id)) {
      const nPrec = fin(base[id]) ? base[id] : 0;
      if (nPrec > 0) conteggi[id] = nPrec;
      nonContate.push({ id, osservazioniConservate: nPrec, motivo: 'svuotato-da-noi',
  ...
  return {
    azione: 'rilascia', daRilasciare, statoNuovo, conteggi: { ...conteggi }, nonContate,

...
function nostriSulLato({ orders = null, tokenId = null, book = null } = {}) {
  ...
  for (const o of orders) {
    ...
    const appartiene = tok
      ? String(o.tokenId || '') === tok
      : (bk ? String(o.book || '').toLowerCase() === bk : false);
...
function selfcheck() {
  ...
  ok('né tokenId né book ⇒ nessuna riga (non si indovina il lato)',
    nostriSulLato({ orders: righe }).ordini.length === 0);

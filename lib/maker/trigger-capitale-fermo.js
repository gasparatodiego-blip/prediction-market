...
//
// ═══ «SALDO pUSD» È IL SEGNALE GIUSTO, MA NON È IL CAPITALE LIBERO ══════════════════════════════════
// ⚠ QUI C'ERA UN'AFFERMAZIONE FALSA, ed è la radice di un bug di sicurezza operativa (CLAUDE.md §5
// punto 58). Diceva: «su questo venue un ordine BUY a riposo IMMOBILIZZA il collaterale: finché
// l'ordine è sul libro, quei dollari non sono nel saldo». **È il contrario.** Su Polymarket l'ordine è
// firmato off-chain e il collaterale resta nel wallet fino al match: il saldo pUSD NON scende quando si
// mette un BUY a riposo. Verificato on-chain il 9 agosto 2026 — cash $633,90 + posizioni $35,19 =
// $669,09, esattamente il Portfolio dell'app, mentre il sistema dichiarava $776,65 sommando anche i
// $107,46 di ordini a riposo come se fossero denaro in più.
//
// COSA RESTA VERO: il saldo è il SEGNALE giusto per accorgersi che del capitale si è liberato — quando
// un merge o una chiusura restituiscono collaterale, il saldo sale, ed è ciò che questo modulo aspetta.
// COSA NON È VERO: che il saldo sia il capitale non allocato. Una parte copre già ordini sul libro.
// Il capitale davvero impegnabile è `misuraUtilizzo().liberoUsd`, che quel nozionale lo sottrae — ed è
// quello che il mini-ciclo passa a `pianificaGiro`, non il saldo grezzo.
//
 ...
 */
// ⚠ ERA `34` FISSO, E IL 12 AGOSTO 2026 E' DIVENTATO IMPOSSIBILE. Con il tetto per mercato derivato
// dal capitale (`concentration.capPerMarketUsd`), a $663,11 il tetto vale **$32,67**: un minimo di $34
// non puo' MAI essere soddisfatto, perche' lo spazio di un mercato non supera il suo tetto. Il
// mini-ciclo rispondeva «nessun mercato del piano ha spazio sufficiente adesso» a ogni giro — un
// fermo totale, silenzioso, prodotto da due numeri che nessuno confrontava.
//
// Adesso e' DERIVATO dal pavimento premiante: sotto `min_incentive_size` il venue non assegna
// punteggio e il reward e' ZERO, quindi quella e' la vera linea fra «un ordine» e «spolverare» — ed e'
// una linea del venue, non la mediana di un campione di 21 wallet. Per costruzione sta sempre sotto il
// tetto per mercato (il tetto e' `max(base, pavimento)`), quindi l'impossibilita' non puo' tornare.
// `COSTO_COPPIA` è IMPORTATO e non ridichiarato: è lo stesso 0,98 con cui il tetto per mercato e il
// pavimento premiante sono costruiti. Due copie qui vorrebbero dire due pavimenti diversi per lo
// stesso mercato — il reperto che il rilevatore D1 cerca.
const { pavimentoPremiante, MIN_PREMIANTE_TIPICO, COSTO_COPPIA } = require('../rewards/concentration');
const MIN_ALLOCAZIONE_USD = Number(process.env.TRIGGER_CAPITALE_MIN_USD || pavimentoPremiante(MIN_PREMIANTE_TIPICO));
 ...
 */
/**
 * IL PAVIMENTO IN DOLLARI DI UNA RIGA — una funzione sola, due chiamanti.
 *
 * `scegliMercato` la usa per decidere se una riga ha spazio; `contaRigheUtili` la usa per dire quante
 * righe del piano sono ancora spendibili. Scriverla due volte vorrebbe dire che il conto delle righe
 * utili e la scelta vera possono dare risposte diverse sullo stesso piano — cioè un piano dichiarato
 * pieno che poi non produce nessun ordine, che è esattamente il modo in cui il vuoto del 13 agosto
 * è rimasto invisibile per tre ore.
 *
 * @returns {{usd:number, come:string}}
 */
function pavimentoDiRiga(r, minAllocazioneUsd = MIN_ALLOCAZIONE_USD) {
  const costoCoppia = fin(r && r.pairCostUsd) && r.pairCostUsd > 0 ? r.pairCostUsd : COSTO_COPPIA;
  if (fin(r && r.minSizeShares) && r.minSizeShares > 0) {
    return { usd: +(r.minSizeShares * costoCoppia).toFixed(2), come: `minimo del venue ${r.minSizeShares} share × ${costoCoppia}` };
  }
  return { usd: minAllocazioneUsd, come: 'minimo tipico, la riga non dichiara il proprio' };
}

/**
 * QUANTE RIGHE DI QUESTO PIANO SONO ANCORA SPENDIBILI ADESSO.
 *
 * Non «quante righe ha il piano»: quante ne hanno ancora spazio sopra il proprio pavimento, dato ciò
 * che è già a riposo. È la differenza fra le due che il 13 agosto non aveva un numero — il piano ne
 * dichiarava 17 e le spendibili erano ZERO — e senza quel numero nessuno poteva accorgersi che il
 * piano si era consumato mentre il ciclo pesante era ancora a ore di distanza.
 *
 * Usa `pavimentoDiRiga` e gli stessi ingressi di `scegliMercato`: è una proiezione della stessa
 * decisione, non una seconda regola. Non chiama `gambeCostruibili` — che costa e legge il tocco vivo:
 * è un conteggio, quindi sovrastima, e sovrastimare qui significa ricostruire un piano che forse non
 * serviva. Il verso opposto (sottostimare e non ricostruire) è quello che lascia il capitale fermo.
 */
function contaRigheUtili({
  righe = [], notionalePerMercato = {}, capPerMercatoUsd = null,
  minAllocazioneUsd = MIN_ALLOCAZIONE_USD, disponibileUsd = Infinity,
} = {}) {
  let utili = 0;
  for (const r of (righe || [])) {
    ...
    const allocabile = Math.min(fin(disponibileUsd) ? disponibileUsd : Infinity, Math.max(0, tetto - vivo));
    if (allocabile >= pavimentoDiRiga(r, minAllocazioneUsd).usd) utili += 1;
  }
  return utili;
}
...
function scegliMercato({
  ...
  for (const r of ordinate) {
    ...
    // ── IL PAVIMENTO SI MISURA SU QUESTA RIGA, NON SU UN MERCATO TIPICO ──────────────────────────
    //
    // ⚠ IL SECONDO METÀ DEL DEADLOCK DEL 13 AGOSTO 2026. `minAllocazioneUsd` è
    // `pavimentoPremiante(20)` = 20 × 0,98 × **1,25** = $24,50: il minimo del venue di un mercato
    // TIPICO, più il margine del 25%. Confrontarlo con l'allocazione vera di una riga fa due errori
    // insieme — usa il minimo di un altro mercato, e applica il margine una SECONDA volta.
    //
    // Il margine esiste già ed è applicato una volta sola, dove decide davvero: `mercatoAmmissibile`
    // ammette un mercato solo se il TETTO sta sopra `minSize × costo × 1,25`. Riapplicarlo qui, sulla
    // somma che il piano ha davvero allocato, è la classe del doppio conteggio — la stessa di §5
    // punto 58 sul capitale. Misurato: una riga da $24,00 su un mercato a `minSize 20` compra
    // `24/0,98 = 24,49` share per lato, cioè il **22% sopra** il minimo del venue: è premiante, il
    // venue la accetta, e **il ciclo da 6 ore la piazza** (22 gambe alle 02:31). La rifiutava solo
    // questo percorso, che è il difetto: due corsie, due minimi, sullo stesso piano.
    //
    // Adesso il pavimento di una riga è la VERITÀ DEL VENUE per QUEL mercato — `minSizeShares ×
    // costo della coppia` — e la costante globale resta come ripiego per una riga che non dichiara il
    // proprio minimo. È più STRETTO dove deve esserlo (minSize 200 ⇒ $196, non $24,50) e smette di
    // esserlo dove non ha senso. Il cancello duro sul minimo del venue non è toccato: sta due
    // controlli più sotto (`share < r.minSizeShares`) e continua a rifiutare in share, non in dollari.
    const pav = pavimentoDiRiga(r, minAllocazioneUsd);

    if (allocabile < pav.usd) {
      esaminate.push({ marketId: r.marketId, pianificato, vivo, spazio, pavimentoRigaUsd: pav.usd,
        motivo: spazio <= 0 ? 'nessuno spazio: il mercato è già al capitale del piano' : `spazio $${spazio.toFixed(2)} sotto il pavimento di questa riga ($${pav.usd.toFixed(2)} — ${pav.come})` });
      continue;
    }
 ...
 * restano intatte — è un limite di RAGGIO D'AZIONE: un giro che potesse piazzare su tutto il piano
 * sarebbe un ciclo completo travestito da trigger, senza la verifica al venue che il ciclo completo fa.
 *
 * ⚠ NON E' PIU' DICHIARATO QUI. Era `Number(process.env.TRIGGER_CAPITALE_MAX_MERCATI || 6)` accanto a
 * un `MAX_NUOVI_PER_GIRO = 6` in `utilizzo-capitale`, con un commento che diceva «è lo stesso numero» —
 * cioè due copie dello stesso raggio d'azione tenute allineate a mano, il reperto che il rilevatore D1
 * cerca. Dal 12 agosto 2026 il numero vive in UN posto solo e questo lo importa: la divergenza «il giro
 * ne apre 12 e il tetto ne ammette 6» non è più esprimibile.
 *
 * `TRIGGER_CAPITALE_MAX_MERCATI` resta accettata come override e vince sul difetto condiviso, così chi
 * la usa oggi nell'ambiente non cambia comportamento.
 */
const MAX_MERCATI_PER_GIRO = (() => {
  const v = Number(process.env.TRIGGER_CAPITALE_MAX_MERCATI);
  if (Number.isFinite(v) && v >= 1) return Math.floor(v);
  return require('./utilizzo-capitale').leggiMaxNuoviPerGiro();
})();

/**
 * SOTTO QUANTE RIGHE UTILI IL PIANO VA RICOSTRUITO, senza aspettare il ciclo da sei ore.
 *
 * ⚠ NON È UN NUMERO NUOVO: è `MAX_MERCATI_PER_GIRO`, cioè quanti mercati un solo giro può toccare.
 * La ragione è aritmetica e non estetica — se le righe spendibili sono meno dei posti che il giro ha,
 * il giro finisce il piano PRIMA di finire i suoi posti, e da lì in poi ogni giro successivo ne trova
 * meno di quello prima. Cioè: sotto quella soglia il piano ha già cominciato a consumarsi, e
 * ricostruirlo costa ~13 s mentre non ricostruirlo è costato TRE ORE di capitale fermo il 13 agosto
 * 2026 (17 righe alle 02:38, 1 alle 05:44, zero ordini a riposo per 180 minuti).
 *
 * Si dichiara con `TRIGGER_CAPITALE_SOGLIA_RIGHE`; un valore assurdo viene scartato in favore del
 * difetto, la stessa regola di fine scala e dell'orizzonte.
 */
const SOGLIA_RIGHE_UTILI = (() => {
  const v = Number(process.env.TRIGGER_CAPITALE_SOGLIA_RIGHE);
  if (Number.isFinite(v) && v >= 1) return Math.floor(v);
  return MAX_MERCATI_PER_GIRO;
})();
...
// ── I TETTI DI CAPITALE PER MERCATO, QUANDO A RICALCOLARE È IL MINI-CICLO ─────────────────────────────
//
// ═══ IL DIFETTO CHE QUESTO CHIUDE (misurato il 9 agosto 2026) ════════════════════════════════════════
// `maker-allocated-capital.json` è la fotografia del tetto per mercato, e la scriveva SOLO il ciclo
// fisso da 6h (`realloc-cycle.js:479`, dopo un reset riuscito). Il mini-ciclo ricalcola un piano fresco
// ogni ~10 minuti e non ne scriveva mai: la fotografia restava quella delle 03:42, dodici tetti per un
// totale di **$600** — il capitale di allora. Nel frattempo il capitale era salito a **$850,82**.
//
// Due conseguenze, entrambe osservate:
//   · il 90% di utilizzo era IRRAGGIUNGIBILE per costruzione. Anche riempiendo ogni mercato fino al suo
//     tetto si arrivava a $600/$850,82 = **70,5%**, e il riallocatore continuava a dichiarare un deficit
//     che nessun piano poteva colmare.
//   · i mercati che il piano fresco sceglieva e la fotografia vecchia non conosceva restavano senza
//     tetto, e a valle un tetto assente vale «nessuna esposizione nuova»: è l'origine di
//     `saltato-tetto-non-leggibile`, dieci volte su Dallas (`cid_a7245f90…`) mentre ci tenevamo sopra
//     una coppia completa.
//
// ═══ PERCHÉ UNIONE E NON SOSTITUZIONE ════════════════════════════════════════════════════════════════
// `writeAllocatedCapital` SOSTITUISCE l'intera mappa, ed è la semantica giusta per chi parla a nome di
// tutto il piano — il ciclo da 6h, che prima cancella tutto e poi ripiazza. Il mini-ciclo non è quello:
// aggiunge capitale su alcuni mercati e non ha nessuna opinione sugli altri. Se scrivesse le sue sole
// righe, ogni mercato fuori dal ricalcolo di quel giro perderebbe il tetto — cioè si vedrebbe negare
// nuova esposizione — ogni dieci minuti. Quindi qui si costruisce l'UNIONE e la si passa al modulo, che
// resta con una semantica sola. È la stessa disciplina «solo acquisire, mai rilasciare» con cui il
// mini-ciclo tratta gestione manuale e uscita automatica (CLAUDE.md §5 punto 41).
//
// ═══ COSA VIENE POTATO, E COS'È INTOCCABILE ══════════════════════════════════════════════════════════
// Un mercato del vecchio piano che NON è nel piano di adesso e su cui non abbiamo NIENTE — nessun ordine
// a riposo, nessuna posizione — esce dalla fotografia: tenerlo vorrebbe dire portarsi dietro per sempre
// i tetti di piani che nessuno rinnova. Un mercato dove c'è del nostro denaro NON esce mai, nemmeno se
// il piano lo ha abbandonato: togliergli il tetto bloccherebbe la gestione di una posizione viva. Se
// l'elenco dei mercati attivi non arriva, non si pota niente — non si butta ciò che non si è potuto
// guardare.
//
// ═══ PERCHÉ NON SI RISCRIVE A OGNI GIRO ══════════════════════════════════════════════════════════════
// Riscrivere ogni dieci minuti sarebbe rumore: `ageSec` viaggia insieme al tetto in ogni verdetto a
// valle, e un file che cambia di continuo rende impossibile capire QUANDO un tetto è stato deciso
// davvero. Si scrive solo quando la fotografia direbbe qualcosa di diverso: un mercato in più, un tetto
// spostato oltre le soglie, il capitale totale cambiato, o la fotografia troppo vecchia. Le soglie sono
// relative E assolute insieme perché su tetti da $24 un 5% è mezzo dollaro, cioè rumore di arrotondamento.
const TETTI_DELTA_FRAC = 0.05;
const TETTI_DELTA_USD = 1;
// 12 h: dentro il ritmo del ciclo da 6h, ma ben sotto le 24 h oltre le quali `readAllocatedCapital`
// dichiara la fotografia scaduta e fa fallire chiuso tutto. Se il ciclo da 6h si fermasse, il mini-ciclo
// tiene in vita i tetti invece di lasciarli scadere — e il motivo `rinfresco` nel referto dice che è
// successo, così un ciclo fisso rotto non resta invisibile.
const TETTI_ETA_MAX_MS = 12 * 3_600_000;

function oltreLaSoglia(nuovo, vecchio, deltaFrac, deltaUsd) {
  const diff = Math.abs(nuovo - vecchio);
  return diff > deltaUsd && diff > Math.abs(vecchio) * deltaFrac;
}

/**
 * DECIDE la fotografia dei tetti che il mini-ciclo dovrebbe scrivere, e SE scriverla. Non tocca il
 * disco: riceve la fotografia corrente e restituisce quella nuova, così la regola si prova senza fs.
 *
 * @param a.righe             le righe del piano di adesso: [{ marketId, capital }]
 * @param a.capPerMercatoUsd  il tetto di concentrazione sul capitale VERO (20%) — nessuna riga lo supera
 * @param a.capitaleTotaleUsd il capitale totale contro cui questa fotografia è stata decisa
 * @param a.snapshot          `readAllocatedCapitalAll()`: { readable, markets, updatedAt, capital }
 * @param a.mercatiAttivi     id dove abbiamo ordini a riposo o posizioni; null = non potare niente
 * @returns {{scrivi:boolean, motivo:string, rows:Array, capital:number|null,
 *            aggiunti:string[], aggiornati:string[], potati:string[], conservati:number}}
 */
function decidiTetti({
  righe = [], capPerMercatoUsd = null, capitaleTotaleUsd = null,
  snapshot = null, mercatiAttivi = null, now = Date.now(),
  deltaFrac = TETTI_DELTA_FRAC, deltaUsd = TETTI_DELTA_USD, etaMaxMs = TETTI_ETA_MAX_MS,
} = {}) {
  const no = (motivo) => ({ scrivi: false, motivo, rows: [], capital: null,
    aggiunti: [], aggiornati: [], potati: [], conservati: 0 });

  // Non si sovrascrive una fotografia che non si è potuta leggere: sostituirebbe tetti ignoti con
  // l'unione parziale che si conosce, cioè cancellerebbe alla cieca.
  if (!snapshot || snapshot.readable !== true) {
    return no(`fotografia dei tetti non leggibile (${(snapshot && snapshot.error) || 'nessuna lettura'}): non si sovrascrive ciò che non si è letto`);
  }

  // ── I TETTI DEL PIANO DI ADESSO ───────────────────────────────────────────────────────────────────
  // Si CLAMPA al tetto di concentrazione: le righe di un ricalcolo fresco lo rispettano già (gli viene
  // passato come `maxPerMarketUsd`), ma quelle di un piano SALVATO sono state decise contro un capitale
  // di allora, che può essere stato più grande. Il verso del clamp è sempre verso il basso.
  const nuovi = new Map();
  for (const r of righe) {
    const id = normId(r && r.marketId);
    if (!id) continue;
    const grezzo = Number(r.capital);
    if (!fin(grezzo) || grezzo <= 0) continue;
    const tetto = fin(capPerMercatoUsd) && capPerMercatoUsd > 0 ? Math.min(grezzo, capPerMercatoUsd) : grezzo;
    nuovi.set(id, +tetto.toFixed(2));
  }
  if (!nuovi.size) return no('il piano di adesso non ha righe con un capitale utilizzabile');

  const vecchi = new Map();
  for (const [id, rec] of Object.entries(snapshot.markets || {})) {
    const v = rec && Number(rec.capitalUsd);
    if (fin(v) && v > 0) vecchi.set(normId(id), +v.toFixed(2));
  }
  const attivi = Array.isArray(mercatiAttivi) ? new Set(mercatiAttivi.map(normId).filter(Boolean)) : null;

  // ── L'UNIONE ──────────────────────────────────────────────────────────────────────────────────────
  const rows = []; const aggiunti = []; const aggiornati = []; const potati = [];
  let conservati = 0;
  for (const [id, v] of vecchi) {
    if (nuovi.has(id)) continue;                       // lo dirà il piano di adesso, più sotto
    if (attivi && !attivi.has(id)) { potati.push(id); continue; }
    rows.push({ marketId: id, capital: v }); conservati += 1;
  }
  for (const [id, v] of nuovi) {
    const prima = vecchi.has(id) ? vecchi.get(id) : null;
    if (prima == null) aggiunti.push(id);
    else if (oltreLaSoglia(v, prima, deltaFrac, deltaUsd)) aggiornati.push(id);
    rows.push({ marketId: id, capital: v });
  }

  // ── SI SCRIVE SOLO SE LA FOTOGRAFIA DIREBBE QUALCOSA DI DIVERSO ───────────────────────────────────
  const eta = fin(snapshot.updatedAt) ? now - snapshot.updatedAt : null;
  const capPrima = fin(snapshot.capital) ? snapshot.capital : null;
  const capOra = fin(capitaleTotaleUsd) ? +capitaleTotaleUsd.toFixed(2) : null;
  const capitaleCambiato = capOra != null
    && (capPrima == null || oltreLaSoglia(capOra, capPrima, deltaFrac, deltaUsd));

  const esito = { rows, capital: capOra, aggiunti, aggiornati, potati, conservati };
  if (aggiunti.length) {
    return { ...esito, scrivi: true,
      motivo: `${aggiunti.length} mercato/i del piano non aveva tetto (senza, a valle vale «nessuna esposizione nuova»)` };
  }
  if (aggiornati.length) {
    return { ...esito, scrivi: true, motivo: `${aggiornati.length} tetto/i spostato/i oltre le soglie (${(deltaFrac * 100).toFixed(0)}% e $${deltaUsd})` };
  }
  if (potati.length) {
    return { ...esito, scrivi: true, motivo: `${potati.length} mercato/i senza denaro nostro e fuori dal piano: escono dalla fotografia` };
  }
  if (capitaleCambiato) {
    return { ...esito, scrivi: true,
      motivo: `il capitale totale è ${capPrima == null ? 'ignoto nella fotografia' : `passato da $${capPrima.toFixed(2)}`} a $${capOra.toFixed(2)}` };
  }
  if (eta != null && eta > etaMaxMs) {
    return { ...esito, scrivi: true, motivo: `rinfresco: la fotografia ha ${Math.round(eta / 3_600_000)} h (limite ${Math.round(etaMaxMs / 3_600_000)})` };
  }
  return { ...esito, scrivi: false,
    motivo: `nessuna differenza sopra le soglie: ${rows.length} tetti invariati${eta != null ? `, fotografia di ${Math.round(eta / 60_000)} min fa` : ''}` };
}

module.exports = {
  decidiTrigger, scegliMercato, pianificaGiro, shareARiscalo, notionalePerMercato, decidiTetti,
  pavimentoDiRiga, contaRigheUtili, SOGLIA_RIGHE_UTILI,
  ...
  TETTI_DELTA_FRAC, TETTI_DELTA_USD, TETTI_ETA_MAX_MS,
};

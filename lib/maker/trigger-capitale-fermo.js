 ...
 * morde per primo: a mordere sono lo spazio sotto il tetto e la rampa.
 */
 ...
 * raggiunto · tetto di mercati per giro · rampa esaurita · nessuna riga più utilizzabile.
 *
 ...
 * @param nuoviAmmessi         quanti mercati NUOVI la rampa consente ancora (Infinity ⇒ nessun limite)
 * @param mercatiGiaAperti     gli id che la rampa ha già contato: riprenderli non consuma un posto
...
function pianificaGiro({
  ...
  nuoviAmmessi = Infinity, mercatiGiaAperti = [],
} = {}) {
  ...
  for (let i = 0; i < maxMercati; i += 1) {
    ...
    if (restanoNuovi <= 0) {
      // Si prova comunque: un mercato GIA' aperto non consuma un posto della rampa, quindi il giro può
      // continuare su quelli. Solo se la scelta cade su un mercato nuovo ci si ferma.
    }
    ...
    if (nuovo && restanoNuovi <= 0) {
      // La rampa è una protezione esistente e vince sul target di utilizzo: si dichiara e si smette.
      motivoStop = `rampa esaurita: ${String(s.riga.marketId).slice(0, 10)}… sarebbe un mercato NUOVO e non ne restano da aprire`;

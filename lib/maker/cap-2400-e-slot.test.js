...
ok('  e il tetto per mercato e finito', Number.isFinite(TETTO) && TETTO > 0, String(TETTO));
{
  ...
  // ⚠ E il soffitto di sorgente e' esattamente il massimo che il cap autorizza, non un numero a caso.
  const S = SEL.MAX_MERCATI_CONTEMPORANEI;
  ok(`  il soffitto di sorgente (${S}) sta sotto il cap`,
    CONC.esposizioneMassimaRaggiungibileUsd(S) <= CAP,
    `${CONC.esposizioneMassimaRaggiungibileUsd(S)} > ${CAP}`);
  ok(`  e uno slot in piu' (${S + 1}) NON ci starebbe: il soffitto e' il massimo, non una scelta`,
    CONC.esposizioneMassimaRaggiungibileUsd(S + 1) > CAP,
    `${CONC.esposizioneMassimaRaggiungibileUsd(S + 1)} <= ${CAP}: il soffitto e' piu' basso del necessario`);
}
...
// ⚠ E' IL BLOCCO CHE HA TROVATO IL GUASTO DEL 18 AGOSTO: `quantiMercati` risponde col DIFETTO, non con
// un errore, quando il valore supera il soffitto. Scrivere 18 con un soffitto a 12 non e' un rifiuto
// rumoroso: e' un bot che gira a 12 mentre il config dice 18, e nessuno se ne accorge.
{
  ...
  ok('  il numero in servizio non supera mai il soffitto', r.quanti <= SEL.MAX_MERCATI_CONTEMPORANEI,
    `${r.quanti} > ${SEL.MAX_MERCATI_CONTEMPORANEI}`);

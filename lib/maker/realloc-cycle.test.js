...
const { runReallocCycle, CONCENTRATION_CAP_FRAC, PLAN_VERIFY_MAX_PASSES } = require('./realloc-cycle');
...
(async () => {
  ...
  console.log('\n══ SCENARIO 1 · UN MERCATO SI RISOLVE DURANTE IL CICLO ⇒ IL RESET SCATTA');
  {
    ...
    ok(`  con il tetto di concentrazione al ${Math.round(CONCENTRATION_CAP_FRAC * 100)}%`, m.fatti.pianiChiesti[0].maxPerMarketUsd === 200, String(m.fatti.pianiChiesti[0].maxPerMarketUsd));
  ...
  console.log('\n══ UN MERCATO DEL PIANO NUOVO È GIÀ RISOLTO ⇒ ESCE E IL CAPITALE VA ALTROVE');
  {
    ...
    ok('  e il ricalcolo ha tenuto lo stesso capitale e lo stesso tetto', (() => {
      ...
      return rif.capital === 1000 && rif.maxPerMarketUsd === 200;
    })());
  ...
  console.log('\n══ IL CAPITALE NON PUO SUPERARE IL TETTO DI ESPOSIZIONE APERTA');
  {
    ...
    ok('  e il tetto per mercato e il 20% di 600, non di 1000', m.fatti.pianiChiesti[0].maxPerMarketUsd === 120,
  ...
  // Dal 7 agosto 2026 il tetto è 20% e non più 30%: era l'unico numero rimasto a esistere in due
  // versioni: qui il 30% del pianificatore, in motore-unico il 20% che il quoting applicava davvero.
  console.log('\n══ IL TETTO DEL 20% È LO STESSO NUMERO DEL PANNELLO «OTTIMIZZA» E DEL MOTORE');
  {
    const { CONCENTRATION_CAP_FRAC: dalModulo, capPerMarketUsd } = require('../rewards/concentration');
    const { MARKET_CAP_PCT } = require('./motore-unico');
    ok('il ciclo e il modulo condiviso espongono LO STESSO valore', CONCENTRATION_CAP_FRAC === dalModulo, String(dalModulo));
    ok('  ed è il 20%', dalModulo === 0.20);
    ok('  e il motore di piazzamento non ne ha un altro', MARKET_CAP_PCT === dalModulo, String(MARKET_CAP_PCT));
    ok('capPerMarketUsd(665.71) = 133.14', capPerMarketUsd(665.71) === 133.14, String(capPerMarketUsd(665.71)));
    ok('capitale non utilizzabile ⇒ null (= nessun tetto), mai zero', capPerMarketUsd(0) === null && capPerMarketUsd(null) === null && capPerMarketUsd(NaN) === null);
  }

...
(async () => {
  ...
  console.log('\n══ IL CAPITALE NON PUO SUPERARE IL TETTO DI ESPOSIZIONE APERTA');
  {
    ...
    // Il tetto per mercato NON dipende piu dal capitale, quindi resta $130 anche quando il tetto di
    ...
    ok('  e il tetto per mercato resta $130 FISSO, indifferente al taglio dell esposizione',
  ...
  // versioni — ed e' ora piu' grave, perche' un pianificatore che propone $130 contro un motore che
  ...
  console.log('\n══ IL TETTO È LO STESSO NUMERO PER PANNELLO, MOTORE, RIMPIAZZO E RISCHIO');
  {
    ...
    ok('  ed e $130 FISSO', dalModulo === 130);
    ...
    ok('capPerMarketUsd(665.71) = 130 — non piu una frazione', capPerMarketUsd(665.71) === 130, String(capPerMarketUsd(665.71)));
    ok('capitale non utilizzabile ⇒ tetto PIENO, mai null (null varrebbe «nessun tetto»)',
      capPerMarketUsd(0) === 130 && capPerMarketUsd(null) === 130 && capPerMarketUsd(NaN) === 130);
    ok('il numero di mercati e una conseguenza: $594 ⇒ 5', mercatiNecessari(594.10) === 5);
  }

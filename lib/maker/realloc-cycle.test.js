...
(async () => {
  ...
  console.log('\n══ UN MERCATO DEL PIANO NUOVO È GIÀ RISOLTO ⇒ ESCE E IL CAPITALE VA ALTROVE');
  {
    ...
    ok('  e il ricalcolo ha tenuto lo stesso capitale e lo stesso tetto', (() => {
      ...
      return rif.capital === 1000 && rif.maxPerMarketUsd === 300;
    })());
    ...
    ok('  il capitale impegnato non è calato: è stato riallocato, non perso', m.fatti.resetChiamato[0].rows.length === 2);
  }
  ...
  console.log('\n══ IL TETTO DEL 30% È LO STESSO NUMERO DEL PANNELLO «OTTIMIZZA»');
  {
    ...
    ok('  ed è il 30%', dalModulo === 0.30);
    ok('capPerMarketUsd(665.71) = 199.71', capPerMarketUsd(665.71) === 199.71, String(capPerMarketUsd(665.71)));

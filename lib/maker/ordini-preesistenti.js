 ...
 */
/**
 * ⚠⚠ L'ADOZIONE — 19 agosto 2026, decisione dell'operatore.
 *
 * LA REGOLA ESISTEVA PER NON TOCCARE GLI ORDINI DI UNA SESSIONE PRECEDENTE, e va tenuta. Ma non
 * distingueva «sessione precedente» da «me stesso, riavviato trenta secondi fa»: col piazzamento
 * armato significava che **ogni deploy condannava il libro alla morte per GTD**. Misurato il 18
 * agosto: riavvio alle 22:44:45, sei ordini scaduti alle 22:50 senza rinnovo, ~8 minuti di finestra
 * vuota su tre mercati. Il rinnovo proattivo parte 180 s prima della scadenza, e il riavvio e' caduto
 * tre minuti prima di quella finestra.
 *
 * SI ADOTTA SOLO CIO' CHE E' PROVATAMENTE NOSTRO, e servono TUTTE E TRE:
 *   ① `origine === 'auto'` **provata dal registro** — non dedotta dal `source`, che vale `manual-ui`
 *      sia per il bot sia per una mano umana ed e' quindi ambiguo per costruzione;
 *   ② il mercato e' nel PIANO CORRENTE;
 *   ③ il token corrisponde a uno dei due del mercato.
 * Tutto il resto — `manual`, `auto-chiusura`, `ignota`, mercati fuori piano — resta invisibile.
 *
 * ⚠ L'ASIMMETRIA CHE DECIDE LA DIREZIONE: non adottare un ordine nostro costa la sua morte per GTD in
 * 23 minuti, cioe' il comportamento di prima. Adottarne uno NON nostro significa riprezzare o
 * cancellare qualcosa che una persona ha messo a mano. Quindi il dubbio lascia l'ordine dov'e'.
 *
 * ⚠ MISURA CHE HA PRECEDUTO IL CODICE: 16 pre-esistenti distinti sui riavvii della notte del 18,
 * **tutti `origine: auto`, zero `ignota`**. Il caso `ignota` nasce quando il giornale ha ruotato oltre
 * l'ordine (tiene ~20 h) e con GTD a 23 minuti non puo' accadere in regime. ⚠ Il campione non
 * conteneva NESSUN ordine piazzato a mano — il pannello non e' nella flotta — quindi il ramo `manual`
 * resta non esercitato dai dati, ed e' esattamente perche' la regola deve continuare a escluderlo.
 *
 * @param deps.adotta  (ordine) => boolean — assente ⇒ NESSUNA adozione, cioe' il comportamento di prima.
 */
function fotografaPreesistenti({ listed = null, now = Date.now(), motivo = 'avvio', file = FILE,
  ...
  for (const o of listed.orders || []) {
    const id = idDi(o);
    if (!id) continue;
    // ⚠ L'ADOZIONE VIENE PRIMA DELLA MARCATURA: un ordine adottato non entra nel deposito, quindi
    // resta VISIBILE al motore e viene riprezzato e rinnovato come se il processo non si fosse mai
    // fermato. Fail-closed: qualunque eccezione del predicato vale «non adottare».
    if (typeof adotta === 'function') {
      let ok = false;
      try { ok = adotta({ orderId: id, ...o }) === true; } catch { ok = false; }
      if (ok) { adottati.push({ orderId: id, marketId: o.marketId ? String(o.marketId) : null }); continue; }
    }
  ...
  return {
    scattata: true, marcati: elenco.length, ordini: elenco, adottati,

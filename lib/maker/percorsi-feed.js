...
// ⚠ I DUE PERCORSI DI PRODUZIONE NON SONO PIU' IN `/tmp` NUDO (17 agosto 2026, migrazione root → bot):
// stanno nella directory di servizio **per utente** di `lib/percorsi-runtime.js`. `/tmp` condiviso ha
// prodotto il guasto peggiore possibile per questo modulo — i file di ieri erano di `root` con permessi
// 644 e lo sticky bit, quindi l'utente nuovo non poteva ne' riscriverli ne' cancellarli, **ma tutti i
// lettori continuavano a leggerli**: prezzi di quaranta minuti prima presentati come di adesso.
// Il nome dei due file NON cambia — cambia solo la directory, e in un punto solo.
const { fileRuntime } = require('../percorsi-runtime');

const PRODUZIONE = {
  get bookVivi() { return fileRuntime('clob-live-books.json'); },
  get boardNormalizzato() { return fileRuntime('liquidity-rewards.json'); },
};

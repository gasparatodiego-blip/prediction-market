...
const mk = (mid, extra = {}) => ({
  readable: true, marketId: `0x${'a'.repeat(64)}`, tick: 0.01, maxSpreadCents: 2.25, minSize: 20,
...
{
  // Il motore unico è puro e non conserva niente: può essere chiamato a ogni giro senza che il giro
  // precedente ne influenzi il risultato. È il prerequisito per agganciarlo al ciclo.
  const arg = (levels) => ({
    marketId: 'M', side: 'BUY', bookLevels: levels, bandBounds: { lo: 0.46, hi: 0.54 },
    bandRadiusCents: 4, scoringMid: 0.50, ownOrders: [], tick: 0.01,
    saldoUsd: 1000, proposedSize: 10, proposedPrice: 0.49,
    liquiditaMediaShare: 300, liquiditaCampioni: 50,
  });
  const sano = [{ price: 0.51, size: 500 }, { price: 0.50, size: 300 }, { price: 0.49, size: 300 }];
  const sottile = [{ price: 0.51, size: 500 }, { price: 0.50, size: 2 }, { price: 0.49, size: 2 }];
  const seq = [sano, sottile, sano].map((l) => valutaMercato(arg(l)).ok);
  ok('il verdetto segue il book del momento', JSON.stringify(seq) === JSON.stringify([true, false, true]), seq.join(','));
  ok('  e un book sottile non lascia strascichi sul giro dopo', seq[2] === true);
}

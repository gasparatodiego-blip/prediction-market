...
const { checkDepthGuardRisk } = require('./depth-guard');
...
{
  // depth-guard è puro e non conserva niente: può essere chiamato a ogni giro senza che il giro
  // precedente ne influenzi il risultato. È il prerequisito per agganciarlo al ciclo continuo.
  const libroSano = [{ price: 0.50, size: 300 }];
  const libroSottile = [{ price: 0.50, size: 10 }];
  const arg = (levels) => ({ marketId: 'M', side: 'BUY', proposedSize: 10, price: 0.50, tick: 0.01, restingBookAtLevel: levels, ownOrders: [] });

  const seq = [libroSano, libroSottile, libroSano].map((l) => checkDepthGuardRisk(arg(l)).allowed);

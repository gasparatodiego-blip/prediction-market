...
// ── IL BERSAGLIO NON E' PIU' IL BORDO NUDO, ED E' UNA PROPRIETA' NON UN NUMERO (15 agosto 2026) ─────
// Dal 15 agosto il ramo «soli sul lato» si ferma `MAKER_DISTANZA_MARGINE_BORDO_TICK` tick DENTRO il
// bordo premiante: al bordo nudo l'ordine dista dal limite meno di un tick e oscilla dentro/fuori
// banda a ogni respiro del mid. Il test NON fotografa il nuovo numero — chiede alla stessa funzione
// che decide, cosi' resta vero a margine 0 (bordo nudo, comportamento di prima) come a margine 2.
const { bordiConMargine } = require('./distanza-obiettivo');
// ⚠ `maxSpreadCents` VA PASSATO: dal 15 agosto 2026 il margine è `max(1 tick, 0,22 × v)`, quindi
// senza la banda si calcolerebbe un margine diverso da quello che il codice applica davvero — e il
// test misurerebbe qualcosa che non è il comportamento.
const bersaglio = (q, tick, v) => {
  const b = bordiConMargine({ bandLo: q.bandLo, bandHi: q.bandHi, tick, maxSpreadCents: v });
  return b.applicato ? b.lo : q.bandLo;
};
...
console.log('── 1 · LATO VUOTO ⇒ PREZZO AL BORDO ESTERNO');
{
  ...
  ok('  al bordo INFERIORE della banda meno il margine (spazio bid: il più lontano dal mid ammesso)',
    p.price === bersaglio(p, 0.01, 3), `${c(p.price)}¢ su [${c(p.bandLo)}–${c(p.bandHi)}]¢`);
  ok('  e non e\' MAI fuori banda', p.price >= p.bandLo - 1e-12 && p.price <= p.bandHi + 1e-12);
  ...
  for (const mid of [0.03, 0.1, 0.5, 0.9, 0.97]) {
    for (const raggio of [0.5, 1, 2.25, 4.5, 15]) {
      for (const tick of [0.001, 0.01, 0.1]) {
        ...
        if (!q.ok) continue;
        prodotti += 1;
        if (q.price !== bersaglio(q, tick, raggio) || !(q.price > 0 && q.price < 1)
          || q.price < q.bandLo - 1e-12 || q.price > q.bandHi + 1e-12) fuori += 1;
      }
    }
  }
  ok(`sweep su ${prodotti} combinazioni: SEMPRE il bordo ammesso, MAI fuori dalla banda`, fuori === 0, `anomalie ${fuori}`);
}
...
console.log('\n── 3 · APPENA COMPARE UN CONCORRENTE SI TORNA A UN TICK DIETRO');
{
  ...
  // ⚠ `>=` E NON `>`, dal 15 agosto 2026, e non e' un ammorbidimento: da quando i soli si fermano un
  // tick DENTRO il bordo, su una banda stretta quel punto puo' COINCIDERE con «un tick dietro il
  // concorrente» (qui banda ±3¢ su mid 50¢: bordo 47¢, +1 tick = 48¢, e un tick dietro il 49¢ altrui
  // e' 48¢). La proprieta' vera non e' mai stata la disuguaglianza stretta: e' che la presenza di un
  // concorrente non puo' MAI spingere l'ordine piu' lontano dal mid di quanto stia da solo.
  ok('  cioè mai più lontano dal mid di dove sta da solo', conLui.price >= soli.price, `${c(conLui.price)}¢ vs ${c(soli.price)}¢`);

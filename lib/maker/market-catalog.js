...
function upsertMarket(market = {}, { by = null, reason = null } = {}, deps = {}) {
  ...
  const record = {
    ...
    // ⚠ IL MINIMO D'ORDINE DEL VENUE, CHE NON E' IL PAVIMENTO PREMIANTE (lib/maker/minimi-del-venue).
    // Assente ⇒ null, mai 0: uno zero direbbe «nessun minimo», cioe' fail-open sulla domanda
    // «il venue accetterebbe quest'ordine?». Chi non lo popola lascia null, e il percorso
    // d'uscita risponde ILLEGGIBILE invece di indovinare.
    minOrderSize: fin(market.minOrderSize) ? market.minOrderSize
      : (fin(market.minimum_order_size) ? market.minimum_order_size : null),
...
function recordDaRigaBoard(row, fetchedAt = null) {
  ...
  const out = {
    ...
    minOrderSize: fin(row.minOrderSize) ? row.minOrderSize
      : (fin(row.minimum_order_size) ? row.minimum_order_size : null),
...
/**
 * UNA LETTURA DEL CLOB (`verifica-mercati-venue.leggiVenueClob`) → L'INGRESSO DI `upsertMarket`.
 * Pura: nessuna lettura, nessuna rete. GEMELLA di `recordDaRigaBoard`, e sta QUI per la stessa ragione
 * scritta la': il formato del record e' una proprieta' del catalogo, non di chi lo riempie. Due mapper
 * in due file sarebbero due definizioni dello stesso record — il reperto D1.
 *
 * ═══ PERCHE' SERVIVA UNA SECONDA SORGENTE (18 agosto 2026) ══════════════════════════════════════════
 * Il ripiego finora si riempiva da UNA fonte sola: una riga di board. Ma un mercato che esce dal board
 * e non e' mai passato di li' non ha nessun modo di entrarci — ed e' esattamente il caso che conta,
 * perche' un mercato fuori dal board e' lo stato normale di una posizione vecchia. Misurato lo stesso
 * giorno su Hong Kong `0xe9b3e28d`: assente dal board (120 righe), assente dal catalogo, e **assente
 * anche da Gamma** («mercato non trovato su Gamma per questo conditionId», 12 tentativi in un giorno).
 * Il CLOB invece lo conosce, e porta tutto: tick 0,001 · negRisk true · i due token · min_size 20 ·
 * max_spread 4,5 · end_date_iso. La terza fonte c'era, non aveva un mapper.
 *
 * ⚠ NON INVENTA NIENTE, esattamente come la gemella: un campo che la lettura non porta resta assente e
 * `upsertMarket` rifiuta il record se mancano i quattro obbligatori. Un tick indovinato produce ordini
 * fuori banda; un tick mancante produce un rifiuto leggibile.
 *
 * @param {string} marketId  il conditionId chiesto (la lettura non lo ripete)
 * @param {object} v         l'esito di `leggiVenueClob`
 * @param {number} [fetchedAt]
 * @returns {object|null} l'ingresso per `upsertMarket`, o null se la lettura non e' utilizzabile
 */
function recordDaLetturaVenue(marketId, v, fetchedAt = null) {
  const id = normId(marketId);
  if (!id || !v || typeof v !== 'object' || v.readable !== true) return null;
  const out = {
    marketId: id,
    question: v.question || null,
    slug: v.slug || null,
    tokenIdYes: v.tokenIdYes ? String(v.tokenIdYes) : null,
    tokenIdNo: v.tokenIdNo ? String(v.tokenIdNo) : null,
    tick: fin(v.tick) ? v.tick : null,
    negRisk: typeof v.negRisk === 'boolean' ? v.negRisk : null,
    rewardsDailyRate: fin(v.rewardsDailyRate) ? v.rewardsDailyRate : null,
    rewardsMaxSpreadCents: fin(v.maxSpreadCents) ? v.maxSpreadCents : null,
    // ⚠ IL CAMPO CHE TUTTO QUESTO ESISTE PER PORTARE: `min_incentive_size`. Vedi `min-size-mercato.js`.
    rewardsMinSize: fin(v.minSizeShares) ? v.minSizeShares : null,
    minOrderSize: fin(v.minOrderSize) ? v.minOrderSize
      : (fin(v.minimum_order_size) ? v.minimum_order_size : null),
    endDate: typeof v.endDate === 'string' && v.endDate.trim() ? v.endDate : null,
    endDateFonte: typeof v.endDate === 'string' && v.endDate.trim() ? 'clob' : null,
  };
  if (fin(fetchedAt)) out.fetchedAt = fetchedAt;
  return out;
}

module.exports = {
  readMarketCatalog, readMarketRecord, upsertMarket, removeMarket, missingFields,
  recordDaRigaBoard, recordDaLetturaVenue,

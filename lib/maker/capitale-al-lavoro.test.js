...
//   2 · il giro pianifica PIU' mercati e si ferma sui vincoli veri (minimo, obiettivo, tetto, rampa)
...
{
  // LA RAMPA VINCE SUL TARGET (Requisito 1.3): una protezione esistente non si allenta per arrivare al 90%.
  const g = TRIG.pianificaGiro({ righe: righeFinte, disponibileUsd: 500, notionalePerMercato: {}, capPerMercatoUsd: 124, nuoviAmmessi: 2 });
  ok('la rampa limita i mercati NUOVI', g.scelte.length === 2, `${g.scelte.length}`);
  ok('  e lo dichiara invece di allentarsi', /rampa esaurita/.test(g.motivoStop), g.motivoStop.slice(0, 40));

  // Un mercato già aperto NON consuma un posto della rampa.

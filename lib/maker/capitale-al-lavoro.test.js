...
// ════════════════════════════════════════════════════════════════════════════════════════════════════
{
  // CORRETTO IL 9 AGOSTO 2026 (CLAUDE.md §5 punto 58). Il fixture era `saldo 100, ordini 600,
  // posizioni 300` e pretendeva `totale 1000`, cioè le tre fonti sommate come indipendenti. Non lo
  // sono: un BUY a riposo è coperto dal cash e un SELL dai token, quindi `ordiniARiposo` è un
  // SOTTOINSIEME di `saldo + posizioni`. Con quel fixture gli ordini a riposo (600) superavano
  // addirittura il cash (100), che è uno stato impossibile per la parte BUY.
  // Il fixture ora è coerente — cash 700, di cui 600 coprono ordini a riposo, più 300 di posizioni —
  // e il PUNTO del test non cambia: il 90% è raggiunto al 90% esatto.
  const u = UTIL.misuraUtilizzo({ saldoUsd: 700, ordiniARiposoUsd: 600, posizioniUsd: 300 });
  ok('misura il totale come liquido + posizioni, senza contare due volte gli ordini',
    u.capitaleTotaleUsd === 1000, String(u.capitaleTotaleUsd));
  ok('l\'impegnato è ordini + posizioni', u.impegnatoUsd === 900, String(u.impegnatoUsd));
  ok('  e il libero è il cash che non copre ordini', u.liberoUsd === 100, String(u.liberoUsd));
...
(async () => {
  ...
  }
  {
    ...
    // 12 agosto 2026 — L'ASSERZIONE È STATA RAFFORZATA, NON TOLTA. Pretendeva la stringa
    // `forzatoDa: 'AVVIA appena premuto'`, cioè che l'AVVIA innescasse il MINI-ciclo: quello sceglie
    // dal piano salvato, quindi il piano vero ripartiva alla cadenza successiva e il capitale restava
    // fermo fino a dieci minuti. Adesso l'AVVIA innesca il ciclo COMPLETO — la stessa funzione della
    // cadenza, quindi stesse regole, stesso fail-closed, stesso freno — e questo è ciò che si pretende.
    ok('agent41 sorveglia l\'interruttore per l\'AVVIA a freddo', /function sorvegliaAvvio/.test(src));
    ok('  e l\'AVVIA innesca il ciclo COMPLETO, non il mini-ciclo',
      /eseguiGiro\('avvia-operatore'\)/.test(src) && /const eseguiGiro = deps\.giro \|\| giro/.test(src));
    ok('  con la cadenza riallineata al momento dell\'avvio',
      /ripianifica\('dopo il ciclo innescato dall/.test(src));

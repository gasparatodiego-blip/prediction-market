...
const {
  classifyRisk, bucketizza, etichettaScadenza,
...
console.log('\n══ LE SOGLIE VENGONO DA DOVE DICONO DI VENIRE');
{
  ...
  ok('la soglia Safe è MIN_HORIZON_DAYS, in minuti',
    // ⚠ QUI C'ERA ANCHE `=== 1080`, cioè una COPIA della costante travestita da verifica: si rompeva
    // ogni volta che la costante si muoveva pur non essendoci nessun difetto. Resta la derivazione,
    // che è la proprietà vera, più il fatto che sia un numero utilizzabile.
    SAFE_FLOOR_MINUTES === MIN_HORIZON_DAYS * 24 * 60 && SAFE_FLOOR_MINUTES > 0,
...
// ── IL PRE-FILTRO PER PROFILO È STATO RIMOSSO (6 agosto 2026) ────────────────────────────────────
// `filtraPerProfilo` esisteva per instradare i candidati dell'allocatore verso il profilo Safe o
// Risk. I profili non esistono più — la formula del venue è una curva continua e non conosce bucket —
// e la funzione era rimasta senza un solo chiamante. Le sue asserzioni sono uscite con lei: provavano
// un instradamento fra due percorsi di cui ne resta uno.
//
// `classifyRisk` e `bucketizza` restano e sono ancora usate: la console ne ricava l'impegnato e la
// segnalazione dei lati singoli. Le loro asserzioni, sopra, sono intatte.

  ...
  return new Promise((resolve) => {
    ...
    const req = mod.get(url, { timeout: VENUE_TIMEOUT_MS, headers: { accept: 'application/json' } }, (res) => {
      ...
      res.on('end', () => {
        ...
        // Il MINIMO D'ORDINE, che sta al primo livello della risposta e non dentro `rewards`.
        const minOrderSize = Number(j.minimum_order_size);
        const tick = Number(j.minimum_tick_size);
        // ── I TOKEN SI PRENDONO PER NOME DELL'ESITO, MAI PER POSIZIONE ─────────────────────────────
        // L'ordine dell'array `tokens` non e' documentato, e scambiare YES con NO significherebbe
        // comprare il lato sbagliato. Un esito che non si chiama ne' «yes» ne' «no» lascia entrambi
        // `null`, e a valle un record di catalogo senza i due token viene RIFIUTATO da `upsertMarket`:
        // e' il verso giusto — meglio nessun ripiego che un ripiego coi lati invertiti.
        const toks = Array.isArray(j.tokens) ? j.tokens : [];
        const tokDi = (nome) => {
          const t = toks.find((x) => x && typeof x.outcome === 'string' && x.outcome.trim().toLowerCase() === nome);
          return t && t.token_id ? String(t.token_id) : null;
        };
        resolve({
          readable: true,
          ...
          // ⚠ IL MINIMO D'ORDINE DEL VENUE, campo `minimum_order_size` della risposta di
          // `GET /markets/<conditionId>`. NON e' `rewards.min_size` (letto qui sopra come
          // `minSizeShares`): quello e' il pavimento PREMIANTE e dice «reward zero», questo dice «il
          // venue rifiuta l'ordine». Vedi `lib/maker/minimi-del-venue.js`.
          // ⚠ Assente o non positivo ⇒ `null`, mai 0: uno zero direbbe «nessun minimo», cioe'
          // fail-open sulla domanda a cui si risponde con capitale (§5.3, `Number(null) === 0`).
          ...
          // ── QUELLO CHE SERVE A SCRIVERE UN RECORD DI CATALOGO COMPLETO (18 agosto 2026) ──────────
          // Aggiunta PURAMENTE ADDITIVA: `market-validity` legge campi per nome e non ne conosce
          // nessuno di questi, quindi nessun verdetto esistente cambia. Servono perche' il catalogo di
          // ripiego RIFIUTA i record parziali (`market-catalog.REQUIRED_FIELDS`): senza tick, token e
          // negRisk una lettura del venue riuscita non si potrebbe salvare, e la terza fonte sarebbe
          // una lettura che non arriva mai a destinazione. Vedi `min-size-mercato.js`.
          tick: Number.isFinite(tick) && tick > 0 ? tick : null,
          // negRisk decide QUALE contratto regola l'ordine e il riscatto: assente ⇒ `null`, mai `false`.
          negRisk: typeof j.neg_risk === 'boolean' ? j.neg_risk : null,
          tokenIdYes: tokDi('yes'),
          tokenIdNo: tokDi('no'),
          question: typeof j.question === 'string' ? j.question : null,
          slug: typeof j.market_slug === 'string' ? j.market_slug : null,
        });
      });
    });

...
const { rischioBeneficio } = require('../rewards/rischio-beneficio');
const { tickOra: tickOraDi, FINESTRA_MIN: FINESTRA_VELOCITA_MIN } = require('./cadenza-adattiva');
const { inventoryWallet } = require('./inventory-read');
// Chi c'era gia' quando il motore si e' avviato. Serve solo a ETICHETTARE: la dashboard mostra tutto
// cio' che e' sul venue, e dice quali righe il bot non sta governando.
const { idsPreesistenti } = require('./ordini-preesistenti');
...
// Una definizione sola per i due percorsi del feed: vedi `./percorsi-feed`.
...
function classifySpread(bookSpread, maxSpreadCents) {
  ...
  const half = raggioBandaCents(maxSpreadCents);
...
async function buildMarketBoard(deps = {}) {
  ...
  // ── L'ESCURSIONE DEL MID, UNA LETTURA PER CHIAMATA ────────────────────────────────────────────
  // Alimenta l'asse «volatilità» del punteggio rischio/beneficio, ed è la STESSA misura del filtro
  // «⚡ Veloci» e della cadenza adattiva: finestra da 15 minuti, `tickOra`. Una lettura sola del
  // giornale per l'intera pagina (~30 ms, vedi lib/rewards/una-lettura-per-ciclo). Se fallisce,
  // l'asse resta cieco e il punteggio lo DICHIARA — non si inventa un mercato immobile.
  let finestraMid = null;
  try {
    const { leggiFinestraTutti } = require('../rewards/velocita-mercato');
    finestraMid = deps.finestraMid !== undefined ? deps.finestraMid : leggiFinestraTutti({ windowMinutes: FINESTRA_VELOCITA_MIN });
  } catch { finestraMid = null; }
  ...
  const markets = poly.map((m) => {
    ...
    return {
      ...
      // ── RISCHIO/BENEFICIO — QUI SOLO IL DENOMINATORE, E PER UNA RAGIONE ────────────────────────
      // Questa rotta non pubblica $/giorno (vedi il blocco «economics» qui sopra: dipende dal capitale,
      // e l'unico capitale onesto è il saldo reale che la console legge una volta). Quindi qui viaggia
      // il FATTORE DI RISCHIO con i suoi quattro componenti — che non dipendono dal capitale, tranne la
      // concentrazione, che resta cieca e lo dichiara — e la console divide il proprio $/giorno, già
      // prezzato al capitale vero, ottenendo il punteggio ordinabile senza che due numeri sulla stessa
      // pagina possano contraddirsi.
      rischioBeneficio: rischioBeneficio({
        beneficioUsdGiorno: null,
        tickOra: tickOraDi(finestraMid && finestraMid.per ? finestraMid.per.get(m.marketId) : null,
          fin(rules.tick) ? rules.tick * 100 : null),
        profonditaUsd: fin(m.bookDepthAtBand) ? m.bookDepthAtBand : null,
        giorniAllaRisoluzione: fin(m.hoursToResolution) ? m.hoursToResolution / 24 : null,
        capitaleSulMercatoUsd: null, capitaleTotaleUsd: null,
      }),
...
async function buildOrderBoard(deps = {}) {
  ...
  // ── I PRE-ESISTENTI RESTANO VISIBILI, MA ETICHETTATI ─────────────────────────────────────────
  // L'invisibilita' e' del MOTORE, non dell'operatore: questi ordini sono sul venue, si vedono li', e
  // nasconderli qui creerebbe la discrepanza peggiore — una dashboard che mostra meno capitale di
  // quanto ce n'e' davvero. Si mostrano con la loro etichetta, e i totali dicono quanto di quel
  // capitale il bot non sta governando.
  const preesistenti = idsPreesistenti();
  const orders = (listed.orders || []).map((o) => {
    const g = judgeOrder(o, rulesFor(o.marketId));
    return { ...g, preesistente: !!(g.orderId && preesistenti.has(String(g.orderId))) };
  });
  ...
  return {
    ...
    totals: {
      ...
      // Quanto del capitale a riposo il bot NON sta governando. `committedUsd` resta il totale vero di
      // cio' che e' sul venue: questa e' la sua parte non gestita, non una sottrazione da esso.
      preesistentiCount: orders.filter((o) => o.preesistente === true).length,
      preesistentiUsd: r2(orders.reduce((s, o) => s + (o.preesistente === true && fin(o.restingNotionalUsd) ? o.restingNotionalUsd : 0), 0)),
    },
  };
}

...
const { estimatedOperatorSharePerDay } = require('../reward-operator-estimate');
...
async function buildMarketBoard(deps = {}) {
  ...
  const markets = poly.map((m) => {
    ...
    // The same operator estimate the public board leads with, capped by the REAL in-band depth.
    const est = estimatedOperatorSharePerDay(m.rewardScore, { inBandDepthUsd: m.bookDepthAtBand });
    ...
    return {
      ...
      estUsdPerDay: est.unknown ? null : r2(est.estUsdPerDay),
      estShare: est.unknown ? null : est.share,
      estAssumedCapitalUsd: est.cappedCapitalUsd != null ? est.cappedCapitalUsd : est.assumedOrderSizeUsd,
      estCapped: !!est.capitalCapped,
      estCapNote: est.capNote,
...
/**
 * The same operator estimate the board shows, priced at the capital the operator ACTUALLY has resting
 * instead of the feed's $1,000 reference.
 *
 * NO SECOND MODEL. lib/reward-operator-estimate already contains the exact rescale this needs — it uses
 * it to cap the reference capital at the book's real in-band depth, with the algebra spelled out there:
 * share = Qu/(Qu+Qc) is proportional to capital, so at capital ratio r the share becomes
 * r·share/(r·share + (1−share)). Re-pricing at the operator's capital IS that same rescale with a
 * different r, so it is delegated to that function rather than reimplemented here. The cap passed is the
 * MINIMUM of the operator's capital and the measured in-band depth, so an order bigger than the book it
 * sits in is still priced against the book that exists.
 *
 * @returns {{ estUsdPerDay:number|null, share:number|null, capitalUsd:number, unknown:boolean }}
 */
function estimateAtCapital(rewardScore, capitalUsd, inBandDepthUsd) {
  if (!fin(capitalUsd) || capitalUsd <= 0) {
    return { estUsdPerDay: 0, share: 0, capitalUsd: 0, unknown: false };
  }
  const cap = fin(inBandDepthUsd) && inBandDepthUsd >= 0 ? Math.min(capitalUsd, inBandDepthUsd) : capitalUsd;
  const est = estimatedOperatorSharePerDay(rewardScore, { inBandDepthUsd: cap });
  if (est.unknown) return { estUsdPerDay: null, share: null, capitalUsd: cap, unknown: true };
  return { estUsdPerDay: r2(est.estUsdPerDay), share: est.share, capitalUsd: cap, unknown: false };
}

/**
 * The four header numbers and the Riepilogo's two findings, derived ONCE from the market board and the
 * judged order list so every section quotes the same figures.
 *
 * The $/day here is deliberately NOT the board's $1,000-reference number: it is priced at the capital
 * the operator has resting IN BAND right now, market by market. Out-of-band capital contributes exactly
 * zero — that is the whole point of the out-of-band alert, and a headline that paid it anyway would
 * contradict the alert sitting next to it.
 */
function buildSummary(markets, orderBoard) {
  ...
  // BEST MARKET BY RETURN, not by pot size: the pot is what everyone shares, the daily yield on the
  // capital the estimate is priced for is what the operator gets. Ranked only over markets whose rules
  // are readable and whose estimate exists — an unscored market is not silently ranked last, it is out.
  const ranked = (markets || [])
    .filter((m) => m.rulesReadable && fin(m.estUsdPerDay) && fin(m.estAssumedCapitalUsd) && m.estAssumedCapitalUsd > 0)
    .map((m) => ({
      marketId: m.marketId,
      title: m.title,
      estUsdPerDay: m.estUsdPerDay,
      assumedCapitalUsd: m.estAssumedCapitalUsd,
      dailyPoolUsd: m.dailyPoolUsd,
      yieldPctPerDay: r2((m.estUsdPerDay / m.estAssumedCapitalUsd) * 100),
      spreadLevel: m.spread.level,
      stabilityLabel: m.stability.label,
      inBotUniverse: m.inBotUniverse,
    }))
    .sort((a, b) => (b.yieldPctPerDay || 0) - (a.yieldPctPerDay || 0));

...
console.log('\n══ 7 · IL MINI-CICLO NON CONOSCE LA CANCELLAZIONE — PROVA SUL SORGENTE');
{
  ...
  // Ogni `enabled: false` del file deve appartenere a uno dei meccanismi che hanno il diritto di
  // spegnere qualcosa, e sono tutti FUORI dal mini-ciclo. Un meccanismo nuovo va guardato con
  // attenzione, ed è per questo che il test lo fa cadere invece di contarli e basta.
  //
  // ⚠ IL TERZO È ENTRATO IL 15 AGOSTO 2026, e questo test l'ha intercettato prima che girasse una
  // volta sola — che è esattamente il suo mestiere. `rilasciaDallaSelezione` è il rilascio della
  // SELEZIONE AUTOMATICA DEI MERCATI: un mercato che ha smesso di rispettare i vincoli dell'operatore
  // (`minSize ≤ 20`, scadenza ≥ 48 h, niente meteo) non deve ricevere ordini nuovi.
  //
  // È stato ammesso qui dentro dopo aver verificato la sola proprietà che rende il permesso sicuro:
  // **spegne l'ingresso e non l'uscita**. La funzione tocca `setAutoReprice` e NIENTE altro — non
  // `setAutoClose`, non `setTracking`, non `cancelManualOrder` — quindi la posizione aperta resta
  // gestita dalla regola di copertura di §4.8. Un rilascio che spegnesse anche la via d'uscita
  // lascerebbe un mercato scaduto senza uscita, che è il guasto di §5-bis p.44; la riga qui sotto lo
  // verifica sul sorgente invece di fidarsi di questo commento.
  const rilascio = src.slice(src.indexOf('async function rilasciaDallaSelezione'), src.indexOf('function selezioneAttiva'));
  ok('  il rilascio della selezione automatica spegne SOLO l\'ingresso, mai la via d\'uscita',
    rilascio.length > 0 && /setAutoReprice\(/.test(rilascio)
      && !/setAutoClose\(/.test(rilascio) && !/setTracking\(/.test(rilascio) && !/cancelManualOrder\(/.test(rilascio),
    'un mercato rilasciato deve restare chiudibile, o §5-bis p.44 si ripete');
  ok('  e ogni `enabled: false` del file appartiene a un meccanismo dichiarato',
    (() => {
      const righe = src.split('\n').filter((l) => /enabled: false/.test(l) && !/^\s*(\*|\/\/)/.test(l));
      // ⚠ NON si allarga il pattern con un `setAutoReprice(` generico: sarebbe un varco largo quanto
      // il file, e il primo `enabled: false` scritto altrove passerebbe. Una riga è ammessa o perché
      // NOMINA uno dei due meccanismi storici, o perché appartiene LETTERALMENTE al corpo del rilascio
      // della selezione — corpo di cui l'asserzione qui sopra ha appena provato il perimetro.
      const dentroIlRilascio = new Set(rilascio.split('\n').filter((l) => /enabled: false/.test(l)));
      return righe.length > 0 && righe.every((l) => /setTracking\(|impostaBot\(/.test(l) || dentroIlRilascio.has(l));
    })(),
    'solo `setTracking` (ciclo 6h), `impostaBot` (fermo di sicurezza) e `rilasciaDallaSelezione` (selezione automatica) possono spegnere');

...
console.log('\n══ 7 · IL MINI-CICLO NON CONOSCE LA CANCELLAZIONE — PROVA SUL SORGENTE');
{
  ...
  ok('  non spegne il tracking e non tocca la allowlist di auto-reprice',
    !/setTracking|setAutoReprice/.test(corpo),
    'quella lista governa dove si PUÒ APRIRE per chi gira in live-min: allargarla da qui darebbe a un altro processo un permesso che nessuno gli ha chiesto');
  ...
  ok('  ma SOLO in acquisizione: mai `manual:false`, mai `enabled:false`',
    /manual: true/.test(prep) && /enabled: true/.test(prep)
      && !/manual: false/.test(prep) && !/enabled: false/.test(prep)
      && !/manual: false|enabled: false/.test(corpo),
    'rilasciare un mercato o spegnere una via d uscita resta del ciclo delle sei ore');
  ok('  e una scrittura fallita FERMA quel mercato invece di piazzarci sopra',
    /if \(!\(mn && mn\.ok\)\)/.test(prep) && /if \(!\(ac && ac\.ok\)\)/.test(prep));

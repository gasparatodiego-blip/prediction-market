...
console.log('\n── 5 · LA FONTE DEL BOARD È LA STESSA CHE LEGGE resolveMarketRules');
{
  ...
  // ⚠ RISCRITTO IL 17 AGOSTO 2026: la proprieta' («un file solo per i due lati») e' INTATTA e adesso e'
  // anche piu' forte, perche' il percorso non e' piu' un letterale in `manual-order` ma UNA definizione
  // in `lib/maker/percorsi-feed` che i cinque lettori e lo scrittore importano. Il test guardava la
  // STRINGA nel sorgente (§5.3, «un test che fotografa il codice invece della proprieta'») e sarebbe
  // rimasto rosso a proprieta' rispettata. Ora si chiede il percorso alla funzione che lo decide.
  const PERCORSI = require('./percorsi-feed');
  const atteso = PERCORSI.fileBoardNormalizzato({});
  ok('il board normalizzato ha UNA definizione, in percorsi-feed', typeof atteso === 'string' && atteso.endsWith('liquidity-rewards.json'), atteso);
  ok('  e agent41 copia da quello STESSO file', A41.BOARD_NORMALIZZATO === atteso, A41.BOARD_NORMALIZZATO);
  // E `manual-order` non ne tiene una copia propria: se tornasse un letterale, i due lati potrebbero
  // divergere di nuovo senza che nessuno lo dica.
  const moCodice = fs.readFileSync(path.join(__dirname, 'manual-order.js'), 'utf8')
    .split('\n').filter((l) => !/^\s*(\/\/|\*)/.test(l)).join('\n');
  ok('  e manual-order non ridichiara il percorso', !/'\/tmp\/liquidity-rewards\.json'/.test(moCodice));

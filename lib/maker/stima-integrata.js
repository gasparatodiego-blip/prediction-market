...
// La cartella dei dati viene SOLO da `safety/store`, mai calcolata da `__dirname`: un secondo modo di
// dire dov'e' `data/` e' un secondo posto dove i file possono finire, e `percorsi-dati.test.js` lo
// vieta a tutto `lib/`. Se il modulo di store non si carica non si inventa un percorso: si solleva, e
// chi chiama (il campionatore di agent40) ha gia' il suo catch — un campione perso e' un buco
// dichiarato, un file scritto nel posto sbagliato no.
function fileCampioni() {
  return path.join(require('../safety/store').DATA_DIR, 'stima-campioni.json');
}

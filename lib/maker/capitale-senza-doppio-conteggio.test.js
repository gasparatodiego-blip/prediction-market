...
console.log('\n── 2 · IL LIBERO È IL CASH MENO GLI ORDINI A RIPOSO');
{
  ...
  ok('  in TUTTI i punti in cui pianifica, non solo nel primo',
    (ag.match(/disponibileUsd: spendibileUsd/g) || []).length === 3
    && !/disponibileUsd: decisione\.saldoUsd/.test(ag), `${(ag.match(/disponibileUsd: spendibileUsd/g) || []).length} punti`);
...
console.log('\n── 3 · IL TETTO DI CONCENTRAZIONE TORNA AL 20% DEL CAPITALE VERO');
{
  ...
  ok('il tetto è il 20% del totale vero', cap === 133.82, `$${cap} (atteso $133.82)`);
  ok('  e non i $155,33 del totale gonfiato', cap !== capPerMarketUsd(776.65), `gonfiato era $${capPerMarketUsd(776.65)}`);
  ok('  cioè un limite di rischio più stretto del 16%',
    Math.abs((capPerMarketUsd(776.65) / cap - 1) * 100 - 16.1) < 0.3, `${((capPerMarketUsd(776.65) / cap - 1) * 100).toFixed(1)}%`);
  ok('la frazione di concentrazione non è stata toccata', CONCENTRATION_CAP_FRAC === 0.20);

...
const rules = (over = {}) => ({ readable: true, tick: 0.01, maxSpreadCents: 2.25,
...
console.log('\n══ IL CONFLITTO CON LA BANDA: VINCE «MAI PRIMI» (invertito il 5 agosto 2026)');
{
  ...
  const q = prezzoInCoda({ book: 'yes', side: 'BUY', rules: rules({ maxSpreadCents: 1.5 }),
...
console.log('\n══ SENZA DATI NON SI INVENTA UN PREZZO');
{
  ...
  ok('soli sul lato ⇒ ripiego sull offset dichiarato, non un prezzo a caso',
    prezzoInCoda({ book: 'yes', side: 'BUY', rules: rules(), depth: depth([], []), offsetCents: 1 }).mode === 'fallback-alone-bordo-esterno');
}
...
console.log('\n══ I TRE PERCORSI LO DICHIARANO, LA CHIUSURA FORZATA NO');
{
  ...
  // Dall'8 agosto 2026 auto-close ha piu' coppie di questi marcatori: l'uscita (maker in coda / chiusura
  // forzata che attraversa), il merge (Livello 2 in coda / Livello 1 che attraversa) e — dal 9 agosto —
  // la chiusura rapida (taker + limit) e il riposizionamento (solo in coda). Il numero cresce col
  // sistema; la proprieta' da difendere no, ed e' che restino righe DISTINTE: nessun punto del codice
  // deve poter essere insieme «aspetto il mio turno» e «attraverso adesso».
  ok('  e i due marcatori non sono mai lo stesso punto',
    (ac.match(/inCoda: true/g) || []).length >= 2 && (ac.match(/attraversaApposta: true/g) || []).length >= 2,
    `inCoda ${(ac.match(/inCoda: true/g) || []).length}, attraversa ${(ac.match(/attraversaApposta: true/g) || []).length}`);
  ok('  e non coesistono mai nello stesso oggetto', !/inCoda: true[^}]*attraversaApposta: true/.test(ac));

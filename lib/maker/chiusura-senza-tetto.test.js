...
const { LIVE_MIN_ORDER_CAP_USD } = require('../rewards/concentration');
  ...
  ok(`il tetto per ordine oggi è $${LIVE_MIN_ORDER_CAP_USD}`,
    LIVE_MIN_ORDER_CAP_USD === +(CONC.MARKET_CAP_FIXED_USD / 2 + CONC.MARGINE_ORDINE_USD).toFixed(2)
    || Math.abs(LIVE_MIN_ORDER_CAP_USD - (CONC.MARKET_CAP_FIXED_USD / 2 + CONC.MARGINE_ORDINE_USD)) < 0.01);

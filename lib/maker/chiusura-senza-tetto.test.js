...
const { LIVE_MIN_ORDER_CAP_USD } = require('../rewards/concentration');
  // DERIVATO, non ricopiato: dal 12 agosto 2026 il tetto per ordine nasce dal tetto per mercato, che
  // a sua volta nasce dal capitale. Un test che scrive il numero a mano diventa rosso a ogni ritaratura
  // senza segnalare nessun difetto — e' gia' successo con $25, $37,50 e adesso.
  // ⚠ LA DERIVAZIONE È CAMBIATA IL 15 AGOSTO 2026 e il test la segue invece di ricopiarla: non è più
  // «metà mercato + margine» (che è la gamba giusta solo a mid 0,49) ma «la gamba più cara che una
  // coppia già limitata dal tetto per mercato può produrre» — vedi `concentration.js`.
  ok(`il tetto per ordine oggi è $${LIVE_MIN_ORDER_CAP_USD}, e resta DERIVATO dal tetto per mercato`,
    Math.abs(LIVE_MIN_ORDER_CAP_USD
      - (CONC.MARKET_CAP_FIXED_USD * CONC.PREZZO_MASSIMO_QUOTABILE / CONC.COSTO_COPPIA + CONC.MARGINE_ORDINE_USD)) < 0.01,
    `$${LIVE_MIN_ORDER_CAP_USD} da un tetto per mercato di $${CONC.MARKET_CAP_FIXED_USD}`);
  ok('  ed è comunque ≥ della gamba più cara che il tetto per mercato consente: se no rifiuterebbe coppie valide',
    LIVE_MIN_ORDER_CAP_USD >= CONC.MARKET_CAP_FIXED_USD * CONC.PREZZO_MASSIMO_QUOTABILE / CONC.COSTO_COPPIA);
  // Una controparte costruita SOPRA il tetto, derivata invece che scritta: serve solo a provare che il
  // tetto morde quando non c'è la prova di chiusura.
  const controvalore = +(LIVE_MIN_ORDER_CAP_USD + 10).toFixed(2);
...
console.log('\n── 3 · LE DUE CINTURE ESENTANO GLI STESSI ORDINI, E SOLO IL CAP live-min');
{
  ...
  // Il Livello 1 (taker) e la sorella a riposo del Livello 2 escono dallo STESSO `placeOrder`, quindi
  // condividono una dichiarazione sola — ed è giusto così, comprano entrambi `manca` share per
  // completare la stessa coppia.
  // ⚠ SETTE DAL 13 AGOSTO 2026: si è aggiunto il ramo `riposizionamento-scoperto` (§5.2 p.18), che
  // riduce come gli altri — SELL di share possedute e BUY entro `manca` — e che prima era l'unico
  // percorso di chiusura a NON dichiararla. Misurato: una SELL di 52,6 share già possedute rifiutata
  // con `manual-order-cap`, e 1.331 rifiuti dello stesso gate nello slice recente.
  // ⚠ NOVE DAL 17 AGOSTO 2026 (era 8, ed era 7 il 13 agosto). L'aggiunta e' l'USCITA ORDINARIA, che
  // ora dichiara la chiusura SEMPRE e non solo quando attraversa. Il banco del ciclo completo ha
  // mostrato perche' serve: senza la dichiarazione i gate a valle trattano la vendita della posizione
  // come un'apertura, e un residuo sotto `min_incentive_size` viene rifiutato da tutti e tre i tetti —
  // cioe' resta incagliato fino alla risoluzione (§5.2 p.1, $26,30).
  // ⚠ E' un SELL di share gia' possedute: RIDUCE per costruzione, ed e' esattamente il caso che
  // §5-bis p.147 dichiara esente su TUTTI i percorsi che riducono. `provaChiusura` rifa' comunque
  // l'aritmetica sullo snapshot del venue, quindi la dichiarazione non e' una parola d'onore.
  ok(`auto-close dichiara la chiusura in ${dichiarazioni} punti (L1+sorella · incremento sorella · chiusura rapida · riposizionamento · i 2 rami pre-scadenza · uscita forzata · uscita ordinaria)`,
    dichiarazioni === 9, String(dichiarazioni));
  // ⚠ LA PROPRIETÀ CHE IL CONTEGGIO FA DA PROXY, asserita direttamente: la corsia che APRE non
  // dichiara mai la chiusura. Un conteggio che sale è legittimo solo finché resta vero questo.
  ok('  e la corsia di APERTURA (bulk-allocate) non la dichiara mai',
    !/chiudePosizione/.test(require('fs').readFileSync(path.join(__dirname, 'bulk-allocate.js'), 'utf8')));
  for (const [nome, f] of [
...
console.log('\n── 6 · IL GIORNALE DI PRODUZIONE NON È STATO TOCCATO');
{
  ...
  ok('il tetto della coppia è quello nuovo, e questo file lo legge dal modulo', CR.TETTO_COPPIA_CENTS === 101);
}
...
console.log('\n── 7 · IL PIAZZAMENTO DI CHIUSURA RIPROVA, MA SOLO CIÒ CHE HA SENSO RIPROVARE');
{
  ...
  // ⚠ SI CERCA LA CHIAMATA, NON LA FORMA DELL'ARGOMENTO. La prima stesura cercava
  //   `res = await deps.placeOrder({` — cioè fotografava anche la parentesi graffa, e il 23 agosto è
  //   diventata rossa quando l'uscita ordinaria ha cominciato a passare lo spec da `chiudendo(...)`
  //   per il GTD della corsia di chiusura. La PROPRIETÀ difesa qui non è come si costruisce lo spec:
  //   è che l'uscita ordinaria chiami `deps.placeOrder` DIRETTAMENTE, cioè senza la sequenza di
  //   ritentativi di `piazzaChiudendo`. Terza occorrenza della classe «test che fotografa il codice
  //   invece della proprietà» (§5.3).
...
console.log('\n── 9 · UN MERCATO MORTO NON RESTA IN SEI REGISTRI');
{
  ...
  // ⚠ DECISIONE RIBALTATA DALL'OPERATORE il 13 agosto 2026: il redeem AUTOMATICO è stato chiesto e
  // implementato, perché per un residuo sotto il minimo del venue la risoluzione è l'unica uscita e
  // «nessun chiamante» significava che il capitale tornava solo se un umano se ne accorgeva.
  // La proprietà da difendere non è più «nessuno lo chiama» ma **chi** lo chiama: il riscatto vive nel
  // suo percorso dedicato, e la pulizia del mercato chiuso continua a non toccarlo.
  ok('il redeem ha UN solo chiamante, ed è il riscatto automatico',
    srcA.includes('redeemPosition') && /riscattaRisolte/.test(srcA)
    && (srcA.match(/redeemPosition/g) || []).length <= 2);
  ok('  e la pulizia del mercato chiuso continua a NON riscattare', !srcP.includes('redeemPosition('));
}

...
// ── L'INTERRUTTORE ──────────────────────────────────────────────────────────────────────────────────
// ACCESO dal 9 agosto 2026, su istruzione esplicita dell'operatore in chat («CTF_RELAYER_ENABLED = true»).
// Con false ogni funzione costruisce, firma NIENTE e non invia niente: restituisce il piano. Con true
// firma e invia.
//
// PRIMA DI OGGI era spento: acceso una volta sola il 7 agosto 2026 per la prova split $2 -> merge $2 su
// Schwartzel (neg-risk) e rimesso a false a prova finita, nella stessa sessione. Restava spento perché
// non esisteva un chiamante: accenderlo non avrebbe fatto succedere niente.
//
// COSA CAMBIA ADESSO, ed è la ragione per cui la frase qui sotto non è più vera: dal 9 agosto
// `lib/maker/auto-close.fondiCoppia` chiama `mergePosition` quando una coppia YES+NO risulta COMPLETA
// (`decidiLivello` → `azione:'merge'`). Quindi da oggi questo interruttore governa un'operazione che
// parte DA SOLA, senza che nessuno la chieda, dentro il ciclo di agent40. Le altre due funzioni —
// `splitPosition` e `redeemPosition` — restano senza chiamanti: nessun percorso automatico le raggiunge.
//
// COSA NON CAMBIA: il confine. `calls` non è mai un parametro, `verificaConfinamento()` ri-decodifica il
// batch prima della firma e rifiuta qualunque target che non sia uno dei due adapter CTF, e nessun ramo
// chiama approve/transfer. L'interruttore decide QUANDO si firma, non COSA.
//
...
//
// QUEL GIORNO È ARRIVATO, ed era previsto qui sopra. Fino all'8 agosto 2026 questa riga diceva che
// l'unico file a fare `require` di questo modulo era il suo test, e che «se un giorno un chiamante
// automatico comparisse, l'interruttore va ripensato prima di aggiungerlo». Il chiamante è comparso il
// 9 agosto — `lib/maker/auto-close.fondiCoppia` — e l'interruttore è stato ripensato: adesso governa
// un'operazione automatica, non un'attrezzatura ferma.
//
// CHI FIRMA, E QUANDO: il ciclo di auto-close di agent40, quando trova una coppia YES+NO completa sullo
// stesso mercato. Non su un fill, non su un ordine: solo su una coppia già completa, che è l'unico stato
// in cui `mergePositions` ha senso. La spesa è ZERO — il merge converte token già nostri in collaterale
// e il gas lo paga il relayer.
const CTF_RELAYER_ENABLED = true;

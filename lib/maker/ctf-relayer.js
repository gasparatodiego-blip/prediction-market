...
//
// ═══ IL TRANELLO DELL'ENDPOINT NONCE ════════════════════════════════════════════════════════════════
// `/v1/account/transactions/params` SENZA credenziali risponde HTTP 200 — non 401 — con un indirizzo
// CASUALE (diverso a ogni chiamata) e nonce "5". Provato cinque volte il 7 agosto 2026. Un client
// ingenuo firmerebbe un batch con un nonce inventato e non capirebbe perché il submit fallisce. Per
// questo `leggiNonce()` PRETENDE che l'`address` restituito sia il nostro, e altrimenti solleva.
...
// ── L'INTERRUTTORE ──────────────────────────────────────────────────────────────────────────────────
// Spento. Come MANUAL_ORDER_PLACEMENT, si accende deliberatamente e solo dopo la prima esecuzione di
// prova approvata a voce. Con questo a false ogni funzione costruisce, firma NIENTE e non invia niente:
// restituisce il piano, che è esattamente ciò che serve per mostrarlo prima di autorizzarlo.
...
const CHAIN_ID = 137;
const DEADLINE_SEC = 240;          // 4 minuti, il valore raccomandato dall'SDK di riferimento
 ...
 *
 * PRETENDE che la risposta parli DEL NOSTRO indirizzo. Vedi «il tranello dell'endpoint nonce» in cima:
 * senza credenziali valide l'endpoint risponde 200 con un indirizzo casuale, e firmare su quel nonce
 * significa costruire un batch che non vale niente senza nessun messaggio d'errore comprensibile.
 */
async function leggiNonce({ signer, cred, http = richiesta }) {
  ...
  const tornato = String(r.json.address || '');
  if (tornato.toLowerCase() !== String(signer).toLowerCase()) {
    throw new ErroreRelayer(
      `il relayer ha risposto per ${tornato || '(nessun indirizzo)'} invece che per ${signer}: credenziali non valide. `
      + 'Senza chiave questo endpoint restituisce 200 con un indirizzo casuale e nonce "5" — firmare su quel nonce non avrebbe senso.',
      { indirizzoTornato: tornato, atteso: signer });
  }
...
async function esegui({ operazione, conditionId, negRisk, quantita = null, deps = {} }) {
  ...
  let nonce = '0';
  if (abilitato) nonce = await leggiNonce({ signer, cred, http });
  ...
  appendMakerAudit({ ...intento, fase: 'inviato', transactionID, stato: r.json.state || null, adapter: piano.adapter, confinamento: piano.confinamento });
...
async function attendiConferma({ transactionID, cred, http, deps = {} }) {
  ...
  for (let i = 0; i < tentativi; i += 1) {
    ...
    if (stato === 'STATE_CONFIRMED') return { stato, transactionHash: (r.json && (r.json.transactionHash || r.json.hash)) || null };
    if (stato === 'STATE_FAILED' || stato === 'STATE_INVALID') {
      return { stato, transactionHash: (r.json && r.json.transactionHash) || null, motivo: `stato terminale ${stato}`, risposta: r.testo };
    }

...
console.log('\n── IL RIPIEGO quando siamo gli unici sul lato');
{
  ...
  ok('nessun altro ⇒ BORDO ESTERNO della banda, non un offset dal mid',
    p.ok && p.price === p.bandLo, `${c(p.price)}c su banda [${c(p.bandLo)}-${c(p.bandHi)}]c`);
...
console.log('\n── il ripiego rispetta la banda come tutto il resto');
{
  ...
  ok('  ed e\' il bordo inferiore, cioe\' il piu\' lontano dal mid nello spazio bid', p.price === p.bandLo);

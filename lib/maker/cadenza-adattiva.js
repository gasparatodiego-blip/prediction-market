...
function decidiCadenza({ now, ultimaValutazioneMs = null, misura = null, tickCents = 1,
  ...
  const scaduta = (cadenzaMs) => {
    if (!fin(ultimaValutazioneMs)) return { valuta: true, attesaMs: 0 };   // mai visto ⇒ si guarda
    ...
    if (trascorso < 0) return { valuta: true, attesaMs: 0 };
    return { valuta: trascorso >= cadenzaMs, attesaMs: Math.max(0, cadenzaMs - trascorso) };
  };

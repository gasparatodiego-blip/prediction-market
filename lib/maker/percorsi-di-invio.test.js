...
console.log('\n══ 3 · L INVENTARIO COMPLETO DELLE STRADE VERSO IL VENUE');
{
  ...
  const PERCORSI = [
    ...
    // L'ECCEZIONE È SPARITA. Fino al 9 agosto 2026 c'era un percorso fuori funnel: agent35-maker
    // parlava direttamente ad `adapter.postOrder`, quindi non conosceva «mai primi sul libro» e non
    // dichiarava `inCoda`. Rimosso dalla flotta e dal repo insieme al meccanismo di ARMING: da oggi
    // ogni strada verso il venue passa da placeManualOrder, e l'asserzione qui sotto lo difende.
  ];
  ...
  ok('NESSUN percorso sfugge al funnel manuale',
    fuoriFunnel.length === 0,
    fuoriFunnel.length ? fuoriFunnel.map((x) => x.nome).join(', ') : 'tutti i percorsi passano da placeManualOrder/runBulkAllocation');

  // E che il percorso fuori funnel non possa RINASCERE in silenzio: nessun agent vivo deve parlare
  // direttamente all'adapter di piazzamento. È la stessa domanda del punto sopra, fatta al filesystem
  // invece che a un elenco scritto a mano — un elenco non aggiornato non fallirebbe da solo.
  const agentiVivi = fs.readdirSync(path.join(ROOT, 'agents')).filter((f) => f.endsWith('.js') && f.startsWith('agent'));
  const diretti = agentiVivi.filter((f) => /adapter\.postOrder\(/.test(leggi('agents', f)));
  ok('  e nessun agent chiama adapter.postOrder direttamente', diretti.length === 0, diretti.join(', ') || 'nessuno');
}

console.log('\n══ 4 · I DUE INTERRUTTORI DI INVIO RESTANO DUE');
{
  // Il primo interruttore era del motore automatico (MAKER_PLACEMENT), il secondo della corsia manuale
  // (MANUAL_ORDER_PLACEMENT). Il motore non c'è più, ma la proprietà che conta resta la stessa e va
  // difesa lo stesso: manual-order.js non deve MAI decidere di inviare guardando MAKER_PLACEMENT.
  const mo = leggi('lib', 'maker', 'manual-order.js');
  ...
  // ⚠ DAL 17 AGOSTO 2026 SERA `MANUAL_ORDER_PLACEMENT` NON E' PIU' LETTA DENTRO `manual-order.js`: la
  // lettura vive in `lib/maker/cinture-armamento.js` e qui si IMPORTA, perche' `scripts/cli/stato.js`
  // deve poter dire lo stato di quella cintura leggendo l'ambiente di un processo vivo senza importare
  // una superficie che sa piazzare. La proprieta' da difendere non e' cambiata di una virgola — i due
  // interruttori restano due, e `manual-order` non deve MAI decidere guardando `MAKER_PLACEMENT` — ma
  // il posto in cui si verifica sì. Questo test si e' fatto rosso al primo giro dopo lo spostamento, ed
  // e' il comportamento giusto: era il solo a sorvegliare quella riga.
  //
  // Si segue quindi l'import invece di fotografare la posizione: e' la stessa lezione di §5.3 («si
  // difende la PROPRIETA', non il conteggio»). Il test cade ancora se qualcuno legge `MAKER_PLACEMENT`
  // dentro `manual-order`, e cade anche se la lettura di `MANUAL_ORDER_PLACEMENT` sparisce da entrambi i
  // posti — cioe' se la cintura smette di esistere.
  const cin = leggi('lib', 'maker', 'cinture-armamento.js').replace(/^\s*\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  const importa = /require\(['"]\.\/cinture-armamento['"]\)/.test(senzaCommenti);
  const leggeManuale = /MANUAL_ORDER_PLACEMENT/.test(cin) && importa;
  ...
  ok('gli interruttori di invio sono due, non uno',
    ...
    `manuale letto: ${leggeManuale} (via cinture-armamento, importata: ${importa}) · motore letto: ${leggeMotore}`
    + ' — nessun armamento per effetto collaterale');
  // E la funzione importata deve essere DAVVERO la stessa: un import che non viene usato lascerebbe
  // passare una seconda copia scritta a mano poche righe sotto.
  const MO = require('./manual-order');
  const CIN = require('./cinture-armamento');
  ok('  e la lettura e la STESSA funzione, non una copia', MO.manualPlacement === CIN.manualPlacement);
  ok('  `send` esatto arma, qualunque altra cosa no',
    MO.manualPlacement({ MANUAL_ORDER_PLACEMENT: 'send' }) === 'send'
    && MO.manualPlacement({ MANUAL_ORDER_PLACEMENT: 'SEND' }) === 'dry-run'
    && MO.manualPlacement({}) === 'dry-run');
}
...
console.log('\n══ 5 · LE REGOLE NUOVE SONO PURE — NESSUNA PUÒ ESSERE AGGIRATA DA UNA CACHE');
{
  ...
  for (const modulo of ['motore-unico']) {

...
console.log('\n══ 5 · SUI DATI VERI DEL REGISTRO — quante cancellazioni si sarebbero riconosciute');
{
  ...
  if (!fs.existsSync(f)) {
    ...
    ok(`nel registro ci sono cancellazioni con elenco valorizzato`, conElenco.length > 0,
      `${conElenco.length} su ${canc.length} chiamate`);
    ok('  e sono TUTTE con l id redatto — cioè tutte lette come fallimento',
      redatti.length === conElenco.length,
      `${redatti.length}/${conElenco.length} · sono i record scritti PRIMA di questa correzione`);

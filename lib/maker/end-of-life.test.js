...
(async () => {
  ...
  console.log('\n── se cancelli tu dall app, il motore se ne accorge');
  {
    ...
    b.resting().length = 0;
    const r = await b.run();
    // PUNTO 6 · IL CONTRATTO E CAMBIATO. Prima un ordine sparito veniva letto come «gone» e il lato
    // ripiazzava; adesso e' trattato come ESEGUITO e il lato si ferma. E' l'asimmetria che permetteva di
    // ricomprare piu' in basso a ripetizione su un mercato in discesa.
    ok('l ordine sparito viene trattato come ESEGUITO', (r.events || []).some((e) => e.type === 'fill' && e.kind === 'totale-presunto'),
      'lettura conservativa: delle tre cause possibili, «eseguito» e l unica che lascia esposizione');
    ok('  e il motore NON ne ripiazza uno nuovo', b.placed.length === 1,
      'il lato si ferma: riquotare mentre si esce raddoppierebbe la posizione che si sta chiudendo');
    ok('  il lato risulta eseguito nello stato', b.state.get(MKT.toLowerCase()).sides.yes.filled === true);
  }

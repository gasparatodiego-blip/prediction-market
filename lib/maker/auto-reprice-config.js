...
const DEFAULTS = Object.freeze({
  ...
  // ⚠ DERIVATA DA `mid-stantio`, NON RIDICHIARATA (16 agosto 2026). Era 60 mentre la cancellazione per
  // cecita' stava a 20 e poi a 120: tre valori per la stessa domanda, e l'asimmetria ha lasciato un
  // ordine vivo e non riprezzabile finche' e' morto per GTD. Ora la soglia e' UNA, e chi la vuole la
  // importa: scollegarle di nuovo richiede di togliere questo require, non di dimenticare un numero.
  maxMidAgeSecLive: require('./mid-stantio').MAX_MID_AGE_SEC,
...
function readAutoRepriceConfig(deps = {}) {
  ...
  // ── I MERCATI DOVE ABBIAMO UNA POSIZIONE APERTA ENTRANO NELLA ALLOWLIST ─────────────────────────
  // Terza e ultima volta, il 9 agosto 2026, che la stessa lacuna si presenta a un livello diverso: una
  // regola seguiva il TABELLONE invece di «tabellone ∪ mercati con posizione». Prima il catalogo dei
  // metadati, poi la sottoscrizione del book (§5 punto 61), adesso la allowlist che il gate live-min
  // legge. Effetto misurato alle 11:48: London 18°C e Chengdu, appena tornati VISIBILI nel book,
  // venivano rifiutati con `live-min-market-mismatch` — visti ma intoccabili, con capitale dentro.
  //
  // NON ALLARGA IL PERIMETRO DI RISCHIO, e va detto con precisione: aggiunge solo mercati dove il
  // capitale E' GIA' ESPOSTO. Non apre un mercato nuovo — apre la gestione di una posizione che
  // esiste. Il verso e' quello della RIDUZIONE del rischio, ed e' lo stesso principio per cui
  // `evaluateReductionProof` lascia passare un ordine che riduce anche fuori allowlist.
  //
  // FAIL-CLOSED, come le altre due volte: snapshot illeggibile ⇒ non si aggiunge NIENTE. E resta
  // subordinato all'interruttore generale: con `globalEnabled` falso la lista e' vuota, posizioni o no.
  const daPosizione = [];
  if (globalEnabled) {
    try {
      const { readVenuePositions } = require('../safety/venue-positions-snapshot');
      const snap = deps.posizioni !== undefined ? deps.posizioni : readVenuePositions();
      if (snap && snap.readable === true) {
        for (const pos of snap.positions || []) {
          const cid = String((pos && (pos.conditionId || pos.marketId)) || '').trim().toLowerCase();
          if (!cid || !(Number(pos.size) > 0)) continue;
          if (!daPosizione.includes(cid)) daPosizione.push(cid);
        }
      }
    } catch { /* illeggibile ⇒ nessuna aggiunta: non si apre niente su una lettura fallita */ }
  }
  // ── E I MERCATI DOVE ABBIAMO ORDINI A RIPOSO — 18 agosto 2026 ───────────────────────────────────
  // QUARTA volta che la stessa lacuna si ripresenta, e la prima che e' costata su capitale vero: alle
  // 16:32 il bot ha piazzato due ordini su 0x1f1c6390, alle ~16:42 quel mercato e' uscito dal BOARD,
  // e' stato rilasciato dalla selezione, e alle 16:55:08 la sua GTD e' scaduta senza che nessuno
  // rinnovasse. Bot armato e fuori dal libro per 52 minuti.
  //
  // §4.8 dichiarava questo buco per intero — «l'unione e' abilitati ∪ posizioni perche' non esiste uno
  // snapshot locale degli ordini a riposo» — e la mitigazione accettata era «l'ordine muore per GTD in
  // 23 minuti o si riempie». Adesso lo snapshot esiste, quindi la meta' scoperta si copre.
  //
  // ⚠ NON ALLARGA IL PERIMETRO DI RISCHIO: aggiunge solo mercati dove il capitale e' GIA' a libro. Non
  // apre un mercato nuovo — tiene aperta la gestione di ordini che esistono gia'. Senza, l'alternativa
  // non e' «meno esposizione»: e' esposizione che nessuno sta guardando.
  //
  // FAIL-CLOSED e subordinato all'interruttore generale, identico a `daPosizione`.
  const daOrdini = [];
  if (globalEnabled) {
    try {
      const { readVenueOrders } = require('../safety/venue-orders-snapshot');
      const snap = deps.ordini !== undefined ? deps.ordini : readVenueOrders();
      if (snap && snap.readable === true) {
        for (const id of snap.marketIds || []) {
          const cid = String(id || '').trim().toLowerCase();
          if (cid && !daOrdini.includes(cid)) daOrdini.push(cid);
        }
      }
    } catch { /* illeggibile ⇒ nessuna aggiunta: non si apre niente su una lettura fallita */ }
  }
  const abilitati = globalEnabled ? Object.keys(markets).filter((k) => markets[k] && markets[k].enabled === true) : [];
  const unione = abilitati.slice();
  for (const id of daPosizione) if (!unione.includes(id)) unione.push(id);
  for (const id of daOrdini) if (!unione.includes(id)) unione.push(id);
  ...
  return {
    ...
    markets,
    // INVARIATO: cosa il WATCHER di riprezzo puo' toccare. L'unione con le posizioni NON entra qui —
    // riguarda solo la allowlist del gate live-min, e allargare il riprezzo non era l'intenzione.
    // Scoperto da `end-of-scale-cycle.test.js`, che leggendo la configurazione vera ha visto tre
    // mercati in piu' e sei rinnovi invece di uno: un accoppiamento nascosto, non un test fragile.
    enabledMarketIds: abilitati,
    // LA ALLOWLIST DEL GATE live-min: qui si', l'unione. La legge il PROVIDER DI DIFETTO dell'adapter
    // (`polymarket-clob-maker/adapter.js`, una lettura per piazzamento), che e' il punto in cui il gate
    // decide davvero. CORRETTO IL 9 AGOSTO 2026: questo commento diceva «la legge manual-order per
    // costruire l'adapter» e NON era vero — `buildPlacementAdapter` non inietta nessuna lista, e i due
    // consumatori in `manual-order` stanno entrambi nell'oggetto di STATO del pannello. L'unione era
    // calcolata e non la leggeva nessun percorso di piazzamento.
    liveMinMarketIds: unione,
    // Le due componenti restano separate: il pannello deve poter dire PERCHE' un mercato e' nella
    // lista, e «l'operatore l'ha abilitato» e «ci abbiamo dentro dei soldi» sono due risposte diverse.
    enabledDaOperatore: abilitati,
    enabledDaPosizione: daPosizione.filter((id) => !abilitati.includes(id)),
    // La terza componente, separata come le altre due: «ci abbiamo degli ordini a libro» e' una terza
    // risposta diversa da «l'operatore l'ha abilitato» e da «ci abbiamo una posizione», e il pannello
    // deve poter dire QUALE delle tre tiene dentro un mercato.
    enabledDaOrdini: daOrdini.filter((id) => !abilitati.includes(id) && !daPosizione.includes(id)),

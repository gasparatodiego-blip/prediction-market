...
//
// WHY 15 MINUTES SPECIFICALLY, and what the snapshot cadence had to do with it. The venue scores maker
// liquidity from ONE RANDOM SAMPLE PER MINUTE — 1,440 samples a day, 10,080 in a weekly epoch
// (docs.polymarket.com/market-makers/liquidity-rewards: "Q_ne is calculated every minute using random
// sampling"; the per-second reading is wrong). That measurement settles the design: a ~3-second gap
// between cancel and replace costs 0.05 of one sample out of 1,440 — about 0.003% of a day's reward.
// Coverage is therefore NOT the thing to optimise, and the old instinct to keep the window long "so the
// order is never out of the book" was protecting against a cost that barely exists. Freed of that, the
// window is chosen almost entirely on how long an unattended order may legally rest: 15 minutes.
//
// THE ARITHMETIC OF THE RENEWAL RATE — renewals/hour = 60 / (window − margin):
//     window 15m, margin 5m → every 10m → 6.0/h    (the requested margin, one above the requested band)
//     window 15m, margin 3m → every 12m → 5.0/h    ← CHOSEN
//     window 12m, margin 3m → every  9m → 6.7/h
//     window 20m, margin 5m → every 15m → 4.0/h    (outside the requested 12–15m window)
// A 15-minute window, a 5-minute margin and a 3–5/hour rate cannot all hold at once. The window and the
// rate are the two that matter (one bounds the unattended exposure, the other bounds the churn), so the
// MARGIN is the variable that gives: 3 minutes. That is still 36 consecutive watcher polls at the 5s
// cadence in which to complete a ~2-second cancel→replace, and a failed attempt simply retries on the
// next poll. Both numbers are single constants — move REFRESH_MARGIN_SECONDS to 300 and the rate becomes
// 6/h with no other change.
const RESTING_GTD_SECONDS = 900;        // 15 minutes of EFFECTIVE life (see order-ttl: stated = now+60+ttl)
const REFRESH_MARGIN_SECONDS = 180;     // renew proactively once this little life remains

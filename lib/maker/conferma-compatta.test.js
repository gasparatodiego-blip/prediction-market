...
console.log('\n══ 5 · IL PULSANTE CHE INVIA RESTA SPENTO (punto 5 del mandato)');
{
  ...
  // Pretendeva la catena letterale `busy || trkBusy || !canReview || !riepilogo.completo`. Quella
  // catena aveva un difetto: tre dei quattro termini spegnevano il pulsante SENZA un messaggio nella
  // schermata di conferma. Ora la condizione è una sola, `blocco.puoInviare`, che per costruzione non
  // può essere falsa con l'elenco dei motivi vuoto — e continua a comprendere `canReview`.
  // Quello che la regola chiede è che i gate del pannello restino dentro la decisione, non in quale
  // forma sintattica ci stiano.
  ok('pannello ordine: il gate di sempre è ancora nella condizione',
    /disabled=\{!blocco\.puoInviare\}/.test(OP)
    && /problemiBloccanti: blocking/.test(OP)
    && /const canReview = blocking\.length === 0;/.test(OP),
    'canReview alimenta `blocco`, e la nuova condizione può solo spegnere');

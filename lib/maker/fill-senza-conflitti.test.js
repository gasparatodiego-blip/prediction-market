...
(async () => {
  ...
  ok(`il Livello 2 ha una scadenza di ${MERGE_WAIT_TIMEOUT_MIN} minuti`, MERGE_WAIT_TIMEOUT_MIN === 60);
  ...
  console.log('\n4 · più aggressivo verso il completamento — ma dentro le regole');
  {
    ...
    ok('  e resta sotto il tetto della coppia (100 − 80 − 1 = 19¢)', l2.price <= 0.19 + 1e-9, String(l2.price));
  }
  ...
  console.log('\n5 · il prezzo del completamento è già il massimo consentito, e si dichiara se è in banda');
  {
    ...
    ok('il prezzo è min(tetto, ask − tick)', g.piazzati[0].price === 0.19, String(g.piazzati[0].price));
    // L'ask deve stare SOPRA il tetto (0,19), altrimenti scatta il Livello 1 e non si sta più
    // guardando il prezzo di riposo: a 0,195 il Livello 1 non è disponibile e a mordere è `ask − tick`.
    const stretto = await ciclo({ asks: [{ price: 0.195, size: 100 }], bandaCents: 4.5, midNo: 0.20 });
    ok('  con un ask vicino, è l\'ask meno un tick a mordere',
      !stretto.piazzati[0].attraversaApposta && stretto.piazzati[0].price <= 0.185 + 1e-9,
      String(stretto.piazzati[0].price));
    ok('  e resta sempre sotto il tetto della coppia', stretto.piazzati[0].price <= 0.19 + 1e-9);
  }

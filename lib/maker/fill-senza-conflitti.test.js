...
(async () => {
  ...
  console.log('\n4 · più aggressivo verso il completamento — ma dentro le regole');
  {
    ...
    // Diceva «e passa da `inCoda`, cioe' dalla regola mai primo sul libro», ed era la codifica del difetto: `inCoda` manda il prezzo a
    ...
    ok('  e\' marcato USCITA: il prezzo deciso dalla scala e\' vincolante',
      l2 && l2.uscita === true, JSON.stringify({ uscita: l2 && l2.uscita, inCoda: l2 && l2.inCoda }));
    ok('  e NON e\' `inCoda` per costruzione — la toglie `chiudendo`, non il chiamante',
      l2 && l2.inCoda === undefined, String(l2 && l2.inCoda));
    ok('  e il prezzo che parte E\' quello deciso, non uno riscritto',
      l2 && Number.isFinite(l2.prezzoDeciso) && Math.abs(l2.prezzoDeciso - l2.price) < 1e-9,
      l2 && `deciso ${l2.prezzoDeciso} vs partito ${l2.price}`);

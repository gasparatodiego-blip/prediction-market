...
//     • auto-reprice ON: the order rests as GTC, with NO venue expiry at all, and is cancelled+re-placed
//       ONLY when the mid has moved enough to push it out of the reward band (lib/maker/auto-reprice.js,
//       driven by agent40-manual-reprice). Time alone never touches it.
...
const { isAutoRepriceEnabled, readAutoRepriceConfig, readAutoRepriceState, AUTO_REPRICE_SOURCE } = require('./auto-reprice-config');
...
const MANUAL_SOURCES = Object.freeze(['manual-ui', AUTO_REPRICE_SOURCE]);
...
/**
 * HOW LONG SHOULD A HAND ORDER REST? — the whole point of the auto-reprice change.
 *
 *   auto-reprice OFF (default)  → GTD, DEFAULT_MANUAL_TTL_SECONDS (180s). Exactly today's behaviour: the
 *                                 venue kills the order on a clock, whatever the price is doing. This is
 *                                 also the FAIL-CLOSED answer — an unreadable switch lands here, so an
 *                                 order can never end up with no expiry because a config could not be read.
 *   auto-reprice ON             → GTC, ttlSeconds 0: NO venue expiry at all. The order rests until it
 *                                 fills, until the operator cancels it, or until the watcher moves it
 *                                 because the mid left it out of band. Time alone never kills it.
 *
 * The trade is stated rather than buried: a GTD order is cleaned up by the VENUE even if this whole host
 * dies, and a GTC order is not. Turning auto-reprice on means accepting that the watcher (and the panel's
 * heartbeat display) is what stands in for that protection.
 *
 * An explicit spec.ttlSeconds ALWAYS wins — a caller that names a lifetime gets it.
 *
 * @returns {{ttlSeconds:number, orderType:'GTC'|'GTD', autoReprice:boolean, source:string, reason:string}}
 */
function resolveManualTtlSeconds({ marketId, ttlSeconds } = {}, deps = {}) {
  if (Number.isFinite(Number(ttlSeconds))) {
    const n = Number(ttlSeconds);
    return {
      ttlSeconds: n, orderType: n > 0 ? 'GTD' : 'GTC', autoReprice: false, source: 'explicit',
  ...
  if (ar.enabled) {
    return {
      ttlSeconds: 0, orderType: 'GTC', autoReprice: true, source: 'auto-reprice',
      reason: 'auto-reprice is ACTIVE on this market — the order rests as GTC (no venue expiry) and is moved only when the mid pushes it out of the band, never by a clock',
    };
  }
  return {
    ttlSeconds: DEFAULT_MANUAL_TTL_SECONDS, orderType: 'GTD', autoReprice: false,
    ...
    reason: ar.readable
      ...
      : `auto-reprice config unreadable (${ar.error}) — falling back to the fixed ${DEFAULT_MANUAL_TTL_SECONDS}s GTD expiry, so an unreadable switch can never produce an order with no expiry`,
  };
}
...
function manualContext({ marketId = null, userId = OPERATOR_USER } = {}, deps = {}) {
  ...
  return {
    ...
    autoReprice: {
      ...
      expiry: ttl ? { orderType: ttl.orderType, ttlSeconds: ttl.ttlSeconds, source: ttl.source, reason: ttl.reason } : null,
      watcher: {
...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  const refuse = (gate, reason, extra = {}) => {
    manualAudit({ op: 'manual-place', outcome: `reject-${gate}`, gate, reason, marketRef: marketId ? `cid_${String(marketId).replace(/^0x/, '')}` : null,
      requested: { book, price, size, notionalUsd: Number.isFinite(notionalUsd) ? +notionalUsd.toFixed(4) : null }, latencyMs: Date.now() - t0, userId }, source);
  ...
  try {
    res = await adapter.postOrder({
      ...
      decision: { source, book, byHand: source === DEFAULT_SOURCE, note: spec.note || null, expiry: ttl.orderType },
    });
  } catch (e) {
  ...
  manualAudit({
    ...
    requested: { book, side: 'BUY', price, size, notionalUsd: +notionalUsd.toFixed(4), orderType: ttl.orderType, ttlSeconds: ttl.ttlSeconds },
...
function manualIdempotencyKeys() {
  try {
    ...
    for (const line of raw.split('\n')) {
      // Cheap pre-filter before the JSON.parse. It must list EVERY panel-owned source: an order the
      // watcher re-priced is still the panel's order, and missing it here would silently re-attribute it
      // to agent35 — and then the watcher, which only touches provably-panel orders, would stop minding
      // the very order it had just placed.
      if (!line || !MANUAL_SOURCES.some((s) => line.indexOf(s) !== -1)) continue;
      let row; try { row = JSON.parse(line); } catch { continue; }
      if (!row || !MANUAL_SOURCES.includes(row.source)) continue;
...
async function replaceManualOrder(spec = {}, deps = {}) {
  ...
  const placed = await placeManualOrder({ ...spec, marketId, book, source }, deps);

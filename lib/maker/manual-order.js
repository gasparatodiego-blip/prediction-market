...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  // Sono due affermazioni diverse, e questo gate le trattava allo stesso modo. Per il motore di
  // market making la differenza conta: un operatore puo' scegliere di quotare piu' lontano dal mid
  // proprio per stare lontano dal fill, accettando di non maturare su quel lato.
  //
  // Quindi `allowOutOfBand` declassa QUEL SOLO codice da bloccante a dichiarato. Ogni altro motivo —
  // fuori griglia del tick, fuori dai limiti di prezzo del venue, sotto la size minima, regole non
  // leggibili — continua a rifiutare esattamente come prima. E' l'unico modo in cui questa deroga puo'
  // esistere senza diventare un interruttore che spegne la validazione.
  //
  // Nessuna route HTTP accetta questo campo: e' per i chiamanti server-side, e il fuori-banda finisce
  // comunque nell'audit e sullo schermo con il suo badge.
  const outOfBandOnly = spec.allowOutOfBand === true
    && vq.reasons.length > 0
    && vq.reasons.every((r) => r.code === 'OUT_OF_BAND');
  if (!vq.valid && !outOfBandOnly) {

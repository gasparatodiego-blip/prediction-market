...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  // ── IL TETTO PER MERCATO SUL NOZIONALE A RIPOSO (16 agosto 2026) ────────────────────────────────
  //
  // ⚠ IL BUCO CHE CHIUDE, MISURATO. Il tetto per mercato ($61,25) governava SOLO il capitale che il
  // PIANO assegna; il tetto per ordine governa UN ordine per volta; `maxOpenNotionalUsd` governa i fill
  // RICONCILIATI, globalmente. Nessuno sommava gli ordini VIVI di uno stesso mercato. Il 16 agosto una
  // corsa nel riprezzo ha lasciato due ordini identici da $44,54 su `0x776841ce…`: **$89,08 a riposo su
  // un mercato con tetto $61,25**, e ogni singolo ordine era legale — nessun gate poteva vederlo.
  // Il duplicato e' anche l'intera ragione per cui il nozionale totale a riposo ha superato il massimo
  // teorico di 3 × $61,25 = $183,75, arrivando a $194,72.
  //
  // ⚠ FAIL-CLOSED: se la lista degli ordini vivi non si legge, si RIFIUTA. Un tetto che si applica solo
  // quando la lettura riesce non e' un tetto: e' un suggerimento. Il costo del rifiuto e' un ciclo di
  // ritardo su un ordine di liquidita', che puo' aspettare.
  //
  // ⚠ LE CHIUSURE E LE RIDUZIONI NON PASSANO DI QUI, per la stessa prova gia' in servizio su
  // `max-open-notional`: `provaChiusura` rifa' l'aritmetica sull'ordine esatto contro lo snapshot del
  // venue (SELL entro il posseduto, BUY entro `manca`). Un ordine che CHIUDE non aggiunge nozionale a
  // riposo che valga la pena limitare — e murarlo ripeterebbe la trappola di §5-bis p.168.
  if (!(esenzioneChiusura && esenzioneChiusura.esente) && spec.chiudePosizione !== true) {
    // ⚠ `caps` NON porta un tetto per mercato — `resolveCaps` risolve i limiti di safety e il cap
    // live-min, non la concentrazione. Il numero viene dalla sua unica fonte e si IMPORTA (§4.2):
    // ricopiarlo qui sarebbe il reperto D1 su una cintura di rischio.
    const tettoMercato = MARKET_CAP_FIXED_USD;
    let vivi;
    try { vivi = await listManualOrders({ marketId, userId: spec.userId }, deps); }
    catch (e) { vivi = { ok: false, error: e && e.message ? e.message : String(e) }; }
    if (!vivi || vivi.ok === false || !Array.isArray(vivi.orders)) {
      return refuse('nozionale-mercato-illeggibile',
        `non si e' potuta leggere la lista degli ordini vivi di questo mercato (${(vivi && vivi.error) || 'motivo ignoto'}):`
        + ` senza sapere quanto c'e' gia' a riposo non si puo' dimostrare di restare sotto il tetto per mercato di $${tettoMercato}`,
        { caps });
    }
    let aRiposo = 0;   //  e non : la cancellazione della gemella lo riduce, sopra.
    for (const o of vivi.orders) {
      const p = Number(o.price);
      const s = Number(o.sizeRemaining != null ? o.sizeRemaining : o.size);
      // `Number(null)` vale 0 ed e' finito: senza il controllo sul segno un ordine col prezzo
      // illeggibile entrerebbe come nozionale ZERO, cioe' come «qui non c'e' niente» — la conclusione
      // che porta a metterci sopra altro capitale. §5.3, e qui costerebbe il duplicato di nuovo.
      if (!Number.isFinite(p) || p <= 0 || !Number.isFinite(s) || s <= 0) {
        return refuse('nozionale-mercato-illeggibile',
          `un ordine vivo di questo mercato ha prezzo o size non leggibili (${o.orderId || 'senza id'}):`
    ...
    // Su un mercato non puo' esistere piu' di un ordine vivo per token+lato. Due corse nel riprezzo
    // ne hanno prodotti due in un'ora il 16 agosto 2026, e ogni singolo ordine era legale.
    // ⚠ Si riusa `vivi.orders`, la lettura che il tetto per mercato ha gia' fatto: due letture dello
    // stesso elenco nello stesso invio potrebbero divergere, e la divergenza qui produrrebbe
    // esattamente il doppione che stiamo togliendo.
    const gem = gemellaEsistente(vivi.orders, { conditionId: marketId, tokenId, side, price, size });
    if (gem.esiste && gem.identico) {
      // Identico ⇒ non aggiunge niente. Rifiutare e' l'unica risposta che non peggiora lo stato.
      return refuse('doppione-identico', gem.motivo, { caps, gemella: gem.ordine && gem.ordine.orderId });
    }
    if (gem.esiste) {
      // Diverso per prezzo o size ⇒ il nuovo ordine sostituisce il vecchio, e il vecchio va tolto
      // PRIMA. Si cancella qui invece di rifiutare, perche' rifiutare lascerebbe a libro un ordine che
      // il chiamante ha gia' deciso di superare — e il riprezzo non avrebbe piu' modo di correggerlo.
      const vecchio = gem.ordine && gem.ordine.orderId;
      const c = vecchio ? await cancelManualOrder({ orderId: vecchio, marketId, userId: spec.userId }, source) : null;
      // ⚠ FAIL-CLOSED: se la cancellazione non riesce NON si piazza. Piazzare comunque sarebbe creare
      // il doppione di proposito invece che per una corsa.
      if (!c || c.ok !== true) {
        return refuse('doppione-vecchio-non-cancellato',
          `esiste una gamba viva su questo token+lato (${String(vecchio).slice(0, 12)}…) e la sua cancellazione`
          + ` non e' riuscita (${(c && c.reason) || 'motivo ignoto'}): non si piazza il secondo, o resterebbero entrambi`,
          { caps, gemella: vecchio });
      }
      manualAudit({ op: 'manual-place', outcome: 'gemella-cancellata-prima-del-nuovo',
        marketRef: marketId ? `cid_${String(marketId).replace(/^0x/, '')}` : null,
        reason: gem.motivo, requested: { orderId: vecchio }, userId }, source);
      // Il nozionale della gemella non pesa piu': e' appena uscita dal libro.
      aRiposo = Math.max(0, aRiposo - (Number(gem.ordine.price) * Number(gem.ordine.sizeRemaining != null ? gem.ordine.sizeRemaining : gem.ordine.size) || 0));
    }

    const dopo = +(aRiposo + notionalUsd).toFixed(4);
    if (dopo > tettoMercato + 1e-9) {
      return refuse('nozionale-mercato-oltre-tetto',
        `questo mercato ha gia' $${aRiposo.toFixed(2)} di ordini a riposo (${vivi.orders.length}) e questo ordine ne aggiungerebbe`
        + ` $${notionalUsd.toFixed(2)}: il totale $${dopo.toFixed(2)} supera il tetto per mercato di $${tettoMercato}.`
        + ' Il tetto per ordine e quello di esposizione non lo vedevano: sommano un ordine per volta o i soli fill riconciliati',
        { caps, aRiposoUsd: +aRiposo.toFixed(4), totaleDopoUsd: dopo, tettoMercatoUsd: tettoMercato });
    }
  }

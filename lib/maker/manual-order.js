...
function resolveMarketDepth(marketId, deps = {}) {
  ...
  return {
    ...
    yes: yesBids ? { bids: yesBids } : null,
    no: noBids ? { bids: noBids } : null,
...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  // IL CONTROLLO FINALE, sul prezzo che partirebbe davvero. Un BUY che raggiunge il miglior ask incrocia.
  if (Number.isFinite(touch && touch.bestAsk) && price >= touch.bestAsk - 1e-12) {
    return refuse('would-cross',
      `a ${(price * 100).toFixed(2)}¢ quest'ordine incrocerebbe il miglior ask (${(touch.bestAsk * 100).toFixed(2)}¢) e si eseguirebbe SUBITO come taker.`
      + (priceAdjusted
      ...
      { bestAsk: touch.bestAsk, priceAdjusted });
  }

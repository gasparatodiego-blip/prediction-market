...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  if (spec.inCoda === true) {
    try {
      ...
      // I NOSTRI ordini a riposo su questo libro, per non inseguire noi stessi di un tick a ogni ciclo.
      const nostri = Array.isArray(spec.ownOrders) ? spec.ownOrders : [];
      const q = prezzoInCoda({ book, side: spec.side === 'SELL' ? 'SELL' : 'BUY', rules, depth, ownOrders: nostri, offsetCents: spec.distanceCents });
      inCodaEsito = { ok: q.ok, mode: q.mode, onTop: q.onTop, bestOther: q.bestOther, reason: q.reason, quotabile: q.quotabile };
      ...
      if (q.ok && Number.isFinite(q.price) && Math.abs(q.price - price) > rules.tick / 1000) {
        priceAdjusted = { ...(priceAdjusted || {}), inCoda: { from: price, to: q.price, mode: q.mode, onTop: q.onTop, bestOther: q.bestOther } };
        price = q.price;
        notionalUsd = (Number.isFinite(price) && Number.isFinite(size)) ? price * size : NaN;
      }
  ...
  const incrocia = lato === 'SELL'
    ...
    : (Number.isFinite(rifTocco) && price >= rifTocco - 1e-12);
  const attraversaApposta = lato === 'SELL' && spec.attraversaApposta === true;

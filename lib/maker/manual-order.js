...
// ⚠ LA SOGLIA DI FRESCHEZZA VIENE DA QUI, NON DA UN NUMERO SCRITTO IN QUESTO FILE: `regimeFeed` e' la
// STESSA funzione con cui `auto-reprice` decide se un mid e' troppo vecchio per muovere un ordine.
// Importarla e' cio' che rende impossibile, per costruzione, che il piazzatore sia piu' permissivo di
// chi poi deve mantenere l'ordine — due numeri per lo stesso concetto divergono, uno solo no (D1).
...
const ENGINE_STATE_FILE = fileRuntime('maker-state.json');
// ⚠ I DUE PERCORSI DEL FEED NON SONO PIU' LETTERALI QUI (17 agosto 2026): una definizione sola in
// `./percorsi-feed`, risolta A OGNI LETTURA e sovrascrivibile da env. `closeTask()` chiama
// `resolveMarketRules(marketId)` senza `deps`, quindi legge da qui: finche' il percorso era cablato,
// il cablaggio di produzione di agent40 non era esercitabile contro un venue simulato e un banco
// poteva solo ricablare `runAutoCloseCycle` da se' — cioe' provare una copia piu' piccola
// dell'originale (17 dep contro 20). Fail-closed invariato: un file assente ⇒ `null` ⇒
// `rules-unreadable`, mai un prezzo inventato.
...
const { LIVE_MIN_ORDER_CAP_USD, MARKET_CAP_FIXED_USD } = require('../rewards/concentration');
// L'esenzione dal tetto PER ORDINE sui percorsi di chiusura — la stessa che l'adapter applica alla sua
// cintura indipendente, importata da entrambi perche' le due devono esentare gli stessi ordini.
const { provaChiusura, leggiCoppiaDetenuta } = require('./esenzione-chiusura');
// IL PREZZO DECISO DALLA SCALA E' VINCOLANTE — 24 agosto 2026. Un modulo solo, importato anche da
// `auto-close` (che marca) e da `auto-reprice` (che non muove), cosi' la regola non esiste in tre copie.
const USCITA = require('./ordini-di-uscita');
// ⚠ IL TETTO DELLA COPPIA SI IMPORTA, NON SI RISCRIVE. `chiusura-rapida` e' la definizione unica
// (`MAKER_TETTO_COPPIA_CENTS`, clamp [100,200], §4.6) ed e' un modulo PURO senza require: nessun ciclo.
// Un `101` scritto qui sarebbe la seconda copia di un limite di rischio — il reperto D1 esatto.
const { TETTO_COPPIA_CENTS } = require('./chiusura-rapida');
// Il divieto di doppioni: su un mercato, un solo ordine vivo per token+lato. Vedi il blocco nel gate.
const { gemellaEsistente } = require('./doppioni');
// La selezione dei NOSTRI ordini su un lato, e l'unione client+server. La stessa funzione la usa
// `auto-reprice` per il percorso di DECISIONE: una definizione sola, o i due percorsi vedono due book.
...
//
// ⚠ L'ARITMETICA VIVE IN `lib/maker/cinture-armamento.js` E QUI SI IMPORTA, dal 17 agosto 2026 sera. La
// ragione e' `scripts/cli/stato.js`: deve poter dire lo stato di questa cintura leggendo l'ambiente di un
// PROCESSO VIVO (`/proc/<pid>/environ`) e non il `.env`, e non puo' importare questo file — il suo
// §PERIMETRO cammina `require.cache` e fallisce se ha caricato una superficie che sa piazzare. Due copie
// della stessa riga sarebbero il reperto D1 su una cintura di armamento; la dipendenza va nel verso
// giusto, e chi decide (questo file) importa da chi la definisce (quello puro).
const { manualPlacement } = require('./cinture-armamento');
...
function resolveMarketRules(marketId, deps = {}) {
  ...
  const books = deps.books || readJson(PERCORSI.fileBookVivi());
  const norm = deps.norm || readJson(PERCORSI.fileBoardNormalizzato());
  ...
  if (!nm && id) {
    ...
    if (catalogRec) {
      nm = {
        ...
        // ⚠ IL SECONDO MINIMO, CON IL SUO NOME. `minSize` qui sopra e' il PAVIMENTO PREMIANTE
        // (`rewards.min_size`, 50/20 sui mercati misurati): dice «non maturera' premio». Questo e' il
        // MINIMO D'ORDINE del venue (`minimum_order_size`, 5): dice «il venue rifiutera' l'ordine».
        // Erano un numero solo, e il percorso d'uscita leggeva il primo credendo fosse il secondo —
        // §4 del giro del 24 agosto 2026, `lib/maker/minimi-del-venue.js`.
        minOrderSize: Number.isFinite(catalogRec.minOrderSize) ? catalogRec.minOrderSize : null,
  ...
  // ── IL MINIMO D'ORDINE DEL VENUE, dalle stesse fonti e con lo stesso ordine di preferenza ────────
  // ⚠ NON ENTRA IN `missing`, ED E' DELIBERATO. `missing` rende il mercato INTERAMENTE non giudicabile
  // e rifiuta anche le APERTURE, che di questo numero non hanno bisogno: il pavimento premiante basta
  // e avanza per decidere se una quota maturera'. A fermarsi rumorosamente e' il solo percorso che se
  // ne serve — l'USCITA — dentro `venue-rules.validateQuote`, che nomina il conditionId nel rifiuto.
  // Metterlo qui vorrebbe dire spegnere il bot su ogni mercato finche' il catalogo non e' ripopolato,
  // cioe' curare 282 rifiuti con 22.206 rifiuti nuovi.
  const minOrderSize = (bm && Number.isFinite(bm.minOrderSize) && bm.minOrderSize > 0) ? bm.minOrderSize
    : ((nm && Number.isFinite(nm.minOrderSize) && nm.minOrderSize > 0) ? nm.minOrderSize : null);
  ...
  return {
    ...
    mid, tick, maxSpreadCents, minSize, minOrderSize, tokenId, tokenIdNo, negRisk,
...
function manualContext({ marketId = null, userId = OPERATOR_USER } = {}, deps = {}) {
  ...
  return {
    ...
    // ⚠ IL PERIMETRO SI CHIEDE ALLA FUNZIONE CHE LO APPLICA, non si ricalcola qui. Fino al 17 agosto 2026
    // queste righe erano una COPIA dell'aritmetica del gate, e divergevano gia' in due modi: la lista
    // mostrata era l'unione (`liveMinMarketIds`) mentre `count` contava il solo piano
    // (`enabledMarketIds`), e il perno veniva SOMMATO — che dal 17 agosto non e' piu' come il gate decide.
    // Un pannello che dichiara un perimetro e un gate che ne applica un altro e' il reperto D1 nella sua
    // forma peggiore: l'operatore legge un numero e il codice usa un altro.
    // Richiesta pigra come nel resto del file: rendere un pannello non deve tirare dentro l'adapter.
    liveMinAllowlist: (() => {
      const lista = arCfg.liveMinMarketIds || arCfg.enabledMarketIds || [];
      let p = null;
      try { p = require('../venues/polymarket-clob-maker/adapter').perimetroLiveMin({ liveMinMarket: engine.pinnedMarketId || '', allowedMarketIds: lista }); }
      catch { p = null; }
      return {
        pinnedMarketId: engine.pinnedMarketId,
        // La lista dell'operatore resta visibile ANCHE quando il perno la esclude: senza di lei il
        // messaggio «il perno restringe» non sarebbe verificabile guardando il pannello.
        enabledMarketIds: lista,
        // ⚠ `null` e non `0` quando il perimetro non si e' potuto calcolare: «non ho letto» non e'
        // «nessun mercato», e uno zero finto qui direbbe «sei al sicuro» senza averlo verificato.
        count: p ? p.allowed.length : null,
        allowed: p ? p.allowed : null,
        ristrettoDalPerno: p ? p.ristretto : null,
        esclusiDalPerno: p ? p.esclusiDalPerno : null,
        targetAllowed: (targetId && p) ? p.allowed.includes(String(targetId).trim().toLowerCase()) : null,
        note: p && p.ristretto
          ? `live-min e' RISTRETTO al perno MAKER_LIVE_MIN_MARKET: un mercato solo (${p.allowed[0]}), e i ${p.esclusiDalPerno.length} abilitati dal pannello che non sono il perno vengono rifiutati. Il perno restringe, non aggiunge.`
          : 'live-min accetta un ordine solo su un mercato di questa lista (opt-in durevole e auditato). Lista vuota e nessun perno ⇒ nessun ordine passa. Un perno impostato diventa l\'intero perimetro.',
      };
    })(),
    isolation: manual ? {
      marketId: targetId,
...
// ═══ LE CINTURE CHE QUI DENTRO ERANO INERTI, E DA OGGI MORDONO — 17 agosto 2026, per decisione ═══════
// §4.14 aveva misurato che delle cinque cinture, sulla strada da cui il bot piazza davvero, ne mordeva
// UNA. La causa era tutta in questa funzione, ed erano due righe:
//
//   · `mode: 'live-min'` era CABLATO A MANO ⇒ `MAKER_MODE` non gate la corsia manuale. Il gate riceveva
//     sempre uno stadio vivo, qualunque cosa dicesse l'ambiente. L'`off` che si legge su `/proc` non
//     fermava niente.
//   · `dryRun` non veniva passato, e l'adapter fa `dryRun = opts.dryRun === true` ⇒ **la variabile
//     `MAKER_ADAPTER_DRYRUN` non veniva letta su questo percorso**, perche' `createMakerAdapter` non ha
//     un ripiego sull'ambiente per quel campo (a differenza di `placement`, che ce l'aveva).
//
// Prova che ne misurava il costo: `evaluatePlacementGate({mode:'live-min', dryRun:false,
// fundingApproved:true, sdk})` → `allow: true`. Tre cinture su cinque non comparivano nel verdetto.
//
// ⚠ ADESSO SI LEGGONO DALL'AMBIENTE **DEL PROCESSO CHE PIAZZA**, e la lettura passa da
// `cinture-armamento` — lo stesso modulo da cui `stato.js` le dichiara. Non e' un'aritmetica nuova: e'
// l'aritmetica che era gia' scritta per RACCONTARE lo stato, ora usata anche per DECIDERLO. Un pannello
// e un motore che leggono la stessa funzione non possono piu' divergere.
// ⚠ LA DIREZIONE E' MONOTONA: `modoVivo` falso ⇒ `off`, che il gate rifiuta; `dryRunInserita` vera ⇒
// ombra forzata. Nessuna configurazione che prima rifiutava ora passa — si aggiungono rifiuti, mai
// permessi. Un ambiente illeggibile fa scattare entrambe (`str` restituisce `undefined`).
// ⚠ E NON TOCCA LETTURE NE' CANCELLAZIONI: questa funzione ha **un solo chiamante**
// (`placeManualOrder`, riga ~1470), e leggere e cancellare passano dall'adapter cancel-only qui sotto,
// che non ha ne' modo ne' dryRun. Il guardiano delle perdite continua a poter cancellare.
function buildPlacementAdapter({ pinnedMarketId, liveMinCapUsd, placement, orderTtlSeconds, env, source = DEFAULT_SOURCE }) {
  const { createMakerAdapter } = require('../venues/polymarket-clob-maker/adapter');
  const { modoVivo, dryRunInserita } = require('./cinture-armamento');
  ...
  return createMakerAdapter({
    // `live-min` SOLO se `MAKER_MODE` dichiara uno stadio vivo. Altrimenti `off`, e il gate rifiuta
    // con `maker-mode`. (v. il blocco sopra: era cablato, ed era la prima delle tre cinture inerti.)
    mode: modoVivo(ambiente) ? 'live-min' : 'off',
    // L'ombra forzata, che prima non arrivava mai qui. `=== true` esatto sia in lettura che qui.
    dryRun: dryRunInserita(ambiente),
...
// ══ IL TETTO PER MERCATO SUL NOZIONALE A RIPOSO — UNA FUNZIONE, DUE CHIAMANTI ═══════════════════════
//
// ⚠ IL BUCO CHE CHIUDE, MISURATO (16 agosto 2026). Il tetto per mercato ($61,25) governava SOLO il
// capitale che il PIANO assegna; il tetto per ordine governa UN ordine per volta; `maxOpenNotionalUsd`
// governa i fill RICONCILIATI. Nessuno sommava gli ordini VIVI di uno stesso mercato: una corsa nel
// riprezzo ha lasciato due ordini identici da $44,54 su `0x776841ce…` — **$89,08 a riposo su un mercato
// con tetto $61,25**, e ogni singolo ordine era legale.
//
// ⚠ E IL SECONDO BUCO, CHIUSO IL 17 AGOSTO 2026 (§5.2 p.38). Questo gate viveva SOLO dentro
// `placeManualOrder`, cioe' DOPO la cancellazione del riprezzo: `replaceManualOrder` passava i suoi cinque
// precontrolli, CANCELLAVA, e poi il sesto gate rifiutava — gamba persa. Misurato: **160,8 minuti di gamba
// singola il 16 agosto**, il 68,1% dei minuti a gamba singola della giornata insieme a `doppione-identico`.
// La cura ha due meta', e servono entrambe:
//   ① LA STESSA FUNZIONE per il precontrollo e per il rifiuto. Precontrollare con un numero diverso da
//      quello che poi rifiuta e' peggio che non precontrollare (e' scritto nel commento del precontrollo
//      del tetto per ordine, `:1782`, ed e' il reperto D1);
//   ② `escludiOrderId`: su un riprezzo l'ordine che si sta sostituendo E' GIA' dentro gli ordini a riposo,
//      quindi verrebbe contato DUE VOLTE. Evidenza dal giornale delle 12:14:42 su FL-27: «$53.67 di ordini
//      a riposo (2) e questo ne aggiungerebbe $11.42» — e $11,42 ERA l'ordine che si stava sostituendo.
//      L'esclusione vale per la somma E per il divieto di doppioni: la gemella di un riprezzo e' l'ordine
//      stesso, e rifiutarlo come «doppione» sarebbe rifiutare il riprezzo per la sua stessa esistenza.
//
// ⚠ FAIL-CLOSED: se la lista degli ordini vivi non si legge, si RIFIUTA. Un tetto che si applica solo
// quando la lettura riesce non e' un tetto: e' un suggerimento.
//
// @returns {{allow:boolean, gate?:string, motivo?:string, aRiposoUsd:number|null,
//            gemellaDaCancellare?:string|null, osservato?:object}}
async function valutaNozionaleMercato({ marketId, tokenId, side, price, size, notionalUsd,
  userId, escludiOrderId = null, source, deps = {} } = {}) {
  // Il numero viene dalla sua unica fonte e si IMPORTA (§4.2): ricopiarlo sarebbe il reperto D1 su una
  // cintura di rischio.
  const tettoMercato = MARKET_CAP_FIXED_USD;
  let vivi;
  try { vivi = await listManualOrders({ marketId, userId }, deps); }
  catch (e) { vivi = { ok: false, error: e && e.message ? e.message : String(e) }; }
  if (!vivi || vivi.ok === false || !Array.isArray(vivi.orders)) {
    return { allow: false, gate: 'nozionale-mercato-illeggibile', aRiposoUsd: null,
      motivo: `non si e' potuta leggere la lista degli ordini vivi di questo mercato (${(vivi && vivi.error) || 'motivo ignoto'}):`
        + ` senza sapere quanto c'e' gia' a riposo non si puo' dimostrare di restare sotto il tetto per mercato di $${tettoMercato}` };
  }
  // ⚠ L'ESCLUSIONE E' LA PRIMA COSA CHE SI FA, e vale per tutto il resto: somma e doppioni.
  const escl = escludiOrderId ? String(escludiOrderId) : null;
  const ordini = escl ? vivi.orders.filter((o) => String(o.orderId) !== escl) : vivi.orders;
  const esclusoTrovato = escl ? vivi.orders.length !== ordini.length : false;

  let aRiposo = 0;
  for (const o of ordini) {
    const p = Number(o.price);
    const sz = Number(o.sizeRemaining != null ? o.sizeRemaining : o.size);
    // `Number(null)` vale 0 ed e' finito: senza il controllo sul segno un ordine col prezzo illeggibile
    // entrerebbe come nozionale ZERO, cioe' come «qui non c'e' niente» — la conclusione che porta a
    // metterci sopra altro capitale. §5.3, e qui costerebbe il duplicato di nuovo.
    if (!Number.isFinite(p) || p <= 0 || !Number.isFinite(sz) || sz <= 0) {
      return { allow: false, gate: 'nozionale-mercato-illeggibile', aRiposoUsd: null,
        motivo: `un ordine vivo di questo mercato ha prezzo o size non leggibili (${o.orderId || 'senza id'}):`
          + ' la somma a riposo non e\' calcolabile, e un totale sbagliato in difetto lascerebbe passare l\'ordine di troppo' };
    }
    aRiposo += p * sz;
  }

  // Il divieto di doppioni, sulla STESSA lettura: due letture dello stesso elenco nello stesso invio
  // potrebbero divergere, e la divergenza qui produrrebbe esattamente il doppione che stiamo togliendo.
  const gem = gemellaEsistente(ordini, { conditionId: marketId, tokenId, side, price, size });
  let gemellaDaCancellare = null;
  if (gem.esiste && gem.identico) {
    return { allow: false, gate: 'doppione-identico', motivo: gem.motivo, aRiposoUsd: +aRiposo.toFixed(4),
      osservato: { gemella: gem.ordine && gem.ordine.orderId, esclusoDalConto: escl, esclusoTrovato } };
  }
  if (gem.esiste) {
    // Diverso per prezzo o size ⇒ il nuovo sostituisce il vecchio, e il vecchio va tolto PRIMA. La
    // cancellazione la fa il CHIAMANTE: questa funzione calcola e non agisce, cosi' il precontrollo del
    // riprezzo puo' usarla senza cancellare niente.
    gemellaDaCancellare = gem.ordine && gem.ordine.orderId;
    aRiposo = Math.max(0, aRiposo - (Number(gem.ordine.price)
      * Number(gem.ordine.sizeRemaining != null ? gem.ordine.sizeRemaining : gem.ordine.size) || 0));
  }

  const dopo = +(aRiposo + Number(notionalUsd)).toFixed(4);
  if (dopo > tettoMercato + 1e-9) {
    return { allow: false, gate: 'nozionale-mercato-oltre-tetto', gemellaDaCancellare,
      aRiposoUsd: +aRiposo.toFixed(4),
      motivo: `questo mercato ha gia' $${aRiposo.toFixed(2)} di ordini a riposo (${ordini.length})`
        + `${esclusoTrovato ? ` — escluso l'ordine che si sta sostituendo (${String(escl).slice(0, 12)}…)` : ''}`
        + ` e questo ordine ne aggiungerebbe $${Number(notionalUsd).toFixed(2)}: il totale $${dopo.toFixed(2)}`
        + ` supera il tetto per mercato di $${tettoMercato}.`
        + ' Il tetto per ordine e quello di esposizione non lo vedevano: sommano un ordine per volta o i soli fill riconciliati',
      osservato: { aRiposoUsd: +aRiposo.toFixed(4), totaleDopoUsd: dopo, tettoMercatoUsd: tettoMercato,
        sforoUsd: +(dopo - tettoMercato).toFixed(4), esclusoDalConto: escl, esclusoTrovato,
        ordiniContati: ordini.length } };
  }
  return { allow: true, gemellaDaCancellare, aRiposoUsd: +aRiposo.toFixed(4), motivo: gem.motivo || null,
    osservato: { aRiposoUsd: +aRiposo.toFixed(4), totaleDopoUsd: dopo, tettoMercatoUsd: tettoMercato,
      esclusoDalConto: escl, esclusoTrovato } };
}
...
async function placeManualOrder(spec = {}, deps = {}) {
  ...
  // ══ ORDINE DI USCITA: IL PREZZO DECISO DALLA SCALA E' VINCOLANTE ═══════════════════════════════
  // 24 agosto 2026, decisione dell'operatore, dopo il caso misurato di `0x4d79d306`: la scala aveva
  // deciso `SELL no 0.495` («carico +1 tick, fuori banda, si puo' uscire in pari») e al venue e'
  // arrivato **0.288**, perche' questo file ha applicato a un'USCITA la regola di coda che serve a chi
  // QUOTA. §7 concedeva 0,1¢; l'ordine partito ne concedeva 20,6¢ — 8,3 volte.
  //
  // Da qui in poi, per un ordine marcato `uscita` (lo marca `auto-close.chiudendo`, punto unico):
  //   · nessun aggiustamento viene APPLICATO, in nessun ramo — ne' il ricalcolo dal mid vivo ne' la
  //     coda «mai primi»;
  //   · l'aggiustamento viene comunque CALCOLATO e scritto, con prezzo deciso, prezzo proposto, delta
  //     e nome del ramo: se non lo si calcolasse, «un aggiustamento voleva muovere il prezzo» non
  //     sarebbe osservabile, ed e' esattamente cio' che ha tenuto il difetto invisibile per sette ore;
  //   · e un'asserzione terminale, un istante prima della POST, pretende che il prezzo che parte sia
  //     ANCORA quello deciso. E' il backstop che rende la regola vera invece che voluta: se un ramo
  //     nuovo domani riscrivesse il prezzo, l'ordine NON PARTE.
  const eUscita = USCITA.eOrdineDiUscita(spec);
  const prezzoDecisoScala = eUscita ? USCITA.prezzoDecisoDi(spec) : null;
  const aggiustamentiRifiutati = [];
  /**
   * Un ramo propone un prezzo. Su un ordine di uscita non si applica mai: si registra e si restituisce
   * il prezzo deciso. Su tutto il resto si applica come sempre — questa funzione e' l'unico punto in
   * cui la differenza esiste, cosi' non ci sono due politiche sparse per il file.
   */
  const proponiPrezzo = (proposto, ramo) => {
    if (!eUscita) return { applica: true, prezzo: proposto };
    const v = USCITA.verdettoAggiustamento({
      prezzoDeciso: prezzoDecisoScala, prezzoProposto: proposto, ramo, tick: rules && rules.tick });
    if (v.muove) {
      aggiustamentiRifiutati.push({ ramo: v.ramo, prezzoDeciso: v.prezzoDeciso, prezzoProposto: v.prezzoProposto, deltaCents: v.deltaCents });
      manualAudit({ op: 'manual-place', outcome: 'uscita-aggiustamento-rifiutato',
        marketRef: marketId ? `cid_${String(marketId).replace(/^0x/, '')}` : null,
        reason: v.motivo,
        observed: { ramo: v.ramo, prezzoDeciso: v.prezzoDeciso, prezzoProposto: v.prezzoProposto,
          deltaCents: v.deltaCents, book, side, size },
        userId }, source);
    }
    return { applica: false, prezzo: prezzoDecisoScala != null ? prezzoDecisoScala : proposto };
  };
  ...
  // ⚠ `marketId` E `minOrderSize` VIAGGIANO CON LE REGOLE, non a parte: `validateQuote` e' pura e non
  // puo' andarseli a cercare, e un rifiuto d'uscita che non nomina il mercato costringe chi legge il
  // giornale a dedurlo dal contesto. Vedi `lib/maker/minimi-del-venue.js`.
  const venueRules = { tick: rules.tick, scoringMid, maxSpreadCents: rules.maxSpreadCents,
    minSize: rules.minSize, minOrderSize: rules.minOrderSize, marketId: rules.marketId };
  ...
  if (Number.isFinite(spec.distanceCents) && spec.distanceCents >= 0 && Number.isFinite(rules.mid) && Number.isFinite(rules.tick)) {
    ...
    if (Number.isFinite(snapped) && snapped > 0 && snapped < 1 && Math.abs(snapped - price) > rules.tick / 1000) {
      // ⚠ ANCHE QUESTO RAMO E' UN AGGIUSTAMENTO, e su un'uscita non si applica. Il difetto del 24
      // agosto e' passato dalla coda, ma «in nessun ramo» vuol dire in nessuno: un divieto scritto su
      // un solo percorso e' la classe di guasto piu' frequente di questo repo.
      const p = proponiPrezzo(snapped, 'ricalcolo-dal-mid-vivo');
      if (p.applica) {
  ...
  if (spec.inCoda === true) {
    try {
      ...
      // ══ IL SERVER NON SI FIDA DEL CLIENT, E NON NE FA NEMMENO A MENO ══════════════════════════
      // Prima erano due rami esclusivi: se il chiamante passava `ownOrders` si usavano quelli e basta,
      // altrimenti si leggeva dal venue. Entrambi i rami hanno un buco.
      //   · fidarsi del client: una lista incompleta (o vuota) fa scambiare i NOSTRI ordini per
      //     concorrenza, e ci si accoda a se stessi un tick per volta fino al bordo della banda;
      //   · leggere e basta: la lettura è rete, e quando fallisce si proseguiva con la lista VUOTA,
      //     che è lo stesso difetto per un'altra strada.
      // Adesso si fa SEMPRE la lettura del server e la si UNISCE a quella del client, deduplicando
      // per `orderId` (`nostri-ordini.unisci`). Il client può solo AGGIUNGERE righe: non può togliere
      // ciò che il venue ha confermato, e non può far passare per «non nostro» un ordine che lo è.
      // ── L'UNICA VIA PER SALTARE LA LETTURA, E IL CLIENT NON PUÒ IMBOCCARLA ──────────────────
      // Un chiamante IN-PROCESS che ha appena letto gli ordini del venue — il watcher di riprezzo, che
      // gira ogni pochi secondi — li ha già in mano: rileggerli qui sarebbe una chiamata sprecata per
      // ogni riprezzo. Può quindi dichiararli autorevoli.
      //
      // ⚠ E IL PANNELLO NON PUÒ FARLO, che è tutto il punto: `ownOrdersAuthoritative` NON è nello
      // schema zod della route (`app/api/maker/manual/order/route.ts`), e zod scarta ciò che non
      // dichiara. Il confine di fiducia è quello, non una promessa: dal browser il campo non arriva,
      // quindi per il pannello la lettura del server è sempre obbligatoria e l'unione sempre fatta.
      const tokenDelLato = book === 'no' ? rules.tokenIdNo : rules.tokenId;
      const dalClient = NO.nostriSulLato({ orders: spec.ownOrders, tokenId: tokenDelLato, book }).ordini;
      const autorevole = spec.ownOrdersAuthoritative === true && Array.isArray(spec.ownOrders);
      let dalServer = []; let letturaOk = false; let letturaMotivo = null;
      if (autorevole) { letturaOk = true; letturaMotivo = 'saltata: il chiamante ha dichiarato la lista autorevole'; }
      else try {
        ...
        // ── LA STESSA SELEZIONE DEL PERCORSO DI DECISIONE, non una scritta a mano qui ────────────
        // `auto-reprice` chiama la stessa funzione per costruire il suo `nostriSulLato`: due filtri
        // scritti a mano sullo stesso concetto produrrebbero due book altrui diversi fra chi DECIDE
        // il prezzo e chi lo PIAZZA, cioè due risposte a «chi è il miglior concorrente».
        dalServer = NO.nostriSulLato({ orders: righe, tokenId: tokenDelLato, book }).ordini;
        letturaOk = true;
      } catch (e) {
        letturaMotivo = e && e.message ? e.message : String(e);
      }
      const u = NO.unisci(dalServer, dalClient);
      const nostri = u.ordini;
      const ownOrdersNota = autorevole
        ? 'passati dal chiamante'
        : (letturaOk
          ? `unione: ${dalServer.length} letti dal venue + ${u.aggiuntiDalClient} aggiunti dal pannello`
            + `${u.duplicati ? ` (${u.duplicati} duplicati per orderId)` : ''}`
          : `venue NON letto (${letturaMotivo}): restano i ${nostri.length} dichiarati dal pannello`);
      ...
      // ══ LA DISTANZA-OBIETTIVO PER MERCATO, LETTA DALLA STESSA FONTE DI AGENT40 ═══════════════
      //
      // ⚠ PERCHE' SI LEGGE QUI E NON ARRIVA IN UN ENV. La distanza per FASCIA di scadenza (§4.13,
      // `lib/maker/distanza-fascia.js`) deve valere sia su chi PIAZZA (agent41) sia su chi RIPREZZA
      // (agent40), o il primo rinnovo la annullerebbe. Ma agent40 non si riavvia — al suo avvio gli
      // ordini a libro diventano PRE-ESISTENTI — quindi non puo' ricevere ne' un env nuovo ne' codice
      // nuovo. L'unico canale che entrambi leggono SENZA riavvio e' `data/maker-offsets.json`, che
      // `resolveOffsetFor` rilegge da disco a ogni chiamata: agent41 ci scrive la distanza quando
      // sceglie un corto, agent40 la trova col suo TRIGGER 3 e ce lo tiene.
      //
      // ⚠ E IL PIAZZAMENTO DEVE ATTERRARCI SUBITO, non arrivarci per inseguimento: misurato, un
      // ordine posato a 2,05¢ con obiettivo 3,0¢ ha una deriva di 1,0¢ contro una soglia di 1,0¢, e
      // `Math.abs(deriva) > soglia + 1e-9` e' FALSO — l'inseguimento non scatterebbe MAI e il corto
      // resterebbe alla distanza dei lunghi per sempre. Un tick di differenza fra le due formule, e
      // la regola non esisterebbe.
      //
      // ⚠ NON SOLLEVA E NON RIPIEGA: file illeggibile o nessuna configurazione ⇒ `null` ⇒
      // `planBehindBest` legge dall'ambiente, cioe' esattamente il comportamento di prima.
      // ⚠ E NON PUO' AVVICINARE AL MID: `planBehindBest` applica la distanza-obiettivo con un
      // `Math.min` sul prezzo che «mai primo sul libro» ha gia' scelto (§4.1), quindi qualunque
      // valore uscisse da questo file puo' solo allontanare. Il vincolo non passa di qui.
      const distObiettivoC = (() => {
        try {
          const r = require('./offset-config').resolveOffsetFor({
            marketId, book, tick: rules && rules.tick });
          return (r && r.source === 'configured' && Number.isFinite(r.targetOffsetCents)
            && r.targetOffsetCents > 0) ? r.targetOffsetCents : null;
        } catch { return null; }
      })();
      const q = prezzoInCoda({
        ...
        distanzaObiettivoCents: distObiettivoC,
      });
      inCodaEsito = { ok: q.ok, mode: q.mode, onTop: q.onTop, bestOther: q.bestOther, reason: q.reason,
        ...
        // ── LA DISTANZA EFFETTIVA DAL MID, SU OGNI ORDINE PIAZZATO ────────────────────────────
        // Finora la distanza si poteva leggere solo dai record di `auto-reprice-band-exit`, cioè da
        // una valutazione di riprezzo e non dal piazzamento (è la lacuna che ha reso il campione di
        // §5-bis p.152 un proxy). Adesso ogni ordine porta la sua, e la manopola che l'ha prodotta.
        distanzaMidC: q.offsetCents != null ? q.offsetCents : null,
        distanzaObiettivo: q.distanzaObiettivo || null,
        ...
        ownOrders: { conteggio: nostri.length, origine: ownOrdersNota,
          dalVenue: dalServer.length, dalPannello: u.aggiuntiDalClient, duplicati: u.duplicati,
          venueLetto: letturaOk && !autorevole, autorevole } };
      ...
      if (q.ok && Number.isFinite(q.price) && Math.abs(q.price - price) > rules.tick / 1000) {
        ...
        // ⚠ IL RAMO CHE HA PRODOTTO IL DIFETTO. `q.price` e' il prezzo della CODA — «un tick dietro il
        // miglior ask altrui» — e su un'uscita non decide niente: la scala ha gia' scelto, e la sua
        // scelta puo' essere deliberatamente FUORI banda (carico +1 tick con la banda sotto il carico).
        // Il verdetto si scrive comunque, cosi' il delta resta leggibile.
        const p = proponiPrezzo(q.price, 'inCoda');
        if (p.applica) {
          price = q.price;
          notionalUsd = (Number.isFinite(price) && Number.isFinite(size)) ? price * size : NaN;
        } else {
          // Su un'uscita `priceAdjusted` NON si scrive: non c'e' stato nessun aggiustamento. Cio' che
          // e' successo viaggia in `uscitaVincolante`, che dice l'opposto — che una proposta e' stata
          // RIFIUTATA — e i due campi non devono poter essere scambiati da chi legge il giornale.
          priceAdjusted = null;
        }
      }
  ...
  // ── LA PROVA DI CHIUSURA, CALCOLATA UNA VOLTA SOLA E RIUSATA ────────────────────────────────────
  // Serve a due gate diversi — qui per `BELOW_MIN_SIZE`, piu' sotto per il tetto per ordine — e
  // costa una lettura dello snapshot posizioni. Calcolarla due volte vorrebbe dire due letture che
  // possono divergere fra loro, cioe' due risposte diverse alla stessa domanda dentro lo stesso
  // ordine. E' la stessa memoizzazione che l'adapter fa per l'esenzione sull'esposizione (§5-bis p.168).
  let _prova = null;
  const provaDiChiusura = () => {
    if (_prova !== null) return _prova;
    if (spec.chiudePosizione !== true) { _prova = { esente: false, motivo: 'il chiamante non dichiara una chiusura' }; return _prova; }
    try {
      const tokenIdOpposto = book === 'no' ? rules.tokenId : rules.tokenIdNo;
      const coppia = leggiCoppiaDetenuta(tokenId, tokenIdOpposto, deps.chiusuraDeps || {});
      _prova = provaChiusura({ side, size, chiudePosizione: true,
        heldSize: coppia.held, heldSizeOpposto: coppia.heldOpposto });
    } catch (e) {
      // ⚠ FAIL-CLOSED: una prova che non si riesce a fare NON e' una prova riuscita.
      _prova = { esente: false, motivo: `prova di chiusura non calcolabile (${e && e.message}): il gate resta applicato` };
    }
    return _prova;
  };

  // ⚠ `BELOW_MIN_SIZE` SI DEROGA SOLO A UN ORDINE CHE CHIUDE, E SOLO SE E' LUI A MORDERE.
  // Il minimo e' quello del programma PREMI: dice «non maturera' nulla», non «il venue rifiutera'».
  // Su una chiusura il premio non e' lo scopo, e il capitale si LIBERA invece di immobilizzarsi —
  // vedi la nota estesa in `venue-rules.splitVerdict`. Si valuta due volte di proposito: la prima
  // senza deroga, cosi' la prova (che costa una lettura) si fa solo quando serve davvero.
  // ⚠⚠ E LA QUOTA DICHIARA SE E' UN'USCITA — 24 agosto 2026. `eUscita` e' gia' calcolato piu' sopra da
  // `USCITA.eOrdineDiUscita(spec)`, cioe' dal timbro che mette `auto-close.chiudendo` (punto unico).
  // Con quel flag `validateQuote` misura la size sul MINIMO D'ORDINE del venue invece che sul pavimento
  // premiante: sono due numeri diversi che rispondono a due domande diverse, e il percorso d'uscita
  // faceva la domanda sbagliata — 22.206 rifiuti in 24 ore, 282 dei quali su un'uscita in profitto che
  // il venue avrebbe accettato (`0x65109969…`, 15,4 share contro un minimo vero di 5).
  // ⚠ NON ALLARGA NIENTE PER CHI APRE: senza il timbro il ramo e' identico a quello di ieri, e il
  // timbro lo mette solo `chiudendo`, che `riposizionaDopoChiusura` non attraversa.
  const quotaDaValidare = { side, price, size, uscita: eUscita };
  let vq = splitVerdict(validateQuote(venueRules, quotaDaValidare), { allowOutOfBand });
  let derogaMinimo = null;
  if (!vq.valid && vq.sottoMinimo === true) {
    const p = provaDiChiusura();
    if (p.esente === true) {
      derogaMinimo = p.motivo || 'chiusura provata contro lo snapshot posizioni';
      vq = splitVerdict(validateQuote(venueRules, quotaDaValidare),
        { allowOutOfBand, allowBelowMinSize: true });
    }
  }
  if (!vq.valid) {
  ...
  // ⚠ LA DEROGA SI DICHIARA SEMPRE, o domani non si potrebbe distinguere «e' passato perche' era
  // sopra il minimo» da «e' passato perche' chiudeva». E' la stessa disciplina dell'esenzione
  // sull'esposizione (§5-bis p.168): un'esenzione che non lascia traccia sporca il conteggio dopo.
  if (derogaMinimo) {
    manualAudit({ op: 'manual-place', outcome: 'deroga-sotto-minimo-per-chiusura',
      marketRef: marketId ? `cid_${String(marketId).replace(/^0x/, '')}` : null,
      requested: { book, side, price, size },
      reason: `size ${size} sotto il minimo premiante ${venueRules && venueRules.minSize}, DEROGATA perche' l'ordine`
        + ` chiude una posizione (${derogaMinimo}). Il minimo e' quello dei PREMI: su una chiusura il premio`
        + ' non e\' lo scopo e il capitale si libera. Se il venue rifiuta, il rifiuto arriva da lui.',
      userId }, source);
  }
  ...
  // ── ⚠ IL PIAZZATORE NON PUO' ESSERE PIU' PERMISSIVO DEL REPRICER — 18 agosto 2026 ────────────────
  //
  // IL FATTO. Il gate qui sotto era SOLO opt-in, e in tutto il repo `requireFreshBookMs` lo passavano
  // due soli chiamanti: `app/components/OrderPanel.tsx` e la sua rotta. **Il pannello non e' nella
  // flotta.** Quindi sul percorso che il bot usa davvero — agent41 → bulk-allocate → questa funzione —
  // il controllo di freschezza NON GIRAVA MAI. Misurato la sera del 18: il piazzatore apriva coppie su
  // un book fermo da minuti, e tre minuti dopo `auto-reprice` le cancellava per `mid-stantio`. Piazza,
  // muore, ripiazza — quaranta minuti di quel ciclo.
  //
  // Un piazzatore piu' permissivo di chi deve mantenere e' il difetto peggiore dei due: apre posizioni
  // che poi nessuno sa tenere in vita.
  //
  // LA CURA, e perche' e' «per costruzione» e non un secondo numero da tenere allineato: la soglia
  // viene da **`regimeFeed`, LA STESSA FUNZIONE che usa `auto-reprice`** — importata, non ricopiata.
  // Se domani cambia il regime del feed, cambiano insieme. E si prende il **piu' stretto** fra quella e
  // quanto il chiamante ha promesso: un chiamante puo' essere piu' severo, mai piu' permissivo.
  //
  // ⚠ RESTA OPT-IN PER CHI NON PUO' PROMETTERE, e la ragione originale non e' stata tolta: uno script o
  // un pannello su un mercato che il feed non segue non deve essere bloccato da un requisito che non ha
  // modo di soddisfare. Il cancello si applica quando il chiamante lo chiede **oppure** quando la
  // vitalita' del feed e' leggibile — cioe' quando una soglia derivata ESISTE davvero.
  const vitalitaLeggibile = rules && rules.feedVitality && typeof rules.feedVitality === 'object';
  const regime = vitalitaLeggibile ? regimeFeed(rules.feedVitality) : null;
  const sogliaDerivataMs = regime && Number.isFinite(regime.limite) ? regime.limite * 1000 : null;
  const sogliaChiesta = Number.isFinite(spec.requireFreshBookMs) && spec.requireFreshBookMs > 0
    ? spec.requireFreshBookMs : null;
  const sogliaEffettivaMs = (sogliaChiesta != null && sogliaDerivataMs != null)
    ? Math.min(sogliaChiesta, sogliaDerivataMs)
    : (sogliaChiesta != null ? sogliaChiesta : sogliaDerivataMs);
  if (Number.isFinite(sogliaEffettivaMs) && sogliaEffettivaMs > 0) {
    const maxMs = sogliaEffettivaMs;
    const ageMs = Number.isFinite(rules.midAgeSec) ? rules.midAgeSec * 1000 : null;
    if (rules.midSource !== 'live-book') {
      return refuse('stale-book', `il prezzo non viene dal book live ma da «${rules.midSource || 'fonte ignota'}»: la sottoscrizione live per questo mercato non e' attiva in questo momento. Rifiuto invece di piazzare su un prezzo che potrebbe essere vecchio di minuti. Riapri il pannello e attendi che indichi «book live».`, { causa: 'fonte-non-live', midSource: rules.midSource, midAgeSec: rules.midAgeSec, requiredMs: maxMs, sogliaDerivataMs, sogliaChiesta, regime: regime && regime.regime });
    }
    if (ageMs == null) {
      return refuse('stale-book', 'il book live non dichiara la propria eta\': il prezzo NON E\' DATABILE. E\' un caso diverso da «prezzo vecchio» — li\' il feed ha parlato e il dato e\' invecchiato, qui il feed non dice quando ha parlato, ed e\' un guasto nostro non del mercato.', { causa: 'prezzo-non-databile', midSource: rules.midSource, midAgeSec: null, requiredMs: maxMs, sogliaDerivataMs, sogliaChiesta, regime: regime && regime.regime });
    }
    if (ageMs > maxMs) {
      return refuse('stale-book', `il book live per questo mercato e' VECCHIO di ${Math.round(ageMs / 1000)}s, oltre i ${Math.round(maxMs / 1000)}s ammessi${regime ? ` in regime «${regime.regime}»` : ''} — e chi dovra' mantenere questo ordine lo rifiuterebbe con la stessa soglia.`, { causa: 'prezzo-vecchio', midSource: rules.midSource, midAgeSec: rules.midAgeSec, requiredMs: maxMs, sogliaDerivataMs, sogliaChiesta, regime: regime && regime.regime });
    }
  }
  ...
  // ── L'ESENZIONE DEI PERCORSI DI CHIUSURA (12 agosto 2026) ────────────────────────────────────────
  // Decisione dell'operatore: il tetto PER ORDINE non deve bloccare chi CHIUDE. Vale per i quattro
  // percorsi di `auto-close` — taker del merge, chiusura rapida, sorella in modalita' chiusura, i due
  // rami della chiusura forzata pre-scadenza — che dichiarano `chiudePosizione: true`.
  //
  // TRE COSE CHE LA TENGONO STRETTA, e sono la ragione per cui non apre un buco:
  //  1. la dichiarazione non basta: `provaChiusura` rifa' l'aritmetica sullo snapshot posizioni del
  //     venue, e un SELL oltre il posseduto o un BUY oltre `manca` NON viene esentato;
  //  2. si esenta SOLO il cap live-min, e solo quando e' lui a mordere. Il tetto di
  //     `safety-risk-limits` resta intatto: se e' quello il piu' stretto, il rifiuto resta;
  //  3. tutti gli altri gate sono altrove e non sono sfiorati — kill, gestione manuale, allowlist,
  //     banda, mai-primo, tetto della coppia, esposizione aperta.
  let esenzioneChiusura = null;
  if (!cg.allow && cg.gate === 'manual-order-cap' && spec.chiudePosizione === true) {
    // ⚠ LA STESSA PROVA DI SOPRA, NON UNA SECONDA: `provaDiChiusura` e' memoizzata, quindi qui non si
    // rilegge lo snapshot. Due letture della stessa posizione dentro lo stesso ordine potrebbero
    // divergere, e due gate darebbero due risposte diverse alla stessa domanda.
    esenzioneChiusura = provaDiChiusura();
    // Se il tetto di SAFETY e' piu' stretto del cap live-min, il rifiuto resta: l'esenzione riguarda
    // il secondo, non il primo. `effectiveOrderCapUsd` e' il minimo dei due, quindi basta guardare
    // quale dei due lo ha determinato.
    const mordeSafety = Number.isFinite(caps.maxOrderNotionalUsd) && notionalUsd > caps.maxOrderNotionalUsd + 1e-9;
    if (esenzioneChiusura.esente && mordeSafety) {
      esenzioneChiusura = { esente: false,
        motivo: `chiusura provata, ma a mordere e' il tetto di safety ($${caps.maxOrderNotionalUsd}) e non il cap live-min:`
          + ' l\'esenzione riguarda il secondo, non il primo — il rifiuto resta' };
    }
    manualAudit({ op: 'manual-place',
      outcome: esenzioneChiusura.esente ? 'esenzione-tetto-chiusura' : 'esenzione-tetto-chiusura-non-provata',
      marketRef: `cid_${String(marketId).replace(/^0x/, '')}`,
      requested: { book, side, price, size, notionalUsd: +notionalUsd.toFixed(4) },
      reason: esenzioneChiusura.motivo,
      observed: { capUsd: caps.effectiveOrderCapUsd, held: coppia.held, heldOpposto: coppia.heldOpposto,
        snapshotLeggibile: coppia.leggibile } }, source);
  }
  if (!cg.allow && !(esenzioneChiusura && esenzioneChiusura.esente)) return refuse(cg.gate, cg.reason, { caps });

  // ── IL TETTO PER MERCATO SUL NOZIONALE A RIPOSO ─────────────────────────────────────────────────
  // L'aritmetica vive in `valutaNozionaleMercato` (sotto), e da li' la chiamano DUE percorsi: questo, che
  // rifiuta, e il precontrollo di `replaceManualOrder`, che rifiuta PRIMA di cancellare. Vedi il blocco di
  // quella funzione per il difetto che questa separazione chiude.
  if (!(esenzioneChiusura && esenzioneChiusura.esente) && spec.chiudePosizione !== true) {
    const vm = await valutaNozionaleMercato({
      marketId, tokenId, side, price, size, notionalUsd,
      userId: spec.userId, escludiOrderId: spec.sostituisceOrderId || null, source, deps,
    });
    if (vm.gemellaDaCancellare) {
      const c = await cancelManualOrder({ orderId: vm.gemellaDaCancellare, marketId, userId: spec.userId }, source);
      if (!c || c.ok !== true) {
        return refuse('doppione-vecchio-non-cancellato',
          `esiste una gamba viva su questo token+lato (${String(vm.gemellaDaCancellare).slice(0, 12)}…) e la sua cancellazione`
          + ` non e' riuscita (${(c && c.reason) || 'motivo ignoto'}): non si piazza il secondo, o resterebbero entrambi`,
          { caps, gemella: vm.gemellaDaCancellare });
      }
      manualAudit({ op: 'manual-place', outcome: 'gemella-cancellata-prima-del-nuovo',
        marketRef: marketId ? `cid_${String(marketId).replace(/^0x/, '')}` : null,
        reason: vm.motivo, requested: { orderId: vm.gemellaDaCancellare }, userId }, source);
    }
    if (!vm.allow) return refuse(vm.gate, vm.motivo, { caps, ...vm.osservato });
  }
  ...
  // ══ I DUE CONTROLLI TERMINALI SULL'USCITA — 24 agosto 2026, un istante prima della POST ═══════
  //
  // ⚠ STANNO QUI E NON PIU' IN ALTO, ED E' IL PUNTO. Il difetto del 24 agosto non e' stato una
  // decisione sbagliata: la decisione era giusta (0.495 il SELL, 0.516 il BUY, coppia 101,0¢ esatta).
  // E' stato cio' che e' successo DOPO la decisione e PRIMA dell'invio. Un controllo sul prezzo
  // deciso avrebbe risposto «ammesso» a un ordine che rompeva il tetto di 20,1¢: si guarda il numero
  // che parte, non quello che si voleva.
  //
  // ⓐ IL PREZZO CHE PARTE E' ANCORA QUELLO DECISO DALLA SCALA?
  // I divieti piu' sopra fanno gia' in modo che nessun ramo lo muova. Questa e' l'asserzione che lo
  // DIMOSTRA a ogni invio, invece di fidarsi: se un ramo nuovo — domani, scritto da qualcun altro —
  // riscrivesse il prezzo di un'uscita, l'ordine non parte e la riga dice quale delta e' comparso.
  if (eUscita && Number.isFinite(prezzoDecisoScala)) {
    const tol = Number.isFinite(rules && rules.tick) && rules.tick > 0 ? rules.tick / 1000 : 1e-9;
    if (Math.abs(price - prezzoDecisoScala) > tol) {
      return refuse('uscita-prezzo-riscritto',
        `ordine di USCITA: la scala aveva deciso ${prezzoDecisoScala} e il prezzo che partirebbe e' `
        + `${price} (${((price - prezzoDecisoScala) * 100).toFixed(3)}¢). Il prezzo deciso e' vincolante: `
        + 'l\'ordine non parte. Rami che hanno proposto uno spostamento in questo ordine: '
        + (aggiustamentiRifiutati.length ? aggiustamentiRifiutati.map((a) => a.ramo).join(', ') : 'nessuno dichiarato'),
        { prezzoDeciso: prezzoDecisoScala, prezzoProposto: price,
          deltaCents: +((price - prezzoDecisoScala) * 100).toFixed(4), aggiustamentiRifiutati });
    }
  }

  // ⓑ UN BUY DI COMPLETAMENTO NON PARTE SE LA COPPIA, SUL PREZZO DI INVIO, SUPERA IL TETTO.
  // ⚠ IL CARICO SI LEGGE DALLO SNAPSHOT DEL VENUE, non da una dichiarazione del chiamante: e' la
  // stessa fonte che l'esenzione di chiusura usa gia' per dimostrare quante share possediamo, e una
  // dichiarazione di cui fidarsi sarebbe proprio il modo in cui questo tetto e' stato aggirato.
  // ⚠ FAIL-CLOSED: carico illeggibile ⇒ non parte. Un completamento che non si sa giudicare e' un
  // completamento che non si manda; la direzione opposta lascerebbe passare il caso non misurato.
  if (eUscita && side === 'BUY') {
    let carico = Number(spec.carico);
    if (!Number.isFinite(carico) || carico <= 0) carico = Number(spec.prezzoCarico);
    if (!Number.isFinite(carico) || carico <= 0) {
      try {
        const tokenIdOpposto = book === 'no' ? rules.tokenId : rules.tokenIdNo;
        const coppia = leggiCoppiaDetenuta(tokenId, tokenIdOpposto, deps.chiusuraDeps || {});
        carico = Number(coppia && coppia.caricoOpposto);
      } catch { carico = NaN; }
    }
    const vt = USCITA.verdettoTettoCoppia({
      side, carico: Number.isFinite(carico) && carico > 0 ? carico : null,
      prezzoInvio: price, tettoCoppiaCents: TETTO_COPPIA_CENTS });
    if (!vt.ammesso) {
      return refuse('uscita-coppia-oltre-il-tetto',
        `BUY di completamento rifiutato al controllo terminale: ${vt.motivo}`,
        { carico: Number.isFinite(carico) ? carico : null, prezzoInvio: price,
          coppiaCents: vt.coppiaCents, tettoCents: vt.tettoCents });
    }
  }
  ...
  try {
    adapter = buildPlacementAdapter({
      ...
      // ⚠ LO STESSO ambiente da cui e' venuto `placement` due righe sopra: se le cinture si leggessero
      // da due fonti diverse, un test che inietta `deps.env` proverebbe una configurazione che il
      // processo non ha. Una domanda, una fonte.
  ...
  try {
    res = await adapter.postOrder({
      ...
      tickSize: rules.tick, negRisk: rules.negRisk === true,
      // ⚠ IL CAMPO SUL FILO SEGUE LA DECISIONE GIA' PRESA — 16 agosto 2026.
      // Qui c'era `postOnly: true` CABLATO FISSO, e rendeva irraggiungibile una decisione che questo
      // stesso file prende venti righe piu' su: `attraversaApposta` (riga ~1100) permette a una SELL
      // dichiarata, o al completamento di una coppia provato, di attraversare lo spread — ma l'ordine
      // partiva comunque post-only e il venue lo rifiutava con «invalid post-only order: order crosses
      // book». Il gate interno diceva si', il campo sul filo diceva no, e vinceva il filo.
      // ⚠ E' LA RAGIONE PER CUI IL LIVELLO 1 (TAKER) DELLA SCALA D'USCITA NON HA MAI POTUTO ESEGUIRE
      // (§5-bis p.29). Misurato il 16/08 su FL-27: posizione scoperta da cinque ore, uscita a 21c che
      // non si riempiva, e ogni tentativo di uscire al bid respinto dal venue.
      // ⚠ NON APRE NIENTE: `attraversaApposta` e' vero solo per una SELL dichiarata dal chiamante o per
      // un completamento di coppia gia' provato — entrambe azioni che RIDUCONO esposizione. Su ogni
      // altro ordine `postOnly` resta `true`, quindi una quotazione di liquidita' non puo' incrociare
      // per sbaglio: era il presidio, e resta.
      postOnly: !attraversaApposta,
      venueRules, userId,
      ...
      allowOutOfBand,
      // ── LA DICHIARAZIONE DI CHIUSURA, E IL TOKEN DELL'ALTRO LATO ─────────────────────────────
      // Stessa ragione di `allowOutOfBand` qui sopra: l'adapter rifa' la propria verifica in modo
      // indipendente sulla propria cintura, quindi senza portargliela il tetto riaprirebbe un passo
      // piu' avanti. Il token opposto lo deriviamo QUI dalle regole del venue e non lo chiediamo al
      // chiamante: e' un fatto del mercato, e farselo dire sarebbe un modo di poterselo far mentire.
      chiudePosizione: spec.chiudePosizione === true ? true : undefined,
      // ── E IL TIMBRO D'USCITA VIAGGIA CON L'ORDINE — 24 agosto 2026 ──────────────────────────
      // Stessa ragione, parola per parola, di `allowOutOfBand` e `chiudePosizione` qui sopra:
      // l'adapter rifa' la propria validazione in modo indipendente, quindi senza portargliela il
      // gate riapre un passo piu' avanti e il rifiuto torna identico, solo da un altro nome. E'
      // esattamente cio' che e' successo: `validateQuote` qui declassava `BELOW_MIN_SIZE` ad avviso
      // sul minimo d'ordine vero, e l'adapter — che non riceveva il timbro — lo ricalcolava
      // bloccante sul pavimento premiante. Terza occorrenza della forma «protezione presente su un
      // percorso e assente sul gemello».
      // ⚠ SI PASSA IL TIMBRO, NON LA DEROGA: l'adapter rifa' la PROVA sullo snapshot del venue con
      // la stessa `provaChiusura`. Portargli «e' gia' stato derogato» sarebbe una dichiarazione di
      // cui fidarsi; portargli «questo e' un'uscita» gli lascia rifare la domanda.
      [USCITA.CAMPO_USCITA]: eUscita ? true : undefined,
      tokenIdOpposto: book === 'no' ? rules.tokenId : rules.tokenIdNo,
      // ── I DUE FATTI CHE ARRIVAVANO SOLO AL GIORNALE MAKER ────────────────────────────────────
      // `inCoda` e `priceAdjusted` finivano in `polymarket-maker-audit.jsonl` e nel valore di
      // ritorno, ma NON in `execution-audit.jsonl` — che e' il registro su cui si ricostruisce cosa
      // e' successo a un ordine, e l'unico che l'idempotenza legge. Si passano all'adapter, che li
      // mette nella riga di intent come campi propri.
      inCoda: inCodaEsito,
      priceAdjusted,
  ...
  // ══ SE E' PARTITA UN'USCITA, IL SUO orderId FINISCE NEL REGISTRO ═══════════════════════════════
  // Serve a `auto-reprice`: gli ordini gli tornano dal VENUE, e il venue conosce prezzo, lato e size —
  // non le nostre intenzioni. Senza questo registro `band-exit` non ha modo di sapere che quel BUY a
  // 0.516 non e' una quotazione fuori banda da rimettere in banda, ed e' esattamente per questo che
  // l'ha spostato nove volte. Il registro porta anche il PREZZO DECISO, che serve a `close-sell-floor`
  // per confrontarsi con la scala invece che con l'ordine vivo.
  // ⚠ SOLO SE E' PARTITO DAVVERO. Un dry-run non ha un ordine al venue da proteggere, e registrarlo
  // riempirebbe il file di id che nessuno rivedra' mai.
  // ⚠ NON SOLLEVA E NON CAMBIA L'ESITO: l'ordine e' gia' al venue, e far fallire il ritorno di questa
  // funzione a cose fatte non lo riporterebbe indietro — trasformerebbe un guasto di osservabilita' in
  // un guasto di esecuzione. Il motivo, se fallisce, va nella riga di audit qui sotto.
  let registroUscita = null;
  if (eUscita && res.ok === true && res.sent === true && res.orderId) {
    registroUscita = USCITA.registraUscita({
      orderId: res.orderId, marketId, book, side,
      prezzoDeciso: Number.isFinite(prezzoDecisoScala) ? prezzoDecisoScala : price });
  }
  ...
  manualAudit({
    ...
    inCoda: inCodaEsito,
    priceAdjusted,
    // ── L'USCITA, DETTA PER INTERO: che lo fosse, quale prezzo aveva deciso la scala, e quali rami
    //    avrebbero voluto spostarlo. Senza questi campi «l'ordine e' partito al prezzo giusto» e
    //    «l'ordine e' partito al prezzo giusto DOPO che due rami hanno provato a muoverlo» hanno la
    //    stessa identica riga — ed e' la seconda che si vuole poter contare.
    uscita: eUscita ? {
      prezzoDeciso: Number.isFinite(prezzoDecisoScala) ? prezzoDecisoScala : null,
      prezzoPartito: price,
      aggiustamentiRifiutati,
      registro: registroUscita,
    } : null,
...
/**
 * LA CHIAVE DI IDEMPOTENZA DI UN RIMPIAZZO — UNA SOLA DERIVAZIONE, DUE LETTORI.
 *
 * La calcolano il PRECONTROLLO (prima di cancellare) e l'INVIO (dopo). Erano la stessa espressione
 * scritta due volte finché il precontrollo non esisteva; ora sono due chiamate alla stessa funzione,
 * perché due copie che divergono qui vorrebbero dire precontrollare una chiave e spedirne un'altra —
 * cioè un precontrollo che non protegge da niente.
 */
function chiaveDiRimpiazzo({ userId, orderId, side, price, size }) {
  return `idem_rep_${crypto.createHash('sha256')
    .update([userId || OPERATOR_USER, VENUE, orderId, side, price, size].map(String).join('|'))
    .digest('hex').slice(0, 20)}`;
}

/**
 * LA CHIAVE È GIÀ BRUCIATA? — precontrollo di idempotenza, e si esegue PRIMA della cancellazione.
 *
 * PERCHÉ ESISTE. Un riprezzo è cancella→ripiazza. Se il ripiazzo viene rifiutato come duplicato, la
 * cancellazione è già avvenuta e la gamba resta scoperta: sei degli otto casi misurati sui venti giorni
 * di storico sono esattamente questi (§5 punto 73). Il gate anti-doppio-invio faceva il suo mestiere —
 * era il momento in cui veniva interrogato a essere sbagliato.
 *
 * LA REGOLA, E PERCHÉ È PIÙ STRETTA DI QUELLA DEL PIAZZAMENTO. Nessun intent per questa chiave ⇒ il gate
 * dei duplicati non può scattare, si procede. Un intent che esiste ⇒ NON si cancella, a meno che il
 * chiamante sappia PROVARE che l'ordine con cui collide è morto sul venue, passando `vivi`. Il percorso
 * di piazzamento può permettersi di scoprirlo dopo (`risolviDuplicato` supera il duplicato e conia una
 * chiave nuova); qui no, perché «scoprirlo dopo» significa averlo già cancellato.
 *
 * E NON COSTA UN RIPREZZO LEGITTIMO: la chiave nomina l'ordine che sostituisce, quindi un intent che
 * esiste già vuol dire che QUELLO stesso ordine è già stato sostituito a quel prezzo e quella size — la
 * fotografia su cui il ciclo sta lavorando è vecchia, e rifiutare è la risposta giusta, non un costo.
 *
 * FAIL CLOSED IN OGNI DIREZIONE: registro illeggibile, `risolviDuplicato` che solleva, insieme dei vivi
 * non accertato ⇒ non si cancella. L'ordine resta a riposo e il ciclo successivo riprova.
 */
function valutaChiaveRimpiazzo(chiave, { vivi = null } = {}, deps = {}) {
  const ea = deps.executionAudit || require('../safety/execution-audit');
  const auditDeps = deps.auditDeps || {};
  let esiste;
  try { esiste = ea.hasIntent(chiave, auditDeps); }
  catch (e) {
    return { allow: false, gate: 'idempotent-duplicate', chiave,
      reason: `il registro di idempotenza non è leggibile (${e && e.message}) — non si cancella un ordine senza poter dire se il suo rimpiazzo sarebbe accettato` };
  }
  if (esiste !== true) {
    return { allow: true, gate: null, chiave, reason: null, motivo: 'nessun intent per questa chiave' };
  }
  if (vivi instanceof Set) {
    let r;
    try { r = ea.risolviDuplicato(chiave, { vivi }, auditDeps); }
    catch (e) {
      return { allow: false, gate: 'idempotent-duplicate', chiave,
        motivo: `la risoluzione del duplicato è fallita (${e && e.message})`,
        reason: `la risoluzione del duplicato è fallita (${e && e.message}) — il vecchio ordine NON è stato cancellato` };
    }
    if (r && r.superabile === true) {
      return { allow: true, gate: null, chiave, reason: null, motivo: r.motivo, superata: r.chiave };
    }
    return { allow: false, gate: 'idempotent-duplicate', chiave, motivo: (r && r.motivo) || 'duplicato non superabile',
      reason: `la chiave di idempotenza del rimpiazzo è già in uso e non è superabile (${(r && r.motivo) || 'motivo non dichiarato'}) — il vecchio ordine NON è stato cancellato` };
  }
  return { allow: false, gate: 'idempotent-duplicate', chiave,
    motivo: 'intent già registrato per questa chiave e gli ordini vivi sul venue non sono stati accertati',
    reason: 'esiste già un intent per la chiave di idempotenza di questo rimpiazzo, e senza la lista degli ordini vivi sul venue non si può dire che sia superabile — il vecchio ordine NON è stato cancellato, il ciclo successivo riprova' };
}
 ...
 *   • CINQUE PRECONTROLLI PRIMA DELLA CANCELLAZIONE, e sono l'unica difesa contro la finestra scoperta
 *     descritta qui sotto: kill switch · orologio del mercato · guard condiviso sul prezzo proposto ·
 *     TETTO PER ORDINE · CHIAVE DI IDEMPOTENZA. Gli ultimi due sono stati aggiunti l'11 agosto 2026 dopo
 *     una misura su venti giorni di storico: otto volte il vecchio ordine era stato cancellato e il nuovo
 *     rifiutato, e in ogni caso il rifiuto veniva da uno di quei due limiti. Ognuno dei cinque risponde
 *     `oldCancelled:false` e lascia l'ordine dov'è: meglio una gamba leggermente fuori posizione che
 *     nessuna gamba. Chi aggiunge un gate a `placeManualOrder` che può rifiutare un rimpiazzo deve
 *     aggiungerlo anche qui, altrimenti riapre esattamente questa finestra.
...
async function replaceManualOrder(spec = {}, deps = {}) {
  ...
  const preflight = validateQuote(
    { tick: rules.tick, scoringMid, maxSpreadCents: rules.maxSpreadCents,
      minSize: rules.minSize, minOrderSize: rules.minOrderSize, marketId: rules.marketId },
    // ⚠ ANCHE IL RIPREZZO ATOMICO DICHIARA SE STA MUOVENDO UN'USCITA: `replaceManualOrder` sostituisce
    // anche gli ordini di chiusura (il rinnovo GTD passa di qui), e giudicarli col pavimento premiante
    // sarebbe la stessa confusione dei due minimi, su un percorso diverso.
    { side: replSide, price: Number(spec.price), size: Number(spec.size), uscita: USCITA.eOrdineDiUscita(spec) },
  );
  if (!preflight.valid) {
  ...
  // ── IL TETTO PER ORDINE, ANCH'ESSO PRIMA DELLA CANCELLAZIONE ────────────────────────────────────────
  // Stesso ragionamento del kill e dell'orologio del mercato, e stesso guasto misurato: il 2026-08-10 alle
  // 04:08:55 l'inseguimento del mid ha spostato una gamba da 0,671 a 0,677, il controvalore è passato a
  // $70,38 contro un tetto di $70,00, il vecchio ordine ERA GIÀ STATO CANCELLATO e il nuovo è stato
  // rifiutato. Ventun secondi dopo la gamba opposta è stata riempita: $33,42 di esposizione direzionale
  // scoperta, che auto-close non ha poi potuto coprire perché rifiutata dallo STESSO tetto, 54 volte.
  //
  // NESSUNA COSTANTE NUOVA, ED È LA PARTE CHE CONTA: `resolveCaps` + `evaluateManualCapGate` sono le due
  // funzioni che il GATE 4 del piazzamento già usa. Il tetto per ordine vive in `lib/rewards/concentration`
  // (`LIVE_MIN_ORDER_CAP_USD`, derivato da `MARKET_CAP_FIXED_USD / 2 + 5`) e arriva qui per la stessa
  // strada. Precontrollare con un numero diverso da quello che rifiuta sarebbe peggio di non
  // precontrollare: darebbe l'impressione di una garanzia che non c'è.
  const notionalRimpiazzo = (Number.isFinite(Number(spec.price)) && Number.isFinite(Number(spec.size)))
    ? Number(spec.price) * Number(spec.size) : NaN;
  const capsRimpiazzo = resolveCaps({ userId: spec.userId || OPERATOR_USER, engine }, deps.limitDeps || {});
  const cgRimpiazzo = evaluateManualCapGate({ notionalUsd: notionalRimpiazzo, caps: capsRimpiazzo });
  if (!cgRimpiazzo.allow) {
    manualAudit({ op: 'manual-replace', outcome: 'reject-cap-preflight',
      requested: { orderId, book, price: spec.price, size: spec.size, notionalUsd: Number.isFinite(notionalRimpiazzo) ? notionalRimpiazzo : null },
      reason: cgRimpiazzo.reason, latencyMs: Date.now() - t0 }, source);
    return { ok: false, gate: cgRimpiazzo.gate, replaced: false, oldCancelled: false, caps: capsRimpiazzo,
      reason: `${cgRimpiazzo.reason} — il vecchio ordine NON è stato cancellato: un riprezzo è cancella+ripiazza, e il ripiazzo sarebbe stato rifiutato dal tetto, lasciando la gamba scoperta. Resta a riposo dov'è; il ciclo successivo riprova.` };
  }

  // ── E LA CHIAVE DI IDEMPOTENZA, L'ULTIMO DEI CINQUE PRECONTROLLI ────────────────────────────────────
  // La chiave si deriva QUI, prima della cancellazione, e la stessa identica chiave viene poi spedita allo
  // STEP 2: è la ragione per cui la derivazione è una funzione sola (`chiaveDiRimpiazzo`) e non
  // un'espressione ripetuta. Sei degli otto guasti misurati sono di questa classe.
  const chiaveRimpiazzo = chiaveDiRimpiazzo({ userId: spec.userId, orderId, side: replSide, price: spec.price, size: spec.size });
  const chiaveOk = valutaChiaveRimpiazzo(chiaveRimpiazzo, { vivi: spec.ordiniVivi || null }, deps);
  if (!chiaveOk.allow) {
    manualAudit({ op: 'manual-replace', outcome: 'reject-idempotency-preflight',
      requested: { orderId, book, price: spec.price, size: spec.size },
      reason: chiaveOk.motivo || chiaveOk.reason, latencyMs: Date.now() - t0 }, source);
    return { ok: false, gate: chiaveOk.gate, replaced: false, oldCancelled: false, reason: chiaveOk.reason };
  }

  // ── IL SESTO PRECONTROLLO: IL TETTO PER MERCATO SUL NOZIONALE A RIPOSO ──────────────────────────────
  // ⚠ ERA IL BUCO DI §5.2 p.38, e costava 160,8 minuti di gamba singola il 16 agosto. I cinque
  // precontrolli sopra esistono tutti per la stessa ragione — «non cancellare cio' che non si puo'
  // ripiazzare» — e questo gate NON era fra loro: viveva solo dentro `placeManualOrder`, cioe' DOPO la
  // cancellazione. Il riprezzo passava i cinque, cancellava, e il sesto rifiutava.
  // ⚠ `escludiOrderId: orderId` E' LA META' CHE CONTA: l'ordine che si sta sostituendo E' GIA' fra quelli
  // a riposo. Senza l'esclusione il conto lo somma DUE volte e il precontrollo rifiuterebbe un riprezzo
  // che non aggiunge un dollaro — che e' il difetto opposto, e altrettanto costoso.
  // ⚠ E si chiama la STESSA funzione che rifiuta a valle: un precontrollo con un numero diverso da quello
  // che poi decide e' peggio di nessun precontrollo (reperto D1).
  if (spec.chiudePosizione !== true) {
    const notionalRimpiazzo = Number(spec.price) * Number(spec.size);
    const vmPre = await valutaNozionaleMercato({
      // Il token del lato che si sta riprezzando, dalle regole del mercato: non si indovina.
      marketId, tokenId: book === 'no' ? rules.tokenIdNo : rules.tokenId,
      side: replSide, price: spec.price, size: spec.size,
      notionalUsd: notionalRimpiazzo, userId: spec.userId, escludiOrderId: orderId, source, deps,
    });
    if (!vmPre.allow) {
      manualAudit({ op: 'manual-replace', outcome: `reject-${vmPre.gate}`,
        requested: { orderId, book, price: spec.price, size: spec.size, notionalUsd: +notionalRimpiazzo.toFixed(4) },
        reason: vmPre.motivo, observed: vmPre.osservato || null, latencyMs: Date.now() - t0 }, source);
      return { ok: false, gate: vmPre.gate, replaced: false, oldCancelled: false, reason: vmPre.motivo,
        osservato: vmPre.osservato || null };
    }
  }
  ...
  //
  // LA CHIAVE È QUELLA GIÀ PRECONTROLLATA, non una calcolata di nuovo: `chiaveRimpiazzo` nasce sopra,
  // prima della cancellazione, e arriva qui invariata. Se il precontrollo l'ha trovata superabile
  // (`chiaveOk.superata`) si spedisce comunque l'originale: superare il duplicato è mestiere
  // dell'adapter, che lo fa già leggendo gli ordini vivi dal venue: due punti che coniano chiavi
  // di superamento sarebbero due catene diverse sullo stesso ordine.
  //
  // ── ⚠ `sostituisceOrderId`: L'ORDINE APPENA CANCELLATO NON E' UN DOPPIONE — 20 agosto 2026 ────────
  //
  // IL FATTO, misurato: 17 episodi in 11 ore, di cui 3 nei 19 minuti dopo il riavvio delle 10:40. Ogni
  // volta la stessa forma — `expiry-refresh` ripiazza allo STESSO prezzo per resettare la GTD
  // (`fromPrice === toPrice` in tutti e 17), il cancel riesce, e il place viene rifiutato come
  // `doppione-identico` contro l'ordine CHE SI E' APPENA CANCELLATO. Esito
  // `{oldCancelled:true, replaced:false}`: la gamba esce dal libro. Alle 09:16:59 su `0x12dc2b61` NO la
  // gamba non e' MAI rientrata, e il mercato e' poi caduto del tutto.
  //
  // ⚠ NON E' UNA CACHE: `listManualOrders` chiama `adapter.listOpenOrders()`, cioe' il VENUE. Il
  // problema e' che fra il `cancel` confermato e la `list` passano ~73 ms (07:07:48.705 → 48.778) e la
  // vista degli ordini aperti del CLOB non ha ancora recepito la cancellazione. E' una corsa con la
  // consistenza eventuale del venue, non un dato stantio nostro.
  //
  // ⚠ IL RIMEDIO ESISTEVA GIA' E NON ERA CABLATO. `escludiOrderId` e' nato il 17 agosto (§5.2 p.38) per
  // questo identico motivo e funziona: il PRECONTROLLO di questa stessa funzione lo passa
  // (`escludiOrderId: orderId`, sopra) e infatti non rifiuta mai. Ma `placeManualOrder` lo legge da
  // `spec.sostituisceOrderId` (:1476) — e in tutto il repo quel campo era LETTO in un punto e SCRITTO in
  // NESSUNO. Quinta occorrenza di «dep dichiarata e mai iniettata ⇒ valore di difetto che nessuno ha
  // chiesto» (§5-bis): il ramo restituiva `null`, cioe' «non escludere niente».
  //
  ...
  // continua a essere rifiutato. E l'esclusione e' anche aritmeticamente giusta sulla somma a riposo:
  // quell'ordine non e' piu' a riposo, contarlo gonfierebbe il nozionale del mercato.
  const placed = await placeManualOrder({ ...spec, marketId, book, side: replSide, source,
    idempotencyKey: chiaveRimpiazzo, sostituisceOrderId: orderId }, deps);
  const replaced = placed.ok === true;

  // ⚠ `side` NEL RECORD DEL RIPREZZO, dal 17 agosto 2026 sera. Mancava, e la sua assenza rendeva il
  // giornale inservibile per una domanda precisa: «a che prezzo era l'ultimo nostro BUY su questo token?»
  // — il secondo livello di `carico-di-ripiego`. Con `book` e senza `side` un rimpiazzo e' indistinguibile
  // fra acquisto e vendita, e dedurlo sarebbe tirare a indovinare su un dato che serve a decidere
  // un'uscita. E' additivo: nessun lettore esistente cambia comportamento.
  manualAudit({ op: 'manual-replace', outcome: replaced ? (placed.sent ? 'sent' : 'dry-run-validated') : `reject-${placed.gate || 'place'}`,
    requested: { orderId, book, side: replSide, price: spec.price, size: spec.size },
...
module.exports = {
  ...
  chiaveDiRimpiazzo, valutaChiaveRimpiazzo,

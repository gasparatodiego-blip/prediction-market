...
// ═══ L'ARMAMENTO DEL GRADINO 6 — DECISIONE DELL'OPERATORE, 13 AGOSTO 2026 ════════════════════════
// Il gradino 6 è stato cablato oggi (`53b80d8`): fino a stamattina chiamava `impostaBot` senza
// importarlo, quindi moriva con un ReferenceError catturato dal `try` e il bot restava su AVVIA.
// Falliva CHIUSO — nessun capitale a rischio — ma la difesa non esisteva.
//
// L'operatore ha deciso di NON armarlo al primo riavvio, e la ragione è precisa: FERMA **non ha
// riarmo automatico**, quindi il primo scatto richiederebbe una mano umana per far ripartire il bot.
// Con la causa a monte ancora aperta (§5.2 p.21) è verosimile che scatti entro ore, e il bot deve
// restare autonomo. Vedi CLAUDE.md §5.2.
//
// ⚠ IL DISARMO NON TOGLIE IL GRADINO: lo rende MUTO E LOQUACE. La scala continua a salire fino a 6,
// il gradino viene raggiunto, valutato e **registrato con il suo motivo** — semplicemente non tocca
// l'interruttore. È esattamente il dato che serve per decidere se armarlo domani: quante volte
// sarebbe intervenuto, e perché.
//
// ⚠ E NON TOCCA NIENTE ALTRO. Guardiano delle perdite (agent43), sentinella del collasso e KILL sono
// difese indipendenti da questa scala e restano attive: nessuna di loro passa da qui.
//
// ═══ IL VERSO DEL DEFAULT, E PERCHÉ È QUESTO ═════════════════════════════════════════════════════
// **Assente, vuoto o illeggibile ⇒ ARMATO.** Solo la stringa esatta `'0'` disarma. È il verso sicuro
// e la stessa regola di `end-of-scale` e dell'orizzonte: un valore che non si capisce non deve poter
// spegnere una difesa. La conseguenza voluta: se il demone pm2 ripartisse da una shell pulita e la
// variabile sparisse, il gradino torna ARMATO da solo invece di restare spento in silenzio.
const ENV_GRADINO6 = 'SBLOCCO_GRADINO6_ARMATO';

/**
 * Il gradino 6 è armato?
 *
 * @returns `{armato:boolean, motivo:string}` — `motivo` è la frase che finisce nel registro, così
 *          «non ha fermato il bot» non si confonde mai con «ha provato e non ci è riuscito».
 */
function gradinoSeiArmato(env = process.env) {
  const raw = env ? env[ENV_GRADINO6] : undefined;
  if (String(raw).trim() === '0') {
    return { armato: false, motivo: `disarmato per decisione dell'operatore (${ENV_GRADINO6}=0, 13 agosto 2026): si registra e non si tocca l'interruttore` };
  }
  return { armato: true, motivo: raw === undefined || String(raw).trim() === ''
    ? 'armato (nessuna configurazione: è il difetto sicuro)'
    : `armato (${ENV_GRADINO6}=${String(raw)}: solo «0» disarma)` };
}

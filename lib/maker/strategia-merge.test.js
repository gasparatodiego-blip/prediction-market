...
(async () => {
  ...
  console.log('\n══ 8 · L INTERRUTTORE E ACCESO, E CIO CHE COMPORTA E SCRITTO');
  {
    ...
    // LA COSA CHE NON DEVE POTER CAMBIARE IN SILENZIO: accendere i due livelli NON ha acceso il merge
    // on-chain. Se un giorno CTF_RELAYER_ENABLED diventasse true senza una decisione, questo cade.
    const { CTF_RELAYER_ENABLED } = require('./ctf-relayer');
    ok('  il merge on-chain resta SPENTO', CTF_RELAYER_ENABLED === false,
      'il capitale della coppia torna alla risoluzione, non subito: e la premessa su cui la decisione e stata presa');

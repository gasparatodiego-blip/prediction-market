...
// da non tagliare l'allocazione che il piano puo' produrre.
{
  ...
  return require('./quanti-mercati').quantiMercati(app.env).quanti;
    ...
    const raggiungibile = CONC.esposizioneMassimaRaggiungibileUsd(N);
    ok('  N si legge dall ambiente e la grandezza e calcolabile', Number.isFinite(raggiungibile),
      `N=${N} ⇒ ${raggiungibile}`);
    ok('  ⚑ il tetto di esposizione COPRE l esposizione massima raggiungibile (riposo + completamento)',
      ...
      `$${limiti.global.maxOpenNotionalUsd} contro N=${N} × 2 × $${CONC.MARKET_CAP_FIXED_USD} = $${raggiungibile}`);

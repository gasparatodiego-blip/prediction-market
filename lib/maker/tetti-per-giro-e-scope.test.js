...
// da non tagliare l'allocazione che il piano puo' produrre.
{
  ...
  ok('  e il tetto di esposizione aperta regge la selezione intera al tetto per mercato',
    limiti.global.maxOpenNotionalUsd
      >= require('./selezione-mercati').MAX_MERCATI_CONTEMPORANEI * CONC.MARKET_CAP_FIXED_USD,
    `$${limiti.global.maxOpenNotionalUsd} contro $${(require('./selezione-mercati').MAX_MERCATI_CONTEMPORANEI * CONC.MARKET_CAP_FIXED_USD).toFixed(2)}`);
}

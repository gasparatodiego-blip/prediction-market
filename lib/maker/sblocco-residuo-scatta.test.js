...
sez('④ lo scatto: dal residuo bloccato all\'ordine inviato');
{
  ...
  const righe = [];
  // ⚠ QUESTO BLOCCO ASSERIVA LA PROPRIETÀ OPPOSTA, ed è stato RISCRITTO — non ammorbidito — il
  // 18 agosto 2026. Diceva: «senza board il minimo è ignoto ⇒ nessun acquisto (fail-closed)», e la
  // frase era vera al 17 e falsa apposta dal 18. Il minimo non viene più dal solo board: viene da
  // board → catalogo di ripiego → venue (`min-size-mercato.js`), perché un mercato fuori dal board
  // è lo stato NORMALE di una posizione vecchia e quel ramo non si raggiungeva mai. Il fail-closed
  // non è stato tolto: si è spostato dove appartiene — «nessuna delle tre fonti risponde» — ed è
  // provato qui sotto, subito dopo lo scatto.
  // ⚠ LE ANCORE VANNO IN UNA DIRECTORY TEMPORANEA, e prima non lo erano: questo test scriveva `yes6`
  // dentro `data/presidio-posizioni.json`, cioè nello stato di PRODUZIONE del presidio. E senza
  // un'ancora vecchia la posizione risultava appena nata — sotto la soglia dei 60 minuti — quindi non
  // veniva nemmeno giudicata: il blocco ④ era verde su un percorso mai raggiunto.
  const fsT = require('fs'); const osT = require('os');
  const dirT = fsT.mkdtempSync(path.join(osT.tmpdir(), 'sblocco-'));
  const fileAncore = path.join(dirT, 'ancore.json');
  fsT.writeFileSync(fileAncore, JSON.stringify({ ancore: { yes6: T } }));

  const guida = (leggiVenue) => {
    righe.length = 0; inviati.length = 0;
    return A.presidioPosizioniVecchie({
      ...
      leggiCatalogo: () => ({}),
      leggiVenue,
      // Nessuna scrittura sul catalogo di produzione: un test che guida una funzione che scrive deve
      // poterle dire dove (§10 di APERTI.md, tre occorrenze in un giorno).
      salvaCatalogo: () => ({ ok: true }),
      piazza: async (spec) => { inviati.push(spec); return { ok: true, orderId: 'X' }; },
      scrivi: (r) => righe.push(r),
    });
  };

  // ① IL VENUE CONOSCE IL MERCATO: 6 share contro un minimo di 20 ⇒ il ramo di R6 si RAGGIUNGE.
  return guida(async () => ({ readable: true, minSizeShares: 20, tick: 0.001, negRisk: true,
    tokenIdYes: 'yes6', tokenIdNo: 'no6', endDate: '2026-08-20T00:00:00Z' })).then((e) => {
    ...
    ok('  la gamba è riconosciuta SOTTO IL MINIMO anche fuori dal board', pres[0] && pres[0].sottoMinimo === true,
      JSON.stringify(pres[0] && pres[0].sottoMinimo));
    // ⚠ QUESTA È LA DIFFERENZA CHE CONTA: l'esito della rinuncia non è più
    // `rinunciata-prezzo-non-calcolabile` — cioè «non ho nemmeno guardato» — ma
    // `rinunciata-sblocco-oltre-tetto`, cioè «ho valutato lo sblocco e non conviene». Il primo è un
    // presidio cieco, il secondo è una decisione.
    ok('  e lo SBLOCCO viene valutato, non saltato', pres[0] && pres[0].esito === 'rinunciata-sblocco-oltre-tetto',
      String(pres[0] && pres[0].esito));
    ok('  col costo che sarebbe servito, a verbale', pres[0] && pres[0].sblocco !== null);
    // Senza la scala degli ask dell'altro lato non si compra al buio: `valutaSblocco` è fail-closed.
    ok('  ma senza il libro dell\'altro lato NON si compra', inviati.length === 0,
      JSON.stringify(inviati).slice(0, 120));

    // ② IL FAIL-CLOSED, DOVE ADESSO APPARTIENE: nessuna delle tre fonti risponde.
    return guida(async () => ({ readable: false, error: 'HTTP 404' })).then(() => {
      const pres2 = righe.filter((r) => r.tipo === 'presidio-posizioni-vecchie');
      ok('nessuna fonte conosce il minimo: NON si marca e NON si compra',
        pres2[0] && pres2[0].sottoMinimo === false && inviati.length === 0);
      ok('  e il fatto è dichiarato a verbale, non taciuto',
        righe.some((r) => r.tipo === 'min-size-recupero' && r.nonTrovati.length === 1));

      try { fsT.rmSync(dirT, { recursive: true, force: true }); } catch { /* niente */ }

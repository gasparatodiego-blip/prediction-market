...
(async () => {
  ...
  console.log('\n══ IL RATE LIMIT NON PUO SPEZZARE UNA COPPIA A META');
  {
    // Tetto 20/60s, 19 gia' consumati: c'e' posto per UN ordine, cioe' per mezza coppia. Prima si
    // sarebbe inviata la prima gamba e il gate per-ordine avrebbe rifiutato la seconda.
    const m = mondo({ rateCap: 20, ordersInWindow: 19 });
    const r = await runBulkAllocation({ rows: coppia(M1, 0.49, 0.49, 100) }, m.deps);
    ok('nessuna gamba viene inviata', m.fatti.piazzati.length === 0, String(m.fatti.piazzati.length));
    ok('  la sequenza si ferma sul rate limit', r.stoppedBy === 'rate-limit', String(r.stoppedBy));
    ok('  ed entrambe le righe sono saltate', r.skipped === 2);
    ok('  il motivo lo dice esplicitamente',
      /coppia spezzata a meta/.test(r.reason.replace(/à/g, 'a')), r.reason.slice(0, 130));
  }

  console.log('\n══ IL RATE LIMIT LASCIA PASSARE LE COPPIE CHE CI STANNO');
  {
    // Tetto 20, 16 consumati: ci stanno 2 coppie (4 ordini), la terza no.
    const m = mondo({ rateCap: 20, ordersInWindow: 16 });
    ...
    ok('  la terza e saltata intera', r.skipped === 2 && r.stoppedBy === 'rate-limit');

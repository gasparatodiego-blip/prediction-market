...
function misuraProfonditaAltrui({ rules = null, depth = null, ownOrders = [] } = {}) {
  ...
  // IL MID DI SCORING STA IN `books.yes`, NON IN CIMA. `resolveMarketRules` espone `mid` al primo
  // livello e i due mid di scoring dentro `books`; `rulesReadable` invece vuole `scoringMid` in cima.
  // Passare `rules` cosi' com'e' a `inBandPriceBounds` la fa rispondere «non leggibile» SEMPRE, e il
  // campionamento morirebbe in silenzio — un modulo che gira e non deposita mai niente. Si costruisce
  // l'oggetto esplicito, esattamente come fa `scalaPerIlMotore` in auto-reprice.
  const scoringMid = (rules.books && rules.books.yes && fin(rules.books.yes.scoringMid))
    ? rules.books.yes.scoringMid
    : (fin(rules.scoringMid) ? rules.scoringMid : (fin(rules.mid) ? rules.mid : null));
  if (!fin(scoringMid)) return nulla('mid di scoring non leggibile per questo mercato');

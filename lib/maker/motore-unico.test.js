...
console.log('\n══ LE COSTANTI: TRE NUMERI, NON DUE INSIEMI');
{
  ...
  ok('tetto per mercato = 20%', MARKET_CAP_PCT === 0.20);
...
{
  // 20% di $1000 = $200.
  const dentro = tettoMercato({ saldoUsd: 1000, esposizioneMercatoUsd: 150, aggiuntaUsd: 49 });
  ok('$150 + $49 = $199 sotto $200 ⇒ consentito', dentro.consentito === true, `cap $${dentro.capUsd}`);
  const fuori = tettoMercato({ saldoUsd: 1000, esposizioneMercatoUsd: 150, aggiuntaUsd: 51 });
  ok('$201 ⇒ rifiutato', fuori.consentito === false, fuori.motivo);
  ok('  e il cap è 20%, non 30 né 10', dentro.capUsd === 200, `$${dentro.capUsd}`);

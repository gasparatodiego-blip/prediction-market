...
const { MARKET_CAP_FIXED_USD, capPerMarketUsd } = require('../rewards/concentration');
...
console.log('\n══ LE COSTANTI: TRE NUMERI, NON DUE INSIEMI');
{
  ...
  // ⚠ NON PIU' FISSO dal 12 agosto 2026: deriva dal capitale (`capPerMarketUsd`). L'asserzione difende
  // ora la PROPRIETA' che conta per il motore — che il tetto sia UNO e venga dal modulo condiviso —
  // invece del valore, che si muove per progetto.
  ok('il tetto per mercato viene dal modulo condiviso, non da una costante locale',
    typeof MARKET_CAP_FIXED_USD === 'number' && MARKET_CAP_FIXED_USD > 0);
...
{
  // Il tetto e' FISSO, quindi NON dipende dal saldo: con $1000 in cassa vale il tetto, non il 20%.
  // I due casi sono costruiti UN CENTESIMO sotto e sopra il confine, derivandoli dal tetto invece di
  // scriverli a mano: e' il confine che il banco difende, non i numeri $129/$131 di quel giorno.
  const { MARKET_CAP_FIXED_USD: T } = require('../rewards/concentration');
  const dentro = tettoMercato({ saldoUsd: 1000, esposizioneMercatoUsd: T - 10, aggiuntaUsd: 9.99 });
  ok(`$${(T - 10).toFixed(2)} + $9,99 sotto $${T} ⇒ consentito`, dentro.consentito === true, `cap $${dentro.capUsd}`);
  const fuori = tettoMercato({ saldoUsd: 1000, esposizioneMercatoUsd: T - 10, aggiuntaUsd: 10.01 });
  ok(`$${(T + 0.01).toFixed(2)} ⇒ rifiutato`, fuori.consentito === false, fuori.motivo);
  ok(`  e il cap e $${T} FISSO, non una frazione del saldo`, dentro.capUsd === T, `$${dentro.capUsd}`);
  // ⚠ RIBALTATA il 12 agosto 2026. Diceva «il saldo raddoppiato NON alza il tetto», che era la
  // proprieta' del tetto FISSO. Col tetto derivato il saldo alza il tetto solo QUANDO servirebbero
  // piu' di MAX_MERCATI mercati: sotto quella soglia cresce N, sopra cresce la size. Si difende la
  // regola nuova, invertendo l'asserzione invece di toglierla.
  // ⚠ LA SOGLIA DI CARICO E' `TETTO_BASE x MAX_MERCATI` E SI MUOVE COL TETTO ($1.306 a $32,67,
  // $2.450 a $61,25): i due saldi sono DERIVATI, o il test diventa rosso a ogni leva sul tetto pur
  // difendendo una regola che non e' cambiata.
  const sogliaCarico = T * require('../rewards/concentration').MAX_MERCATI;
  ok('  il saldo alza il tetto solo oltre il tetto di carico',
    tettoMercato({ saldoUsd: sogliaCarico * 2, aggiuntaUsd: 1 }).capUsd > T
    && tettoMercato({ saldoUsd: sogliaCarico * 0.5, aggiuntaUsd: 1 }).capUsd === T,
    `$700→${tettoMercato({ saldoUsd: 700, aggiuntaUsd: 1 }).capUsd} · $2000→${tettoMercato({ saldoUsd: 2000, aggiuntaUsd: 1 }).capUsd}`);
  ...
  ok('con $50 in cassa il tetto NON scende piu\' a $50: la base derivata e\' piu\' bassa',
      tettoMercato({ saldoUsd: 50, aggiuntaUsd: 1 }).capUsd === capPerMarketUsd(50),
      `tetto a $50 di saldo: $${tettoMercato({ saldoUsd: 50, aggiuntaUsd: 1 }).capUsd}`);

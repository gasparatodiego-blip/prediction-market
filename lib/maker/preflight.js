...
async function defaultSigning({ prisma, env }) {
  try {
    ...
    // The value states WHICH proof carried it. An ERC-7739 wrapped signature has no address to recover,
    // so printing "recover —" there would read as a missing result rather than an inapplicable method.
    const value = !r.pass
      ? 'MISMATCH'
      : r.provenBy === 'ecrecover'
        ? `MATCH · recover ${short(r.recovered)}`
        : `MATCH · on-chain ${r.onChain && r.onChain.exchange ? r.onChain.exchange : 'validateOrder'}`;
    return { pass: r.pass === true, value, detail: r.detail };

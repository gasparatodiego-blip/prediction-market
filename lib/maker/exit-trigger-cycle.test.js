...
const regole = (mid = 0.50, raggioC = 4.5, bid = 0.495) => ({
...
{
  const resting = [{ orderId: 'e1', tokenId: 'ty', side: 'SELL', price: 0.51, sizeRemaining: 50, createdMs: NOW - 2 * H }];
  const d = decideClose({ position: pos, restingOrders: resting, rules: regole(0.42, 4.5, 0.415), book: 'yes', now: NOW });
...
{
  const resting = [{ orderId: 'e1', tokenId: 'ty', side: 'SELL', price: 0.51, sizeRemaining: 50, createdMs: NOW - 2 * H }];
  const largo = decideClose({ position: pos, restingOrders: resting, rules: regole(0.50, 4.5), book: 'yes', now: NOW });
  ok('con banda larga si aspetta', largo.action === 'already-covered');
  const stretto = decideClose({ position: pos, restingOrders: resting, rules: regole(0.50, 1.0), book: 'yes', now: NOW });
...
(async () => {
  console.log('\n── IL CICLO: prima cancella, poi vende');
  {
    ...
    const res = await runAutoCloseCycle({
      ...
      resolveRules: () => regole(0.42, 4.5, 0.415),

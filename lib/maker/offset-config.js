...
// ─── LA PROFONDITÀ DAVANTI, IN MULTIPLI DELLA PROPRIA SIZE ──────────────────────────────────────────
// `depthMultiple` (N) chiede all'ordine di arretrare oltre il minimo di «un tick dietro» finché davanti
// non ha almeno N × la propria size in share altrui.
//
// PERCHÉ IL DEFAULT È ZERO, cioè SPENTA. Accenderla sposta ordini veri più lontano dal mid, e il
// punteggio reward scala col QUADRATO della distanza: su una banda di ±2,25¢ passare da 0,1¢ a 0,5¢
// costa già il 35% del punteggio di quella gamba. È una scelta per mercato, da prendere guardando il
// book di quel mercato — non un default che si eredita in silenzio su tutta la produzione.
//
// L'ORDINE DI GRANDEZZA quando la si accende è 1,5–2×: sotto 1,5 la protezione è nominale (basta un
// ordine grosso quanto il nostro a spazzarla), sopra 2 su un book sottile si finisce contro il bordo
// banda senza mai raggiungere la soglia, pagando il punteggio senza comprare protezione. Il tetto di 10
// non è una preferenza: oltre, su qualunque book reale osservato, la soglia non è raggiungibile in
// banda e il parametro diventa un modo indiretto di scrivere «vai sempre al bordo».
const DEPTH_MULTIPLE_MAX = 10;
...
function resolveOffsetFor({ marketId, book, observedOffsetCents = null, tick = null } = {}, deps = {}) {
  ...
  // ── N, LA PROFONDITÀ RICHIESTA DAVANTI ────────────────────────────────────────────────────────
  // Per mercato (non per lato): descrive la struttura del libro di quel mercato, che è la stessa cosa
  // per il bid e per l'ask. ASSENTE ⇒ 0 ⇒ spenta ⇒ il comportamento di oggi, esattamente.
  const depthMultiple = rec && fin(rec.depthMultiple) && rec.depthMultiple > 0 ? rec.depthMultiple : 0;

  if (fin(configured) && configured > 0) {
    return { targetOffsetCents: configured, minMoveCents: minMove, depthMultiple, source: 'configured', record: rec };
  }
  const remembered = st.observed[key(marketId, book)];
  if (fin(remembered) && remembered > 0) {
    return { targetOffsetCents: remembered, minMoveCents: minMove, depthMultiple, source: 'remembered', record: rec };
  }
  if (fin(observedOffsetCents) && observedOffsetCents > 0) {
    return { targetOffsetCents: +observedOffsetCents.toFixed(4), minMoveCents: minMove, depthMultiple, source: 'observed', record: rec };
  }
  return { targetOffsetCents: null, minMoveCents: minMove, depthMultiple, source: 'unknown', record: rec };
}
 ...
 */
function validateOffset({ targetOffsetCents, minMoveCents, depthMultiple, bandRadiusCents, tick }) {
  const errors = [];
  // ZERO È VALIDO e significa «spenta»: è l'unico modo di rispegnerla dal pannello dopo averla accesa.
  // Un negativo no — non vuol dire niente, e trattarlo come zero nasconderebbe un errore di battitura.
  if (depthMultiple != null) {
    if (!fin(depthMultiple) || depthMultiple < 0) {
      errors.push({ field: 'depthMultiple', detail: 'la profondità richiesta deve essere un numero ≥ 0 (0 = protezione spenta)' });
    } else if (depthMultiple > DEPTH_MULTIPLE_MAX) {
      errors.push({ field: 'depthMultiple', detail: `la profondità richiesta ${depthMultiple}× supera il massimo di ${DEPTH_MULTIPLE_MAX}×: oltre, su un book reale, la soglia non è raggiungibile dentro la banda e il parametro equivarrebbe a «vai sempre al bordo»` });
    }
  }
...
function setMarketOffset({ marketId, targetOffsetCents = undefined, minMoveCents = undefined, depthMultiple = undefined, book = null, by = null, reason = null }, deps = {}) {
  const c = cfg(deps);
  const id = normId(marketId);
  ...
  markets[id] = {
    ...
    // Non dichiarato ⇒ si conserva quello che c'era. Salvare solo la distanza non deve poter
    // rispegnere una protezione accesa in un'altra schermata.
    depthMultiple: depthMultiple !== undefined ? depthMultiple : (fin(prev.depthMultiple) ? prev.depthMultiple : undefined),
  ...
  try {
    ...
    c.fs.appendFileSync(c.auditFile, JSON.stringify({ ts: at, event: 'offset-set', marketId: id, book, targetOffsetCents, minMoveCents, depthMultiple, by, reason }) + '\n');
  } catch { /* best-effort */ }

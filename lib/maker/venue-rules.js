 ...
 */
function splitVerdict(verdict, { allowOutOfBand = false } = {}) {
  ...
  if (!allowOutOfBand) {
    return { valid: verdict.valid === true, reasons: all, advisories: [], outOfBand };
  }
  const advisories = all.filter((r) => r.code === CODES.OUT_OF_BAND);
  const reasons = all.filter((r) => r.code !== CODES.OUT_OF_BAND);
  return { valid: reasons.length === 0, reasons, advisories, outOfBand };
}
...
function inBandPriceBounds(rules) {
  ...
  const radius = rules.maxSpreadCents / 200;                 // half the band, in price units
  const priceMin = fin(rules.priceMin) ? rules.priceMin : tick;
  const priceMax = fin(rules.priceMax) ? rules.priceMax : (1 - tick);

...
// Il timbro «ordine di uscita» e il divieto di riscrittura del prezzo: un modulo solo, importato anche
// da `manual-order` e `auto-reprice`, cosi' la regola non esiste in tre copie (reperto D1).
const ORDINI_USCITA = require('./ordini-di-uscita');
...
const { decidiLivello, MERGE_STRATEGY_ENABLED, MERGE_DISABLED_REASON,
  MERGE_WAIT_TIMEOUT_MIN } = require('./strategia-merge');
// ⚠ IL MARGINE DI RINNOVO VIENE DALLA STESSA COSTANTE del repricer, importata e non ricopiata: il GTD
// della corsia di chiusura si DERIVA da lei e dall'attesa del Livello 2 (vedi `GTD_CHIUSURA_SECONDS`).
const { REFRESH_MARGIN_SECONDS } = require('./auto-reprice-config');
// ⚠ LA SOGLIA DI FRESCHEZZA VIENE DALLA STESSA FUNZIONE del repricer e del piazzatore, importata e non
// ricopiata: tre numeri per lo stesso concetto divergono, uno solo no (reperto D1).
...
// L'ABBANDONO DI UNA POSIZIONE SCOPERTA (R6 letta come cancello, 23 agosto 2026). Modulo PURO: qui
// si giudica, non si agisce. La soglia e' DERIVATA da `PERDITA_MAX_FRAZIONE` x `MARKET_CAP_FIXED_USD`
// dentro quel modulo, e non e' ricopiata da nessuna parte.
const { valutaAbbandono, chiaveAbbandono, SOGLIA_ABBANDONO_USD } = require('./abbandono-posizione');
...
/**
 * L'USCITA INSEGUE IL BID — UNA FUNZIONE SOLA, USATA DA ENTRAMBI I RAMI.
 *
 * ⚠ ESTRATTA IL 17 AGOSTO 2026, e non per eleganza: l'inseguimento viveva SOLO nel ramo
 * `already-covered`, cioe' si applicava a un'uscita gia' a libro e MAI a quella che nasce. Il primo
 * piazzamento partiva all'obiettivo, sopra il bid, e restava li' finche' qualcuno non veniva a
 * prenderlo. Misurato il 16 agosto: **146 ordini SELL su 146 mai colpibili**.
 * Decisione dell'operatore, 17 agosto: «un'uscita che nasce a un prezzo che nessuno puo' colpire e'
 * inutile». Copiare l'espressione nei due rami sarebbe stato il reperto D1 su un numero che decide
 * quanto si perde: e' una funzione, e i due rami la chiamano.
 *
 * ⚠ I TRE VINCOLI RESTANO, e sono quelli che impediscono all'inseguimento di diventare una svendita:
 *   · dal GRADINO 1 in su. Al gradino 0 la scala non ha concesso niente e l'uscita sta all'obiettivo:
 *     abbassarla li' vorrebbe dire rinunciare al guadagno prima che la regola lo permetta;
 *   · MAI SOTTO IL PAVIMENTO — `Math.max` — che e' quanto la scala concede a quel gradino;
 *   · e' un `Math.min` sul prezzo del piano: puo' solo ABBASSARE, mai alzare.
 * Se uno dei tre ingressi non e' leggibile si torna al prezzo del piano, cioe' al comportamento di
 * prima: un dato mancante non muove un prezzo.
 *
 * ⚠ VALE SOLO PER LE USCITE E I COMPLETAMENTI. Questa funzione e' chiamata dai due rami di chiusura di
 * `decideClose` e da nessun altro posto: le aperture di liquidita' passano dal motore, che non la
 * conosce. Un test lo verifica per assenza.
 */
function inseguiIlBid({ prezzoPiano, pavimento, bid, gradino }) {
  const fin_ = (x) => typeof x === 'number' && Number.isFinite(x);
  if (!fin_(prezzoPiano)) return { prezzo: null, inseguito: false, motivo: 'nessun prezzo dal piano' };
  if (!fin_(bid) || !(bid > 0)) return { prezzo: prezzoPiano, inseguito: false, motivo: 'miglior bid non leggibile' };
  if (!fin_(pavimento)) return { prezzo: prezzoPiano, inseguito: false, motivo: 'pavimento della scala non leggibile' };
  const g = fin_(gradino) ? gradino : 0;
  if (g < 1) return { prezzo: prezzoPiano, inseguito: false, motivo: `gradino ${g}: la scala non ha ancora concesso di scendere` };
  const p = Math.max(pavimento, Math.min(prezzoPiano, bid));
  return { prezzo: +p.toFixed(10), inseguito: p < prezzoPiano - 1e-12,
    motivo: p < prezzoPiano - 1e-12
      ? `insegue il bid ${bid} restando sopra il pavimento ${pavimento}`
      : `il prezzo del piano ${prezzoPiano} e' gia' al bid o sotto` };
}

/**
 * LE DUE SCALE DEL LIBRO, RIPULITE DAI NOSTRI ORDINI — un punto solo, due chiamanti.
 *
 * `bidsMioLato`   i bid del token che possiedo: li colpisco vendendo. Tolti i NOSTRI BUY su quel token.
 * `asksAltroLato` gli ask del token contrario: li pagherei per completare la coppia. Tolti i NOSTRI SELL.
 *
 * ⚠ LA PULIZIA USA `othersLadder`, LA STESSA FUNZIONE DI «MAI PRIMO SUL LIBRO». Senza, il bid
 * conterrebbe i nostri stessi BUY di liquidita' e si venderebbe contro se' stessi — il fill fantasma
 * di cui parla il gate anti-auto-incrocio.
 * ⚠ ESISTE PERCHE' HA DUE CHIAMANTI: `presa-di-profitto` dentro `decideClose` e il giudizio di
 * ABBANDONO dentro `runAutoCloseCycle`. Erano dodici righe inline; ricopiarle sarebbe stato il
 * reperto D1 su un numero che decide se il bot smette di provare a uscire.
 * ⚠ FAIL-SOFT COME PRIMA: `depth` non cablato o `othersLadder` che solleva ⇒ si restituisce la scala
 * grezza (o `null`), esattamente il comportamento precedente riga per riga.
 */
function scalePulite({ depth, book, restingOrders, tokenId, rules }) {
  const altro = book === 'yes' ? 'no' : 'yes';
  const pulisci = (livelli, nostri) => {
  ...
  const nostriDi = (stessoToken, lato) => (Array.isArray(restingOrders) ? restingOrders : [])
    .filter((o) => o && String(o.side || '').toUpperCase() === lato
      && (stessoToken ? String(o.tokenId) === String(tokenId) : String(o.tokenId) !== String(tokenId)))
  ...
  return {
    bidsMioLato: pulisci(depth && depth[book] && Array.isArray(depth[book].bids) ? depth[book].bids : null,
      nostriDi(true, 'BUY')),
    asksAltroLato: pulisci(depth && depth[altro] && Array.isArray(depth[altro].asks) ? depth[altro].asks : null,
      nostriDi(false, 'SELL')),
  };
}

function decideClose({ position, restingOrders = [], rules, book, venue = null, now = Date.now(),
  maxWaitMs = undefined, urgenza = null, depth = null, sizeAltroLato = null } = {}) {
  ...
  // ══ ⚠ NON SI VENDE CONTRO UN BID CHE NON SI PUO' DATARE — 18 agosto 2026 ══════════════════════
  //
  // IL BUCO. `auto-close` non produce prezzi propri: il prezzo d'uscita e' il **bid del libro camminato
  // per la nostra size**. Ma questo file non conteneva nessun controllo di freschezza — ne'
  // `midSource`, ne' `midAgeSec`, ne' `requireLiveBook` — mentre il repricer rifiuta di muovere un
  // ordine su un mid vecchio e il piazzatore rifiuta di aprirne uno. Cioe' la funzione che decide di
  // VENDERE era l'unica delle tre a non guardare l'orologio, e la sera del 18 agosto il libro di un
  // mercato e' rimasto fermo per minuti mentre gli altri 124 erano freschi.
  //
  // LA REGOLA, decisione dell'operatore: non si vende contro un bid che non si puo' datare. Si resta al
  // gradino corrente e si dichiara l'anomalia con la causa. Le due cause restano distinte, come nel
  // piazzatore: «vecchio» (il feed ha parlato e il dato e' invecchiato) e «non databile» (il feed non
  // dice quando ha parlato — guasto nostro).
  //
  // ⚠⚠ L'UNICA ECCEZIONE E' LA REGOLA 6, IL RESIDUO SOTTO IL MINIMO DEL VENUE. Li' si chiude comunque,
  // perche' il capitale bloccato costa piu' della perdita immediata: un residuo sotto
  // `min_incentive_size` non e' ne' ripiazzabile ne' completabile, e la sua unica alternativa e'
  // aspettare la risoluzione (§5-bis p.187). Fra «vendere a un prezzo forse vecchio» e «restare fermi
  // per giorni» l'operatore ha scelto il primo, ed e' una scelta di rischio esplicita.
  //
  // ⚠ E STA QUI, DOPO le guardie sul mercato e PRIMA di tutto cio' che legge il libro. Non piu' in su:
  // un mercato chiuso si riscatta e non ha bisogno di un bid. Non piu' in giu': la presa di profitto,
  // `already-covered` e `close-at-market` camminano tutti il bid, e passarci davanti con un libro fermo
  // e' esattamente cio' che si vuole impedire.
  // ⚠ E NON TOCCA IL MERGE: `decideClose` non lo decide (lo fa `decidiLivello`), quindi il Livello 0
  //   resta valutabile a libro fermo — che e' la proprieta' che R8 ha stabilito e che qui non si perde.
  {
    const minVenue = Number(rules && rules.minSize);
    const residuoSottoMinimo = Number.isFinite(minVenue) && minVenue > 0 && size < minVenue;
    if (!residuoSottoMinimo) {
      const reg = regimeFeed(rules.feedVitality && typeof rules.feedVitality === 'object' ? rules.feedVitality : null);
      // ⚠⚠ `Number(null) === 0`, E CI SONO CASCATO SCRIVENDO QUESTA RIGA. Con `Number(rules.midAgeSec)`
      // un'eta' ASSENTE diventava zero, cioe' «freschissimo», e il ramo «non databile» non si
      // raggiungeva mai: il gate lasciava passare esattamente il caso che esiste per fermare. E' la
      // settima occorrenza di questa classe in questo repo (§5.3) e l'ha presa il test, non la
      // rilettura. Si guarda il TIPO, non si converte.
      const eta = (typeof rules.midAgeSec === 'number' && Number.isFinite(rules.midAgeSec))
        ? rules.midAgeSec : null;
      if (rules.midSource && rules.midSource !== 'live-book') {
        return out('skip', 'book-non-live',
          `il prezzo viene da «${rules.midSource}», non dal book live: non si vende contro un bid di seconda mano. Si resta al gradino corrente.`,
          { size, causa: 'fonte-non-live', midSource: rules.midSource, midAgeSec: rules.midAgeSec });
      }
      if (eta === null) {
        return out('skip', 'book-non-databile',
          'il book non dichiara la propria eta: il bid NON E\' DATABILE, e non si vende contro un prezzo di cui non si sa quando e stato letto. Si resta al gradino corrente.',
          { size, causa: 'prezzo-non-databile', midSource: rules.midSource, midAgeSec: null });
      }
      if (Number.isFinite(reg.limite) && eta > reg.limite) {
        return out('skip', 'book-vecchio',
          `il book e' fermo da ${Math.round(eta)}s, oltre i ${reg.limite}s ammessi in regime «${reg.regime}»: non si vende contro un bid vecchio. Si resta al gradino corrente.`,
          { size, causa: 'prezzo-vecchio', midAgeSec: eta, limiteSec: reg.limite, regime: reg.regime });
      }
    }
  }
  ...
  // ⚠ LE DUE SCALE SI COSTRUISCONO IN UN PUNTO SOLO — `scalePulite`, poco piu' su in questo file — e
  // non qui: le usa anche il giudizio di ABBANDONO nel ciclo, e due copie dello stesso ripulisci-e-
  // cammina divergerebbero in silenzio sul numero che decide se si smette di provare (reperto D1).
  const scale = scalePulite({ depth, book, restingOrders, tokenId: p.tokenId, rules });

  const tp = presaDiProfitto({
    carico: entry, size,
    bidsMioLato: scale.bidsMioLato,
    asksAltroLato: scale.asksAltroLato,
  });
  if (tp.scatta === true) {
  ...
  // ══ LA COPPIA E' ECONOMICAMENTE IMPOSSIBILE? — 22 agosto 2026 ════════════════════════════════
  //
  // ⚠ IL MERGE VIENE PRIMA, SEMPRE, E QUI E' SCRITTO ESPLICITAMENTE. Se il bot possiede gia' l'altra
  // gamba, fonde e non vende: `sizeAltroLato > 0` chiude questo ramo prima di qualunque prezzo. La
  // precedenza esisteva gia' per struttura (`provaCoppia` gira prima della vendita in tutti i rami
  // del ciclo), ma «esiste per struttura» non e' «e' scritta»: adesso e' una condizione, e un test la
  // prova. `sizeAltroLato` NON LETTA (null) ⇒ nessuna deroga: non si esce dalla banda su un dato che
  // non si e' letto, ed e' la stessa direzione di fail-closed di tutto questo file.
  //
  // LA PROVA E' QUELLA CHE LA PRESA DI PROFITTO HA GIA' FATTO, non una seconda: `coppiaCents` e'
  // `carico + ask camminato dell'altro lato` sullo STESSO snapshot, e `tettoCoppiaCents` e' il tetto
  // di 101¢ importato da `chiusura-rapida`. Ricalcolarli qui sarebbe il reperto D1 su un numero che
  // decide se una strada e' percorribile. `misurabile === false` ⇒ non si e' letto l'altro lato ⇒
  // nessuna deroga.
  const coppiaImpossibile = tp.misurabile === true
    && Number.isFinite(tp.coppiaCents) && Number.isFinite(tp.tettoCoppiaCents)
    && tp.coppiaCents > tp.tettoCoppiaCents + 1e-9
    && Number.isFinite(sizeAltroLato) && sizeAltroLato <= 0;

  // IL MIGLIOR BID DEL NOSTRO LATO, in un punto solo. Erano tre copie della stessa espressione in
  // questa funzione (il ramo dell'uscita gia' a riposo, quello dell'uscita nuova, il referto finale):
  // servono tutte e tre lo stesso numero, e adesso lo leggono dalla stessa riga.
  const bidDelLato = (rules.books && rules.books[book] && Number.isFinite(Number(rules.books[book].bestBid)))
  ...
  if (coveredSize + 1e-9 >= size) {
    ...
    const verdetto = decideExit({
      ...
      maxWaitMs,
      // v. `exit-plan.decideExit`: un'uscita fuori banda per decisione non si chiude a mercato al bid,
      // che starebbe SOTTO il pavimento della scala. Il tetto di attesa resta.
      fuoriBandaVoluta: coppiaImpossibile,
    });
    ...
    // trasformarsi in un modo di pretendere di più da una posizione che il mercato ha già superato.
    {
      ...
      const pianoOra = planExit({
        ...
        // Stessa deroga del ramo che nasce: se la coppia e' impossibile l'uscita gia' a libro puo'
        // essere riprezzata FUORI banda invece di restare senza prezzo.
        uscitaFuoriBanda: coppiaImpossibile, miglioreBid: bidDelLato,
      });
      ...
      const ins = pianoOra.ok
        ? inseguiIlBid({ prezzoPiano: pianoOra.price, pavimento, bid: askOra, gradino })
        : { prezzo: null };
      const bersaglio = ins.prezzo;
      if (bersaglio !== null) {
      ...
      if (pianoOra.ok && piuBasso !== null && Number.isFinite(tick) && tick > 0
        ...
        return out('close', 'uscita-da-abbassare',
          ...
          { price: pianoOra.price, size: coveredSize, entryPrice: entry,
            ...
            fuoriBandaVoluta: pianoOra.fuoriBanda === true, coppiaImpossibile,
  ...
  const plan = planExit({
    ...
    ...(urgenza && Number.isFinite(urgenza.profitPct) ? { profitPct: urgenza.profitPct } : {}),
    ...(urgenza && Number.isFinite(urgenza.concessioneTick) ? { concessioneTick: urgenza.concessioneTick } : {}),
    // ⚠ LA DEROGA FUORI BANDA — v. `exit-plan.planExit`. Arriva PROVATA (coppia oltre il tetto di
    // 101¢ e gamba sorella non posseduta), non dichiarata, e senza la prova non cambia niente.
    uscitaFuoriBanda: coppiaImpossibile, miglioreBid: bidDelLato,
  });
  if (!plan.ok) {
  ...
  // ══ E L'USCITA CHE NASCE INSEGUE IL BID COME QUELLA GIA' A LIBRO — 17 agosto 2026 ═════════════════
  // Fino a oggi questo ramo — il PRIMO piazzamento dell'uscita — proponeva il prezzo del piano e
  // basta: l'inseguimento viveva solo nel ramo `already-covered`. Un'uscita nasceva quindi
  // all'obiettivo, sopra il bid, e diventava colpibile solo se sopravviveva a un ciclo E il ramo
  // dell'uscita gia' coperta la riprendeva. Il 16 agosto non e' successo mai: 146 SELL, zero eseguiti.
  // ⚠ STESSA FUNZIONE, non una copia: `inseguiIlBid` porta con se' i tre vincoli — gradino ≥ 1, mai
  // sotto il pavimento, e solo verso il basso. Al gradino 0 restituisce il prezzo del piano
  // invariato, quindi un'uscita appena nata su un fill fresco non viene abbassata di un centesimo.
  {
    const bidQui = rules.books && rules.books[book] && Number.isFinite(Number(rules.books[book].bestBid))
      ? Number(rules.books[book].bestBid)
      : (venue && Number.isFinite(Number(venue.bestBid)) ? Number(venue.bestBid) : null);
    const pavQui = Number.isFinite(plan.pavimento) ? plan.pavimento : null;
    const gQui = urgenza && Number.isFinite(urgenza.livello) ? urgenza.livello : 0;
    const ins = inseguiIlBid({ prezzoPiano: target.price, pavimento: pavQui, bid: bidQui, gradino: gQui });
    if (ins.inseguito === true && Number.isFinite(ins.prezzo)) {
      target.price = ins.prezzo;
      // ⚠ LA BANDIERA SEGUE IL PREZZO, come nell'altro ramo: senza ricalcolarla una vendita SOTTO il
      // carico sarebbe indistinguibile nell'audit da una in guadagno.
      target.profitCents = +((target.price - entry) * 100).toFixed(3);
      target.peggiorativa = target.price < entry - 1e-9;
      target.inseguitoIlBid = true;
      target.bidInseguito = bidQui;
      target.reason = `${target.reason} · ${ins.motivo}`;
    }
  }

  // ── THE SHARED GUARD, on the exact order we are about to propose. Same function the server re-runs. ──
  // ⚠⚠ QUESTA E' UN'USCITA, E DAL 24 AGOSTO 2026 LO DICHIARA. Con `uscita:true` la size si misura sul
  // MINIMO D'ORDINE del venue (`minimum_order_size`) invece che sul pavimento premiante
  // (`min_incentive_size`): sono due numeri diversi per due domande diverse, e questo guard faceva
  // quella sbagliata. Minimo d'ordine non pubblicato ⇒ `MIN_ORDER_SIZE_UNREADABLE`, che nomina il
  // conditionId e rifiuta — non si indovina e non si ripiega sul pavimento (lib/maker/minimi-del-venue).
  // ⚠ E' IL GEMELLO DEL GATE DI `placeManualOrder`: il 17 agosto la deroga era stata messa la' e QUI
  // no, quindi non veniva mai raggiunta. Correggerne uno solo e' gia' costato una volta.
  const vq = validateQuote({ tick: rules.tick, scoringMid, maxSpreadCents: rules.maxSpreadCents,
    minSize: rules.minSize, minOrderSize: rules.minOrderSize, marketId: rules.marketId },
    { side: 'SELL', price: target.price, size: toClose, uscita: true });
  ...
  if (!vq.valid) {
    ...
    if (!codes.every((c) => c === 'OUT_OF_BAND')) {
      // ══ `BELOW_MIN_SIZE` SU UN'USCITA NON E' UN RIFIUTO — corretto il 17 agosto 2026 ═══════════════
      //
      // ⚠ IL COMMENTO QUI SOTTO DICEVA IL CONTRARIO, e la sua ultima frase era il difetto: «il resto
      // resta aperto e va chiuso a mano». Non c'e' nessuna mano: quello e' capitale incagliato fino
      // alla risoluzione, ed e' §5.2 p.1 — $26,30 in cinque residui che nessun percorso puo' chiudere.
      //
      // `min_incentive_size` e' il minimo del programma PREMI: dice «non maturera' nulla», non «il
      // venue rifiutera'». Su una VENDITA che chiude una posizione il premio non e' lo scopo e il
      // capitale si LIBERA — la stessa aritmetica per cui il tetto per ordine e quello di esposizione
      // sono gia' esentati (§5-bis p.147, p.168). Quinta occorrenza della classe «regola nata per
      // limitare l'APERTURA applicata a un'azione che non apre».
      //
      // ⚠ IL BANCO L'HA PRESO, LA RILETTURA NO: la deroga era stata messa in `placeManualOrder`, e
      // nel ciclo intero non veniva MAI raggiunta perche' QUESTO guard rifiuta prima — due gate, e ne
      // avevo corretto uno solo. Un test unitario sul secondo gate resta verde per sempre.
      //
      // ⚠ SI DEROGA SOLO ALL'USCITA, e solo se `BELOW_MIN_SIZE` e' l'UNICO motivo rimasto: un prezzo
      // fuori griglia o regole illeggibili continuano a rifiutare. E l'ordine parte comunque dai gate
      // di `placeManualOrder`, che rifara' la stessa prova sullo snapshot delle posizioni.
      // Si deroga SOLO se `BELOW_MIN_SIZE` (con l'eventuale fuori-banda, che non e' un rifiuto) e'
      // l'unico motivo rimasto. Un prezzo fuori griglia o regole illeggibili rifiutano come prima.
      // ⚠ DAL 24/08 `BELOW_MIN_SIZE` NON PUO' PIU' ARRIVARE QUI COME MOTIVO BLOCCANTE su un'uscita —
      // `validateQuote` lo declassa ad avviso da solo, perche' il numero che decide e' un altro. La
      // riga resta perche' questo guard serve anche i verdetti costruiti altrove, e perche' toglierla
      // renderebbe illeggibile la storia. Il motivo bloccante nuovo e' `BELOW_MIN_ORDER_SIZE`, e non
      // si deroga: quello e' il venue che rifiuta, non il programma premi.
      // ⚠ IL MINIMO NON LETTO NON E' UN RIFIUTO, IL MINIMO LETTO SI'. Sono due esiti diversi e vanno
      // tenuti distinti: `MIN_ORDER_SIZE_UNREADABLE` dice «non so», e su un'uscita «non so» non deve
      // bloccare piu' di quanto bloccasse il pavimento premiante ieri (`placeManualOrder` rifara'
      // comunque la prova sullo snapshot posizioni, ed e' li' che si decide davvero);
      // `BELOW_MIN_ORDER_SIZE` dice «il venue rifiutera'», ed e' letto: quello ferma qui, e ferma
      // apposta — sono i 21.889 invii che nelle 24 ore misurate erano rifiuti garantiti.
      const soloSottoMinimo = (codes.includes('BELOW_MIN_SIZE') || codes.includes('MIN_ORDER_SIZE_UNREADABLE'))
        && codes.every((c) => c === 'BELOW_MIN_SIZE' || c === 'MIN_ORDER_SIZE_UNREADABLE' || c === 'OUT_OF_BAND');
      if (!soloSottoMinimo) {
        if (codes.includes('BELOW_MIN_ORDER_SIZE')) {
          return out('skip', 'remainder-below-min-order-size',
            `${vq.reasons.filter((r) => r.code === 'BELOW_MIN_ORDER_SIZE').map((r) => r.detail).join('; ')}`
            + (codes.length > 1 ? ` (e anche: ${codes.filter((c) => c !== 'BELOW_MIN_ORDER_SIZE').join(',')})` : ''),
        ...
        if (codes.includes('BELOW_MIN_SIZE')) {
          return out('skip', 'remainder-below-min-size',
            `restano ${toClose} share da chiudere, sotto la size minima ${rules.minSize} di questo mercato,`
            + ` e c'e' anche altro che non va (${codes.join(',')}): non viene piazzato.`,
            { price: target.price, size: toClose, profitCents: target.profitCents, inBand: stillInBand });
        }
      ...
      // ⚠ SI PROSEGUE, e il fatto viaggia nel verdetto: `sottoMinimoDerogato` finisce nell'audit del
      // chiamante, cosi' un'uscita sotto il minimo resta distinguibile da una normale. Una deroga che
      // non lascia traccia sporca il conteggio del giorno dopo.
      target.sottoMinimoDerogato = true;
      target.minSizeMercato = rules.minSize;
      // ⚠ SI DICHIARA QUALE DEI DUE MINIMI HA CEDUTO. «derogato il pavimento premiante» e «il minimo
      // d'ordine non era pubblicato» sono due fatti diversi, e il secondo e' una cosa da RIPARARE
      // (agent24 non ha ancora riscritto il board per questo mercato), non una condizione normale.
      if (codes.includes('MIN_ORDER_SIZE_UNREADABLE')) {
        target.minimoOrdineNonLetto = true;
        target.minimoOrdineMotivo = (vq.reasons.find((r) => r.code === 'MIN_ORDER_SIZE_UNREADABLE') || {}).detail || null;
      }
    }
  }
  ...
  return out('close', null,
    ...
    { price: target.price, size: toClose, profitCents: target.profitCents, inBand: stillInBand,
      ...
      ...(target.sottoMinimoDerogato === true
        ? { sottoMinimoDerogato: true, minSizeMercato: target.minSizeMercato } : {}),
      ...(target.minimoOrdineNonLetto === true
        ? { minimoOrdineNonLetto: true, minimoOrdineMotivo: target.minimoOrdineMotivo } : {}),
      ...(target.inseguitoIlBid === true
        ? { inseguitoIlBid: true, bidInseguito: target.bidInseguito, peggiorativa: target.peggiorativa } : {}),
      ...
      bidLato: bidDelLato,
      // ⚠ VIAGGIA FINO AL PIAZZAMENTO, e non e' cosmetico: un'uscita fuori banda per decisione NON
      // deve dichiarare `inCoda`, o `prezzoInCoda` la riporterebbe dentro banda (`manual-order`
      // riassegna `price = q.price`) — cioe' il prezzo scelto verrebbe annullato in silenzio.
      fuoriBandaVoluta: plan.fuoriBanda === true,
      coppiaImpossibile,
...
// ══ LA DEROGA DI BANDA SULLA SCALA D'USCITA — 20 agosto 2026 ═══════════════════════════════════════
//
// IL FATTO, misurato su 456.339 righe di giornale (15-20 agosto): 29 rifiuti bloccanti con causa
// OUT_OF_BAND, e **24 di quelli hanno `source: auto-close-on-fill`**, cioe' vengono da questa scala.
// Due attese vere, di 7,4 e 6,2 minuti, in cui l'uscita non e' mai partita.
//
// ⚠ OUT_OF_BAND E' UN GATE NOSTRO, NON DEL VENUE. Lo dice per esteso `adapter.js:764`: significa
// «quest'ordine non maturera' premio», non «il venue lo rifiutera'». E `fill-strategy.js:263` lo
// tratta gia' come non bloccante (`!codes.every(c => c === 'OUT_OF_BAND')`): la strategia VUOLE
// uscire, ed era il gate di piazzamento a rifiutarla lo stesso perche' nessuno gli passava la deroga.
// `allowOutOfBand:true` arrivava solo da `mm-tracking` (motore inerte) e dalla chiusura d'emergenza
// R10 (`agent41:2080,2252,2310`). Terza occorrenza in un giorno di «dep dichiarata e mai iniettata».
//
// ⚠ LA DEROGA NON E' INDISCRIMINATA: e' per GRADINO, e la regola e' dell'operatore.
//   · gradino 1 — taker immediato: SI'. Deve uscire subito, il premio non c'entra.
//   · gradino 2 — acquisto a riposo per 30 minuti: SOLO se nessun prezzo dentro la banda rispetta il
//     tetto della coppia. Quell'ordine sta a libro PER maturare premio: metterlo fuori banda quando
//     un prezzo in banda esiste vuol dire aspettare mezz'ora senza incassare niente.
//   · gradino 3 — vendita sulla scala d'urgenza: SI'. E' un'uscita, non una quotazione.
//
// ⚠ E SOLLEVA ESCLUSIVAMENTE IL GATE DI BANDA. Tetto della coppia, cap per ordine, cap di esposizione,
// tetto per mercato e regola 6 restano tutti davanti e identici: `splitVerdict` declassa il SOLO
// codice OUT_OF_BAND (`venue-rules.js:159`), e ogni altra verifica vive fuori da quella funzione.
/**
 * Il gradino 2 merita la deroga solo se la banda non offre nessun prezzo utilizzabile.
 * `fuoriBanda === true`  ⇒ il tetto della coppia e' piu' stretto della banda: nessun prezzo in banda
 *                          rispetta il tetto, quindi restare in banda significa non piazzare affatto.
 * `fuoriBanda === false` ⇒ un prezzo in banda esiste ed e' quello che si sta usando: NIENTE deroga.
 * `fuoriBanda` assente   ⇒ banda non leggibile: NIENTE deroga. Non si concede un'esenzione su un dato
 *                          che non si e' letto — fail-closed, come ovunque in questo file.
 */
function derogaBandaPerGradino(gradino, { fuoriBanda = null } = {}) {
  if (gradino === 1 || gradino === 3) return true;
  if (gradino === 2) return fuoriBanda === true;
  return false;
}

/**
 * La riga di giornale di una deroga che e' SERVITA DAVVERO.
 *
 * ⚠ SI SCRIVE SOLO SE L'ORDINE E' PASSATO **E** IL GATE HA EMESSO L'AVVISO. `bandAdvisory` e' la
 * stringa che `manual-order` produce quando declassa OUT_OF_BAND (`manual-order.js:1356`): se e'
 * assente, l'ordine era dentro la banda e la deroga non e' servita a niente. Contare i due casi
 * insieme direbbe «la deroga e' servita N volte» quando N e' il numero di ordini, non di deroghe.
 * ⚠ LA DISTANZA SI LEGGE DALL'AVVISO, non si ricalcola: e' il numero che il gate ha davvero usato.
 */
function annotaDeroga({ audit, t0, marketId, gradino, book, side, price, mid, minutiScoperto, r }) {
  const avviso = r && typeof r.bandAdvisory === 'string' ? r.bandAdvisory : null;
  if (!avviso) return false;
  const m = avviso.match(/([0-9.]+)¢ exceeds/);
  audit({ ts: t0, venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
    outcome: 'deroga-banda-usata', marketRef: `cid_${String(marketId).replace(/^0x/, '')}`,
    reason: avviso,
    requested: { book, side, price },
    observed: {
      gradino,
      distanzaMidC: m ? Number(m[1]) : null,
      mid: Number.isFinite(mid) ? mid : null,
      minutiScoperto: Number.isFinite(minutiScoperto) ? minutiScoperto : null,
      orderId: (r && r.orderId) || null,
    } });
  return true;
}

/**
 * IL GTD DELLA CORSIA DI CHIUSURA — 23 agosto 2026, e il numero e' DERIVATO, non scelto.
 *
 * ═══ IL FATTO: IL VENUE NON SA ESTENDERE, E OGGI L'ORDINE MUORE PRIMA DELLA REGOLA CHE LO ASPETTA ═
 * L'`expiration` di un ordine sta DENTRO la struct EIP-712 firmata, e nessuno dei due SDK installati
 * espone un `modify`/`amend`/`extend` (88 metodi in `@polymarket/clob-client-v2`, zero corrispondenze).
 * Cambiare la scadenza significa quindi una firma nuova, cioe' un ORDINE nuovo, cioe' **in fondo alla
 * coda**. Prolungare senza perdere la posizione in coda NON E' POSSIBILE su questo venue: e' un fatto
 * verificato sul pacchetto installato, non una supposizione.
 *
 * Da li' l'inversione, misurata sul giornale del 23 agosto: `MERGE_WAIT_TIMEOUT_MIN` concede **30
 * minuti** all'ordine di completamento del Livello 2, ma quell'ordine porta i **23 minuti** della
 * corsia di quotazione — quindi il venue lo ritira PRIMA che la regola smetta di aspettarlo. Sul
 * giornale si legge `merge-in-attesa ... 29.8 min` su un ordine che a 23 minuti era gia' morto e
 * rinato in fondo alla coda. Su `0x4d79d3068b` il SELL d'uscita e' stato ripiazzato **24 volte** in
 * 673 minuti; su `0xd947c421f6` **18 volte**.
 *
 * ═══ IL NUMERO ══════════════════════════════════════════════════════════════════════════════════
 *     GTD_CHIUSURA_SECONDS = MERGE_WAIT_TIMEOUT_MIN x 60 + REFRESH_MARGIN_SECONDS = 1800 + 180 = 1980
 * cioe' **33 minuti**: la piu' lunga attesa che una regola conceda a un ordine di chiusura, piu' il
 * margine con cui il rinnovo scatta. Entrambe le costanti sono IMPORTATE dai moduli dove gia' vivono;
 * chi cambia l'attesa del Livello 2 muove anche questo, e non puo' dimenticarsene.
 *
 * ═══ PERCHE' SOLO LA CORSIA DI CHIUSURA, E LA QUOTAZIONE RESTA A 23 MINUTI ══════════════════════
 * ⚠ IL PREMIO NON CONOSCE LA CODA. `quadraticUserShare` prende sei parametri — concorrenza, mid,
 * banda, minSize, capitale, distanza — e la posizione in coda non e' uno di quelli; `scoreOrder` e'
 * `((v−d)/v)²`. Un ordine di quotazione che va in fondo alla coda **matura esattamente lo stesso
 * premio**. Misurato il costo residuo del churn sulla quotazione: 43 rinnovi/h con un buco
 * cancel→place di 12,47 s medi, cioe' lo **0,53% del tempo-libro**, che al ritmo di premio di oggi
 * vale **~$0,03/giorno**. Allungare li' non compra niente, e costerebbe esposizione non presidiata:
 * per questo non si tocca (`RESTING_GTD_SECONDS` resta 1380).
 * ⚠ E LA CALIBRAZIONE DI `ripristino-gambe` RESTA VALIDA: il suo tetto di 30 minuti sta sopra la GTD
 * *della corsia di quotazione*, che non e' cambiata. Muovere il GTD globale l'avrebbe invertita in
 * silenzio.
 *
 * ⚠ COSA COSTA, DETTO PER INTERO: se l'host muore, un ordine di CHIUSURA resta a libro fino a 33
 * minuti invece di 23. E' l'unica direzione accettabile di questo scambio, perche' un ordine di
 * chiusura puo' solo RIDURRE l'esposizione: un SELL vende cio' che possediamo, un BUY di
 * completamento chiude una coppia che rende $1/share. Nessun ordine di APERTURA e' toccato.
 * ⚠ IL TETTO DELL'OROLOGIO DEL MERCATO RESTA DAVANTI: `resolveManualTtlSeconds` limita comunque la
 * vita alla vita residua del mercato e rifiuta con `tooClose`. Un ttl esplicito non lo scavalca.
 */
const GTD_CHIUSURA_SECONDS = MERGE_WAIT_TIMEOUT_MIN * 60 + REFRESH_MARGIN_SECONDS;

/**
 * TIMBRA UNO SPEC COME ORDINE DI CHIUSURA. Un punto solo, quattro chiamanti.
 *
 * ⚠ ESISTE PERCHE' `piazzaChiudendo` NON E' IL PUNTO UNICO, e crederlo era l'errore: in questo file ci
 * sono CINQUE chiamate a `deps.placeOrder`, e solo una passa da li'. Le altre quattro sono l'uscita
 * ordinaria, il riposizionamento scoperto, il rimasuglio sotto il minimo e — l'unica che NON e' una
 * chiusura — il riposizionamento DOPO la fusione, che riapre due gambe di liquidita'.
 * ⚠ QUELLA SI LASCIA STARE, ED E' DICHIARATO: `riposizionaDopoChiusura` non chiude niente, APRE. Un
 * ordine di apertura tiene il GTD della corsia di quotazione, che non e' cambiato.
 * ⚠ UN CHIAMANTE CHE NOMINA UNA VITA SE LA TIENE: qui si dichiara solo il difetto.
 */
// ⚠⚠ E DAL 24 AGOSTO 2026 TIMBRA ANCHE `uscita: true` E IL PREZZO DECISO — richiesta dell'operatore.
// Questo e' il punto in cui un ordine diventa «un'uscita», e da qui in poi il suo prezzo e' VINCOLANTE:
// niente coda, niente rientro in banda, nessuna riscrittura in nessun ramo (`lib/maker/ordini-di-uscita`).
// ⚠ E `inCoda` VIENE TOLTO QUI, non lasciato ai chiamanti. Il 24 agosto il lato posseduto lo dichiarava
// — con una motivazione scritta e ragionevole, «e' un ordine che ASPETTA e matura premi» — che e' vera
// finche' la scala sceglie un prezzo DENTRO la banda e falsa appena lo sceglie fuori: a carico +1 tick
// con la banda sotto il carico, `prezzo-in-coda` ha riportato 0.495 a 0.288. Lasciare la decisione al
// chiamante significa rifarla giusta in quattro posti e sbagliarla nel quinto; toglierla qui la rende
// una proprieta' della CLASSE «ordine di chiusura», che e' cio' che effettivamente e'.
// ⚠ NON TOCCA CHI APRE: `riposizionaDopoChiusura` non passa da questa funzione (lo dice il commento
// sopra, e un test lo verifica per assenza), quindi continua a dichiarare `inCoda` come sempre.
const chiudendo = (spec) => {
  const conTtl = Number.isFinite(Number(spec && spec.ttlSeconds))
    ? { ...spec } : { ...spec, ttlSeconds: GTD_CHIUSURA_SECONDS };
  const { [ORDINI_USCITA.CAMPO_USCITA]: _u, inCoda: _c, ...senzaCoda } = conTtl;
  return {
    ...senzaCoda,
    [ORDINI_USCITA.CAMPO_USCITA]: true,
    // Il prezzo deciso viaggia ACCANTO al prezzo, non al posto suo: serve a chi dovra' dimostrare che
    // quello partito e' lo stesso, e a `close-sell-floor` per confrontarsi con la SCALA invece che con
    // l'ordine vivo. Un chiamante che lo dichiara gia' se lo tiene.
    [ORDINI_USCITA.CAMPO_PREZZO_DECISO]: Number.isFinite(Number(conTtl[ORDINI_USCITA.CAMPO_PREZZO_DECISO]))
      ? Number(conTtl[ORDINI_USCITA.CAMPO_PREZZO_DECISO])
      : (Number.isFinite(Number(conTtl.price)) ? Number(conTtl.price) : null),
  };
};

async function piazzaChiudendo(spec, { deps, audit, t0, etichetta }) {
  const rif = `cid_${String(spec.marketId).replace(/^0x/, '')}`;
  // ⚠ UN CHIAMANTE CHE NOMINA UNA VITA SE LA TIENE: qui si dichiara solo il difetto della corsia, e
  // si dichiara in UN PUNTO SOLO — tutti e sei i piazzamenti di chiusura passano da questa funzione.
  spec = chiudendo(spec);
  let ultimo = null;
...
async function completaCoppia({
  ...
  // ⚠ DECISIONE DELL'OPERATORE, 17 agosto 2026: «su fill parziale, gamba opposta per la quantita'
  // riempita e residuo CANCELLATO SEMPRE, anche sotto il minimo». Quindi qui non c'e' piu' nessuna
  // condizione sul residuo. Due cose che la rilettura ha trovato e che vanno dette:
  //
  //   ① IL TERNARIO DI PRIMA ERA UNA TAUTOLOGIA. `fillOrdine` e' DERIVATO dalla presenza del residuo
  //      (`modalita-chiusura.js:299`: `out.some(x => x.quale === 'residuo-non-fillato') ? PARZIALE :
  //      TOTALE`), quindi quando la condizione era falsa il `filter` non aveva niente da togliere: i
  //      due rami erano lo stesso ramo. Il comportamento era gia' quello chiesto, ma la riga si leggeva
  //      come se fosse selettiva — e §5.2 p.39 descriveva ancora la versione precedente, che guardava
  //      `tipoFill` (la COPERTURA) invece di `fillOrdine` (l'ORDINE). Una condizione inerte che sembra
  //      una politica e' peggio di nessuna condizione.
  //
  //   ② IL RESIDUO NON SI CANCELLA PIU' SOLO AL PRIMO GIRO, e questo cambia il comportamento davvero.
  //      Il blocco era dentro `if (statoChiusura.nuova)`, cioe' girava solo al ciclo in cui la modalita'
  //      chiusura si APRE. Ma un fill parziale arriva quando arriva: un secondo riempimento a ciclo gia'
  //      aperto lasciava il residuo a libro fino al completamento della coppia. E' la stessa ragione per
  //      cui il PASSO 2-bis non era condizionato a `nuova` — solo che quel passo aspettava `manca <= 0`,
  //      e nel frattempo il residuo poteva riempirsi e INGROSSARE la gamba nuda.
  //
  // ⚠ COSA SI PERDE, dichiarato: che il residuo si riempia da solo e completi la coppia senza pagare lo
  // spread. Vale meno di cio' che si evita — una posizione direzionale che cresce mentre la scala
  // d'uscita cerca di ridurla — ed e' la decisione dell'operatore, non un'inferenza.
  const residuiTolti = [];
  const residuiDellaGambaRiempita = resid.daCancellare.filter((x) => x.quale === 'residuo-non-fillato');
  // ⚠ LA GUARDIA E' «IL REGISTRO E' CABLATO», non uno stato del registro, e ci sono volute due prove per
  // arrivarci:
  //   · `statoChiusura.nuova` (com'era) restringe al PRIMO giro ⇒ il residuo di un fill parziale arrivato
  //     dopo sopravviveva. E' il difetto;
  //   · `statoChiusura.attiva` (primo tentativo) sembra giusto e NON lo e': a coppia GIA' completa la
  //     modalita' chiusura non viene nemmeno aperta, quindi `attiva` e' falso — ed e' precisamente il caso
  //     per cui esisteva il PASSO 2-bis, che era gated su NIENTE. Provato dal test, non dal ragionamento.
  // Sulla presenza della dep, invece, il contratto resta quello di sempre: senza `deps.chiusura` non si
  // cancella niente («dep non cablata ⇒ comportamento di prima», asserito in `modalita-chiusura.test.js`).
  const registroChiusuraCablato = !!(deps.chiusura && typeof deps.chiusura === 'object');
  if (registroChiusuraCablato && residuiDellaGambaRiempita.length && typeof deps.cancelOrder === 'function') {
    for (const x of residuiDellaGambaRiempita) {
      let c = null;
      ...
      audit({ ts: t0, venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
        outcome: cok ? 'modalita-chiusura-residuo-non-fillato-cancellato' : 'modalita-chiusura-residuo-non-fillato-cancellazione-fallita',
        ...
        observed: { orderId: x.orderId, size: x.size, fillOrdine: resid.fillOrdine,
          alPrimoGiro: statoChiusura.nuova === true } });
    }
  }
  // Gli ALTRI residui d'ingresso (la sorella dimensionata sulla coppia intera) restano legati al primo
  // giro: descrivono lo stato in cui la modalita' chiusura si APRE, non un fatto che puo' ripresentarsi.
  const residuiIngresso = resid.daCancellare.filter((x) => x.quale !== 'residuo-non-fillato');
  ...
  // ── PASSO 2-bis, RIMOSSO IL 17 AGOSTO 2026 ────────────────────────────────────────────────────
  // Cancellava il residuo della gamba riempita quando `manca <= 0`, cioe' quando la coppia si
  // completava. Esisteva perche' il residuo sopravviveva all'ingresso: adesso il residuo viene
  // cancellato SUBITO e a ogni giro, quindi `manca <= 0` non puo' piu' trovarne uno — il ramo era
  // diventato irraggiungibile. Si toglie invece di lasciarlo: un ramo che non puo' scattare e' un
  // ramo che qualcuno un giorno legge come una difesa attiva (reperto D7).
  // Gli esiti `coppia-completa-residuo-cancellato` / `…-cancellazione-fallita` non esistono piu': chi
  // li cerca nei giornali passati li trovera' fino a oggi, e da oggi trovera'
  // `modalita-chiusura-residuo-non-fillato-cancellato` con `alPrimoGiro:false`.
  ...
  for (const t of tentativi) {
    ...
    try {
      cres = await piazzaChiudendo({
        ...
        // ── LA DEROGA DI BANDA, PER GRADINO — v. `derogaBandaPerGradino` in testa al file ─────────
        // Gradino 1 (`t.taker`): sempre. Gradino 2: solo quando `t.fuoriBanda === true`, cioe' quando
        // il tetto della coppia e' piu' stretto della banda e nessun prezzo in banda lo rispetta.
        // `t.fuoriBanda` e' gia' calcolato qui sopra e finiva SOLO nella nota testuale: adesso decide.
        allowOutOfBand: derogaBandaPerGradino(t.taker ? 1 : 2, { fuoriBanda: t.fuoriBanda }),
    ...
    // ⚠ LA RIGA DELLA DEROGA, e solo quando e' SERVITA. Si scrive dopo l'esito, non prima: prima non
    // si sa se l'ordine e' passato, e una deroga che non ha prodotto un ordine non e' una deroga usata.
    if (cok) {
      annotaDeroga({ audit, t0, marketId, gradino: t.taker ? 1 : 2, book: altroBook, side: 'BUY',
        price: t.prezzo, mid: Number(rules && rules.mid), minutiScoperto: null, r: cres });
    }
    if (cok) {
  ...
  if (CHIUSURA_RAPIDA_ENABLED) {
    ...
    if (rip.ok) {
      ...
      for (const [quale, g] of [['lato-posseduto', rip.latoPosseduto], ['controparte', rip.controparte]]) {
        ...
        let r;
        try {
          r = await deps.placeOrder(chiudendo({
...
async function runAutoCloseCycle(deps = {}) {
  ...
  // ══ L'ABBANDONO: LO STATO SI LEGGE UNA VOLTA IN CIMA, I GIUDIZI SI APPLICANO UNA VOLTA IN FONDO ══
  // ⚠ Si legge PRIMA di visitare le posizioni, cosi' tutte le posizioni di questo giro sono giudicate
  // contro lo STESSO stato: leggere dentro il ciclo darebbe risposte diverse alla stessa domanda a
  // seconda dell'ordine di visita.
  // ⚠ `deps.abbandoni` non cablato ⇒ insieme VUOTO e nessuna scrittura: il comportamento e' esattamente
  // quello di prima, riga per riga. E' la stessa disciplina di `urgenza` e di `deps.chiusura`.
  const giudiziAbbandono = [];
  let abbandonatePrec = new Set();
  try {
    if (deps.abbandoni && typeof deps.abbandoni.leggi === 'function') {
      const letto = deps.abbandoni.leggi();
      if (letto && letto.readable === true && Array.isArray(letto.chiavi)) abbandonatePrec = new Set(letto.chiavi.map(String));
    }
  } catch { abbandonatePrec = new Set(); }
  ...
  for (const marketId of marketIds) {
    ...
    for (let pos of mine) {
      ...
      // ⚠ `sizeAltroLato` VIENE DA `decidiLivello`, NON DA UNA SECONDA LETTURA: e' lo stesso numero
      // che decide il merge, quindi «possiedo la sorella» non puo' avere due risposte diverse nello
      // stesso ciclo. `liv` assente (l'osservazione non e' riuscita) ⇒ `null` ⇒ nessuna deroga fuori
      // banda, che e' il verso fail-closed.
      const sizeAltroLatoOra = (liv && liv.numeri && Number.isFinite(Number(liv.numeri.sizeAltroLato)))
        ? Number(liv.numeri.sizeAltroLato) : null;
      let d = decideClose({ position: { tokenId: tok, size: pos.size, avgPrice: pos.avgPrice }, restingOrders: ordiniVivi, rules, book, venue, urgenza, depth: dpMerge, sizeAltroLato: sizeAltroLatoOra });
      ...
      // ══ L'ABBANDONO — R6 LETTA COME CANCELLO, 23 agosto 2026, decisione dell'operatore ══════════
      // «Quando una posizione scoperta vale meno di una soglia E il costo stimato di uscire supera il
      // valore residuo, la posizione e' dichiarata ABBANDONATA: esce dal ciclo di uscita, libera lo
      // slot, e non si cancella nulla al venue.»
      //
      // ⚠ STA DOPO L'ANOMALIA DELLE QUATTRO ORE, E L'ORDINE E' UN REQUISITO, NON UN CASO. Una
      // posizione abbandonata resta scoperta, quindi deve continuare a gridarlo: se il `continue` di
      // questo blocco stesse sopra, l'abbandono renderebbe MUTO il presidio dei 240 minuti — cioe'
      // spegnerebbe una difesa mentre dichiara di non toccarne nessuna. Un test lo verifica.
      //
      // ⚠ IL GIUDIZIO SI RIFA' SUL LIBRO DI ADESSO, ma la DECISIONE usa la conferma maturata nei giri
      // precedenti (`deps.abbandoni.leggi()`): e' cosi' che «la prima osservazione arma soltanto»
      // diventa vera senza un secondo orologio. Il giudizio di questo giro alimenta il registro a fine
      // ciclo, e da li' il giro dopo.
      // ⚠ SI ESCE SUBITO DALL'ABBANDONO: basta che il giudizio di ADESSO dica `abbandonabile:false`
      // perche' il `continue` non scatti, senza aspettare che il registro si aggiorni. Entrare costa
      // due osservazioni, uscire una: il costo dei due errori non e' lo stesso.
      // ⚠ FAIL-CLOSED: `deps.abbandoni` non cablato ⇒ nessun abbandono, comportamento di prima.
      let giudizioAbb = null;
      try {
        const sc = scalePulite({ depth: dpMerge, book, restingOrders: ordiniVivi, tokenId: tok, rules });
        giudizioAbb = valutaAbbandono({
          carico: Number(pos.avgPrice), size: Number(pos.size),
          bidsMioLato: sc.bidsMioLato, asksAltroLato: sc.asksAltroLato,
          // ⚠ LO STESSO NUMERO CHE DECIDE IL MERGE, non una seconda lettura: «possiedo la sorella?»
          // non puo' avere due risposte diverse nello stesso ciclo. `liv` assente ⇒ `null` ⇒ non
          // giudicabile ⇒ non si abbandona.
          sizeAltroLato: sizeAltroLatoOra,
        });
      } catch { giudizioAbb = null; }
      const chiaveAbb = chiaveAbbandono(marketId, tok);
      giudiziAbbandono.push({ chiave: chiaveAbb, marketId, tokenId: tok, book,
        size: Number(pos.size), carico: Number(pos.avgPrice), giudizio: giudizioAbb });
      const eraAbbandonata = abbandonatePrec.has(chiaveAbb);
      const ancoraAbbandonabile = !giudizioAbb || giudizioAbb.giudicabile === false
        || giudizioAbb.abbandonabile === true;
      if (eraAbbandonata && ancoraAbbandonabile) {
        m.abbandonate = (m.abbandonate || 0) + 1;
        actions.push({ marketId, tokenId: tok, book, action: 'skip', gate: 'posizione-abbandonata',
          reason: giudizioAbb ? giudizioAbb.motivo : 'giudizio non calcolabile: la voce resta com\'era' });
        // ⚠ SI SCRIVE A OGNI GIRO, non solo al primo: «una posizione abbandonata resta visibile nel
        // giornale e nel conteggio del P&L» e' un requisito dell'operatore. La posizione resta al
        // venue, quindi resta dentro lo snapshot, dentro il totale del guardiano, dentro
        // `capitale-al-lavoro` e dentro il P&L: abbandonare e' smettere di AGIRE, non di CONTARE.
        audit({ ts: now(), venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
          outcome: 'posizione-abbandonata',
          marketRef: `cid_${String(marketId).replace(/^0x/, '')}`,
          reason: giudizioAbb ? giudizioAbb.motivo
            : 'posizione gia\' dichiarata abbandonata e giudizio non ricalcolabile in questo giro: resta abbandonata',
          observed: { book, size: Number(pos.size), prezzoCarico: Number(pos.avgPrice),
            valoreResiduo: giudizioAbb ? giudizioAbb.valoreResiduo : null,
            costoUscita: giudizioAbb ? giudizioAbb.costoUscita : null,
            costoVendita: giudizioAbb ? giudizioAbb.costoVendita : null,
            costoCoppia: giudizioAbb ? giudizioAbb.costoCoppia : null,
            soglia: SOGLIA_ABBANDONO_USD,
            bidCamminato: giudizioAbb ? giudizioAbb.bidCamminato : null,
            askAltroCamminato: giudizioAbb ? giudizioAbb.askAltroCamminato : null,
            urgenzaLivello: urgenza ? urgenza.livello : null,
            scopertoDaMin: urgenza ? urgenza.minuti : null,
            nozionaleAlCaricoUsd: +(Number(pos.size) * Number(pos.avgPrice) || 0).toFixed(2) } });
        // ⚠ L'OBBLIGO DI ESITO SI CHIUDE DICHIARANDO, non tacendo (§4.6): questo e' il quarto ramo che
        // non chiama `completaCoppia`, e lo dice. Non chiamarla E' la regola — completare la coppia
        // significherebbe COMPRARE la gamba sorella, cioe' esattamente la spesa che R6 rifiuta.
        // Per costruzione qui `sizeAltroLato === 0`: una coppia completa non e' mai abbandonabile.
        audit({ ts: now(), venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
          outcome: 'merge-saltato-posizione-abbandonata',
          marketRef: `cid_${String(marketId).replace(/^0x/, '')}`,
          reason: 'posizione abbandonata: completare la coppia vorrebbe dire comprare la gamba sorella, '
            + 'cioe\' spendere per uscire piu\' di quanto la posizione valga (R6). Non si compra e non si vende.',
          observed: { book, ramo: 'abbandono' } });
        chiudiObbligo(`${marketId}:${tok}`);
        continue;
      }
      ...
      let res;
      try {
        res = await deps.placeOrder(chiudendo({
          marketId, book, side: 'SELL', price: d.price, size: d.size,
          source: AUTO_CLOSE_SOURCE,
          ...
          // ⚠ `chiudePosizione: true` SEMPRE, non solo quando attraversa — corretto il 17 agosto 2026.
          // Questa e' la vendita della posizione detenuta: RIDUCE esposizione per costruzione, ed e'
          // esattamente il caso che §5-bis p.147 dichiara esente su TUTTI i percorsi che riducono.
          // Senza la dichiarazione i gate a valle la trattano come un'apertura: il tetto per ordine,
          // quello di esposizione e il minimo premiante la rifiutano tutti e tre, e un residuo sotto
          // `min_incentive_size` non ha nessuna via d'uscita (§5.2 p.1, $26,30 incagliati).
          // ⚠ NON ALLENTA NIENTE: `provaChiusura` rifa' l'aritmetica sullo snapshot del venue — SELL
          // entro il posseduto — e una lettura mancante lascia i tetti applicati.
          chiudePosizione: true,
          // ── GRADINO 3 · DEROGA DI BANDA SEMPRE ────────────────────────────────────────────────
          // Questa e' la VENDITA della posizione sulla scala d'urgenza (30/60/240 min): e' un'uscita,
          // non una quotazione, e non sta a libro per maturare premio. Rifiutarla perche' «non
          // maturerebbe premio» e' rifiutare l'uscita per la ragione sbagliata — ed e' cio' che e'
          // successo 24 volte su questa scala. Solleva il SOLO gate di banda: `chiudePosizione` e
          // `provaChiusura` continuano a governare tetti ed esposizione, invariati.
          allowOutOfBand: derogaBandaPerGradino(3),
          // ── E UN'USCITA FUORI BANDA PER DECISIONE NON DICHIARA `inCoda` — 22 agosto 2026 ───────
          // Dichiarare `inCoda` manda il prezzo a `prezzo-in-coda`, e `manual-order` RIASSEGNA
          // (`price = q.price`): su un'uscita deliberatamente fuori banda quel ricalcolo la
          // riporterebbe dentro banda, cioe' annullerebbe in silenzio il prezzo appena scelto e
          // rimetterebbe la gamba nella condizione da cui si sta uscendo. Senza `inCoda` il prezzo
          // parte come deciso; «mai primo sul libro» non c'entra e non e' toccata — quella regola
          // non rifiuta mai un SELL (`manual-order.js:1190`, `spec.side !== 'SELL'`).
          ...(attr.attraversa === true
            ? { attraversaApposta: true }
            : (d.fuoriBandaVoluta === true ? {} : { inCoda: true })),
      ...
      // ⚠ GRADINO 3: qui i minuti di scoperta ESISTONO (`urgenza.minuti`, da `livelloUrgenza`), quindi
      // si registrano. Al gradino 1/2 non sono in questo scope e si scrive `null` invece di dedurli.
      if (ok) {
        annotaDeroga({ audit, t0, marketId, gradino: 3, book, side: 'SELL', price: d.price,
          mid: Number(rules && rules.mid),
          minutiScoperto: Number(urgenza && urgenza.minuti), r: res });
      }
  ...
  // ══ IL REGISTRO DEGLI ABBANDONI SI SCRIVE UNA VOLTA SOLA, A POSIZIONI VISITATE ═══════════════════
  // ⚠ QUI E NON DENTRO IL CICLO: una scrittura per posizione vorrebbe dire che l'ordine di visita
  // decide quali conferme maturano, e due giri sullo stesso stato darebbero risposte diverse.
  // ⚠ NON PUO' FAR FALLIRE IL GIRO: un registro che non si scrive e' un abbandono in meno, cioe' il
  // bot che continua a provare — il verso prudente. Il contrario (un giro di chiusure perso perche'
  // un file non si scrive) sarebbe un difetto vero.
  if (deps.abbandoni && typeof deps.abbandoni.applica === 'function' && giudiziAbbandono.length) {
    try {
      const esito = deps.abbandoni.applica(giudiziAbbandono, now()) || {};
      for (const v of (esito.entrati || [])) {
        audit({ ts: now(), venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
          outcome: 'posizione-abbandonata-dichiarata', marketRef: `cid_${String(v.marketId).replace(/^0x/, '')}`,
          reason: `ABBANDONO DICHIARATO dopo ${v.osservazioni} osservazioni contigue: ${v.motivo}`,
          observed: { book: v.book, size: v.size, prezzoCarico: v.carico,
            valoreResiduo: v.valoreResiduo, costoUscita: v.costoUscita, soglia: v.soglia,
            causa: v.causa, osservazioni: v.osservazioni } });
      }
      for (const v of (esito.usciti || [])) {
        audit({ ts: now(), venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
          outcome: 'posizione-abbandonata-rientrata', marketRef: `cid_${String(v.marketId).replace(/^0x/, '')}`,
          reason: `l'abbandono e' RIENTRATO: ${v.motivoUscita}`,
          observed: { book: v.book, size: v.size, prezzoCarico: v.carico } });
      }
    } catch (e) {
      audit({ ts: now(), venue: 'polymarket', source: AUTO_CLOSE_SOURCE, op: 'auto-close',
        outcome: 'registro-abbandoni-non-scritto',
        reason: `il registro degli abbandoni non e' stato aggiornato (${e && e.message ? e.message : e}):`
          + ' nessuna posizione viene abbandonata in questo giro, si continua a provare' });
    }
  }
...
module.exports = { runAutoCloseCycle, decideClose, fondiCoppia, completaCoppia, riposizionaDopoChiusura,
  ...
  // ⚠ Esportate per essere PROVATE, non per essere chiamate da fuori: `derogaBandaPerGradino` e'
  // la regola di scoping del gradino e `annotaDeroga` la riga di giornale che la rende contabile.
  // Provarle attraverso `runAutoCloseCycle` vorrebbe dire costruire un ciclo intero per verificare
  // un `if`, ed e' il modo in cui una regola finisce provata di sponda (§5-bis p.181).
  derogaBandaPerGradino, annotaDeroga,
  // ⚠ ESPORTATO PER ESSERE PROVATO, non per essere chiamato da fuori: e' il punto unico in cui un
  // ordine diventa «un'uscita», e una marcatura che nessun test puo' vedere e' una marcatura di cui ci
  // si fida. `uscita-prezzo-vincolante.test.js` ⑥ lo interroga direttamente.
  chiudendo,
  CHIUSURA_RAPIDA_ENABLED, TETTO_COPPIA_CENTS, AUTO_CLOSE_SOURCE, EXIT_PROFIT_PCT, MAX_WAIT_HOURS,
  // Esportate per essere PROVATE: il GTD della corsia di chiusura e' un numero DERIVATO, e un test
  // pretende che resti tale invece di essere ricopiato; `scalePulite` e' il punto unico da cui la
  // presa di profitto e il giudizio di abbandono prendono le stesse due scale.
  GTD_CHIUSURA_SECONDS, scalePulite };

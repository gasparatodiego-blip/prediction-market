...
async function completaCoppia({
  ...
  // La classificazione parziale/completo e' esplicita e viene dai numeri gia' letti, non da un'euristica.
...
async function runAutoCloseCycle(deps = {}) {
    ...
    for (const pos of mine) {
      ...
      const registraCoppia = (esito, ramo) => {
        ...
        if (esito.esito === 'fuso') {
          ...
          riposizionamenti.push({ marketId, rules, causa: 'merge riuscito' });
          return true;
        }
        if (esito.esito === 'piazzato') {
          ...
          if (esito.rimasuglio === true || esito.riposizionamentoScoperto === true) {
            riposizionamenti.push({ marketId, rules,
  ...
  for (const r of riposizionamenti) {
    ...
    try {
      ...
      esitoRip.motivo = esitoRip.motivo || q.motivo;
    }

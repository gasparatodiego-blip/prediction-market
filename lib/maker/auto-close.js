...
function decideClose({ position, restingOrders = [], rules, book, venue = null, now = Date.now(),
  ...
  if (coveredSize + 1e-9 >= size) {
    ...
    // trasformarsi in un modo di pretendere di più da una posizione che il mercato ha già superato.
    {
      ...
      const askOra = venue && Number.isFinite(Number(venue.bestAsk)) ? Number(venue.bestAsk) : null;
...
async function completaCoppia({
  ...
  //
  // ⚠ SI CANCELLA SOLO SU UN PARZIALE. Su un fill TOTALE non c'e' residuo per definizione, e su
  // qualunque altro ramo la lista resta quella di prima: `residuiDaCancellare` guarda il libro, non
  // un flag, quindi una lista vuota resta vuota.
  const residuiIngresso = fill.tipo === FILL_PARZIALE
    ? resid.daCancellare

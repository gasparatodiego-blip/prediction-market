...
async function runAutoCloseCycle(deps = {}) {
  ...
  for (const marketId of marketIds) {
    ...
    for (const pos of mine) {
      ...
      //
      // Quindi la decisione si CALCOLA e si REGISTRA, e non agisce: l'audit mostra quale livello
      // sarebbe scattato e con che numeri, cosi' la scelta di accendere si fa sui dati veri invece che
      // su una previsione. Con l'interruttore acceso, qui si dirameranno i Livelli 1 e 2.
      let liv = null;
      try {
        const altro = book === 'yes' ? 'no' : 'yes';
        const dp = typeof deps.readDepth === 'function' ? deps.readDepth(marketId) : null;

...
//     • band radius v = maxSpread/2 (cents)               — existing SSOT convention (see note below)
...
// maxSpread/2. We DO NOT change the reward math here (explicit constraint); we compute against the SSOT
...
function measuredCompetingDepth(book, mid, maxSpreadC, minSize) {
  const v = maxSpreadC > 0 ? maxSpreadC / 2 : null;
  const within = (arr) => {
    let size = 0, levels = 0;
    for (const o of arr || []) {
...
function earningRange({ book, mid, maxSpreadC, minSize, rewardsDailyRate, capitalUsd = 1000, candidateOffsetsC }) {
  ...
  return {
    measured: {
      ...
      vLabel: 'v = maxSpread/2 (project SSOT; docs phrase v as "max spread from midpoint" — verify empirically before live)',

...
{
  // 12 lunghi + 6 corti disponibili. ⚠ LA COMPOSIZIONE DEL FIXTURE E' PARTE DELL'ESPERIMENTO: la
  // quota «basso» a 12 slot vale 4 (dal 23/08, era 1), quindi servono ALMENO 4 candidati «basso» o a
  // fermare i dodici sarebbe il secchio invece della fascia — che e' cio' che questo blocco NON sta
  // provando (la composizione si prova a parte, in ⑥).
  const board = [];
  for (let i = 0; i < 12; i += 1) board.push(M(`0xL${i}`, 200 + i, i < 3 ? 20 : 50));
  for (let i = 0; i < 6; i += 1) board.push(M(`0xC${i}`, 30 + i, i < 2 ? 20 : 50));
...
// posti lunghi. Con le fasce ne entrano esattamente cinque.
{
  ...
  // OTTO posti (4 basso + 8 alto, dal 23/08; erano 1 + 11). Cinque li prendono i corti ⇒ ne restano
  // TRE per i lunghi, e i quattro posti «basso» restano vuoti perche' nessun candidato ha
  // `minSize ≤ 20`. Le due partizioni sono ortogonali e mordono ENTRAMBE: e' il comportamento voluto,
  // e va asserito com'e' invece di aspettarsi che una delle due si faccia da parte.
  ok('  i posti lunghi vanno ai LUNGHI, e a fermarli e\' il SECCHIO non la fascia',
    dentro.filter((id) => id.startsWith('0xl')).length === 3
...
// Due soli corti ammissibili per cinque posti: i tre che avanzano NON vanno ai lunghi.
{
  ...
  const d = SEL.decidiSelezione({ board, stato: vuoto, posizioni: nessunaPosizione, ora: ORA,
    max: 12, slotCorti: 5, nettoPerMercato: netto, conOrdiniVivi: ordiniVuoti });
  const dentro = Object.keys(d.statoNuovo.selezionati);
  // Anche qui il secchio «basso» resta vuoto (nessun `minSize ≤ 20`), quindi gli otto posti «alto»
  // sono l'unica riserva: due li prendono i corti, SEI i lunghi. ⚠ Dal 23/08 a fermare i lunghi e' il
  // SECCHIO (8 posti alto) e non piu' la fascia (7 posti lunghi): la proprieta' che questo blocco
  // difende — «i posti corti avanzati NON vanno ai lunghi» — regge lo stesso, ed e' quella che conta.
  ok('③ due corti su cinque posti ⇒ otto mercati, NON dodici (i tre posti corti restano vuoti)',
    dentro.length === 8, `${dentro.length}`);
  ok('  i lunghi non prendono i posti corti avanzati (si fermano al secchio «alto»)',

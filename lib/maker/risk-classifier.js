...
/**
 * IL PRE-FILTRO DELL'ALLOCATORE — punto (c). Dato un elenco di candidati e un profilo, dice quali
 * l'allocatore è disposto a proporre e perché gli altri no.
 *
 * Non duplica classifyRisk: la chiama DUE VOLTE, con due soglie diverse e due scopi diversi, e la
 * distinzione è il cuore di questo modulo:
 *
 *   · AMMISSIONE — classifyRisk con la soglia del PROFILO. Decide se il candidato entra. Sotto il
 *     profilo Risk la soglia di scadenza è il pavimento del venue, quindi un mercato che chiude fra 8
 *     minuti non viola niente e passa.
 *   · ETICHETTE — classifyRisk con la soglia SAFE, sempre. Sono i flag che la card mostra, e «rischio»
 *     significa per definizione «non passerebbe il profilo Safe». Se le etichette usassero la soglia
 *     del profilo, una card della tab Risk uscirebbe SENZA avvisi — un mercato che chiude fra 8 minuti
 *     apparirebbe pulito proprio nella tab che esiste per dire che non lo è.
 *
 * Quello che NESSUN profilo tollera è `tradable:false`: un mercato sotto il pavimento del venue non è
 * una scelta di rischio, è un ordine che verrebbe rifiutato.
 */
function filtraPerProfilo(candidati = [], profile = {}, opts = {}) {
  const ammessi = [];
  const scartati = [];
  for (const c of Array.isArray(candidati) ? candidati : []) {
    if (!c) continue;
    // Ammissione: la soglia del profilo. Etichette: sempre quella Safe.
    const v = classifyRisk(c, { ...opts, safeFloorMinutes: profile.safeFloorMinutes });
    const etichette = classifyRisk(c, { ...opts, safeFloorMinutes: undefined });

    if (!v.tradable) {
      scartati.push({ ...c, rischio: etichette, motivo: FLAG_SOTTO_PAVIMENTO });
      continue;
    }
    // Sotto la soglia di scadenza DEL PROFILO. Con Risk la soglia è il pavimento del venue, quindi qui
    // non ci si arriva mai per la scadenza — ci si arriva solo col profilo Safe.
    const troppoVicino = v.flags.find((f) => f.startsWith('scade fra'));
    if (troppoVicino) {
      scartati.push({ ...c, rischio: etichette, motivo: troppoVicino });
      continue;
    }
    if (profile.allowOutOfBand !== true && v.flags.includes(FLAG_FUORI_BANDA)) {
      scartati.push({ ...c, rischio: etichette, motivo: FLAG_FUORI_BANDA });
      continue;
    }
    if (profile.allowStaleData !== true && v.flags.includes(FLAG_STALE)) {
      scartati.push({ ...c, rischio: etichette, motivo: FLAG_STALE });
      continue;
    }
    ammessi.push({ ...c, rischio: etichette });
  }

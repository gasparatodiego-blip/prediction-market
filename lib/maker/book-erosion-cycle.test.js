...
(async () => {
  console.log('\n══ IL CASO NORMALE · si sta in banda e non si tocca niente');
  {
    ...
    // Il bersaglio ora lo detta il book: un tick dietro il miglior bid altrui (49.5¢) ⇒ 49.4¢.
    // NON piu' l'offset configurato di 1¢, che e' diventato solo il ripiego per quando siamo soli.
    ok('  il lato YES un tick dietro il miglior bid altrui', h.placed[0].priceCents === 49.4, `${h.placed[0].priceCents}c`);
  ...
  console.log('\n══ DOPO UN RIPOSIZIONAMENTO LA SERIE RIPARTE DA ZERO');
  {
    ...
    // LA DIFESA NON SI DISFA DA SOLA: `armed` sopravvive all'azzeramento della serie, quindi il
    // bersaglio resta il bordo premiante e l'ordine NON rientra dietro al miglior bid.
    ok('  ma l erosione resta ARMATA', e.erosion === true, 'senza questo l ordine rientrerebbe subito');
    ok('  e il bersaglio resta il bordo premiante', subito.markets[0].target.yes.mode === 'erosion-retreat',
      subito.markets[0].target.yes.mode);
    ok('  quindi l ordine resta arretrato', h.placed[h.placed.length - 1].priceCents === 47.8,
      `${h.placed[h.placed.length - 1].priceCents}c`);
  }

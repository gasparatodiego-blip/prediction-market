 ...
 */
async function fetchVenuePositions({ address, httpGet } = {}) {
  const get = httpGet || require('../httpGet').httpGet;
  if (!address) return { ok: false, positions: null, reason: 'indirizzo del conto non risolvibile' };
  try {
    const r = await get(`${DATA_API}/positions?user=${address}`, { timeoutMs: TRADES_TIMEOUT_MS, headers: { Accept: 'application/json' } });
    if (!r || r.status !== 200 || !Array.isArray(r.data)) {
      return { ok: false, positions: null, reason: `data-api /positions ha risposto ${r ? r.status : 'nessuna risposta'}` };
    }
    return { ok: true, positions: r.data, reason: `data-api /positions: ${r.data.length} posizioni aperte` };
  } catch (e) {
    return { ok: false, positions: null, reason: `lettura /positions fallita (${e.message})` };
  }
}
...
function cancelledOrderIds() {
  ...
  try {
    ...
    for (const line of raw.split('\n')) {
      if (!line || line.indexOf('cancel') === -1) continue;
...
module.exports = { runOperatorReset, diagnoseExposure, managedMarketIds, fetchVenueTrades, fetchVenuePositions, reconcileManualLane };

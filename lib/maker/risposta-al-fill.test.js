...
(async () => {
  ...
  console.log('\n══ SCENARIO 5 · «MAI PRIMO SUL LIBRO» RESTA ATTIVO OVUNQUE ALTRO');
  {
    ...
    // QUARTA eccezione, o una qualunque omissione incondizionata, continua a far cadere il blocco.
    const omissioni = src.match(/(?:\.\.\.)?\((?:primoAssoluto|esente|regoleAttive) \? \{\} : \{ inCoda: true \}\)/g) || [];
    ok('le omissioni di `inCoda` sono ESATTAMENTE tre, tutte condizionate',
      omissioni.length === 3, `${omissioni.length} trovate`);
    ...
    ok('  e non esiste nessuna omissione incondizionata di `inCoda`',
      !/\{\s*\}\s*:\s*\{\s*inCoda:\s*true\s*\}/.test(src.replace(/(?:\.\.\.)?\((?:primoAssoluto|esente|regoleAttive) \? \{\} : \{ inCoda: true \}\)/g, '')));

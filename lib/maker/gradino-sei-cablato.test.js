...
console.log('\n══ 2 · IL GRADINO 6, ESEGUITO — nessun ReferenceError, e l\'interruttore viene chiamato');
{
  ...
  //
  // ── LA PRECONDIZIONE, DICHIARATA E NON EREDITATA (13 agosto 2026) ─────────────────────────────
  // Da oggi il gradino 6 si può DISARMARE per configurazione (`SBLOCCO_GRADINO6_ARMATO=0`, vedi
  // `gradino-sei-disarmato.test.js`). Questo file prova il ramo ARMATO, quindi lo dichiara invece di
  // ereditarlo dall'ambiente: eseguito in una shell dove la variabile è a '0' misurerebbe il ramo
  // sbagliato e diventerebbe rosso senza nessun difetto.
  delete process.env[require('./sblocco-progressivo').ENV_GRADINO6];
  const chiamate = [];

...
// ══ ⑥-bis · L'OCCUPANTE DI CODA LUNGA VIENE LIBERATO, o il vincolo non serve a niente ═════════════
//
// ⚠ LA PRIMA STESURA FILTRAVA SOLO I CANDIDATI, e sul bot vivo il deadlock e' rimasto intatto: lo slot
// era GIA' occupato dal mercato a 134 giorni, quindi nessun corto poteva entrare — non c'era posto.
// Il giornale mostrava «2 scartati per coda lunga» e zero righe di piano. Misurato, non dedotto.
{
  const board = [mkt('0xlungo', 134), mkt('0xcorto', 2)];
  const stato = S.normalizzaStato({
    selezionati: { '0xlungo': { entratoAt: ORA - 1000, question: 'lungo', scaglione: 'basso', inGestione: false } },
  });
  const base = { board, stato, posizioni: POSIZIONI, ora: ORA, max: 1, codaLungaGiorni: CODA };

  // ① senza la lista degli ordini non si libera nessuno: fail-closed
  const cieco = S.decidiSelezione({ ...base, conOrdiniVivi: { leggibile: false } });
  ok(cieco.tenuti.some((t) => t.id === '0xlungo'),
    '⑥-bis ⚑ ordini non leggibili ⇒ l occupante NON si libera: non si toglie capitale su un ipotesi');

  // ② con ordini a riposo su quel mercato: intoccabile
  const conOrdini = S.decidiSelezione({ ...base, conOrdiniVivi: { leggibile: true, ids: ['0xlungo'] } });
  ok(conOrdini.tenuti.some((t) => t.id === '0xlungo'),
    '⑥-bis ⚑ con ordini a riposo l occupante resta: quel capitale sta maturando reward');

  // ③ libero da ordini: lo slot si libera e il corto entra
  const libero = S.decidiSelezione({ ...base, conOrdiniVivi: { leggibile: true, ids: [] } });
  ok(!libero.tenuti.some((t) => t.id === '0xlungo'), '⑥-bis ⚑ senza ordini l occupante ESCE');
  ok(libero.uscenti.some((u) => u.id === '0xlungo' && u.motivo === 'coda-lunga-senza-fascia-corta'),
    '⑥-bis e l uscita e dichiarata col suo motivo');
  ok(ids(libero).includes('0xcorto'),
    '⑥-bis ⚑ e il corto ENTRA nello slot liberato — che e il punto di tutta la regola');
}

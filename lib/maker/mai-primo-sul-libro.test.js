...
console.log('\n══ 3 · SOLI SUL LATO → COMPORTAMENTO INVARIATO');
{
  ...
  // ⚠ Dal 15 agosto 2026 il bordo ha un MARGINE (`distanza-obiettivo.bordiConMargine`): la proprieta'
  // difesa qui non e' «coincide col bordo» ma «e' dentro banda e il lato resta quotabile».
  ok('soli sul lato → si quota al bordo esterno ammesso, non si rinuncia',
    alBordo.ok === true && alBordo.mode === 'fallback-alone-bordo-esterno'
    && alBordo.price >= alBordo.bandLo - 1e-12 && alBordo.price <= alBordo.bandHi + 1e-12, c(alBordo.price));

...
console.log('\n══ 3 · SOLI SUL LATO → COMPORTAMENTO INVARIATO');
{
  ...
  ok('  col modo «soli»', dentro.mode === 'fallback-alone');

  // E se l'offset configurato uscisse dalla banda, il bordo resta ammesso: qui «in cima» non ha
  // nessuno davanti a cui stare.
  const alBordo = planBehindBest({ bestOther: null, tick: 0.01, scoringMid: 0.50, bandRadiusCents: 2.25, fallbackOffsetCents: 10 });
  ok('offset fuori banda → si aggancia al bordo, come prima', alBordo.ok === true && alBordo.mode === 'band-clamped', c(alBordo.price));
...
{
  const rules = { readable: true, tick: 0.01, maxSpreadCents: 4.5, books: { yes: { scoringMid: 0.785 }, no: { scoringMid: 0.215 } } };
...
console.log('\n══ 6 · ORDINE GIÀ A RIPOSO CHE DIVENTA IL PRIMO');
{
  ...
  const mk = (mid) => ({ readable: true, marketId: `0x${'a'.repeat(64)}`, tick: 0.01, maxSpreadCents: 4.5, minSize: 20,
  ...
  ok('  il ciclo passa TUTTI i nostri ordini di quel lato',
    /ownOrders: owned\.filter\(\(x\) => x\.book === order\.book\),/.test(ar));
...
console.log('\n══ 8 · IL PIANO ESCLUDE A MONTE E DICHIARA IL CAPITALE NON IMPIEGATO');
{
  ...
  ok('un mercato scartato NON entra in coda', /if \(gg\.scarto\) \{[\s\S]{0,200}setCodaErr/.test(panel));
  ok('  col motivo E il dettaglio, non solo il codice', /gg\.scarto\.motivo\} — \$\{gg\.scarto\.dettaglio\}/.test(panel));

...
const { leggiRewardDaMercati, estraiTotale, METODO_UNICO } = require('./reward-reale');
...
console.log('\n══ 4 · IL RETRY QUANDO IL VENUE NON RISPONDE (401/404/timeout)');
{
  for (const [nome, risposta] of [
    ...
    // Dall'8 agosto 2026 sera il percorso CLOB e' la fonte SECONDARIA e si chiama
    // `leggiRewardDaMercati`; `leggiRewardReale` e' l'orchestratore, provato piu' sotto. Le proprieta'
    // verificate qui — un errore di rete non diventa mai zero — sono le stesse e restano sue.
    const p = r.leggiRewardDaMercati({
      giorno: '2026-08-06',
      deps: {
...
console.log('\n══ 5 · IL PERCORSO DI LETTURA NON PUÒ PIAZZARE — PROVA SUL SORGENTE');
{
  ...
  // Aggiornata l'8 agosto 2026 sera: le funzioni di rete sono DUE — la GET firmata verso il CLOB e la
  // GET PUBBLICA verso il registro attività, che è la fonte che attribuisce davvero. La proprietà che
  // conta non è «una sola richiesta», è «solo GET»: la seconda non ha nemmeno le credenziali, quindi
  // allarga la superficie di lettura e RESTRINGE quella di firma.
  ok('le funzioni di rete sono due, ed entrambe sono GET',
    (codice.match(/https\.request/g) || []).length === 2
    && (codice.match(/method:\s*METODO_UNICO/g) || []).length === 2,
    'firmata verso il CLOB + pubblica verso il registro attività');
  ok('  e la seconda non porta nessuna credenziale',
    /function httpGetPubblico[\s\S]{0,400}Accept: 'application\/json'/.test(codice)
    && !/httpGetPubblico[\s\S]{0,400}POLY_SIGNATURE/.test(codice));
...
console.log('\n══ 6 · UN 200 NON È UN CONSUNTIVO: LO È UN 200 ATTRIBUITO');
{
  ...
  attesi.push(R.leggiRewardDaMercati({
    giorno: '2026-08-06',
    deps: {
...
// ═══ 6-bis · LA FONTE CHE ATTRIBUISCE DAVVERO ══════════════════════════════════════════════════════
// Il percorso CLOB rifiutava tutto, e faceva bene: `maker_address` è l'indirizzo zero su tutte le
// 5.065 righe. La domanda vera — «dove sta scritto quanto ho incassato?» — ha risposta nel registro
// attività PUBBLICO del venue, keyed sul FUNDER (che su un deposit-wallet ERC-1271 è il maker vero,
// mentre le credenziali L2 sono dell'EOA: è esattamente per questo che l'altra strada non attribuiva).
// Misurato l'8 agosto 2026: un pagamento, $1,3042, alle 00:00:03 del 7 agosto — cioè la giornata del 6.
console.log('\n══ 6-bis · I PAGAMENTI VERI, E A QUALE GIORNATA APPARTENGONO');
{
  const R = require('./reward-reale');
  const FUNDER = '0x4c81f19a436e8174f1f3b07d7c0169150fbdbdee';
  const pagamento = (tsIso, usd, wallet = FUNDER) => ({
    proxyWallet: wallet, timestamp: Math.floor(Date.parse(tsIso) / 1000), type: 'REWARD',
    size: usd, usdcSize: usd, transactionHash: '0xtx',
  });
  const attesi = [];

  // La regola di competenza, pura e provabile senza aspettare una notte.
  ok('un pagamento alle 00:00:03 appartiene al giorno PRIMA — è il caso osservato',
    R.giornoDiCompetenza(Date.parse('2026-08-07T00:00:03Z')) === '2026-08-06');
  ok('  e uno a metà giornata appartiene alla sua',
    R.giornoDiCompetenza(Date.parse('2026-08-07T12:00:00Z')) === '2026-08-07');
  ok('  il margine è dichiarato e configurabile',
    R.giornoDiCompetenza(Date.parse('2026-08-07T05:00:00Z'), 6) === '2026-08-06'
    && R.giornoDiCompetenza(Date.parse('2026-08-07T05:00:00Z'), 1) === '2026-08-07');
  ok('  un istante illeggibile non produce un giorno inventato', R.giornoDiCompetenza(NaN) === null);

  const deps = (righe, extra = {}) => ({
    funder: FUNDER,
    getPubblico: async () => ({ ok: true, status: 200, data: righe }),
    ...extra,
  });

  attesi.push(R.leggiPagamentiReward({ deps: deps([pagamento('2026-08-07T00:00:03Z', 1.3042)]) }).then((P) => {
    ok('il registro si legge e raggruppa per giornata di COMPETENZA', P.ok && P.pagamenti === 1);
    ok('  il pagamento del 7 alle 00:00 finisce sul 6', P.perGiorno.has('2026-08-06') && !P.perGiorno.has('2026-08-07'));
    ok('  con l importo e la transazione, non solo il totale',
      P.perGiorno.get('2026-08-06').usd === 1.3042 && P.perGiorno.get('2026-08-06').pagamenti[0].tx === '0xtx');
  }));

  attesi.push(R.leggiPagamentiReward({ deps: deps([pagamento('2026-08-07T00:00:03Z', 9, '0xqualcunaltro')]) }).then((P) => {
    ok('un pagamento di un ALTRO wallet non entra mai, anche se il venue lo restituisse',
      P.ok && P.pagamenti === 0 && P.perGiorno.size === 0);
  }));

  attesi.push(R.leggiPagamentiReward({ deps: { funder: 'non-un-indirizzo', getPubblico: async () => ({ ok: true, data: [] }) } }).then((P) => {
    ok('funder non configurato ⇒ si rifiuta, non si legge a caso', P.ok === false && /funder non configurato/.test(P.motivo));
  }));

  // ── L'ORCHESTRATORE: quando uno zero è uno zero, e quando è un «non lo so» ──────────────────────
  const ORA = Date.parse('2026-08-09T12:00:00Z');
  const conUnPagamento = deps([pagamento('2026-08-07T00:00:03Z', 1.3042)], { now: () => ORA });

  attesi.push(R.leggiRewardReale({ giorno: '2026-08-06', deps: conUnPagamento }).then((r) => {
    ok('la giornata pagata esce col suo totale', r.disponibile === true && r.totaleUsd === 1.3042);
    ok('  dalla fonte che attribuisce', r.fonte === 'attivita-venue' && r.attribuito === true);
    ok('  la scomposizione per mercato è null CON IL MOTIVO, non inventata dividendo il totale',
      r.perMercato === null && /non porta il mercato/.test(r.notaMercati));
  }));

  attesi.push(R.leggiRewardReale({ giorno: '2026-08-05', deps: conUnPagamento }).then((r) => {
    ok('una giornata senza pagamenti vale ZERO — ma solo perché il canale ha già attribuito altrove e la finestra è chiusa',
      r.disponibile === true && r.totaleUsd === 0);
    ok('  e lo zero è motivato, non silenzioso', /zero perché nessun pagamento/.test(r.notaMercati));
  }));

  attesi.push(R.leggiRewardReale({ giorno: '2026-08-09', deps: conUnPagamento }).then((r) => {
    ok('una giornata la cui finestra NON è chiusa non è zero: è «non lo so»',
      r.disponibile === false && r.totaleUsd === null && /finestra di pagamento/.test(r.motivo));
  }));

  attesi.push(R.leggiRewardReale({ giorno: '2026-08-06', deps: deps([], { now: () => ORA }) }).then((r) => {
    ok('registro SENZA nessun nostro pagamento ⇒ mai zero: non si distingue «non ho maturato» da «non mi vede»',
      r.disponibile === false && r.totaleUsd === null && /NESSUN pagamento/.test(r.motivo));
  }));

  attesi.push(R.leggiRewardReale({ giorno: '2026-08-06', deps: { funder: FUNDER, getPubblico: async () => ({ ok: false, error: 'HTTP 503' }), now: () => ORA } }).then((r) => {
    ok('registro irraggiungibile ⇒ non disponibile, MAI zero', r.disponibile === false && r.totaleUsd === null);
  }));

  // La fonte secondaria resta SPENTA per difetto: ~51 richieste a notte per un catalogo non attribuito.
  let chiamataCLOB = 0;
  attesi.push(R.leggiRewardReale({ giorno: '2026-08-06', deps: { ...conUnPagamento, daMercati: async () => { chiamataCLOB += 1; return { disponibile: false, motivo: 'non attribuito' }; } } }).then(() => {
    ok('la scomposizione dal CLOB NON viene chiesta per difetto: sono ~51 richieste per un catalogo non attribuito', chiamataCLOB === 0);
  }));
  attesi.push(R.leggiRewardReale({ giorno: '2026-08-06', deps: { ...conUnPagamento, conScomposizione: true, daMercati: async () => ({ disponibile: true, perMercato: [{ marketId: '0xAA', usd: 1.3 }] }) } }).then((r) => {
    ok('  ma si accende, e allora la scomposizione arriva', Array.isArray(r.perMercato) && r.perMercato.length === 1);
  }));
}

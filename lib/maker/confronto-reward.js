...
/**
 * QUALE STIMA VA CONFRONTATA COL BONIFICO — e c'è una sola risposta giusta.
 *
 * `stimaUsd` è la fotografia delle 23:55: un TASSO ($/giorno) letto in un istante. `stimaIntegrataUsd`
 * è Σ(tasso × durata) sulla giornata, cioè una QUANTITÀ — la stessa grandezza del bonifico. Il
 * confronto usa la seconda quando c'è, e dichiara quale ha usato: uno scarto calcolato su due basi
 * diverse in due giornate diverse non è una serie storica, è due serie mescolate.
 *
 * La fotografia NON viene buttata: resta accanto come `stimaUsd`, perché è la serie con cui sono stati
 * misurati i +465% e toglierla renderebbe il prima e il dopo non confrontabili.
 */
function baseStima(riga) {
  if (fin(riga && riga.stimaIntegrataUsd)) return { usd: riga.stimaIntegrataUsd, base: 'integrata' };
  if (fin(riga && riga.stimaUsd)) return { usd: riga.stimaUsd, base: 'istantanea' };
  return { usd: null, base: null };
}
...
/**
 * SCRIVE LA STIMA INTEGRATA — Σ(tasso × durata) — e ricalcola lo scarto se il consuntivo c'è già.
 *
 * A differenza di `registraStima` questa **sovrascrive**, e deve: la fotografia delle 23:55 è un fatto
 * avvenuto una volta sola, l'integrale è un calcolo che può essere rifatto meglio (più campioni, o un
 * ricalcolo a ritroso sullo storico del board). Rifiutare la riscrittura vorrebbe dire congelare per
 * sempre il primo risultato, compreso quello di una giornata campionata a metà.
 *
 * La COPERTURA viaggia sempre col numero: un integrale al 40% di copertura è una sottostima nota, e
 * chi legge deve poterlo distinguere da una giornata magra davvero.
 */
function registraStimaIntegrata({ giorno, integrataUsd = null, coperturaFrazione = null, campioni = null, completo = null, fonte = null }, deps = {}) {
  const file = deps.confrontoFile || CONFRONTO_FILE;
  const giorni = leggiGrezzo(file);
  const esistente = giorni.find((g) => g.giorno === giorno);
  const riga = esistente || { giorno, stimaUsd: null, stimaPerMercato: null, stimaAt: null };

  riga.stimaIntegrataUsd = fin(integrataUsd) ? +Number(integrataUsd).toFixed(6) : null;
  riga.stimaCopertura = fin(coperturaFrazione) ? +Number(coperturaFrazione).toFixed(4) : null;
  riga.stimaCampioni = Number.isFinite(campioni) ? campioni : null;
  riga.stimaCompleta = completo === true;
  if (fonte) riga.stimaIntegrataFonte = fonte;
  riga.stimaIntegrataAt = new Date((deps.now || Date.now)()).toISOString();

  // Se il consuntivo è già lì, lo scarto va rifatto sulla base nuova: lasciarlo calcolato sulla
  // fotografia significherebbe pubblicare un numero che il file stesso dichiara superato.
  if (fin(riga.realeUsd)) {
    const b = baseStima(riga);
    riga.scartoBase = b.base;
    Object.assign(riga, scarto({ stimaUsd: b.usd, realeUsd: riga.realeUsd }));
  }

  if (!esistente) giorni.push(riga);
  const w = scriviAtomico(file, giorni);
  return { ok: w.ok, scritto: w.ok, motivo: w.reason, base: baseStima(riga).base };
}
...
function registraReale({ giorno, disponibile, realeUsd = null, motivo = null, tentativo = 1, perMercato = null, attribuito = null, righeLette = null, fonte = null, pagamenti = null }, deps = {}) {
  ...
  if (disponibile && fin(realeUsd)) {
    ...
    // Si confronta la QUANTITÀ integrata col bonifico, non il tasso fotografato. Vedi `baseStima`.
    const b = baseStima(riga);
    riga.scartoBase = b.base;
    Object.assign(riga, scarto({ stimaUsd: b.usd, realeUsd: riga.realeUsd }));
...
module.exports = {
  ...
  registraStimaIntegrata, baseStima,

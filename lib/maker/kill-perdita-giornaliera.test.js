...
// ⚠ LA FORMA DEI LIMITI SI PRENDE DALLA FUNZIONE VERA, NON SI INVENTA. La prima stesura di questo test
// iniettava `{readable:true, maxDailyLossUsd:100}` — una forma che `resolveLimits` non produce: lei
// risponde `{ok, limits:{...}}`. Il test passava e il codice era INERTE, perche' provava la decisione e
// non il cablaggio. Ora la fixture COPIA la forma vera e ne cambia solo il numero: se un domani
// `resolveLimits` cambiasse forma, questo test cade con lei.
const { resolveLimits: RESOLVE_VERO } = require('../safety/risk-limits');
const FORMA_VERA = RESOLVE_VERO({ userId: 'operator' });
const LIMITI_FINTI = (tetto) => ({ ...FORMA_VERA, limits: { ...(FORMA_VERA.limits || {}), maxDailyLossUsd: tetto } });
...
console.log('\n══ 2 · LA SOGLIA E IL NUMERO VENGONO DAL GATE DI PIAZZAMENTO, NON DA UNA COPIA');
{
  ...
  // ⚠ L'ASSERZIONE CHE MANCAVA, e la sua assenza e' costata un kill inerte: che il guardiano legga il
  // tetto DOVE `resolveLimits` lo mette davvero. Non una forma inventata dal test: quella vera.
  ok('  e il tetto sta in `limits.maxDailyLossUsd` nella risposta VERA di resolveLimits',
    FORMA_VERA && FORMA_VERA.limits && Number.isFinite(FORMA_VERA.limits.maxDailyLossUsd),
    `maxDailyLossUsd = ${FORMA_VERA && FORMA_VERA.limits && FORMA_VERA.limits.maxDailyLossUsd}`);
  ok('  e il guardiano legge `lim.limits`, non `lim` in piano', /lim\.limits && typeof lim\.limits/.test(src));
...
console.log('\n══ 3 · LO SCATTO ARRIVA ALL\'AZIONE: SI CANCELLA E SI VA SU FERMA');
{
  ...
  const base = {
    ...
    // ⚠ §5.2 p.54 (22 agosto 2026): senza una lettura precedente il capitale non e' misurabile, e il
    // caso ② — «il ramo del venue e' stato raggiunto» — non arriverebbe piu' al riferimento. Regime:
    // cassa e posizioni ferme ⇒ le due fonti concordano, e il kill R10 resta l'unica cosa provata qui.
    precedenteFonti: { at: 1_700_000_000_000 - 30_000, saldoUsd: 1000, posizioni: [] },
    // ⚠ R10 · DOVE SI DEPOSITA LA RICHIESTA DI CHIUSURA, E PERCHE' SI INIETTA. Senza questo, la prima
    // stesura di R10 ha scritto una richiesta FINTA nel `data/` di produzione — che agent41 avrebbe
    // eseguito al giro dopo. Questo test guida `spazzaEFerma` VERA, quindi ogni sua scrittura deve
    // finire nella directory temporanea. E' la classe del 7 agosto 2026 (residui sullo stato vero).
    chiusuraFile: path.join(tmp, 'chiusura-emergenza-richiesta.json'),
  };

  // ① SCATTA: perdita −$120 contro soglia $100.
  const r = await G.poll({ ...base,
    resolveLimits: () => LIMITI_FINTI(100),
  ...
  // ── R10 · LA RICHIESTA DI CHIUSURA E' DEPOSITATA, E NEL POSTO GIUSTO ────────────────────────────
  ok('  R10 · la richiesta di chiusura e\' depositata nel file INIETTATO', (() => {
    const q = JSON.parse(fs.readFileSync(path.join(tmp, 'chiusura-emergenza-richiesta.json'), 'utf8'));
    return q.eseguita === false && q.causa === 'perdita-giornaliera';
  })());
  // ⚠ E NON nel `data/` vero. Un test che guida una funzione che scrive deve provare anche DOVE non
  // ha scritto: e' l'asserzione che sarebbe servita il 7 agosto 2026.
  ok('  R10 · e NON nel data/ di produzione', (() => {
    const vero = path.join(__dirname, '..', '..', 'data', 'chiusura-emergenza-richiesta.json');
    if (!fs.existsSync(vero)) return true;
    // Se esiste, dev'essere di qualcun altro: non puo' portare l'istante finto di questo test.
    try { return JSON.parse(fs.readFileSync(vero, 'utf8')).at !== 1_700_000_000_000; } catch { return true; }
  })());
  ok('  R10 · il latch dichiara cosa e\' stato richiesto', (() => {
    const l = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
    return l.chiusuraPosizioni && l.chiusuraPosizioni.richiesta === true;
  })());
  ...
  const r3 = await G.poll({ ...base,
    resolveLimits: () => ({ ok: false, limits: null }),

 ...
 *
 *  ═══ NON È PIÙ UNA COSTANTE DI QUESTO FILE (9 agosto 2026) ═══════════════════════════════════════
 *  Qui c'era `MARKET_CAP_PCT = 0.20`, gemella di `CONCENTRATION_CAP_FRAC` nel pianificatore. Due
 *  costanti tenute uguali a mano sono due modi di divergere, e con il passaggio al tetto FISSO la
 *  divergenza sarebbe stata immediata e silenziosa: il pianificatore avrebbe proposto $130 e questa
 *  regola avrebbe verificato contro il 20% del saldo — $118,82 sul saldo del 9 agosto — rifiutando
 *  ogni riga del piano al momento di quotare. È esattamente il difetto che l'unificazione del 7 agosto
 *  aveva chiuso, e che un tetto fisso scritto in un posto solo riapriva se questo file restava indietro.
 *
 *  Adesso il numero arriva da `lib/rewards/concentration.js`, che è già il posto dove vive per il
 *  pianificatore. `concentration` non importa niente, quindi non c'è ciclo. */
const { capPerMarketUsd, MARKET_CAP_FIXED_USD, COSTO_COPPIA } = require('../rewards/concentration');
...
function trovaLivello({
  ...
  // ── IL PAVIMENTO NON SI APPLICA A UN RINNOVO — decisione dell'operatore, 16 agosto 2026 ─────────
  // Il pavimento limita l'APERTURA di esposizione su un libro sottile. Un rinnovo non apre niente:
  // sposta un ordine gia' a libro, alla stessa size o meno. Il termine di paragone giusto non e'
  // «nessuna esposizione» — quella non e' un'opzione, l'ordine c'e' gia' — ma «la gamba muore e
  // restiamo direzionali», che e' il rischio peggiore dei due.
  // MISURATO: 34 `anomalia-rinnovo-fermato` in 22 minuti su un mercato, 6 ordini morti per GTD, e
  // quel mercato rimasto a GAMBA SINGOLA — cioe' il rischio che il pavimento doveva evitare,
  // prodotto dal pavimento stesso. Chiude §5.2 p.21.
  // ⚠ NON E' UNA DICHIARAZIONE: `rinnovo` arriva gia' PROVATO da `esenzione-rinnovo.provaRinnovo`,
  // che rifa' l'aritmetica contro gli ordini VIVI del venue — stesso mercato/token/lato, size non
  // in aumento, nozionale non in aumento — e qualunque incertezza vale «apertura».
  // ⚠ SI GUARDA `=== true`, mai la truthiness, e si azzera SOLO questo pavimento: banda, «mai primo»,
  // tetti e fine scala restano identici perche' vivono altrove.
  const esenteRinnovo = !!(rinnovo && rinnovo.esente === true);
  // ⚠ L'ESENZIONE E' CONDIZIONATA AL PREZZO, e senza questo pezzo non sarebbe sicura per costruzione.
  // Nel ciclo di riprezzo la size e' nota (invariata) ma il prezzo NO: lo sceglie questa funzione. Un
  // livello piu' caro dello scaduto sarebbe piu' nozionale a riposo, cioe' un'APERTURA travestita da
  // rinnovo. Quindi l'esenzione vale, ma solo per i livelli che non superano il prezzo di riferimento.
  const prezzoMaxRinnovo = esenteRinnovo && fin(rinnovo.prezzoMassimo) ? rinnovo.prezzoMassimo : null;
  if (esenteRinnovo) pavimentoUsd = 0;
  ...
  let cum = 0;
  // Serve solo al ramo di rinnovo qui sotto: dice che il pavimento era soddisfatto e che a scartare il
  // livello e' stato il confronto col prezzo di riferimento, non la profondita'.
  let scartatoPerPrezzo = false;
  for (let i = 1; i < dentro.length; i++) {
    const l = dentro[i];
    cum += l.price * l.size;
    if (cum + 1e-9 < pavimentoUsd) continue;
    // Il livello e' ammesso dal pavimento; se ci siamo arrivati grazie all'esenzione di rinnovo, deve
    // anche non costare piu' dell'ordine che sostituisce, o il nozionale a riposo crescerebbe.
    if (prezzoMaxRinnovo !== null && l.price > prezzoMaxRinnovo + 1e-9) { scartatoPerPrezzo = true; continue; }
    ...
    return {
      ...
      motivo: `profondità altrui $${cum.toFixed(2)} ≥ pavimento $${pavimentoUsd.toFixed(2)} al livello ${i + 1}`
        ...
        + (esenteRinnovo ? ` · PAVIMENTO ESENTATO: ${rinnovo.motivo}` : ''),
    };
  }
  // ── UN RINNOVO NON HA BISOGNO DI UN LIVELLO NUOVO: HA BISOGNO DI TENERE IL SUO ────────────────────
  //
  // ⚠ IL COMMENTO CHE STAVA QUI SOPRA DESCRIVEVA UN COMPORTAMENTO INESISTENTE (reperto D7). Diceva:
  // «nel ciclo di riprezzo la size e' nota ma il prezzo NO: lo sceglie questa funzione». Non e' vero.
  // Nel ciclo di riprezzo il prezzo lo sceglie `decideReprice` e arriva a `valutaMercato` come
  // `proposedPrice`; questa funzione e' un VETO e il prezzo che restituisce non lo legge nessuno
  // (unico chiamante: `auto-reprice`). Quindi il confronto col prezzo di riferimento, che esiste per
  // proteggere il prezzo RESTITUITO, li' non proteggeva niente e si limitava a scartare livelli che
  // il pavimento pieno accettava — trasformando un'esenzione in una restrizione.
  //
  // MISURATO il 23 agosto sugli 11 ordini vivi dei mercati che avevano perso gambe: col confronto
  // come veto l'esenzione recuperava 1 rifiuto e ne CREAVA 4.
  //
  // LA RISPOSTA GIUSTA. Se il pavimento (che per un rinnovo e' zero) era soddisfatto e a scartare
  // sono stati SOLO i prezzi, il rinnovo e' ammissibile e il prezzo giusto e' quello che l'ordine ha
  // GIA': `prezzoMassimo`. Non e' un livello del book — `level` resta `null` e lo si dichiara — ed e'
  // per costruzione il piu' prudente disponibile, perche' `provaRinnovo` ha gia' preteso che size e
  // nozionale non salgano rispetto a quel prezzo. Non si restituisce mai un livello piu' caro.
  if (esenteRinnovo && scartatoPerPrezzo && prezzoMaxRinnovo !== null) {
    return {
      ok: true, price: prezzoMaxRinnovo, level: null, depthAheadUsd: +cum.toFixed(4),
      prezzoDiRiferimento: true,
      punteggioRelativo: punteggioDiUnLivello({ price: prezzoMaxRinnovo, scoringMid, bandRadiusCents }),
      motivo: `RINNOVO: nessun livello altrui in banda costa quanto o meno dell'ordine sostituito`
        + ` ($${prezzoMaxRinnovo}), quindi non se ne propone uno nuovo: il rinnovo tiene il prezzo che ha`
        + ` gia'. Profondita' altrui davanti $${cum.toFixed(2)} · ${rinnovo.motivo}`,
    };
  }
  return no(
 ...
 */
function tettoMercato({ saldoUsd = null, esposizioneMercatoUsd = 0, aggiuntaUsd = 0, chiusura = null } = {}) {
  ...
  // ── L'ESENZIONE DELLE GAMBE DI CHIUSURA (13 agosto 2026, decisione dell'operatore) ─────────────
  //
  // ⚠ IL FATTO. Alle 07:45:36 su Tel Aviv `0xcb034071`: posizione YES di 32,6 share **scoperta da
  // un'ora e sopra il minimo del venue**, quindi copribile — e `completaCoppia` non agiva perché la
  // gamba di copertura avrebbe portato il mercato a **$45,63 contro un tetto di $32,67**. Misurato
  // sulle 24 ore: **6.226 righe `tetto-mercato`** su sei mercati, tutti con posizione aperta. Non era
  // un caso isolato: una posizione aperta consuma il budget del mercato, e da lì il bot non riesce più
  // né a coprirla né a rinnovare le gambe che ha.
  //
  // Le due regole giudicano in verso opposto ed è il punto: **coprire riduce il rischio direzionale e
  // aumenta il capitale impegnato**. Il tetto per mercato esiste per limitare l'APERTURA di posizioni
  // nuove; applicarlo a chi sta chiudendo significa tenere aperta un'esposizione per rispettare un
  // limite pensato contro le esposizioni.
  //
  // ⚠ NON È UN'ESENZIONE IN BIANCO: il tetto non sparisce, si alza a un secondo tetto **assoluto**.
  // `TETTO_CHIUSURA = 2 × capPerMarketUsd`, e il fattore due non è arrotondato a piacere — è
  // esattamente «quanto basta a chiudere qualunque posizione che il bot possa davvero aprire»: la
  // posizione più grande apribile su un mercato vale il tetto ($32,67), e comprarne la controparte
  // costa al più `Q × p` con `p → 1`, cioè ancora ~$33,34. Sopra quella cifra non esiste nessuna
  // chiusura legittima, quindi sopra quella cifra si rifiuta come prima. Sul capitale di oggi vale
  // **$65,34, il 9,8% del portafoglio**: un mercato non può diventare una frazione irragionevole.
  //
  // ⚠ E VALE SOLO PER CHI CHIUDE, con una PROVA rifatta sull'ordine esatto — non una dichiarazione.
  // `chiusura.ok` lo decide `esenzione-chiusura`/`prova-riduzione`, cioè le stesse funzioni del tetto
  // per ORDINE (§5 p.76): SELL entro il posseduto, BUY entro `manca`. Un BUY così può solo APPAIARE —
  // nel caso limite porta i due lati in parti uguali, cioè esposizione direzionale ZERO — e mai
  // aprire. Qualunque lettura mancante lascia il tetto ordinario applicato.
  const chiude = !!(chiusura && chiusura.ok === true);
  const capOrdinario = capPerMarketUsd(saldoUsd);
  // Il fattore è DERIVATO, non arrotondato: la posizione più grande apribile vale `capOrdinario`, e
  // comprarne la controparte costa al più `Q × p` con `Q = capOrdinario / costoCoppia` e `p → 1`,
  // cioè `capOrdinario / costoCoppia`. Il tetto che basta è quindi `capOrdinario × (1 + 1/costoCoppia)`
  // ≈ **2,02×**. Il «2» tondo lasciava scoperto il caso peggiore di 63 centesimi — e un tetto che non
  // copre il caso per cui è stato costruito è un tetto tarato a occhio.
  const capChiusura = +Math.min(capOrdinario * (1 + 1 / COSTO_COPPIA), saldoUsd).toFixed(2);
  // Il tetto è FISSO e arriva dal modulo condiviso — lo STESSO numero che il pianificatore ha usato per
  // costruire la riga che sta per essere quotata. `capPerMarketUsd` si clampa al saldo, quindi con meno
  // di $130 in cassa il tetto scende al saldo: può solo stringere.
  const capUsd = chiude ? capChiusura : capOrdinario;
  ...
  const comeDetto = chiude
    ? `gamba di CHIUSURA: il tetto sale da $${capOrdinario.toFixed(2)} a $${capUsd.toFixed(2)} (${chiusura.motivo || 'chiusura provata'})`
    : (capUsd < MARKET_CAP_FIXED_USD
      ? `il tetto per mercato è $${MARKET_CAP_FIXED_USD} ma il saldo è più basso, quindi vale $${capUsd.toFixed(2)}`
      : `il tetto per mercato è $${capUsd.toFixed(2)}, YES+NO sommati`);
  if (dopo > capUsd + 1e-9) {
    return { consentito: false, capUsd, dopoUsd: dopo, chiusura: chiude,
      motivo: `il mercato arriverebbe a $${dopo.toFixed(2)}: ${comeDetto}` };
  }
  // `sforaOrdinario` è il segnale per il chiamante: la gamba passa SOLO grazie all'esenzione, quindi
  // sta impegnando capitale oltre il tetto normale e le aperture nuove vanno fermate finché rientra.
  const sforaOrdinario = chiude && dopo > capOrdinario + 1e-9;
  return { consentito: true, capUsd, dopoUsd: dopo, chiusura: chiude, sforaOrdinario,
    capOrdinarioUsd: capOrdinario,
    motivo: sforaOrdinario
      ? `$${dopo.toFixed(2)} OLTRE il tetto ordinario di $${capOrdinario.toFixed(2)}, consentito perché chiude: ${comeDetto}`
      : `$${dopo.toFixed(2)} entro il tetto ($${capUsd.toFixed(2)})` };
}
...
function valutaMercato(a = {}) {
  const {
    ...
    // La PROVA di chiusura, non una dichiarazione: la costruisce il chiamante con le stesse funzioni
    // del tetto per ordine, e senza di essa il tetto per mercato resta quello ordinario.
    chiusura = null,
    // ── LA PROVA DI RINNOVO — IL FILO CHE MANCAVA (23 agosto 2026) ────────────────────────────────
    // `trovaLivello` sa esentare un rinnovo dal pavimento di profondita' dal 16 agosto, e
    // `auto-reprice` costruisce la prova (`esenzione-rinnovo.provaRinnovo`) e la passa QUI dentro.
    // Ma `valutaMercato` non la destrutturava e non la inoltrava: il parametro arrivava all'unica
    // funzione che non lo riceveva, e l'esenzione era INERTE da sette giorni. E' la sesta occorrenza
    // in questo repo della classe «una dep col nome giusto che nessuno inietta e' un valore di
    // difetto che nessuno ha chiesto» — con l'aggravante che qui il mittente c'era e il filo era
    // tagliato a meta' strada, dentro il ricevitore.
    // MISURATO PRIMA DI TOCCARE: 63 ordini morti per GTD senza rinnovo fra le 06:13Z e le 13:18Z del
    // 23 agosto, $862,58 fuori dal libro, e 49 di quelle morti col gate `motore-non-conforme` —
    // TUTTE E 49 `profondita-insufficiente`, di cui 39 «la banda finisce prima del pavimento», cioe'
    // esattamente il caso che l'esenzione esiste per coprire.
    // ⚠ NON E' UNA DICHIARAZIONE: la prova la rifa' `esenzione-rinnovo` contro gli ordini VIVI del
    // venue, e qualunque incertezza vale «apertura». Qui si inoltra e basta.
    rinnovo = null,
  ...
  const r5 = (deps.tettoMercato || tettoMercato)({ saldoUsd, esposizioneMercatoUsd, aggiuntaUsd: aggiunta, chiusura });
  controlli.tetto = r5;
  ...
  // ── L'ESENZIONE DI RINNOVO E' MONOTONA PER COSTRUZIONE: SI PROVA PRIMA SENZA ─────────────────────
  //
  // ⚠ MISURATO, NON RAGIONATO. Inoltrare `rinnovo` direttamente a `trovaLivello` sembrava la
  // correzione ovvia, ed e' stata provata a secco su 11 ordini vivi il 23 agosto: recuperava 1
  // rifiuto e ne CREAVA 4 — quattro ordini che passavano prima e non passavano dopo. La causa e'
  // `prezzoMaxRinnovo` dentro `trovaLivello`: esiste per proteggere il PREZZO che quella funzione
  // restituisce, ma sul percorso del riprezzo `valutaMercato` e' un VETO e il prezzo lo decide
  // `decideReprice` — quindi quel filtro non protegge niente e si limita a scartare livelli che il
  // pavimento pieno accettava. Un'esenzione che RIFIUTA di piu' non e' un'esenzione.
  //
  // LA FORMA GIUSTA: si valuta col pavimento PIENO; solo se cade, e solo se il rinnovo e' PROVATO,
  // si rivaluta con l'esenzione. Cosi' l'esenzione puo' solo trasformare un rifiuto in un'accettazione
  // — mai il contrario — e la proprieta' e' vera per struttura, non per taratura. Costa una seconda
  // passata sui soli casi che stavano gia' per essere rifiutati.
  const livPieno = (deps.trovaLivello || trovaLivello)({
    side, bookLevels, bandBounds, ownOrders, tick, pavimentoUsd: pav.usd, scoringMid, bandRadiusCents,
  });
  const esente = !!(rinnovo && rinnovo.esente === true);
  const liv = (livPieno.ok || !esente)
    ? livPieno
    : (() => {
      const conEsenzione = (deps.trovaLivello || trovaLivello)({
        side, bookLevels, bandBounds, ownOrders, tick, pavimentoUsd: pav.usd, scoringMid, bandRadiusCents,
        rinnovo,
      });
      // Se anche l'esenzione non basta si tiene il verdetto PIENO: il suo motivo descrive il pavimento
      // vero, che e' l'informazione utile a chi legge il giornale. Un motivo «contro $0.00» direbbe che
      // il pavimento non c'era, e non e' quello che e' successo.
      return conEsenzione.ok ? { ...conEsenzione, perRinnovo: true } : livPieno;
    })();
  controlli.livello = liv;
  // L'esenzione si DICHIARA, o un pavimento che vale 0 su un rinnovo sarebbe indistinguibile da un
  // pavimento che vale 0 perche' nessuno l'ha misurato. Forma compatta: l'audit la scrive per intero.
  controlli.rinnovo = rinnovo
    ? { esente: esente, applicata: liv.perRinnovo === true,
      sostituisce: rinnovo.sostituisce || null, motivo: rinnovo.motivo || null }
    : null;

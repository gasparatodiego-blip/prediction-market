 ...
 *
 * @param {object} a
 *   liquiditaMediaShare  media della profondità in banda (bid+ask) sulla finestra, in SHARE
 *   prezzoRif            il prezzo a cui convertire le share in dollari (il mid di scoring)
 *   campioni             quanti campioni ha la media (sotto `minCampioni` non è affidabile)
 * @returns {{usd:number, fonte:'media'|'fallback', mediaUsd:number|null, motivo:string}}
 */
function pavimentoDepth({ liquiditaMediaShare = null, prezzoRif = null, campioni = 0, minCampioni = 5 } = {}) {
  const affidabile = fin(liquiditaMediaShare) && liquiditaMediaShare > 0
    && fin(prezzoRif) && prezzoRif > 0 && campioni >= minCampioni;
  if (!affidabile) {
    return {
      usd: DEPTH_FLOOR_FALLBACK_USD, fonte: 'fallback', mediaUsd: null,
  ...
  return {
    usd, fonte: 'media', mediaUsd: +mediaUsd.toFixed(2),

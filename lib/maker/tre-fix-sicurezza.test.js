...
(async () => {
  ...
  console.log('\n══ FIX 3b · L USCITA FORZATA PUO ATTRAVERSARE, MA SOLO SE LO DICHIARA');
  {
    ...
    // ⚠ 5 → 6 IL 17 AGOSTO 2026, e il punto nuovo si AMMETTE dopo averlo provato, non allargando il
    // pattern. Il sesto e' l'USCITA ORDINARIA quando la scala d'urgenza concede di attraversare
    // (permesso dell'operatore, `attraversamento-uscita.js`). E' un SELL di una posizione detenuta —
    // esattamente il caso che `provaChiusura` copre — ed e' vincolato da quattro limiti: solo su
    // un'uscita o un completamento coppia, solo dal gradino 1 in su, mai sotto il pavimento della
    // scala, e sempre dichiarato a verbale. Il conteggio resta perche' la proprieta' difesa e' che
    // attraversare sia RARO, CONTATO e DICHIARATO: alzarlo in silenzio la spegnerebbe.
    ok('  e SOLO in sei punti dichiarati: uscita forzata, Livello 1, chiusura rapida, i DUE della chiusura pre-scadenza, e l uscita che attraversa su concessione della scala',
      nAttraversa === 6, `${nAttraversa} occorrenze nel codice`);
    ok('  e il sesto passa dal modulo dei quattro limiti, non da un `if` scritto a mano',
      /valutaAttraversamento\(/.test(soloCodice) && /attr\.attraversa === true[\s\S]{0,200}attraversaApposta: true/.test(soloCodice));
    ...
    ok('  il secondo e il Livello 1 del merge, e compra (non vende)',
      // ⚠ AGGIORNATO ANCORA L'11 AGOSTO: il ramo `taker` ora dichiara i TRE campi che l'eccezione
      // `completaCoppiaOk` richiede — prima ne dichiarava uno, e i 852 `merge-livello-1-reject-would-cross`
      // misurati sui giornali sono la prova che il Livello 1 non poteva eseguire. La proprieta' difesa e'
      // sempre la stessa: il L1 e' un TAKER che COMPRA, e compra dentro un costo massimo DICHIARATO.
      /t\.taker[\s\S]{0,320}attraversaApposta: true, completaCoppia: true/.test(ac)
      && /prezzoCaricoCoppia: prezzoCarico, tettoCoppiaCents: 100/.test(ac)

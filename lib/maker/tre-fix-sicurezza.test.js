...
(async () => {
  ...
  console.log('\n══ FIX 3b · L USCITA FORZATA PUO ATTRAVERSARE, MA SOLO SE LO DICHIARA');
  {
    ...
    ok('  e SOLO in tre punti dichiarati: uscita forzata, Livello 1, chiusura rapida',
      (ac.match(/attraversaApposta/g) || []).length === 3,
      `${(ac.match(/attraversaApposta/g) || []).length} occorrenze`);
    ok('  il secondo e il Livello 1 del merge, e compra (non vende)',
      /t\.taker \? \{ attraversaApposta: true \} : \{ inCoda: true \}/.test(ac)

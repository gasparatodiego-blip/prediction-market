...
(async () => {
  ...
  console.log('\n══ 3 · UN MERCATO CHE HA GIÀ ORDINI NOSTRI NON VIENE RISCRITTO A OGNI GIRO');
  {
    ...
    // ⚠ RITARATA DI NUOVO il 12 agosto 2026, e per la seconda volta si tara l'INGRESSO. Col tetto
    // derivato ($32,67 a $663) un mercato con $20 dentro avrebbe $12,67 di spazio, sotto il minimo di
    // allocazione — e uscirebbe dal giro per un motivo che NON e' quello che questo caso prova.
    // Il nozionale si DERIVA dai due moduli invece di essere scritto a mano: cosi' la prossima
    // ritaratura del tetto non produce un rosso che non segnala niente.
    const CONC = require('../rewards/concentration');
    const TRIG_ = require('./trigger-capitale-fermo');
    const nozAperto = Math.max(1, +(CONC.MARKET_CAP_FIXED_USD - TRIG_.MIN_ALLOCAZIONE_USD - 2).toFixed(2));
    const ordini = [{ marketId: MKT_APERTO, price: 0.5, size: +(nozAperto / 0.5).toFixed(2) }];
  ...
  console.log('\n══ 6 · LA REGRESSIONE SAREBBE SILENZIOSA — quindi è il sorgente a doverla escludere');
  {
    ...
    // ⚠ QUESTA ASSERZIONE E' STATA ROVESCIATA IL 17 AGOSTO 2026, PER DECISIONE DELL'OPERATORE, e va
    // detto perche' — il fatto che difendeva era VERO e adesso e' falso apposta.
    //
    // Diceva: «la corsia manuale chiede `live-min` a prescindere da `MAKER_MODE` del processo», e
    // cercava `mode: 'live-min'` cablato in `buildPlacementAdapter`. Serviva a smentire l'idea che
    // «agent41 gira in MAKER_MODE=off, quindi quel gate non lo tocca»: il gate live-min lo toccava
    // eccome, perche' il modo era scritto a mano.
    //
    // Il rovescio della medaglia l'ha misurato §4.14: **`MAKER_MODE` non gate la corsia manuale**, cioe'
    // una delle cinture dell'operatore non fermava niente sulla strada da cui il bot piazza davvero.
    // Delle cinque ne mordevano due. Adesso il modo si legge da `cinture-armamento.modoVivo`, quindi
    // `MAKER_MODE=off` produce `mode: 'off'` e il gate rifiuta con `maker-mode`.
    //
    // La proprieta' da difendere non e' sparita, e' cambiata: **il modo non deve tornare a essere un
    // letterale**, e quando l'ambiente e' vivo deve valere `live-min` (cosi' il gate live-min resta
    // raggiungibile, che era il punto originale di questa riga).
    const { modoVivo } = require('./cinture-armamento');
    const senzaCommentiMo = mo.replace(/^\s*\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
    ok('la corsia manuale NON cabla piu\' il modo: lo legge dalle cinture',
      /mode:\s*modoVivo\(/.test(senzaCommentiMo) && !/mode:\s*'live-min'/.test(senzaCommentiMo),
      'era la cintura MAKER_MODE che non mordeva (§4.14)');
    ok('  e con un ambiente VIVO il modo resta `live-min` — il gate live-min e\' ancora raggiungibile',
      modoVivo({ MAKER_MODE: 'live-min' }) === true && modoVivo({ MAKER_MODE: 'off' }) === false);
  }

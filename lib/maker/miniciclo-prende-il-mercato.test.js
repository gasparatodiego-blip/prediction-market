 ...
 */
async function esegui({ righe, ordini = [], setManual, setAutoClose, saldoUsd = 668.25 } = {}) {
  ...
  const r = await A.miniCiclo({ saldoUsd, forzato: true }, {
    ...
    rampa: () => ({ attiva: false, residuo: Infinity, aperti: 0, motivo: 'test: rampa non applicata' }),
    registraMercatoAperto: ({ marketId }) => { eventi.push(['rampa', marketId]); return { ok: true, giaPresente: false }; },
...
(async () => {
  ...
  console.log('\n══ 2 · L ORDINE NON È UN DETTAGLIO — fra preparazione e ordini non esiste un istante scoperto');
  {
    ...
    ok('la gestione manuale è presa PRIMA del primo ordine', iMan !== -1 && iPia !== -1 && iMan < iPia, `man@${iMan} piazza@${iPia}`);
    ok('l uscita automatica è accesa PRIMA del primo ordine', iAcc !== -1 && iAcc < iPia, `acc@${iAcc} piazza@${iPia}`);
    ok('  ed è accesa prima che gli ordini esistano, non «subito dopo»', iMan < iAcc && iAcc < iPia);
  }
  ...
  console.log('\n══ 3 · UN MERCATO CHE HA GIÀ ORDINI NOSTRI NON VIENE RISCRITTO A OGNI GIRO');
  {
    ...
    const ordini = [{ marketId: MKT_APERTO, price: 0.5, size: 120 }];
  ...
  console.log('\n══ 4 · PREPARAZIONE FALLITA ⇒ QUEL MERCATO ESCE DAL GIRO (e gli altri proseguono)');
  {
    ...
    const d = await A.preparaMercatoNuovo(MKT_NUOVO, undefined, () => ({ ok: true }));
    ok('setManual non cablato ⇒ rifiuto esplicito, mai un via libera',
      d.ok === false && /nessuna funzione setManual cablata/.test(d.motivo), d.motivo);
    const e = await A.preparaMercatoNuovo(MKT_NUOVO, () => ({ ok: true }), undefined);
    ...
    const f = await A.preparaMercatoNuovo(MKT_NUOVO, () => { throw new Error('guasto simulato'); }, () => ({ ok: true }));

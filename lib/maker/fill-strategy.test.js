...
console.log('\n── 9 · IL CICLO: spento di default non fa nulla, e il kill lo ferma ──');
{
  ...
  return (async () => {
    ...
    // ── 10 · IL CONTRATTO CON LA UI ───────────────────────────────────────────────────────────────
    // Gli stessi payload che FillStrategyPanel manda in POST, fatti passare per il vero setFillStrategy.
    // Non e' un test del DOM (questo repo non ha un harness DOM) — e' il contratto fra i due lati, che e'
    // la parte che puo' davvero rompersi senza che nessuno se ne accorga.
    console.log('\n── 10 · CONTRATTO UI → configurazione persistita ──');
    const uiDeps = {
      fillStrategyConfigFile: path.join(TMP, 'ui.json'),
      fillStrategyAuditFile: path.join(TMP, 'ui-audit.jsonl'),
    };
    const post = (payload) => CFG.setFillStrategy({ ...payload, by: 'fill-strategy-panel' }, uiDeps);

    ok('UI: toggle globale ON scrive davvero', post({ scope: 'global', enabled: true }).ok === true
      && CFG.readFillStrategyConfig(uiDeps).globalEnabled === true);
    ok('UI: toggle globale OFF scrive davvero', post({ scope: 'global', enabled: false }).ok === true
      && CFG.readFillStrategyConfig(uiDeps).globalEnabled === false);
    post({ scope: 'global', enabled: true });

    ok('UI: toggle per mercato scrive davvero', post({ scope: 'market', marketId: '0xui', enabled: true }).ok === true
      && CFG.isFillStrategyEnabled('0xui', uiDeps).marketEnabled === true);

    ok('UI: slider take-profit scrive il valore', post({ scope: 'market', marketId: '0xui', patch: { takeProfitCents: 0.9 } }).ok === true
      && near(CFG.paramsFor('0xui', uiDeps).takeProfitCents, 0.9));
    ok('UI: slider stop-loss scrive il valore', post({ scope: 'market', marketId: '0xui', patch: { stopLossPct: 4 } }).ok === true
      && near(CFG.paramsFor('0xui', uiDeps).stopLossPct, 4));
    ok('UI: scrivere uno slider non spegne il mercato', CFG.isFillStrategyEnabled('0xui', uiDeps).marketEnabled === true);
    ok('UI: scrivere uno slider non tocca il globale', CFG.readFillStrategyConfig(uiDeps).globalEnabled === true);

    // Il pannello non ha alcun campo per il tetto: se un giorno qualcuno lo aggiungesse, questo fallisce.
    ok('UI: un payload con il tetto viene RIFIUTATO dal server', post({ scope: 'market', marketId: '0xui', patch: { positionCapUsd: 999 } }).ok === false);

    // ── Invarianti di sorgente del pannello: gli slider disabilitati sono legati allo STESSO booleano
    //    del toggle, e il tetto non ha nessun controllo che lo scriva.
    const panel = fs.readFileSync(path.join(__dirname, '..', '..', 'app', 'components', 'FillStrategyPanel.tsx'), 'utf8');
    ok('pannello: `off` e definito come !m.enabled', /const off = !m\.enabled;/.test(panel));
    ok('pannello: la classe is-off degli slider usa quel booleano', /fst-ctl \$\{off \? 'is-off' : ''\}/.test(panel));
    ok('pannello: .is-off riduce l opacita (disabilitazione visiva)', /\.fst-ctl\.is-off \{ opacity:/.test(panel));
    ok('pannello: entrambi gli slider portano disabled={off || ...}', (panel.match(/disabled=\{off \|\|/g) || []).length === 2);
    ok('pannello: il tetto e reso come riga con lucchetto, senza input', /🔒 Tetto posizione/.test(panel));
    ok('pannello: nessun controllo scrive un tetto', !/positionCapUsd:\s*[^}]*\bpatch\b/.test(panel) && !/patch: \{ positionCapUsd/.test(panel));

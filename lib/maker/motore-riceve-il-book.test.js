...
function giro({ depth, liquiditaMedia, campioni = 200, ordine = null, mid = undefined, ordini = null }) {
  const righe = [];
  const visti = [];
  ...
  return AR.runAutoRepriceCycle({
    ...
    resolveRules: () => (mid === undefined ? REGOLE() : REGOLE(mid)),
    ...
    listOrders: async () => ({ ok: true, simulated: false, orders: ordini || [o] }),
...
(async () => {
  ...
  {
    // ⚠ RISCRITTO IL 23 AGOSTO 2026, NON AMMORBIDITO. Fino a ieri questo blocco chiedeva il veto su un
    // ordine che il ciclo stava RINNOVANDO allo stesso prezzo — e da oggi quel caso e' esente dal
    // pavimento per decisione dell'operatore (§4.1, «il rinnovo non e' un ingresso»). La proprieta'
    // che questo file difende non e' «il pavimento boccia sempre»: e' «il motore riceve la scala e le
    // Regole 2-5 girano davvero», e si difende meglio ADESSO, perche' un motore cieco non saprebbe
    // distinguere un rinnovo da un ingresso.
    // Quindi il pavimento si prova dove vive: sull'INGRESSO, cioe' con `rinnovo: null`, che e'
    // esattamente cio' che il ciclo passa quando l'ordine non sta sostituendo niente.
    // Media 4000 share x 0.6515 = $2606 ⇒ pavimento $260.6. Il secondo livello ne porta ~$7.8.
    const { visti } = await giro({ depth: BOOK_SOTTILE, liquiditaMedia: 4000 });
    const v = valutaMercato({ ...visti[0], rinnovo: null });
    ...
    // L'ALTRA META', sullo STESSO book: lo stesso ordine, che pero' sta rinnovando, passa.
    const r = valutaMercato(visti[0]);
    ok('lo STESSO book, ma per un RINNOVO provato, non e piu bocciato', r.ok === true, r.motivo.slice(0, 100));
    ok('  e il ciclo lo dichiara invece di applicarlo in silenzio',
      r.controlli.rinnovo && r.controlli.rinnovo.esente === true && r.controlli.rinnovo.applicata === true);
  }

  console.log('\n══ 2-bis · IL VETO DEL CICLO SOPRAVVIVE DOVE DEVE: SU UN INSEGUIMENTO CHE ALZA IL NOZIONALE');
  {
    // Il mid si e' mosso a 0.68: l'ordine a 0.649 e' fuori banda e va inseguito VERSO L ALTO. Un
    // inseguimento al rialzo non e' un rinnovo — mette piu' capitale a riposo — quindi
    // `esenzione-rinnovo` lo rifiuta e il pavimento torna pieno. E' il caso in cui il veto del ciclo
    // deve continuare a esistere.
    const { righe, visti } = await giro({ depth: BOOK_SOTTILE, liquiditaMedia: 4000, mid: 0.68 });
    ok('il ciclo ha davvero interpellato il motore', visti.length >= 1, `${visti.length} chiamate`);
    ok('  e l inseguimento chiede un prezzo PIU ALTO dell ordine a libro',
      visti[0] && visti[0].proposedPrice > 0.649, String(visti[0] && visti[0].proposedPrice));
    ok('  quindi NON e un rinnovo: l esenzione e negata',
      visti[0] && visti[0].rinnovo && visti[0].rinnovo.esente === false,
      visti[0] && visti[0].rinnovo && visti[0].rinnovo.motivo);
  ...
  {
    // ⚠ SERVE UN VETO CHE SIA DAVVERO IL PAVIMENTO. Con un rinnovo puro il pavimento non morde piu'
    // (blocco 2), e con l'inseguimento del blocco 2-bis a rifiutare e' la Regola 1, che esce PRIMA di
    // calcolare il pavimento — quindi i due numeri sarebbero `null` a ragione.
    // Il caso scelto e' reale e non costruito ad arte: DUE nostre gambe sullo stesso token e lato
    // (un doppione, cio' che `doppioni.js` esiste per togliere). `esenzione-rinnovo` confronta col
    // piu' PICCOLO — il confronto piu' severo — quindi la gamba grande non sta rinnovando e il
    // pavimento la giudica per intero, con i suoi due numeri.
    const DOPPI = [
      { orderId: '0xgrande', source: 'manual-ui', side: 'BUY', price: 0.649, size: 60.1, sizeRemaining: 60.1,
        marketId: MKT, tokenId: 'ty', secondsToExpiry: 120, orderType: 'GTD' },
      { orderId: '0xpiccolo', source: 'manual-ui', side: 'BUY', price: 0.649, size: 30, sizeRemaining: 30,
        marketId: MKT, tokenId: 'ty', secondsToExpiry: 900, orderType: 'GTD' },
    ];
    const { righe } = await giro({ depth: BOOK_SOTTILE, liquiditaMedia: 4000, ordini: DOPPI });

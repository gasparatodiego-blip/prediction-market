...
// LA REGOLA DELLA GAMBA RIMASTA SOLA. Pura: riceve i lati vivi e l'istante in cui il mercato e'
// diventato orfano, e risponde. Lo store dei timer resta INIETTATO (deps.leggiOrfanaDa /
// deps.aggiornaTimerOrfana), cosi' il ciclo si puo' esercitare senza toccare il disco.
const { valutaGambaOrfana } = require('./gamba-orfana');
...
async function runAutoRepriceCycle(deps = {}) {
  ...
  // `orfaneAperte`: i mercati che in questo istante hanno una gamba sola, col countdown residuo.
  ...
  for (const marketId of cfgState.enabledMarketIds) {
    ...
    for (const order of owned) {
      ...
      const nostriSulLato = owned.filter((x) => x.book === order.book);
      ...
      if (d.action !== 'hold' && d.action !== 'skip'
          && typeof deps.leggiProfilo === 'function' && typeof deps.valutaPiazzamento === 'function') {
        let veto = null;
        try {
          const prof = deps.leggiProfilo(marketId);
          const profilo = prof && prof.profile ? prof.profile : null;
          const v = deps.valutaPiazzamento({
            profilo, marketId, side: order.side || 'BUY',
            bookLevels: d.bookLevels || null, bandBounds: d.bandBounds || null,
            bandRadiusCents: d.bandRadiusC, tick: rules.tick, scoringMid: d.scoringMid,
            ownOrders: nostriSulLato,
            proposedSize: order.size, proposedPrice: d.targetPrice != null ? d.targetPrice : order.price,
            spreadCorrente: d.spreadCorrente != null ? d.spreadCorrente : null,
            saldoUsd: Number.isFinite(deps.saldoUsd) ? deps.saldoUsd : null,
            esposizioneMercatoUsd: Number.isFinite(deps.esposizioneMercatoUsd) ? deps.esposizioneMercatoUsd : 0,
            now: t0,
          });
          if (!v || v.ok !== true) {
            veto = { profilo, motivo: (v && v.reason) || 'percorso di profilo non superato',
              profiloMotivo: prof ? prof.reason : null };
          }
        } catch (e) {
          ...
          veto = { profilo: null, motivo: `valutazione di profilo fallita: ${e.message}` };
        }
        if (veto) {
          d.action = 'skip';
          d.gate = 'profilo-non-conforme';
          d.reason = `profilo ${veto.profilo || 'sconosciuto'}: ${veto.motivo}`
            + (veto.profiloMotivo ? ` (${veto.profiloMotivo})` : '');
        }
      }
      ...
      if (d.action === 'cancel') {
        ...
        if (okc) {
          ...
          cancellazioni.push({
            ...
            motivo: 'mai-primo-sul-libro', dettaglio: d.reason,
    ...
    // ══ LA GAMBA RIMASTA SOLA — DIECI MINUTI PER RITROVARE L'ALTRA ═════════════════════════════
    //
    // Si guarda DOPO aver processato gli ordini del mercato, perché è allora che si sa quali lati sono
    // ancora vivi: una cancellazione decisa in questo stesso giro deve contare subito.
    //
    // NON PIAZZA NIENTE. La finestra esiste proprio perché il ciclo normale abbia tempo di ritentare
    // l'altra gamba con le SUE regole (mai-primo, depth adattiva, profilo). Qui non c'è nessuna
    // scorciatoia per «richiudere in fretta la coppia»: sarebbe il percorso che salta i controlli per
    // un motivo che suona ragionevole.
    //
    // Assente per difetto: senza `deps.leggiOrfanaDa` e `deps.aggiornaTimerOrfana` questo blocco non
    // gira e il ciclo è quello di prima.
    if (typeof deps.leggiOrfanaDa === 'function' && typeof deps.aggiornaTimerOrfana === 'function') {
      try {
        // I lati ANCORA vivi: quelli cancellati in questo giro sono già usciti da `vivi`.
        const vivi = owned.filter((o) => !tolti.has(o.orderId));
        const v = valutaGambaOrfana({
          marketId, bookAttivi: vivi.map((o) => o.book),
          orfanaDa: deps.leggiOrfanaDa(marketId), now: t0,
        });
        m.gambaOrfana = { stato: v.stato, azione: v.azione, bookSuperstite: v.bookSuperstite, restaSec: v.restaSec };

        if (v.azione === 'cancella') {
          // Si cancella la superstite. Una sola: se ce ne fosse più d'una sullo stesso lato, tutte.
          for (const o of vivi.filter((x) => x.book === v.bookSuperstite)) {
            let rc;
            try { rc = typeof deps.cancelOrder === 'function' ? await deps.cancelOrder({ orderId: o.orderId, marketId }) : null; }
            catch (e) { rc = { ok: false, reason: e && e.message ? e.message : String(e) }; }
            const okc = !!(rc && rc.ok !== false);
            audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice',
              outcome: okc ? 'cancelled-gamba-orfana' : 'reject-cancel-failed', trigger: 'gamba-orfana-scaduta',
            ...
            if (okc) {
              ...
              cancellazioni.push({
                ...
                notionalUsd: Number.isFinite(o.price) && Number.isFinite(o.size) ? +(o.price * o.size).toFixed(2) : null,
                motivo: 'gamba-orfana-scaduta', dettaglio: v.motivo, at: new Date(t0).toISOString(),
              });
            } else m.failed = (m.failed || 0) + 1;
            actions.push({ marketId, orderId: o.orderId, action: okc ? 'cancel' : 'cancel-failed',
              gate: 'gamba-orfana-scaduta', reason: v.motivo, price: o.price, targetPrice: null });
          }
        }
        // Il timer si aggiorna SEMPRE secondo l'azione decisa: avvia, annulla, oppure resta com'è.
        deps.aggiornaTimerOrfana({ marketId, azione: v.azione, orfanaDa: v.orfanaDa, bookSuperstite: v.bookSuperstite });
        if (v.stato === 'orfana' && v.azione !== 'cancella') {
          orfaneAperte.push({ marketId, marketTitle: rules.title || null, book: v.bookSuperstite,
            orfanaDa: v.orfanaDa, scadeAms: v.scadeAms, restaSec: v.restaSec, motivo: v.motivo });
        }
      } catch (e) {
        // Un errore qui non deve fermare il ciclo né cancellare niente: si registra e si va avanti.
        m.gambaOrfanaError = e && e.message ? e.message : String(e);
      }
    }
  ...
  return { at: new Date(t0).toISOString(), ran: true, gate: null, reason: null, markets, actions, events,
    cancellazioni, orfaneAperte, latencyMs: now() - t0 };
}

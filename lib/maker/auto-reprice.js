...
const { decidiStantio, registroStantio, timeoutMs: timeoutStantioMs, eCieco } = require('./mid-stantio');
  ...
  for (const marketId of cfgState.enabledMarketIds) {
    ...
    for (const order of owned) {
      ...
      if (d.action === 'skip') {
        ...
        // solo tipo di cecità.
        if (eCieco(d.gate)) ordiniCiechi += 1;
    ...
    if (owned.length > 0) {
      ...
      if (!cieco) {
        ...
        audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice', outcome: 'mid-stantio-timeout',
          marketRef: `cid_${String(marketId).replace(/^0x/, '')}`, gate: 'mid-stantio', reason: st.motivo,
          observed: { cecitaMs: st.cecitaMs, timeoutMs: timeoutStantioMs(), ordini: owned.length } });
        for (const o of owned) {
          ...
          audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice',
            outcome: (c && c.ok) ? 'mid-stantio-cancellato' : 'mid-stantio-cancel-fallito',
            marketRef: `cid_${String(marketId).replace(/^0x/, '')}`, orderId: o.orderId,
            reason: (c && c.reason) || st.motivo, observed: { cecitaMs: st.cecitaMs } });
        }

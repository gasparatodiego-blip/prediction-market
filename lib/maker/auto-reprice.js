...
function decideReprice({ order, rules, config, lastRepriceAt = null, consecutiveBreaches = 0, repricesThisHour = 0, now = Date.now(), ownOrders = null } = {}, deps = {}) {
  ...
  if (inBand(price, scoringMid, maxSpreadCents)) {
    ...
    if (inseguimentoDovuto && !railInseguimento) {
      ...
      if (typeof deps.resolveDepth === 'function') {
        ...
        if (q.quotabile === false) {
          return out('skip', 'mai-primo-non-quotabile',
            `l'inseguimento vorrebbe ${want} ma «mai primi sul libro» non ammette un prezzo su questo lato: ${q.reason}`
            ...
            { ...withTtl, ...chase, targetPrice: want, bestOther: q.bestOther });
        }
        if (q.ok && Number.isFinite(q.price)) {
          ...
          if (codaTiraDentro && giaMeglioDelBersaglio) {
            return out('hold', 'inseguimento-contro-mai-primo',
              ...
              + `ma al piazzamento «mai primi» imporrebbe ${q.price} (${distCodaC.toFixed(3)}¢ dal mid, un tick dietro il miglior bid altrui ${q.bestOther}) — `
              ...
              + `non c'e' nessuna ragione economica per allontanarlo. L'ordine non viene toccato finche' non cambia il book `
              + `(il concorrente si sposta, il mid si muove abbastanza, o l'ordine esce dalla banda premiante).`,
              { ...withTtl, ...chase,
                // I quattro numeri su cui la soppressione e' stata decisa, per poterla verificare a
      ...
      if (q.ok && Number.isFinite(q.price) && q.onTop === false && Math.abs(q.price - price) > tick / 1000 && price > q.price) {
        ...
        if (vqCoda.valid) {
          return out('reprice', 'top-of-book',
            ...
            { ...withTtl, targetPrice: q.price, breachConfirmed: true, bestOther: q.bestOther });
        }
      }
    }
...
async function runAutoRepriceCycle(deps = {}) {
  ...
  for (const marketId of cfgState.enabledMarketIds) {
    ...
    for (const o of owned) vistiARiposo.add(o.orderId);
    ...
    for (const order of owned) {
      ...
      if (d.action === 'cancel') {
        ...
        audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice',
          ...
          requested: { book: order.book, fromPrice: order.price, toPrice: null, size: order.size } });
        if (okc) m.repriced = (m.repriced || 0);

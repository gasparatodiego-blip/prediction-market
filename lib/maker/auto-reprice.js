...
const { decidiStantio, registroStantio, timeoutMs: timeoutStantioMs, eCieco, causaCecita, motivoCecita, cecitaVera } = require('./mid-stantio');
 ...
 */
/**
 * IL LETTORE DEL REGISTRO DELLE USCITE — definito UNA volta, iniettato da agent40, e usato come
 * default da chi non lo inietta. Non e' un ripiego «di comodo»: e' la funzione di produzione, e il
 * fatto che sia anche il default e' cio' che impedisce che esista un secondo comportamento.
 * ⚠ Il `require` sta dentro e non in testa al file di proposito: `ordini-di-uscita` tocca il
 * filesystem, e caricarlo all'import obbligherebbe ogni test che importa `auto-reprice` a montare
 * `DATA_DIR`.
 */
const leggiStatoUscita = (orderId) => require('./ordini-di-uscita').statoOrdine(orderId);
...
function decideReprice({ order, rules, config, lastRepriceAt = null, consecutiveBreaches = 0, repricesThisHour = 0, now = Date.now(), ownOrders = null } = {}, deps = {}) {
  ...
  // ══ QUESTO ORDINE E' UN'USCITA? — 24 agosto 2026, decisione dell'operatore ═════════════════════
  // Gli ordini arrivano qui RILETTI DAL VENUE, e il venue conosce prezzo, lato e size: non le nostre
  // intenzioni. E' per questo che `band-exit` ha spostato NOVE VOLTE il BUY di completamento da 0.516
  // — che non e' una quotazione fuori banda ma `101¢ − carico`, il tetto della coppia al centesimo —
  // fino a 0.717, cioe' coppia 121,1¢: venti centesimi oltre il tetto, $11,28 di perdita garantita al
  // fill. Il marcatore lo mette `auto-close.chiudendo` all'origine e lo persiste `manual-order`; qui
  // lo si LEGGE.
  //
  // ⚠ FAIL-CLOSED, E COSTA: registro illeggibile ⇒ `noto:false` ⇒ **nessun ordine viene spostato dalla
  // banda**. La direzione e' scelta, non comoda. Non spostare un ordine costa il PREMIO che avrebbe
  // maturato rientrando in banda — zero dollari di perdita, un ricavo mancato, per il tempo in cui il
  // file resta illeggibile; spostare un'uscita costa la perdita misurata sopra. Fra un ricavo mancato
  // e una perdita certa non c'e' scelta. E il rinnovo GTD non passa di qui e non e' toccato: un ordine
  // continua a essere ri-piazzato allo stesso prezzo anche mentre il registro e' rotto.
  // ⚠ LA DEP NON E' FACOLTATIVA E NON HA UN RAMO CHE VIVE SOLO NEI TEST. Il ripiego e' un default
  // NOMINATO (`leggiStatoUscita`, definito una volta in fondo al file) che e' **la stessa identica
  // funzione** che agent40 inietta: chi non la passa non ottiene «un altro comportamento», ottiene
  // quello di produzione. Cosi' non esiste un ramo che funziona nei test e uno che funziona in
  // servizio — che era il reperto di `dipendenze-collegate`, e la classe che questo repo conta
  // quattro volte («dep non cablata ⇒ valore di difetto che nessuno ha chiesto», §5.3).
  const statoUscita = (() => {
    try { return (deps.statoUscita || leggiStatoUscita)(o.orderId); }
    catch (e) {
      return { noto: false, uscita: null, prezzoDeciso: null, motivo: `registro non consultabile: ${e.message}` };
    }
  })();
  // «E' un'uscita» oppure «non lo so»: in entrambi i casi il prezzo non si muove. Solo un `noto:true`
  // con `uscita:false` autorizza lo spostamento — cioe' si agisce sulla PROVA che non sia un'uscita,
  // mai sulla sua assenza.
  const prezzoBloccato = !(statoUscita && statoUscita.noto === true && statoUscita.uscita === false);
  const prezzoDecisoScala = statoUscita && Number.isFinite(statoUscita.prezzoDeciso) ? statoUscita.prezzoDeciso : null;
  ...
  if (inBand(price, scoringMid, maxSpreadCents)) {
    ...
    if (inseguimentoDovuto && !railInseguimento) {
      ...
      } else if (prezzoBloccato) {
        // ⚠ L'INSEGUIMENTO DEL MID E' UN AGGIUSTAMENTO COME GLI ALTRI, e su un'uscita non si applica:
        // il prezzo l'ha deciso la scala d'urgenza, non la distanza dal mid. Sta PRIMA del pavimento
        // qui sotto perche' e' piu' forte — il pavimento ferma solo la discesa, questo ferma anche la
        // salita, e su un'uscita anche salire e' riscrivere una decisione che non era di questo modulo.
        noInseguire = { action: 'skip', gate: 'uscita-prezzo-vincolante',
          reason: statoUscita && statoUscita.noto === true
            ? `ordine di USCITA (prezzo deciso dalla scala ${prezzoDecisoScala != null ? prezzoDecisoScala : price}): `
              + `l'inseguimento proporrebbe ${want} ma il prezzo deciso e' vincolante — non si tocca`
            : `non si sa se questo ordine sia un'uscita (${statoUscita && statoUscita.motivo}): `
              + 'fail-closed, non si sposta un ordine che potrebbe essere un\'uscita',
          extra: { targetPrice: want, uscita: statoUscita } };
  ...
  // ══ UN'USCITA NON SI SPOSTA DALLA BANDA — 24 agosto 2026 ══════════════════════════════════════
  // ⚠⚠ SI BLOCCA IL PREZZO, NON SI ESCE DALLA FUNZIONE, ed e' tutta la differenza. Un `return` qui
  // toglierebbe a questi ordini anche il RINNOVO GTD (`forcedByExpiry`), cioe' li farebbe morire per
  // scadenza in 23 minuti: si curerebbe uno spostamento uccidendo l'ordine, che e' il guasto del
  // 18 agosto rifatto con altre mani. Portando `targetPrice` al prezzo attuale, invece:
  //   · senza scadenza imminente il ramo `target-unchanged` piu' sotto risponde `hold` — nessun
  //     movimento, che e' cio' che si vuole;
  //   · con la scadenza imminente si cade nel `reprice` finale ALLO STESSO PREZZO — cioe' esattamente
  //     il rinnovo GTD, che non e' toccato da questa regola e non doveva esserlo.
  // ⚠ E IL PAVIMENTO SI CONFRONTA CON LA SCALA, NON CON L'ORDINE VIVO. `close-sell-floor` qui sotto
  // difende `price`, il prezzo dell'ordine che c'e': su `0x4d79d306` ha difeso 6.337 volte un ordine
  // a 28,8¢ contro un pavimento di scala di 49,5¢ — un pavimento che protegge un prezzo gia' sceso
  // sotto di se' non e' un pavimento, e' un'eco. Il confronto vero e' con `prezzoDecisoScala`, e
  // quando l'ordine vivo e' sotto lo si DICHIARA: non lo si sposta (spostarlo e' cancella-e-ripiazza,
  // e la cura di un'uscita sbagliata non e' lasciare la gamba nuda), ma smette di essere invisibile.
  let uscitaSottoIlPavimento = null;
  if (prezzoBloccato) {
    if (o.side === 'SELL' && prezzoDecisoScala != null && price < prezzoDecisoScala - tick / 1000) {
      uscitaSottoIlPavimento = {
        prezzoVivo: price, prezzoDecisoScala,
        deltaCents: +((price - prezzoDecisoScala) * 100).toFixed(4),
      };
    }
    targetPrice = price;

    // ⚠ E SI RISPONDE QUI, PRIMA DEL GUARD CONDIVISO. Il `validateQuote` piu' sotto rifiuterebbe la
    // «sostituzione» con `OUT_OF_BAND` — verissimo e fuori tema: quel prezzo E' fuori banda **per
    // decisione della scala**, ed e' la stessa deroga che §4.6 gia' dichiara sull'uscita. Lasciar
    // rispondere quel ramo darebbe `replacement-invalid`, cioe' il comportamento giusto col nome
    // sbagliato — e un gate che si chiama come un guasto e' un gate che nessuno riesce a contare.
    if (!forcedByExpiry) {
      return out('hold', 'uscita-prezzo-vincolante',
        `ordine di USCITA fuori banda di ${distanceC.toFixed(2)}¢ (banda ±${bandRadiusC.toFixed(2)}¢): `
        + 'il prezzo l\'ha deciso la scala d\'urgenza e band-exit NON lo sposta. Fuori banda non matura '
        + 'premi, ed e\' la meta\' cara del compromesso: l\'altra sarebbe una coppia oltre il tetto della '
        + 'coppia (misurato il 24/08: 0.516 → 0.717, coppia 121,1¢). '
        + (statoUscita && statoUscita.noto === true ? '' : `⚠ E non si sa nemmeno se lo sia: ${statoUscita && statoUscita.motivo} — fail-closed. `)
        + (uscitaSottoIlPavimento
          ? `⚠ E QUESTO ORDINE STA SOTTO IL PAVIMENTO DELLA SCALA: vivo a ${uscitaSottoIlPavimento.prezzoVivo}, `
            + `deciso ${uscitaSottoIlPavimento.prezzoDecisoScala} (${uscitaSottoIlPavimento.deltaCents}¢). `
            + 'Non lo si sposta — cancellarlo lascerebbe la gamba nuda — ma non e\' piu\' invisibile.'
          : ''),
        { ...withTtl, targetPrice, breachConfirmed: true, uscita: statoUscita, uscitaSottoIlPavimento });
    }
    // ⚠⚠ E QUANDO LA GTD STA PER SCADERE SI RINNOVA, ALLO STESSO PREZZO. Senza questo ramo la regola
    // nuova ucciderebbe per scadenza ogni uscita fuori banda in 23 minuti: si sarebbe curato uno
    // spostamento togliendo l'ordine, che e' il guasto del 18 agosto rifatto con altre mani. Il
    // rinnovo GTD non e' toccato da questa regola, e non doveva esserlo.
    return out('reprice', 'uscita-rinnovo-gtd',
      `ordine di USCITA fuori banda di ${distanceC.toFixed(2)}¢ con solo ${ttlLeft}s di vita al venue: `
      + `si RINNOVA allo stesso prezzo ${targetPrice} — il prezzo deciso dalla scala non si muove, `
      + 'ma l\'ordine non deve morire per scadenza.',
      { ...withTtl, targetPrice, breachConfirmed: true, uscita: statoUscita, uscitaSottoIlPavimento });
  }
...
async function runAutoRepriceCycle(deps = {}) {
  ...
  // ── LA QUARTA COMPONENTE: LO SNAPSHOT SU DISCO DEGLI ORDINI A RIPOSO — 18 agosto 2026 ────────────
  // La (3) qui sopra e' una MEMORIA DI PROCESSO, e il commento la dichiarava con un limite solo («un
  // mercato uscito dallo scope mentre il processo era giu' non rientra»). Il limite vero era piu'
  // grave, e il 18 agosto e' costato 52 minuti fuori dal libro a bot armato: quella memoria si
  // sovrascrive INTERA a ogni giro (agent40) ma si popola solo nei giri che superano quattro cancelli
  // — e `cadenza-adattiva` fa `continue` PRIMA che `owned` sia calcolato, su un mercato «lento» quasi
  // ogni giro. Un solo giro saltato per cadenza cancella il mercato dalla memoria; al giro dopo non e'
  // piu' nello scope, quindi non verra' piu' guardato, quindi non tornera' mai in memoria. La
  // correzione non poteva stare dentro la memoria: doveva diventare durevole e FONDERSI per mercato.
  //
  // Arriva da `cfgState`, cioe' dalla STESSA lettura di configurazione che questo ciclo fa gia', e non
  // da un secondo `require` qui dentro: il perimetro ha una definizione sola (§4.8) e questa ne e' una
  // conseguenza, non una copia.
  for (const id of (Array.isArray(cfgState.enabledDaOrdini) ? cfgState.enabledDaOrdini : [])) {
    aggiungiAlloScope(id, 'ordini-a-riposo-snapshot');
  }
  ...
  // ⚠ INSIEME DIVERSO, E LA DIFFERENZA E' COSTATA UN GIRO. `mercatiConOrdiniQuestoGiro` nasce da
  // `owned`, cioe' dagli ordini che QUESTO CICLO GESTISCE — e `owned` esclude i **pre-esistenti**
  // (riga ~1347: gli ordini gia' a riposo quando il processo si e' avviato sono invisibili da li' in
  // poi, di proposito). Va benissimo per decidere cosa riprezzare; e' la domanda SBAGLIATA per il
  // perimetro.
  //
  // Il perimetro chiede: «questo mercato ha il NOSTRO CAPITALE a libro?». Dopo ogni riavvio di agent40
  // gli ordini vivi diventano pre-esistenti, quindi `owned` e' vuoto e lo snapshot li avrebbe
  // dichiarati inesistenti — misurato il 18 agosto alle 20:29: il venue rispondeva `count 2` e lo
  // snapshot scriveva `{}` cinque millisecondi dopo. Con la fusione «guardato e vuoto ⇒ la voce esce»
  // quella lettura sbagliata TOGLIE il mercato dal perimetro: esattamente il guasto che lo snapshot
  // esiste per impedire, riprodotto da un riavvio.
  const mercatiConNostriOrdini = new Set();
  ...
  for (const marketId of scopeRinnovo) {
    ...
    // Il perimetro guarda TUTTI i nostri ordini, pre-esistenti compresi: sono capitale a libro anche
    // quando questo ciclo ha deciso di non gestirli.
    if (tuttiNostri.length > 0) mercatiConNostriOrdini.add(String(marketId).trim().toLowerCase());
    for (const o of owned) {
    ...
    // Chi ha DECISO che era cecita' (gate di assenza · resnapshot richiesto · feed non vivo), e quante
    // volte invece il silenzio e' stato tollerato perche' il feed era vivo. Entrambi nel referto: senza,
    // «non ho cancellato» e «non ho guardato» sarebbero la stessa riga.
    const decisoriCecita = new Map();
    const silenziTollerati = new Map();
    ...
    for (const order of owned) {
      ...
      const d = decideReprice(
        ...
        // ⚠ `statoUscita` SI PASSA ESPLICITAMENTE, e non si lascia al default. Il default esiste per i
        // banchi e per i selfcheck; il percorso vivo dichiara cio' che usa, o «chi legge il registro
        // delle uscite» sarebbe una domanda a cui si risponde leggendo l'implementazione.
        { resolveOffset: deps.resolveOffset, offsetDeps: deps.offsetDeps, resolveDepth: deps.resolveDepth,
          statoUscita: deps.statoUscita || leggiStatoUscita },
      );
      ...
      if (d.action !== 'hold' && d.action !== 'skip' && typeof deps.valutaMercato === 'function') {
        ...
        try {
          ...
          // ── IL PREZZO CHE PARTIREBBE DAVVERO, CALCOLATO UNA VOLTA SOLA ─────────────────────
          // Lo leggono DUE regole: il tetto per mercato (`proposedPrice`) e la prova di rinnovo
          // (condizione ③, «il nozionale non aumenta»). Erano due espressioni diverse — `d.targetPrice
          // ?? order.price` per il tetto, `order.price` secco per la prova — cioe' due risposte alla
          // stessa domanda: un inseguimento che alza il prezzo si sarebbe dichiarato «rinnovo» sul
          // prezzo VECCHIO e sarebbe passato con piu' nozionale a riposo. Un numero solo, e la
          // divergenza non e' piu' esprimibile.
          ...
          const v = !espo.leggibile ? { ok: false, controlli: {}, motivo: `tetto-mercato: ${espo.motivo}` } : deps.valutaMercato({
            ...
            // ⚠ E IL PREZZO DI RIFERIMENTO VA NELLO SPAZIO DEL MOTORE. `scalaPerIlMotore` SPECCHIA i
            // prezzi su una gamba SELL (`p → 1−p`) perche' il motore ragiona sempre come un BUY;
            // `prezzoMassimo` nasce invece dagli ordini VERI, non specchiati. Passarlo com'e' su una
            // SELL confronterebbe 0,127 con una scala che vive intorno a 0,873 — cioe' scarterebbe
            // ogni livello, e l'esenzione non si applicherebbe mai proprio dove serve. Si specchia
            // con la STESSA funzione che ha specchiato la scala, non con una seconda.
            // ⚠ Le condizioni ② e ③ (size e nozionale) restano nello spazio VERO, dentro
            // `provaRinnovo`: sono un conto di esposizione in dollari, e non si specchiano.
            rinnovo: (() => {
              try {
                const prova = RINNOVO.provaRinnovo({
                  conditionId: marketId, tokenId: order.tokenId, side: order.side,
                  // ⚠ IL PREZZO E' QUELLO CHE PARTE, NON QUELLO CHE C'E' GIA'. Con `order.price` la
                  // condizione ③ si sarebbe confrontata con se stessa — sempre vera — e un
                  // inseguimento verso il mid avrebbe ottenuto l'esenzione mentre ALZAVA il nozionale
                  // a riposo. E' la stessa espressione del tetto per mercato, non una seconda.
                  size: order.size, price: prezzoCheParte, ordiniVivi: owned,
                });
                if (!prova || prova.esente !== true || String(order.side).toUpperCase() !== 'SELL') return prova;
                return Number.isFinite(prova.prezzoMassimo)
                  ? { ...prova, prezzoMassimo: specchiaPrezzo(prova.prezzoMassimo) }
                  : prova;
              } catch { return null; }
            })(),
          ...
          // L'esenzione di rinnovo, accanto al pavimento che esenta: senza questo campo un pavimento
          // che non morde e uno che non e' stato applicato sono la stessa riga nel giornale.
          d.rinnovoEsente = c.rinnovo ? c.rinnovo.esente === true : null;
          d.rinnovoSostituisce = c.rinnovo ? c.rinnovo.sostituisce : null;
      ...
      if (d.action === 'skip') {
        ...
        // solo tipo di cecità.
        // ⚠⚠ E «IL MERCATO TACE» NON AVVIA PIU' L'OROLOGIO — 18 agosto 2026, decisione dell'operatore.
        // `mid-stale` significa «nessun evento su questo asset da 30 s», che su un mercato tranquillo e'
        // lo stato normale: il quadro memorizzato resta perfetto. Cancellare li' toglieva gli ordini
        // proprio dai mercati che un maker di rewards vuole. La cecita' vera si giudica sul FEED nel suo
        // insieme (`feedVitality`), non sul silenzio di un singolo libro — e la decisione, con quale
        // delle due condizioni l'ha presa, viaggia nel referto.
        const cv = cecitaVera({
          gate: d.gate,
          // ⚠ `config`, NON `cfg`: e' il nome della manopola in questo file (riga 1010), e sbagliarlo
          // qui era la terza volta oggi che ci cascavo. Con `cfg` non definito il ciclo esplodeva.
          feedVivo: regimeFeed(rules.feedVitality, config).regime === 'vivo',
          needsResnapshot: rules.needsResnapshot === true,
        });
        if (cv.cieco) {
          ordiniCiechi += 1;
          if (cv.causa) causeCieche.set(cv.causa, (causeCieche.get(cv.causa) || 0) + 1);
          decisoriCecita.set(cv.decisoDa, (decisoriCecita.get(cv.decisoDa) || 0) + 1);
        } else if (cv.decisoDa) {
          // Il silenzio tollerato si CONTA e si dichiara: senza, «non ho cancellato» sarebbe
          // indistinguibile da «non ho guardato», che e' il silenzio che e' costato tutta la serata.
          silenziTollerati.set(marketId, (silenziTollerati.get(marketId) || 0) + 1);
        }
        ...
        if (d.gate !== 'awaiting-confirmation') {
          audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice', outcome: `skip-${d.gate}`, marketRef: `cid_${String(marketId).replace(/^0x/, '')}`, orderId: order.orderId, gate: d.gate, reason: d.reason, observed: { price: order.price, scoringMid: d.scoringMid, distanceC: d.distanceC, bandRadiusC: d.bandRadiusC,
            ...
            rinnovoEsente: d.rinnovoEsente != null ? d.rinnovoEsente : null,
            rinnovoSostituisce: d.rinnovoSostituisce || null,
    ...
    if (owned.length > 0) {
      ...
      if (!cieco) {
        ...
        if (silenziTollerati.size) m.silenziTollerati = Object.fromEntries(silenziTollerati);
        ...
        // ⚠ E QUALE CONDIZIONE HA DECISO che era cecita' e non silenzio: `gate-di-assenza` (non c'e' un
        // libro, o non se ne puo' datare il contenuto), `resnapshot-richiesto` (agent34 ha perso
        // l'ancoraggio), `feed-non-vivo` (la vitalita' complessiva non regge, o non e' leggibile).
        // Senza questa riga «ho cancellato» non direbbe PERCHE', e la serata del 18 agosto e' finita
        // per non aver distinto «il mercato tace» da «siamo ciechi».
        m.cecita = { causa, per: Object.fromEntries(causeCieche),
          decisoDa: Object.fromEntries(decisoriCecita) };
        audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice',
  ...
  // Le morti di QUESTO giro, per il referto aggregato in fondo. Non e' un secondo elenco: `events`
  // le contiene gia' insieme a tutto il resto, e filtrarlo per tipo qui sotto sarebbe una seconda
  // definizione di «morte per scadenza» accanto a quella che le produce.
  const morti = [];
  ...
  // ── UN CICLO CHE PERDE ORDINI PER GTD LO DICHIARA COME ANOMALIA, COL NUMERO E COL NOZIONALE ──────
  //
  // PERCHE' NON BASTAVANO LE RIGHE PER ORDINE. `scaduto-senza-rinnovo` c'era gia', una riga per
  // ordine, e in un giornale che ne scrive centinaia all'ora una morte in piu' e' indistinguibile da
  // una riga di routine — esattamente il motivo per cui i 63 ordini morti fra le 06:13Z e le 13:18Z
  // del 23 agosto, $862,58 usciti dal libro, sono stati visti solo contandoli a posteriori con un
  // grep. Il degrado non era silenzioso per assenza di righe: era silenzioso perche' nessuna riga
  // diceva QUANTO.
  //
  // ⚠ E' UN REFERTO, NON UN GATE: non ferma niente, non cancella niente, non tocca nessun ordine.
  // Arriva dopo la morte, come `scaduto-senza-rinnovo`; `anomalia-rinnovo-fermato` resta la diagnosi
  // che arriva PRIMA, mentre restano secondi per accorgersene. Le tre non si sostituiscono.
  //
  // ⚠ SI SCRIVE SOLO SE QUALCUNO E' MORTO IN QUESTO GIRO. Un giro sano non aggiunge una riga: un
  // avviso che compare sempre non e' un avviso.
  //
  // ⚠ IL NOZIONALE E' UNA SOMMA DI CIO' CHE SI E' POTUTO MISURARE, e chi non si e' potuto misurare si
  // conta a parte (`senzaNozionale`) invece di entrare come zero. «Non ho letto» non e' «non c'e'».
  if (morti.length) {
    const conNoz = morti.filter((e) => Number.isFinite(e.notionalUsd));
    const nozionaleUsd = +conNoz.reduce((acc, e) => acc + e.notionalUsd, 0).toFixed(2);
    const perGate = {};
    for (const e of morti) {
      const g = e.bloccoGate || '(rinnovo mai valutato)';
      perGate[g] = (perGate[g] || 0) + 1;
    }
    const mercati = [...new Set(morti.map((e) => String(e.marketId)))];
    audit({ ts: t0, venue: 'polymarket', source: AUTO_REPRICE_SOURCE, op: 'auto-reprice',
      outcome: 'anomalia-scadenze-senza-rinnovo', gate: null, anomalia: true, corsia: 'rinnovo',
      reason: `ANOMALIA: ${morti.length} ordine${morti.length === 1 ? '' : 'i'} morto per GTD senza rinnovo in questo ciclo`
        + ` · $${nozionaleUsd.toFixed(2)} usciti dal libro su ${mercati.length} mercat${mercati.length === 1 ? 'o' : 'i'}`
        + ` · per gate: ${Object.entries(perGate).map(([g, n]) => `${g}×${n}`).join(', ')}`
        + (conNoz.length === morti.length ? '' : ` · ${morti.length - conNoz.length} senza nozionale misurabile`)
        + ' — quel capitale non matura piu\' premi e va rimesso in gioco.',
      observed: {
        ordini: morti.length, nozionaleUsd, senzaNozionale: morti.length - conNoz.length,
        mercati: mercati.length, perGate,
        orderIds: morti.map((e) => e.orderId),
      } });
  }
  ...
  return { at: new Date(t0).toISOString(), ran: true, gate: null, reason: null, markets, actions, events,
    ...
    // ⚠ I DUE INSIEMI VIAGGIANO INSIEME PERCHE' LO SNAPSHOT SU DISCO HA BISOGNO DI ENTRAMBI.
    // `mercatiConOrdini` da solo non basta a fondere: senza sapere COSA e' stato guardato, un mercato
    // assente da questa lista e' indistinguibile fra «guardato e vuoto» (una lettura, che deve
    // togliere la voce) e «non guardato» (un silenzio, che deve conservarla). E' esattamente la
    // distinzione che il 18 agosto e' costata 52 minuti fuori dal libro. `mercatiLetti` nasce alla
    // riga 1370, cioe' DOPO tutti i cancelli e accanto al calcolo di `owned`: e' la lista dei mercati
    // per cui il venue ha davvero risposto, ed e' gia' la stessa che il confronto di riga 2345 usa
    // per non giudicare cio' che non ha letto.
    mercatiLetti: [...mercatiLetti],
    // ⚠ QUESTO e' l'insieme che alimenta lo SNAPSHOT DEL PERIMETRO, non `mercatiConOrdini`: include i
    // pre-esistenti, perche' «c'e' il nostro capitale a libro» non dipende da quale processo li ha
    // piazzati ne' da quando e' stato riavviato.
    mercatiConNostriOrdini: [...mercatiConNostriOrdini],
    scope: {
...
// `scalaPerIlMotore` e' esportata perche' la simulazione a secco del rinnovo deve usare LA funzione
// che costruisce la scala e la banda per il motore, non una copia: ricopiarla in uno script di
// ricerca sarebbe il reperto D1 dentro la misura con cui si decide. Non ha effetti: e' un export.
module.exports = { decideReprice, selectOwnedOrders, esposizioneDelMercato, runAutoRepriceCycle, regimeFeed, scalaPerIlMotore, AUTO_REPRICE_SOURCE };

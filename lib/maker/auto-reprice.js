...
async function runAutoRepriceCycle(deps = {}) {
  ...
  for (const marketId of cfgState.enabledMarketIds) {
    ...
    for (const order of owned) {
      ...
      if (d.action !== 'hold' && d.action !== 'skip' && typeof deps.valutaMercato === 'function') {
        let veto = null;
        try {
          const v = deps.valutaMercato({
            marketId, side: order.side || 'BUY',
            ...
            liquiditaMediaShare: typeof deps.liquiditaMedia === 'function' ? deps.liquiditaMedia(marketId).media : null,
            liquiditaCampioni: typeof deps.liquiditaMedia === 'function' ? deps.liquiditaMedia(marketId).campioni : 0,
            now: t0,
          });
  ...
  return { at: new Date(t0).toISOString(), ran: true, gate: null, reason: null, markets, actions, events,
    cancellazioni, latiSingoli, latencyMs: now() - t0 };
}

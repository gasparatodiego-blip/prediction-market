...
function leggiCoppiaDetenuta(tokenId, tokenIdOpposto, deps = {}) {
  const vuoto = (motivo) => ({ leggibile: false, held: null, heldOpposto: null, carico: null, caricoOpposto: null, motivo });
  try {
    ...
    const trova = (want) => {
      const w = String(want || '');
      if (!w) return null;
      return (snap.positions || []).find((p) => String((p && (p.tokenId ?? p.asset)) || '') === w) || null;
    };
    const cerca = (want) => {
      const hit = trova(want);
      if (!hit) return null;
      const n = Number(hit.size);
      return fin(n) ? Math.abs(n) : null;
    };
    // ⚠ IL CARICO, DALLO STESSO IDENTICO SNAPSHOT — 24 agosto 2026, additivo.
    // Serve al controllo terminale sul tetto della coppia (`ordini-di-uscita.verdettoTettoCoppia`), che
    // deve ricalcolare `carico + prezzo di invio` un istante prima della POST. Si legge QUI e non in un
    // secondo lettore perche' questa e' gia' la funzione che risponde alle domande sulla coppia detenuta:
    // due letture dello stesso file darebbero due idee dello stesso numero (reperto D1).
    // ⚠ NESSUN CHIAMANTE ESISTENTE CAMBIA: i due campi si AGGIUNGONO, non sostituiscono niente.
    const carico = (want) => {
      const hit = trova(want);
      if (!hit) return null;
      const n = Number(hit.avgPrice);
      return fin(n) && n > 0 ? n : null;
    };
    return { leggibile: true, held: cerca(tokenId), heldOpposto: cerca(tokenIdOpposto),
      carico: carico(tokenId), caricoOpposto: carico(tokenIdOpposto), motivo: null };
  } catch (e) {

...
(async () => {
  ...
  console.log('\n══ 2 · LA REGOLA 5 CON INGRESSI VERI');
  {
    ...
    // $100 di saldo ⇒ tetto $20, e l'ordine ne vale ~$39.
    const sopra = await giro({ saldo: { usd: 100, affidabile: true, fonte: 'cache', etaMs: 0 } });
    const rSopra = skipDelMotore(sopra.righe);
    ok('sopra il tetto la Regola 5 BLOCCA', !!rSopra && /oltre il 20%/.test(rSopra.reason || ''), rSopra && rSopra.reason);
    ok('  e l audit porta il saldo che l ha deciso', rSopra && rSopra.observed && rSopra.observed.saldoUsd === 100,

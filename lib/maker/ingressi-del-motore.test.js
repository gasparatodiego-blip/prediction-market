...
(async () => {
  ...
  console.log('\n══ 2 · LA REGOLA 5 CON INGRESSI VERI');
  {
    ...
    const ricco = await giro({ saldo: { usd: 5000, affidabile: true, fonte: 'cache', etaMs: 0 },
      posizioni: { leggibile: true, usd: require('../rewards/concentration').capPerMarketUsd(5000), motivo: null } });
    ...
    // ⚠ Col tetto DERIVATO, a $5.000 il tetto non «resta»: cresce, perche' oltre MAX_MERCATI e' la size
    // a salire. Il banco difende comunque la stessa proprieta' — a capitale abbondante morde IL TETTO,
    // non una frazione del saldo — ma il numero si prende da `capPerMarketUsd(5000)`, non dalla costante.
    ok(`  con $5.000 in cassa morde il tetto ($${require('../rewards/concentration').capPerMarketUsd(5000)}): un mercato già al tetto non ne prende altro`,
      !!rRicco && rRicco.reason.includes(`$${require('../rewards/concentration').capPerMarketUsd(5000).toFixed(2)}`),
      rRicco && rRicco.reason);

...
const TICK = 0.01, BANDA = 2.25, MINSIZE = 20;
const regole = (mid) => ({
...
console.log('\n══ 4 · NEL CODICE L INSIEME È UNO SOLO, IN TUTTI I PUNTI DI CHIAMATA');
{
  ...
  // ⚠ AGGIORNATO IL 12 AGOSTO 2026: il filtro a mano e' diventato la funzione condivisa. «Uno solo e
  // con un nome» resta la proprieta' difesa; in piu' ora quel nome lo produce lo STESSO codice che usa
  // il percorso di piazzamento, quindi la simmetria non e' piu' una coincidenza da verificare a vista.
  ok('è calcolato una volta e ha un nome',
    /const nostriSulLato = nostriOrdiniSulLato\(\{ orders: owned, book: order\.book \}\)\.ordini;/.test(src));
  ok('  e viene dalla funzione condivisa, non da un filtro scritto qui',
    src.includes("require('./nostri-ordini')") && !/const nostriSulLato = owned\.filter/.test(src));

...
//
// Il timeout di mid stantio vale 120 secondi dal 16 agosto 2026 (`TIMEOUT_DEFAULT_MS`, env
// `MAKER_MID_STANTIO_TIMEOUT_MS` con clamp [5 s, 120 s]). Era 20 s, e cancellava a 20 s cio' che
// `decideReprice` non era disposto a riprezzare prima di 60: due orologi che si contraddicevano.
// ⚠ QUI IL NUMERO NON SI RIPETE: il valore lo asserisce `mid-stantio.test.js` in un punto solo, e
// questo test lo IMPORTA e ci deriva sopra i confini. Ripeterlo era il motivo per cui una decisione
// sola rendeva rossi due file (CLAUDE.md §5.3).
//
...
{
  const T = MS.TIMEOUT_DEFAULT_MS;   // ← importato, mai ricopiato
  ok(`il difetto è leggibile e sensato (${T / 1000} s)`, Number.isFinite(T) && T >= 5_000 && T <= 120_000, String(T));
  ...
  ok('  con clamp [5 s, 120 s]: sotto e sopra si torna al difetto',
    MS.timeoutMs({ MAKER_MID_STANTIO_TIMEOUT_MS: '1000' }) === T
    && MS.timeoutMs({ MAKER_MID_STANTIO_TIMEOUT_MS: '999999' }) === T);
  ok('  e un valore illeggibile non spegne la protezione', MS.timeoutMs({ MAKER_MID_STANTIO_TIMEOUT_MS: 'x' }) === T);

  const ORA = 1_800_000_000_000;
  ok('sotto il timeout si aspetta', MS.decidiStantio({ stantio: true, daMs: ORA - (T - 1_000), now: ORA }).azione === 'attendi');
  ok('al timeout si cancella', MS.decidiStantio({ stantio: true, daMs: ORA - T, now: ORA }).azione === 'cancella');

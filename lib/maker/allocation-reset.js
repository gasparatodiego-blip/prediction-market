...
async function runAllocationReset({ rows = [], userId = OPERATOR_USER, dryRunOnly = false } = {}, deps = {}) {
  ...
  if (dryRunOnly) {
    ...
    return referto(null, null, {
      ...
      cancellazione: { daCancellare, cancellati: 0, falliti: 0, simulata: true },
  ...
  if (falliti.length) {
    ...
    return referto('cancel-failed',
      ...
      { inventario: { abilitatiPrima, trackingPrima, nelPiano, gestiti }, cancellazione: { daCancellare, cancellati, falliti } });
  }
  ...
  for (const marketId of nelPiano) {
    ...
    // La proprietà manuale accompagna l'abilitazione, come fa già il percorso a mano del pannello:
    // senza di essa il piazzamento verrebbe rifiutato al gate della proprietà.
    let mn = null;
    if (en && en.ok && typeof deps.setManual === 'function') {
    ...
    const acOk = !!(ac && ac.ok);
    traccia('accensione', (en && en.ok && acOk) ? 'abilitato' : 'abilitazione-fallita',
      { marketId, manual: !!(mn && mn.ok), uscitaAutomatica: acOk, error: (en && en.error) || (ac && ac.error) || null });
    accesi.push({
      marketId, ok: !!(en && en.ok) && acOk, manual: !!(mn && mn.ok), uscitaAutomatica: acOk,
      error: (en && en.error) || (ac && ac.error) || null,
    });
  }
  ...
  if (nonAccesi.length) {
    ...
    return referto('enable-failed',
      `${nonAccesi.length} mercato/i del piano non è stato possibile abilitarlo (allowlist o uscita automatica): nessun ordine viene piazzato.`
      + ' Un mercato con due gambe vive e nessuna via d\'uscita è peggio di un mercato in meno.'
      + ' Il libro è già stato liberato, quindi il sistema resta fermo e senza esposizione.',
      { inventario: { abilitatiPrima, trackingPrima, nelPiano, gestiti },
        cancellazione: { daCancellare, cancellati, falliti },

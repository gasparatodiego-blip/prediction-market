...
//   3. ACCENDE   esattamente i mercati del piano.
...
async function runAllocationReset({ rows = [], userId = OPERATOR_USER, dryRunOnly = false } = {}, deps = {}) {
  ...
  if (dryRunOnly) {
    ...
    return referto(null, null, {
      ...
      accensione: { markets: nelPiano, simulato: true },
      piazzamento,
    });
  }
  ...
  for (const marketId of nelPiano) {
    ...
    traccia('accensione', (en && en.ok) ? 'abilitato' : 'abilitazione-fallita', { marketId, manual: !!(mn && mn.ok), error: (en && en.error) || null });
    accesi.push({ marketId, ok: !!(en && en.ok), manual: !!(mn && mn.ok), error: (en && en.error) || null });
  }
  ...
  if (nonAccesi.length) {
    ...
    return referto('enable-failed',
      `${nonAccesi.length} mercato/i del piano non è stato possibile abilitarlo: nessun ordine viene piazzato.`
      ...
      { inventario: { abilitatiPrima, trackingPrima, nelPiano, gestiti },
        ...
        spegnimento: { tracking: spentiTracking, abilitati: spentiAbilitati },

...
async function runAllocationReset({ rows = [], userId = OPERATOR_USER, dryRunOnly = false } = {}, deps = {}) {
  ...
  if (typeof deps.leggiOrigini === 'function') {
    ...
    if (risparmiati.length) {
      traccia('cancellazione', 'risparmiati', {
        ...
        // Quante erano USCITE PROTETTIVE, contate a parte: «il reset ha lasciato 4 righe» e «il reset
        // ha risparmiato 4 vie d'uscita di posizioni aperte» sono due fatti diversi, e il secondo e'
        // una DECISIONE presa il 12 agosto 2026, non piu' l'effetto di una stringa sbagliata.
        usciteProtette: Array.isArray(sep.protetti) ? sep.protetti.length : 0,
  ...
  for (const marketId of nelPiano) {
    ...
    // ── LA QUARTA SCRITTURA: LA COPIA DI SICUREZZA DELLE REGOLE DI VENUE ──────────────────────────
    // Aggiunta il 9 agosto 2026, e questo percorso era il piu' importante dei due: il mini-ciclo apre
    // qualche mercato ogni tanto, il ciclo delle sei ore apre TUTTO IL PIANO. Fino a oggi nessuno dei
    // mercati che questa fase accende riceveva una copia delle proprie regole.
    //
    // Perche' serve: un mercato in gestione vive sulle regole del board reward, e dal board esce per
    // DUE motivi distinti che portano allo stesso guasto —
    //   · ROTAZIONE: agent24 riscrive il board tenendo i primi 120 per montepremi;
    //   · SCADENZA: il mercato si avvicina alla risoluzione e cade fuori dalla finestra di scoperta.
    // In entrambi i casi `resolveMarketRules` perde tick, banda, minSize e negRisk, e con essi si
    // fermano chiusura automatica, riprezzatura, tracking e qualunque ordine. La posizione resta senza
    // via d'uscita proprio nei giorni in cui piu' probabilmente ne ha bisogno.
    //
    // Il secondo motivo e' la ragione per cui questa riga sta QUI e non solo nel mini-ciclo: un mercato
    // che il piano lascia perche' `in-scadenza` e' esattamente un mercato che sta per risolvere, cioe'
    // uno su cui una posizione aperta va gestita fino alla fine. Il 9 agosto alle 03:41 il reset ne ha
    // lasciati dieci in una volta sola, tutti `in-scadenza`.
    //
    // NON E' UN FERMO DURO, a differenza delle tre sopra. Quelle decidono se il mercato e' operabile
    // ADESSO — senza, ogni gamba muore a un gate. Questa decide se sara' gestibile DOPO: rinunciare a
    // piazzare per una copia di sicurezza mancata scambierebbe un danno certo per uno possibile.
    let cat = null;
    if (en && en.ok) {
      if (typeof deps.registraCatalogo !== 'function') {
        cat = { ok: false, error: 'nessuna funzione registraCatalogo iniettata' };
      } else {
        try { cat = await deps.registraCatalogo({ marketId }); }
        catch (e) { cat = { ok: false, error: e && e.message ? e.message : String(e) }; }
      }
    }
    const catOk = !!(cat && cat.ok);

    traccia('accensione', (en && en.ok && acOk && mnOk) ? 'abilitato' : 'abilitazione-fallita',
      { marketId, manual: mnOk, uscitaAutomatica: acOk, ripiegoRegole: catOk,
        ripiegoMotivo: catOk ? null : ((cat && cat.error) || null),
        error: (en && en.error) || (ac && ac.error) || (mn && mn.error) || null });
    accesi.push({
      marketId, ok: !!(en && en.ok) && acOk && mnOk, manual: mnOk, uscitaAutomatica: acOk,
      ripiegoRegole: catOk, ripiegoMotivo: catOk ? null : ((cat && cat.error) || null),
      error: (en && en.error) || (ac && ac.error) || (mn && mn.error) || null,
    });
  }

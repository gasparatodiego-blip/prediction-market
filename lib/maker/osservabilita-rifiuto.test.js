...
(async () => {
  ...
  console.log('\n══ 4 · ANCHE L\'AUDIT LI RICEVE, non solo il chiamante');
  {
    ...
    // La proprietà è «`...extra` viene per ULTIMO», non l'elenco dei campi che lo precedono: quello
    // cresce (poi è arrivato `origine`) e un test legato alla sua grafia diventa rosso a ogni aggiunta.
    ok('  con `extra` che può ancora sovrascriverli',
      /priceAdjusted,[^}]*\.\.\.extra \};/.test(corpo),

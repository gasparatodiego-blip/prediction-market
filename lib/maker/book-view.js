...
function midCoherence({ mid, midKind, scoringMid, bestBid, bestAsk, minSize, spreadCents }) {
  ...
  //
  // APPROVATA DA DIEGO IL 2026-08-02. E' l'unica soglia di questo lavoro scelta a tavolino invece che
  // derivata da una fonte: 0-6¢/passo 0.25/default 2¢ vengono dalla richiesta, i 12 livelli da agent34,
  // i 10¢ dello spread ampio dalla documentazione Polymarket. Questa no — quindi porta scritto chi l'ha
  // approvata, perche' cambiarla cambia quando l'operatore viene avvisato e non e' una modifica muta.
...
/**
 * IL VERDETTO SUL PREZZO, in una forma sola per tutti e due i lati.
 *
 * TRE LIVELLI, PERCHÉ I FATTI SONO DI TRE TIPI:
 *   'ok'   — l'ordine resta sul book come maker e dentro la banda premiante.
 *   'warn' — FUORI DALLA BANDA REWARD e nient'altro. L'ordine è valido, il venue lo accetta e resta a
 *            riposo esattamente come chiesto: semplicemente non matura reward. È un COSTO, non un
 *            errore, e l'operatore può volerlo di proposito (stare lontano dal fill, chiudere una
 *            posizione, prendere una quota che il programma premi non paga). Quindi si dice, non si
 *            impedisce — il blocco su questo codice è stato tolto da tutta la corsia manuale.
 *   'bad'  — l'ordine NON farebbe quello che sembra: a quel prezzo incrocia il book, quindi si esegue
 *            subito come taker invece di riposare. Questo resta rosso perché descrive un ordine
 *            DIVERSO da quello che l'operatore crede di piazzare, non un ordine più caro.
 *
 ...
 *
 * @returns {{level:'ok'|'warn'|'bad'|'unknown', crosses:boolean, outOfBand:boolean|null, messages:string[]}}
 */
  ...
  if (fin(scoringMid) && fin(bandRadiusCents) && bandRadiusCents > 0) {
    ...
    if (outOfBand) {
      messages.push(
        ...
        + 'ma è piazzabile: questo è un avviso, non un blocco.',
      );
    }
  }

  // L'INCROCIO È L'UNICA COSA CHE RESTA ROSSA. Fuori banda da solo è giallo: costa i premi, non
  // trasforma l'ordine in qualcos'altro. Quando valgono entrambi vince il rosso, perché il fatto più
  // grave è che l'ordine non riposerebbe affatto.
  if (x.readable && x.crosses) return { level: 'bad', crosses: true, outOfBand, messages };
  if (outOfBand === true) return { level: 'warn', crosses: false, outOfBand, messages };
...
/**
 * La distanza di un livello dal mid, in centesimi. Serve al pannello per DIRLO su ogni riga del book.
 *
 * Qui accanto viveva `levelBlocked`, che a partire da una soglia rendeva inerti le righe piu' vicine al
 * mid. E' stato rimosso con il cursore che lo governava: la distanza dal mid si misura e si mostra, ma
 * non decide piu' quali livelli l'operatore possa toccare.
 */
...
module.exports = {
  ...
  crossesBook, priceVerdict, distanceCents,
};

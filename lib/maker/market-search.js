 ...
 *     is RETURNED, labelled with exactly that, and left to the operator.
 *
 ...
 */
async function searchMarkets({ q = '', limit = 25, includeClosed = false, nowMs = Date.now(), timeoutMs } = {}) {
  const query = typeof q === 'string' ? q.trim() : '';
  const cap = Math.max(1, Math.min(MAX_RESULTS, Number(limit) || 25));
  const empty = { ok: true, error: null, query, count: 0, markets: [], withRewards: 0, withoutRewards: 0, fetchedAt: nowMs };
  ...
  if (isConditionId(query)) {
    ...
    const r = await httpGetJson(`/public-search?q=${encodeURIComponent(query)}&limit_per_type=${cap}${statusQ}`, { timeoutMs });
  ...
  const markets = en.markets
    .filter((m) => includeClosed || !m.closed)
    .slice(0, cap)

...
console.log('\n══ 5 · LA VIA DI RIDUZIONE (SELL) NON È STATA SFIORATA');
{
  ...
  const diff = execSync('git -C ' + path.join(__dirname, '..', '..') + ' diff --stat HEAD -- lib/venues/polymarket-clob-maker/adapter.js', { encoding: 'utf8' });
  ok('e il diff su questo file resta piccolo: una funzione, non una riscrittura del gate',
    /adapter\.js/.test(diff), diff.trim().split('\n').pop());
}

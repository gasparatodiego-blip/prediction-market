...
async function runBulkAllocation({ rows = [], userId = OPERATOR_USER, dryRunOnly = false } = {}, deps = {}) {
  ...
  const rateCap = Number.isFinite(caps.maxOrdersPerWindow) ? caps.maxOrdersPerWindow : null;
  ...
  for (let gi = 0; gi < gruppi.length; gi++) {
    ...
    if (rateCap != null && inviati + gruppo.righe.length > rateCap) {
      for (const { r, index } of gruppo.righe) {
        results.push({ ...rowRef(r), index, status: 'skipped', notionalUsd: +(Number(r.price) * Number(r.size)).toFixed(4),
          reason: `il rate limit del venue (${rateCap} ordini per finestra) non lascia spazio a ${accoppiato ? 'entrambe le gambe' : 'questa riga'}: ${inviati} già inviati in questa finestra` });
      }
      saltaResto(gi + 1, 'sequenza fermata al raggiungimento del rate limit');
      audit({ ts: now(), venue: 'polymarket', source: BULK_SOURCE, op: 'bulk-allocate', outcome: 'stopped-rate',
        userId, atRow: gruppo.righe[0].index, inviati, rateCap });
      return report('rate-limit',
        `fermata al mercato ${gi + 1} di ${gruppi.length}: ${inviati} ordini già inviati nella finestra, ${accoppiato ? 'una coppia ne chiede 2' : 'questa riga ne chiede 1'} e il tetto è ${rateCap}`
        + ' — meglio una coppia non inviata che una coppia spezzata a metà');
    }
    ...
    for (const { r, index } of gruppo.righe) {
      ...
      try {
        res = await place({ marketId: r.marketId, book: r.book, side: r.side === 'SELL' ? 'SELL' : 'BUY',
          price, size, userId, source: BULK_SOURCE, note: `allocazione in blocco, mercato ${gi + 1}/${gruppi.length}${accoppiato ? ` gamba ${r.gamba || r.book}` : ''}` });

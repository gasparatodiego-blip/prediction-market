...
// ⚠ IL TETTO DI QUELLA NOTTE E' UN LETTERALE, E DEVE ESSERLO. Questo blocco RIPRODUCE l'incidente
// del 13 agosto 2026, e un incidente e' successo con i numeri che c'erano allora: leggerlo dalla
// costante viva legava la riproducibilita' di un fatto storico al valore di oggi, e infatti il test
// e' diventato rosso quando il tetto e' passato a $61,25 senza che il difetto del 13 fosse cambiato.
// E' la stessa classe di §5.3, «test che fotografa il codice invece della proprieta'», al contrario:
// qui la proprieta' da difendere E' il numero storico.
const TETTO_QUELLA_NOTTE = 32.67;

ok('il caso vero: capitale $609,10 ⇒ unità $12 ⇒ massimo allocabile $24,00', () => {
  const tetto = TETTO_QUELLA_NOTTE;
  assert.strictEqual(capPerMarketUsd(664.6) >= pavimentoPremiante(MIN_PREMIANTE_TIPICO), true,
    'il tetto di OGGI resta sopra il pavimento premiante: e\' l\'invariante che il deadlock violava');
...
ok('e $24,00 stava SOTTO il pavimento di $24,50: il deadlock, in due numeri', () => {
  ...
  assert.ok(maxAlloc(unitVecchia(609.1), TETTO_QUELLA_NOTTE) < pavimento,
...
ok('NON era un caso isolato: la griglia vecchia è un dente di sega e cade sotto il pavimento a bande', () => {
  const rotti = [609.1, 900, 1000, 1200].filter((c) => {
    return maxAlloc(unitVecchia(c), TETTO_QUELLA_NOTTE) < pavimentoPremiante(MIN_PREMIANTE_TIPICO);
  });

...
function mondo(opts = {}) {
  ...
  const deps = {
    ...
    // L'uscita automatica e' una dipendenza OBBLIGATORIA da quando ogni mercato gestito deve avere una
    // via d'uscita: senza, il reset si ferma prima di piazzare. Vedi uscita-sempre-accesa.test.js.
    setAutoClose: async ({ marketId, enabled }) => { (fatti.uscita = fatti.uscita || []).push({ marketId, enabled }); return { ok: true }; },

...
// ═══════════════════════════════════════════════════════════════════════════════════════════════════
{
  ...
  // ══ LA FOTOGRAFIA E' STATA SOSTITUITA DALL'INVARIANTE — 19 agosto 2026 ══════════════════════════
  // Qui c'era `maxOpenNotionalUsd === 150`, e il suo mestiere dichiarato era «accorgersi se qualcuno lo
  // muove senza dirlo». Quel mestiere oggi ce l'ha `lib/safety/limiti-versionati.test.js`, che dal 18
  // agosto confronta il file VERSIONATO col disco chiave per chiave e fallisce se divergono — una
  // difesa piu' forte, perche' non vive nella memoria di chi ha scritto il numero. Tenerne due copie
  // significava solo che una delle due sarebbe invecchiata, ed e' invecchiata: e' rimasta a $150 per
  // un giorno dopo che l'operatore aveva deciso $650.
  //
  // Al suo posto la PROPRIETA', che non invecchia: il tetto deve COPRIRE l'esposizione massima
  // raggiungibile — N coppie a riposo piu' il loro completamento — o il gate murerebbe la gestione a
  // meta' strada, come e' successo il 16 agosto con $150 contro 3 x $61,25 (§5-bis p.168).
  {
    const { esposizioneMassimaRaggiungibileUsd } = require('../rewards/concentration');
    const { quantiMercati } = require('./quanti-mercati');
    const raggiungibile = esposizioneMassimaRaggiungibileUsd((() => {
  ...
  return require('./quanti-mercati').quantiMercati(app.env).quanti;
    ...
    ok('il tetto di esposizione copre l esposizione massima raggiungibile',
      r.ok && Number.isFinite(raggiungibile) && r.limits.maxOpenNotionalUsd >= raggiungibile,
      `$${r.ok && r.limits.maxOpenNotionalUsd} contro $${raggiungibile}`);
  }
...
(async () => {
  ...
  // ═════════════════════════════════════════════════════════════════════════════════════════════════
  {
    ...
    // ⚠ `midSource`/`midAgeSec` sono parte della fixture dal 19 agosto 2026: dal commit `c919981`
    //   `decideClose` non vende contro un bid che non si puo' DATARE, e una fixture senza eta' riceve
    //   `skip/book-non-databile` senza arrivare al codice che questo blocco prova. Il gate e' provato
    //   a fondo in `bid-databile.test.js`; qui si difende che stia a monte ANCHE di questo percorso.
    ...
    // ⚑ E il gate morde su questo stesso percorso: il CONTROLLO e' la riga qui sopra, che vende.
    const cieco = decideClose({ position: { tokenId: 'ty', size: 50, avgPrice: 0.50 }, restingOrders: [],
      rules: { ...rules, midAgeSec: undefined }, book: 'yes' });
    ok('  e senza l eta del book non si vende', cieco.action === 'skip' && cieco.gate === 'book-non-databile',
      `${cieco.action}/${cieco.gate}`);

...
// ═══════════════════════════════════════════════════════════════════════════════════════════════════
{
  ...
  // I NUMERI SI SONO MOSSI DUE VOLTE DA QUANDO QUESTO TEST E' NATO: $500 → $600 col deposito del
  // 3 agosto 2026, e il tetto per ordine $40 → $1000 quando si e' deciso che non doveva piu' tagliare
  // l'allocazione. Restano fissi qui, e non letti dalla configurazione, perche' il senso del test e'
  // proprio accorgersi se qualcuno li muove senza dirlo.
  ok('il tetto di esposizione totale e 600', r.ok && r.limits.maxOpenNotionalUsd === 600, String(r.ok && r.limits.maxOpenNotionalUsd));
  ok('  rate limit e perdita giornaliera INVARIATI: sono l unica protezione strutturale rimasta',
    r.limits.maxOrdersPerWindow === 20 && r.limits.maxDailyLossUsd === 25,
    `rate ${r.limits.maxOrdersPerWindow}/60s · giorno ${r.limits.maxDailyLossUsd}`);
  ok('  il tetto per ordine e stato alzato apposta, e non vincola piu', r.limits.maxOrderNotionalUsd === 1000,
...
(async () => {
  ...
  // ═════════════════════════════════════════════════════════════════════════════════════════════════
  {
    ...
    ok('  e il motivo dice che la banda e sotto il carico', /sotto il prezzo di carico/.test(crollo.reason), crollo.reason.slice(0, 70));

...
// ⚠ `midSource` E `midAgeSec` SONO PARTE DELLA FIXTURE DAL 19 AGOSTO 2026, e non sono un dettaglio.
// Dal commit `c919981` `decideClose` non vende contro un bid che non si puo' DATARE: una fixture senza
// eta' riceve `skip/book-non-databile` e non arriva mai al codice che questo test esiste per provare.
// Il gate non si aggira — si ALIMENTA con un book databile, e lo si difende a parte nel blocco ⑤.
const mkRules = (mid, extra = {}) => ({ readable: true, tick: TICK, maxSpreadCents: 4.5,
  tokenId: 'tokY', tokenIdNo: 'tokN', midSource: 'live-book', midAgeSec: 3,
  books: { yes: { scoringMid: 1 - mid }, no: { scoringMid: mid } }, ...extra });
...
const decidi = (minuti, prezzoALibro, mid = 0.175, extraRules = {}) => decideClose({
  position: posizione, restingOrders: uscita(prezzoALibro), rules: mkRules(mid, extraRules), book: 'no',
...
// ── ⑤ IL GATE CHE HA RESO ROSSO QUESTO TEST, DIFESO INVECE CHE AGGIRATO ─────────────────────────
// La proprieta' e' di `c919981` e vale piu' della scala: `auto-close` non produce prezzi propri — il
// prezzo d'uscita E' il bid del libro camminato — quindi vendere contro un bid che non si sa quando e'
// stato letto e' l'unico modo in cui questa macchina puo' regalare capitale. Il repricer e il
// piazzatore guardavano gia' l'orologio; chi VENDE era l'unico a non guardarlo.
{
  // ⚑ eta' ASSENTE ⇒ non si vende, a nessun gradino. Nemmeno al 2, dove la scala CONCEDE.
  for (const min of [10, 35, 65, 250]) {
    const d = decidi(min, 0.21, 0.175, { midAgeSec: undefined });
    ok(`⑤ a ${min} min con eta assente: skip/book-non-databile`,
      d.action === 'skip' && d.gate === 'book-non-databile', `(${d.action}/${d.gate})`);
  }
  // ⚠ `Number(null) === 0` — la settima occorrenza del repo nacque proprio qui: un'eta' `null`
  //   convertita valeva zero, cioe' «freschissimo», e il gate lasciava passare il caso che esiste per
  //   fermare. Si guarda il TIPO.
  for (const eta of [null, undefined, '3', NaN, {}]) {
    const d = decidi(65, 0.21, 0.175, { midAgeSec: eta });
    ok(`⑤ eta ${JSON.stringify(eta)} non e un numero ⇒ non databile`,
      d.action === 'skip' && d.gate === 'book-non-databile', `(${d.gate})`);
  }
  // Una fonte che non e' il book live si ferma prima, con la sua causa distinta.
  {
    const d = decidi(65, 0.21, 0.175, { midSource: 'catalogo-di-ripiego' });
    ok('⑤ fonte non live ⇒ skip/book-non-live, causa distinta da «non databile»',
      d.action === 'skip' && d.gate === 'book-non-live', `(${d.gate})`);
  }
  // ⚑ E IL CONTROLLO, senza il quale i rifiuti qui sopra non proverebbero niente: con un book
  //   databile e fresco il gradino 2 arriva davvero all'azione.
  {
    const d = decidi(65, 0.21);
    ok('⑤ ⚑ CONTROLLO: book databile e fresco ⇒ il gate non morde e la scala agisce',
      d.action !== 'skip' || d.gate !== 'book-non-databile', `(${d.action}/${d.gate || '-'})`);
  }
}

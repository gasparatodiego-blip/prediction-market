...
sez('③ agent41 esegue, e gira PRIMA del cancello su AVVIA');
{
  ...
  // ⚠ IL CANCELLO SI È SDOPPIATO IL 18 AGOSTO 2026, e questo test lo ha preso: `TRIGGER_ATTIVO` (il
  // processo) e `botAttivo()` (AVVIA/FERMA) erano una condizione sola, e il presidio d'uscita stava
  // dietro entrambi. Adesso sono due, e in mezzo c'è il presidio — vedi
  // `presidio-gira-a-bot-fermo.test.js`. L'ancora è quella su AVVIA, che è il cancello che conta qui.
  const iCancello = src.indexOf('if (!botAttivo()) return;');
  ...
  // ⚠⚠ QUESTA ASSERZIONE ERA ROVESCIATA, ED E' STATA RISCRITTA, NON AMMORBIDITA. Diceva «il presidio
  // dei 60 minuti sta DOPO il cancello (e infatti a bot FERMO non gira)»: era VERA, ed era il DIFETTO.
  // L'operatore l'ha nominato il 18 agosto — «se il kill mette FERMA e FERMA spegne l'ultima rete,
  // allora dopo un kill le gambe scoperte restano senza nessuno che le guardi» — e il presidio e'
  // stato spostato PRIMA del cancello. Adesso le due reti stanno dalla stessa parte, ed e' il punto.
  const iPresidio = src.indexOf('await presidioPosizioniVecchie()');
  ok('  e anche il presidio dei 60 minuti sta PRIMA del cancello (era il difetto del 18 agosto)',
    iPresidio > 0 && iPresidio < iCancello, `presidio@${iPresidio} cancello@${iCancello}`);
  // ⚠ E le due reti restano DUE, in quest'ordine: R10 vende subito le gambe che il kill ha elencato,
  // il presidio raccoglie dopo 60 minuti quelle che R10 non ha potuto vendere e quelle nate dopo.
  ok('  e la chiusura di emergenza viene PRIMA del presidio', iChiusura < iPresidio,
    `chiusura@${iChiusura} presidio@${iPresidio}`);

...
console.log('\n════ ② un deposito non è un guadagno ════');
{
  ...
  ok('  e il riferimento sale dell\'importo depositato', Math.abs(r.riferimentoUsd - 2150) < 0.01, String(r.riferimentoUsd));
  ok('  quindi il drawdown resta ZERO: da un versamento non si guadagna',
    Math.abs(2150 - r.riferimentoUsd) < 0.01);
  ok('  e il cumulato dei movimenti lo registra', Math.abs(r.stato.movimentiEsterniUsd - 1500) < 0.01, String(r.stato.movimentiEsterniUsd));
}
...
console.log('\n════ ⑥ il massimo è MOBILE e non scende da solo ════');
{
  let s = R.aggiornaRiferimento({ stato: null, capitale: cap(1000, 100), now: T0 }).stato;
  s = R.aggiornaRiferimento({ stato: s, capitale: cap(1010, 110), now: T0 + 30_000 }).stato;
  ok('un guadagno alza il massimo', Math.abs(Number(s.riferimentoUsd) - 1010) < 0.01, String(s.riferimentoUsd));
  const giu = R.aggiornaRiferimento({ stato: s, capitale: cap(990, 90), now: T0 + 60_000 });

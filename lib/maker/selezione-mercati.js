...
function valutaAmmissibilita(riga, { ora, orizzonteMassimoOre = null, quota = QUOTA_SCAGLIONI,
  ...
  if (bookVivi && bookVivi.leggibile === true) {
    ...
    if (b.live !== true) {
      return { ammissibile: false, motivo: 'book-non-live',
        dettaglio: `il feed dichiara questo book NON live${b.reason ? ` (${b.reason})` : ''}: la sottoscrizione e caduta`, oreAllaScadenza: ore };
    }
    if (!fin(b.ageMs)) {
      // ⚠ Caso distinto da «vecchio», e volutamente: qui il feed non dice QUANDO ha parlato. E' un
      // guasto nostro, non del mercato, e va chiamato col suo nome o domani non lo si distingue.
      return { ammissibile: false, motivo: 'book-non-databile',
        dettaglio: 'il book non dichiara la propria eta: non si puo affermare che il prezzo sia fresco', oreAllaScadenza: ore };
    }
    if (fin(bookVivi.etaMassimaMs) && b.ageMs > bookVivi.etaMassimaMs) {
      return { ammissibile: false, motivo: 'book-vecchio',
        dettaglio: `book fermo da ${Math.round(b.ageMs / 1000)}s, oltre i ${Math.round(bookVivi.etaMassimaMs / 1000)}s ammessi: chi dovra mantenere l ordine lo rifiuterebbe con la stessa soglia`,
        oreAllaScadenza: ore };
    }
  }

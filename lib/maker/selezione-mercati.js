...
function decidiSelezione({ board, stato, posizioni, ora, max = MAX_MERCATI_CONTEMPORANEI, escludi = [],
  ...
  for (const [id, voce] of Object.entries(S.selezionati)) {
    ...
    const v = valutaAmmissibilita(riga, { ora, orizzonteMassimoOre, quota });
  ...
  if (fin(codaLungaGiorni) && codaLungaGiorni > 0) {
    ...
    if (!cortoGiaAttivo) {
      const corti = candidati.filter((r) => eCorto(valutaAmmissibilita(r, { ora, orizzonteMassimoOre, quota }).oreAllaScadenza));

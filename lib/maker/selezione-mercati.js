...
// ⚠ 12 → 19 il 23 agosto 2026, decisione dell'operatore, e di nuovo il numero e' un CONTO.
// Il vincolo che ha fissato il 12 era il CAP, non il capitale: 12 x 2 x $61,25 = $1.470,00 contro un
// cap di $1.470,00, cioe' zero franco. Portando il cap a $2.400 il conto si riapre:
//     19 x 2 x $61,25 = $2.327,50  ≤ cap $2.400,00      (20 x 2 x $61,25 = $2.450,00 > $2.400: NO)
// DICIANNOVE e' quindi il massimo che il cap autorizza tenendo OGNI mercato al pavimento premiante
// dello scaglione 50 ($61,25) — sotto quel pavimento il reward non e' piu' basso, e' ZERO.
// ⚠⚠ IL SOFFITTO NON E' IL NUMERO IN SERVIZIO, ed e' la distinzione che conta qui. Questo e' cio' che
// il CAP autorizza; quanti slot il bot apra davvero lo dice `MAKER_MERCATI_CONTEMPORANEI`
// nell'ambiente di agent41, e quel numero e' limitato da una SECONDA grandezza che il cap non
// conosce: la CASSA. Il piano compra le sorelle con denaro vero, e serve un residuo per il gradino 1
// della scala d'uscita (§4.6). Al 23/08, saldo $1.391,57: 19 x $61,25 = $1.163,75 lascerebbe
// **$227,82**, sotto il pavimento di cassa di $250 posto dall'operatore; 18 x $61,25 = $1.102,50 ne
// lascia **$289,07**. Per questo in servizio ci sono DICIOTTO slot su un soffitto di diciannove: il
// soffitto e' una regola di rischio, il numero in servizio e' una decisione sul capitale di oggi.
// ⚠ CONSEGUENZA DI RISCHIO, dichiarata: il cap di esposizione cumulativa sale da $1.470 a $2.400
// nello stesso momento, e con lui il TETTO DURO di `lib/safety/risk-limits.HARD_CEILINGS`, che stava
// a $2.000 e avrebbe clampato il $2.400 in silenzio — un cap dichiarato e non in servizio e' peggio
// del cap vecchio, perche' fa contare una difesa che non c'e'. A $2.400 il cap vale il **161%** del
// capitale ($1.484,39 = cassa + posizioni): il cap NON e' piu' una difesa, e restano il **kill a
// −$100** e il **guardiano**. E' esattamente la conseguenza gia' scritta a $1.470 (98,3%), portata
// oltre l'unita': chi rimette il cap deve rimettere anche questo numero.
// ⚠⚠⚠ IL LETTERALE E' STATO TOLTO IL 24 AGOSTO 2026, E NON TORNA. `MAX_MERCATI_CONTEMPORANEI` era
// una costante di sorgente che faceva TRE mestieri incompatibili: era il DIFETTO (quanti mercati se
// nessuno lo dice), era il SOFFITTO (il massimo che il cap autorizza) ed era il valore che
// `quanti-mercati.js` importava per derivare entrambi. Un numero che e' anche il proprio fallback non
// puo' fallire rumorosamente: qualunque errore di configurazione — variabile assente, vuota, non
// numerica, fuori scala — atterrava su di lui **in silenzio**, e il processo partiva quotando un
// numero di mercati che nessuno aveva chiesto. E' la famiglia «un valore che non si capisce vale il
// difetto», che va bene per una soglia di prezzo e NON va bene per una decisione di capitale.
//
// ADESSO IL NUMERO NON ESISTE IN QUESTO MODULO. Viene esclusivamente da
// `process.env.MAKER_MERCATI_CONTEMPORANEI`, letto in UN SOLO PUNTO — `lib/maker/quanti-mercati.js`,
// che e' il layer impuro — e se manca, non e' numerico o cade fuori da 1..20 quel modulo **solleva**:
// il processo non parte. Qui dentro `max` e' un PARAMETRO OBBLIGATORIO, e chi non lo passa ottiene
// un'eccezione, mai un numero.
//
// ⚠ QUESTO MODULO RESTA PURO: non legge l'ambiente, non ha orologio, non ha `require`. E' il
// chiamante a portare il numero, come porta gia' `ora`.
//
// ⚠ I SECCHI NON SONO IL NUMERO DI MERCATI. I confini (`basso` <= 20, `alto` <= MIN_SIZE_MASSIMA)
// non dipendono da N: dipende da N solo quanti POSTI ha ciascun secchio. Per questo la tabella dei
// confini resta una costante — non e' un tetto travestito, e classificare un `rewardsMinSize` non ha
// mai avuto bisogno di sapere quanti slot ci sono.
const SECCHI_SCAGLIONE = Object.freeze([
  Object.freeze({ chiave: 'basso', maxMinSize: 20 }),
  Object.freeze({ chiave: 'alto', maxMinSize: MIN_SIZE_MASSIMA }),
]);

/** L'intervallo SINTATTICO ammesso per il numero di mercati. ⚠ NON e' un difetto e non e' un tetto di
 *  rischio: e' il range oltre il quale un valore e' certamente un errore di battitura. Il tetto di
 *  rischio vero e' l'invariante `N x 2 x $61,25 <= min(cap versionato, tetto duro)`, che vive in
 *  `lib/safety/invariante-cap-slot.js` ed e' verificata all'avvio di agent40 e agent41. A cap $2.400
 *  quell'invariante ferma N a 19, cioe' MORDE PRIMA di questo range: sono due cancelli distinti
 *  apposta, uno contro le dita storte e uno contro il rischio. */
const LIMITE_SLOT = Object.freeze({ min: 1, max: 20 });

/**
 * IL NUMERO DI MERCATI, O UN'ECCEZIONE. Non c'e' una terza risposta, ed e' il punto di questo modulo.
 *
 * ⚠⚠ NESSUN FALLBACK, NESSUN DEGRADO SILENZIOSO. Prima ogni percorso qui dentro finiva su
 * `MAX_MERCATI_CONTEMPORANEI`: variabile assente, valore vuoto, `'diciotto'`, `2.5`, `0`, `99` —
 * cinque errori diversi, una sola risposta, e quella risposta era un numero plausibile che nessuno
 * aveva chiesto. Un bot che apre mercati con capitale vero deve rifiutarsi di partire, non indovinare.
 *
 * ⚠ IL MESSAGGIO NOMINA LA VARIABILE E IL VALORE LETTO, perche' chi legge lo stderr alle tre di notte
 * deve sapere cosa correggere senza aprire il codice.
 *
 * @param {*} valore   il numero che il chiamante ha ricevuto (di norma da `quanti-mercati`)
 * @param {string} dove chi sta chiedendo, per il messaggio
 * @returns {number} un intero in `LIMITE_SLOT`
 * @throws {Error} se non lo e'
 */
function esigiSlot(valore, dove) {
  const n = Number(valore);
  if (valore === undefined || valore === null || valore === ''
      || !Number.isFinite(n) || !Number.isInteger(n) || n < LIMITE_SLOT.min || n > LIMITE_SLOT.max) {
    throw new Error(
      `MAKER_MERCATI_CONTEMPORANEI: quanti mercati contemporanei non e' un intero fra ${LIMITE_SLOT.min}`
      + ` e ${LIMITE_SLOT.max} — letto ${JSON.stringify(valore)} (${dove}).`
      + ' Il numero viene ESCLUSIVAMENTE da process.env.MAKER_MERCATI_CONTEMPORANEI e non ha difetto:'
      + ' si dichiara in agents/ecosystem.config.js e si riavvia DAL FILE. Il processo non parte.');
  }
  return n;
}
...
function quotaScaglioni(max) {
  // ⚠ NIENTE DIFETTO E NIENTE CLAMP. Prima un `max` illeggibile diventava il soffitto e un `max` oltre
  // il soffitto ci veniva schiacciato: in tutti e due i casi la composizione usciva DIVERSA da quella
  // chiesta senza che nessuno lo sapesse. Adesso un numero che non si capisce e' un'eccezione.
  const n = esigiSlot(max, 'quotaScaglioni(max)');
...
// ⚠ `QUOTA_SCAGLIONI` E' STATA TOLTA insieme al letterale: era `quotaScaglioni(MAX_...)`, cioe' il
// numero di mercati sotto un altro nome, e chi la usava come difetto stava ereditando un tetto senza
// saperlo. Chi deve CLASSIFICARE un `rewardsMinSize` usa `SECCHI_SCAGLIONE` (i confini, senza posti);
// chi deve COMPORRE gli slot chiama `quotaScaglioni(max)` col numero vero.
...
function partizionaSlot(totale, corti) {
  const T = esigiSlot(totale, 'partizionaSlot(totale)');
...
function scaglioneDi(minSize, quota = SECCHI_SCAGLIONE) {
  if (!fin(minSize) || minSize <= 0) return null;
  for (const b of (Array.isArray(quota) && quota.length ? quota : SECCHI_SCAGLIONE)) {
 ...
 */
function valutaAmmissibilita(riga, { ora, orizzonteMassimoOre = null, quota = SECCHI_SCAGLIONE,
  bookVivi = null } = {}) {
...
// ══ IL TRAVASO FRA FASCE — 24 agosto 2026, decisione dell'operatore ══════════════════════════════
//
// IL FATTO, misurato sul board vivo del 24 agosto alle 10:58Z e non dedotto: 307 valutati, 31
// ammissibili, 15 slot occupati su 18, `slotCorti 2 · slotLunghi 16`, tenuti `corta 2 · lunga 13`.
// I posti liberi erano quindi TUTTI E TRE della fascia LUNGA (`postiVuoti [{lunga, 3}]`), gli
// entranti erano ZERO in entrambe le fasce, e i **dodici** candidati che la classifica offriva erano
// tutti CORTI — respinti uno per uno con `fascia-piena`. Tre slot fermi, dodici candidati pronti, e
// la regola che li teneva separati non stava proteggendo niente: la fascia lunga non aveva NESSUNO
// da mettere in quei posti.
//
// ⚠ NON E' L'ABOLIZIONE DELLA PARTIZIONE, ED E' LA DIFFERENZA CHE CONTA. La partizione resta il
// vincolo ordinario: finche' la fascia di destinazione ha anche UN SOLO candidato entrante, i suoi
// posti sono suoi e nessuno li attraversa. Il travaso vive solo nel caso degenere «posti vuoti E
// zero entranti», cioe' esattamente quando il vincolo non sta piu' scegliendo fra due usi del
// capitale ma fra un uso e NESSUN uso. Un posto che nessuno della sua fascia puo' prendere non e'
// una riserva: e' capitale fermo.
//
// LE CINQUE COSE CHE IL TRAVASO NON FA, e ognuna e' una riga di codice qui sotto:
//   ① NON ALZA IL NUMERO TOTALE DI SLOT — `slotResidui` e' lo stesso budget globale del 3-ter
//      (`slotLiberi + spodestati − entranti`), e il travaso puo' solo consumarlo. Da qui discende
//      che l'invariante d'avvio `N × 2 × $61,25 ≤ cap` (§4.2) non e' toccata: N non cambia.
//   ② NON TOCCA GLI OCCUPANTI — non esiste un ramo che cancelli una voce da `selezionati`. Il
//      travaso RIEMPIE posti gia' vuoti, non ne libera. Nessuna cancellazione al venue ne discende.
//   ③ NON ATTRAVERSA IL SECCHIO DI CAPITALE — la quota basso/alto e' una partizione ORTOGONALE
//      (§4.13) e resta intera: un candidato travasa solo se il SUO secchio ha ancora un posto.
//   ④ NON TRAVASA CHI NON E' STATO RESPINTO DALLA FASCIA — solo `motivo === 'fascia-piena'`. Chi e'
//      caduto per il secchio, per la coda lunga o per ammissibilita' resta caduto: il travaso apre
//      una sola porta, quella che ha chiuso lui stesso.
//   ⑤ NON TRAVASA NEL DUBBIO — netto non misurabile ⇒ quel candidato non travasa, e lo si dichiara.
//      E' la stessa regola di ① dello spodestamento: senza netto non si dimostra che lo scambio
//      migliora, e qui non si dimostrerebbe nemmeno in che ordine servire dodici candidati. Un netto
//      che non si sa NON diventa zero (§5.3).
//
// ⚠ SI ORDINA PER NETTO GIORNALIERO DECRESCENTE, la stessa grandezza con cui 3-bis spodesta e con
// cui il knapsack sceglie. Il pareggio si rompe sul conditionId, come in `ordinaCandidati`: due giri
// sullo stesso board devono dare la stessa risposta.
//
// ⚠ PURA E SENZA EFFETTI: riceve numeri e liste, restituisce assegnazioni e una riga di verbale. Non
// scrive lo stato — lo fa il chiamante, con le stesse strutture del 3-ter e non con altre.
/**
 * @param postiPerFascia  [{fascia, posti}] i posti ANCORA vuoti al termine del 3-ter
 * @param entrantiPerFascia {corta, lunga} quanti sono entrati in questo giro, per fascia
 * @param candidati       [{id, fascia, scaglione, netto}] i soli scartati con `fascia-piena`
 * @param postiPerScaglione [{scaglione, posti}] i posti di capitale ancora liberi
 * @param slotResidui     quanti ingressi il budget globale consente ancora
 */
function decidiTravasoFasce({ postiPerFascia = [], entrantiPerFascia = null,
  candidati = [], postiPerScaglione = [], slotResidui = 0 } = {}) {
  const FASCE = ['corta', 'lunga'];
  const posti = new Map();
  for (const x of (postiPerFascia || [])) {
    if (x && FASCE.includes(x.fascia) && fin(x.posti) && x.posti > 0) posti.set(x.fascia, Math.floor(x.posti));
  }
  const secchi = new Map();
  for (const x of (postiPerScaglione || [])) {
    if (x && typeof x.scaglione === 'string' && fin(x.posti)) secchi.set(x.scaglione, Math.floor(x.posti));
  }
  let budget = (fin(slotResidui) && slotResidui > 0) ? Math.floor(slotResidui) : 0;
  ...
  for (const dest of FASCE) {
    const liberi = posti.get(dest) || 0;
    if (liberi <= 0) continue;
    // ⚠ «ZERO CANDIDATI ENTRANTI» E' STRETTO, E DEV'ESSERLO. Un solo entrante nella fascia di
    // destinazione dimostra che quella fascia e' viva e che i suoi posti hanno ancora un mercato
    // proprio che potrebbe prenderli al giro dopo; travasarne uno sarebbe scegliere per lei.
    const entrati = entrantiPerFascia && fin(entrantiPerFascia[dest]) ? entrantiPerFascia[dest] : 0;
    if (entrati !== 0) {
      motiviFascia.push(`«${dest}» ha ${liberi} posti vuoti ma ${entrati} entrante/i propri: non si travasa`);
      continue;
    }
    const daFascia = (candidati || [])
      .filter((c) => c && typeof c.id === 'string' && c.id && !usati.has(c.id))
      .filter((c) => c.fascia !== dest && FASCE.includes(c.fascia));
    if (!daFascia.length) {
      motiviFascia.push(`«${dest}» ha ${liberi} posti vuoti e zero entranti, ma nessun candidato dell'altra fascia e' stato respinto per fascia-piena`);
      continue;
    }
    const ordinabili = [];
    for (const c of daFascia) {
      if (!fin(c.netto)) { nonTravasati.push({ id: c.id, fascia: c.fascia, motivo: 'netto-non-misurabile' }); continue; }
      ordinabili.push(c);
    }
    if (!ordinabili.length) {
      motiviFascia.push(`«${dest}» ha ${liberi} posti vuoti e ${daFascia.length} candidati dell'altra fascia, ma NESSUNO ha un netto misurabile: nel dubbio non si travasa`);
      continue;
    }
    ordinabili.sort((a, b) => (b.netto !== a.netto ? b.netto - a.netto : (a.id < b.id ? -1 : 1)));

    const presi = [];
    for (const c of ordinabili) {
      if ((posti.get(dest) || 0) <= 0) break;
      if (budget <= 0) break;
      const s = c.scaglione;
      if (!(secchi.get(s) > 0)) { nonTravasati.push({ id: c.id, fascia: c.fascia, motivo: 'secchio-di-capitale-pieno', scaglione: s }); continue; }
      secchi.set(s, secchi.get(s) - 1);
      posti.set(dest, posti.get(dest) - 1);
      budget -= 1;
      usati.add(c.id);
      const a = { id: c.id, da: c.fascia, a: dest, scaglione: s, netto: c.netto };
      assegnazioni.push(a);
      presi.push(a);
    }
    if (presi.length) {
      travasi.push({ da: presi[0].da, a: dest, posti: presi.length,
        ids: presi.map((x) => x.id), netti: presi.map((x) => x.netto),
        candidatiDisponibili: ordinabili.length });
    } else if ((posti.get(dest) || 0) > 0) {
      motiviFascia.push(budget <= 0
        ? `«${dest}» ha ${posti.get(dest)} posti vuoti ma il budget globale di ingressi e' esaurito: non si travasa`
        : `«${dest}» ha ${posti.get(dest)} posti vuoti e ${ordinabili.length} candidati con netto, ma nessuno ha un posto libero nel proprio secchio di capitale`);
    }
  }

  const postiVuotiDopo = FASCE
    .map((f) => ({ fascia: f, posti: posti.get(f) || 0 }))
    .filter((x) => x.posti > 0);

  // ⚠ LA RIGA SI SCRIVE ANCHE QUANDO NON SI TRAVASA NIENTE, e dice PERCHE'. Un travaso che non
  // avviene e un travaso che non e' stato nemmeno valutato producono lo stesso disco se si tace, ed
  // e' esattamente la coppia di stati che questa regola esiste per distinguere.
  const riga = {
    tipo: 'travaso-fasce',
    travasi,
    posti: assegnazioni.length,
    postiVuotiDopo,
    nonTravasati,
    motivo: assegnazioni.length
      ? travasi.map((t) => `${t.posti} posto/i dalla fascia «${t.da}» alla fascia «${t.a}»`
        + ` (${t.ids.map((id, i) => `${id.slice(0, 10)}… netto ${t.netti[i].toFixed(3)}/g`).join(', ')})`
        + ` — restano ${postiVuotiDopo.reduce((n, x) => n + x.posti, 0)} posti vuoti`).join(' · ')
      : (motiviFascia.length ? motiviFascia.join(' · ')
        : 'nessuna fascia con posti vuoti: non c\'era niente da travasare'),
  };
  return { assegnazioni, travasi, nonTravasati, postiVuotiDopo, riga };
}
 ...
 * @param max          quanti mercati contemporanei. ⚠ OBBLIGATORIO e senza difetto: viene da
 *                     `quanti-mercati.quantiMercati(process.env)`. Assente o fuori da 1..20 ⇒ THROW.
 ...
 */
function decidiSelezione({ board, stato, posizioni, ora, max, escludi = [],
  ...
  // ⚠ IL NUMERO ENTRA DA QUI E DA NESSUN'ALTRA PARTE. Nessun difetto: chi chiama la decisione senza
  // dire quanti mercati vuole non ottiene «quelli di sempre», ottiene un'eccezione — perche' «quelli
  // di sempre» era esattamente il modo in cui un errore di configurazione diventava una posa.
  const slotTotaliN = esigiSlot(max, 'decidiSelezione({ max })');
  ...
  const quota = quotaScaglioni(slotTotaliN);
  const vuoto = (motivo) => ({
  ...
  if (nettoPerMercato && ordiniLeggibili) {
    ...
    // Quanti occupanti per secchio ADESSO, e quanti ne vuole la quota. Serve alla deroga qui sotto.
    const occupantiPerScaglione = new Map();
    for (const [, voce] of attivi) {
      if (!voce || !voce.scaglione) continue;
      occupantiPerScaglione.set(voce.scaglione, (occupantiPerScaglione.get(voce.scaglione) || 0) + 1);
    }
    const postiDiQuota = new Map(quota.map((b) => [b.chiave, b.posti]));
    /** Vero se attraversare il secchio muove la composizione VERSO la quota, e solo allora. */
    function derogaSecchio(scagSfidante, scagOccupante, nettoSfidante, nettoOccupante) {
      if (!scagSfidante || !scagOccupante) return false;
      if (!fin(nettoSfidante) || !fin(nettoOccupante)) return false;      // fail-closed
      if (!(nettoOccupante < 0)) return false;                            // ① l'occupante deve COSTARE
      if (!(nettoSfidante > 0)) return false;                             // ② lo sfidante deve RENDERE
      const quotaOcc = postiDiQuota.get(scagOccupante);
      const quotaSfi = postiDiQuota.get(scagSfidante);
      if (!fin(quotaOcc) || !fin(quotaSfi)) return false;                 // fail-closed
      const haOcc = occupantiPerScaglione.get(scagOccupante) || 0;
      const haSfi = occupantiPerScaglione.get(scagSfidante) || 0;
      return haOcc > quotaOcc && haSfi < quotaSfi;                        // ③ e ④
    }
    ...
    for (const occ of occupantiOrdinati) {
      ...
      const sf = sfidanti.find((r) => {
        ...
        if (!v.ammissibile) return false;
        // ══ IL SECCHIO, E L'UNICA DEROGA — 23 agosto 2026 ════════════════════════════════════════
        //
        // La regola resta «stesso secchio»: uno scaglione vuoto non si riempie col vicino, o si
        // cambierebbe in silenzio la cifra di capitale che l'operatore ha deciso (§4.13).
        //
        // ⚠ MA IL SECCHIO STAVA PROTEGGENDO CHI PERDE SOLDI, ed e' misurato. Il 23/08 alle 10:20Z:
        // quattro occupanti «alto» con netto NEGATIVO (−0,04 · −0,17 · −2,27 · **−7,86** $/g) e, fuori,
        // `0xddcb215d8c` («Democratic Party — PA-08 House seat») a **+22,73 $/g**, ammissibile, non in
        // quarantena, scaglione «basso». Non compariva in NESSUNA lista di scarto del referto: non lo
        // scartava niente, semplicemente non poteva essere considerato. Con gli slot a 12/12 l'unica
        // porta era lo spodestamento, e lo spodestamento chiedeva lo STESSO secchio. Risultato: il
        // mercato migliore del board per netto — di un fattore CINQUE sul secondo — restava fuori
        // mentre quattro mercati che costano capitale tenevano il loro posto.
        //
        // LA DEROGA, e le quattro condizioni che la rendono stretta. Si attraversa il secchio SOLO se:
        //   ① l'occupante ha netto NEGATIVO — cioe' il piano dice che quel mercato COSTA;
        //   ② lo sfidante ha netto POSITIVO — non si scambia una perdita con un'altra;
        //   ③ il secchio dell'OCCUPANTE e' SOPRA la sua quota (ce ne sono piu' del dovuto);
        //   ④ il secchio dello SFIDANTE e' SOTTO la sua quota (ce ne sono meno del dovuto).
        // ③ e ④ insieme sono la ragione per cui questo NON viola §4.13: lo scambio muove la
        // composizione VERSO la cifra decisa dall'operatore, mai lontano. Con quota 4+8 e occupanti
        // 1+11, un «basso» che entra al posto di un «alto» porta la composizione da 1+11 a 2+10.
        //
        // ⚠ E NON CAMBIA IL CAPITALE, per costruzione: `MARKET_CAP_FIXED_USD` vale $61,25 per mercato
        // QUALUNQUE sia lo scaglione, quindi `N × 2 × tetto` non contiene la quota. Il secchio decide
        // QUALI mercati, mai QUANTO — ed e' esattamente perche' non tocca il capitale che questa
        // deroga e' ammissibile: se il secchio governasse la size, attraversarlo sarebbe un'altra cosa.
        //
        // ⚠ FAIL-CLOSED SU OGNI INGRESSO: netto dell'occupante o dello sfidante non finito, quota non
        // leggibile, conteggio degli occupanti non calcolabile ⇒ **niente deroga**, cioe' la regola di
        // sempre. Un dato che manca non apre mai una porta.
        if (v.scaglione !== occ.voce.scaglione) {
          if (!derogaSecchio(v.scaglione, occ.voce.scaglione, nettoDi(id), occ.netto)) return false;
        }
  ...
  // ⚠ LA RIGA DELLO SCARTATO SI TIENE, o il travaso qui sotto avrebbe solo un `id` e dovrebbe
  // ricostruire `valutaAmmissibilita` per conto suo — cioe' una SECONDA valutazione della stessa
  // riga, che e' il reperto D1 su un cancello di selezione. Questa mappa non esce dalla funzione.
  const righeScartatePerFascia = new Map();
  for (const r of ordinaCandidati(candidati, nettoPerMercato)) {
    ...
    // ⚠ DUE CONTATORI SEPARATI: un corto che non trova posto corto NON prende un posto lungo qui, e
    // un lungo non prende un posto corto. E' esattamente cio' che l'operatore ha chiesto, ed e'
    // l'unica riga che lo rende vero DENTRO QUESTO CICLO.
    // ⚠⚠ E DAL 24 AGOSTO 2026 QUESTO `continue` HA UN RAMO DI RIPIEGO, UNO SOLO E FUORI DI QUI: il
    // TRAVASO (`decidiTravasoFasce`, subito dopo questo ciclo) puo' assegnare un posto vuoto della
    // fascia OPPOSTA a chi e' finito qui con `fascia-piena` — ma solo se quella fascia ha chiuso il
    // giro con ZERO entranti propri, cioe' solo quando il vincolo non stava piu' scegliendo fra due
    // usi del capitale ma fra un uso e nessuno. Chi legge questa riga legga anche quella: la
    // partizione ordinaria e' ancora questa, il travaso e' il caso degenere.
    ...
    // candidati». Sono tre cause con tre cure diverse — e il travaso guarda SOLO la seconda.
    const fCand = fasciaDi(v.oreAllaScadenza, sogliaFascia);
    if (!postoPerFascia(fCand)) {
      const motivoF = fCand === null ? 'fascia-non-determinabile' : 'fascia-piena';
      scartatiPerFascia.push({ id, motivo: motivoF, fascia: fCand, oreAllaScadenza: v.oreAllaScadenza });
      // ⚠ SOLO `fascia-piena` E' TRAVASABILE. Una fascia non determinabile e' una scadenza non
      // letta, e «non ho letto» non diventa «va bene» (§5.3): quella riga non entra nella mappa.
      if (motivoF === 'fascia-piena') righeScartatePerFascia.set(id, { riga: r, v, fascia: fCand });
      continue;
    }
  ...
  // ── 3-quater · IL TRAVASO FRA FASCE ────────────────────────────────────────────────────────────
  // Qui c'e' SOLO il cablaggio: la decisione e' in `decidiTravasoFasce` (pura, sopra), e l'ingresso
  // passa dalle STESSE due strutture del 3-ter (`statoNuovo.selezionati` ed `entranti`), non da
  // strutture nuove. Se un giorno qualcuno cambiera' la forma di un ingresso, la cambiera' per
  // entrambi o il test se ne accorgera'.
  const travaso = decidiTravasoFasce({
    postiPerFascia: fasciaAttiva
      ? [...postiPerFascia].map(([fascia, posti]) => ({ fascia, posti }))
      : [],
    entrantiPerFascia: {
      corta: entranti.filter((e) => e.fascia === 'corta').length,
      lunga: entranti.filter((e) => e.fascia === 'lunga').length,
    },
    candidati: [...righeScartatePerFascia].map(([id, x]) => ({
      id, fascia: x.fascia, scaglione: x.v.scaglione,
      // ⚠ IL NETTO, E SOLO IL NETTO. `valoreCandidato` ripiega sul LORDO quando il netto manca, e qui
      // un ripiego sarebbe un ordinamento fra grandezze non confrontabili su una decisione che sposta
      // capitale: `netto:false` ⇒ `null` ⇒ quel candidato non travasa. E' la regola ⑤.
      netto: (() => { const p = valoreCandidato(x.riga, nettoPerMercato); return p.netto ? p.valore : null; })(),
    })),
    postiPerScaglione: [...postiLiberi].map(([scaglione, posti]) => ({ scaglione, posti })),
    // ⚠ LO STESSO BUDGET GLOBALE DEL 3-ter, scritto una volta sola: e' cio' che rende vero «il
    // travaso non alza mai il numero totale di slot».
    slotResidui: Math.max(0, (slotLiberi + spodestati.length) - entranti.length),
  });
  for (const a of travaso.assegnazioni) {
    const x = righeScartatePerFascia.get(a.id);
    if (!x) continue;                                   // non puo' accadere: la lista viene da qui
    const p = valoreCandidato(x.riga, nettoPerMercato);
    postiLiberi.set(x.v.scaglione, postiLiberi.get(x.v.scaglione) - 1);
    if (postiPerFascia.has(a.a)) postiPerFascia.set(a.a, postiPerFascia.get(a.a) - 1);
    statoNuovo.selezionati[a.id] = { entratoAt: ora, question: x.riga.question || null, uscenteDal: null,
      motivoUscita: null, scaglione: x.v.scaglione, categoria: x.v.categoria,
      inGestione: false, inGestioneDal: null };
    entranti.push({ id: a.id, question: x.riga.question || null, punteggio: p.valore, fontePunteggio: p.fonte,
      minSize: numero(x.riga.rewardsMinSize), scaglione: x.v.scaglione, categoria: x.v.categoria,
      oreAllaScadenza: x.v.oreAllaScadenza, fascia: x.fascia, riga: x.riga,
      // ⚠ CHI E' ENTRATO PER TRAVASO LO DICE. La sua `fascia` resta la SUA (e' un corto, e lo sara'
      // anche domani); il posto che occupa e' dell'altra, e sono due fatti diversi.
      travasato: true, postoDiFascia: a.a });
  }
  ...
  const fasce = {
    ...
    // ⚠ DUE CONTEGGI DIVERSI SULLA STESSA COSA, ED E' VOLUTO. `entrantiPerFascia` conta i mercati per
    // la LORO fascia (un corto travasato resta un corto); `postiOccupatiPerFascia` conta i POSTI, cioe'
    // di chi era lo slot. Prima del travaso i due numeri coincidevano sempre e uno solo bastava; da
    // oggi divergono esattamente di quanto si e' travasato, e fonderli renderebbe illeggibile quale
    // delle due domande si sta facendo.
    postiOccupatiPerFascia: {
      corta: entranti.filter((e) => (e.travasato ? e.postoDiFascia : e.fascia) === 'corta').length,
      lunga: entranti.filter((e) => (e.travasato ? e.postoDiFascia : e.fascia) === 'lunga').length,
    },
    scartatiPerFascia,
    // Il verbale del travaso, sempre presente: `travasi: []` con un `motivo` che dice perche' non e'
    // avvenuto e' un'informazione, un campo assente no.
    travaso: travaso.riga,
  };
  ...
  return {
    ...
    slotLiberi: Math.max(0, slotTotaliN - attiviFinali),
    statoNuovo, valutati: perId.size, ammissibili,
    postiNonAssegnati, scartatiPerComposizione, fasce,
    // ⚠ ANCHE AL PRIMO LIVELLO, e non solo dentro `fasce`: il chiamante ne scrive una RIGA DI GIORNALE
    // propria (`tipo:'travaso-fasce'`), e una riga di giornale non deve andarsela a pescare dentro un
    // altro record. E' lo stesso oggetto, non una copia rifatta.
    travaso: travaso.riga,
    ...
    // ── ⚠⚠ E IL MOTIVO PUNTA AL CAMPO CHE CONTIENE DAVVERO LA CAUSA — 24 agosto 2026 ─────────────
    // IL FATTO: fino a oggi il ramo «restano N posti liberi» diceva «la ragione e' nella composizione
    // o negli scarti dichiarati qui accanto» a prescindere. Misurato il 24/08 alle 10:58Z:
    // `scartatiPerComposizione` era **[]** e i dodici scarti stavano tutti in `fasce.scartatiPerFascia`
    // — cioe' il referto mandava il lettore a guardare una lista VUOTA mentre la causa era in un altro
    // campo. E' §5.2 p.72, ed e' la stessa classe D7 di un commento che descrive una riga inesistente:
    // qui era un referto che nominava un campo che non conteneva niente.
    // ⚠ SI NOMINANO I CAMPI CHE HANNO DAVVERO RIGHE, e nessun altro. Se non ne ha nessuno, lo si dice:
    // «nessuna lista spiega questi posti» e' una risposta onesta, «guarda qui accanto» no.
    // ⚠ E IL CASO FASCIA SI DICE PER NOME E PER NUMERO, perche' e' il caso che si e' presentato:
    // «fascia «corta» piena, 12 candidati respinti» — chi legge non deve contare a mano una lista.
    slotVuotiPerScarsita: (() => {
      const tetto = slotTotaliN;
      const vuoti = Math.max(0, tetto - attiviFinali);
      if (vuoti === 0) return null;
      const campi = [];
      const parti = [];
      const perFasciaPiena = scartatiPerFascia.filter((x) => x.motivo === 'fascia-piena');
      const perFasciaIgnota = scartatiPerFascia.filter((x) => x.motivo === 'fascia-non-determinabile');
      if (perFasciaPiena.length) {
        const conta = new Map();
        for (const x of perFasciaPiena) conta.set(x.fascia, (conta.get(x.fascia) || 0) + 1);
        campi.push('fasce.scartatiPerFascia');
        parti.push(`gli scarti sono PER FASCIA: ${[...conta].map(([f, n]) => `fascia «${f}» piena, ${n} candidat${n === 1 ? 'o respinto' : 'i respinti'}`).join(' e ')}`
          + ` (in fasce.scartatiPerFascia)`);
      }
      if (perFasciaIgnota.length) {
        if (!campi.includes('fasce.scartatiPerFascia')) campi.push('fasce.scartatiPerFascia');
        parti.push(`${perFasciaIgnota.length} candidati con fascia non determinabile (in fasce.scartatiPerFascia)`);
      }
      if (scartatiPerComposizione.length) {
        campi.push('scartatiPerComposizione');
        parti.push(`${scartatiPerComposizione.length} scartati per la quota di scaglione (in scartatiPerComposizione)`);
      }
      if (postiNonAssegnati.length) {
        campi.push('postiNonAssegnati');
        parti.push(`posti di capitale senza candidati: ${postiNonAssegnati.map((x) => `${x.scaglione} ×${x.posti}`).join(', ')} (in postiNonAssegnati)`);
      }
      if ((scartatiPerCodaLunga || []).length) {
        campi.push('scartatiPerCodaLunga');
        parti.push(`${scartatiPerCodaLunga.length} tolti dalla quota di coda lunga (in scartatiPerCodaLunga)`);
      }
      if ((scartatiPerCodaLungaSottoPavimento || []).length) {
        campi.push('scartatiPerCodaLungaSottoPavimento');
        parti.push(`${scartatiPerCodaLungaSottoPavimento.length} sotto il pavimento premiante della coda lunga (in scartatiPerCodaLungaSottoPavimento)`);
      }
      const motivo = ammissibili <= attiviFinali
        ? `il board non offre abbastanza mercati ammissibili: ${ammissibili} su ${perId.size} valutati, e ${attiviFinali} sono gia' occupati`
        : (parti.length
          ? `restano ${vuoti} posti liberi con ${ammissibili} ammissibili — ${parti.join(' · ')}`
          : `restano ${vuoti} posti liberi con ${ammissibili} ammissibili e NESSUNA lista di scarto e' popolata: nessun campo di questo referto spiega questi posti`);
      return { quanti: vuoti, tetto, occupati: attiviFinali, ammissibili,
        // ⚠ `campi` E' LA VERSIONE MACCHINA DI `motivo`, e serve perche' un lettore automatico non
        // debba fare il parsing di una frase italiana per sapere dove guardare.
        campi,
        perFasciaPiena: perFasciaPiena.length
          ? [...perFasciaPiena.reduce((m, x) => m.set(x.fascia, (m.get(x.fascia) || 0) + 1), new Map())]
            .map(([fascia, quanti]) => ({ fascia, quanti }))
          : [],
        // Il travaso e' l'ultima cosa che ha provato a riempire questi posti: il suo esito appartiene
        // alla domanda «perche' sono ancora vuoti».
        travaso: travaso.riga.motivo,
        motivo };
    })(),
...
module.exports = {
  MIN_SIZE_MASSIMA, ORIZZONTE_MINIMO_ORE, ORIZZONTE_MINIMO_MS,
  LIMITE_SLOT, SECCHI_SCAGLIONE, esigiSlot,
  quotaScaglioni, scaglioneDi, categoriaDi,

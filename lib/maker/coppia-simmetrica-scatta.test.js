...
(async () => {
  ...
  // ── ⑧ SENZA RIGA NEL PIANO NON SI RICALCOLA (la regola che non deve regredire) ────────────────
  {
    const { r, sequenza } = await esegui({ ordiniVivi: [ordineVivo({ orderId: '0xORD6' })], riga: null, id: '0x111' });
    ok('⑧ nessuna riga nel piano ⇒ si dichiara e NON si ricalcola', r.tentato === false && sequenza.length === 0);
    ok('  ed e il motivo di sempre', /NON si ricalcola/.test(String(r.motivo)));
  }

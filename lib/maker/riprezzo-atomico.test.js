...
{
  // Un riprezzo vero che passava e deve continuare a passare: la gamba Houston del 9 agosto,
  // 66,3 share @ 0,43 = $28,51 (§5 punto 67), sotto il tetto di oggi.
  const g = MO.evaluateManualCapGate({ notionalUsd: 66.3 * 0.43, caps: capsFinti(C.LIVE_MIN_ORDER_CAP_USD) });
  ok('un riprezzo legittimo ($28,51 sotto il tetto) passa', g.allow === true, `$${(66.3 * 0.43).toFixed(2)}`);
}
{
...
}
{
  ...
  ok('  e il modulo continua a importare il tetto da lib/rewards/concentration',
    /LIVE_MIN_ORDER_CAP_USD\s*\}\s*=\s*require\(['"]\.\.\/rewards\/concentration['"]\)/.test(src));
}
...
}
{
  ...
  ok('  e allo STEP 2 si spedisce la stessa variabile già precontrollata',
    /idempotencyKey: chiaveRimpiazzo \}, deps\)/.test(src));
...

{
  ...
  for (const [nome, marcatore] of [
    ...
    ['mid stantio oltre soglia', "outcome: 'mid-stantio-timeout'"],
...
}
{
  ...
  const rules = {
    readable: true, missing: [], marketId: MKT, mid: 0.785, tick: 0.01, minSize: 50, maxSpreadCents: 4.5,

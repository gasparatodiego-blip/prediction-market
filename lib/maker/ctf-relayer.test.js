...
console.log('\n── 7 · IL NONCE: L\'`address` IN RISPOSTA È EFFIMERO, NON UN\'ECO');
(async () => {
  // Il vecchio test qui pretendeva che un `address` diverso dal nostro facesse SOLLEVARE. Misurato
  // contro il relayer vero il 7 agosto 2026: `address` è diverso a OGNI chiamata anche con
  // credenziali buone, quindi quella guardia sollevava sempre e rendeva il modulo ineseguibile.
  // Il `nonce` invece è vero. Adesso si verifica la forma, non un'eco che non esiste.
  {
    const r = await R.leggiNonce({
    ...
    ok('un address effimero NON fa sollevare: si legge il nonce lo stesso', r.nonce === '5', r.nonce);
    ok('e l\'address effimero viene restituito per finire a registro',
      r.indirizzoEffimero === '0x2d988c087c95596c3804e34b2b994aa758c7e185', r.indirizzoEffimero);
  }
  ...
  await sollevaAsync('un nonce assente fa sollevare',
    () => R.leggiNonce({ signer: SIGNER, cred: { key: 'k', address: SIGNER }, http: async () => ({ status: 200, json: { address: SIGNER }, testo: '' }) }), 'nonce non numerico');
  {
    const r = await R.leggiNonce({ signer: SIGNER, cred: { key: 'k', address: SIGNER }, http: async () => ({ status: 200, json: { address: SIGNER.toLowerCase(), nonce: '12' }, testo: '' }) });
    ok('il nonce buono passa', r.nonce === '12', r.nonce);
  }
  ...
  console.log('\n── 8 · L\'INTERRUTTORE SPENTO: NESSUNA FIRMA, NESSUNA RETE');
  {
    ...
    // Acceso il 7 agosto 2026 per la prova split/merge autorizzata. Il test non asserisce più un
    // valore fisso — asserisce che `abilitato: false` passato dal chiamante VINCE comunque, che è
    // la proprietà che serve davvero e che vale a interruttore acceso o spento.
    ok('con abilitato:false non si firma, qualunque sia CTF_RELAYER_ENABLED', res.eseguito === false);

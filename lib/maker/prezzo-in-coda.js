...
function prezzoInCoda({ book, side, rules, depth = null, ownOrders = [], offsetCents = null,
  ...
  // ── LA DISTANZA-OBIETTIVO DI QUESTO MERCATO, IN CENTESIMI (22 agosto 2026) ─────────────────────
  // `null`/assente ⇒ `planBehindBest` la legge dall'ambiente, cioe' il comportamento di sempre. E'
  // la stessa forma di `depthMultiple`: una configurazione PER MERCATO, risolta dal chiamante da
  // `offset-config` e passata qui, mai letta da questo modulo — che resta puro (nessun file).
  // ⚠ SI PASSA IN CENTESIMI E SI CONVERTE QUI IN FRAZIONE DELLA BANDA, perche' `planBehindBest`
  // ragiona in frazioni di `v` e il raggio della banda lo conosce solo questo livello: convertire
  // dal chiamante vorrebbe dire ricalcolare `raggioBandaCents` una seconda volta (reperto D1).
  distanzaObiettivoCents = null } = {}) {
  ...
  const piano = planBehindBest({
    ...
    // ⚠ SOLO SE E' UN NUMERO UTILIZZABILE: `undefined` significa «leggi dall'ambiente» e va lasciato
    // `undefined`, non trasformato in `null` — che in `planBehindBest` significa «manopola SPENTA».
    // Sono tre stati distinti, e confonderne due spegnerebbe la distanza-obiettivo su ogni ordine.
    distanzaObiettivoFrazione: (fin(distanzaObiettivoCents) && distanzaObiettivoCents > 0
      && fin(bandRadiusCents) && bandRadiusCents > 0)
      ? distanzaObiettivoCents / bandRadiusCents
      : undefined,
  });
  ...
  return {
    ...
    // La manopola della posizione (§5-bis p.158): `null` quando è spenta. Viaggia fino al referto
    // di piazzamento perché l'osservatore possa misurare l'effetto del test quando verrà attivata.
    distanzaObiettivo: piano.distanzaObiettivo || null,
    bestOther: bo.readable ? (vendita ? specchia(bo.price) : bo.price) : null,
    alone: bo.alone === true,

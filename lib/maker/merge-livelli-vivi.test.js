...
// Il tetto del secondo lato per il carico delle fixture, chiesto al modulo e non ricopiato.
const TETTO_L2 = require('./strategia-merge').tettoSecondoLato(0.80);
...
// ═══ 4 · IL TIMEOUT DEL LIVELLO 2 ESISTE DAVVERO ═══════════════════════════════════════════════════
// ⚠ Le due attese si prendono ATTORNO alla costante, non attorno a 60: il valore e' una manopola
// dell'operatore (30 minuti dal 15 agosto 2026) e un test che lo fotografa diventa rosso a ogni giro
// pur essendo il codice corretto (§5.3). Quello che va difeso e' che il timeout esista e morda.
console.log('\n4 · timeout del Livello 2');
{
  const t0 = 1_000_000;
  const TO = MERGE_WAIT_TIMEOUT_MIN;
  const dentro = decidiLivello({ ...posBase, asksAltroLato: [{ price: 0.9, size: 5 }], attesaDaMs: t0, now: t0 + (TO - 1) * 60_000 });
  const oltre  = decidiLivello({ ...posBase, asksAltroLato: [{ price: 0.9, size: 5 }], attesaDaMs: t0, now: t0 + (TO + 1) * 60_000 });
  ok(`a ${TO - 1} minuti si aspetta ancora (Livello 2)`, dentro.livello === 2, `livello ${dentro.livello}`);
  ok(`a ${TO + 1} minuti scatta il Livello 3`, oltre.livello === 3 && oltre.azione === 'auto-close', `livello ${oltre.livello}`);
  ok('  il motivo cita i minuti trascorsi', new RegExp(`${TO + 1} minuti`).test(oltre.motivo));
  ok('  attesaMin è un numero, non null', oltre.numeri.attesaMin === TO + 1);
  ok(`  il limite è ${MERGE_WAIT_TIMEOUT_MIN} minuti, ed è una costante positiva`,
...
async function ciclo({ asks, registro, now = 5_000_000, ordini = [], regoleExtra = {} }) {
  const piazzati = [];
  const cancellati = [];
...
(async () => {
  ...
  let attesaAperta = null;
  {
    ...
    const o = piazzati[0] || {};
    // ⚠ FORMA, NON SOSTANZA — 24 agosto 2026. `inCoda` era usato qui come PROXY di «maker / ordine che
    // riposa». Il Livello 2 fa esattamente quello che faceva rispetto a §7 gradini 1-2: un solo BUY
    // maker sul secondo lato, prezzo arrotondato GIU' al tetto `101¢ − carico`, sotto il miglior ask,
    // orologio avviato, orderId ricordato, e tentato nello STESSO ciclo dopo il rifiuto del Livello 1.
    // Cio' che e' cambiato e' che `auto-close.chiudendo` toglie `inCoda` all'intera classe delle
    // chiusure, perche' su un'uscita il prezzo lo decide la scala e non la coda. La maker-ness vera si
    // legge da `attraversaApposta` e dal prezzo sotto l'ask — entrambi gia' asseriti qui accanto — e
    // adesso si asserisce anche la marcatura, che e' cio' che rende quel prezzo intoccabile.
    ok('L2 · è un BUY maker sul secondo lato', o.side === 'BUY' && o.book === 'no' && o.attraversaApposta !== true);
    ok('  ed è marcato USCITA, quindi il suo prezzo non lo tocca nessun aggiustamento',
      o.uscita === true && o.inCoda === undefined, JSON.stringify({ uscita: o.uscita, inCoda: o.inCoda }));
    ok('L2 · NON attraversa lo spread', !o.attraversaApposta);
    // Il tetto si CHIEDE al modulo (carico 0,80 ⇒ 101 − 80 = 21¢ dal 15 agosto 2026, era 19¢).
    ok('L2 · prezzo arrotondato GIÙ al tetto', o.price === TETTO_L2, `${o.price} (tetto ${TETTO_L2})`);
  ...
  // ── 5c · attesa in corso: non si ripiazza niente ────────────────────────────────────────────────
  {
    ...
    const meno = MERGE_WAIT_TIMEOUT_MIN - 1;
    const { piazzati } = await ciclo({ asks: [{ price: 0.90, size: 100 }], registro: reg, now: 5_000_000 + meno * 60_000 });
    ok(`attesa · a ${meno} minuti (sotto il timeout) non piazza nulla`, piazzati.length === 0, `${piazzati.length}`);
  }
  ...
  // ── 5f-bis · CANCELLATORE NON INIETTATO: vale come cancellazione FALLITA, non come riuscita ─────
  // È il caso vero, non un'ipotesi: `closeTask` in agent40 non inietta `cancelOrder` (misurato l'8
  // agosto 2026), quindi in produzione questo ramo riceveva `null`. La guardia `c && c.ok === false`
  // lasciava passare quel `null` per «riuscita»: si ripuliva il registro e si vendeva il primo lato con
  // l'ordine di completamento ancora sul libro. Il ramo gemello della chiusura a mercato tratta `null`
  // come fallimento da sempre; questo test tiene le due guardie allineate.
  {
    const reg = registroFinto({ [attesaAperta[0]]: attesaAperta[1] });
    const piazzati = [];
    await runAutoCloseCycle({
      ...
      attesaMerge: reg,
      placeOrder: async (s) => { piazzati.push(s); return { ok: true, sent: true, orderId: 'x' }; },
      // cancelOrder DELIBERATAMENTE ASSENTE — è la forma esatta della produzione.
      audit: () => {},
    });
    ok('senza cancellatore · NON vende', piazzati.length === 0,
      'un cancellatore assente non è una cancellazione riuscita');
    ok('senza cancellatore · l\'attesa NON viene ripulita', reg._m.size === 1,
      'ripulirla farebbe ripiazzare un secondo completamento al giro dopo');
  }
  ...
  console.log('\n5h · degradazione L1 → L2 quando il BUY aggressivo è rifiutato');
  {
    ...
    // ⚠ FORMA: la sostanza e' «due tentativi, taker poi maker, nello stesso ciclo» — e si legge da
    // `attraversaApposta`, che e' cio' che distingue davvero un taker da un maker. `inCoda` non lo
    // distingueva: era un flag di politica di prezzo, e sulle chiusure non c'e' piu'.
    ok('rifiutato, ha tentato il Livello 2 nello STESSO ciclo',
      visti.length === 2 && visti[1].side === 'BUY' && visti[1].attraversaApposta !== true,
      JSON.stringify(visti.map((v) => `${v.side}/${v.attraversaApposta ? 'taker' : 'maker'}`)));
    ok('  e il tentativo di Livello 2 e\' marcato USCITA', visti[1] && visti[1].uscita === true);
  ...
  {
    const reg = registroFinto();
    // Miglior ask del secondo lato mezzo centesimo SOPRA il tetto: sopra il tetto (quindi Livello 2,
    // non 1), ma cosi' vicino che prezzare AL tetto lascerebbe l'ordine a un tick dall'incrocio. Deve
    // scendere. L'ask si deriva dal tetto invece di essere scritto a mano, o cambiando il tetto la
    // fixture finirebbe dalla parte sbagliata e proverebbe il Livello 1.
    const askVicino = +(TETTO_L2 + 0.005).toFixed(6);
    const { piazzati } = await ciclo({ asks: [{ price: askVicino, size: 100 }], registro: reg });
    const o = piazzati.find((p) => p.side === 'BUY') || {};
    ok('prezza sotto il miglior ask, non sul tetto', o.price === +(TETTO_L2 - 0.01).toFixed(6),
      `${o.price} (tetto ${TETTO_L2}, ask ${askVicino})`);
    // ⚠ FORMA: «riposa» si dimostra col prezzo sotto il miglior ask (asserito qui sopra) e con
    // `attraversaApposta` falso, non con `inCoda`.
    ok('  e resta un ordine che riposa', o.attraversaApposta !== true && o.uscita === true,
      JSON.stringify({ attraversa: o.attraversaApposta, uscita: o.uscita }));
  }
  ...
  console.log('\n5g · agent40 inietta davvero le dipendenze del merge');
  {
    ...
    // Dal 12 agosto il registro si costruisce UNA volta per ciclo in una variabile, invece che inline:
    // la pulizia di un mercato morto deve toccare lo STESSO oggetto, e due istanze scriverebbero lo
    // stesso file da due letture diverse. La proprieta' difesa non cambia — closeTask riceve il
    // registro delle attese, ed e' quello su disco — quindi si accetta anche la forma per variabile.
    ok('closeTask inietta il registro delle attese',
      /attesaMerge:\s*(registroAttesaMerge\(\)|regMergeCiclo)/.test(blocco));
    ok('  e quel registro e\' costruito una volta per giro, prima del ciclo',
      /const regMergeCiclo = registroAttesaMerge\(\);[\s\S]{0,400}?runAutoCloseCycle\(/.test(src)
      || /attesaMerge:\s*registroAttesaMerge\(\)/.test(blocco));
...
{
  const { piazzati } = await ciclo({ asks: [{ price: 0.90, size: 100 }], registro: registroFinto(), regoleExtra: { midAgeSec: undefined } });
  const sell = piazzati.filter((x) => x && x.side === 'SELL');
  ok('gate: senza eta del book nessuna vendita', sell.length === 0,
    `${sell.length} SELL su ${piazzati.length}`);
  ...
  const sano = await ciclo({ asks: [{ price: 0.90, size: 100 }], registro: registroFinto() });
  ok('  CONTROLLO: con un book databile lo stesso scenario agisce', sano.piazzati.length > 0,
  ...
  const c = await ciclo({ asks: [{ price: 0.90, size: 100 }], registro: registroFinto() });

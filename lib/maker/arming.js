...
/**
 * PER-CYCLE INVARIANT RE-CHECK — the auto-disarm the maker runs every cycle (not only at arming). Reads the
 * record (enforcing TTL), then disarms if a preflight condition has gone false WHILE armed, the collateral
 * ceiling is breached, or the heartbeat failed. Each transition disarms with a distinct, audited reason.
 * @param {object} args
 *   preflight        the latest (re-run) preflight result — go:false ⇒ a check went red while armed.
 *   collateralUsedUsd current collateral in use ⇒ disarm if it exceeds the record's cap.
 *   heartbeatOk      false ⇒ the maker heartbeat failed ⇒ disarm.
 * @returns {{armed:boolean, disarmedReason:(string|null)}}
 */
function checkArmedInvariants({ preflight = null, collateralUsedUsd = null, heartbeatOk = true } = {}, deps = {}) {
  const cur = readArming(deps); // enforces TTL first — a just-expired record is already disarmed + audited
  if (!cur.armed) return { armed: false, disarmedReason: cur.source === 'ttl-expiry' ? 'ttl-expiry' : null };
  const rec = cur.record;
  // 1. heartbeat failed → stand down (the same signal the dead-man watchdog keys on).
  if (heartbeatOk === false) { disarm('heartbeat-failed', deps); return { armed: false, disarmedReason: 'heartbeat-failed' }; }
  // 2. collateral ceiling breached (position/exposure grew past the cap the arm was granted under).
  if (rec.collateralCapUsd != null && Number.isFinite(collateralUsedUsd) && collateralUsedUsd > rec.collateralCapUsd + 1e-9) {
    disarm('collateral-cap-exceeded', deps); return { armed: false, disarmedReason: 'collateral-cap-exceeded' };
  }
  // 3. a preflight condition went red while armed (balance dropped, approval revoked, cancel path down, …).
  if (preflight && preflight.go === false) {
    const red = (preflight.checks || []).filter((c) => !c.pass).map((c) => c.key).join(',') || 'unknown';
    disarm('preflight-red:' + red, deps); return { armed: false, disarmedReason: 'preflight-red:' + red };
  }
  return { armed: true, disarmedReason: null };
}
...
module.exports = { readArming, arm, renew, disarm, checkArmedInvariants, appendArmingAudit, STATE_FILE, AUDIT_FILE, TTL_DEFAULT_SECONDS, TTL_MAX_SECONDS, EMPTY };

...
// ⚠ DUE VALORI, E LA DISTINZIONE E' IL PUNTO (15 agosto 2026, chiude §5.2 p.31).
//   · `VALORE` e' la frazione con cui si prova l'ARITMETICA. Resta 0,444 perche' e' un valore comodo
//     per leggere i numeri a mano, e perche' le proprieta' che i blocchi 1-4 difendono (dentro banda,
//     mai davanti a nessuno, scala con v) non dipendono da quale frazione si sceglie — il blocco 2 le
//     verifica su otto frazioni diverse, dallo 0,444 all'assurdo.
//   · `VALORE_IN_SERVIZIO` e' quello che l'ecosystem dichiara davvero, e il blocco 5 controlla QUELLO.
//     Dal 15 agosto 2026 vale **0,95**: l'operatore ha chiesto gli ordini al BORDO ESTERNO della banda
//     premiante, e 0,95 e' la frazione che lo chiede. Il test lo dichiarava ancora 0,444 ed era rosso
//     per questo, senza nessun difetto nel codice.
// A decidere dove l'ordine si ferma davvero e' comunque il MARGINE DAL BORDO
// (`distanza-obiettivo.bordiConMargine`, 1 tick di difetto), che vive nel CODICE e non nell'ecosystem
// — quindi i due processi che decidono un prezzo non possono divergere su di lui.
const VALORE = '0.444';
const VALORE_IN_SERVIZIO = '0.95';
...
console.log('\n══ 2 · IL PALETTO — mai oltre il bordo premiante, per QUALUNQUE frazione');
{
  ...
  ok('  con il motivo che dichiara il bordo raggiunto', /fermato a/.test(rAssurdo.motivo), rAssurdo.motivo);
  ok('  e non e\' MAI fuori banda, che e\' la proprieta\' vera',
    rAssurdo.prezzo >= b.lo - 1e-12 && rAssurdo.prezzo <= b.hi + 1e-12);
...
console.log('\n══ 6 · IL VALORE È LO STESSO SU OGNI PROCESSO CHE DECIDE UN PREZZO');
//
// ⚠ RISCRITTA il 15 agosto 2026. Questa sezione elencava TRE nomi scritti a mano — agent41, agent40 e
// `dashboard` — e il terzo non esiste più: l'operatore ha tolto il pannello dalla flotta perché le
// decisioni si prendono da terminale (`scripts/cli/`).
//
// La proprietà difesa NON è «quei tre nomi dichiarano il valore»: è **«nessun processo che decide un
// prezzo diverge dagli altri»**, che è la classe D1. Un elenco scritto a mano difende quella proprietà
// solo finché qualcuno si ricorda di aggiornarlo — e un processo NUOVO che decidesse un prezzo senza
// dichiarare la manopola passerebbe inosservato. Quindi l'insieme si DERIVA dalla flotta vera.
{
  const cfg = require(path.join(ROOT, 'agents', 'ecosystem.config.js'));
  const apps = cfg.apps || [];

  // I processi che decidono un prezzo sono quelli che passano da `prezzoInCoda`/`planBehindBest`. Si
  // riconoscono dal loro script, non da un elenco di nomi: chi importa il motore di prezzo lo dichiara
  // nel proprio albero dei require, e qui basta il file di partenza.
  const DECIDE_UN_PREZZO = /agent41-realloc-scheduler|agent40-manual-reprice/;
  const decisori = apps.filter((a) => typeof a.script === 'string' && DECIDE_UN_PREZZO.test(a.script));

  ok('la flotta contiene almeno due processi che decidono un prezzo', decisori.length >= 2,
    `${decisori.length}: ${decisori.map((a) => a.name).join(', ')}`);

  const valori = decisori.map((a) => ({ n: a.name, v: a.env ? a.env[D.ENV_FRAZIONE] : undefined }));
  for (const { n, v } of valori) ok(`${n} dichiara la manopola`, v === VALORE_IN_SERVIZIO, String(v));
  ok('  e TUTTI i valori COINCIDONO (una divergenza qui è la classe D1)',
    new Set(valori.map((x) => x.v)).size === 1);
  ok(`  e il valore dichiarato, letto dalla funzione vera, vale ${VALORE_IN_SERVIZIO}`,
    D.leggiFrazione({ [D.ENV_FRAZIONE]: valori[0].v }) === Number(VALORE_IN_SERVIZIO));
  // ⚠ E IL MARGINE DAL BORDO NON E' DICHIARATO IN NESSUN BLOCCO, ed e' voluto: se lo fosse, un riavvio
  // scoordinato dei due processi produrrebbe due prezzi diversi — la stessa trappola che la manopola
  // qui sopra porta con se'. Vivendo nel codice, entrambi leggono lo stesso difetto.
  ok('  il margine dal bordo NON e\' un env dichiarato: vive nel codice, quindi non puo\' divergere',
    decisori.every((a) => !a.env || a.env[D.ENV_MARGINE_BORDO] === undefined));
  ok('    e il suo difetto e\' un margine VERO, non zero', D.leggiMargineBordoTick({}) >= 1);

  // ── E IL PANNELLO NON DEVE ESSERE UN PROCESSO ────────────────────────────────────────────────────
  // Decisione dell'operatore del 15 agosto 2026. Va asserita, non lasciata implicita: se un domani il
  // blocco `dashboard` tornasse nella flotta senza la manopola nel suo `env`, il rinnovo riporterebbe
  // gli ordini alla distanza di prima e nessuno se ne accorgerebbe — è esattamente la divergenza che
  // questa sezione esiste per impedire.
  ok('il dashboard NON è nella flotta: le decisioni si prendono da terminale',
    !apps.some((a) => a.name === 'dashboard'),
    apps.map((a) => a.name).join(', '));
  ok('…e nemmeno fra i processi critici da tenere sempre vivi',
    !(cfg.PROCESSI_CRITICI || []).includes('dashboard'));
}

...
// The two position guards (a SELL needs inventory; BUY YES + BUY NO at or above $1 is a self-match).
const { applyPositionGuards } = require('./inventory-guard');
 ...
 */
function planQuotes({ legs, mid, maxSpreadC, minSize, tick, tokenId, tokenIdNo, defaultSizeShares = 0, balances }) {
  const bandRadiusC = maxSpreadC > 0 ? raggioBandaCents(maxSpreadC) : null;
  ...
  // ── POSITION GUARDS, BEFORE ANYTHING COUNTS AS POSTABLE ────────────────────────────────────────────
  // A SELL delivers an ERC-1155 token you must already own, and a BUY YES + BUY NO priced at or above
  // $1 is the operator crossing with themselves. Both are decided HERE, in the construction chain, so a
  // blocked leg is genuinely not postable rather than merely greyed out somewhere. `balances` are REAL
  // measured share counts; omitted or null ⇒ unreadable ⇒ every SELL is blocked (fail closed).
  const guarded = applyPositionGuards({ quotes, balances });
  quotes.length = 0;
  quotes.push(...guarded.quotes);
  ...
  return {
    ...
    market: {
      ...
      collapsedNote: canon.collapsed.length
        ...
        : null,
      // What the position guards refused, and why — externally observable, never a silent drop.
      sellBlocks: guarded.sellBlocks,
      selfMatches: guarded.selfMatches,
      guardBlockedCount: guarded.blockedCount,

...
console.log('\n══ 8 · LA SOPPRESSIONE NON PUÒ OSCURARE UN RINNOVO DOVUTO');
{
  // IL RISCHIO CHE QUESTA CORREZIONE PORTA CON SÉ, e che è esattamente il difetto che precede. Prima,
  // su Barlow, l'inseguimento riprezzava — e un riprezzo conia un ordine con una finestra piena, quindi
  // rinnovava l'orologio del venue come effetto collaterale. Sopprimendolo, quell'effetto sparisce: se la
  // soppressione tornasse un `hold` con la scadenza dentro il margine, il ramo del rinnovo (che vive più
  // in basso) non verrebbe raggiunto e l'ordine morirebbe — lo stesso esito del 5 agosto, da un'altra
  // porta. Quindi con la scadenza addosso si prosegue fino al rinnovo.
  const d = decidi({
    rules: BARLOW, order: { ...GAMBA, secondsToExpiry: 120 },
    targetOffsetCents: 0.55, minMoveCents: 0.1, bids: CONCORRENTE,
  });
  ok('scadenza dentro il margine ⇒ si RINNOVA', d.action === 'reprice', `${d.action}/${d.gate}`);
  ok('  col trigger del rinnovo', d.gate === 'expiry-refresh', String(d.gate));
  ok('  allo STESSO prezzo: la soppressione non diventa un movimento',
    d.targetPrice === 0.649, String(d.targetPrice));
  ok('  e il rinnovo dichiara che l inseguimento era stato soppresso',
    d.railInseguimento === 'inseguimento-contro-mai-primo', String(d.railInseguimento));
  ok('  dicendolo anche a parole', /inseguimento del mid in questo stesso ciclo è stato fermato/.test(d.reason));
}
{
  // E con la scadenza lontana la soppressione resta la risposta: non si rinnova per il gusto di agire.
  const d = decidi({
    rules: BARLOW, order: { ...GAMBA, secondsToExpiry: 1345 },
    targetOffsetCents: 0.55, minMoveCents: 0.1, bids: CONCORRENTE,
  });
  ok('scadenza lontana ⇒ resta un hold soppresso', d.action === 'hold' && d.gate === 'inseguimento-contro-mai-primo',
    `${d.action}/${d.gate}`);
}

console.log('\n══ 9 · IL CICLO LO SCRIVE NEL REGISTRO — LA TRANSIZIONE, NON OGNI CICLO');
(async () => {

...
// ═══ IL FILTRO DISTANZA ══════════════════════════════════════════════════════════════════════════════
console.log('\n── distanza dal mid e righe bloccate');
{
  ok('0.21 dista 1.5c da 0.225', B.distanceCents(0.21, 0.225) === 1.5, String(B.distanceCents(0.21, 0.225)));
  ok('soglia 2c blocca una riga a 1.5c', B.levelBlocked(0.21, 0.225, 2) === true);
  ok('soglia 2c non blocca una riga a 2.5c', B.levelBlocked(0.20, 0.225, 2) === false);
  ok('sul bordo esatto la riga NON e bloccata', B.levelBlocked(0.205, 0.225, 2) === false, '2.00c = soglia');
  ok('soglia 0 non blocca niente', B.levelBlocked(0.2249, 0.225, 0) === false);
  ok('senza mid non si blocca nulla', B.levelBlocked(0.21, null, 2) === false);
  // Simmetria: la distanza e un valore assoluto, quindi il filtro morde uguale sopra e sotto il mid.
  ok('la soglia agisce simmetricamente sui due lati',
    B.levelBlocked(0.24, 0.225, 2) === B.levelBlocked(0.21, 0.225, 2));
}

 ...
 */
// ── 0,90 → 0,95 IL 13 AGOSTO 2026, decisione dell'operatore ───────────────────────────────────────
// E il numero vive QUI e in nessun altro posto. `capitale-al-lavoro` dichiara un proprio
// `OBIETTIVO_DEFAULT = 0.95` come ripiego, ma interroga PRIMA questa funzione — che rispondeva 0,90 e
// vinceva. Effetto misurato il 12 agosto alle 23:54: il modulo nuovo era stato scritto per il 95% e il
// pannello stampava «obiettivo 90%». Due difetti per la stessa grandezza sono una divergenza che
// aspetta solo di essere letta da qualcuno.
function leggiTarget(env = process.env) {
  const v = Number(env.MAKER_TARGET_UTILIZZO);
  return fin(v) && v > 0 && v <= 1 ? v : 0.95;
}
...
//
// ═══ 6 → 12, IL 12 AGOSTO 2026, E IL NUMERO VIENE DA UNA MISURA ═════════════════════════════════════
// Il 6 era tarato su «sette righe del piano tipico», cioe' su un tetto per mercato che allora valeva il
// 20% del capitale. Con il tetto derivato da `f_min` ($32,67, allocato ~$26) le righe del piano tipico
// sono 17-18 e i mercati necessari a coprire il capitale sono ~20: 6 per giro non e' piu' «appena
// sopra» il piano, e' meno di un terzo. Misurato sul mini-ciclo delle 20:57:26Z del 12 agosto:
//     motivoStop: «tetto di 6 mercati per giro raggiunto» · allocatoUsd 156 · residuoUsd 331,67
// cioe' il tetto lasciava fermo META' del capitale con 17 righe disponibili sul tavolo.
//
// 12 e non 20: due giri bastano a coprire il capitale da zero (24 mercati, sopra i ~20 necessari) e il
// limite continua a fare il lavoro per cui esiste — un errore di piano diventa un errore su meta' del
// conto, non su tutto, e c'e' un giro per accorgersene. Con la cadenza operativa di 10 minuti il pieno
// si raggiunge in venti minuti invece di un'ora.
//
// ⚠ IL VINCOLO CHE LO LEGA AL RATE LIMIT, e va tenuto: un giro da N mercati chiede `2N` invii nella
// stessa finestra da 60s, e quella finestra e' CONDIVISA con la corsia di rinnovo. `maxOrdersPerWindow`
// deve quindi stare sopra `2N` con margine per i rinnovi — a 12 mercati sono 24 invii contro un tetto
// di 40. Se questo numero risale, quello va risalito con lui: e' la relazione che il 12 agosto ha
// prodotto 36 `skip-rate-limited` e 9 ordini `scaduto-senza-rinnovo`.
//
// QUESTO E' L'UNICO PUNTO in cui il numero vive: `trigger-capitale-fermo` lo IMPORTA invece di
// ridichiararlo. Due copie dello stesso raggio d'azione sono il reperto che il rilevatore D1 cerca, e
// qui la divergenza vorrebbe dire «il giro ne apre 12 e il tetto ne ammette 6» senza che nulla lo dica.
// ⚠ 12 → 10 IL 13 AGOSTO 2026, e il numero non è arbitrario: col tetto per mercato a $61,25 un giro
// pieno da 12 chiedeva `12 × $61,25 = $735` contro il capitale che c'era, e la coerenza
// `mercatiPerGiro × tetto ≤ capitale` — difesa da `tetti-per-giro-e-scope.test.js` — era rotta.
// 10 è `mercatiSostenibili($652)`, cioè il numero che il tetto e il capitale ammettono insieme.
// ⚠ Col capitale salito a $2.150 la coerenza reggerebbe anche a 12 (12 × $61,25 = $735 ≪ $2.150):
// 10 resta per decisione dell'operatore, non per necessità aritmetica, e va saputo.
function leggiMaxNuoviPerGiro(env = process.env) {
  const v = Number(env.MAKER_MAX_NUOVI_PER_GIRO);
  return fin(v) && v >= 1 ? Math.floor(v) : 10;
}

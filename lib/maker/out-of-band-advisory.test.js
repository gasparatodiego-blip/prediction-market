...
console.log('\n── il cablaggio, punto per punto (lettura del sorgente)');
{
  ...
  // ⚠ SI DIFENDE LA PROPRIETA', NON L'ESPRESSIONE — riscritto il 24 agosto 2026.
  // Questa riga fotografava il letterale `splitVerdict(split, {...})`, cioe' anche il NOME della
  // variabile intermedia: e' diventata rossa quando il guard ha smesso di avere quella variabile,
  // senza che nulla della proprieta' fosse cambiato. E' la classe «test che fotografa il codice invece
  // della proprieta'» di §5.3, terza occorrenza. La proprieta' vera e' doppia e si asserisce doppia:
  // il verdetto nasce dalla funzione CONDIVISA (non da un filtro locale), e la deroga che lo declassa
  // e' quella del CHIAMANTE letta con `=== true` (non una truthiness, non un difetto locale).
  // ⚠ I COMMENTI SI FILTRANO: un commento che nomina la funzione avrebbe fatto passare l'asserzione
  // senza che il codice la chiamasse (§5.3).
  const adCodice = ad.split('\n').map((r) => r.replace(/\/\/.*$/, '')).join('\n');
  ok('l adapter usa la STESSA funzione, non un filtro suo',
    /\bvalidateQuote\(\s*s\.venueRules/.test(adCodice) && /\bsplitVerdict\(/.test(adCodice)
    && /require\('\.\.\/\.\.\/maker\/venue-rules'\)/.test(adCodice));
  ok('  e la deroga che declassa e quella del CHIAMANTE, letta con === true',
    /allowOutOfBand:\s*s\.allowOutOfBand === true/.test(adCodice));

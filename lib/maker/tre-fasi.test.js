...
console.log('\n══ FASE 3 · «UN TICK DIETRO AL MIGLIORE» E IL CONFLITTO CON LA BANDA');
{
  ...
  // Banda stretta: un tick dietro cadrebbe FUORI. La priorità è restare premianti.
  const stretto = planBehindBest({ bestOther: 0.42, tick: 0.01, scoringMid: 0.43, bandRadiusCents: 1.0 });
  ok('con banda stretta il prezzo viene RIPORTATO al bordo premiante',
    stretto.ok === true && stretto.mode === 'band-clamped', `${stretto.price} · ${stretto.mode}`);
  ok('  la banda VINCE su «un tick dietro»', stretto.price >= 0.42 - 1e-9, `${stretto.price} contro miglior bid 0.42`);
  ok('  e il conflitto e dichiarato, non nascosto',
    /fuori banda/.test(stretto.reason) && /IN CIMA al book/.test(stretto.reason), stretto.reason.slice(0, 100));
  ok('  con onTop=true, cosi chi legge sa che siamo primi e perche', stretto.onTop === true);

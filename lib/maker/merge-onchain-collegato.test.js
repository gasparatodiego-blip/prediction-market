...
console.log('\n── 4 · LO STATO DI OGGI: IL FLAG È SPENTO E NON PARTE NIENTE');
{
  const R = require('./ctf-relayer');
  ok('CTF_RELAYER_ENABLED è false', R.CTF_RELAYER_ENABLED === false);
  // Il percorso VERO (senza mergeOnChain iniettato) con il flag spento: `esegui` non firma e non invia.
  // Si passano credenziali finte per arrivare fino al ramo dell'interruttore senza toccare la rete.
  ...
  let rete = 0;
  R.mergePosition(ID, 21.18, { negRisk: true, deps: { env, http: () => { rete += 1; throw new Error('la rete non deve essere toccata'); } } })
    .then((res) => {
  ...
  R.mergePosition(ID, 21.18, { negRisk: false, deps: { env, http: () => { throw new Error('mai'); } } })
    .then((res) => {

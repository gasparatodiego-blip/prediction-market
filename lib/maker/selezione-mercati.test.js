...
titolo('3-ter · il soffitto in servizio');
{
  ...
  // Questa e' la cosa che, se saltasse, farebbe un danno; il numero da solo no.
  {
    ...
    const espMax = CONC.esposizioneMassimaRaggiungibileUsd(SEL.MAX_MERCATI_CONTEMPORANEI);
    let capVers = null;
    try {
    ...
    ok(`il soffitto in servizio e' ${SEL.MAX_MERCATI_CONTEMPORANEI} ed e' un intero >= 1`,
      Number.isInteger(SEL.MAX_MERCATI_CONTEMPORANEI) && SEL.MAX_MERCATI_CONTEMPORANEI >= 1);
  ...
  const q = SEL.quotaScaglioni(SEL.MAX_MERCATI_CONTEMPORANEI);
  const posti = q.reduce((a, b) => a + b.posti, 0);
  ok('  e la quota offre esattamente quel numero di posti, non meno',
    posti === SEL.MAX_MERCATI_CONTEMPORANEI, `posti ${posti}`);
  ok('  con un solo posto allo scaglione «basso», il resto «alto»',
    q.find((x) => x.chiave === 'basso').posti === 1
    && q.find((x) => x.chiave === 'alto').posti === SEL.MAX_MERCATI_CONTEMPORANEI - 1);
  // Un valore oltre il soffitto non alza l'esposizione: viene clampato.
  ok('  e un `max` oltre il soffitto viene clampato, mai onorato',
    SEL.quotaScaglioni(SEL.MAX_MERCATI_CONTEMPORANEI + 3).reduce((a, b) => a + b.posti, 0)
      === SEL.MAX_MERCATI_CONTEMPORANEI);
  ...
  const richiesto = SEL.MAX_MERCATI_CONTEMPORANEI * capMercato;
  ok(`  ${SEL.MAX_MERCATI_CONTEMPORANEI} mercati chiedono $${richiesto.toFixed(2)} di ordini a riposo`,
    richiesto > 0);
  let tetto = null;
...
titolo('R1 \u00b7 un numero solo, e la quota lo deriva');
{
  ...
  ok(`max oltre il soffitto si clampa a ${SEL.MAX_MERCATI_CONTEMPORANEI}`,
    SEL.quotaScaglioni(99).reduce((a, b) => a + b.posti, 0) === SEL.MAX_MERCATI_CONTEMPORANEI);
  for (const cattivo of [0, -1, null, undefined, NaN, 'due']) {
    ok(`max "${String(cattivo)}" \u21d2 difetto, mai zero posti`,
      SEL.quotaScaglioni(cattivo).reduce((a, b) => a + b.posti, 0) === SEL.MAX_MERCATI_CONTEMPORANEI);
  }
  ...
  // qualcuno ci riscrivesse un letterale, questa asserzione cadrebbe.
  {
    ...
    ok('`quanti-mercati` deriva difetto e massimo da MAX_MERCATI_CONTEMPORANEI',
      Q.QUANTI_DI_DIFETTO === SEL.MAX_MERCATI_CONTEMPORANEI && Q.QUANTI_MASSIMO === SEL.MAX_MERCATI_CONTEMPORANEI);

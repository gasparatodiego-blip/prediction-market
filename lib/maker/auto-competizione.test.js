...
// Alias locale, usato nella sezione 3. Stessa funzione: serve solo a tenere le righe corte.
const place = placeManualOrder;
...
(async () => {
  ...
  console.log('\n══ 3 · CHI LI PASSA CONTINUA A DECIDERE LUI — MA SOLO SE PUÒ DICHIARARLI AUTOREVOLI');
  {
    // ⚠ RISCRITTA IL 12 AGOSTO 2026, E LA PROPRIETÀ DIFESA È CAMBIATA IN MEGLIO.
    // Prima: «se il chiamante passa `ownOrders`, il server non legge». Quella regola risparmiava una
    // chiamata al watcher di riprezzo — che è giusto — ma valeva anche per il PANNELLO, cioè per un
    // client: una lista incompleta arrivata dal browser diventava l'unica verità, e i nostri ordini
    // mancanti finivano contati come concorrenza.
    // Adesso il risparmio esiste solo per chi DICHIARA la lista autorevole, e il pannello non può
    // farlo — il campo non è nello schema zod della sua rotta, e zod scarta ciò che non conosce.
    const tok = [{ orderId: 'A', price: 0.79, size: 20 }, { orderId: 'B', price: 0.78, size: 20 }];

    let chiamato = false;
    const r = await place(
      { marketId: MKT, book: 'yes', side: 'BUY', price: 0.78, size: 20, inCoda: true,
        ownOrders: tok, ownOrdersAuthoritative: true },
      mondo({ resolveOwnOrders: async () => { chiamato = true; return { ok: true, orders: [] }; } }));
    ok('lista dichiarata AUTOREVOLE ⇒ NON si interroga il venue', chiamato === false,
      'il watcher di riprezzo li ha già, e una lettura in più sarebbe una chiamata sprecata a ogni riprezzo');
    ...
    // ── LA METÀ NUOVA: SENZA LA DICHIARAZIONE, IL SERVER LEGGE COMUNQUE ─────────────────────────
    let chiamato2 = false;
    const r2 = await place(
      { marketId: MKT, book: 'yes', side: 'BUY', price: 0.78, size: 20, inCoda: true, ownOrders: tok },
      mondo({ resolveOwnOrders: async () => { chiamato2 = true; return { ok: true, orders: [] }; } }));
    ok('SENZA la dichiarazione il server legge comunque e UNISCE', chiamato2 === true);
    ok('  e il referto distingue le due provenienze',
      /unione:/.test(r2.inCoda.ownOrders.origine) && r2.inCoda.ownOrders.dalPannello === 2,
      r2.inCoda.ownOrders.origine);
    ok('  quindi un client che mente in DIFETTO non può nascondere un nostro ordine',
      r2.inCoda.ownOrders.conteggio >= 2);
  }
  ...
  console.log('\n══ 4 · UNA LETTURA CHE FALLISCE NON BLOCCA IL PIAZZAMENTO — ma si vede');
  {
    ...
    ok('  e il referto dice perché', /NON letto.*venue irraggiungibile/.test(r.inCoda.ownOrders.origine),
      r.inCoda.ownOrders.origine);
    ok('  dichiarando che la lettura del venue non è riuscita', r.inCoda.ownOrders.venueLetto === false);
  ...
  console.log('\n══ 5 · IL MOTORE DI DECISIONE RESTA COLLEGATO — su ENTRAMBI i rami');
  {
    ...
    // ⚠ AGGIORNATA IL 12 AGOSTO 2026: il filtro scritto a mano è diventato una chiamata alla funzione
    // CONDIVISA con il percorso di piazzamento. «Uno solo, con un nome» resta la proprietà difesa; in
    // più adesso quel nome lo produce lo stesso codice che usa l'altro percorso, quindi i due non
    // possono più vedere due book altrui diversi.
    ...
    ok('  e viene dalla funzione condivisa', ar.includes("require('./nostri-ordini')"));

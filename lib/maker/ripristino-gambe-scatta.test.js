...
async function giro({ ordini = [], righePiano = [RIGA], board = [RIGA_BOARD], piazza = null,
  idsAttivi = [MKT], pianoFresco = null } = {}) {
  const inviati = [];
  const cancellati = [];
  const esito = await A41.riconciliaCopertura({
    ...
    // ⚠ IL BANCO DEVE RESTARE ERMETICO. Dal 20 agosto `ripristinaGamba` ricalcola il piano quando la
    // riga manca (§25): senza questa iniezione il percorso di DIFETTO fa partire un `pianoLeggero`
    // VERO — un processo figlio da 13-22 s sul board vivo — e tocca lo stato di produzione. Il difetto
    // si e' visto: la suite ha segnalato `maker-auto-reprice.json` e `maker-manual-mode.json` toccati.
    // `eseguito: false` e' anche semanticamente giusto: in questo banco nessun ricalcolo avviene,
    // quindi la scala di raffreddamento (§26) non deve salire per causa sua.
    pianoFresco: pianoFresco || (async () => ({ eseguito: false, errore: 'banco: nessun ricalcolo disponibile' })),
  });
...
(async () => {
  ...
  sez('⑤ IL MERCATO NON E NEL PIANO SALVATO: si RICALCOLA, e se non basta si dice la causa vera');
  {
    pulisci();
    // ⚠ RIBALTAMENTO CONSAPEVOLE DEL 20 AGOSTO. Qui c'era «si dichiara e NON si ricalcola», scritta
    // il 17 agosto come «la differenza esatta con le 799 ricostruzioni del 16» (§5-bis p.171). Il
    // motivo di allora era giusto e il contenimento mancava; adesso c'e' — la scala di
    // raffreddamento di `ripristino-gambe` decide SE tentare e il ricalcolo sta DIETRO di lei, quindi
    // il caso peggiore e' un tentativo ogni 30 min (48/giorno) e non 720. Il difetto che il vecchio
    // comportamento produceva e' misurato: mercati SELEZIONATI e «quotabili adesso» restavano senza
    // un solo ordine perche' il piano su disco aveva 21 ore. Diagnosi e prove: APERTI.md §25 e
    // `ripristino-ricalcola.test.js`.
    // In questo banco non c'e' un piano fresco da offrire, quindi cio' che si difende qui e' che
    // l'esito NON sia piu' un'azione al buio e che il motivo dica PERCHE'.
    const g = await giro({ ordini: [ordine(TOK_YES)], righePiano: [] });
    ok('zero invii', g.inviati.length === 0, `${g.inviati.length}`);
    ok('  e il motivo non dice piu\' «manca dal piano»: dice cosa ha risposto il ricalcolo',
      !/NON si ricalcola/.test((g.esito.ripristini[0] || {}).motivo || ''), (g.esito.ripristini[0] || {}).motivo);
  }

...
/**
 * Resolve a repo-relative path. Under plain node __dirname is lib/, but Next bundles
 * this module into each route's server output where __dirname points somewhere else
 * entirely, so the project root is tried as well. Both candidates name the SAME file.
 */
function resolveRepoFile(...segments) {
  ...
  // Return the first candidate so the caller's own error names a real path.
  return candidates[0];
}

const FEES_FILE  = resolveRepoFile('data', 'venue-fees-official.json');

...
// ── LA CARTELLA `data/` SI CHIEDE, NON SI CERCA A TENTATIVI ────────────────────────────────────────
// Qui viveva `resolveRepoFile`, un risolutore fatto in casa: provava `__dirname/..` e poi
// `process.cwd()`, e FUNZIONAVA — il ripiego sul cwd copre proprio il caso del bundle di Next, dove
// `__dirname` è `.next/server/…`. Non era rotto. Era però la TERZA implementazione della stessa cosa
// in questo repo, e la quarta è quella che il 5 agosto 2026 ha bloccato ogni piazzamento: il modulo
// dello snapshot delle posizioni si era calcolato la cartella con i «..» e senza ripiego, e la
// dashboard leggeva in `.next/data/` mentre agent40 scriveva in `data/`.
//
// Il difetto non è mai stato «hanno sbagliato il calcolo»: è che ognuno se lo rifà. `lib/safety/store.js`
// la risposta ce l'ha già, e sa anche saltare il `.next/package.json` che ferma la risalita un livello
// troppo presto. Una fonte sola, come per tutto il resto.
const { DATA_DIR } = require('./safety/store');

const FEES_FILE  = path.join(DATA_DIR, 'venue-fees-official.json');
 ...
 */
function loadApyCap() {
  // __dirname is lib/ under plain node, but Next bundles this module into the route's
  // own server output, where __dirname points at .next/server/app/api/... — so the
  // project-root path is tried too. Both candidates resolve to the SAME file; this is
  // path resolution, not a second source of truth.
  const candidates = [
    path.join(__dirname, 'honest-display.ts'),
    path.join(process.cwd(), 'lib', 'honest-display.ts'),
  ];
  let src = null, from = null;
  for (const p of candidates) {
    try { src = fs.readFileSync(p, 'utf8'); from = p; break; } catch { /* try next */ }
  }
  if (src == null) {
    throw new Error(`lib/honest-display.ts not readable from any of [${candidates.join(', ')}] — refusing to guess APY_CAP`);
  }
  const cap = src.match(/export const APY_CAP\s*=\s*([\d.]+)/);
  const label = src.match(/export const APY_CAP_LABEL\s*=\s*'([^']+)'/);
  if (!cap) throw new Error(`APY_CAP not parseable from ${from} — refusing to guess`);

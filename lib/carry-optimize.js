...
function singleVenueRoute(opp, official, books, staleMs, now) {
  ...
  return {
    ...
    // The stablecoin the spot leg is actually bought against. USDC and USDe are not the
    // same instrument: USDe is a synthetic dollar with its own peg and counterparty
    // profile. The deepest-book rule that picks the pair does not judge that risk.
    quoteRisk: ladder.quote && ladder.quote !== 'USDC'
      ? `Spot leg quotes in ${ladder.quote}, not USDC/USD. The deepest-book rule selected it on depth alone `
      + 'and does not price its peg or counterparty risk — assess that before sizing.'
      : null,

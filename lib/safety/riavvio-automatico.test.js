...
// agent35-maker era il quarto di questo elenco: rimosso dalla flotta e dal repo il 9 agosto 2026
// insieme al meccanismo di ARMING e al suo dead-man (agent37-maker-watchdog).
const AGENTI = ['agent40-manual-reprice', 'agent41-realloc-scheduler', 'agent43-guardian'];
  ...
  for (const [nome, file] of fonti) {
    ...
    const trovato = ['lib', 'agents'].some((d) => {
      const cerca = (dir) => fs.readdirSync(dir, { withFileTypes: true }).some((e) => {
        ...
        if (['node_modules', '.next', '.git', '_archivio'].includes(e.name)) return false;

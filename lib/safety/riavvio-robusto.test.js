...
console.log('\n2 · i processi che devono esserci, ci sono');
{
  ...
  // ⚠ `dashboard` TOLTO da questo elenco il 15 agosto 2026: non è più un processo della flotta —
  // l'operatore prende le decisioni da terminale (`scripts/cli/`) e il pannello non deve mai girare.
  // Non è una sorveglianza in meno: il dashboard serviva pagine, non sorvegliava capitale. I cinque
  // rimasti sono quelli senza cui il capitale resta senza gestione o senza dati.
  for (const n of ['agent40-manual-reprice', 'agent41-realloc-scheduler', 'agent43-guardian',
    'agent24-liquidity-rewards', 'agent34-clob-ws']) {
    ok(`${n} è nell'elenco dei critici`, PROCESSI_CRITICI.includes(n));
  }
  ok('dashboard NON è fra i critici, e non è nemmeno un\'app', !PROCESSI_CRITICI.includes('dashboard'),
    'le decisioni si prendono da terminale: un pannello che non gira non può essere «sempre vivo»');

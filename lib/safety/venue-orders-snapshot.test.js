...
// ══ BLOCCO C · LA VALVOLA STA SOPRA LA GTD, e si asserisce la RELAZIONE, non il numero ═══════════════
{
  ...
  // ⚠ LE DUE ASSERZIONI QUI SOPRA NON BASTAVANO, E L'HA DIMOSTRATO IL REPLAY DELLA SERATA.
  // La prima stesura aveva ENTRY_MAX_AGE_MS a 30 minuti: 30 > 23, quindi passavano ENTRAMBE — e il
  // valore era comunque sbagliato, perche' faceva uscire dal perimetro un mercato con ordini vivi che
  // nessuno aveva guardato per un'ora. Una soglia «di poco sopra la GTD» descrive un meccanismo a
  // tempo; qui serve un BACKSTOP, cioe' una soglia che il funzionamento normale non sfiora mai.
  // Si asserisce quello: un ordine dev'essere morto da un pezzo — un ordine di distanza, non un
  // margine — prima che la valvola possa anche solo essere presa in considerazione.
  ok(S.ENTRY_MAX_AGE_MS >= 10 * GTD_MS,
    'C · ⚑ e ci sta un ORDINE DI GRANDEZZA sopra: e un backstop, non un meccanismo a tempo');
}

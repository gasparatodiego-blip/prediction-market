...
//
// HOW IT RUNS. `planReconcile`/`applyReconcile` are the live path today: lib/maker/manual-reset.js drives
// them for the manual lane. `reconcileOnce()` is the loop-driven wrapper — THROTTLED to at most once per
// RECONCILE_INTERVAL_MS (default 60s) — that the automatic engine used to call before it was removed on
// 2026-08-09; it is kept because it is the proven orchestration around the same two functions, but NOTHING
// calls it on a schedule right now. It reads the operator's resting orders from the venue
// (adapter.listOpenOrders → per-order size_matched, the same size_matched truth lib/maker/reconcile.js
// already trusts) and optionally the public /trades feed, and appends confirmed fills.
//
...
/**
 * Quanto risulta GIA' REGISTRATO per (venue|token|lato), sommato su TUTTE le chiavi.
 *
 * ═══ IL DIFETTO CHE QUESTA FUNZIONE ESISTE PER CHIUDERE ══════════════════════════════════════════
 * Il ramo `/trades` piu' sotto confronta `totalFilled` — la somma dei trade del venue su QUEL
 * token+lato, cioe' una grandezza per TOKEN — con `already`, che invece e' quanto risulta registrato
 * per QUELLA SINGOLA idempotencyKey. Due grandezze su scale diverse.
 *
 * Conseguenza, misurata il 9 agosto 2026 su Chengdu 37°C: il ciclo di riprezzo sostituisce la stessa
 * gamba ogni ~60 secondi, e ogni sostituzione produce un ordine con una chiave NUOVA. Appena
 * l'ordine sostituito sparisce dagli ordini aperti cade in questo ramo, trova lo stesso identico
 * volume di trade del venue, e siccome il suo `already` vale 0 lo registra INTERO come fill proprio.
 *
 *   136 ordini inviati ⇒ 136 righe di fill, 136 chiavi distinte, `filledSize` sempre lo stesso
 *   valore reale (21,69 · 14 · 7,69) ⇒ 2.892,46 share registrate, 2.790,32 di netto FIFO,
 *   contro una posizione VERA al venue di ZERO share.
 *
 * Il danno non e' contabile ma operativo: `openNotionalUsd` saliva a $2.405 contro un tetto di $600,
 * quindi `limit-max-open-notional` rifiutava OGNI piazzamento — con il bot su AVVIA e il kill
 * revocato. Il bot si e' fermato per una somma sbagliata.
 *
 * Il confronto giusto e' fra grandezze OMOGENEE: volume del venue per token+lato contro volume gia'
 * registrato per token+lato. Il ramo `size_matched` non ha mai avuto questo problema — li'
 * `vo.size_matched` e' il cumulato di QUELL'ordine, quindi il confronto per chiave e' quello corretto
 * e resta invariato.
 */
function recordedFilledByTokenSide(ledgerRows) {
  const m = new Map();
  for (const row of ledgerRows) {
    if (row.kind !== 'fill') continue;
    const k = tokenSideKey(row);
    if (!k) continue;
    m.set(k, (m.get(k) || 0) + (Number(row.filledSize) || 0));
  }
  return m;
}

/**
 * Quanto risulta gia' registrato per ORDINE DEL VENUE. E' la grandezza omogenea a `vo.size_matched`,
 * che e' il cumulato di QUELL'ordine.
 *
 * Serve perche' due nostri invii possono mappare sullo STESSO ordine del venue — `findVenueOrder`
 * riconosce un ordine anche per token+lato+prezzo+size, non solo per id — e in quel caso il confronto
 * per idempotencyKey li tratta come due ordini distinti e conta il volume due volte. E' la stessa
 * famiglia del difetto del ramo `/trades`, trovata dal test invece che dalla produzione.
 */
function recordedFilledByOrderId(ledgerRows) {
  const m = new Map();
  for (const row of ledgerRows) {
    if (row.kind !== 'fill' || !row.orderId) continue;
    const k = String(row.orderId);
    m.set(k, (m.get(k) || 0) + (Number(row.filledSize) || 0));
  }
  return m;
}

/** La chiave omogenea: venue + token + lato. Un lato non leggibile non produce chiave (fail-closed). */
function tokenSideKey(o) {
  const tok = String((o && (o.tokenId ?? o.asset ?? o.asset_id)) || '');
  const side = String((o && o.side) || '').toUpperCase();
  if (!tok || (side !== 'BUY' && side !== 'SELL')) return null;
  return `${(o && o.venue) || 'polymarket'}|${tok}|${side}`;
}
...
function planReconcile({ userId, sentOrders = [], ledgerRows = [], venueReachable = false, venueOrders = [], venueFills = null, venuePositions = null, feeResolver = null, tick = 0.01, now = Date.now(), source = 'reconcile' } = {}) {
  ...
  // Il registrato per token+lato, che questo giro aggiorna man mano: due ordini dello stesso token e
  // lato nella STESSA passata non devono rivendicare due volte lo stesso delta.
  const recordedTS = recordedFilledByTokenSide(ledgerRows);
  // Il registrato per ORDINE DEL VENUE, omogeneo a `vo.size_matched`.
  const recordedVO = recordedFilledByOrderId(ledgerRows);
  ...
  for (const o of sentOrders) {
    ...
    if (vo) {
      ...
      // ── OMOGENEO ANCHE QUI: `matched` e' il cumulato di QUELL'ordine del venue ─────────────────
      // Si sottrae quanto risulta gia' registrato per lo STESSO ordine del venue, non per la nostra
      // chiave: due invii possono mappare sullo stesso `vo` (findVenueOrder riconosce anche per
      // token+lato+prezzo+size) e il confronto per chiave li conterebbe due volte. Se l'id del venue
      // non e' leggibile si ripiega sul conteggio per chiave, che e' il comportamento di prima.
      const idVO = vo && (vo.id ?? vo.orderId) != null ? String(vo.id ?? vo.orderId) : null;
      const giaVO = idVO ? (recordedVO.get(idVO) || 0) : already;
      const delta = matched - giaVO;
      if (delta > 1e-9) {
        const price = Number(o.price);
        toRecord.push(mkFill(o, delta, price, feeResolver, now, source + ':size_matched'));
        if (idVO) recordedVO.set(idVO, giaVO + delta);
        // Questo ramo alimenta anche il totale per token+lato: lo stesso volume del venue non deve
        // poter essere rivendicato una seconda volta dal ramo `/trades` nella stessa passata.
        const k = tokenSideKey(o);
        if (k) recordedTS.set(k, (recordedTS.get(k) || 0) + delta);
      }
    ...
    if (Array.isArray(venueFills)) {
      ...
      // ── IL CONFRONTO E' FRA GRANDEZZE OMOGENEE — vedi `recordedFilledByTokenSide` ────────────
      // `totalFilled` e' il volume del venue su QUESTO token+lato. Va confrontato con quanto risulta
      // gia' registrato per lo STESSO token+lato, non con quanto risulta per questa singola chiave:
      // in un ciclo di riprezzo ogni sostituzione porta una chiave nuova, e il confronto per chiave
      // faceva registrare l'intero volume una volta per ordine.
      const kTS = tokenSideKey(o);
      const giaTS = kTS ? (recordedTS.get(kTS) || 0) : already;
      const delta = totalFilled - giaTS;
      if (delta > 1e-9) {
        ...
        // Si aggiorna SUBITO: il prossimo ordine dello stesso token+lato in questa stessa passata deve
        // vedere il delta come gia' rivendicato, altrimenti si ricrea il difetto dentro un solo giro.
        if (kTS) recordedTS.set(kTS, giaTS + delta);
        ...
        // ══ ① LA POSIZIONE E' PROVA DI FILL QUANTO UN TRADE — 21 agosto 2026 ═══════════════════════
        //
        // IL FATTO, misurato: il ledger contiene **5 fill e ZERO SELL** dal 31 luglio, mentre il venue
        // dichiara 561,90 share comprate e 1.167,53 vendute. `runFifo` non chiude mai niente, quindi
        // `openNotionalUsd` sale e non scende ($77,73 contro posizioni reali ZERO) e
        // `realisedDailyPnl` vale **$0 per costruzione** — cioe' il kill a −$100 guarda un registro
        // cieco e non puo' scattare.
        //
        // LA CAUSA: le due sole prove positive erano `size_matched` (che pretende l'ordine ANCORA
        // APERTO) e `/trades`. Un ordine riempito per intero sparisce dagli ordini aperti, e `/trades`
        // NON riporta i fill maker: misurato, 53 trade in tre settimane su 51 token distinti, con le
        // SELL su token che i BUY non toccano mai. Restava solo `gone-and-no-trades` ⇒ **nofill su un
        // ordine davvero eseguito**.
        //
        // ⚠ QUESTO RAMO ESISTEVA GIA' E NON DECIDEVA. Il commento qui sopra (scritto dopo il caso del
        // 31 luglio) aveva ragione sulla diagnosi — «/positions mostra le share subito, /trades e'
        // in ritardo» — e concludeva «non risolvere niente». Ma aspettare che `/trades` recuperi non
        // funziona quando `/trades` non arrivera' MAI. La posizione, su un token dove avevamo un
        // ordine vivo, e' prova positiva quanto un trade: adesso decide.
        //
        // ⚠ SOLO I BUY. Una posizione che COMPARE prova un acquisto. Una vendita si prova con una
        // posizione che CALA, e questo ramo non vede la storia: registrare una SELL da qui vorrebbe
        // dire dedurla, ed e' esattamente il modo in cui nascerebbe uno short fantasma (fills.js:169).
        //
        // ⚠ IL PREZZO E' QUELLO DELL'ORDINE, e per un maker e' ESATTO, non una stima: un limite a
        // riposo si riempie al proprio prezzo. Non e' il caso di `/trades`, dove serve il VWAP perche'
        // il volume puo' venire da piu' esecuzioni.
        //
        // ⚠ NIENTE DOPPIONI: si confronta la size della POSIZIONE con quanto risulta gia' registrato
        // per lo STESSO token+lato (`recordedTS`, la stessa contabilita' che ha chiuso §5 p.72), e si
        // registra solo il delta. Una posizione gia' coperta da `size_matched` o da `/trades` produce
        // delta zero e nessuna riga.
        const posSize = Number(positionByToken.get(String(o.tokenId || ''))) || 0;
        const kPos = tokenSideKey(o);
        const giaPos = kPos ? (recordedTS.get(kPos) || 0) : 0;
        const nonRegistrato = posSize - giaPos;
        // Non si rivendica piu' di quanto QUESTO ordine potesse eseguire: la posizione puo' venire
        // anche da ordini che non sono nostri o da giri precedenti.
        const dPos = Math.min(Number(o.size) || 0, nonRegistrato);
        if (String(o.side || '').toUpperCase() === 'BUY' && dPos > 1e-9 && Number(o.price) >= 0) {
          toRecord.push(mkFill(o, dPos, Number(o.price), feeResolver, now, source + ':positions'));
          if (kPos) recordedTS.set(kPos, giaPos + dPos);
        } else {
          // Delta zero (gia' registrato), oppure un SELL: si resta all'esito di prima, dichiarato.
 ...
 */
async function reconcileOnce({ userId, sentOrders = [], adapter, feeResolver = null, now = Date.now(), source = 'reconcile-loop' } = {}, deps = {}) {
...
module.exports = {
  recordedFilledByTokenSide, recordedFilledByOrderId, tokenSideKey, planReconcile, applyReconcile, reconcileOnce, findVenueOrder, recordedFilledByKey, RECONCILE_INTERVAL_MS };

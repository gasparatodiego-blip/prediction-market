 ...
 */
function planReconcile({ userId, sentOrders = [], ledgerRows = [], venueReachable = false, venueOrders = [], venueFills = null, feeResolver = null, tick = 0.01, now = Date.now(), source = 'reconcile' } = {}) {
  ...
  for (const o of sentOrders) {
    ...
    if (Array.isArray(venueFills)) {
      ...
      if (delta > 1e-9) {
        ...
        toNoFill.push({ idempotencyKey: o.idempotencyKey, orderId: o.orderId || null, reason: 'gone-and-no-trades' });
      }
      continue;
    }
...
async function reconcileOnce({ userId, sentOrders = [], adapter, feeResolver = null, now = Date.now(), source = 'agent35-reconcile' } = {}, deps = {}) {
  ...
  const plan = planReconcile({ userId, sentOrders, ledgerRows: ledger.rows, venueReachable, venueOrders, feeResolver, now, source });

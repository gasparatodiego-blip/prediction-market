...
console.log('\n══ 1b · SCRITTORE E LETTORE DEVONO GUARDARE LO STESSO FILE');
{
  // ── IL GUASTO VERO, TROVATO SOLO DOPO IL RIAVVIO ────────────────────────────────────────────────
  // Il file c'era e da riga di comando era leggibile, ma la dashboard rispondeva «mai scritto». Non era
  // il momento: era il PERCORSO. `path.join(__dirname, '..', '..', 'data')` dà due cartelle diverse a
  // seconda di chi carica il modulo:
  //
  //     agent40 (node semplice)   __dirname = lib/safety/          → /root/rewards-bot/data/       ✔
  //     dashboard (bundle Next)   __dirname = .next/server/chunks/ → /root/rewards-bot/.next/data/ ✘
  //
  // Lo scrittore scriveva in una, il lettore guardava nell'altra. «mai scritto» era letteralmente vero
  // per chi leggeva, mentre il file esisteva. Nessun processo, nessuna credenziale, nessun permesso.
  const { DATA_DIR } = require('./store');
  ok('lo snapshot sta nella cartella data/ risolta, non contata con i «..»',
    S.SNAPSHOT_FILE === path.join(DATA_DIR, 'venue-positions.json'));
  ok('  e quella cartella NON e dentro una cartella di build',
    !/[\\/](\.next|dist|build|out|node_modules)[\\/]/.test(S.SNAPSHOT_FILE), S.SNAPSHOT_FILE);
  ok('  ed esiste davvero', fs.existsSync(DATA_DIR));

  // Il modulo non deve tornare a calcolarsela per conto suo.
  const src = leggi('lib', 'safety', 'venue-positions-snapshot.js');
  ok('il modulo CHIEDE la cartella invece di calcolarla',
    /const \{ DATA_DIR \} = require\('\.\/store'\);/.test(src)
    && !/path\.join\(__dirname, '\.\.', '\.\.'\)/.test(src));

  // Stessa trappola sui due registri d'audit, che stanno sul percorso di piazzamento: un audit scritto
  // dalla dashboard sarebbe finito in .next/data/, invisibile alla riconciliazione.
  for (const [dove, file] of [
    [['lib', 'venues', 'polymarket-clob-maker', 'audit.js'], 'polymarket-maker-audit.jsonl'],
    [['lib', 'venues', 'polymarket-clob', 'audit.js'], 'polymarket-clob-audit.jsonl'],
  ]) {
    const t = leggi(...dove);
    ok(`${file} usa la stessa cartella risolta`,
      /require\('\.\.\/\.\.\/safety\/store'\)/.test(t) && !/join\(__dirname, '\.\.', '\.\.', '\.\.'/.test(t));
  }
}

...
console.log('\n══ 2 · IL PIAZZAMENTO NASCOSTO — il caso per cui l\'hook esiste');
{
  ...
  fs.writeFileSync(diretto, "const { placeManualOrder } = require('/root/rewards-bot/lib/maker/manual-order');\nplaceManualOrder({});\n");
  ...
  fs.writeFileSync(foglia, "module.exports = require('/root/rewards-bot/lib/maker/bulk-allocate');\n");
  ...
  // (d) Codice inline.
  const d1 = esegui(`node -e "require('/root/rewards-bot/lib/maker/manual-order').placeManualOrder({})"`);
  ok('node -e che piazza ⇒ BLOCCATO', d1.bloccato);
  const d2 = esegui(`node -e "console.log(require('/root/rewards-bot/lib/maker/manual-order'))"`);
...
require('/root/rewards-bot/lib/maker/manual-order').placeManualOrder({});
EOF`;
...
console.log('\n══ 3-bis · UN RIAVVIO pm2 NON È UN PIAZZAMENTO — e ha già il suo presidio');
{
  ...
  for (const cmd of ['node agents/agent35-maker.js', 'cd /root/rewards-bot && node agents/agent41-realloc-scheduler.js',
...
console.log('\n══ 4 · CANCELLARE NON È PIAZZARE');
{
  ...
  for (const cmd of ['node -e "require(\'/root/rewards-bot/lib/maker/cancel-all\')"',

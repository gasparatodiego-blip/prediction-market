...
console.log('\n══ 3-bis · UN RIAVVIO pm2 NON È UN PIAZZAMENTO — e ha già il suo presidio');
{
  ...
  // ⚠ QUI SI VERIFICAVANO LE REGOLE `ask` SU pm2 — E NON ESISTONO PIU' (19 agosto 2026, decisione
  //   dell'operatore: v. `policy-permessi.test.js`). Le asserzioni non sono state ammorbidite ma
  //   TOLTE, perche' difendevano una policy che e' stata svuotata di proposito: tenerle verdi avrebbe
  //   richiesto di allentarle, e un'asserzione allentata e' peggio di un'asserzione assente.
  //   Cio' che resta a presidiare pm2 e' la regola 2 di CLAUDE.md §2 — si chiede in chat, ogni volta.
  //   Il presidio verificabile qui e' l'altro, ed e' quello che l'hook fa da solo:
  ok('  e comunque l hook, se riarmato, non giudica un comando pm2', !esegui('pm2 restart agent41-realloc-scheduler').bloccato,
    'l hook chiede una FORMA DI ESECUZIONE davanti al nome, e `pm2 restart` non lo e');
}
...
console.log('\n══ 5 · L\'HOOK NON È PIÙ REGISTRATO — E IL SUO CODICE FUNZIONA ANCORA');
{
  // ⚠ QUESTA SEZIONE DICEVA IL CONTRARIO, e la proprieta' e' cambiata di segno il 19 agosto 2026 per
  //   decisione esplicita dell'operatore. Il codice dell'hook e' rimasto sul disco e i blocchi 1-4 qui
  //   sopra continuano a esercitarlo: se un giorno viene riarmato, si sa gia' che analizza ancora
  //   correttamente il grafo dei `require`. Cio' che NON e' piu' vero e' che qualcuno lo chiami.
  ...
  ok('NON registrato: nessun PreToolUse lo chiama', !nostro,
    'decisione dell operatore del 19 agosto — il presidio resta la regola 3 di CLAUDE.md §2');
  ok('  ma il file esiste ancora: riarmare e una riga di configurazione',
    fs.existsSync(path.join(ROOT, '.claude', 'hooks', 'blocca-piazzamento.js')));

  const altroTool = esegui('curl -X POST localhost:3000/api/maker/manual/order', 'Read');
  ok('  e se riarmato, un tool che non è Bash non viene giudicato', !altroTool.bloccato,
    'il matcher è Bash, e l hook lo ricontrolla');
}

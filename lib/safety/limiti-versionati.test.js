...
// ══ ⑥ IL VALORE IN SERVIZIO SI DICHIARA — cosi' un cambio resta visibile in un diff ══════════════
{
  ...
  const richiesto = esposizioneMassimaRaggiungibileUsd(SEL.MAX_MERCATI_CONTEMPORANEI);
  ok(Number.isFinite(richiesto), '⑥ la grandezza e calcolabile da N', `N=${SEL.MAX_MERCATI_CONTEMPORANEI} ⇒ ${richiesto}`);
  ok(suDisco.global.maxOpenNotionalUsd >= richiesto,
    ...
    `$${suDisco.global.maxOpenNotionalUsd} contro ${SEL.MAX_MERCATI_CONTEMPORANEI} x 2 x $${MARKET_CAP_FIXED_USD} = $${richiesto}`);

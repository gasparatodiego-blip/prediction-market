...
// ══ ⑥ IL VALORE IN SERVIZIO SI DICHIARA — cosi' un cambio resta visibile in un diff ══════════════
{
  ...
  // ⚠ CONTAVA SOLO IL RIPOSO, cioe' META' DELLA GRANDEZZA (19 agosto 2026). La relazione era gia'
  //   quella giusta — il tetto deve COPRIRE, non stare sotto — ma `N × tetto per mercato` e' il
  //   capitale delle coppie a riposo e ignora il loro completamento, che il gate somma perche'
  //   `openNotionalUsd` conta i fill riconciliati. A N=5: $306,25 contro i $612,50 veri.
  //   La grandezza vive ora in `concentration.esposizioneMassimaRaggiungibileUsd` e si IMPORTA.
  // ⚠ N NON VIENE PIU' DA UN LETTERALE DI SORGENTE (24 agosto 2026): `MAX_MERCATI_CONTEMPORANEI` e'
  // stato tolto perche' faceva anche da DIFETTO. Si legge la CONFIGURAZIONE DICHIARATA con la
  // funzione vera — che e' anche il numero che i processi riceveranno al prossimo riavvio dal file.
  ...
  const N = require('../maker/quanti-mercati').quantiMercati(app41.env).quanti;
  const richiesto = esposizioneMassimaRaggiungibileUsd(N);
  ok(Number.isFinite(richiesto), '⑥ la grandezza e calcolabile da N', `N=${N} ⇒ ${richiesto}`);
  ok(suDisco.global.maxOpenNotionalUsd >= richiesto,
    '⑥ ⚑ il tetto di esposizione copre l esposizione massima RAGGIUNGIBILE (riposo + completamento)',
    `$${suDisco.global.maxOpenNotionalUsd} contro ${N} x 2 x $${MARKET_CAP_FIXED_USD} = $${richiesto}`);
  // ⚠ E LA STESSA RELAZIONE E' UN CANCELLO D'AVVIO, non solo un test: se qui e' verde e li' e' rosso
  // uno dei due sta guardando un numero diverso, che e' il guasto che questo file esiste per trovare.
  ok(require('./invariante-cap-slot').misuraInvariante({ env: app41.env }).ok === true,
    '⑥ e l invariante d avvio dei due processi dice la stessa cosa',
    require('./invariante-cap-slot').misuraInvariante({ env: app41.env }).motivo);

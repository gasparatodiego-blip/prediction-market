 ...
 */
function readUsage({ userId, now = Date.now(), windowMs = 60_000, marks = null }, deps = {}) {
  ...
  const exp = computeExposure({ userId, now, sentOrders, marks }, deps);
  ...
  return { openNotionalUsd, ordersInWindow, realisedDailyPnlUsd, sentOrders };
}

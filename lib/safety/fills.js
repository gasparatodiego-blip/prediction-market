...
function computeExposure({ userId, now = Date.now(), sentOrders = [], marks = null, markFreshMs = 60_000, staleMs = DEFAULT_STALE_MS, venuePositions = null } = {}, deps = {}) {
  ...
  let daVenue = 0;
  // Quanto il venue ha smentito: viaggia nel risultato perche' «esposizione scesa» e «ledger nettato»
  // sono due fatti diversi, e chi legge un tetto deve poter distinguerli.
  let nettateUsd = 0;
  const nettate = [];
  ...
  if (venuePositions && venuePositions.readable === true && vp) {
    ...
    // ── E LE POSIZIONI CHE IL VENUE DICE CHIUSE — 12 agosto 2026 ────────────────────────────────
    //
    // Fin qui la fusione sapeva fare due cose: tenere la lettura più alta su un token visto da
    // entrambe le parti, e AGGIUNGERE un token che esiste solo al venue. Mancava la terza, ed è
    // quella che rende il ledger una fonte che non guarisce mai: **togliere una posizione che il
    // ledger crede aperta e che al venue non c'è più**.
    //
    // Il ledger si chiude solo se la riconciliazione riesce a scrivere la riga di uscita. Una
    // posizione venduta, fusa on-chain, redenta o risolta alla scadenza non produce quella riga:
    // i lotti FIFO restano aperti per sempre, al loro nozionale d'ingresso. Misurato il 12 agosto:
    // **14 posizioni per $16.960,06** contro **zero posizioni al venue** — 26 volte il tetto di
    // esposizione aperta di $600, quindi TUTTE le gambe di ogni giro venivano saltate
    // (`skipped`) e il bot non poteva piazzare niente.
    //
    // ═══ PERCHÉ L'ASSENZA VALE COME PROVA, QUI E SOLO QUI ═══════════════════════════════════════
    // `readVenuePositions` restituisce `readable:false` oltre `MAX_AGE_MS` (180 s): quindi
    // `venuePositions.readable === true` significa già «lettura FRESCA del venue», e su questo venue
    // la risposta è l'elenco COMPLETO delle posizioni aperte, non una pagina. In quelle condizioni
    // «il token non c'è» è un fatto, non un dato mancante.
    // Fuori da quelle condizioni non si netta NIENTE: questo blocco vive dentro
    // `if (venuePositions && venuePositions.readable === true && vp)`, quindi uno snapshot assente,
    // vecchio o illeggibile lascia il ledger esattamente com'era — che è il comportamento di prima
    // e il verso prudente su un tetto.
    //
    // NON SI CANCELLA NIENTE DAL LEDGER: il file dei fill è append-only e resta la storia di cosa è
    // stato eseguito. Si corregge l'ESPOSIZIONE, che è una lettura derivata, e la riga resta
    // nell'elenco marcata `chiusaAlVenue` — sparire in silenzio renderebbe indistinguibile
    // «nettata» da «non è mai esistita».
    const tokenAlVenue = new Set(vp.map((p) => String(p.tokenId ?? p.asset ?? '')).filter(Boolean));
    for (const p of positions) {
      if (p.soloAlVenue === true) continue;                 // è venuta DAL venue: c'è per definizione
      if (tokenAlVenue.has(String(p.tokenId))) continue;    // confermata da entrambe le fonti
      if (!(p.exposureUsd > 0)) continue;
      openNotionalUsd -= p.exposureUsd;
      nettateUsd += p.exposureUsd;
      p.esposizionePrimaUsd = p.exposureUsd;
      p.exposureUsd = 0;
      p.chiusaAlVenue = true;
      p.markSource = 'chiusa-al-venue';
      nettate.push({ tokenId: p.tokenId, shares: p.shares, esposizioneTolta: p.esposizionePrimaUsd });
    }
    // Somme e sottrazioni di valori già arrotondati lasciano un residuo sotto il centesimo (misurato:
    // $0,0001 dopo aver nettato 14 posizioni). Sotto un centesimo non è esposizione, è rumore di
    // virgola mobile, e lasciarlo farebbe sembrare «quasi zero» ciò che è zero. Si azzera SOLO in
    // questo ramo, cioè solo dopo che il venue ha smentito qualcosa.
    if (openNotionalUsd < 0.01) openNotionalUsd = 0;
  }
  ...
  return {
    ...
    venuePositions: {
      ...
      // Quanto il venue ha SMENTITO: posizioni che il ledger credeva aperte e che al venue non ci
      // sono più. Viaggia nel risultato perché «l'esposizione è scesa» e «il ledger è stato nettato»
      // sono due fatti diversi, e sul secondo un operatore vuole poter mettere gli occhi.
      nettateUsd: +nettateUsd.toFixed(4),
      nettateCount: nettate.length,
      nettate,

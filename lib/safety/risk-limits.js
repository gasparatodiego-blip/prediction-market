...
const HARD_CEILINGS = Object.freeze({
  ...
  // ── IL TETTO DURO DELL'ESPOSIZIONE, DA $2.000 A $2.400 — 23 agosto 2026 ──────────────────────
  // ⚠⚠ QUESTO NUMERO NON E' IL CAP: E' IL TETTO SOPRA IL CAP, e va mosso INSIEME o non serve a
  // niente e fa danno. `clampNum` fa `min(valore su disco, tetto duro)` SENZA sollevare: con il cap
  // versionato portato a $2.400 e questo fermo a $2.000, il cap in servizio sarebbe stato $2.000 —
  // cioe' un numero deciso dall'operatore, scritto su disco, e silenziosamente diverso da quello che
  // il gate applica. E l'invariante `N x 2 x $61,25 <= cap` sarebbe stata calcolata sul numero
  // dichiarato e verificata su un altro: a N=19 servono $2.327,50, che stanno sotto $2.400 e SOPRA
  // $2.000, quindi il gate avrebbe smesso di piazzare a meta' strada — il guasto del 16 agosto ($150
  // contro 3 x $61,25), con in piu' l'aggravante che il referto avrebbe detto che ci stava.
  // ⚠ RESTA UN TETTO VERO: si alza allo STRETTO NECESSARIO ($2.400, non «tanto»), quindi continua a
  //   rifiutare qualunque cap piu' alto scritto su disco, e `limiti-versionati.test.js` ⑤ continua a
  //   fallire se qualcuno lo supera.
  // ⚠ NON TOCCA NESSUN ALTRO TETTO: per ordine ($1.000), invii per finestra (60), perdita
  //   giornaliera ($200) restano identici. Il freno vero resta il kill a −$100.
  maxOpenNotionalUsd: 2400,   // biggest total open exposure across all positions
 ...
 *
 * esenzioneEsposizione: `{esente:true, motivo}` da `lib/maker/esenzione-chiusura.provaChiusura` — vedi
 * il blocco sul LIMITE 2 qui sotto. Assente/false ⇒ il tetto si applica come sempre.
 *
 * @returns {{allow:boolean, gate?:string, reason?:string, autoKill?:boolean, esenzione?:string}}
 */
function evaluateLimits({ order, usage, limits, esenzioneEsposizione = null } = {}) {
  const L = limits || {};
  ...
  // ── UN ORDINE CHE CHIUDE NON SFONDA IL TETTO DI ESPOSIZIONE (16 agosto 2026) ───────────────────
  // `openNotionalUsd + notional` e' l'aritmetica di un ordine che APRE. Su un ordine che chiude e'
  // sbagliata di SEGNO, e al tetto produce una trappola nei due versi: la gamba riempita e' gia'
  // dentro `openNotionalUsd`, quindi il BUY che completa la coppia viene rifiutato — e viene
  // rifiutata anche la SELL che liquiderebbe la gamba nuda, perche' anche la sua size viene sommata.
  // Al tetto la posizione resterebbe in gabbia in entrambe le direzioni: non un limite di rischio,
  // un deadlock. Terza occorrenza della classe «regola nata per limitare l'APERTURA applicata a
  // un'azione che non apre» (§5-bis p.133 tetto per mercato, p.147 tetto per ordine).
  //
  // ⚠ NON E' UNA DICHIARAZIONE DI CUI FIDARSI. `esenzioneEsposizione` arriva gia' PROVATA da
  // `esenzione-chiusura.provaChiusura`, che rifa' l'aritmetica sull'ordine esatto contro lo snapshot
  // posizioni del venue — SELL entro il posseduto, BUY entro cio' che manca alla coppia — e nessuna
  // lettura mancante produce un'esenzione. Qui si guarda `=== true`, mai la truthiness.
  //
  // ⚠ ESENTA QUESTO TETTO E BASTA. I due rami fail-closed sopra (posizioni illeggibili, esposizione
  // non misurabile) restano DAVANTI e non sono aggirabili — del resto senza snapshot leggibile la
  // prova non esisterebbe. Il tetto PER ORDINE (limite 1), il rate limit (3), la perdita giornaliera
  // (4), la allowlist e il KILL sono intatti: un'uscita provata non e' un lasciapassare.
  const esenzione = esenzioneEsposizione && esenzioneEsposizione.esente === true ? esenzioneEsposizione : null;
  if (usage.openNotionalUsd + notional > L.maxOpenNotionalUsd + 1e-9 && !esenzione) return { allow: false, gate: 'max-open-notional', reason: `open exposure $${usage.openNotionalUsd.toFixed(2)} + this order $${notional.toFixed(2)} exceeds cap $${L.maxOpenNotionalUsd}` };
  const esenzioneUsata = esenzione && usage.openNotionalUsd + notional > L.maxOpenNotionalUsd + 1e-9
    ? `max-open-notional esentato: ${esenzione.motivo}` : null;
  ...
  // L'esenzione viaggia nel risultato: «il tetto non ha morso» e «il tetto e' stato esentato» devono
  // restare due righe distinguibili nel giornale fra un mese.
  return esenzioneUsata ? { allow: true, esenzione: esenzioneUsata } : { allow: true };
}

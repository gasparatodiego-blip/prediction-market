 ...
 */
function capitalToQualifyUsd(mid, minSize) {
  if (!fin(mid) || !(mid > 0) || !fin(minSize) || !(minSize > 0)) return null;
 ...
 */
function minSizeVerdict({ capitalUsd, mid, minSize } = {}) {
  ...
  const perSide = (capitalUsd / 2) / clampPrice(mid);
  const need = capitalToQualifyUsd(mid, minSize);
  if (perSide < minSize) {

...
// Provisional stability (structure-only) — a REAL band-relative 24h-volatility score, or null when
// unmeasured. Shared so the server filter, the UI cell and the sort all read ONE definition.
const { computeStability } = require('./reward-stability');
...
function passes(m, f) {
  ...
  // "Stabilità minima" — the ONE filter that does NOT exclude an unknown-field row. Stability is a
  // structure-only, not-yet-final metric; a market whose stability is unmeasured must stay visible
  // (the real engine will replace this). Only a row with a KNOWN score BELOW the floor is removed.

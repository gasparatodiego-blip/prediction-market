...
console.log('\n── 1 · LA SOGLIA: stessa formula dell allocatore, provata contro di essa ──');
{
  ...
  ok('  soglia nota: 2 × 0.675 × 20 = $27.00', near(E.capitalToQualifyUsd(0.675, 20), 27));
  ok('  e clampa il prezzo come il venue (mid 0.001 → trattato come 0.01)', near(E.capitalToQualifyUsd(0.001, 100), 2));
  ok('input non rispondibili → null, mai una soglia inventata',
    E.capitalToQualifyUsd(0, 20) === null && E.capitalToQualifyUsd(0.5, 0) === null && E.capitalToQualifyUsd(null, 20) === null);
}
...
console.log('\n── 2 · IL BUG CORRETTO: capitale sotto il minimo → ZERO, non una cifra positiva ──');
{
  ...
  ok('  con il capitale che lo sbloccherebbe', near(under.capitalToQualifyUsd, 27));
  ok('  e il motivo dice share/lato e minimo', /7\.4 share per lato/.test(under.reason) && /minimo di 20/.test(under.reason));
...
console.log('\n── 3 · SOPRA LA SOGLIA: la stima resta quella di prima ──');
{
  ...
  ok('a $100 su una soglia di $27 → stima positiva', over.estUsdPerDay > 0 && over.belowVenueMinSize === false);
  ok('  esattamente al limite ($27.00) qualifica', E.estimateAtCapital(RS, 27, 100000).belowVenueMinSize === false);
  ok('  un centesimo sotto NON qualifica', E.estimateAtCapital(RS, 26.99, 100000).belowVenueMinSize === true);
...
{
  // La regola dell allocatore, riprodotta: sizePerSideShares = (capital/2)/clamp(mid) < minSize ⇒ gross 0.
  ...
  for (const mid of [0.03, 0.12, 0.5, 0.675, 0.95]) {
    for (const minSize of [5, 20, 50, 100]) {
      for (const cap of [1, 5, 10, 25, 50, 200, 1000]) {
        checked++;
        const allocatorZero = ((cap / 2) / clamp(mid)) < minSize;                    // net.js

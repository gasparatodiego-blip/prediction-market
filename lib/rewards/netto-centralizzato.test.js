...
console.log('\n══ 2 · NESSUNO LA RISCRIVE: I TRE PUNTI IMPORTANO');
{
  ...
  ok('la vecchia forma delle card non c è più — era QUESTA il difetto',
    !/best && fin\(best\.net5m\) \? best\.net5m : null/.test(alloc));
...
// Era «il tetto del 30%». Dal 7 agosto 2026 è 20%: il pianificatore aveva 30% mentre il motore di
// piazzamento (motore-unico.MARKET_CAP_PCT) ne rifiutava da sempre più del 20%, quindi il pianificatore
// proponeva righe che il motore tagliava al quoting. Stesso vincolo, un numero solo, ed è quello del
// motore — il tetto di sicurezza sul rischio di risoluzione.
console.log('\n══ 5 · IL TETTO DEL 20% È GIÀ IN UN PUNTO SOLO (verifica, non modifica)');
{
  const conc = leggi('lib', 'rewards', 'concentration.js');
  ok('la frazione è definita in concentration.js', /CONCENTRATION_CAP_FRAC = 0\.20/.test(conc));
  // Chi la usa la importa: agent41 (via realloc-cycle) e la route del pannello.
  ok('il riallocatore periodico la importa',
    /require\('\.\.\/rewards\/concentration'\)/.test(leggi('lib', 'maker', 'realloc-cycle.js')));
  ok('la route del pannello la importa',
    /from '@\/lib\/rewards\/concentration'/.test(leggi('app', 'api', 'rewards', 'allocate', 'route.ts')));
  const { CONCENTRATION_CAP_FRAC, capPerMarketUsd } = require('./concentration');
  ok('  ed è davvero 0.20', CONCENTRATION_CAP_FRAC === 0.20);
  ok('  ed è LO STESSO numero del motore di piazzamento',
    CONCENTRATION_CAP_FRAC === require('../maker/motore-unico').MARKET_CAP_PCT);
  ok('  $200 → tetto $40', capPerMarketUsd(200) === 40);
  ok('  capitale non utilizzabile → nessun tetto (null), non zero', capPerMarketUsd(null) === null);
}

...
console.log('\n══ 5 · IL TETTO PER MERCATO È IN UN PUNTO SOLO, E LO IMPORTANO IN QUATTRO');
{
  ...
  // Non piu' `= 65;` letterale: il tetto e' DERIVATO. Si verifica che la definizione sia in questo
  // modulo e che nasca da f_min, non che sia scritta come numero.
  ok('il tetto è definito in concentration.js e DERIVATO da f_min',
    /TETTO_BASE_USD\s*=/.test(conc) && /F_MIN_OBIETTIVO/.test(conc) && /MARKET_CAP_FIXED_USD\s*=\s*TETTO_BASE_USD/.test(conc));
  ...
  ok('  ed è il tetto al capitale di riferimento, derivato da f_min',
    MARKET_CAP_FIXED_USD === +(C.MIN_PREMIANTE_TIPICO * C.COSTO_COPPIA / C.F_MIN_OBIETTIVO).toFixed(2));
  // ⚠ LE TRE ASSERZIONI QUI SOTTO SONO STATE RIBALTATE il 12 agosto 2026, non tolte. Difendevano le
  // proprieta' del tetto FISSO; adesso il tetto e' derivato dal capitale, e le proprieta' vere sono
  // altre — che il capitale faccia crescere il NUMERO di mercati finche' il carico regge, e che la
  // size cresca solo oltre quella soglia.
  // ⚠ LA SOGLIA DI CARICO E' `TETTO_BASE x MAX_MERCATI` E SI MUOVE COL TETTO: a $32,67 stava a
  // $1.306, a $61,25 sta a $2.450. Le due fixture sono DERIVATE, o il test tornerebbe rosso a ogni
  // leva sul tetto pur difendendo una proprieta' che non e' cambiata.
  const sogliaCarico = MARKET_CAP_FIXED_USD * C.MAX_MERCATI;
  ok('  piu\' capitale alza il tetto solo oltre la soglia di carico (MAX_MERCATI mercati pieni)',
    capPerMarketUsd(sogliaCarico * 2) > MARKET_CAP_FIXED_USD && capPerMarketUsd(sogliaCarico * 0.5) === MARKET_CAP_FIXED_USD,
    `soglia $${sogliaCarico.toFixed(0)} · sotto→${capPerMarketUsd(sogliaCarico * 0.5)} · sopra→${capPerMarketUsd(sogliaCarico * 2)}`);
  ok('  e sotto la base il tetto può solo stringere', capPerMarketUsd(10) === 10);
  ok('  capitale non utilizzabile → pavimento premiante, mai null',
    typeof capPerMarketUsd(null) === 'number' && capPerMarketUsd(null) > 0, String(capPerMarketUsd(null)));
  ok('  e il numero di mercati è una conseguenza, non un parametro',
    mercatiNecessari(2000) === Math.ceil(2000 / capPerMarketUsd(2000)));
}

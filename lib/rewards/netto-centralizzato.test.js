...
console.log('\n══ 5 · IL TETTO PER MERCATO È IN UN PUNTO SOLO, E LO IMPORTANO IN QUATTRO');
{
  ...
  ok('il tetto è definito in concentration.js, in dollari', /MARKET_CAP_FIXED_USD = \d+;/.test(conc));
  ...
  ok('  ed è davvero $65', MARKET_CAP_FIXED_USD === 65);
  ok('  $2.000 di capitale NON alzano il tetto', capPerMarketUsd(2000) === MARKET_CAP_FIXED_USD);
  ...
  ok('  capitale non utilizzabile → tetto PIENO, mai null (null varrebbe «nessun tetto»)',
    capPerMarketUsd(null) === MARKET_CAP_FIXED_USD);
  ok('  e il numero di mercati è una conseguenza, non un parametro',
    mercatiNecessari(2000) === Math.ceil(2000 / MARKET_CAP_FIXED_USD));
}

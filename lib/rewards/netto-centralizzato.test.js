...
console.log('\n══ 5 · IL TETTO DEL 30% È GIÀ IN UN PUNTO SOLO (verifica, non modifica)');
{
  const conc = leggi('lib', 'rewards', 'concentration.js');
  ok('la frazione è definita in concentration.js', /CONCENTRATION_CAP_FRAC = 0\.30/.test(conc));
  ...
  ok('  ed è davvero 0.30', CONCENTRATION_CAP_FRAC === 0.30);
  ok('  $200 → tetto $60', capPerMarketUsd(200) === 60);
...
console.log('\n══ 6 · IL SIZING LEGGE IL MID VERO, MAI 0.50');
{
  ...
  ok('con il costo della coppia la size non dipende dal mid',
    /a\.capital \/ pairCostUsd/.test(alloc));
}

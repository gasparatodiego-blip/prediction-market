...
const riga = ({ midStorico, scoringMid, bestBid, bestAsk, tick = 0.01, banda = 2.25, minSize = 20, capitale = 28 }) => ({
...
console.log('\n══ 1 · IL CASO VERO: mediana 0,835 · mid di scoring 0,795 · ask 0,82');
{
  ...
  ok('  la regola morde: almeno un offset viene escluso', esclusi > 0, `${esclusi} esclusi`);

  // ── E DAL 5 AGOSTO 2026 QUESTO MERCATO NON SI QUOTA PIÙ AFFATTO ────────────────────────────────
  // La priorità è stata invertita: «mai primi sul libro» vince sulla banda premiante. Su questo book
  // — miglior bid altrui 78¢, mid di scoring 79,5¢, banda ±2,25¢ ⇒ [78¢, 81¢] — un tick dietro il
  // concorrente darebbe 77¢, FUORI banda. Restare premianti richiederebbe di quotare A 78¢, cioè in
  // cima al libro. Quindi il mercato viene escluso a ogni offset, e questo test lo dichiara invece di
  // pretendere il contrario: prima chiedeva «almeno un offset resta proponibile», che era vero sotto
  // la vecchia politica ed è falso sotto questa.
  //
  // È esattamente il mercato su cui il 5 agosto sono stati piazzati i primi ordini reali a 77¢ e 20¢:
  // con la politica di oggi quei due ordini non sarebbero partiti.
  ok('nessun offset è proponibile: sarebbe sempre in cima', proposti === 0, `${proposti} proposti`);
  for (const off of [1, 2, 3, 4]) {
    const g = gambeDiUnaRiga(rVero, off);
    ok(`  offset ${off}t escluso col motivo giusto`,
      g.scarto != null && ['sarebbe-primo-sul-libro', 'fuori-banda-sul-libro-vivo', 'gamba-fuori-banda', 'incrocia-il-libro'].includes(g.scarto.motivo),
      g.scarto ? g.scarto.motivo : 'PROPOSTO');
  }
  const uno = gambeDiUnaRiga(rVero, 1);
  ok('  e il motivo è «sarebbe primo sul libro»', uno.scarto && uno.scarto.motivo === 'sarebbe-primo-sul-libro',
    uno.scarto ? uno.scarto.motivo : 'proposto');
  ok('  spiegato a parole, non solo col codice',
    uno.scarto && /posizione peggiore del libro/.test(uno.scarto.dettaglio));
  // Le tre asserzioni che stavano qui — «non incrocia», «dentro banda», «il prezzo viene dal mid vivo»
  // — guardavano il prezzo proposto. Con la nuova politica un prezzo non viene proposto affatto, quindi
  // non c'è niente da guardare: la proprietà che le sostituisce è quella qui sopra, «nessun offset è
  // proponibile». Il fatto che il prezzo venisse dal mid vivo e non dalla mediana resta verificato dal
  // blocco PRIMA/DOPO in cima a questo file, che non dipende da questa politica.
  // L'esclusione dipende da DOVE STA IL CONCORRENTE, non dalla mediana: con un miglior bid piu' VICINO
  // al mid (81¢) un tick dietro cade a 80¢, dentro la banda [78¢, 81¢], e il mercato torna proponibile.
  // Un concorrente piu' LONTANO peggiorerebbe le cose, non le migliorerebbe — e' la prima cosa che ho
  // sbagliato scrivendo questa asserzione.
  ok('e l esclusione dipende da DOVE sta il concorrente',
    gambeDiUnaRiga(riga({ midStorico: 0.835, scoringMid: VERO, bestBid: 0.79, bestAsk: 0.80 }), 1).rows != null,
    'miglior bid 79¢ ⇒ un tick dietro e 78¢, dentro banda su ENTRAMBI i lati');
}
...
console.log('\n══ 2 · UN PREZZO CHE COINCIDE CON L ASK NON VIENE MAI PROPOSTO (punto 6, primo test)');
{
  ...
  // bestBid 58¢ e non 55¢: con la politica «mai primi» del 5 agosto 2026 un concorrente a 55¢
  // farebbe escludere il mercato per un motivo diverso (un tick dietro sarebbe 54¢, fuori dalla
  // banda [56¢, 64¢]) e questo blocco verifica l'INCROCIO, non quella regola.
  const r = riga({ midStorico: 0.60, scoringMid: 0.60, bestBid: 0.58, bestAsk: 0.59, banda: 8 });
...
console.log('\n══ 3 · IL MID DI SCORING DECIDE LA BANDA, NON QUELLO GREZZO (punto 6, secondo test)');
{
  ...
  // Il tocco è STRETTO (49¢/51¢) di proposito: con un concorrente lontano la politica «mai primi»
  // escluderebbe il mercato prima, e questo blocco verifica quale MID comanda, non quella regola.
  const dentro = gambeDiUnaRiga(riga({ midStorico: 0.50, scoringMid: 0.50, bestBid: 0.49, bestAsk: 0.51, banda: 4.5 }), 2);
  ...
  const div = gambeDiUnaRiga(riga({ midStorico: 0.70, scoringMid: 0.50, bestBid: 0.49, bestAsk: 0.51, banda: 4.5 }), 2);

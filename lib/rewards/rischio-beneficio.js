...
//
//   4 · CONCENTRAZIONE — `1 + quotaDelMercato / CONCENTRATION_CAP_FRAC`
//       `quotaDelMercato` = capitale già destinato a quel mercato ÷ capitale totale. Al tetto del 20%
//       (lib/rewards/concentration) il fattore vale esattamente 2: cioè il ventesimo dollaro messo su un
//       mercato è scontato il doppio del primo. Sopra il tetto non si arriva, perché il tetto è un
//       cancello vero e vive altrove.
//
...
function rischioBeneficio({
  ...
  // 4 · CONCENTRAZIONE
  if (fin(capitaleSulMercatoUsd) && capitaleSulMercatoUsd >= 0 && fin(capitaleTotaleUsd) && capitaleTotaleUsd > 0
      && fin(CONCENTRATION_CAP_FRAC) && CONCENTRATION_CAP_FRAC > 0) {
    const quota = capitaleSulMercatoUsd / capitaleTotaleUsd;
    componenti.concentrazione = { fattore: +limita(1 + quota / CONCENTRATION_CAP_FRAC).toFixed(4), misura: +(quota * 100).toFixed(2),
      soglia: +(CONCENTRATION_CAP_FRAC * 100).toFixed(1),
      nota: `% del capitale su questo mercato; al tetto del ${(CONCENTRATION_CAP_FRAC * 100).toFixed(0)}% il fattore vale 2` };
  } else { componenti.concentrazione = { fattore: 1, misura: null, soglia: +(CONCENTRATION_CAP_FRAC * 100).toFixed(1), nota: 'capitale sul mercato o capitale totale non noti' }; nonMisurati.push('concentrazione'); }

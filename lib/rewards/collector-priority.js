 ...
 */
function unioneMobile({ precedenti = [], freschi = [], candidati = [], prioritari = [], nowMs, retentionMs = RETENTION_MS, max = MAX_MARKETS } = {}) {
  ...
  // ══ I CANDIDATI PRIORITARI: L'UNICA CLASSE CHE SCAVALCA I «TRATTENUTI» ═════════════════════════
  //
  // ⚠ IL FATTO, misurato il 22 agosto: con la corsia a 60 posti pieni — 9 righe di piano, 11 mercati
  // selezionati, 40 trattenuti per freschezza — i CANDIDATI ricevevano **ZERO** posti. E i candidati
  // sono l'unica classe da cui un mercato NUOVO puo' entrare: finche' la corsia e' piena, nessun
  // mercato che il bot non guarda gia' potra' mai essere guardato. E' l'anello di §5.2 p.55 nella
  // sua forma piu' stretta, e nessuna promozione DENTRO la classe candidati puo' scioglierlo,
  // perche' il problema non e' l'ordine fra candidati: e' che la classe non riceve niente.
  //
  // ⚠⚠ UN «TRATTENUTO» E' UN MERCATO CHE NESSUNO HA SCELTO — sta li' solo perche' e' stato visto di
  // recente, e la sua ritenzione esiste per non spegnere un libro che potrebbe tornare utile fra un
  // ciclo. Un prioritario invece e' un mercato per cui ci sono SLOT DEDICATI VUOTI che aspettano.
  // Fra «forse servira'» e «c'e' un posto vuoto che aspetta lui», il secondo viene prima. Questa e'
  // l'unica classe che scavalca i trattenuti, e la sola ragione per cui e' lecito.
  //
  // ⚠ RESTA SOTTO LE RIGHE DEL PIANO E SOTTO I SELEZIONATI, che sono capitale gia' deciso ed esposto:
  // l'ordine di sacrificio si allunga di un gradino, non si riscrive.
  // ⚠ ED E' LIMITATA DA CHI CHIAMA, non da qui: chi passa `prioritari` ne passa pochi e bounded (la
  // fascia corta ammissibile, al piu' una dozzina). Una classe prioritaria senza tetto svuoterebbe
  // la corsia dei trattenuti, cioe' spegnerebbe i libri dei mercati su cui il bot sta lavorando —
  // un presidio che produce il guasto che cura.
  const prioritariOra = [];
  for (const pr of prioritari) {
    const id = normId(typeof pr === 'string' ? pr : (pr && pr.id));
    if (!id || per.has(id)) continue;   // gia' presente per un motivo piu' forte: non si declassa
    per.set(id, { id, piano: null, topK: null, prioritario: oraIso, visto: oraIso, rank: null });
    prioritariOra.push(id);
  }
  ...
  const prioritariSet = new Set(prioritariOra);
  const ordine = [
    ...righeOra,
    ...topKOra,
    ...prioritariOra,
    ...trattenuti
      .filter((id) => !freschiSet.has(id) && !prioritariSet.has(id))
  ...
  return {
    ...
    marketIds: tenuti,
    scaduti,
    trattenuti: trattenuti.filter((id) => !freschiSet.has(id) && !prioritariSet.has(id) && tenuti.includes(id)),
    prioritari: prioritariOra.filter((id) => tenuti.includes(id)),
    prioritariTagliati: prioritariOra.filter((id) => !tenuti.includes(id)),
...
function writeCollectorPriority(plan, opts = {}) {
  ...
  // ══ I MERCATI SCELTI DALLA SELEZIONE ENTRANO FRA LE RIGHE DEL PIANO — 22 agosto 2026 ═══════════
  //
  // ⚠ E' L'ANELLO CHIUSO DI §5.2 p.55, misurato ancora una volta il 22 agosto: dei SETTE mercati
  // corti (≤ 48 h) ammissibili alla selezione, **ZERO** erano sottoscritti da agent34. La catena e':
  // il piano scarta chi ha `profondita: 'non-verificata'` → la verifica accetta solo campioni
  // websocket → agent34 sottoscrive `collector-priority` → e li' i corti entravano solo come
  // CANDIDATI, cioe' l'ultima classe, ordinati per montepremi e tagliati dal tetto di 60.
  // I corti hanno montepremi PICCOLI per costruzione ($24–$207 contro $500+ dei lunghi): ordinarli
  // per montepremi li mette in fondo **sempre**, quindi la fascia corta non poteva mai ricevere un
  // ordine, per nessun board e per nessun capitale.
  //
  // ⚠ NON ALLARGA IL PERIMETRO DI RISCHIO, ed e' il motivo per cui e' lecito: un mercato SCELTO
  // dalla selezione e' gia' abilitato al riprezzo, gia' in gestione manuale e gia' con l'uscita
  // automatica accesa (`preparaMercatoNuovo`). Sottoscriverne il libro non apre niente — toglie
  // soltanto la cecita' che gli impedisce di ricevere il capitale che qualcuno ha gia' deciso di
  // dargli. E' la stessa argomentazione, parola per parola, con cui entrano le POSIZIONI.
  // ⚠ FAIL-CLOSED A MONTE: chi chiama passa una lista vuota se lo stato non si legge, e allora
  // questa classe semplicemente non esiste — il comportamento di prima.
  const scelti = (opts.selezionati || []).map((id) => ({ id: normId(id), motivo: 'piano' })).filter((x) => x.id);
  const protetti = [...conPosizione, ...scelti];
  const u = unioneMobile({
    precedenti,
    freschi: [...freschi, ...protetti.filter((p, i) =>
      !freschi.some((f) => normId(f.id) === p.id)
      && protetti.findIndex((q) => q.id === p.id) === i)],
    candidati: opts.candidati || [],
    prioritari: opts.prioritari || [],
  ...
  const body = {
    ...
    selezionati: scelti.length,
    prioritari: u.prioritari.length,
    prioritariTagliati: u.prioritariTagliati.length,

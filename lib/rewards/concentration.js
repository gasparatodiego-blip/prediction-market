...
/**
 * L'ESPOSIZIONE MASSIMA CHE IL BOT PUO' LEGITTIMAMENTE RAGGIUNGERE, in dollari.
 *
 * ═══ PERCHE' ESISTE, E PERCHE' NON E' `N × tetto per mercato` ═══════════════════════════════════════
 * Tre test difendevano l'invariante `maxOpenNotionalUsd <= N × MARKET_CAP_FIXED_USD` — «il tetto di
 * esposizione non deve superare il massimo impegnabile, o non morderebbe mai». La RELAZIONE era
 * giusta e la GRANDEZZA sbagliata, e la differenza e' costata un pomeriggio di capitale fermo.
 *
 * Il gate confronta `openNotionalUsd + notional` **anche sugli ordini di APERTURA**, e
 * `openNotionalUsd` conta i **fill riconciliati**. Quindi lo stato peggiore che il bot attraversa
 * mentre lavora normalmente non e' «N mercati pieni», e':
 *
 *     N mercati con la coppia A RIPOSO        →  N × tetto per mercato
 *   + quegli stessi N mercati che si RIEMPIONO →  N × tetto per mercato
 *   ────────────────────────────────────────────────────────────────────
 *     esposizione massima raggiungibile        =  2 × N × tetto per mercato
 *
 * A N = 5 e tetto $61,25: $306,25 a riposo + $306,25 di fill = **$612,50**. Il cap in servizio e'
 * **$650**, deciso dall'operatore il 18 agosto 2026, e sta sopra — che e' il punto.
 *
 * ⚠ E IL CONTRARIO E' GIA' SUCCESSO, IL 16 AGOSTO 2026: il cap fu portato a $150 contro 3 × $61,25 =
 * $183,75 proprio per «farlo mordere». Risultato misurato: la gamba piu' cara del piano smetteva di
 * essere piazzabile quando i fill riconciliati superavano ~$95, cioe' **il tetto murava la gestione a
 * meta' strada** — rifiutava il BUY che completa la coppia e la SELL che liquiderebbe la gamba nuda
 * (§5-bis p.168). Un tetto che impedisce di CHIUDERE non e' un limite di rischio: e' un rischio.
 *
 * ⚠ QUESTO NON E' IL FRENO. Il freno e' il kill a −$100 di perdita giornaliera. Questo numero serve a
 * non murare la gestione, e chi lo alza non sta alzando quanto si puo' perdere.
 *
 * @param {number} mercatiContemporanei  N, cioe' `MAKER_MERCATI_CONTEMPORANEI` letto dall'ambiente
 * @returns {number|null} i dollari, oppure `null` se N non e' un numero utilizzabile — mai uno zero
 *   di comodo: uno zero qui renderebbe l'invariante vera per qualunque cap, cioe' la spegnerebbe.
 */
function esposizioneMassimaRaggiungibileUsd(mercatiContemporanei) {
  const n = Number(mercatiContemporanei);
  if (!Number.isFinite(n) || n <= 0) return null;
  // Due contributi dello stesso importo — il riposo e il suo completamento — e non e' una
  // coincidenza: la coppia a riposo costa il capitale del mercato, e riempirsi lo costa di nuovo.
  const aRiposo = n * MARKET_CAP_FIXED_USD;
  const completamento = n * MARKET_CAP_FIXED_USD;
  return +(aRiposo + completamento).toFixed(2);
}
...
function selfcheck() {
  ...
  // ⚠ QUESTE TRE RIGHE DESCRIVEVANO LA DERIVAZIONE FINO AL 16 AGOSTO 2026 — `tetto / 2 + $5` — e
  // sono rimaste a raccontarla dopo che era cambiata. Il selfcheck non gira in questa forma, quindi
  // nessuno se n'è accorto: un secondo posto che dice la cosa vecchia sullo stesso numero, cioè
  // esattamente il reperto D1 che questo modulo esiste per impedire.
  ok('il tetto per ordine è derivato dalla GAMBA PIÙ CARA quotabile, non da metà mercato',
    LIVE_MIN_ORDER_CAP_USD === +(MARKET_CAP_FIXED_USD * PREZZO_MASSIMO_QUOTABILE / COSTO_COPPIA
      + MARGINE_ORDINE_USD).toFixed(2), String(LIVE_MIN_ORDER_CAP_USD));
...
module.exports = { MARKET_CAP_FIXED_USD, LIVE_MIN_ORDER_CAP_USD, MARGINE_ORDINE_USD,
  ...
  esposizioneMassimaRaggiungibileUsd,

...
function riga(over = {}) {
  return {
    ...
    maxSpreadCents: 2, computedDefaultOffsetTicks: 1,
    capital: 100,
...
console.log('\n══ IL CAPITALE DELLA RIGA NON VIENE RADDOPPIATO: VIENE DIVISO');
{
  ...
  const basso = planToOrders({ rows: [riga({
    mid: 0.055, tick: 0.001, capital: 195, maxSpreadCents: 2.75, computedDefaultOffsetTicks: 10,
...
console.log('\n══ UN LATO SOLO NON SI PIAZZA MAI');
{
  ...
  const r = planToOrders({ rows: [riga({
    mid: 0.02, tick: 0.01, capital: 100, maxSpreadCents: 5, computedDefaultOffsetTicks: 3,
...
console.log('\n══ LE QUATTRO ESCLUSIONI, E NESSUNA E SILENZIOSA');
{
  const casi = [
    ...
    ['fuori-banda', riga({ maxSpreadCents: 0.5 })],                       // raggio 0.5c, offset 1c
...
console.log('\n══ rowAt AZZERA IL LORDO DOVE IL VENUE LO AZZERA');
{
  ...
  ok('fuori banda idem', rowAt(riga({ maxSpreadCents: 0.5 }), 1).gross === 0);

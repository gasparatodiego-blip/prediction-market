...
console.log('\n══ IL PANNELLO E QUESTO MODULO DICONO LA STESSA COSA');
{
  ...
  // I campi che il pannello manda a bulk-allocate.
  ...
  // ── UNA DIVERGENZA DICHIARATA, NON UNA DERIVA ────────────────────────────────────────────────
  // Da questa revisione i due percorsi NON costruiscono più le stesse righe, e la differenza non è
  // un dettaglio: questo modulo emette DUE gambe per mercato (BUY yes + BUY no), il pannello ne
  // manda ancora UNA. Il test lo registra invece di lasciarlo scoprire a chi legge un estratto conto:
  // finché `book: 'yes' as const` è scritto nel pannello, il bottone manuale piazza un lato solo, e
  // un lato solo matura ZERO fuori dal range [0.10, 0.90] e un terzo dentro.
  ok('DIVERGENZA NOTA: il pannello manda ancora UN LATO SOLO (bottone manuale), questo modulo due',
    src.includes("book: 'yes' as const"),
    'se questa asserzione cade, il pannello e stato reso a due gambe: allineare la nota qui sopra');

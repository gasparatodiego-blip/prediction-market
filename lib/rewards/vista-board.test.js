...
console.log('\n════ ① un mercato ammissibile oltre la vecchia soglia DEVE essere visto ════');
{
  ...
  // Il tetto vivo del modulo si legge dal modulo invece di essere ricopiato qui.
  // ⚠ NON SI ASSERISCE IL NUMERO, SI ASSERISCE LA PROPRIETA': il tetto non e' mai piu' stretto di
  // quello di partenza (150) e sta nel periodo di scansione anche al ritmo PEGGIORE misurato. Un
  // test su `=== 300` era una fotografia della costante: e' caduto il 23/08 quando il numero e'
  // passato a 343, senza che nessun comportamento fosse peggiorato.
  const OVERHEAD_S = 113, PERIODO_S = 900, RITMO_PEGGIORE_S = 2.29;   // misurati il 23/08 su 4 cicli
  const staNelPeriodo = Math.floor((PERIODO_S - OVERHEAD_S) / RITMO_PEGGIORE_S);
  ok('il tetto di produzione non e\' mai piu\' stretto di quello di partenza',
    A24.MAX_CLOB_MARKETS >= 150, String(A24.MAX_CLOB_MARKETS));
  ok(`  e sta nel periodo anche al ritmo peggiore (${staNelPeriodo})`,
    A24.MAX_CLOB_MARKETS <= staNelPeriodo, `${A24.MAX_CLOB_MARKETS} > ${staNelPeriodo}`);
}

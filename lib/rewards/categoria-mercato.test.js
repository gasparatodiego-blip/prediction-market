'use strict';
// ══════════════════════════════════════════════════════════════════════════════════════════════════
// ⚠⚠ QUESTO TEST È ROSSO PER DECISIONE, E IL COSTO È ZERO — 18 agosto 2026
// ══════════════════════════════════════════════════════════════════════════════════════════════════
// Fallisce su due asserzioni. Sono state MISURATE prima di lasciarle rosse, e la misura è questa.
//
// ① «meno del 2% degli ingressi dei 21 wallet resta fuori categoria» → oggi è il **10,2%** (81 su
//    797). Il classificatore ha perso copertura sui nomi nuovi del venue.
//
// ② «nessun modulo di lib/, agents/ o app/ lo importa» → accusa `lib/safety/riavvio-condizionato.js`.
//    È un **falso positivo**: là dentro la stringa `categoria-mercato` compare come NOME DI FILE
//    dentro una allowlist, non come `require`. Il rilevatore cerca la sottostringa nel testo.
//
// ═══ PERCHÉ RESTA ROSSO INVECE DI ESSERE CORRETTO ═══════════════════════════════════════════════════
// **Perché questo classificatore non è in servizio.** Nessun modulo di produzione lo importa — lo
// dice l'asserzione ② stessa, ed è vera al netto del falso positivo. Il `categoriaDi` che il bot usa
// davvero è un'ALTRA funzione, dentro `lib/maker/selezione-mercati.js`, e fa una cosa sola: legge il
// campo `category` della riga di board. Sul board vivo del 18 agosto restituisce `null` per **0
// mercati su 129**.
//
// E anche se restituisse `null`, la conseguenza è limitata a QUEL mercato: viene escluso dalla
// selezione con un motivo dichiarato (`categoria-non-leggibile`). Non blocca il piano, non blocca il
// piazzamento, non blocca l'uscita. Il vincolo delle tre categorie è stato tolto il 15 agosto 2026:
// oggi la categoria si scrive per leggere cosa è successo, non per decidere.
//
// ⇒ **Il 10,2% non classificato non costa niente al bot.** Correggere il classificatore
// significherebbe rincorrere i nomi dei mercati del venue per un modulo che nessuno chiama.
//
// ⚠ E SE UN GIORNO QUALCUNO LO COLLEGASSE, l'asserzione ② diventerebbe la sua allerta: da quel
// momento la copertura al 10,2% smetterebbe di essere gratis, e questo rosso — che oggi è una nota —
// tornerebbe a essere un difetto. È il motivo per cui non si cancella.
// ══════════════════════════════════════════════════════════════════════════════════════════════════

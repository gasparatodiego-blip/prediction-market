 ...
 */
function fillsByTickCurve(rows, trades, sizeUsd, tick, maxSpreadCents, spanHours, maxInventoryUsd) {
  const out = [];
  const radiusTicks = (fin(maxSpreadCents) && fin(tick) && tick > 0) ? Math.ceil((maxSpreadCents / 2) / (tick * 100)) : 3;
  const maxTick = Math.min(40, radiusTicks + 2); // cover the band + a couple beyond (out-of-band shows 0 gross)
  const spanDays = fin(spanHours) && spanHours > 0 ? spanHours / 24 : null;
  for (let t = 0; t <= maxTick; t++) {
    const offsetCents = t * tick * 100;
    const fills = reconstructTapeFillsForMarket(rows, trades, { offsetCents, sizeUsd, maxInventoryUsd }).fills;
    ...
    out.push({ tick: t, offsetCents, fills: fills.length, costPerDay: (spanDays && measured) ? adverse / spanDays : (fills.length ? null : 0) });
  }
  return out;
}
...
function planAllocation(cfg) {
  ...
  const rows = alloc.allocation.map((a) => {
    ...
    return {
      ...
      spanHours: meta.spanHours, grossPerDay: fin(a.grossPerDay) ? a.grossPerDay : null,
      ...
      fillsByTick: fillsByTickCurve(rowsJ, trades, a.sizeUsd, meta.tick, maxSpreadCents, meta.spanHours, maxInventoryUsd),
    };
  });
...
module.exports = { ...allocate, planAllocation, planFromCollection, marketMeta, frontierByCount, snapBidAsk, fillsByTickCurve, identify };

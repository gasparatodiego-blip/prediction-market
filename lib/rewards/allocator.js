...
function planAllocation(cfg) {
  const {
    ...
    maxSpreadByMarket = null, fillScoreByMarket = null, endDateByMarket = null,
  } = cfg;
...
function loadBoard() {
  ...
  return { nameMap, potByCond, maxSpreadByMarket, endDateByMarket };
}
...
function planFromCollection(opts = {}) {
  ...
  const { nameMap, potByCond: boardPots, maxSpreadByMarket, endDateByMarket } = loadBoard();
  ...
  const plan = capital > 0
    ? planAllocation({ byMarket: fundable, marketTokens, tapeByToken: tape.byToken, potByCond, budgetUsd: capital, offsetCents: 1, maxInventoryUsd: 5000, policy: 'hold', maxCount: 25, maxSpreadByMarket, fillScoreByMarket, endDateByMarket })

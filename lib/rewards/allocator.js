...
const { verdettoProfondita, esclude: profonditaEsclude, MAX_QUOTA_CREDIBILE } = require('./profondita-minima');
...
function planAllocation(cfg) {
  ...
  for (const c of allocFull.curves) {
    ...
    const v = verdettoProfondita({ sharePerUsd, depthShares: metaProf.depthShares });
    profonditaByMarket.set(c.marketId, v);
    // `ignota` non esclude mai — la regola cardinale del modulo. Solo un verdetto MISURATO toglie.
    if (filtroProfondita && profonditaEsclude(v)) sottiliRejects.add(c.marketId);
  }

  const preKnapsackRejects = sottiliRejects.size
    ? new Set([...horizonRejects, ...sottiliRejects])
    : horizonRejects;
  const keptCurves = preKnapsackRejects.size
    ? allocFull.curves.filter((c) => !preKnapsackRejects.has(c.marketId))
    : allocFull.curves;
  const alloc = preKnapsackRejects.size
    ? (() => {
  ...
  //
  // 0,30 e non un altro numero: il tetto di concentrazione per MERCATO di questo progetto è 0,20 sul
  // CAPITALE (`concentration.CONCENTRATION_CAP_FRAC`), e questo vincola una cosa diversa — il lordo
  // modellato di una CATEGORIA — quindi è deliberatamente più largo. Resta sotto la metà, che è la
  // soglia che conta: il piano non deve poter dipendere in maggioranza da mercati in cui saremmo l'unico
  // maker e che la stima realistica si rifiuta di stimare.
  ...
  const candidates = allocFull.curves.map((c) => {
    ...
    if (sottiliRejects.has(c.marketId)) {
      ...
      return { ...base, status: 'scartato', capital: 0, reasonCode: 'profondita-sottile', reason: pv ? pv.motivo : 'book strutturalmente sottile' };
    }

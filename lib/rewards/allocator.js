...
function marketMeta(rows, wsOnly = false) {
  ...
  // ── IL TICK SI CERCA, NON SI PRENDE DAL PRIMO CAMPIONE — 22 agosto 2026 ────────────────────────
  //
  // Qui c'era `rows[0] && fin(rows[0].tick) ? rows[0].tick : null`, cioe' il tick letto dal SOLO
  // primo campione. E il primo campione di un mercato e' precisamente quello che il tick non ce
  // l'ha: agent34 scrive la prima riga di `mid-history` nell'istante in cui sottoscrive il libro,
  // prima di avere i metadati del mercato, quindi ci mette `tick: null`. Dal secondo in poi c'e'.
  //
  // ⚠ LA CONSEGUENZA NON ERA UN NUMERO SBAGLIATO, ERA UN MERCATO CHE SPARIVA. `tick: null` arriva
  // fino a `plan-to-orders.js:124` (`offsetCents = fin(r.tick) ? … : null`), che a `:163` respinge la
  // riga intera con `gamba-impossibile` — quindi il mercato entra in piano, occupa uno slot, e non
  // riceve MAI un ordine. Niente lo dichiarava: il giornale registra solo `gamba-impossibile`, che
  // suona come una decisione di rischio e invece era un campo mancante.
  //
  // MISURATO sul giornale prima della correzione: **24 mercati in 48 ore** (9 il 21/08, 15 il 22/08)
  // avevano il tick nei campioni successivi e sono stati dichiarati non quotabili per via del primo.
  // Il 22 agosto erano i **tre mercati della fascia corta** appena cablata — 66, 66 e 60 campioni col
  // tick contro 1, 1 e 7 senza — cioe' sei gambe mai inviate e ~$184 di capitale fermo.
  // ⚠ E COLPISCE PER COSTRUZIONE LE SOTTOSCRIZIONI NUOVE: il primo campione e' senza tick per
  // definizione, quindi il difetto puniva sempre e solo i mercati appena entrati nel feed — che dal
  // 22 agosto sono, per disegno, tutta la fascia corta.
  //
  // ⚠ LA CORREZIONE E' MONOTONA E NON PUO' SPOSTARE UN PREZZO. Il tick e' una proprieta' del venue,
  // costante dentro un mercato: verificato su 454 mercati campionati, **zero** con piu' di un valore.
  // Quindi «il primo non nullo» e «il primo» coincidono ovunque il secondo esista gia', e la
  // correzione puo' solo trasformare un `null` in un valore vero — mai cambiarne uno.
  // ⚠ E RESTA FAIL-CLOSED: se NESSUN campione porta il tick, il risultato e' ancora `null`, e a valle
  // il mercato viene rifiutato esattamente come prima. Non si inventa un tick di difetto: 0,01 e'
  // il valore comune ma non universale (esistono griglie a 0,001), e indovinarlo metterebbe ordini
  // veri su una griglia sbagliata — che il venue rifiuterebbe, o peggio accetterebbe al prezzo storto.
  // ⚠ SI GUARDA `rows` E NON `src`: il tick non dipende dalla fonte del campione (websocket o REST),
  // e restringere a `src` toglierebbe righe buone senza aggiungere nessuna garanzia.
  // ⚠ `findIndex` E NON `find`, ed e' la regola di §5.3 di questo repo: in un array che puo'
  // contenere elementi falsy, «esiste un elemento che...» si scrive col sentinella `-1`, che non puo'
  // collidere con un elemento legittimo. Qui il predicato guarda gia' `r &&`, quindi `find` sarebbe
  // corretto — ma e' la forma che in questo repo ha gia' prodotto un guasto (`plan-to-orders.js:157`),
  // e scriverla giusta costa una riga.
  const iTick = rows.findIndex((r) => r && fin(r.tick));
  const tick = iTick !== -1 ? rows[iTick].tick : null;
...
function planAllocation(cfg) {
  ...
  if (CAP_CODA_FRAC > 0 && CAP_CODA_FRAC < 1) {
    ...
    // ── ⚠ UNA QUOTA E' UNA PROPORZIONE, E SENZA FASCIA CORTA DIVENTA UN DIVIETO — 18 agosto 2026 ───
    //
    // IL FATTO, misurato sul bot vivo alle 20:30Z. Il board offriva **6 mercati ammissibili e ZERO in
    // fascia corta**: tutti oltre i 7 giorni. Con `corte` vuoto, `capCorto = 0`, quindi
    // `budgetCodaLungaUsd` restituisce 0 (riga 266), quindi `unitsCoda = 0`, quindi **nessuna
    // allocazione**. Il piano usciva con zero righe e il bot restava con il perimetro pieno e il libro
    // VUOTO — per mezz'ora, con l'operatore che aveva chiesto un mercato quotato 24 ore su 24.
    //
    // La quota esiste per limitare **quanta parte** del capitale resta immobilizzata oltre una
    // settimana. Quando NON esiste una fascia corta non c'e' nessuna parte da limitare: la scelta non
    // e' piu' «quanto in coda lunga contro quanto in fascia corta», e' «coda lunga oppure NIENTE». E
    // rispondere «niente» non protegge da nessun rischio — lascia il capitale fermo, che e' l'unico
    // esito che la quota non stava cercando di produrre.
    //
    // ⚠⚠ QUESTO ALLARGA UN LIMITE DI RISCHIO, e va detto per intero: da qui il **100%** del piano puo'
    // stare oltre i 7 giorni, dove la chiusura e' a redeem nel 94% dei casi misurati. Cio' che continua
    // a limitare l'esposizione, e non e' stato toccato: il **tetto per mercato** ($61,25), il **numero
    // di slot** (`MAKER_MERCATI_CONTEMPORANEI`), l'**esposizione cumulativa** ($150) e il **kill a
    // −$100**. A uno slot il caso peggiore resta $61,25 — cioe' la quota, qui, non stava aggiungendo
    // nessuna protezione che quegli altri quattro non diano gia'.
    //
    // ⚠ E NON SI ALLENTA QUANDO LA FASCIA CORTA C'E': se `corte` ha anche una sola curva, la doppia
    // passata resta identica a prima, quota compresa. La condizione e' l'ASSENZA, non la scarsita'.
    if (lunghe.length && corte.length === 0) {
      // Nessuna passata: `allocFinale` resta quello del knapsack libero, che rispetta gia' tetto per
      // mercato, minimi del venue e tutti i filtri a monte. Si dichiara, perche' un piano tutto in coda
      // lunga e' un fatto che chi legge il referto domani deve poter vedere senza dedurlo.
      codaBudgetUsd = null;
    } else if (lunghe.length) {
...
function planFromCollection(opts = {}) {
  ...
  // ⚠ I DUE CAMPI PESANTI NON SI COSTRUISCONO AFFATTO — 21 agosto 2026.
  // `levels` e `no` valgono l'85,3% del testo di ogni riga (misurato: 40,1% + 45,2%) e NESSUN
  // consumatore di righe di giornale li legge. Che `levels` fosse scartabile lo diceva gia' la riga
  // qui sotto, che resta: ma un `r.levels = undefined` DOPO `loadJournal` libera la memoria dopo
  // averla allocata, cioe' dopo il picco — ed era il picco a uccidere il figlio (924 MB).
  // `no` e' il gemello aggiunto il 19 agosto per la ricerca su R4 (§5.2 p.43), e da quel giorno il
  // file giornaliero e' passato da ~148 a ~283 MB: e' la data in cui il piano ha smesso di girare.
  const J = loadJournal({ fromMs, toMs, scartaCampi: ['levels', 'no'] });
  const tape = loadTape({ fromMs, toMs });
  // Resta come cintura: se un giorno `scartaCampi` non venisse onorato, la garanzia di prima e' ancora qui.

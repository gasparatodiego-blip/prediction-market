...
function planAllocation(cfg) {
  ...
  const alloc = allocateBudget(byMarket, marketTokens, tapeByToken, potByCond, {
    offsetCents, maxInventoryUsd, budgetUsd, unitUsd, maxPerMarketUsd: budgetUsd, policy, minSizeByMarket,
  });
...
function loadBoard() {
  ...
  return { nameMap, potByCond, maxSpreadByMarket, endDateByMarket, minSizeByMarket };
}
...
function planFromCollection(opts = {}) {
  ...
  const { nameMap, potByCond: boardPots, maxSpreadByMarket, endDateByMarket, minSizeByMarket } = loadBoard();
  ...
  const plan = capital > 0
    ? planAllocation({ byMarket: fundable, marketTokens, tapeByToken: tape.byToken, potByCond, budgetUsd: capital, offsetCents: 1, maxInventoryUsd: 5000, policy: 'hold', maxCount: 25, maxSpreadByMarket, fillScoreByMarket, endDateByMarket, minSizeByMarket })
    : { budgetUsd: 0, unitUsd: 0, offsetCents: 1, marketsUsed: 0, totalCapital: 0, unallocated: 0, totalGrossPerDay: 0, totalNetPerDay: null, totalRealisticPerDay: { grossPerDay: 0, realisticPerDay: 0, ratio: null, rowsCounted: 0, rowsUnknown: 0 }, frontier: [], rows: [] };

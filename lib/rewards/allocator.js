...
function planAllocation(cfg) {
  ...
  // costa `1 − 2d` dollari, e la cosa notevole è che NON dipende dal mid. Quindi è uno scalare per tutto
  // il piano, non una colonna per mercato.
  //
  ...
  //
  // `pairCostUsd` viaggia come opzione, assente per difetto in tutta la catena a valle, così il
  // backtest e la pipeline del ceiling — che modellano il lato ask come vendita di inventario già
  // posseduto — restano byte per byte quelli di prima.
  const pairCostUsd = usePairCost ? +(1 - 2 * (offsetCents / 100)).toFixed(6) : null;
  const allocFull = allocateBudget(byMarket, marketTokens, tapeByToken, potByCond, {
    offsetCents, maxInventoryUsd, budgetUsd, unitUsd, maxPerMarketUsd: capPerMarketUsd, policy, minSizeByMarket, pairCostUsd,
  });
  ...
  const rows = alloc.allocation.map((a) => {
    ...
    const sizePerSideShares = pairCostUsd != null
      ? a.capital / pairCostUsd
  ...
  return {
    ...
    // Il costo della coppia con cui questo piano e' stato classificato. null = modello storico
    // (lato ask come inventario). Dichiarato perche' due piani con due modelli di size non sono
    // confrontabili, e la differenza non si vede dai totali.
    pairCostUsd,
...
function loadBoard() {
  ...
  return { nameMap, potByCond, maxSpreadByMarket, endDateByMarket, endDateSourceByMarket, minSizeByMarket, boardAtMs };
}
...
function planFromCollection(opts = {}) {
  ...
  const { nameMap, potByCond: boardPots, maxSpreadByMarket, endDateByMarket, endDateSourceByMarket, minSizeByMarket, boardAtMs } = loadBoard();
  ...
  const plan = capital > 0
    ? planAllocation({ byMarket: fundable, marketTokens, tapeByToken: tape.byToken, potByCond, budgetUsd: capital, offsetCents: 1, maxInventoryUsd: 5000, policy: 'hold', maxCount: 25, maxSpreadByMarket, fillScoreByMarket, endDateByMarket, endDateSourceByMarket, minSizeByMarket, horizonFilter, maxPerMarketUsd: opts.maxPerMarketUsd ?? null, usePairCost: opts.usePairCost !== false, nowMs })
  ...
  return {
    ...
    sizing: {
      ...
      model: plan.pairCostUsd != null ? 'coppia-in-collaterale' : 'ask-da-inventario',
      note: plan.pairCostUsd != null
        ? `share per lato = capitale / ${plan.pairCostUsd} (comprare YES a mid−d e NO a 1−mid−d costa 1−2d a coppia, indipendentemente dal mid)`

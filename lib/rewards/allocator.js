...
// IL PAVIMENTO DEL VENUE, per il profilo Risk. Importato da risk-classifier, che a sua volta lo deriva
// da order-ttl.VENUE_GTD_MIN_FUTURE_SEC: nessun 3 scritto qui dentro.
const { VENUE_FLOOR_MINUTES } = require('../maker/risk-classifier');
const { RULE_VENUE_FLOOR, resolveProfile } = require('./allocator-profiles');
...
function planAllocation(cfg) {
  const {
    ...
    horizonFilter = false,
    // ── QUALE REGOLA DI SCADENZA APPLICA IL FILTRO ────────────────────────────────────────────────
    // `null` (il difetto) è il ramo che esisteva prima che i profili esistessero, e resta letteralmente
    // lo stesso codice: nessun chiamante che non passi questa opzione può vedere un piano diverso.
    // Vedi lib/rewards/allocator-profiles.js.
    //
    //   null | 'adverse-selection-recovery'  il rientro dal costo di adverse selection (horizon.js).
    //                                        È il profilo SAFE, ed è il comportamento storico.
    //   'venue-floor'                        scarta SOLO ciò che il venue rifiuterebbe: meno di 3
    //                                        minuti alla chiusura. È il profilo RISK.
    //
    // Il filtro NON viene spento dal profilo Risk, gli viene cambiata la regola: un mercato già chiuso
    // deve restare fuori da entrambi i piani, e un profilo che lo lasciasse passare sarebbe rotto, non
    // tollerante.
    minTimeToCloseRule = null,
  ...
  if (horizonFilter) {
    // La regola del PAVIMENTO DEL VENUE, usata solo dal profilo Risk. Scarta ciò che il venue
    // rifiuterebbe comunque — meno di VENUE_FLOOR_MINUTES alla chiusura — e nient'altro. Una scadenza
    // ILLEGGIBILE non scarta: qui vale la stessa regola di horizonVerdict, l'assenza di un fatto non
    // veste i panni del fatto.
    const sottoIlPavimento = (mid) => {
      const min = minutiAllaChiusuraDelMercato(endDateByMarket, mid, nowMs);
      return min != null && min <= VENUE_FLOOR_MINUTES;
    };
    const perPavimento = minTimeToCloseRule === RULE_VENUE_FLOOR;
    for (const [mid, v] of horizonByMarket.entries()) {
      const bindsOnHorizon = perPavimento
        ? sottoIlPavimento(mid)
        : (v.state === 'resolved' || (v.state === 'short' && fin(v.payback)));
  ...
  return {
    ...
    candidates,
    horizonFilter,
    // QUALE regola di scadenza ha applicato il filtro. `null` = quella storica (profilo Safe). Sta nel
    // referto perché un piano deve poter dire con che criterio è stato costruito, e perché il test di
    // non-regressione lo confronta.
    minTimeToCloseRule,
...
function planFromCollection(opts = {}) {
  ...
  // ── IL PROFILO ────────────────────────────────────────────────────────────────────────────────
  // Assente ⇒ nessun profilo, e il percorso resta ESATTAMENTE quello di prima: `horizonFilter` dal
  // chiamante e `minTimeToCloseRule` a null. Il profilo Safe passato esplicitamente produce gli stessi
  // due valori — è la ragione per cui la tab Ottimizza può adottarlo senza cambiare un piano.
  const profile = opts.profile ? resolveProfile(opts.profile) : null;
  const horizonFilter = profile ? profile.horizonFilter === true : opts.horizonFilter === true;
  const minTimeToCloseRule = profile && profile.minTimeToCloseRule === RULE_VENUE_FLOOR ? RULE_VENUE_FLOOR : null;
  const plan = capital > 0
    ? planAllocation({ byMarket: fundable, marketTokens, tapeByToken: tape.byToken, potByCond, budgetUsd: capital, offsetCents: 1, maxInventoryUsd: 5000, policy: 'hold', maxCount: 25, maxSpreadByMarket, fillScoreByMarket, endDateByMarket, endDateSourceByMarket, minSizeByMarket, touchByMarket, horizonFilter, minTimeToCloseRule, maxPerMarketUsd: opts.maxPerMarketUsd ?? null, usePairCost: opts.usePairCost !== false, nowMs })
  ...
  return {
    ...
    universe: {
      ...
      horizonFilter,
      // Il profilo con cui il piano è stato costruito, per nome. `null` = chiamata senza profilo, cioè
      // il percorso storico. Il pannello lo mostra, così una card non può mai essere letta sotto il
      // profilo sbagliato.
      profile: profile ? profile.key : null,
      minTimeToCloseRule,

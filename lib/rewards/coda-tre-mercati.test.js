...
console.log('\n══ 6 · IL CABLAGGIO, E CIÒ CHE NON È STATO TOCCATO');
{
  const ap = leggi('app', 'components', 'RewardsAllocatePanel.tsx');
  ok('il pannello usa il modulo della coda, non una copia',
    /from '@\/lib\/rewards\/coda-piazzamento'/.test(ap));
  ok('  e avanza solo tramite decidiAvanzamento', /decidiAvanzamento\(\{ coda, esito: placed/.test(ap));
  ok('«Conferma e piazza» apre il pannello ordine col target del piano',
    /onPlaceOrder\(testaTarget\)/.test(ap));
  ok('  cioè lo STESSO target del bottone della singola riga', /targetFromPlanRow\(testaRiga,/.test(ap));
  ok('esistono «Salta questo» e «Annulla coda»',
    /data-alloc-queue-skip/.test(ap) && /data-alloc-queue-cancel/.test(ap));
  ok('la persistenza salva SOLO gli id, mai i prezzi',
    /sessionStorage\.setItem\(CODA_KEY, JSON\.stringify\(coda\)\)/.test(ap)
    && !/JSON\.stringify\(\{ *coda, *prezzi/.test(ap));

  // La coda non ha un percorso di invio suo: la prova è che il modulo non conosce nessun endpoint.
  ...
  // L'AVVISO CHE LEGA I DUE PERCORSI. Il reset cancella anche ciò che la coda ha appena piazzato:
  // è ciò che un reset deve fare, ma dalla schermata non si leggeva.
  ok('c è un avviso quando il reset disferebbe il lavoro della coda',
    /data-alloc-bulk-vs-queue/.test(ap));
  ok('  condizionato ai piazzamenti VERI, non sempre a schermo',
    /codaRiepilogo\.piazzati > 0 && \(/.test(ap),
...
console.log(`\ncoda di tre mercati: ${pass} passati, ${fail} falliti`);

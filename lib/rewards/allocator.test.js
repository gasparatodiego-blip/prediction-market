...
console.log('planAllocation — sotto il minimo del venue la riga vale ZERO, e dice quanto serve');
{
  ...
  // 500 share per lato × costo della coppia. ⚠ Era «$250/lato = $500 in tutto», cioe' il modello
  // `(C/2)/mid`, tolto il 12 agosto 2026: le due gambe costano insieme ~$1 per coppia, non 2×mid.
  const SDC_A = require('./size-da-capitale');
  ok(`e il capitale che lo sbloccherebbe ($${SDC_A.capitalePerQualificare({ minSize: 500 })}), calcolato dalla stessa regola`,
    b && near(b.capitalToQualifyUsd, SDC_A.capitalePerQualificare({ minSize: 500 })));

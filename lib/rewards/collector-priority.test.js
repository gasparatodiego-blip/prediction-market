...
console.log('\n══ TRE CICLI CONSECUTIVI CON DATI NOTI — LO SCENARIO CHIESTO');
{
  // La prova end-to-end sul FILE, non sulla funzione pura: tre piani di seguito, con un mercato che
  // entra come QUASI-VINCITORE al primo ciclo e poi non viene più né scelto né valutato in alto.
  // Deve restare caldo per tutta la finestra di isteresi, e uscire solo dopo.
  const F = path.join(tmp, 'tre-cicli.json');
  const OSPITE = ID(77);                    // entra fra i candidati al ciclo 1 e non torna mai
  const pianoDi = (rigaId, extra = []) => ({
    rows: [{ marketId: rigaId }],
    candidates: [{ marketId: rigaId, bestObiettivoPerDay: 100 }, ...extra],
  });

  // Ciclo 1 (t=0): OSPITE è top-K ma non riga.
  writeCollectorPriority(pianoDi(ID(1), [{ marketId: OSPITE, bestObiettivoPerDay: 50 }]), { file: F, nowMs: ORA });
  const c1 = readCollectorPriority({ file: F, nowMs: ORA + 1000 });
  ok('ciclo 1: il quasi-vincitore entra in elenco senza essere una riga', c1.marketIds.includes(OSPITE));

  // Ciclo 2 (t=+6h): un altro piano, OSPITE non compare da nessuna parte.
  writeCollectorPriority(pianoDi(ID(2)), { file: F, nowMs: ORA + 6 * H });
  const c2 = readCollectorPriority({ file: F, nowMs: ORA + 6 * H + 1000 });
  ok('ciclo 2 (+6h): non è più né riga né top-K, ma è ANCORA caldo', c2.marketIds.includes(OSPITE));
  ok('  e la riga nuova c è', c2.marketIds.includes(ID(2)));
  ok('  e quella vecchia non è sparita: era una riga un ciclo fa', c2.marketIds.includes(ID(1)));

  // Ciclo 3 (t=+11h): ancora dentro la finestra di 12h.
  writeCollectorPriority(pianoDi(ID(3)), { file: F, nowMs: ORA + 11 * H });
  const c3 = readCollectorPriority({ file: F, nowMs: ORA + 11 * H + 1000 });
  ok('ciclo 3 (+11h): dentro la finestra di isteresi, è ancora caldo', c3.marketIds.includes(OSPITE));
  ok('  insieme a tutte e tre le righe dei tre cicli',
    [ID(1), ID(2), ID(3)].every((x) => c3.marketIds.includes(x)));

  // Ciclo 4 (t=+13h): oltre RETENTION (12h). Adesso, e solo adesso, esce.
  const c4scritto = writeCollectorPriority(pianoDi(ID(4)), { file: F, nowMs: ORA + 13 * H });
  const c4 = readCollectorPriority({ file: F, nowMs: ORA + 13 * H + 1000 });
  ok(`ciclo 4 (+13h, oltre RETENTION ${RETENTION_MS / H}h): esce davvero`, !c4.marketIds.includes(OSPITE));
  ok('  e la scadenza è contata, non silenziosa', c4scritto.scaduti >= 1, `${c4scritto.scaduti} scaduti`);
  ok('  mentre la riga di questo ciclo c è', c4.marketIds.includes(ID(4)));
}
{
  // IL TETTO MORDE, E MORDE NEL VERSO GIUSTO. Venti cicli con righe sempre nuove e trenta candidati
  // ciascuno: senza tetto l'elenco crescerebbe di venti-e-passa mercati per volta.
  const F = path.join(tmp, 'crescita.json');
  const dimensioni = [];
  for (let c = 0; c < 20; c++) {
    const b = writeCollectorPriority({
      rows: [{ marketId: ID(200 + c) }],
      candidates: Array.from({ length: 30 }, (_, i) => ({ marketId: ID(300 + c * 30 + i), bestObiettivoPerDay: 100 - i })),
    }, { file: F, nowMs: ORA + c * H });
    dimensioni.push(b.marketIds.length);
  }
  ok(`venti cicli con mercati sempre nuovi: l elenco non supera MAI il tetto di ${MAX_MARKETS}`,
    dimensioni.every((x) => x <= MAX_MARKETS), `massimo raggiunto ${Math.max(...dimensioni)}`);
  ok('  e non decresce fino a svuotarsi: la corsia resta popolata', dimensioni[dimensioni.length - 1] === MAX_MARKETS);
  const ultimo = readCollectorPriority({ file: F, nowMs: ORA + 19 * H + 1000 });
  ok('  la riga dell ultimo ciclo è dentro: al tetto cedono i trattenuti, mai chi si sta per piazzare',
    ultimo.marketIds.includes(ID(219)));
}
{
  // I TRE NUMERI STANNO IN PIEDI INSIEME. Un tetto più basso della somma «righe + TOP_K» farebbe
  // sfrattare mercati su cui si sta per piazzare, che è l'unica cosa che questa corsia non può fare.
  const RIGHE_OSSERVATE_MAX = 9;    // misurato l'8 agosto 2026 su 22 piani reali (7-9 righe)
  ok(`il tetto (${MAX_MARKETS}) regge righe osservate (${RIGHE_OSSERVATE_MAX}) + TOP_K (${TOP_K})`,
    MAX_MARKETS >= RIGHE_OSSERVATE_MAX + TOP_K, `${RIGHE_OSSERVATE_MAX} + ${TOP_K} = ${RIGHE_OSSERVATE_MAX + TOP_K}`);
  ok('  e resta spazio per l isteresi', MAX_MARKETS - (RIGHE_OSSERVATE_MAX + TOP_K) >= 5,
    `${MAX_MARKETS - RIGHE_OSSERVATE_MAX - TOP_K} slot per i trattenuti`);
}
{
  // LA GRADUATORIA SI ORDINA SULL OBIETTIVO DEL KNAPSACK, NON SULLA CIFRA DA MOSTRARE.
  // `bestNetPerDay` è null quando nessun fill è stato osservato: ordinare su quello escludeva i
  // mercati silenziosi, cioè proprio quelli che il piano sceglie. Misurato l'8 agosto 2026: 412 delle
  // 755 righe future esaminate erano fuori graduatoria per questo motivo, contro 142 su 666 dopo.
  const piano = {
    rows: [],
    candidates: [
      { marketId: ID(31), bestObiettivoPerDay: 90, bestNetPerDay: null },   // silenzioso: 0 fill
      { marketId: ID(32), bestObiettivoPerDay: 10, bestNetPerDay: 10 },
    ],
  };
  const p = priorityFromPlan(piano);
  ok('un mercato SENZA fill osservati entra in graduatoria: il netto assente non è un netto basso', p.includes(ID(31)));
  ok('  e ci entra al posto giusto, davanti a chi rende meno', p[0] === ID(31), p.map((x) => x.slice(0, 6)).join(','));
  const vecchio = priorityFromPlan({ rows: [], candidates: [{ marketId: ID(33), bestNetPerDay: 7 }] });
  ok('i piani di formato vecchio (solo bestNetPerDay) continuano a funzionare', vecchio.includes(ID(33)));
}

...
//
// Il venue paga S(v,s) = ((v−s)/v)². Con v = max_spread = 4,5¢ (la semiampiezza ufficiale, SSOT in
// lib/banda-premiante — NON maxSpread/2, che era la lettura sbagliata corretta il 13 agosto 2026):
//     tick 0,01  → 1,0¢ dal mid → S = 0,6049
//     tick 0,001 → 0,1¢ dal mid → S = 0,9560        → 1,58 volte tanto
...
console.log('\n══ 1 · IL PESO DELLA POSIZIONE REALE');
{
  ...
  ok('il rapporto fra i due è ~1,58: è tutta qui la sottovalutazione', Math.abs(fine.S / grosso.S - 1.580) < 0.01, (fine.S / grosso.S).toFixed(3));
...
//   PIENO: quota 100/(100+1000) = 9,09% · fattore posizione 0,6049 → obiettivo = pot × 0,05499
//   VUOTO: quota 100/(100+5)   = 95,2% · fattore posizione 0,9560 → obiettivo = pot × 0,9105
//                                        col tetto (0,6/0,952 = 0,6303) → pot × 0,5738
// Con pot 100 e 7,5: senza tetto VUOTO vale 6,83 contro 5,50; col tetto 4,30 contro 5,50. Il tetto
// non e' una penalita' generica — sposta la scelta esattamente dove il book non regge la quota.
// ⚠ I FATTORI DI POSIZIONE SONO CAMBIATI il 13 agosto 2026 (0,3293→0,6049 e 0,9039→0,9560) perche' e'
// cambiata la lettura di v: la banda e' ±max_spread, non ±max_spread/2 (SSOT lib/banda-premiante). Il
// montepremi di VUOTO e' stato rimesso in scala da 4,5 a 7,5 per tenere la finestra in cui il tetto
// RIBALTA la scelta — che e' il soggetto di questa sezione. La finestra utile e' pot_VUOTO in (6,0 · 9,6).
const potByCond2 = new Map([['PIENO', 100], ['VUOTO', 7.5]]);

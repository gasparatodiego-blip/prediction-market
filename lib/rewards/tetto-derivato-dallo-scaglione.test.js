...
//
// ⚠ QUESTA ASSERZIONE PRETENDEVA CHE QUATTRO MERCATI PIENI STESSERO DENTRO IL TETTO, cioè che il
// tetto di esposizione fosse più largo del massimo impegnabile — che è chiedergli di non mordere mai.
// E il «4» era un numero scritto a mano: da R1 quanti mercati il bot tiene attivi lo decide
// l'operatore da una variabile d'ambiente, quindi moltiplicare per una costante difende una
// configurazione e non una proprietà. Adesso N si LEGGE, e le due asserzioni sono quelle vere.
{
  const { quantiMercati } = require('../maker/quanti-mercati');
  const N = (() => {
  // ⚠ N NON HA PIU' UN DIFETTO (24/08): si legge dalla CONFIGURAZIONE DICHIARATA con la funzione
  // VERA, mai da `process.env` di una shell — che non ha la variabile e farebbe SOLLEVARE.
  const eco = require('../../agents/ecosystem.config.js');
  const app = eco.apps.find((a) => a.name === 'agent41-realloc-scheduler');
  return require('../maker/quanti-mercati').quantiMercati(app.env).quanti;
})();
  // ══ L'INVARIANTE, RISCRITTA SULLA GRANDEZZA GIUSTA — 19 agosto 2026, decisione dell'operatore ══
  //
  // Qui c'era `maxOpenNotionalUsd <= N × tetto per mercato`, con la motivazione «chiedergli di essere
  // piu' grande del massimo impegnabile e' chiedergli di non mordere mai». **La relazione era giusta e
  // la grandezza sbagliata**, e le due cose vanno separate perche' e' facile confonderle.
  //
  // Il gate somma `openNotionalUsd + notional` ANCHE sugli ordini di apertura, e `openNotionalUsd`
  // conta i fill riconciliati: lo stato peggiore che il bot attraversa lavorando normalmente e'
  // **N coppie a riposo PIU' il loro completamento**, non «N mercati pieni». A N = 5 sono $306,25 +
  // $306,25 = **$612,50**, contro un cap di **$650**.
  //
  // ⚠ IL CONTRARIO E' GIA' STATO PROVATO SU CAPITALE VERO, il 16 agosto: cap a $150 contro 3 x $61,25
  // «per farlo mordere», e il tetto ha murato la gestione a meta' strada — rifiutava il BUY che
  // completa la coppia e la SELL che liquida la gamba nuda (§5-bis p.168). Un tetto che impedisce di
  // CHIUDERE non e' un limite di rischio, e' un rischio.
  // ⚠ E IL FRENO NON E' QUESTO: e' il kill a −$100 di perdita giornaliera. Alzare questo numero non
  // alza quanto si puo' perdere — toglie soltanto un muro alla gestione.
  // ⚠ LA GRANDEZZA VIVE IN UN POSTO SOLO (`concentration.esposizioneMassimaRaggiungibileUsd`), o le
  // tre copie di questa aritmetica divergerebbero come e' gia' successo col tetto per ordine.
  const raggiungibile = C.esposizioneMassimaRaggiungibileUsd(N);
  ok('N si legge dall ambiente e la grandezza e calcolabile', Number.isFinite(raggiungibile), `N=${N} ⇒ ${raggiungibile}`);
  ok('⚑ il tetto di esposizione COPRE l esposizione massima raggiungibile (riposo + completamento)',
    limiti.global.maxOpenNotionalUsd >= raggiungibile,
    `$${limiti.global.maxOpenNotionalUsd} contro N=${N} × 2 × $${C.MARKET_CAP_FIXED_USD} = $${raggiungibile}`);
  ok('  ma resta abbastanza largo da aprire almeno un mercato intero',
    limiti.global.maxOpenNotionalUsd >= C.MARKET_CAP_FIXED_USD,
    `$${limiti.global.maxOpenNotionalUsd} contro $${C.MARKET_CAP_FIXED_USD}`);
}
...
for (const p of file) {
  ...
  // ⚠ IL CORPO DI `selfcheck()` È ESENTE, ed è la stessa disciplina per cui `_archivio` è escluso dai
  // test strutturali: là dentro il valore non DECIDE, è una fixture. `coppia-simmetrica.js` è un
  // modulo PURO che riceve il tetto come parametro — non lo conosce, ed è il punto — e il suo
  // selfcheck deve passargliene uno per provarlo. Diciannove righe di prove non sono diciannove
  // copie di una costante: accusarle rende il rilevatore rumoroso, e un rilevatore rumoroso viene
  // ignorato proprio il giorno in cui trova una copia vera.
  const testo = fs.readFileSync(p, 'utf8');
  const iSelf = testo.indexOf('function selfcheck(');
  const dentroSelfcheck = iSelf < 0 ? () => false
    : ((soglia) => (n) => n >= soglia)(testo.slice(0, iSelf).split('\n').length - 1);
  testo.split('\n').forEach((l, i) => {
    const t = l.trim();
    if (t.startsWith('//') || t.startsWith('*') || t.startsWith('/*')) return;
    if (dentroSelfcheck(i)) return;

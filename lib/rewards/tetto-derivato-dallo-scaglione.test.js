...
ok('il tetto per ORDINE è metà tetto più il margine',
  Math.abs(C.LIVE_MIN_ORDER_CAP_USD - (C.MARKET_CAP_FIXED_USD / 2 + C.MARGINE_ORDINE_USD)) < 1e-9,
  `$${C.LIVE_MIN_ORDER_CAP_USD}`);
ok('  e la versione parametrica sul capitale dà lo stesso numero',
  C.liveMinOrderCapUsd(C.CAPITALE_RIFERIMENTO_USD) === +(C.capPerMarketUsd(C.CAPITALE_RIFERIMENTO_USD) / 2 + C.MARGINE_ORDINE_USD).toFixed(2));
ok('  e resta STRETTAMENTE sotto il tetto per mercato: una gamba non è un mercato',
  C.LIVE_MIN_ORDER_CAP_USD < C.MARKET_CAP_FIXED_USD);

// ⚠ LA FINESTRA DI MID SI STRINGE QUANDO IL TETTO SALE, e non è un difetto: `MARGINE_ORDINE_USD` è
// fisso a $5, quindi pesa relativamente meno su un tetto più grande. Va DICHIARATO perché è il
// prezzo nascosto di ogni leva sul tetto — misurato il 13 agosto: da [0,36 · 0,64] a [0,43 · 0,57],
// e 6 righe su 7 del piano vengono ridotte da questo, non dal tetto.
...
ok('  e si STRINGE se il tetto sale, perché il margine per ordine è fisso',
  (() => {
    const stretta = C.COSTO_COPPIA * (C.MARKET_CAP_FIXED_USD * 2 / 2 + C.MARGINE_ORDINE_USD) / (C.MARKET_CAP_FIXED_USD * 2);
    const larga = C.COSTO_COPPIA * (C.MARKET_CAP_FIXED_USD / 2 + C.MARGINE_ORDINE_USD) / C.MARKET_CAP_FIXED_USD;
    return stretta < larga;
  })(), `${f.lo}–${f.hi}`);
...
const SALTA = new Set(['node_modules', '.next', '.git', 'data', 'public', 'docs']);

...
console.log('\n══ LA SOGLIA DELLA SIZE MINIMA SI MUOVE INSIEME ALLA FORMULA');
{
  ...
  const minSize = 200;
  // ⚠ RIBALTATA il 12 agosto 2026. Diceva «senza costo della coppia: 2 × mid × minSize (invariato)».
  // Quel ramo NON esisteva piu' come ripiego accettabile: era la formula sbagliata, e senza il costo
  // della coppia si usa il TIPICO 0,98 — dichiarato — invece di tornare a dipendere dal mid.
  ok('senza costo della coppia: minSize × costo tipico, NON piu 2 × mid × minSize',
    vicino(capitalToQualify(0.055, minSize), minSize * require('./size-da-capitale').COSTO_COPPIA_TIPICO)
  ...
  // E la controprova sulla formula VECCHIA, riprodotta qui a mano perche' nel repo non esiste piu':
  // $22 su un minimo di 200 share non bastavano, ed e' il difetto che il lavoro chiude.
  const vecchiaSoglia = 2 * 0.055 * minSize;
  ok(`  mentre la vecchia soglia ($${vecchiaSoglia.toFixed(2)}) NON basta`,
    shareForCapital(0, 0.055, vecchiaSoglia, minSize, PAIR) === 0, `vecchia soglia $${vecchiaSoglia.toFixed(2)}`);
}

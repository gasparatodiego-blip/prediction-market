...
console.log('\n1 · UNA sola misura, e le due parti la leggono dallo stesso punto');
{
  ...
  // ⚠ QUI C'ERA `=== 18`, cioè il valore del giorno in cui il test è stato scritto. Il soggetto di
  // questo file è che le due parti leggano LA STESSA misura, non quale sia: il pavimento è passato a
  // 12 ore il 13 agosto 2026 e questa asserzione sarebbe diventata rossa senza nessun difetto.
  ok('  ed è un numero di ore utilizzabile', HORIZON_MIN_HOURS > 0 && HORIZON_MIN_HOURS < 48, `${HORIZON_MIN_HOURS} h`);
...
// ────────────────────────────────────────────────────────────────────────────────────────────────
console.log('\n3 · IL CASO VERO DEL 12 AGOSTO ⇒ ESCLUSO A MONTE');
{
  // La coorte che ha fermato il ciclo aveva 14,3 ore residue e col pavimento di allora (18 h) veniva
  // bocciata. ⚠ Col pavimento di OGGI (12 h) quelle 14,3 ore PASSANO, ed è la decisione del 13 agosto:
  // la fascia 12-18 h è quella che l'operatore ha deliberatamente riaperto. Quindi il caso si
  // riformula sulla proprietà che non è cambiata — sotto il pavimento si viene bocciati — usando un
  // orizzonte DERIVATO dal pavimento invece di una data fissa.
  const sottoIlPavimento = MIN_HORIZON_DAYS * 24 * 0.75;   // tre quarti del pavimento, qualunque sia
  const v = horizonVerdict({ endDate: iso(sottoIlPavimento), nowMs: NOW, grossPerDay: 10, costPerDay: 0 });
  ...
  // Stessa scadenza DERIVATA: le due parti devono bocciare lo stesso mercato, qualunque sia il pavimento.
  const mv = marketValidity({ marketId: '0xbb', venue: { endDate: iso(sottoIlPavimento), closed: false, acceptingOrders: true }, nowMs: NOW });

...
// L'UNICA formula capitale→share del repo, e da oggi anche l'unica definizione del costo della coppia.
// Importata e non ricopiata: due copie di questa aritmetica sono il reperto D1, e su questo numero una
// divergenza farebbe mostrare al piano una size diversa da quella con cui e' stato classificato.
 ...
 * half-width v = maxSpreadCents:  S(v,s) = ((v − s)/v)², zero outside the band.
...
/**
 * ── LA QUOTA-CEILING: QUELLA SU CUI LA CIFRA LORDA È COSTRUITA ──────────────────────────────────────
 * `share = size/(size + competitorQ)` con il proprio ordine al denominatore. È la stessa identità di
 * `rewards-ceiling/lib/curve.shareForCapital`, e qui serve come punto di partenza delle due correzioni
 * che seguono. `null` quando gli ingressi non la sanno dire — mai uno zero di comodo.
 */
function ceilingShare(sizeShares, competitorQ) {
  if (!fin(sizeShares) || !fin(competitorQ) || competitorQ < 0) return null;
  const denom = sizeShares + competitorQ;
  return denom > 0 ? sizeShares / denom : null;
}

/**
 * ── (b) IL FATTORE DELLA POSIZIONE REALE, COME FRAZIONE DELLA QUOTA-CEILING ─────────────────────────
 * La cifra lorda prezza un ordine appoggiato sul mid (S=1). Un ordine vero sta `offsetCents` più in là
 * e vale S<1 per share, quindi la quota diventa `S·size/(S·size + cQ)`. Il fattore è il rapporto fra le
 * due quote — algebra esatta sulla formula pubblicata del venue, non uno sconto a occhio.
 *
 * ATTENZIONE, ED È IL MOTIVO PER CUI QUESTA FUNZIONE ESISTE (8 agosto 2026): il fattore NON è `S`.
 * Moltiplicare il lordo per S vorrebbe dire `pot·shareCeiling·S`, mentre la quota vera è
 * `pot·S·size/(S·size + cQ)` — e siccome `S·size + cQ < size + cQ`, la seconda è sempre PIÙ GRANDE.
 * Usare S penalizza troppo, e penalizza di più proprio i mercati a tick grosso (S piccolo). Su una riga
 * reale: S=0,3086 contro il fattore esatto 0,3265 (−5,5%), e S=0,929 contro 0,9728 (−4,6%).
 *
 * @returns {number|null} il fattore, o null se non calcolabile (mai 1 di comodo)
 */
function placementShareFactor(sizeShares, competitorQ, S) {
  const c = ceilingShare(sizeShares, competitorQ);
  if (c == null || !fin(S) || S < 0) return null;
  const qu = S * sizeShares;
  const shareReal = (qu + competitorQ) > 0 ? qu / (qu + competitorQ) : 0;
  return c > 0 ? shareReal / c : 1;
}

/**
 * ── (f) IL TETTO DI CREDIBILITÀ DELLA QUOTA ────────────────────────────────────────────────────────
 * `share → 1` quando `competitorQ → 0`. «Possiederesti il 100% del montepremi» in un book vuoto non è
 * un'opportunità: è il mercato che ti spiega perché nessun altro sta quotando. Oltre `maxCredibleShare`
 * la stima si taglia a quella quota, e il fattore è il rapporto.
 *
 * Si misura sulla quota-CEILING, non su quella corretta dal punteggio, e la scelta è deliberata: il
 * ceiling è il numero su cui la cifra lorda è stata costruita, quindi è quello di cui si sta giudicando
 * la credibilità. Cambiarlo cambierebbe la stima realistica, che qui non si tocca.
 *
 * @returns {{factor:number, shareCeiling:number, capped:boolean}|null} null se la quota non è leggibile
 */
function credibleShareFactor(sizeShares, competitorQ, maxCredibleShare = DEFAULTS.maxCredibleShare) {
  const c = ceilingShare(sizeShares, competitorQ);
  if (c == null || !fin(maxCredibleShare) || !(maxCredibleShare > 0)) return null;
  if (!(c > maxCredibleShare)) return { factor: 1, shareCeiling: c, capped: false };
  return { factor: maxCredibleShare / c, shareCeiling: c, capped: true };
}
...
function bandExitRate(rows, maxSpreadCents, offsetCents) {
  const pts = (rows || [])
    ...
    .sort((a, b) => a.t - b.t);
  const v = fin(maxSpreadCents) ? raggioBandaCents(maxSpreadCents) : null;
...
function realisticEstimate(input = {}) {
  ...
  // ── LE SHARE CHE IL CAPITALE COMPRA: IL COSTO DELLA COPPIA, NON IL MID ────────────────────────
  // Qui c'era `(capitalUsd / 2) / clampPrice(mid)`, cioe' la forma che l'intestazione di
  // `size-da-capitale` dichiara SBAGLIATA: assume che entrambi i lati costino `mid`, vero solo a
  // mid = 0,50. A mid 0,035 gonfiava la size di ~14×, e la stima realistica con lei.
  //
  // ⚠ IL `/2` NON ERA UNA CONVENZIONE DEL CHIAMANTE — e la distinzione conta, perche' nel gemello
  // `reward-price-row` lo era e andava tolta AL CHIAMANTE. Qui `capitalUsd` e' gia' il TOTALE delle
  // due gambe (`allocate.js:167` costruisce i livelli con `capital: 2 * sizeUsd`), quindi il `/2` era
  // interno alla formula sbagliata ed e' sparito insieme a lei. Nessun chiamante da correggere.
  //
  // ⚠ E COSI' LE DUE META' DELLA CATENA COINCIDONO AL BIT: l'OBIETTIVO del knapsack passa da
  // `curve.shareForCapital(..., pairCostUsd)` → `sharePerLato(capitaleUsd, pairCostUsd)`, e adesso ci
  // passa anche la STIMA. Il costo della coppia si deriva dalla stessa distanza con la stessa
  // funzione che usa `allocate.js:387`.
  //
  // ⚠ FALLISCE CHIUSO: distanza non leggibile ⇒ `null`, e le due correzioni di quota si dichiarano
  // NON misurabili e applicano 1,0. E' lo stesso ramo che prima si prendeva con il mid illeggibile.
  const pairCostUsd = SDC.costoCoppiaAllaDistanza(input.offsetCents);
  const sizeShares = (fin(input.capitalUsd) && pairCostUsd != null)
    ? SDC.sharePerLato({ capitaleUsd: input.capitalUsd, pairCostUsd }).shares
    : null;
  const cQ = fin(input.competitorQ) && input.competitorQ >= 0 ? input.competitorQ : null;
  // Le due correzioni di quota si CALCOLANO ALTROVE, in `placementShareFactor` e `credibleShareFactor`
  // qui sopra — non perché stiano meglio lì, ma perché dall'8 agosto 2026 le usa anche l'obiettivo del
  // knapsack (scripts/rewards-replay/lib/allocate.js). Due implementazioni della stessa algebra sono
  // due opinioni su quanto vale un mercato, e possono divergere senza che nessuno se ne accorga: era
  // esattamente il difetto che questa sessione è venuta a chiudere.
  const fPlacement = (S != null && sizeShares != null && cQ != null) ? placementShareFactor(sizeShares, cQ, S) : null;
  if (fPlacement != null) {
    const shareCeiling = ceilingShare(sizeShares, cQ);
    const shareReal = fPlacement * shareCeiling;
    const factor = fPlacement;
    f *= push({
      ...
      note: `la cifra lorda prezza un ordine appoggiato ESATTAMENTE sul mid (punteggio 1, il massimo). A ${Math.abs(input.offsetCents).toFixed(2)}¢ dal mid, con una banda di ±${(raggioBandaCents(input.maxSpreadCents)).toFixed(2)}¢, la formula pubblicata S=((v−s)/v)² dà ${S.toFixed(3)} — cioè ${(S * 100).toFixed(0)}% del punteggio massimo per share. Quota corretta ${(shareReal * 100).toFixed(2)}% invece di ${(shareCeiling * 100).toFixed(2)}%.`,
    });
  } else {
  ...
  const thin = credibleShareFactor(sizeShares, cQ, cfg.maxCredibleShare);
  const shareCeilingForThin = thin ? thin.shareCeiling : null;
  if (thin && thin.capped) {
    const factor = thin.factor;
    f *= push({
...
module.exports = {
  ...
  // Le tre funzioni di quota, esportate perche' l'obiettivo del knapsack le RIUSA invece di riscriverle.
  ceilingShare, placementShareFactor, credibleShareFactor,

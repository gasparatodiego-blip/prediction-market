...
console.log('\n══ 4 · LE DUE DIREZIONI DI FALLIMENTO, E SONO OPPOSTE APPOSTA');
{
  ...
  // ── ⚠ RISCRITTA IL 24 AGOSTO 2026: DIFENDEVA LA FORMA, ADESSO DIFENDE LA PROPRIETA' ────────────
  // L'asserzione di prima fotografava una riga esatta (`return c ? c.iso : null;`). Il 24 agosto la
  // lettura del CLOB e' stata estesa per portare anche `minimum_order_size` — stessa richiesta HTTP,
  // stesso fail-open, stesso «non inventa una data» — e questo test e' diventato rosso senza che
  // nessuna proprieta' fosse cambiata. E' la classe «test che fotografa il codice invece della
  // proprieta'» di §5.3, che qui ha gia' colpito tre volte. La proprieta' e' DOPPIA e si asserisce
  // cosi': ① la lettura non RILANCIA (un errore del venue non puo' far saltare la scansione);
  // ② ha un RIPIEGO che puo' valere `null` (che vale «una fonte sola», non «escludi tutto»).
  const a24 = fs.readFileSync(path.join(ROOT, 'agents', 'agent24-liquidity-rewards.js'), 'utf8');
  const iLettura = a24.indexOf('async function leggiMercatoClob');
  const iFine = a24.indexOf('async function getScadenzaClob');
  const lettura = iLettura >= 0 && iFine > iLettura ? a24.slice(iLettura, iFine) : '';
  ok('la lettura del venue esiste e si trova per nome', lettura.length > 0,
    'se e\' stata rinominata, questo test va aggiornato — non e\' un difetto del bot');
  ok('la lettura del venue non può far saltare la scansione',
    /catch\s*\{/.test(lettura) && !/catch[\s\S]{0,120}throw/.test(lettura),
    'il catch non rilancia: un errore del venue non interrompe il giro');
  ok('  e lascia un ripiego che può valere `null`',
    /return\s+c\s*\|\|\s*\{[^}]*iso:\s*null/.test(lettura),
    'un errore lascia `null`, che vale «una fonte sola», non «escludi tutto»');
  ok('  e chi chiede la sola scadenza ottiene `null` quando non c\'è',
    /return r \? r\.iso : null;/.test(a24.slice(iFine, iFine + 400)));

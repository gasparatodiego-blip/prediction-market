...
console.log('\n══ REGRESSIONE — i quattro verdetti storici rispondono identici');
{
  ...
  // IL PAVIMENTO È CAMBIATO DUE VOLTE, e questa riga era rimasta alla penultima.
  //   0,25 → 0,75 g (18 h)  ·  8 agosto 2026, dalla ricerca per categoria: separando la popolazione
  //     premiante (40 ingressi su 450) la mediana è 22,7 h, e fra 12,4 h e 19,6 h il campione era vuoto.
  //   0,75 → 0,50 g (12 h)  ·  13 agosto 2026 (§4.4, §5-bis p.129), e il motivo è MISURATO, non di
  //     comodo: il confine di rischio vero sta a 6 ore — sotto, il 35,1% delle uscite arriva DOPO la
  //     risoluzione (13/37); fra 6 e 12 h è 0/36 e fra 12 e 18 h 0/15. A 12 h resta il doppio del
  //     margine. E il vuoto 12,4–19,6 h su cui poggiava la scelta delle 18 h non esiste più: la stessa
  //     soglia toglieva il 76% del board, 78 mercati su 102.
  // ⚠ RIMESSA IN SCALA il 15 agosto 2026: il test asseriva ancora 0,75 e falliva perché il codice è
  // CORRETTO — la sesta occorrenza di «test che fotografa il codice invece della proprietà» (§5.3).
  // La proprietà difesa non cambia: esiste un pavimento, è quello dichiarato in horizon.js, e la
  // finestra non è vuota.
  ok('MIN_HORIZON_DAYS è 0,50 g = 12 ore', MIN_HORIZON_DAYS === 0.5, String(MIN_HORIZON_DAYS));
...
// ══ 5 · L'EFFETTO SUL PIANO, SU UNA FIXTURE — e poi l'osservazione sul piano vivo ════════════════
// ⚠ QUESTO BLOCCO LEGGEVA `data/realloc-ultimo-piano.json` E ASSERIVA SU QUELLO. Era il piano dell'8
// agosto, cioe' una fotografia storica: quel file adesso contiene l'ultimo piano che il bot ha
// scritto davvero, e a bot disarmato non ha righe. L'asserzione «la coda lunga e' tornata
// ammissibile» trovava ZERO righe e diventava rossa — non perche' la regola fosse sbagliata, ma
// perche' non c'era niente da giudicare. Un test che misura una REGOLA non puo' dipendere da cosa il
// bot ha pianificato stamattina.
//
// Adesso la regola si prova su una FIXTURE che non cambia, e il piano vivo resta un'OSSERVAZIONE
// dichiarata: se ha righe si stampa cosa direbbe il muro su di esse, e non si asserisce niente.
console.log('\n══ IL MURO DEI 150 GIORNI, SU UNA FIXTURE CHE NON CAMBIA');
{
  const ora = Date.parse('2026-08-18T00:00:00Z');
  const g = (giorni) => new Date(ora + giorni * 86_400_000).toISOString();
  // Tre righe scelte per coprire i tre verdetti, e nient'altro: sotto il muro, oltre il muro, e senza
  // scadenza leggibile. La coda lunga (120 g) DEVE essere ammissibile — e' il cambiamento voluto — e
  // i 200 g DEVONO essere esclusi, perche' il muro e' l'unica cosa che questo file ancora garantisce.
  const FIXTURE = [
    { name: 'corta', endDate: g(3), grossPerDay: 2.0 },
    { name: 'coda lunga (dentro il muro)', endDate: g(120), grossPerDay: 1.0 },
    { name: 'oltre il muro', endDate: g(200), grossPerDay: 5.0 },
    { name: 'scadenza non leggibile', endDate: null, grossPerDay: 1.0 },
  ];
  let sopravvissuti = 0; let esclusi = 0; let ignoti = 0;
  for (const r of FIXTURE) {
    const v = horizonVerdict({ endDate: r.endDate, nowMs: ora, grossPerDay: r.grossPerDay, costPerDay: 0.10 });
    if (v.state === 'too-far') esclusi += 1;
    else if (v.state === 'unknown') ignoti += 1;
    else sopravvissuti += 1;
    console.log(`     ${r.name.padEnd(30)} → ${v.state}`);
  }
  ok('il muro dei 150 g esclude la riga a 200 g', esclusi === 1, `esclusi ${esclusi}`);
  ok('  e la coda lunga a 120 g resta AMMISSIBILE, non cancellata', sopravvissuti === 2,
    `ammissibili ${sopravvissuti}`);
  ok('  e la scadenza non leggibile resta `unknown`, non ammessa per difetto', ignoti === 1);
}

// ── L'OSSERVAZIONE SUL PIANO VIVO: si guarda, non si asserisce ───────────────────────────────────
{
  const ora = Date.now();
  let piano = null; let arr = [];
  try {
    const board = require(path.join(__dirname, '..', '..', 'data', 'liquidity-rewards.json'));
    arr = Array.isArray(board) ? board : (board.markets || []);
    piano = require(path.join(__dirname, '..', '..', 'data', 'realloc-ultimo-piano.json'));
  } catch { piano = null; }
  const righe = (piano && Array.isArray(piano.righe)) ? piano.righe : [];
  if (!righe.length) {
    console.log('\n  ~ il piano salvato non ha righe (bot disarmato, o nessun piano scritto):'
      + ' niente da osservare, e niente da asserire.');
  } else {
    const byId = new Map();
    for (const m of arr) { const id = String(m.conditionId || m.marketId || '').toLowerCase(); if (id) byId.set(id, m); }
    let s2 = 0; let e2 = 0; let i2 = 0;
    for (const r of righe) {
      ...
      if (v.state === 'too-far') e2 += 1; else if (v.state === 'unknown') i2 += 1; else s2 += 1;
    }
    console.log(`\n  ~ piano vivo: ${righe.length} righe → ${s2} ammissibili, ${e2} oltre il muro, ${i2} senza scadenza nel board`);
    // ⚠ UNA SOLA ASSERZIONE ANCHE QUI, ed è quella che non dipende da quante righe ci sono: il muro
    // non deve MAI far passare qualcosa oltre i 150 giorni. Se le righe sono zero è vera a vuoto, ed
    // è corretto — «nessuna riga sfonda il muro» su zero righe è vero, non è una prova mancata.
    ok('nessuna riga del piano vivo supera il MURO dei 150 g', e2 === 0, `oltre il muro ${e2}`);
  }
}

// ══ 6 · LA MISURA SUL BOARD, che è uno stato del mondo e non un requisito ════════════════════════
{
  const ora = Date.now();
  let arr = [];
  try {
  ...
  const piuCorto = orizzonti.length ? Math.min(...orizzonti) : null;
  console.log(`\n     board: ${orizzonti.length} mercati · il più corto scade fra ${piuCorto === null ? '?' : piuCorto.toFixed(2)} g · eleggibili col tetto: ${eleggibili}`);

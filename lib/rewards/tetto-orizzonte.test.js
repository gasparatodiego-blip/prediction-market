#!/usr/bin/env node
'use strict';
// IL TETTO MASSIMO DI ORIZZONTE — 1,5 GIORNI, E IL NUMERO VIENE DAI FILL.
//
...
console.log('\n══ IL TETTO');
{
  ok('il difetto è 1,5 giorni', MAX_HORIZON_DAYS_DEFAULT === 1.5, String(MAX_HORIZON_DAYS_DEFAULT));
  ok('  ed è nettamente sopra il Q3 dei 21 (0,80 g del manuale v2)', MAX_HORIZON_DAYS_DEFAULT > 0.80);
  ok('  e nettamente sotto la mediana del piano dell\'8 agosto (144,4 g)', MAX_HORIZON_DAYS_DEFAULT < 144.4);
...
{
  const env = { MAKER_MAX_HORIZON_DAYS: '1.5' };
  ...
  ok('  e il motivo dice il numero e il perché',
    /oltre il tetto di 1\.5 g/.test(v(1.51).reason) && /immobilizzato/.test(v(1.51).reason),
...
console.log('\n══ REGRESSIONE — i quattro verdetti storici rispondono identici');
{
  ...
  const casi = [
    ...
    ['rientro più lungo della vita → short', { endDate: iso(0.5), nowMs: NOW, grossPerDay: 10, costPerDay: 4 }, 'short'],
  ...
  // Il pavimento NON è stato toccato: è la mezza riga che il lavoro non doveva cambiare.
  ok('MIN_HORIZON_DAYS è ancora 0,25 — invariato', MIN_HORIZON_DAYS === 0.25, String(MIN_HORIZON_DAYS));
...
console.log('\n══ IL PUNTO DI APPLICAZIONE — uno solo, quello del pavimento');
{
  ...
  ok('  `unknown` continua a NON essere rifiutato (assenza di prova ≠ prova)',
...
console.log('\n══ IL PIANO DELL\'8 AGOSTO, riga per riga (lettura da disco, nessun venue)');
{
  ...
  // L'INVARIANTE, e non l'istantanea. La prima versione di questa riga pretendeva «tutte e sei
  // escluse», ed è durata mezza giornata: agent24 riscrive il board di continuo, e alla riscansione
  // successiva «Matt Klein» era semplicemente sparito dal board — scadenza non più leggibile, quindi
  // `unknown`, quindi 5 su 6. Non era una regressione, era un'asserzione scritta su un file vivo.
  // Quello che deve valere sempre è: nessuna riga del piano con scadenza LEGGIBILE supera il tetto.
  ok('nessuna riga del piano con scadenza leggibile sopravvive al tetto',
    sopravvissuti === 0, `sopravvissute ${sopravvissuti}, escluse ${esclusi}, senza scadenza nel board ${ignoti}`);
  ok('  e almeno una è stata davvero misurata e scartata (l\'asserzione non è vuota)', esclusi >= 1, `${esclusi}`);

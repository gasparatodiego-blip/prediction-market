...
// IL VERDETTO SUL PREZZO, LO STESSO DEL PANNELLO ORDINE: incrocio del libro e banda misurata contro il
// mid di scoring. Non se ne scrive un secondo.
...
function rowAt(r, offsetTicks) {
  ...
  const inBand = bandKnown && offsetCents != null ? offsetCents <= raggioBandaCents(r.maxSpreadCents) + 1e-9 : null;
...
function gambeDiUnaRiga(r, offsetTicks) {
  ...
  const offsetCents = fin(r.tick) ? offsetTicks * r.tick * 100 : null;
  const bandRadiusCents = fin(r.maxSpreadCents) ? raggioBandaCents(r.maxSpreadCents) : null;

  // ── SU QUALE MID SI QUOTA ───────────────────────────────────────────────────────────────────────
  // NON su `r.mid`. Quello è la MEDIANA dello storico su cui la riga è stata SCORATA — giusta per
  // decidere quanto vale un mercato, sbagliata per decidere a che prezzo mandarci un ordine.
  // Misurato il 5 agosto 2026 su «Republican Senate members who retire in 2026 be exactly 7»:
  //
  //     mediana dello storico   0,835      →  gamba YES a 0,835 − 2¢ = 0,82
  //     mid di scoring VIVO     0,795      →  0,82 dista 2,5¢, FUORI dalla banda ±2,25¢
  //     ask vivo                0,82       →  e quel prezzo È l'ask: si eseguirebbe da taker
  //
  // Il venue giudica la banda contro il mid di SCORING (il midpoint al netto dei livelli sotto
  // min_incentive_size), non contro una mediana. Se il riferimento vivo c'è, si quota su quello.
  const vivo = r.rif && typeof r.rif === 'object' ? r.rif : null;
  const midVivo = vivo && fin(vivo.scoringMid) && vivo.scoringMid > 0 && vivo.scoringMid < 1 ? vivo.scoringMid : null;
  const midQuotazione = midVivo != null ? midVivo : r.mid;

  const q = planQuotes({ mid: midQuotazione, offsetCents, tick: r.tick, bandRadiusCents });
  const gambe = [q.yes, q.no];

  // ── `findIndex` E NON `find`, E NON È UNO STILE ─────────────────────────────────────────────────
  // `find` restituisce L'ELEMENTO trovato, e qui l'elemento cercato può essere `null`: `planQuotes`
  // torna con `yes:null, no:null` quando il mid, l'offset o il TICK non sono leggibili
  // (mm-quote-math.js:32-34). In quel caso `find` trovava la gamba nulla, restituiva `null`, e
  // `if (impossibile)` era FALSO — la guardia veniva ingannata esattamente dal caso che esisteva per
  // intercettare, e due righe più sotto `g.inBand` esplodeva su `null`.
  //
  // Successo davvero: dall'8 agosto 2026 il mini-ciclo del trigger a capitale fermo è andato in
  // TypeError a OGNI scatto, ogni ~10 minuti, perché la prima riga del piano salvato («Will Matt Klein
  // be the Democratic nominee for MN-02?») ha `tick: null`. Falliva chiuso — nessun ordine — ma la
  // funzione non ha mai funzionato una sola volta.
  //
  // `findIndex` ha un sentinella (`-1`) che NON può collidere con un elemento legittimo. Qualunque
  // guardia che cerchi «esiste un elemento che...» in un array che può contenere valori falsy deve
  // usare `findIndex` o `some`, mai la truthiness di `find`.
  const iImpossibile = gambe.findIndex((g) => !g || g.placeable !== true);
  if (iImpossibile !== -1) {
    const g = gambe[iImpossibile];
    return no('gamba-impossibile',
      `${(g && g.reason) || q.reason || 'lato non piazzabile'} — un lato solo renderebbe zero fuori dal range [0,10-0,90] e un terzo dentro, quindi non si piazza nessuno dei due`);
  }
  ...
  // Qui `find` è sicuro SOLO perché la guardia qui sopra ha già escluso gli elementi nulli: da questo
  // punto in poi ogni `g` è un oggetto con `placeable === true`. Se un giorno quella guardia si sposta
  // o si allenta, questa riga tornerà a essere il punto in cui il programma esplode.
  ...
  // ── IL LIBRO VERO: NON SI PROPONE UN PREZZO CHE SI ESEGUIREBBE SUBITO ───────────────────────────
  // `planQuotes` non ha mai visto il tocco: giudica `inBand` confrontando l'OFFSET col raggio, mai il
  // prezzo col libro. Con un mid di riferimento sbagliato quel controllo passa e il prezzo finisce
  // sull'ask — un BUY all'ask non riposa, si esegue da taker, e per i reward serve un maker.
  //
  // Il verdetto NON viene riscritto qui: è `priceVerdict` di lib/maker/book-view.js, la stessa funzione
  // che il pannello ordine usa per colorare il prezzo. Un secondo giudizio sarebbe un secondo posto da
  // cui divergere.
  //
  // Sul libro NO il tocco è lo specchio di quello YES: un ordine NO a q È un ordine YES a 1 − q,
  // quindi l'ask del NO è 1 − bestBid(YES). È la stessa identità che questo modulo già usa per i prezzi.
  if (vivo) {
    const toccoDi = (book) => (book === 'yes'
      ? { bestBid: vivo.bestBid ?? null, bestAsk: vivo.bestAsk ?? null }
      : { bestBid: fin(vivo.bestAsk) ? +(1 - vivo.bestAsk).toFixed(6) : null,
        bestAsk: fin(vivo.bestBid) ? +(1 - vivo.bestBid).toFixed(6) : null });

    for (const g of gambe) {
      const t = toccoDi(g.book);
      const smid = g.book === 'yes' ? midQuotazione : +(1 - midQuotazione).toFixed(6);
      const v = priceVerdict({
        price: g.price, bestBid: t.bestBid, bestAsk: t.bestAsk,
        scoringMid: smid, bandRadiusCents, side: 'BUY',
      });
      if (v.crosses) {
        return no('incrocia-il-libro',
          `il lato ${g.book.toUpperCase()} a ${(g.price * 100).toFixed(2)}¢ raggiunge il miglior ask (${(t.bestAsk * 100).toFixed(2)}¢): si eseguirebbe subito da taker invece di riposare come maker, e un ordine che non riposa non matura reward. ${v.messages[0] || ''}`.trim());
      }
      if (v.outOfBand === true) {
        return no('fuori-banda-sul-libro-vivo',
          `il lato ${g.book.toUpperCase()} a ${(g.price * 100).toFixed(2)}¢ dista ${(Math.abs(g.price - smid) * 100).toFixed(2)}¢ dal mid di scoring vivo (${(smid * 100).toFixed(2)}¢), oltre il raggio premiante ±${bandRadiusCents}¢: riposerebbe senza maturare nulla`);
      }

      // ── «MAI PRIMI SUL LIBRO», GIÀ QUI ────────────────────────────────────────────────────────
      // Il giudizio definitivo lo dà `prezzoInCoda` al momento del piazzamento, con la profondità
      // vera e i nostri ordini esclusi. Ma se aspettassimo lì, il mercato sarebbe già entrato nel
      // piano con il suo capitale assegnato — e un mercato che poi viene rifiutato non restituisce
      // quel capitale a nessuno: resta su una riga che non produce niente, e il piano vale meno di
      // quanto dichiara senza che nessuno lo dica (è il difetto descritto in realloc-cycle.js:201).
      //
      // Qui non serve la profondità: la decisione dipende solo dal MIGLIOR bid e dalla banda, e il
      // tocco `rif` li porta entrambi. È un PRE-FILTRO — più prudente del giudizio finale, mai più
      // permissivo: al piazzamento si rifà con i dati completi.
      //
      // L'approssimazione, dichiarata: `bestBid` è il miglior bid del libro INTERO, nostri ordini
      // compresi. Se avessimo già un ordine lì, il pre-filtro ci vedrebbe come concorrenti di noi
      // stessi. Nel percorso del piano non capita — il reset cancella tutto prima di ripiazzare — e
      // dove capita è `prezzoInCoda` a rimediare, perché lui i nostri ordini li toglie.
      if (fin(t.bestBid) && fin(bandRadiusCents) && fin(r.tick) && r.tick > 0) {
        const dietro = +(t.bestBid - r.tick).toFixed(10);
        const distanza = Math.abs(dietro - smid) * 100;
        if (distanza > bandRadiusCents + 1e-9) {
          return no('sarebbe-primo-sul-libro',
            `sul lato ${g.book.toUpperCase()} il miglior bid altrui è ${(t.bestBid * 100).toFixed(2)}¢: un tick dietro darebbe `
            + `${(dietro * 100).toFixed(2)}¢, che dista ${distanza.toFixed(2)}¢ dal mid di scoring (${(smid * 100).toFixed(2)}¢) `
            + `e cade fuori dalla banda ±${bandRadiusCents}¢. Restare premianti richiederebbe di quotare IN CIMA al libro, `
            + 'e questo mercato non si quota: meglio non impegnare capitale che impegnarlo nella posizione peggiore del libro.');
        }
      }
    }
  }
...
function planToOrders(plan, opts = {}) {
  ...
  for (const r of (plan && plan.rows) || []) {
    ...
    // Il capitale che il piano aveva assegnato a questa riga viaggia con lo scarto: senza, un mercato
    // escluso sparirebbe portandosi via la sua quota in silenzio, e il piano varrebbe meno di quanto
    // dichiara. Vedi `capitaleNonImpiegatoUsd` nei totali.
    rif.capitalUsd = fin(r.capital) ? r.capital : null;
    ...
    // `gambeDiUnaRiga` costruisce il suo scarto con marketId e titolo, non con il capitale: il capitale
    // è una proprietà della RIGA DI PIANO, e lo sa solo questo ciclo. Senza questa riga i mercati
    // scartati là dentro sparivano dal conteggio del capitale non impiegato — cioè proprio i due nuovi
    // scarti sul libro vivo, che sono quelli che questo conteggio esiste per contare.
    if (g.scarto) { scartate.push({ ...g.scarto, capitalUsd: rif.capitalUsd }); continue; }
  ...
  return {
    ...
    totals: {
      ...
      // ── IL CAPITALE CHE IL PIANO AVEVA ASSEGNATO E CHE NON VERRÀ IMPIEGATO ────────────────────
      // Il piano alloca con un knapsack: se un mercato entra e poi viene scartato qui, quel capitale
      // NON si redistribuisce da solo — resta su una riga che non produce niente. Non lo si
      // redistribuisce nemmeno adesso (vedi la nota nel rapporto: un ricalcolo costa 52 secondi e
      // ~950 MB, e la scelta di come farlo è dell'operatore) — ma lo si DICHIARA, che è la parte
      // che non poteva restare com'era: un piano da $600 che ne impiega $576 deve dirlo.
      capitaleNonImpiegatoUsd: +scartate.reduce((s, x) => s + (fin(x.capitalUsd) ? x.capitalUsd : 0), 0).toFixed(2),
      capitalePianificatoUsd: +((plan && plan.rows) || []).reduce((s, r) => s + (fin(r.capital) ? r.capital : 0), 0).toFixed(2),
    },
  };
}

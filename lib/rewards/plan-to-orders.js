...
function planToOrders(plan, opts = {}) {
  ...
  for (const r of (plan && plan.rows) || []) {
    ...
    // ── I DUE LATI, con l'aritmetica del motore ───────────────────────────────────────────────────
    const offsetCents = c.offsetCents;
    ...
    if (impossibile) {
      scartate.push({ ...rif, motivo: 'gamba-impossibile',
        dettaglio: `${impossibile.reason || 'lato non piazzabile'} — un lato solo renderebbe zero fuori dal range [0,10-0,90] e un terzo dentro, quindi non si piazza nessuno dei due` });
      continue;
    }
    ...
    if (fuori) {
      scartate.push({ ...rif, motivo: 'gamba-fuori-banda',
        dettaglio: `il lato ${fuori.book.toUpperCase()} riposa fuori dal raggio premiante: ${fuori.bandNote}` });
      continue;
    }
    ...
    if (capitale == null) {
      scartate.push({ ...rif, motivo: 'senza-capitale', dettaglio: 'il piano non assegna capitale a questa riga' });
      continue;
    }
    const sommaPrezzi = q.yes.price + q.no.price;
    const shares = troncaShare(capitale / sommaPrezzi);
    if (shares <= 0) {
      scartate.push({ ...rif, motivo: 'capitale-insufficiente',
        dettaglio: `$${capitale} non comprano nemmeno un decimo di share sui due lati (${q.yes.price} + ${q.no.price} = ${sommaPrezzi.toFixed(4)}/coppia)` });
      continue;
    }

    // ── LA SIZE MINIMA PREMIANTE DEL VENUE ────────────────────────────────────────────────────────
    // Sotto min_incentive_size il venue non assegna punteggio: quel lato varrebbe zero, e con Q_min a
    // due lati un lato a zero azzera anche l'altro. Il capitale resterebbe impegnato per niente.
    const minSize = fin(r.minSizeShares) && r.minSizeShares > 0 ? r.minSizeShares : null;
    if (minSize != null && shares < minSize) {
      scartate.push({ ...rif, motivo: 'sotto-size-minima',
        dettaglio: `$${capitale} comprano ${shares} share per lato, sotto il minimo premiante del venue (${minSize}): a due lati un lato sotto minimo azzera anche l'altro` });
      continue;
    }
    ...
    coppie.push({
      ...
      // Quante share il PIANO credeva di poter comprare per lato, contro quelle vere. Il rapporto è la
      // misura onesta di quanto il modello sovrastimi la size su questo mercato.
      sharePiano: r.sizePerSideShares, shareReali: shares,
      rapportoSize: +(shares / r.sizePerSideShares).toFixed(4),
    });
...
module.exports = { planToOrders, rowAt, troncaShare, STALE_S };

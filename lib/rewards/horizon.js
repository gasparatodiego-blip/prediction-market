...
/** Below this the entry is not worth making at all: a position that must be established, tracked and
 *  unwound inside two days spends its whole life in setup, whatever the pot says. Declared, not derived —
 *  it is an assumption, and callers surface it as one. */
...
function selfcheck() {
  ...
  const vFloor = horizonVerdict({ endDate: iso(1), nowMs: NOW, grossPerDay: 10, costPerDay: 0 });
  ok('sotto il minimo dichiarato di 2 g → resolved', vFloor.state === 'resolved');

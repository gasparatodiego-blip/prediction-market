 ...
 *
 *  ═══ 13 AGOSTO 2026: 0,75 → 0,50 g (12 ORE). DECISIONE DELL'OPERATORE ══════════════════════════
 *  Il ragionamento qui sopra resta corretto ma risponde alla domanda sbagliata: era tarato su «cosa
 *  fanno i vincitori», cioè su una popolazione SELEZIONATA, e non su «cosa ci costa». Il costo,
 *  misurato: il capitale al lavoro passa da **$96 a $648** e il rendimento prudente da **$31,75 a
 *  $173,51/giorno** portando il pavimento da 0,75 a 0,50. È la leva più grande misurata su questo bot.
 *
 *  ⚠ IL CONFINE DI RISCHIO È A 6 ORE, NON A 18, ED È MISURATO: sotto le 6 h il **35,1%** delle uscite
 *  arriva DOPO la risoluzione — cioè non si esce più, si subisce l'esito. Fra 6-12 h è **0 su 36**,
 *  fra 12-18 h **0 su 15**, fra 18-48 h **0 su 32**. A 0,50 g (12 h) restano **due volte** il margine
 *  sul confine vero. **0,25 g (6 h) è sconsigliato**: è esattamente il bordo dove il fenomeno comincia.
 *
 *  ⚠ RESTA COERENTE CON LA CHIUSURA FORZATA: `ORE_CHIUSURA_FORZATA = 3` (modalita-chiusura), quindi su
 *  un mercato entrato a 12 ore restano **9 ore** di quotazione prima che la chiusura forzata prenda il
 *  comando — un margine di 4×. Un test lo asserisce come rapporto, non come numero.
 *
 *  Il numero vive QUI e in nessun altro posto: `market-validity.HORIZON_MIN_HOURS` e
 *  `risk-classifier.SAFE_FLOOR_MINUTES` lo IMPORTANO e si muovono da soli. */
const MIN_HORIZON_DAYS = 0.50;
...
const LONG_TAIL_CAP_FRAC_MISURATA = 0.12;

// ══ LA QUOTA E' UNA MANOPOLA DELL'OPERATORE — 21 agosto 2026 ═══════════════════════════════════════
//
// IL FATTO che l'ha resa necessaria: alle 05:00Z il board offriva **7 mercati ammissibili, TUTTI di
// coda lunga** (1.699-3.163 ore). Sei hanno `minSize 50`, cioe' un pavimento premiante di $61,25, e la
// quota al 12% su 4 slot corti vale `4 x 61,25 x 0,12/0,88 = $33,41`: il cancello 2-ter li scartava
// tutti e sei, e il bot teneva **1 slot su 5** con l'unico `minSize 20` del board.
//
// A p = 0,50 la quota diventa `4 x 61,25 x 1,0 = $245,00`, cioe' esattamente 4 x il pavimento: tutti e
// sette passano e i 5 slot diventano riempibili. Esposizione massima raggiungibile a 5 mercati
// **$612,50 contro il cap di $650** — invariante di §5.2 p.37 rispettata, margine $37,50.
//
// ⚠ E' UN AUMENTO DI RISCHIO DICHIARATO, non una taratura: 0,50 e' **quattro volte** la frazione
// MISURATA (10,4% di ingressi oltre il P90) e oltre il 15% che il selfcheck difendeva. Sopra i 7 giorni
// la chiusura e' a redeem nel 94% dei casi misurati, quindi meta' del piano puo' restare immobilizzata
// fino alla risoluzione. Cio' che continua a limitare l'esposizione, e NON e' stato toccato: il tetto
// per mercato ($61,25), il numero di slot (5), il cap cumulativo ($650) e il kill a −$100.
//
// ⚠ LA BANDA CORTA NON VIENE RIDOTTA, ed e' la ragione per cui alzarla e' sicuro sul lato del corto:
// `planAllocation` serve la fascia corta per PRIMA con il budget pieno (`knapsack(corte, budgetUnits)`)
// e solo dopo concede alla coda `capitaleCorto x p/(1-p)`, limitato al RESIDUO. La coda prende da
// cio' che avanza, non da cio' che il corto ha gia' preso.
//
// ⚠ SI LEGGE UNA VOLTA SOLA, AL CARICAMENTO, e per questo deve stare in `ecosystem.config.js`: e' un
// valore che governa una decisione di rischio, e deve essere leggibile da `/proc/<pid>/environ` invece
// che solo da un file. Un valore illeggibile o fuori dai limiti viene SCARTATO in favore della
// frazione misurata — un `.env` sbagliato non puo' allargare una quota di rischio.
const QUOTA_CODA_MIN = 0.05;   // sotto, la coda non finanzierebbe mai nemmeno il pavimento piu' basso
const QUOTA_CODA_MAX = 0.75;   // sopra, la coda dominerebbe il piano 3:1 sulla fascia corta
function leggiQuotaCodaLunga(env = process.env) {
  const raw = env && typeof env.MAKER_QUOTA_CODA_LUNGA === 'string' ? env.MAKER_QUOTA_CODA_LUNGA.trim() : '';
  if (!raw) return LONG_TAIL_CAP_FRAC_MISURATA;
  const v = Number(raw);
  if (!Number.isFinite(v) || v < QUOTA_CODA_MIN || v > QUOTA_CODA_MAX) return LONG_TAIL_CAP_FRAC_MISURATA;
  return v;
}
const LONG_TAIL_CAP_FRAC = leggiQuotaCodaLunga();
...
function selfcheck() {
  ...
  // ⚠ QUESTA ASSERZIONE E' STATA RISCRITTA IL 21 AGOSTO, NON AMMORBIDITA. Diceva «sotto il 15%», cioe'
  // fotografava la DECISIONE del 12% invece di difendere una proprieta'. Dal 21 agosto la quota e' una
  // manopola dell'operatore (`MAKER_QUOTA_CODA_LUNGA`), e un'asserzione sul valore avrebbe reso rosso
  // il repo a ogni scelta legittima. Cio' che va difeso e' il DOMINIO — fuori da (0,1) la quota non e'
  // esprimibile e `budgetCodaLungaUsd` restituisce null — e il fail-closed del lettore.
  ok('  e il confine della coda lunga e\' il P90 misurato', LONG_TAIL_DAYS === 7);
  ok('  la quota sta nel dominio in cui la formula ha senso', LONG_TAIL_CAP_FRAC > 0 && LONG_TAIL_CAP_FRAC < 1,
    String(LONG_TAIL_CAP_FRAC));
  ok('  la frazione MISURATA resta il difetto, e vale 12%', LONG_TAIL_CAP_FRAC_MISURATA === 0.12);
  ok('  un env illeggibile NON allarga la quota', leggiQuotaCodaLunga({ MAKER_QUOTA_CODA_LUNGA: 'tantissimo' }) === LONG_TAIL_CAP_FRAC_MISURATA);
  ok('  un env fuori dai limiti NON allarga la quota',
    leggiQuotaCodaLunga({ MAKER_QUOTA_CODA_LUNGA: '0.99' }) === LONG_TAIL_CAP_FRAC_MISURATA
    && leggiQuotaCodaLunga({ MAKER_QUOTA_CODA_LUNGA: '0.01' }) === LONG_TAIL_CAP_FRAC_MISURATA);
  ok('  un env valido si applica', leggiQuotaCodaLunga({ MAKER_QUOTA_CODA_LUNGA: '0.5' }) === 0.5);
...
module.exports = {
  ...
  LONG_TAIL_DAYS, LONG_TAIL_CAP_FRAC, LONG_TAIL_CAP_FRAC_MISURATA, leggiQuotaCodaLunga,
  QUOTA_CODA_MIN, QUOTA_CODA_MAX,

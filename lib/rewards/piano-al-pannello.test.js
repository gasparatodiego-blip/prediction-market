...
console.log('\n══ 6 · LA SECONDA GAMBA RICHIEDE UN TOCCO ESPLICITO (regola fissa)');
{
  ...
  // ── RIAGGANCIATA ALL'INTENTO, 2026-08-05 ──────────────────────────────────────────────────────
  // Prima pretendeva la stringa esatta `setSheetStep('form')`. Col riepilogo compatto la gamba 2
  // atterra direttamente su `'review'`, che è PIÙ vicino alla regola, non meno: si vede subito cosa
  // si sta per inviare e «Modifica» è a un tocco. Quello che la regola chiede è che la gamba avanzi e
  // che si passi comunque da una schermata di conferma esplicita — non da quale delle due si parte.
  ok('  e passa comunque da una schermata di conferma esplicita',
    /setLegIdx\(legIdx \+ 1\)/.test(op) && /setSheetStep\('(form|review)'\)/.test(op));
  // I VALORI DELLA GAMBA NUOVA SI SCRIVONO NELLO STESSO GESTORE, non solo nell'effetto: altrimenti
  // esiste un fotogramma col riepilogo della gamba 2 e i numeri della gamba 1.
  ok('  scrivendo prezzo e size della gamba nuova nello stesso tocco',
    /setPriceStr\(String\(legNext\.price\)\)/.test(op) && /setSizeStr\(String\(\+legNext\.size/.test(op));
...
{
  // ── RIAGGANCIATE AL MODULO, 2026-08-05 ────────────────────────────────────────────────────────
  // Queste cinque asserzioni leggevano attributi DOM del riepilogo scritto a mano. Il riepilogo ora
  // lo costruisce `riepilogoOrdine`, quindi si interroga QUELLO: è la differenza fra verificare che
  // una stringa esista nel sorgente e verificare che l'informazione ci sia davvero.
  const op = leggi('app', 'components', 'OrderPanel.tsx');
  const R = require('../maker/riepilogo-ordine');
  const base = {
    title: 'M', book: 'yes', price: 0.78, size: 24.7, distanceCents: 1.5,
    bandRadiusCents: 2.25, verdict: { level: 'ok', crosses: false, outOfBand: false, messages: [] },
  };
  const val = (r, k) => (r.righe.find((x) => x.chiave === k) || {}).v;
  const nota = (r, k) => (r.righe.find((x) => x.chiave === k) || {}).nota;

  const conGambe = R.riepilogoOrdine({ ...base, legIdx: 1, legsTotal: 2 });
  ok('la gamba corrente è nel riepilogo', val(conGambe, 'gamba') === '2 di 2');
  ok('la provenienza del prezzo è dichiarata',
    nota(R.riepilogoOrdine({ ...base, fonte: 'piano' }), 'prezzo') != null);
  ok('  e dice «dal piano» quando viene dal piano',
    nota(R.riepilogoOrdine({ ...base, fonte: 'piano' }), 'prezzo') === 'dal piano'
    && nota(R.riepilogoOrdine({ ...base, fonte: 'digitato' }), 'prezzo') == null);
  ok('la banda reward è nel riepilogo accanto alla distanza dal mid',
    /±2\.25¢/.test(String(nota(R.riepilogoOrdine(base), 'distanza'))));
  ok('il controvalore c era già', /data-op-qs-review-total/.test(op));
  // E che il pannello passi davvero quei valori al modulo, invece di costruirsene di propri.
  ok('  e il pannello alimenta il modulo con i valori del piano',
    /fonte: active \? 'piano' : 'digitato'/.test(op) && /legsTotal: legs \? legs\.length : null/.test(op));
}

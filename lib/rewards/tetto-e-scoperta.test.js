...
//   2 · via il taglio ai primi 120 in agent24: non era una soglia di qualita', era una posizione in
//       classifica. Nessuna soglia di rate lo sostituisce — tutti i 309 mercati superano gia' $25/g.
...
console.log('\n── 2 · IL TAGLIO PER NUMERO NON C\'È PIÙ');
{
  ...
  ok('MAX_CLOB_MARKETS non è più una costante viva', !vive.some((l) => /const MAX_CLOB_MARKETS/.test(l)));
  ok('  e non tronca più la lista', !vive.some((l) => /slice\(0, MAX_CLOB_MARKETS\)/.test(l)));
  ok('l\'ordinamento per rate RESTA', /markets\.sort\(\(a, b\) => b\.rewardsDailyRate - a\.rewardsDailyRate\)/.test(src));
  ok('  e si processano tutti', /const toProcess = markets;/.test(src));
  ok('il log dichiara che non c\'è taglio', /nessun taglio per numero/.test(src));

 ...
 */
function fondi(precedenti, trovati, { adessoIso, scansioneN, parziale = false } = {}) {
  ...
  // ── UNA SCANSIONE PARZIALE NON RISOLVE NIENTE ──────────────────────────────────────────────────
  // Scoperto provando il tetto di tempo: con la deadline a 5 secondi la scansione arrivava a 29 test
  // su 126 e marcava «risolti» cinque reperti che non aveva nemmeno guardato. È precisamente il
  // «sparire in silenzio» che questo modulo esiste per impedire, entrato dalla porta di servizio.
  // Non aver cercato non è aver cercato e non trovato: se il giro è incompleto, la memoria resta ferma.
  const risolti = [];
  if (!parziale) {

...
// ── DATE-AWARE EVENT IDENTITY ────────────────────────────────────────────────────
// event_key is `sport|away@home` (no date), so the SAME matchup on different days collapses
// to one key. Kalshi over-matches a matchup to EVERY date's ticker (measured: MIN@CLE carried
// Jul20/21/22/23 tickers under one key), so best-home×best-away pairs legs from DIFFERENT
// GAMES — 88.4% of Kalshi/PM crossings in data/sport-raw paired different dates, which is why
// the "real" stream still showed a ~9.78% median. So we resolve each leg's game date from its
// NATIVE identifier and only pair legs whose dates match EXACTLY.
//
// Tolerance = 0 (exact calendar date), derived from the data: cross-game pairings differ by
// >=1 day (55 by one day = consecutive series games, 21 by four); same-game legs share the
// identical date string across venues (Kalshi ticker YYMONDD and Polymarket slug YYYY-MM-DD
// agree exactly). Any >=1-day tolerance would readmit the one-day-apart series-game artifacts.
const _MON = { JAN: 1, FEB: 2, MAR: 3, APR: 4, MAY: 5, JUN: 6, JUL: 7, AUG: 8, SEP: 9, OCT: 10, NOV: 11, DEC: 12 };
/** Kalshi ticker `KX…GAME-26JUL221840…` → '2026-07-22'. */
function dateFromKalshiTicker(t) {
  const m = /-(\d{2})([A-Z]{3})(\d{2})/.exec(String(t || ''));
  if (!m || !_MON[m[2]]) return null;
  return `20${m[1]}-${String(_MON[m[2]]).padStart(2, '0')}-${m[3]}`;
}
/** Polymarket slug `wnba-chi-nyl-2026-07-22` → '2026-07-22'. */
function dateFromPolySlug(s) {
  const m = /(\d{4}-\d{2}-\d{2})/.exec(String(s || ''));
  return m ? m[1] : null;
}
/**
 * Resolve a leg's game date (YYYY-MM-DD) from its native venue identifier — never from a
 * shared assumption or our fetch time. Returns null when unresolvable (the leg is then
 * UNVERIFIED_DATE and cannot be paired). Fixed-odds books/exchanges carry no native game-date
 * field in the feed; the recorder attaches commence_time, but aligning a book's UTC commence
 * to the prediction venues' LOCAL game-date needs a timezone model not derivable from the data
 * (getting it wrong would fabricate or falsely exclude), so book/exchange dates stay null here
 * — a documented, conservative follow-up. Kalshi/Polymarket, which drive every real crossing,
 * are resolved exactly.
 */
function resolveGameDate(row) {
  if (row.source === 'kalshi')     return dateFromKalshiTicker(row.venue_ticker);
  if (row.source === 'polymarket') return dateFromPolySlug(row.venue_ticker);
  return null;
}
...
function detectArbs(rows, ts) {
  ...
  const real = [], phantom = [], incomplete = [], crossGame = [], unverifiedDate = [];
  ...
  for (const [key, evRows] of byEvent) {
    ...
    for (const h of home) for (const a of away) {
      ...
      // DATE-AWARE IDENTITY — a crossing is only real if BOTH legs resolve to the SAME game.
      // event_key alone collapses a matchup's multiple dates, so best-home×best-away can pair
      // legs from different games. Resolve each leg's date from its native id and match exactly.
      const dh = resolveGameDate(h), da = resolveGameDate(a);
      if (dh == null || da == null) {
        unverifiedDate.push({ ts, event_key: key, sport: h.sport,
          legs: [{ source: h.source, date: dh }, { source: a.source, date: da }],
          reason: 'UNVERIFIED_DATE: a leg has no resolvable native game date (not same-game verifiable)' });
        continue;
      }
      if (dh !== da) {
        crossGame.push({ ts, event_key: key, sport: h.sport, dateHome: dh, dateAway: da,
          legs: [{ source: h.source, date: dh }, { source: a.source, date: da }],
          reason: `ARTIFACT_CROSS_GAME: legs resolve to different games (${dh} vs ${da})` });
        continue;
      }
      const gameDate = dh;
  ...
  return { real, phantom, incomplete, crossGame, unverifiedDate };
}
...
module.exports = {
  ...
  resolveGameDate, dateFromKalshiTicker, dateFromPolySlug,
};

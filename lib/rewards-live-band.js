...
// the band radius is the existing maxSpread convention and the in-band test is
...
//   bandRadiusC = maxSpreadC            — half the reward band, in cents
...
function bandFromMid(mid, maxSpreadC) {
  ...
  const r = raggioBandaCents(maxSpreadC);
...
function inBand(price, mid, maxSpreadC) {
  ...
  return Math.abs(price - mid) * 100 <= raggioBandaCents(maxSpreadC);
}
...
function offsetExceedsBand(offsetC, maxSpreadC) {
  ...
  return Math.abs(offsetC) > raggioBandaCents(maxSpreadC) + 1e-9;
}
...
function legStatus(leg, mid, maxSpreadC, now) {
  ...
  return {
    ...
    mid: mid ?? null,
    bandRadiusC: maxSpreadC > 0 ? raggioBandaCents(maxSpreadC) : null,
    targetPrice: target,

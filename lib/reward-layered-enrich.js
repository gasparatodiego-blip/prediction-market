...
const LIVE_BOOKS = fileRuntime('clob-live-books.json');
const MID_DIR = require('./safety/store').DATA_DIR;
const { raggioBandaPrezzo } = require('./banda-premiante');
...
function enrichLayeredDepth(markets, opts = {}) {
  ...
  return markets.map((m) => {
    ...
    // Live fallback: derive band from mid ± maxSpread, read the ladder once.
    const r = raggioBandaPrezzo(rs.maxSpreadCents); // cents→price half-width
    const doc = getLive();

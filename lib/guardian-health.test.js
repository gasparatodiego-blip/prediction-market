...
// ── rule 48/62: a far-future `now` makes every feed stale ⇒ global banner + degraded ──
{
  ...
  assert.strictEqual(r.banner, 'dati non aggiornati', 'rule 48/62: stale pipeline raises the global banner');
  assert.strictEqual(r.degraded, true, 'stale pipeline ⇒ calm-degraded');
  ok('rule 48/62: stale pipeline → "dati non aggiornati" banner + degraded');
}

...
function scoreLayeredPlan(args = {}) {
  ...
  const mid = fin(rs.mid) ? rs.mid : null;
  const maxSpreadCents = fin(rs.maxSpreadCents) ? rs.maxSpreadCents : null;
  const v = maxSpreadCents != null ? raggioBandaCents(maxSpreadCents) : null;
  const minSize = fin(rs.minSize) ? rs.minSize : null;
...
/**
 * Cap each layer's assumed per-side size by that layer's OWN real in-band depth (USD), exactly as the
 * single-level estimate caps capital at competitorDepthUsd. The remainder a layer's cap cannot absorb is
 * disclosed as UNCOMMITTED on that row — never silently reassigned to another layer. Returns a new plan
 * whose per-layer sizeUsd is the COMMITTED size (so scoreLayeredPlan scores the capped capital), plus a
 * reconciliation proving committed + uncommitted = the operator's configured per-side size.
 *
 * @param {object} plan          from computeLayeredPlan
 * @param {Array}  perLevelDepth aligned to plan.layers: [{ bidSizeAtLevel, askSizeAtLevel } | null]
 */
function capLayeredPlan(plan, perLevelDepth = []) {
  const layers = plan.layers.map((L, i) => {
    const d = perLevelDepth[i] || null;
    const bidSize = d && fin(d.bidSizeAtLevel) ? d.bidSizeAtLevel : null;
    const askSize = d && fin(d.askSizeAtLevel) ? d.askSizeAtLevel : null;
    const assignedUsd = fin(L.sizeUsd) ? L.sizeUsd : null;
    // Real in-band depth AT this layer in USD, both sides (mirrors competitorDepthUsd = near + opp).
    const bidDepthUsd = bidSize != null && fin(L.bidPrice) ? bidSize * L.bidPrice : null;
    const askDepthUsd = askSize != null && fin(L.askPrice) ? askSize * L.askPrice : null;
    const eligibleDepthUsd = (bidDepthUsd != null && askDepthUsd != null) ? r4(bidDepthUsd + askDepthUsd) : null;

    let committedUsd, uncommittedUsd, capBound;
    if (assignedUsd == null) {
      committedUsd = null; uncommittedUsd = null; capBound = false;
    } else if (eligibleDepthUsd == null) {
      // FAIL CLOSED: depth we cannot read caps to nothing — the whole assigned size is uncommitted, not
      // modelled as if it would fill. (scoreLayeredPlan then shows "—" for this layer.)
      committedUsd = null; uncommittedUsd = assignedUsd; capBound = true;
    } else {
      const committedTotal = Math.min(2 * assignedUsd, eligibleDepthUsd); // assigned is per-side; depth is both-sides
      committedUsd = r4(committedTotal / 2);
      uncommittedUsd = r4(assignedUsd - committedUsd);
      capBound = committedUsd < assignedUsd - 1e-9;
    }
    return {
      ...L,
      sizeUsd: committedUsd,          // what gets scored (the capped, committed per-side capital)
      assignedUsd, committedUsd, uncommittedUsd, eligibleDepthUsd, capBound,
      capNote: capBound
        ? (eligibleDepthUsd == null
            ? 'profondità del livello non leggibile: nessun capitale impegnato su questo livello'
            : `livello limitato dalla sua profondità: impegnati $${committedUsd} dei $${assignedUsd} assegnati, $${uncommittedUsd} non impegnati su questo livello`)
        : null,
    };
  });
  const sum = (k) => r4(layers.reduce((a, l) => a + (fin(l[k]) ? l[k] : 0), 0));
  const totalAssigned = sum('assignedUsd');
  const totalCommitted = sum('committedUsd');
  const totalUncommitted = sum('uncommittedUsd');
  return {
    ...plan,
    layers,
    reconciliation: {
      perSideSizeUsd: plan.perSideSizeUsd,
      totalAssigned, totalCommitted, totalUncommitted,
      // committed + uncommitted must equal the configured per-side size (never silently reassigned)
      reconciles: Math.abs(totalCommitted + totalUncommitted - totalAssigned) < 0.01,
    },
  };
}

module.exports = { computeLayeredPlan, capLayeredPlan, scoreLayeredPlan, layerSizeSplit, depthSourceLabel, TAIL_LO, TAIL_HI };

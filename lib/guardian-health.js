...
//     stale → banner:"Data may be stale" (48/62); per-feed ages let the UI downgrade
...
const MONITOR_FILE   = fileRuntime('monitor-status.json');
...
function getGuardianHealth(now = Date.now()) {
  ...
  const banner = pipelineStale ? 'Data may be stale' : null;

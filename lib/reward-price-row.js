...
//   • "Your size" is the TOTAL notional deployed across BOTH sides, and that TOTAL is what feeds the
//     score. ⚠ Fino al 21 agosto 2026 qui si passava `totalUsd / 2`, perché `quadraticUserShare`
//     convertiva capitale→share dividendo per il MID e la convenzione «per lato» reggeva. Chiuso il
//     reperto D1, la conversione passa dal COSTO DELLA COPPIA (`1 − 2s/100`), e un budget «per lato»
//     non è più esprimibile: le due gambe devono portare la stessa size, quindi il vincolo è il
//     totale. Continuare a dimezzare sottostimerebbe il $/giorno di ESATTAMENTE 2×.
//     La UI resta etichettata come totale e continua a dichiarare lo split per lato.
...
function computePriceRow({ rewardScore, tick, totalSizeUsd, offsetCents, market } = {}) {
  ...
  // the feed's real competitorQ. Il capitale che entra nel punteggio è il TOTALE (v. CONVENTIONS).
  // When the offset is beyond the band OR the resulting order is below min_size, quadraticUserShare
  // returns 0 — i.e. the number shown is already the DEGRADED case, not the both-sides-valid case (B3).
  ...
  if (estimateCapitalUsd != null && off != null && competitorQ != null && mid != null && fin(maxSpreadCents) && minSize != null) {
    // ⚠ IL TOTALE, non `estimatePerSideUsd`: v. CONVENTIONS. `estimatePerSideUsd` resta perché la UI
    // dichiara lo split per lato, ma non è più il numero che entra nel punteggio.
    share = quadraticUserShare(competitorQ, mid, maxSpreadCents, minSize, estimateCapitalUsd, off);

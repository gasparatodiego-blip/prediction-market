...
// L'UNICA formula capitale→share del repo. Importata, non ricopiata: due copie di questa aritmetica
// sono il reperto D1, ed e' esattamente il difetto che questo file portava (§5-bis, 21 agosto 2026).
// `size-da-capitale` e' puro (zero `require`), quindi questo modulo resta browser-safe.
...
/**
 * IL COSTO DELLA COPPIA per una posa BILATERALE SIMMETRICA a `s` centesimi dal mid, in dollari/share.
 *
 * ═══ DERIVAZIONE, DALLA FORMULA DEL VENUE E NON PER SIMMETRIA COL PIAZZAMENTO ═══════════════════
 * Il venue scora SHARE (`Q = Σ S(v,s)·size_i`) e non conosce il capitale. L'unica domanda è quindi
 * quante share compra `C` nella posa che questo modulo dichiara di modellare — bilaterale e
 * simmetrica. Quella posa costa, per share:
 *
 ...
 *
 * ⚠ `capital / mid` — quello che c'era prima — è la size di una posa UNILATERALE che spende tutto
 * sulla gamba YES: finanzia UN lato e poi ne scora DUE (`qMin(Qu, Qu, mid)`). E su un mid fuori da
 * [0,10 · 0,90] il venue PRETENDE il bilaterale, quindi quella posa varrebbe zero — cioè l'errore era
 * massimo (10,0× misurato) proprio dove la posa descritta non avrebbe preso niente.
 *
 * ⚠ FALLISCE CHIUSO: `null` quando la posa non è ESPRIMIBILE, cioè quando il bid cadrebbe a prezzo
 * ≤ 0 o l'ask a ≥ 1. Non si clampa il mid a [0,01 · 0,99] per farla esistere lo stesso: quel clamp
 * era il modo in cui un mercato non quotabile riceveva comunque una size, e quindi un punteggio.
 */
function pairCostAtDistance(s_cents, mid) {
  const fin = (x) => typeof x === 'number' && Number.isFinite(x);
  if (!fin(s_cents) || s_cents < 0 || !fin(mid)) return null;
  const d = s_cents / 100;
  // La posa dev'essere ESPRIMIBILE su questo mid: il bid a `mid − d` e l'ask a `mid + d` devono stare
  // dentro (0,1). E' la guardia in piu' rispetto alla SSOT, che non conosce il mid.
  if (!(mid - d > 0) || !(mid + d < 1)) return null;
  // ⚠ `1 − 2d` NON si riscrive qui: vive in `size-da-capitale.costoCoppiaAllaDistanza`, la stessa che
  // usano `realistic-estimate` e — con la propria copia storica — `allocate.js:387`. Era gia' scritta
  // a mano in due posti, e un terzo l'avrebbe resa il reperto D1 sul numero che decide la size.
  return SDC.costoCoppiaAllaDistanza(s_cents);
}

/**
 * Le share che `capital` compra in quella posa. Passa da `sharePerLato`, la funzione del PIAZZAMENTO:
 *
 * ⚠⚠ `capital` È IL TOTALE DELLE DUE GAMBE, NON UN BUDGET PER LATO — e la distinzione ora MORDE.
 * Con il vecchio denominatore (`capital/mid`) «per lato» aveva un senso apparente: tanti dollari su
 * UNA gamba al prezzo del mid. Con la coppia non ce l'ha più, perché le due gambe devono portare la
 * STESSA size e il vincolo è il costo della coppia: `n = C_totale / (1 − 2s/100)`. Un budget «per
 * lato» non è nemmeno esprimibile — è la forma `(C/2)/mid` che `size-da-capitale` ha già scartato
 * come sbagliata, non come meno precisa. Chi chiama passa il TOTALE.
 * se questa riga e `plan-to-orders` divergessero, la stima parlerebbe di una size che non è quella che
 * finirebbe al venue. `null` ⇒ posa non esprimibile, e il chiamante non deve inventare un numero.
 */
function sizeAllaDistanza(capital, mid, s_cents) {
  const pc = pairCostAtDistance(s_cents, mid);
  if (pc == null) return null;
  const r = SDC.sharePerLato({ capitaleUsd: capital, pairCostUsd: pc });
  return r && Number.isFinite(r.shares) ? r.shares : null;
}
...
function estimateCapitalLevelRange(competitorQ, maxSpreadCents, minSize, rewardsDailyRate, capital) {
  ...
  const v        = raggioBandaCents(maxSpreadCents);
  // ⚠ LA SIZE DIPENDE DALLA DISTANZA, perché il costo della coppia dipende dalla distanza
  // (`1 − 2s/100`): posarsi più lontano dal mid costa MENO per coppia, quindi compra più share.
  // Prima era un numero solo, calcolato dal mid, e valeva per tutte e quattro le pose.
  // Il flag `aboveMin` si giudica sulla posa TIPICA — quella che tutti i chiamanti leggono come
  // titolo — invece che su una size che non appartiene a nessuna posa in particolare.
  const sizeTipica = v == null ? null : sizeAllaDistanza(capital, mid, v / 2);
  const aboveMin   = sizeTipica != null && sizeTipica >= minSize;

  function atDist(s_cents) {
    const sc = scoreOrder(s_cents, v);
    const size = sizeAllaDistanza(capital, mid, s_cents);
    if (size == null || size < minSize || sc === 0) {
...
function recoverCompetitorQ(levels, mid, maxSpreadCents, minSize) {
  ...
  for (const C of [5000, 500, 50000]) {
    ...
    // ⚠ LO STESSO DENOMINATORE DI `estimateCapitalLevelRange`, ALLA STESSA POSA. Questa è la sua
    // inversa algebrica: se le due usassero due denominatori, la Q recuperata sarebbe sbagliata
    // **in silenzio** — e da quella Q nasce `refShare`, cioè il $/giorno che l'operatore legge.
    const size = sizeAllaDistanza(C, mid, v / 2);
...
function quadraticUserShare(competitorQ, mid, maxSpreadCents, minSize, capital, distanceCents) {
  ...
  const v     = raggioBandaCents(maxSpreadCents);
  const d     = Math.max(0, distanceCents);
  // Stesso denominatore delle altre due: è la funzione che produce `rewardScore.refShare`
  // (`lib/rewards-normalize.js:137`), cioè il numero da cui esce il «$/giorno» del pannello.
  const size  = sizeAllaDistanza(capital, mid, d);
  if (size == null) return 0;
  if (size < (minSize || 0)) return 0;
  const sc    = scoreOrder(d, v);
